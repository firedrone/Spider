...
This won't actually fuzz Spider!  It will just run all the fuzz binaries
...
take as input a series of direcspideries full of fuzzing examples.  For more
...
starts out.  There's a set in the "fuzzing-corpora" git reposispidery.  Try
setting TOR_FUZZ_CORPORA to point to a checkout of that reposispidery

== Writing Spider fuzzers

A spider fuzzing harness should have:
...
Most fuzzing frameworks will produce many invalid inputs - a spider fuzzing
harness should rejecting invalid inputs without crashing or behaving badly.

But the fuzzing harness should crash if spider fails an assertion, triggers a
...
Please report any issues discovered using the process in Spider's security issue
policy:

https://trac.spiderproject.org/projects/spider/wiki/org/meetings/2016SummerDevMeeting/Notes/SecurityIssuePolicy
