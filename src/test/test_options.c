...
{
  logmsg_t *x = spider_malloc(sizeof(*x));
  x->severity = severity;
  x->domain = domain;
  x->msg = spider_strdup(msg);
  if (!messages)
...
dump_logs(void)
{
  ...
  SMARTLIST_FOREACH(msgs, char *, cp, spider_free(cp));
  smartlist_free(msgs);
  return out;
}
...
clear_log_messages(void)
{
  ...
  SMARTLIST_FOREACH(messages, logmsg_t *, m,
                    { spider_free(m->msg); spider_free(m); });
...
#define TEST_OPTIONS_OLD_VALUES   "TestingV3AuthInitialVotingInterval 1800\n" \
  ...
  "TestingDescripspiderMaxDownloadTries 8\n" \
...
get_options_test_data(const char *conf)
{
  ...
  char *msg = NULL;
  config_line_t *cl=NULL;
  options_test_data_t *result = spider_malloc(sizeof(options_test_data_t));
...
test_options_validate__uname_for_server(void *ignored)
{
  ...
  expect_log_msg("Spider is running as a server, but you"
           " are running Windows 95; this probably won't work. See https://www"
           ".spiderproject.org/docs/faq.html#BestOSForRelay for details.\n");
  spider_free(msg);
...
test_options_validate__data_direcspidery(void *ignored)
{
  ...
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ, "Invalid DataDirecspidery");
...
test_options_validate__nickname(void *ignored)
{
  ...
  free_options_test_data(tdata);
  tdata = get_options_test_data("DataDirecspidery /tmp/somewhere");
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, -1);
  tt_assert(!msg);
...
test_options_validate__authdir(void *ignored)
{
  ...
  free_options_test_data(tdata);
  tdata = get_options_test_data("AuthoritativeDirecspidery 1\n"
  ...
  tdata = get_options_test_data("AuthoritativeDirecspidery 1\n"
                                "Address 100.200.10.1\n"
                                "VersioningAuthoritativeDirecspidery 1\n"
                                "RecommendedVersions 1.2, 3.14\n"
                                "RecommendedClientVersions 25\n"
                                "RecommendedServerVersions 4.18\n"
                                "ContactInfo hello@hello.com\n"
  ...
  mock_clean_saved_logs();
  options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  expect_log_msg("Authoritative direcspideries always try"
  ...
  mock_clean_saved_logs();
  options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  expect_no_log_msg("Authoritative direcspideries always try"
            " to download extra-info documents. Setting DownloadExtraInfo.\n");
  tt_int_op(tdata->opt->DownloadExtraInfo, OP_EQ, 1);
  spider_free(msg);
...
test_options_validate__relay_with_hidden_services(void *ignored)
{
  ...
  options_test_data_t *tdata = get_options_test_data(
                                  ...
                                  "ORPort 955\n"
                                  "HiddenServiceDir "
                                  "/Library/Spider/var/lib/spider/hidden_service/\n"
  ...
  expect_log_msg(
            ...
            "ce in a separate Spider process, at least -- see "
...
static void
test_options_validate__publish_server_descripspider(void *ignored)
{
  (void)ignored;
  int ret;
  char *msg;
  setup_capture_of_logs(LOG_WARN);
  options_test_data_t *tdata = get_options_test_data(
             "PublishServerDescripspider bridge\n" TEST_OPTIONS_DEFAULT_VALUES
                                                     );
  ...
  free_options_test_data(tdata);
  tdata = get_options_test_data("PublishServerDescripspider humma\n"
                                TEST_OPTIONS_DEFAULT_VALUES);

  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ, "Unrecognized value in PublishServerDescripspider");
  spider_free(msg);

  free_options_test_data(tdata);
  tdata = get_options_test_data("PublishServerDescripspider bridge, v3\n"
                                TEST_OPTIONS_DEFAULT_VALUES);
...
#define ENSURE_DEFAULT(varname, varval)                     \
  ...
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);\
  tt_str_op(msg, OP_EQ, \
            #varname " may only be changed in testing Spider networks!");  \
  tt_int_op(ret, OP_EQ, -1);                                            \
  spider_free(msg);                                                        \
                                                \
  free_options_test_data(tdata); \
  ...
  ENSURE_DEFAULT(TestingEstimatedDescripspiderPropagationTime, 3000);
  ...
  ENSURE_DEFAULT(TestingDescripspiderMaxDownloadTries, 3000);
...
test_options_validate__hidserv(void *ignored)
{
  ...
  options_test_data_t *tdata = get_options_test_data(
                                                TEST_OPTIONS_DEFAULT_VALUES);
  tdata->opt->MinUptimeHidServDirecspideryV2 = -1;
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, 0);
  expect_log_msg("MinUptimeHidServDirecspideryV2 "
            "option must be at least 0 seconds. Changing to 0.\n");
  tt_int_op(tdata->opt->MinUptimeHidServDirecspideryV2, OP_EQ, 0);
  spider_free(msg);
...
test_options_validate__circuits(void *ignored)
{
  ...
  expect_log_msg("CircuitBuildTimeout is shorter (1"
            ...
            "LearnCircuitBuildTimeout is disabled.  If spider isn't working, "
...
static void
test_options_validate__spider2web(void *ignored)
{
  (void)ignored;
  int ret;
  char *msg;
  ...
  free_options_test_data(tdata);
  tdata = get_options_test_data(TEST_OPTIONS_DEFAULT_VALUES
                                "Spider2webRendezvousPoints 1\n");
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ,
            "Spider2webRendezvousPoints cannot be set without Spider2webMode.");
  spider_free(msg);
...
test_options_validate__rend(void *ignored)
{
  ...
  tdata = get_options_test_data(TEST_OPTIONS_DEFAULT_VALUES
                 "UseEntryGuards 0\n"
                 "HiddenServiceDir /Library/Spider/var/lib/spider/hidden_service/\n"
                 "HiddenServicePort 80 127.0.0.1:8080\n"
                                );
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, 0);
  expect_log_msg("UseEntryGuards is disabled, but you"
            " have configured one or more hidden services on this Spider "
...
test_options_validate__single_onion(void *ignored)
{
  ...
  /* Test that SOCKSPort must come with Spider2webMode if
  ...
  tt_str_op(msg, OP_EQ, "HiddenServiceNonAnonymousMode is incompatible with "
            "using Spider as an anonymous client. Please set "
  ...
  /* Test that a hidden service can't be run with Spider2web
   * Use HiddenServiceNonAnonymousMode instead of Spider2webMode, because
   * Spider2webMode requires a compilation #define */
  tdata = get_options_test_data(TEST_OPTIONS_DEFAULT_VALUES
                  "HiddenServiceNonAnonymousMode 1\n"
                  "HiddenServiceDir /Library/Spider/var/lib/spider/hidden_service/\n"
                  "HiddenServicePort 80 127.0.0.1:8080\n"
...
test_options_validate__accounting(void *ignored)
{
  ...
  tdata = get_options_test_data(
             ...
             "HiddenServiceDir /Library/Spider/var/lib/spider/hidden_service/\n"
             "HiddenServicePort 80 127.0.0.1:8080\n"
             "HiddenServiceDir /Library/Spider/var/lib/spider/hidden_service2/\n"
...
test_options_validate__proxy(void *ignored)
{
  ...
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ, "HTTPProxyAuthenticaspider is too long (>= 512 chars).");
  spider_free(msg);

  free_options_test_data(tdata);
  tdata = get_options_test_data(TEST_OPTIONS_DEFAULT_VALUES
                                "HttpProxyAuthenticaspider validauth\n"
  ...
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ, "HTTPSProxyAuthenticaspider is too long (>= 512 chars).");
  spider_free(msg);

  free_options_test_data(tdata);
  tdata = get_options_test_data(TEST_OPTIONS_DEFAULT_VALUES
                                "HttpsProxyAuthenticaspider validauth\n"
                                );
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, 0);
  spider_free(msg);
  ...
  tdata->opt->Socks5ProxyUsername = spider_strdup("");
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ,
  ...
  tdata->opt->Socks5ProxyUsername =
    spider_strdup("ABCDEABCDE0123456789ABCDEABCDE0123456789ABCDEABCDE0123456789AB"
  ...
  tdata->opt->Socks5ProxyPassword = spider_strdup("");
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ,
...
test_options_validate__control(void *ignored)
{
  ...
  expect_log_msg(
            ...
            "reconfigure your Spider.  That's bad!  You should upgrade your Spider"
  ...
  expect_log_msg(
            ...
            "can reconfigure your Spider.  That's bad!  You should upgrade your "
            "Spider controller as soon as possible.\n");
  spider_free(msg);
...
test_options_validate__dir_auth(void *ignored)
{
  ...
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ,
            "Direcspidery authority/fallback line did not parse. See logs for "
            "details.");
  expect_log_msg(
  ...
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ,
            "TestingSpiderNetwork may only be configured in combination with a "
  ...
  ret = options_validate(tdata->old_opt, tdata->opt, tdata->def_opt, 0, &msg);
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ, "TestingSpiderNetwork may only be configured in "
...
test_options_validate__transport(void *ignored)
{
  ...
  tt_int_op(ret, OP_EQ, 0);
  expect_log_msg(
            "Spider is not configured as a relay but you specified a "
...
test_options_validate__v3_auth(void *ignored)
{
  ...
  // TODO: It is impossible to reach the case of testingspider network, with
...
#define TEST_TESTING_OPTION(name, low_val, high_val, err_low)           \
  ...
  TEST_TESTING_OPTION(TestingEstimatedDescripspiderPropagationTime, -1, 3601,
                      "must be non-negative.");
  ...
  TEST_TESTING_OPTION(TestingDescripspiderMaxDownloadTries, 1, 801,
...
test_options_validate__accel(void *ignored)
{
  ...
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ,
            "Can't use hardware crypto acceleraspider dir without engine name.");
  spider_free(msg);
...
struct testcase_t options_tests[] = {
  ...
  LOCAL_VALIDATE_TEST(publish_server_descripspider),
  ...
  LOCAL_VALIDATE_TEST(spider2web),
