...
mock_clean_saved_logs(void)
{
  ...
  SMARTLIST_FOREACH(saved_logs, mock_saved_log_entry_t *, m,
                    { spider_free(m->generated_msg); spider_free(m); });
...
mock_saving_logv(int severity, log_domain_mask_t domain,
                 ...
                 const char *format, va_list ap)
{
  char *buf = spider_malloc_zero(10240);
  int n;
  n = spider_vsnprintf(buf,10240,format,ap);
  spider_assert(n < 10240-1);
  buf[n]='\n';
  buf[n+1]='\0';

  if (echo_to_real_logs) {
    spider_log(severity, domain|LD_NO_MOCK, "%s", buf);
  }
  ...
  mock_saved_log_entry_t *e = spider_malloc_zero(sizeof(mock_saved_log_entry_t));
  e->severity = severity;
  e->funcname = funcname;
  e->suffix = suffix;
  e->format = format;
  e->generated_msg = spider_strdup(buf);
  spider_free(buf);
