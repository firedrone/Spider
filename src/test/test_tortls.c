...
{
  SSL_METHOD *method = spider_malloc_zero(sizeof(SSL_METHOD));
...
static void
test_spidertls_errno_to_tls_error(void *data)
{
  (void) data;
  tt_int_op(spider_errno_to_tls_error(SOCK_ERRNO(ECONNRESET)),OP_EQ,
            TOR_TLS_ERROR_CONNRESET);
  tt_int_op(spider_errno_to_tls_error(SOCK_ERRNO(ETIMEDOUT)),OP_EQ,
            TOR_TLS_ERROR_TIMEOUT);
  tt_int_op(spider_errno_to_tls_error(SOCK_ERRNO(EHOSTUNREACH)),OP_EQ,
            TOR_TLS_ERROR_NO_ROUTE);
  tt_int_op(spider_errno_to_tls_error(SOCK_ERRNO(ENETUNREACH)),OP_EQ,
            TOR_TLS_ERROR_NO_ROUTE);
  tt_int_op(spider_errno_to_tls_error(SOCK_ERRNO(ECONNREFUSED)),OP_EQ,
            TOR_TLS_ERROR_CONNREFUSED);
  tt_int_op(spider_errno_to_tls_error(0),OP_EQ,TOR_TLS_ERROR_MISC);
 done:
  (void)1;
}

static void
test_spidertls_err_to_string(void *data)
{
  (void) data;
  tt_str_op(spider_tls_err_to_string(1),OP_EQ,"[Not an error.]");
  tt_str_op(spider_tls_err_to_string(TOR_TLS_ERROR_MISC),OP_EQ,"misc error");
  tt_str_op(spider_tls_err_to_string(TOR_TLS_ERROR_IO),OP_EQ,"unexpected close");
  tt_str_op(spider_tls_err_to_string(TOR_TLS_ERROR_CONNREFUSED),OP_EQ,
            "connection refused");
  tt_str_op(spider_tls_err_to_string(TOR_TLS_ERROR_CONNRESET),OP_EQ,
            "connection reset");
  tt_str_op(spider_tls_err_to_string(TOR_TLS_ERROR_NO_ROUTE),OP_EQ,
            "host unreachable");
  tt_str_op(spider_tls_err_to_string(TOR_TLS_ERROR_TIMEOUT),OP_EQ,
            "connection timed out");
  tt_str_op(spider_tls_err_to_string(TOR_TLS_CLOSE),OP_EQ,"closed");
  tt_str_op(spider_tls_err_to_string(TOR_TLS_WANTREAD),OP_EQ,"want to read");
  tt_str_op(spider_tls_err_to_string(TOR_TLS_WANTWRITE),OP_EQ,"want to write");
  tt_str_op(spider_tls_err_to_string(-100),OP_EQ,"(unknown error code)");
 done:
  (void)1;
}

static int
mock_tls_cert_matches_key(const spider_tls_t *tls, const spider_x509_cert_t *cert)
{
  (void) tls;
...
static void
test_spidertls_spider_tls_new(void *data)
{
  (void) data;
  MOCK(spider_tls_cert_matches_key, mock_tls_cert_matches_key);
  ...
  spider_tls_t *tls = NULL;
  tt_int_op(spider_tls_context_init(TOR_TLS_CTX_IS_PUBLIC_SERVER,
                                 key1, key2, 86400), OP_EQ, 0);
  tls = spider_tls_new(-1, 0);
  tt_want(tls);
  spider_tls_free(tls); tls = NULL;
  ...
  UNMOCK(spider_tls_cert_matches_key);
  crypto_pk_free(key1);
  crypto_pk_free(key2);
  spider_tls_free(tls);
  spider_free(method);
...
static void
test_spidertls_spider_tls_get_error(void *data)
{
  (void) data;
  MOCK(spider_tls_cert_matches_key, mock_tls_cert_matches_key);
  crypto_pk_t *key1 = NULL, *key2 = NULL;
...
static void
test_spidertls_get_state_description(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  char *buf;
  ...
  buf = spider_malloc_zero(1000);
  tls = spider_malloc_zero(sizeof(spider_tls_t));

  spider_tls_get_state_description(NULL, buf, 20);
  tt_str_op(buf, OP_EQ, "(No SSL object)");

  SSL_free(tls->ssl);
  tls->ssl = NULL;
  spider_tls_get_state_description(tls, buf, 20);
  tt_str_op(buf, OP_EQ, "(No SSL object)");

  tls->ssl = SSL_new(ctx);
  spider_tls_get_state_description(tls, buf, 200);
...
static void
test_spidertls_get_by_ssl(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  spider_tls_t *res;
  ...
  SSL_library_init();
  SSL_load_error_strings();
  spider_tls_allocate_spider_tls_object_ex_data_index();
  ...
  res = spider_tls_get_by_ssl(ssl);
  tt_assert(!res);

  SSL_set_ex_data(ssl, spider_tls_object_ex_data_index, tls);
...
static void
test_spidertls_allocate_spider_tls_object_ex_data_index(void *ignored)
{
  (void)ignored;
  int first;
  ...
  first = spider_tls_object_ex_data_index;
  spider_tls_allocate_spider_tls_object_ex_data_index();
  tt_int_op(first, OP_EQ, spider_tls_object_ex_data_index);
...
static void
test_spidertls_log_one_error(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  SSL_CTX *ctx;
  ...
  spider_tls_log_one_error(NULL, 0, LOG_WARN, 0, "something");
  expect_log_msg("TLS error while something: "
            "(null) (in (null):(null):---)\n");

  mock_clean_saved_logs();
  spider_tls_log_one_error(tls, 0, LOG_WARN, 0, NULL);
  expect_log_msg("TLS error: (null) "
            "(in (null):(null):---)\n");

  mock_clean_saved_logs();
  tls->address = spider_strdup("127.hello");
  spider_tls_log_one_error(tls, 0, LOG_WARN, 0, NULL);
  expect_log_msg("TLS error with 127.hello: "
            "(null) (in (null):(null):---)\n");
  spider_free(tls->address);

  mock_clean_saved_logs();
  tls->address = spider_strdup("127.hello");
  spider_tls_log_one_error(tls, 0, LOG_WARN, 0, "blarg");
  expect_log_msg("TLS error while blarg with "
            "127.hello: (null) (in (null):(null):---)\n");

  mock_clean_saved_logs();
  spider_tls_log_one_error(tls, ERR_PACK(1, 2, 3), LOG_WARN, 0, NULL);
  expect_log_msg("TLS error with 127.hello: "
            "BN lib (in unknown library:(null):---)\n");

  mock_clean_saved_logs();
  spider_tls_log_one_error(tls, ERR_PACK(1, 2, SSL_R_HTTP_REQUEST),
                        LOG_WARN, 0, NULL);
  expect_log_severity(LOG_INFO);

  mock_clean_saved_logs();
  spider_tls_log_one_error(tls, ERR_PACK(1, 2, SSL_R_HTTPS_PROXY_REQUEST),
                        LOG_WARN, 0, NULL);
  expect_log_severity(LOG_INFO);

  mock_clean_saved_logs();
  spider_tls_log_one_error(tls, ERR_PACK(1, 2, SSL_R_RECORD_LENGTH_MISMATCH),
                        LOG_WARN, 0, NULL);
  expect_log_severity(LOG_INFO);

#ifndef OPENSSL_1_1_API
  mock_clean_saved_logs();
  spider_tls_log_one_error(tls, ERR_PACK(1, 2, SSL_R_RECORD_TOO_LARGE),
                        LOG_WARN, 0, NULL);
  expect_log_severity(LOG_INFO);
#endif

  mock_clean_saved_logs();
  spider_tls_log_one_error(tls, ERR_PACK(1, 2, SSL_R_UNKNOWN_PROTOCOL),
                        LOG_WARN, 0, NULL);
  expect_log_severity(LOG_INFO);

  mock_clean_saved_logs();
  spider_tls_log_one_error(tls, ERR_PACK(1, 2, SSL_R_UNSUPPORTED_PROTOCOL),
                        LOG_WARN, 0, NULL);
  expect_log_severity(LOG_INFO);
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_get_error(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  int ret;
  ...
  ret = spider_tls_get_error(tls, 0, 0, "something", LOG_WARN, 0);
  ...
  mock_clean_saved_logs();
  ret = spider_tls_get_error(tls, 2, 0, "something", LOG_WARN, 0);
  tt_int_op(ret, OP_EQ, 0);
  expect_no_log_entry();

  mock_clean_saved_logs();
  ret = spider_tls_get_error(tls, 0, 1, "something", LOG_WARN, 0);
  ...
  ret = spider_tls_get_error(tls, -1, 0, "something", LOG_WARN, 0);
  ...
  mock_clean_saved_logs();
  ret = spider_tls_get_error(tls, 0, 2, "something", LOG_WARN, 0);
...
static void
test_spidertls_always_accept_verify_cb(void *ignored)
{
  (void)ignored;
  int ret;
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_x509_cert_free(void *ignored)
{
  (void)ignored;
  spider_x509_cert_t *cert;

  cert = spider_malloc_zero(sizeof(spider_x509_cert_t));
  spider_x509_cert_free(cert);

  cert = spider_malloc_zero(sizeof(spider_x509_cert_t));
  cert->cert = spider_malloc_zero(sizeof(X509));
  cert->encoded = spider_malloc_zero(1);
  spider_x509_cert_free(cert);
}
#endif

static void
test_spidertls_x509_cert_get_id_digests(void *ignored)
{
  (void)ignored;
  spider_x509_cert_t *cert;
  common_digests_t *d;
  ...
  res = spider_x509_cert_get_id_digests(cert);
  tt_assert(!res);
    ...
    if (cert->cert_info) {
      if (cert->cert_info->key) {
        if (cert->cert_info->key->pkey) {
          spider_free(cert->cert_info->key->pkey);
        }
        spider_free(cert->cert_info->key);
      }
      spider_free(cert->cert_info);
    }
    spider_free(cert);
  }
}
...
static void
test_spidertls_cert_matches_key(void *ignored)
{
  (void)ignored;
  int res;
  spider_tls_t *tls;
  ...
  tls = spider_malloc_zero(sizeof(spider_tls_t));
  cert = spider_malloc_zero(sizeof(spider_x509_cert_t));
  one = spider_malloc_zero(sizeof(X509));
  one->references = 1;
  two = spider_malloc_zero(sizeof(X509));
  two->references = 1;

  res = spider_tls_cert_matches_key(tls, cert);
  tt_int_op(res, OP_EQ, 0);

  tls->ssl = spider_malloc_zero(sizeof(SSL));
  tls->ssl->session = spider_malloc_zero(sizeof(SSL_SESSION));
  ...
  one->cert_info = spider_malloc_zero(sizeof(X509_CINF));
  one->cert_info->key = spider_malloc_zero(sizeof(X509_PUBKEY));
  one->cert_info->key->pkey = spider_malloc_zero(sizeof(EVP_PKEY));
  one->cert_info->key->pkey->references = 1;
  one->cert_info->key->pkey->ameth = meth;
  one->cert_info->key->pkey->type = 1;

  two->cert_info = spider_malloc_zero(sizeof(X509_CINF));
  two->cert_info->key = spider_malloc_zero(sizeof(X509_PUBKEY));
  two->cert_info->key->pkey = spider_malloc_zero(sizeof(EVP_PKEY));
...
static void
test_spidertls_cert_get_key(void *ignored)
{
  (void)ignored;
  spider_x509_cert_t *cert = NULL;
  crypto_pk_t *res = NULL;
  cert = spider_malloc_zero(sizeof(spider_x509_cert_t));
  X509 *key = NULL;
  key = spider_malloc_zero(sizeof(X509));
  key->references = 1;

  res = spider_tls_cert_get_key(cert);
  tt_assert(!res);

  cert->cert = key;
  key->cert_info = spider_malloc_zero(sizeof(X509_CINF));
  key->cert_info->key = spider_malloc_zero(sizeof(X509_PUBKEY));
  key->cert_info->key->pkey = spider_malloc_zero(sizeof(EVP_PKEY));
...
static void
test_spidertls_get_my_client_auth_key(void *ignored)
{
  (void)ignored;
  crypto_pk_t *ret;
  crypto_pk_t *expected;
  spider_tls_context_t *ctx;
  RSA *k = RSA_new();

  ctx = spider_malloc_zero(sizeof(spider_tls_context_t));
  expected = crypto_new_pk_from_rsa_(k);
  ctx->auth_key = expected;

  client_tls_context = NULL;
  ret = spider_tls_get_my_client_auth_key();
  tt_assert(!ret);
  ...
  RSA_free(k);
  spider_free(expected);
  spider_free(ctx);
}

static void
test_spidertls_get_my_certs(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_context_t *ctx;
  const spider_x509_cert_t *link_cert_out = NULL;
  const spider_x509_cert_t *id_cert_out = NULL;

  ctx = spider_malloc_zero(sizeof(spider_tls_context_t));

  client_tls_context = NULL;
  ret = spider_tls_get_my_certs(0, NULL, NULL);
  tt_int_op(ret, OP_EQ, -1);

  server_tls_context = NULL;
  ret = spider_tls_get_my_certs(1, NULL, NULL);
  tt_int_op(ret, OP_EQ, -1);
  ...
  client_tls_context = ctx;
  ret = spider_tls_get_my_certs(0, &link_cert_out, &id_cert_out);
  tt_int_op(ret, OP_EQ, 0);

  server_tls_context = ctx;
  ret = spider_tls_get_my_certs(1, &link_cert_out, &id_cert_out);
  tt_int_op(ret, OP_EQ, 0);
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_get_ciphersuite_name(void *ignored)
{
  (void)ignored;
  const char *ret;
  spider_tls_t *ctx;
  ctx = spider_malloc_zero(sizeof(spider_tls_t));
  ctx->ssl = spider_malloc_zero(sizeof(SSL));

  ret = spider_tls_get_ciphersuite_name(ctx);
...
static void
test_spidertls_classify_client_ciphers(void *ignored)
{
  (void)ignored;
  int i;
  int ret;
  ...
  ret = spider_tls_classify_client_ciphers(ssl, NULL);
  tt_int_op(ret, OP_EQ, -1);
  ...
  ret = spider_tls_classify_client_ciphers(ssl, ciphers);
  tt_int_op(ret, OP_EQ, 1);
  tt_int_op(tls->client_cipher_list_type, OP_EQ, 1);

  tls->client_cipher_list_type = 0;
  ret = spider_tls_classify_client_ciphers(ssl, SSL_get_ciphers(ssl));
...
static void
test_spidertls_client_is_using_v2_ciphers(void *ignored)
{
  (void)ignored;
  ...
  ret = spider_tls_client_is_using_v2_ciphers(ssl);
  tt_int_op(ret, OP_EQ, -1);
...
static void
fixed_try_to_extract_certs_from_tls(int severity, spider_tls_t *tls,
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_verify(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
  ...
  cert1 = spider_malloc_zero(sizeof(X509));
  cert1->references = 10;

  cert2 = spider_malloc_zero(sizeof(X509));
  cert2->references = 10;
  ...
  tls = spider_malloc_zero(sizeof(spider_tls_t));
  ret = spider_tls_verify(LOG_WARN, tls, &k);
  tt_int_op(ret, OP_EQ, -1);
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_check_lifetime(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
  ...
  tls = spider_malloc_zero(sizeof(spider_tls_t));
  ret = spider_tls_check_lifetime(LOG_WARN, tls, time(NULL), 0, 0);
  tt_int_op(ret, OP_EQ, -1);
  ...
  ret = spider_tls_check_lifetime(LOG_WARN, tls, time(NULL), 0, -1000);
  tt_int_op(ret, OP_EQ, -1);

  ret = spider_tls_check_lifetime(LOG_WARN, tls, time(NULL), -1000, 0);
  tt_int_op(ret, OP_EQ, -1);
...
static void
test_spidertls_get_pending_bytes(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
...
static void
test_spidertls_get_forced_write_size(void *ignored)
{
  (void)ignored;
  long ret;
  spider_tls_t *tls;
  ...
  tls->wantwrite_n = 43;
  ret = spider_tls_get_forced_write_size(tls);
...
static void
test_spidertls_get_write_overhead_ratio(void *ignored)
{
  (void)ignored;
  double ret;
...
static void
test_spidertls_used_v1_handshake(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
  ...
  ret = spider_tls_used_v1_handshake(tls);
  tt_int_op(ret, OP_EQ, 1);
...
static void
test_spidertls_get_num_server_handshakes(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
  ...
  ret = spider_tls_get_num_server_handshakes(tls);
  tt_int_op(ret, OP_EQ, 3);
...
static void
test_spidertls_server_got_renegotiate(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
  ...
  ret = spider_tls_server_got_renegotiate(tls);
  tt_int_op(ret, OP_EQ, 1);
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_SSL_SESSION_get_master_key(void *ignored)
{
  (void)ignored;
  size_t ret;
  spider_tls_t *tls;
  uint8_t *out;
  out = spider_malloc_zero(1);
  tls = spider_malloc_zero(sizeof(spider_tls_t));
  tls->ssl = spider_malloc_zero(sizeof(SSL));
  tls->ssl->session = spider_malloc_zero(sizeof(SSL_SESSION));
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_get_tlssecrets(void *ignored)
{
  (void)ignored;
  int ret;
  uint8_t *secret_out = spider_malloc_zero(DIGEST256_LEN);;
  spider_tls_t *tls;
  tls = spider_malloc_zero(sizeof(spider_tls_t));
  tls->ssl = spider_malloc_zero(sizeof(SSL));
  tls->ssl->session = spider_malloc_zero(sizeof(SSL_SESSION));
  tls->ssl->session->master_key_length = 1;
  tls->ssl->s3 = spider_malloc_zero(sizeof(SSL3_STATE));

  ret = spider_tls_get_tlssecrets(tls, secret_out);
  tt_int_op(ret, OP_EQ, 0);

 done:
  spider_free(secret_out);
  spider_free(tls->ssl->s3);
  spider_free(tls->ssl->session);
  spider_free(tls->ssl);
  spider_free(tls);
}
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_get_buffer_sizes(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
  ...
  ret = spider_tls_get_buffer_sizes(tls, &rbuf_c, &rbuf_b, &wbuf_c, &wbuf_b);
  ...
  tls->ssl->s3->rbuf.buf = spider_malloc_zero(1);
  tls->ssl->s3->wbuf.buf = spider_malloc_zero(1);
  ret = spider_tls_get_buffer_sizes(tls, &rbuf_c, &rbuf_b, &wbuf_c, &wbuf_b);
  tt_int_op(ret, OP_EQ, 0);
  ...
  spider_free(tls->ssl->s3->rbuf.buf);
  spider_free(tls->ssl->s3->wbuf.buf);
  spider_free(tls->ssl->s3);
  spider_free(tls->ssl);
  spider_free(tls);
}
...
static void
test_spidertls_evaluate_ecgroup_for_tls(void *ignored)
{
  (void)ignored;
  int ret;
...
static void
test_spidertls_try_to_extract_certs_from_tls(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  ...
  tls->ssl = spider_malloc_zero(sizeof(SSL));
  tls->ssl->session = spider_malloc_zero(sizeof(SSL_SESSION));
  sess = spider_malloc_zero(sizeof(SESS_CERT_local));
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_get_peer_cert(void *ignored)
{
  (void)ignored;
  spider_x509_cert_t *ret;
  spider_tls_t *tls;
  X509 *cert = NULL;
  ...
  ret = spider_tls_get_peer_cert(tls);
  tt_assert(!ret);
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_peer_has_cert(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
  ...
  ret = spider_tls_peer_has_cert(tls);
  tt_assert(!ret);
...
static void
test_spidertls_is_server(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  int ret;
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_session_secret_cb(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  SSL_CTX *ctx;
  ...
  ctx = SSL_CTX_new(TLSv1_method());
  tls->ssl = SSL_new(ctx);
  SSL_set_ex_data(tls->ssl, spider_tls_object_ex_data_index, tls);

  SSL_set_session_secret_cb(tls->ssl, spider_tls_session_secret_cb, NULL);

  spider_tls_session_secret_cb(tls->ssl, NULL, NULL, NULL, NULL, NULL);
  ...
  tls->client_cipher_list_type = 0;
  spider_tls_session_secret_cb(tls->ssl, NULL, NULL, ciphers, NULL, NULL);
  tt_assert(!tls->ssl->tls_session_secret_cb);
...
static void
test_spidertls_block_renegotiation(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
...
#define SSL3_FLAGS_ALLOW_UNSAFE_LEGACY_RENEGOTIATION 0
  ...
  spider_tls_block_renegotiation(tls);
...
static void
test_spidertls_unblock_renegotiation(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  ...
  tls = spider_malloc_zero(sizeof(spider_tls_t));
  tls->ssl = spider_malloc_zero(sizeof(SSL));
  spider_tls_unblock_renegotiation(tls);
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_assert_renegotiation_unblocked(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  ...
  tls->ssl = spider_malloc_zero(sizeof(SSL));
  spider_tls_unblock_renegotiation(tls);
  spider_tls_assert_renegotiation_unblocked(tls);
  /* No assertion here - this test will fail if spider_assert is turned on
   * and things are bad. */
...
static void
test_spidertls_set_logged_address(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  ...
  spider_tls_set_logged_address(tls, "foo bar");

  tt_str_op(tls->address, OP_EQ, "foo bar");

  spider_tls_set_logged_address(tls, "foo bar 2");
...
#ifndef OPENSSL_OPAQUE
static void
example_cb(spider_tls_t *t, void *arg)
{
  (void)t;
  (void)arg;
}
...
static void
test_spidertls_set_renegotiate_callback(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  ...
  spider_tls_set_renegotiate_callback(tls, example_cb, (void*)arg);
  ...
  tt_assert(tls->ssl->info_callback == spider_tls_server_info_callback);

  spider_tls_set_renegotiate_callback(tls, NULL, (void*)arg);
  tt_assert(tls->ssl->info_callback == spider_tls_debug_state_callback);
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_find_cipher_by_id(void *ignored)
{
  (void)ignored;
  int ret;
  SSL *ssl;
  ...
  SSL_METHOD *empty_method = spider_malloc_zero(sizeof(SSL_METHOD));

  fixed_cipher1 = spider_malloc_zero(sizeof(SSL_CIPHER));
  fixed_cipher2 = spider_malloc_zero(sizeof(SSL_CIPHER));
  ...
  spider_free(empty_method);
  SSL_free(ssl);
  SSL_CTX_free(ctx);
  spider_free(fixed_cipher1);
}
#endif

#ifndef OPENSSL_OPAQUE
static void
test_spidertls_debug_state_callback(void *ignored)
{
  (void)ignored;
  SSL *ssl;
  char *buf = spider_malloc_zero(1000);
  int n;

  setup_capture_of_logs(LOG_DEBUG);

  ssl = spider_malloc_zero(sizeof(SSL));

  spider_tls_debug_state_callback(ssl, 32, 45);

  n = spider_snprintf(buf, 1000, "SSL %p is now in state unknown"
               " state [type=32,val=45].\n", ssl);
  /* spider's snprintf returns -1 on error */
  tt_int_op(n, OP_NE, -1);
  expect_log_msg(buf);
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_server_info_callback(void *ignored)
{
  (void)ignored;
  spider_tls_t *tls;
  SSL_CTX *ctx;
  ...
  mock_clean_saved_logs();
  spider_tls_server_info_callback(ssl, SSL_CB_ACCEPT_LOOP, 0);
...
static void
test_spidertls_shutdown(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
...
static void
negotiated_callback_setter(spider_tls_t *t, void *arg)
{
  (void)t;
  (void)arg;
...
static void
test_spidertls_read(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
  ...
  ret = spider_tls_read(tls, buf, 10);
  tt_int_op(ret, OP_EQ, -9);
...
static void
test_spidertls_write(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
  ...
  ret = spider_tls_write(tls, buf, 0);
  tt_int_op(ret, OP_EQ, 0);

  ret = spider_tls_write(tls, buf, 10);
  tt_int_op(ret, OP_EQ, -9);
...
static void
test_spidertls_handshake(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
...
#ifndef OPENSSL_OPAQUE
static void
test_spidertls_finish_handshake(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_t *tls;
  ...
  ret = spider_tls_finish_handshake(tls);
  tt_int_op(ret, OP_EQ, 0);
  ...
  if (tls->ssl && tls->ssl->session) {
    spider_free(tls->ssl->session->sess_cert);
  }
  SSL_free(tls->ssl);
  spider_free(tls);
  SSL_CTX_free(ctx);
...
static int fixed_spider_tls_create_certificate_result_index;
static X509 *fixed_spider_tls_create_certificate_result[5];
static int fixed_spider_x509_cert_new_result_index;
static spider_x509_cert_t *fixed_spider_x509_cert_new_result[5];
...
static X509 *
fixed_spider_tls_create_certificate(crypto_pk_t *rsa,
                                 ...
                                 unsigned int cert_lifetime)
{
  ...
  (void)cname_sign;
  (void)cert_lifetime;
  return fixed_spider_tls_create_certificate_result[
                             fixed_spider_tls_create_certificate_result_index++];
}

static spider_x509_cert_t *
fixed_spider_x509_cert_new(X509 *x509_cert)
{
  (void) x509_cert;
  return fixed_spider_x509_cert_new_result[
                                      fixed_spider_x509_cert_new_result_index++];
}

static void
test_spidertls_context_new(void *ignored)
{
  (void)ignored;
  spider_tls_context_t *ret;
  ...
  ret = spider_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  ...
  MOCK(spider_tls_create_certificate, fixed_spider_tls_create_certificate);
  ...
  fixed_spider_tls_create_certificate_result_index = 0;
  fixed_spider_tls_create_certificate_result[0] = NULL;
  fixed_spider_tls_create_certificate_result[1] = spider_malloc_zero(sizeof(X509));
  fixed_spider_tls_create_certificate_result[2] = spider_malloc_zero(sizeof(X509));
  ret = spider_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  ...
  fixed_crypto_pk_generate_key_with_bits_result_index = 0;
  fixed_spider_tls_create_certificate_result_index = 0;
  fixed_spider_tls_create_certificate_result[0] = spider_malloc_zero(sizeof(X509));
  fixed_spider_tls_create_certificate_result[1] = NULL;
  fixed_spider_tls_create_certificate_result[2] = spider_malloc_zero(sizeof(X509));
  ret = spider_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  ...
  fixed_spider_tls_create_certificate_result[0] = spider_malloc_zero(sizeof(X509));
  fixed_spider_tls_create_certificate_result[1] = spider_malloc_zero(sizeof(X509));
  fixed_spider_tls_create_certificate_result[2] = NULL;
  ret = spider_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);

  MOCK(spider_x509_cert_new, fixed_spider_x509_cert_new);
  fixed_crypto_pk_new_result_index = 0;
  ...
  fixed_spider_tls_create_certificate_result[1] = spider_malloc_zero(sizeof(X509));
  fixed_spider_tls_create_certificate_result[2] = spider_malloc_zero(sizeof(X509));
  fixed_spider_x509_cert_new_result_index = 0;
  fixed_spider_x509_cert_new_result[0] = NULL;
  fixed_spider_x509_cert_new_result[1] = NULL;
  fixed_spider_x509_cert_new_result[2] = NULL;
  ret = spider_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  ...
  fixed_spider_tls_create_certificate_result[2] = spider_malloc_zero(sizeof(X509));
  fixed_spider_x509_cert_new_result_index = 0;
  fixed_spider_x509_cert_new_result[0] = spider_malloc_zero(sizeof(spider_x509_cert_t));
  fixed_spider_x509_cert_new_result[1] = NULL;
  fixed_spider_x509_cert_new_result[2] = NULL;
  ret = spider_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  ...
  fixed_spider_x509_cert_new_result_index = 0;
  fixed_spider_x509_cert_new_result[0] = spider_malloc_zero(sizeof(spider_x509_cert_t));
  fixed_spider_x509_cert_new_result[1] = spider_malloc_zero(sizeof(spider_x509_cert_t));
  fixed_spider_x509_cert_new_result[2] = NULL;
  ret = spider_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  ...
  fixed_spider_x509_cert_new_result[0] = spider_malloc_zero(sizeof(spider_x509_cert_t));
  fixed_spider_x509_cert_new_result[1] = spider_malloc_zero(sizeof(spider_x509_cert_t));
  fixed_spider_x509_cert_new_result[2] = spider_malloc_zero(sizeof(spider_x509_cert_t));
  ret = spider_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);

 done:
  UNMOCK(spider_x509_cert_new);
  UNMOCK(spider_tls_create_certificate);
...
static void
test_spidertls_create_certificate(void *ignored)
{
  (void)ignored;
  X509 *ret;
  crypto_pk_t *pk1, *pk2;
  ...
  ret = spider_tls_create_certificate(pk1, pk2, "hello", "hello2", 1);
  tt_assert(!ret);
...
static void
test_spidertls_cert_new(void *ignored)
{
  (void)ignored;
  spider_x509_cert_t *ret;
  X509 *cert = read_cert_from(validCertString);

  ret = spider_x509_cert_new(NULL);
  tt_assert(!ret);

  ret = spider_x509_cert_new(cert);
  tt_assert(ret);
  spider_x509_cert_free(ret);
  ret = NULL;
...
static void
test_spidertls_cert_is_valid(void *ignored)
{
  (void)ignored;
  int ret;
  spider_x509_cert_t *cert = NULL, *scert = NULL;

  scert = spider_malloc_zero(sizeof(spider_x509_cert_t));
  ret = spider_tls_cert_is_valid(LOG_WARN, cert, scert, time(NULL), 0);
  tt_int_op(ret, OP_EQ, 0);
  ...
  cert = spider_x509_cert_new(read_cert_from(validCertString));
  scert = spider_x509_cert_new(read_cert_from(caCertString));
  ret = spider_tls_cert_is_valid(LOG_WARN, cert, scert, time(NULL), 0);
  tt_int_op(ret, OP_EQ, 1);

#ifndef OPENSSL_OPAQUE
  spider_x509_cert_free(cert);
  spider_x509_cert_free(scert);
  cert = spider_x509_cert_new(read_cert_from(validCertString));
  scert = spider_x509_cert_new(read_cert_from(caCertString));
  ...
  ret = spider_tls_cert_is_valid(LOG_WARN, cert, scert, time(NULL), 1);
  tt_int_op(ret, OP_EQ, 0);
#endif
...
static void
test_spidertls_context_init_one(void *ignored)
{
  (void)ignored;
  int ret;
  spider_tls_context_t *old = NULL;

  MOCK(crypto_pk_new, fixed_crypto_pk_new);

  fixed_crypto_pk_new_result_index = 0;
  fixed_crypto_pk_new_result[0] = NULL;
  ret = spider_tls_context_init_one(&old, NULL, 0, 0, 0);
  tt_int_op(ret, OP_EQ, -1);
...
#define LOCAL_TEST_CASE(name, flags)                    \
  { #name, test_spidertls_##name, (flags|TT_FORK), NULL, NULL }
...
struct testcase_t spidertls_tests[] = {
  LOCAL_TEST_CASE(errno_to_tls_error, 0),
  LOCAL_TEST_CASE(err_to_string, 0),
  LOCAL_TEST_CASE(spider_tls_new, TT_FORK),
  LOCAL_TEST_CASE(spider_tls_get_error, 0),
  LOCAL_TEST_CASE(get_state_description, TT_FORK),
  LOCAL_TEST_CASE(get_by_ssl, TT_FORK),
  LOCAL_TEST_CASE(allocate_spider_tls_object_ex_data_index, TT_FORK),
