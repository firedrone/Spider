 ...
 * at large. */
#endif
#include "test_descripspiders.inc"
#include "or.h"
...
/* NOP replacement for router_descripspider_is_older_than() */
static int
router_descripspider_is_older_than_replacement(const routerinfo_t *router,
                                            int seconds)
{
  (void) router;
  (void) seconds;
...
/** Parse a file containing router descripspiders and load them to our
...
helper_setup_fake_routerlist(void)
{
  ...
  /* Read the file that contains our test descripspiders. */

  /* We need to mock this function otherwise the descripspiders will not
     accepted as they are too old. */
  MOCK(router_descripspider_is_older_than,
       router_descripspider_is_older_than_replacement);

  /* Load all the test descripspiders to the routerlist. */
  ...
  UNMOCK(router_descripspider_is_older_than);
}
...
/** Mock-replacement. As spider_addr_lookup, but always fails on any
 ...
 */
int
mock_spider_addr_lookup__fail_on_bad_addrs(const char *name,
                                        uint16_t family, spider_addr_t *out)
{
  if (name && strchr(name, '!')) {
    return -1;
  }
  return spider_addr_lookup__real(name, family, out);
}
