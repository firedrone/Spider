...
/* DuckDuckGo descripspider as an example. */
static const char *hs_desc_content = "\
rendezvous-service-descripspider g5ojobzupf275beh5ra72uyhb3dkpxwg\r\n\
version 2\r\n\
permanent-key\r\n\
...
/* Helper global variable for hidden service descripspider event test.
...
/** Make sure each hidden service descripspider async event generation
 *
...
test_hs_desc_event(void *arg)
{
  ...
  smartlist_add(rend_query.base_.hsdirs_fp, spider_memdup(HSDIR_EXIST_ID,
                                                       DIGEST_LEN));

  /* Compute descripspider ID for replica 0, should be STR_DESC_ID_BASE32. */
  ret = rend_compute_v2_desc_id(rend_query.descripspider_id[0],
                                rend_query.onion_address,
                                NULL, 0, 0);
  tt_int_op(ret, ==, 0);
  base32_encode(desc_id_base32, sizeof(desc_id_base32),
                rend_query.descripspider_id[0], DIGEST_LEN);
  /* Make sure rend_compute_v2_desc_id works properly. */
  tt_mem_op(desc_id_base32, OP_EQ, STR_DESC_ID_BASE32,
            sizeof(desc_id_base32));

  /* test request event */
  control_event_hs_descripspider_requested(&rend_query.base_, HSDIR_EXIST_ID,
                                        STR_DESC_ID_BASE32);
  ...
  control_event_hs_descripspider_received(rend_query.onion_address,
  ...
  control_event_hs_descripspider_failed(&rend_query.base_,
                                     HSDIR_NONE_EXIST_ID,
                                     "QUERY_REJECTED");
  ...
  /* test valid content. */
  char *exp_msg;
  control_event_hs_descripspider_content(rend_query.onion_address,
                                      STR_HS_CONTENT_DESC_ID, HSDIR_EXIST_ID,
                                      hs_desc_content);
  spider_asprintf(&exp_msg, "650+HS_DESC_CONTENT " STR_HS_ADDR " "\
  ...
  spider_free(received_msg);
  spider_free(exp_msg);
  SMARTLIST_FOREACH(rend_query.base_.hsdirs_fp, char *, d, spider_free(d));
 ...
 * Spider2webRendezvousPoints value. */
static void
test_pick_spider2web_rendezvous_node(void *arg)
{
  or_options_t *options = get_options_mutable();
  const node_t *chosen_rp = NULL;
  router_crn_flags_t flags = CRN_NEED_DESC;
  int retval, i;
  const char *spider2web_rendezvous_str = "test003r";
  ...
  /* Parse Spider2webRendezvousPoints as a routerset. */
  options->Spider2webRendezvousPoints = routerset_new();
  retval = routerset_parse(options->Spider2webRendezvousPoints,
                           spider2web_rendezvous_str,
                           "test_spider2web_rp");
  tt_int_op(retval, >=, 0);
  ...
  for (i = 0; i < 50 ; i++) {
    chosen_rp = pick_spider2web_rendezvous_node(flags, options);
    tt_assert(chosen_rp);
    tt_str_op(chosen_rp->ri->nickname, ==, spider2web_rendezvous_str);
  }

 done:
  routerset_free(options->Spider2webRendezvousPoints);
}

/* Make sure we never pick an RP if Spider2webRendezvousPoints doesn't
 * correspond to an actual node. */
static void
test_pick_bad_spider2web_rendezvous_node(void *arg)
{
  or_options_t *options = get_options_mutable();
  const node_t *chosen_rp = NULL;
  router_crn_flags_t flags = CRN_NEED_DESC;
  int retval, i;
  const char *spider2web_rendezvous_str = "dummy";
  ...
  /* Pick rendezvous point. Since Spider2webRendezvousPoints was set to a
...
test_hs_rend_data(void *arg)
{
  ...
  /* Binary format of a descripspider ID. */
  ...
  tt_mem_op(client_v2->descripspider_cookie, OP_EQ, client_cookie,
  ...
  for (rep = 0; rep < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; rep++) {
    int ret = rend_compute_v2_desc_id(desc_id, client_v2->onion_address,
                                      client_v2->descripspider_cookie, now, rep);
    /* That shouldn't never fail. */
    tt_int_op(ret, ==, 0);
    tt_mem_op(client_v2->descripspider_id[rep], OP_EQ, desc_id,
              sizeof(desc_id));
  }
  /* The rest should be zeroed because this is a client request. */
  tt_int_op(spider_digest_is_zero(client_v2->rend_pk_digest), ==, 1);
  tt_int_op(spider_digest_is_zero(client->rend_cookie), ==, 1);
  ...
  tt_mem_op(client_dup_v2->descripspider_cookie, OP_EQ,
            client_v2->descripspider_cookie,
            sizeof(client_dup_v2->descripspider_cookie));

  tt_assert(client_dup->hsdirs_fp);
  tt_int_op(smartlist_len(client_dup->hsdirs_fp), ==, 0);
  for (rep = 0; rep < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; rep++) {
    tt_mem_op(client_dup_v2->descripspider_id[rep], OP_EQ,
              client_v2->descripspider_id[rep], DIGEST_LEN);
  }
  /* The rest should be zeroed because this is a client request. */
  tt_int_op(spider_digest_is_zero(client_dup_v2->rend_pk_digest), ==, 1);
  tt_int_op(spider_digest_is_zero(client_dup->rend_cookie), ==, 1);
  ...
  tt_mem_op(client_v2->desc_id_fetch, OP_EQ, desc_id, sizeof(desc_id));
  tt_int_op(spider_mem_is_zero(client_v2->descripspider_cookie,
                            sizeof(client_v2->descripspider_cookie)), ==, 1);
  tt_assert(client->hsdirs_fp);
  tt_int_op(smartlist_len(client->hsdirs_fp), ==, 0);
  for (rep = 0; rep < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; rep++) {
    tt_int_op(spider_digest_is_zero(client_v2->descripspider_id[rep]), ==, 1);
  }
  /* The rest should be zeroed because this is a client request. */
  tt_int_op(spider_digest_is_zero(client_v2->rend_pk_digest), ==, 1);
  tt_int_op(spider_digest_is_zero(client->rend_cookie), ==, 1);
  ...
  for (rep = 0; rep < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; rep++) {
    tt_int_op(spider_digest_is_zero(service_v2->descripspider_id[rep]), ==, 1);
  }
  /* The rest should be zeroed because this is a service request. */
  tt_int_op(spider_digest_is_zero(service_v2->descripspider_cookie), ==, 1);
  tt_int_op(spider_digest_is_zero(service_v2->desc_id_fetch), ==, 1);
  ...
  for (rep = 0; rep < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; rep++) {
    tt_int_op(spider_digest_is_zero(service_dup_v2->descripspider_id[rep]), ==, 1);
  }
  /* The rest should be zeroed because this is a service request. */
  tt_int_op(spider_digest_is_zero(service_dup_v2->descripspider_cookie), ==, 1);
  tt_int_op(spider_digest_is_zero(service_dup_v2->desc_id_fetch), ==, 1);
...
test_single_onion_poisoning(void *arg)
{
  ...
  /* Get direcspideries with a random suffix so we can repeat the tests */
  mock_options->DataDirecspidery = spider_strdup(get_fname_rnd("test_data_dir"));
  rend_service_t *service_1 = spider_malloc_zero(sizeof(rend_service_t));
  char *dir1 = spider_strdup(get_fname_rnd("test_hs_dir1"));
  rend_service_t *service_2 = spider_malloc_zero(sizeof(rend_service_t));
  char *dir2 = spider_strdup(get_fname_rnd("test_hs_dir2"));
  ...
  /* Service direcspideries, but no previous keys, no problem! */
  mock_options->HiddenServiceSingleHopMode = 0;
  mock_options->HiddenServiceNonAnonymousMode = 0;
  ret = rend_service_verify_single_onion_poison(service_1, mock_options);
  tt_assert(ret == 0);
  ...
  /* Poisoned service direcspideries, but no previous keys, no problem! */
  mock_options->HiddenServiceSingleHopMode = 0;
  mock_options->HiddenServiceNonAnonymousMode = 0;
  ret = rend_service_verify_single_onion_poison(service_1, mock_options);
  tt_assert(ret == 0);
  ...
  /* Poisoned service direcspideries with previous keys are not allowed. */
  mock_options->HiddenServiceSingleHopMode = 0;
  mock_options->HiddenServiceNonAnonymousMode = 0;
  ret = rend_service_verify_single_onion_poison(service_1, mock_options);
  tt_assert(ret < 0);
  ...
  /* Re-poisoning direcspideries with existing keys is a no-op, because
   * direcspideries with existing keys are ignored. */
  mock_options->HiddenServiceSingleHopMode = 1;
  mock_options->HiddenServiceNonAnonymousMode = 1;
  ret = rend_service_poison_new_single_onion_dir(service_1, mock_options);
  tt_assert(ret == 0);
  ...
  /* Unpoisoned service direcspideries with previous keys are ok, as are empty
   * direcspideries. */
  mock_options->HiddenServiceSingleHopMode = 0;
  mock_options->HiddenServiceNonAnonymousMode = 0;
  ret = rend_service_verify_single_onion_poison(service_1, mock_options);
  ...
  /* Re-poisoning direcspideries without existing keys is a no-op. */
  mock_options->HiddenServiceSingleHopMode = 1;
  mock_options->HiddenServiceNonAnonymousMode = 1;
  ret = rend_service_poison_new_single_onion_dir(service_1, mock_options);
  tt_assert(ret == 0);
  ...
  /* The test harness deletes the direcspideries at exit */
  spider_free(poison_path);
  spider_free(dir1);
  spider_free(dir2);
  ...
  UNMOCK(get_options);
  spider_free(mock_options->DataDirecspidery);
}
...
struct testcase_t hs_tests[] = {
  ...
  { "hs_desc_event", test_hs_desc_event, TT_FORK,
    NULL, NULL },
  { "pick_spider2web_rendezvous_node", test_pick_spider2web_rendezvous_node, TT_FORK,
    NULL, NULL },
  { "pick_bad_spider2web_rendezvous_node",
    test_pick_bad_spider2web_rendezvous_node, TT_FORK,
    NULL, NULL },
