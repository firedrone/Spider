...
test_parse_guardfraction_file_bad(void *arg)
{
  ...
  spider_asprintf(&guardfraction_bad,
...
test_parse_guardfraction_file_good(void *arg)
{
  ...
  spider_asprintf(&guardfraction_good,
               "guardfraction-file-version 1\n"
               "written-at %s\n"
               "n-inputs 420 3\n"
               "guard-seen %s %d 420\n",
  ...
  spider_free(guardfraction_good);
}
 ...
 * according to the spiderrc option and consensus parameter. */
static void
test_should_apply_guardfraction(void *arg)
{
  ...
  /* If spiderrc option is set to yes, we should always use
   * guardfraction.*/
  options->UseGuardFraction = 1;
  tt_int_op(should_apply_guardfraction(&vote_disabled), ==, 1);

  /* If spiderrc option is set to no, we should never use
   * guardfraction.*/
  options->UseGuardFraction = 0;
  tt_int_op(should_apply_guardfraction(&vote_enabled), ==, 0);

  /* Now let's test spiderrc option set to auto. */
  options->UseGuardFraction = -1;

  /* If spiderrc option is set to auto, and consensus parameter is set to
  ...
  /* If spiderrc option is set to auto, and consensus parameter is not
   * set, we should fallback to "no". */
  tt_int_op(should_apply_guardfraction(&vote_missing), ==, 0);

 done:
  SMARTLIST_FOREACH(vote_enabled.net_params, char *, cp, spider_free(cp));
  SMARTLIST_FOREACH(vote_disabled.net_params, char *, cp, spider_free(cp));
  SMARTLIST_FOREACH(vote_missing.net_params, char *, cp, spider_free(cp));
