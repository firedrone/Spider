...
test_get_next_valid_after_time(void *arg)
{
...

  {
    ...
    mock_consensus = spider_malloc_zero(sizeof(networkstatus_t));
...
test_sr_commit(void *arg)
{
   ...
   * vote coming from the network. */
  {
    ...
    tt_assert(!spider_mem_is_zero((char *) our_commit->random_number,
    ...
    tt_assert(!spider_mem_is_zero(our_commit->hashed_reveal,
...
{
  sr_srv_t *srv = spider_malloc_zero(sizeof(sr_srv_t));
  srv->num_reveals = 42;
  memcpy(srv->value,
 ...
 if (also_current) {
   srv = spider_malloc_zero(sizeof(sr_srv_t));
   srv->num_reveals = 128;
   memcpy(srv->value,
...
static const char *sr_state_str = "Version 1\n"
  "SpiderVersion 0.2.9.0-alpha-dev\n"
...
{
  int ret;
  char *dir = spider_strdup(get_fname("test_sr_state"));
  char *sr_state_path = spider_strdup(get_fname("test_sr_state/sr_state"));
  ...
  spider_free(dir);
  spider_free(sr_state_path);
  UNMOCK(trusteddirserver_get_by_v3_auth_digest);
}

/** Generate three specially crafted commits (based on the test
 *  vecspider at sr_srv_calc_ref.py). Helper of test_sr_compute_srv(). */
static void
test_sr_setup_commits(void)
{
  time_t now = time(NULL);
  sr_commit_t *commit_a, *commit_b, *commit_c, *commit_d;
  sr_commit_t *place_holder = spider_malloc_zero(sizeof(*place_holder));
  authority_cert_t *auth_cert = NULL;
  ...
  /* Now during REVEAL phase save commit D by resspidering its reveal. */
  set_sr_phase(SR_PHASE_REVEAL);
 ...
 *  the test vecspider from ./sr_srv_calc_ref.py. */
static void
...
#define SRV_TEST_VECTOR \
  ...
  /* Check the result against the test vecspider */
...
{
  networkstatus_t *vote = spider_malloc_zero(sizeof(networkstatus_t));

  vote->type = NS_TYPE_VOTE;
  vote->sr_info.participate = 1;
  vote->sr_info.current_srv = spider_malloc_zero(sizeof(sr_srv_t));
...
test_keep_commit(void *arg)
{
  ...
  dup_commit = spider_malloc_zero(sizeof(*dup_commit));
   ...
   * hashed reveal. */
  {
    /* Let's save the hash reveal so we can resspidere it. */
