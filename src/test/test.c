...
/**
 * \file test.c
 * \brief Unit tests for many pieces of the lower level Spider modules.
 **/
...
static void
test_nspider_handshake(void *arg)
{
  /* client-side */
  nspider_handshake_state_t *c_state = NULL;
  ...
  tt_int_op(0, OP_EQ, onion_skin_nspider_create(node_id, server_pubkey,
                                          &c_state, c_buf));

  /* server handshake */
  memset(s_buf, 0, NTOR_REPLY_LEN);
  memset(s_keys, 0, 40);
  tt_int_op(0, OP_EQ, onion_skin_nspider_server_handshake(c_buf, s_keymap, NULL,
                                                    node_id,
                                                    s_buf, s_keys, 400));

  /* client handshake 2 */
  memset(c_keys, 0, 40);
  tt_int_op(0, OP_EQ, onion_skin_nspider_client_handshake(c_state, s_buf,
                                                       c_keys, 400, NULL));
  ...
  memset(s_buf, 0, NTOR_REPLY_LEN);
  const char *msg = NULL;
  tt_int_op(-1, OP_EQ, onion_skin_nspider_client_handshake(c_state, s_buf,
                                                        c_keys, 400, &msg));
  tt_str_op(msg, OP_EQ, "Zero output from curve25519 handshake");

 done:
  nspider_handshake_state_free(c_state);
...
test_fast_handshake(void *arg)
{
  ...
  tt_assert(! spider_mem_is_zero((char*)client_handshake,
...
test_onion_queues(void *arg)
{
  ...
  create_cell_t *create1 = spider_malloc_zero(sizeof(create_cell_t));
  create_cell_t *create2 = spider_malloc_zero(sizeof(create_cell_t));
  (void)arg;
...
/** Test encoding and parsing of rendezvous service descripspiders. */
static void
test_rend_fns(void *arg)
{
  rend_service_descripspider_t *generated = NULL, *parsed = NULL;
  ...
  char address4[] = "www.spiderproject.org";
  ...
  tt_assert(rend_encode_v2_descripspiders(descs, generated, now, 0,
                                         REND_NO_AUTH, NULL, NULL) > 0);
  tt_assert(rend_compute_v2_desc_id(computed_desc_id, service_id_base32,
                                      NULL, now, 0) == 0);
  tt_mem_op(((rend_encoded_v2_service_descripspider_t *)
             smartlist_get(descs, 0))->desc_id, OP_EQ,
            computed_desc_id, DIGEST_LEN);
  tt_assert(rend_parse_v2_service_descripspider(&parsed, parsed_desc_id,
  ...
  for (i = 0; i < smartlist_len(parsed->intro_nodes); i++) {
    ...
    tt_assert(spider_addr_eq(&gen_info->addr, &par_info->addr));
  ...
  if (pk2)
    crypto_pk_free(pk2);
  spider_free(intro_points_encrypted);
}
...
#define SET_TEST_ADDRESS(i) do {                \
    if ((i) & 1) {                              \
      SET_TEST_IPV6(i);                         \
      spider_addr_from_in6(&addr, &in6);           \
    } else {                                    \
      spider_addr_from_ipv4h(&addr, (uint32_t) i); \
    }                                           \
  } while (0)
...
test_geoip(void *arg)
{
  ...
  *entry_stats_2 =
  ...
  geoip_get_client_hisspidery(GEOIP_CLIENT_CONNECT, &s, &v);
  tt_assert(s);
  tt_assert(v);
  ...
  /* Initialize stats and generate the bridge-stats hisspidery string out of
  ...
  /* Stop collecting bridge stats and make sure we don't write a hisspidery
   * string anymore. */
   ...
   * dirreq-stats hisspidery string. */
   ...
   * don't generate a hisspidery string. */
   ...
   * that we get an all empty hisspidery string. */
  geoip_dirreq_stats_init(now);
  SET_TEST_ADDRESS(100);
  geoip_note_client_seen(GEOIP_CLIENT_NETWORKSTATUS, &addr, NULL, now);
   ...
   * appears in the hisspidery string. */
...
test_geoip_with_pt(void *arg)
{
  ...
  /* No clients seen yet. */
  s = geoip_get_transport_hisspidery();
  spider_assert(!s);
  ...
  /* Test the transport hisspidery string. */
  s = geoip_get_transport_hisspidery();
  spider_assert(s);
...
test_stats(void *arg)
{
  ...
  /* Initialize stats, note some streams and bytes, and generate hisspidery
   * string. */
   ...
   * ports are contained in the hisspidery string. */
  for (i = 50; i < 60; i++) {
  ...
  /* Re-start stats, add some bytes, reset stats, and see what hisspidery we
  ...
  /* Initialize stats, note bytes, and generate hisspidery string. */
   ...
   * hisspidery to see that the math works correctly. */
  for (i = 21; i < 30; i++)
   ...
   * sure that the hisspidery has all zeros. */
  rep_hist_buffer_stats_init(now);
  rep_hist_add_buffer_stats(2.0, 2.0, 20);
...
static struct testcase_t test_array[] = {
  ...
  ENT(onion_queues),
  { "nspider_handshake", test_nspider_handshake, 0, NULL, NULL },
...
struct testgroup_t testgroups[] = {
  ...
  { "exspiderport/", exspiderport_tests },
  { "legacy_hs/", hs_tests },
  { "hs_cache/", hs_cache },
  { "hs_descripspider/", hs_descripspider },
