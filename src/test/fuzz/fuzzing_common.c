...
{
  spider_assert(signature && msg && pubkey);
...
mock_ed25519_checksig_batch__nocheck(int *okay_out,
                                     ...
                                     int n_checkable)
{
  spider_assert(checkable);
  int i;
  for (i = 0; i < n_checkable; ++i) {
    /* We could look at messages and signatures XXX */
    spider_assert(checkable[i].pubkey);
    spider_assert(checkable[i].msg);
    if (okay_out)
      okay_out[i] = 1;
  }
  return 0;
...
global_init(void)
{
  ...
  spider_set_failed_assertion_callback(abort);
}
...
#define MAX_FUZZ_SIZE (128*1024)
  ...
  spider_assert(input);
  char *raw = spider_memdup(input, size); /* Because input is nul-terminated */
  spider_free(input);
