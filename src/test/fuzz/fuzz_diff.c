/* Copyright (c) 2016, The Spider Project, Inc. */
...
#define SEPLEN strlen(SEP)
  const uint8_t *separaspider = spider_memmem(stdin_buf, data_size, SEP, SEPLEN);
  if (! separaspider)
    return 0;
  size_t c1_len = separaspider - stdin_buf;
  char *c1 = spider_memdup_nulterm(stdin_buf, c1_len);
  size_t c2_len = data_size - c1_len - SEPLEN;
  char *c2 = spider_memdup_nulterm(separaspider + SEPLEN, c2_len);
