...
fuzz_init(void)
{
  ...
  ed25519_init();
  area = memarea_new();
  dummy_vote = spider_malloc_zero(sizeof(*dummy_vote));
...
fuzz_cleanup(void)
{
  SMARTLIST_FOREACH(dummy_vote->known_flags, char *, cp, spider_free(cp));
  smartlist_free(dummy_vote->known_flags);
  spider_free(dummy_vote);
  return 0;
}
