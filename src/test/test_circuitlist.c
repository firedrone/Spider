...
test_clist_maps(void *arg)
{
  ...
  ch1->cmux = spider_malloc(1);
  ch2->cmux = spider_malloc(1);
  ch3->cmux = spider_malloc(1);
  ...
  if (or_c2)
    circuit_free(TO_CIRCUIT(or_c2));
  if (ch1)
    spider_free(ch1->cmux);
  if (ch2)
    spider_free(ch2->cmux);
  if (ch3)
    spider_free(ch3->cmux);
  spider_free(ch1);
  spider_free(ch2);
  spider_free(ch3);
...
test_rend_token_maps(void *arg)
{
  ...
  tt_ptr_op(c3, OP_EQ, hs_circuitmap_get_rend_circ(tok2));
  /* Change its cookie.  This never happens in Spider per se, but hey. */
...
test_pick_circid(void *arg)
{
  ...
  chan1 = spider_malloc_zero(sizeof(channel_t));
  chan2 = spider_malloc_zero(sizeof(channel_t));
