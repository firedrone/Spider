...
test_config_addressmap(void *arg)
{
  ...
  strlcpy(buf, "MapAddress .invalidwildcard.com *.spiderserver.exit\n" // invalid
          "MapAddress *invalidasterisk.com *.spiderserver.exit\n" // invalid
          "MapAddress *.google.com *.spiderserver.exit\n"
          "MapAddress *.yahoo.com *.google.com.spiderserver.exit\n"
          "MapAddress *.cn.com www.cnn.com\n"
          "MapAddress *.cnn.com www.cnn.com\n"
          "MapAddress ex.com www.cnn.com\n"
          "MapAddress ey.com *.cnn.com\n"
          "MapAddress www.spiderproject.org 1.1.1.1\n"
          "MapAddress other.spiderproject.org "
            "this.spiderproject.org.otherserver.exit\n"
          "MapAddress test.spiderproject.org 2.2.2.2\n"
...
#define addressmap_rewrite(a,s,eo,ao)                                   \
  ...
  /* MapAddress .invalidwildcard.com .spiderserver.exit  - no match */
  strlcpy(address, "www.invalidwildcard.com", sizeof(address));
  tt_assert(!addressmap_rewrite(address, sizeof(address), &expires, NULL));

  /* MapAddress *invalidasterisk.com .spiderserver.exit  - no match */
  ...
  tt_assert(addressmap_rewrite(address, sizeof(address), &expires, NULL));
  tt_str_op(address,OP_EQ, "reader.spiderserver.exit");

  /* MapAddress *.yahoo.com *.google.com.spiderserver.exit */
  strlcpy(address, "reader.yahoo.com", sizeof(address));
  tt_assert(addressmap_rewrite(address, sizeof(address), &expires, NULL));
  tt_str_op(address,OP_EQ, "reader.google.com.spiderserver.exit");
  ...
  strlcpy(address, "www.spiderproject.org", sizeof(address));
  tt_assert(addressmap_rewrite(address, sizeof(address), &expires, NULL));
  tt_str_op(address,OP_EQ, "1.1.1.1");

  strlcpy(address, "other.spiderproject.org", sizeof(address));
  tt_assert(addressmap_rewrite(address, sizeof(address), &expires, NULL));
  tt_str_op(address,OP_EQ, "this.spiderproject.org.otherserver.exit");

  strlcpy(address, "test.spiderproject.org", sizeof(address));
  tt_assert(addressmap_rewrite(address, sizeof(address), &expires, NULL));
  ...
  */
  ...
  strlcpy(buf, "MapAddress *.com *.spiderserver.exit\n"
          "MapAddress *.spiderproject.org 1.1.1.1\n"
  ...
  tt_assert(addressmap_rewrite(address, sizeof(address), &expires, NULL));
  tt_str_op(address,OP_EQ, "www.abc.spiderserver.exit");

  strlcpy(address, "www.def.com", sizeof(address));
  tt_assert(addressmap_rewrite(address, sizeof(address), &expires, NULL));
  tt_str_op(address,OP_EQ, "www.def.spiderserver.exit");
  ...
  strlcpy(address, "spiderproject.net", sizeof(address));
  tt_assert(addressmap_rewrite(address, sizeof(address), &expires, NULL));
  tt_str_op(address,OP_EQ, "2.2.2.2");
  ...
  config_free_lines(get_options_mutable()->AddressMap);
  addressmap_clear_configured();
  strlcpy(buf, "MapAddress * *.spiderserver.exit\n", sizeof(buf));
  config_get_lines(buf, &(get_options_mutable()->AddressMap), 0);
  config_register_addressmaps(get_options());
...
test_config_check_or_create_data_subdir(void *arg)
{
  ...
  datadir = options->DataDirecspidery = spider_strdup(get_fname("datadir-0"));
  subpath = get_datadir_fname(subdir);

#if defined (_WIN32)
  tt_int_op(mkdir(options->DataDirecspidery), OP_EQ, 0);
#else
...
test_config_write_to_data_subdir(void *arg)
{
  ...
  spider_free(options->DataDirecspidery);
  datadir = options->DataDirecspidery = spider_strdup(get_fname("datadir-1"));
  ...
  rmdir(options->DataDirecspidery);
  spider_free(datadir);
  spider_free(filepath);
  spider_free(cp);
}
                      ...
                      const smartlist_t *test_socks_args)
{
  ...
  /* test addrport */
  tmp = spider_strdup(fmt_addrport(&bridge_line->addr, bridge_line->port));
  ...
  if (test_digest && spider_digest_is_zero(bridge_line->digest))
    tt_assert(0);
  ...
  if (test_digest) {
    tmp = spider_strdup(hex_str(bridge_line->digest, DIGEST_LEN));
    spider_strlower(tmp);
...
test_config_parse_bridge_line(void *arg)
{
...

  {
    ...
    SMARTLIST_FOREACH(sl_tmp, char *, s, spider_free(s));
...
test_config_parse_transport_options_line(void *arg)
{
  ...
  { /* correct -- no transport specified */
    ...
    smartlist_free(sl_tmp);
    sl_tmp = NULL;
    SMARTLIST_FOREACH(options_sl, char *, s, spider_free(s));
...
static int transport_add_from_config_mock(const spider_addr_t *addr,
...
static int
transport_add_from_config_mock(const spider_addr_t *addr,
                               uint16_t port, const char *name,
                               int socks_ver)
{
  (void) addr;
...
test_config_fix_my_family(void *arg)
{
  ...
  options->MyFamily = spider_strdup(family);
...
/** This mock function is meant to replace spider_lookup_hostname().
 ...
 */
static int
spider_lookup_hostname_01010101(const char *name, uint32_t *addr)
{
  n_hostname_01010101++;
 ...
 */
static int
spider_lookup_hostname_localhost(const char *name, uint32_t *addr)
{
 ...
 */
static int
spider_lookup_hostname_failure(const char *name, uint32_t *addr)
{
  (void)name;
  (void)addr;
 ...
 */
static int
spider_gethostname_localhost(char *name, size_t namelen)
{
                                   ...
                                   spider_addr_t *addr)
{
  ...
  spider_addr_from_ipv4h(addr,0x09090909);
...
test_config_resolve_my_address(void *arg)
{
  ...
  */
  ...
  options->Address = spider_strdup("128.52.128.105");
  ...
  spider_free(options->Address);

/*
 * CASE 2:
 * If options->Address is a valid DNS address, we want resolve_my_address()
 * function to ask spider_lookup_hostname() for help with resolving it
 * and return the address that was resolved (in host order).
 */

  MOCK(spider_lookup_hostname,spider_lookup_hostname_01010101);

  spider_free(options->Address);
  options->Address = spider_strdup("www.spiderproject.org");
  ...
  tt_want_str_op(hostname_out,==,"www.spiderproject.org");
  tt_assert(resolved_addr == 0x01010101);

  UNMOCK(spider_lookup_hostname);

  spider_free(options->Address);
  spider_free(hostname_out);

/*
 * CASE 3:
 * Given that options->Address is NULL, we want resolve_my_address()
 * to try and use spider_gethostname() to get hostname AND use
 * spider_lookup_hostname() to get IP address.
 */
  ...
  MOCK(spider_gethostname,spider_gethostname_replacement);
  MOCK(spider_lookup_hostname,spider_lookup_hostname_01010101);
 ...
 */
  ...
  resolved_addr = 0;
  spider_free(options->Address);
  options->Address = spider_strdup("127.0.0.1");
 ...
 */
  ...
  MOCK(spider_lookup_hostname,spider_lookup_hostname_failure);

  prev_n_hostname_failure = n_hostname_failure;

  spider_free(options->Address);
  options->Address = spider_strdup("www.spider-project.org");
 ...
 */
  ...
  MOCK(spider_gethostname,spider_gethostname_failure);
 ...
 * get_interface_address() if hostname returned by spider_gethostname() cannot be
...
/*
 * CASE 8:
 * Suppose options->Address is NULL AND hostname returned by spider_gethostname()
...
/*
 * CASE 9:
 * Given that options->Address is NULL AND spider_lookup_hostname()
 ...
 */
   ...
   *      spider_inet_aton()
   *   3. ... and spider_lookup_hostname() fails to resolve the
   *      options->Address
   */
  ...
  spider_free(options->Address);
  options->Address = spider_strdup("some_hostname");
   ...
   *   2. spider_gethostname() succeeds to get hostname of machine Spider
   *      if running on.
   *   3. Hostname from previous step cannot be converted to
   *      address by using spider_inet_aton() function.
   *   4. However, spider_lookup_hostname() succeds in resolving the
   *      hostname from step 2.
   *   5. Unfortunately, spider_addr_is_internal() deems this address
   *      to be internal.
  ...
  MOCK(spider_gethostname,spider_gethostname_replacement);
  MOCK(spider_lookup_hostname,spider_lookup_hostname_localhost);
  MOCK(get_interface_address6,get_interface_address6_replacement);
   ...
   *   2. spider_gethostname() succeeds in getting hostname of a machine ...
   *   3. ... which is successfully parsed by spider_inet_aton() ...
   *   4. into IPv4 address that spider_addr_is_inernal() considers to be
   *      internal.
   *
  ...
  spider_free(options->Address);
  options->Address = NULL;
  options->DirAuthorities = spider_malloc_zero(sizeof(config_line_t));

  MOCK(spider_gethostname,spider_gethostname_localhost);
  ...
  spider_free(options->Address);
  spider_free(options->DirAuthorities);
  or_options_free(options);
  spider_free(hostname_out);
...
test_config_adding_trusted_dir_server(void *arg)
{
  ...
  rv = spider_addr_port_parse(LOG_WARN, "[::1]:9061", &ipv6.addr, &ipv6.port, -1);
  tt_assert(rv == 0);
...
test_config_adding_fallback_dir_server(void *arg)
{
  ...
  rv = spider_addr_parse(&ipv4, "127.0.0.1");
...
test_config_adding_dir_servers(void *arg)
{
  ...
  /* allocate options */
  or_options_t *options = spider_malloc_zero(sizeof(or_options_t));

  /* Allocate and populate configuration lines:
   *
   * Use the same format as the hard-coded direcspideries in
   ...
   */
  config_line_t *test_dir_authority = spider_malloc_zero(sizeof(config_line_t));
  test_dir_authority->key = spider_strdup("DirAuthority");
  test_dir_authority->value = spider_strdup(
    "D0 orport=9000 "
    "v3ident=0023456789012345678901234567890123456789 "
    "127.0.0.1:60090 0123 4567 8901 2345 6789 0123 4567 8901 2345 6789"
    );

  config_line_t *test_alt_bridge_authority = spider_malloc_zero(
                                                      sizeof(config_line_t));
  test_alt_bridge_authority->key = spider_strdup("AlternateBridgeAuthority");
  test_alt_bridge_authority->value = spider_strdup(
    "B1 orport=9001 bridge "
    "127.0.0.1:60091 1123 4567 8901 2345 6789 0123 4567 8901 2345 6789"
    );

  config_line_t *test_alt_dir_authority = spider_malloc_zero(
                                                      sizeof(config_line_t));
  test_alt_dir_authority->key = spider_strdup("AlternateDirAuthority");
  test_alt_dir_authority->value = spider_strdup(
    "A2 orport=9002 "
   ...
   *
   * 1. Outcome: Use Set Direcspidery Authorities
   ...
   *  - Use Default Non-Bridge Direcspidery Authorities
   ...
   *
   * 3. Outcome: Use Set Alternate Direcspidery Authority
   *  - Use Default Bridge Authorities
   *  - Use FallbackDir if it is set, otherwise No Default Fallback Direcspideries
   *  Cases expected to yield this outcome:
   *    2 & 3 (the 2 cases where DirAuthorities and AlternateBridgeAuthority
   *           are both NULL, but AlternateDirAuthority is set)
   *
   * 4. Outcome: Use Set Custom Fallback Direcspidery
   *  - Use Default Bridge & Direcspidery Authorities
   *  Cases expected to yield this outcome:
   ...
   *  - Use Default Bridge & Direcspidery Authorities, and
   *    Default Fallback Direcspideries
   *  Cases expected to yield this outcome:
  ...
  /*
   * Find out how many default Bridge, Non-Bridge and Fallback Direcspideries
...
#define n_default_authorities ((n_default_alt_bridge_authority) \
   ...
   * Use 0000: No Direcspidery Authorities or Fallback Direcspideries Set
   */
  {
  ...
  */
     ...
     AlternateDirAuthority & FallbackDir Set */
  {
...

    {
      ...
      const smartlist_t *fallback_servers = router_get_fallback_dir_servers();
      /* (No D0), (No B1), Default Bridge Authorities, A2,
       * Custom Fallback Direcspidery, (No Default Fallback Direcspideries) */
      tt_assert(smartlist_len(fallback_servers) ==
   ...
   AlternateDirAuthority Set, FallbackDir Not Set */
  {
...

    {
      ...
      const smartlist_t *dir_servers = router_get_trusted_dir_servers();
      /* (No D0), (No B1), Default Bridge Authorities, A2,
       * No Default or Custom Fallback Direcspideries */
      tt_assert(smartlist_len(dir_servers) ==
                1 + n_default_alt_bridge_authority);
...

    {
      ...
      const smartlist_t *fallback_servers = router_get_fallback_dir_servers();
      /* (No D0), (No B1), Default Bridge Authorities, A2,
       * No Custom or Default Fallback Direcspideries */
      tt_assert(smartlist_len(fallback_servers) ==
                1 + n_default_alt_bridge_authority);
  ...
  */
    ...
    & AlternateDirAuthority Not Set, FallbackDir Set */
  {
...

    {
       ...
       * (No A2), Default v3 Direcspidery Authorities */
      ...
      /* There's no easy way of checking that we have included all the
       * default Bridge & V3 Direcspidery authorities, so let's assume that
       * if the total count above is correct, we have the right ones.
       */
    }
  ...
  /*
   5. Outcome: Use All Defaults
     - Use Default Bridge & Direcspidery Authorities, Default Fallback Direcspideries
     Cases expected to yield this outcome:
       0 (DirAuthorities, AlternateBridgeAuthority, AlternateDirAuthority
          and FallbackDir are all NULL)
  */
  ...
  /* Case 0: 0000 - All Not Set */
  {
...

    {
      ...
      /* (No D0), (No B1), Default Bridge Authorities,
       * (No A2), Default v3 Direcspidery Authorities,
       * (No Custom Fallback Direcspidery), Default Fallback Direcspideries */
      tt_assert(smartlist_len(fallback_servers) ==
      ...
      /* There's no easy way of checking that we have included all the
       * default Bridge & V3 Direcspidery authorities, and the default
       * Fallback Direcspideries, so let's assume that if the total count
  ...
  spider_free(test_dir_authority->key);
  spider_free(test_dir_authority->value);
  spider_free(test_dir_authority);

  spider_free(test_alt_dir_authority->key);
  spider_free(test_alt_dir_authority->value);
  spider_free(test_alt_dir_authority);

  spider_free(test_alt_bridge_authority->key);
...
test_config_default_dir_servers(void *arg)
{
  ...
  opts = spider_malloc_zero(sizeof(or_options_t));
...
test_config_direcspidery_fetch(void *arg)
{
   ...
   * know their own address, but never use multiple direcspideries for bootstrap
   */
  memset(options, 0, sizeof(or_options_t));
  options->ORPort_set = 1;
   ...
   * refuse unknown exits, but never use multiple direcspideries for bootstrap
   */
  memset(options, 0, sizeof(or_options_t));
  options->ORPort_set = 1;
  options->ExitRelay = 1;
...
{
  config_line_t *config_line = spider_malloc(sizeof(config_line_t));
  memset(config_line, 0, sizeof(config_line_t));
  config_line->key = spider_strdup(key);
  config_line->value = spider_strdup(val);
  return config_line;
}
...
test_config_parse_port_config__ports__ports_given(void *data)
{
  ...
  port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
  tt_int_op(port_cfg->port, OP_EQ, CFG_AUTO_PORT);
  spider_addr_parse(&addr, "127.0.0.46");
  tt_assert(spider_addr_eq(&port_cfg->addr, &addr))
  ...
  port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
  tt_int_op(port_cfg->port, OP_EQ, CFG_AUTO_PORT);
  spider_addr_parse(&addr, "127.0.0.122");
  tt_assert(spider_addr_eq(&port_cfg->addr, &addr))
  ...
  MOCK(spider_addr_lookup, mock_spider_addr_lookup__fail_on_bad_addrs);
  ret = parse_port_config(NULL, config_port_invalid, NULL, "DNS", 0,
                          "127.0.0.46", 0, 0);
  UNMOCK(spider_addr_lookup);
  tt_int_op(ret, OP_EQ, -1);
  ...
  spider_addr_parse(&addr, "127.0.0.123");
  tt_assert(spider_addr_eq(&port_cfg->addr, &addr))
...
test_config_parse_log_severity(void *data)
{
  ...
  severity = spider_malloc(sizeof(log_severity_list_t));
