/* Copyright (c) 2013-2017, The Spider Project, Inc. */
...
test_policies_general(void *arg)
{
  ...
  spider_addr_from_ipv4h(&tar, 0xc0a80000u);
  tt_int_op(0,OP_EQ, spider_addr_compare(&p->addr, &tar, CMP_EXACT));
  ...
  spider_addr_from_ipv4h(&tar, 0x01020304u);
  tt_assert(ADDR_POLICY_ACCEPTED ==
          compare_spider_addr_to_addr_policy(&tar, 2, policy));
  spider_addr_make_unspec(&tar);
  tt_assert(ADDR_POLICY_PROBABLY_ACCEPTED ==
          compare_spider_addr_to_addr_policy(&tar, 2, policy));
  spider_addr_from_ipv4h(&tar, 0xc0a80102);
  ...
  spider_addr_from_ipv4h(&tar, 0x0306090cu);
  spider_addr_parse(&tar2, "[2000::1234]");
...
#define TT_BAD_SHORT_POLICY(s)                                          \
  ...
  /* Test a too-long policy. */
  {
    ...
    SMARTLIST_FOREACH(chunks, char *, ch, spider_free(ch));
    smartlist_free(chunks);
    short_parsed = parse_short_policy(policy_strng);/* shouldn't be accepted */
    spider_free(policy_strng);
  ...
  for (i=1; i<2000; i+=2) {
    ...
    spider_snprintf(buf, sizeof(buf), "reject *:%d", i);
  ...
  spider_free(policy_str);
  if (sm) {
    SMARTLIST_FOREACH(sm, char *, s, spider_free(s));
    smartlist_free(sm);
  }
...
test_policies_reject_exit_address(void *arg)
{
  ...
  spider_addr_t ipv4_addr, ipv6_addr;
  smartlist_t *ipv4_list, *ipv6_list, *both_list, *dupl_list;
  (void)arg;

  spider_addr_from_ipv4h(&ipv4_addr, TEST_IPV4_ADDR);
  spider_addr_parse(&ipv6_addr, TEST_IPV6_ADDR);
...
test_policies_reject_port_address(void *arg)
{
  ...
  spider_addr_from_ipv4h(&ipv4_port->addr, TEST_IPV4_ADDR);
  smartlist_add(test_configured_ports, ipv4_port);

  ipv6_port = port_cfg_new(0);
  spider_addr_parse(&ipv6_port->addr, TEST_IPV6_ADDR);
                            ...
                            int include_internal)
{
  ...
  SMARTLIST_FOREACH_BEGIN(template_list, spider_addr_t *, src_addr) {
    spider_addr_t *dest_addr = spider_malloc(sizeof(spider_addr_t));
    memset(dest_addr, 0, sizeof(*dest_addr));
    spider_addr_copy_tight(dest_addr, src_addr);
...
test_dump_exit_policy_to_string(void *arg)
{
 ...
 routerinfo_t *ri = spider_malloc_zero(sizeof(routerinfo_t));
...
test_policies_getinfo_helper_policies(void *arg)
{
  ...
  spider_addr_parse(&mock_my_routerinfo.ipv6_addr, TEST_IPV6_ADDR);
  append_exit_policy_string(&mock_my_routerinfo.exit_policy, "accept *4:*");
  append_exit_policy_string(&mock_my_routerinfo.exit_policy, "reject *6:*");

  mock_options.IPv6Exit = 1;
  spider_addr_from_ipv4h(
...
{
  (void)arg;
  spider_addr_t ipv4_addr, ipv6_addr, r_ipv4_addr, r_ipv6_addr;
  spider_addr_t n_ipv4_addr, n_ipv6_addr;
  ...
  /* Parse the addresses */
  spider_addr_parse(&ipv4_addr, TEST_IPV4_ADDR_STR);
  spider_addr_parse(&ipv6_addr, TEST_IPV6_ADDR_STR);
  spider_addr_parse(&r_ipv4_addr, OTHER_IPV4_ADDR_STR);
  spider_addr_parse(&r_ipv6_addr, OTHER_IPV6_ADDR_STR);
  spider_addr_make_null(&n_ipv4_addr, AF_INET);
  spider_addr_make_null(&n_ipv6_addr, AF_INET6);
  ...
  /* NULL and spider_addr_is_null addresses are rejected */
...
#define CHECK_CHOSEN_ADDR_RS(fake_rs, fw_connection, pref_only, expect_rv, \
                             expect_ap) \
  STMT_BEGIN \
    spider_addr_port_t chosen_rs_ap; \
    spider_addr_make_null(&chosen_rs_ap.addr, AF_INET); \
    chosen_rs_ap.port = 0; \
    tt_int_op(fascist_firewall_choose_address_rs(&(fake_rs), \
                                                 ...
                                                 &chosen_rs_ap), \
              OP_EQ, (expect_rv)); \
    tt_assert(spider_addr_eq(&(expect_ap).addr, &chosen_rs_ap.addr)); \
...
#define CHECK_CHOSEN_ADDR_NODE(fake_node, fw_connection, pref_only, \
                               expect_rv, expect_ap) \
  STMT_BEGIN \
    spider_addr_port_t chosen_node_ap; \
    spider_addr_make_null(&chosen_node_ap.addr, AF_INET); \
    ...
    tt_int_op(fascist_firewall_choose_address_node(&(fake_node), \
                                                   ...
                                                   &chosen_node_ap), \
              OP_EQ, (expect_rv)); \
    tt_assert(spider_addr_eq(&(expect_ap).addr, &chosen_node_ap.addr)); \
...
{
  (void)arg;
  spider_addr_port_t ipv4_or_ap, ipv4_dir_ap, ipv6_or_ap, ipv6_dir_ap;
  spider_addr_port_t n_ipv4_ap, n_ipv6_ap;
  ...
  /* Parse the addresses */
  spider_addr_parse(&ipv4_or_ap.addr, TEST_IPV4_ADDR_STR);
  ipv4_or_ap.port = TEST_IPV4_OR_PORT;
  spider_addr_parse(&ipv4_dir_ap.addr, TEST_IPV4_ADDR_STR);
  ipv4_dir_ap.port = TEST_IPV4_DIR_PORT;

  spider_addr_parse(&ipv6_or_ap.addr, TEST_IPV6_ADDR_STR);
  ipv6_or_ap.port = TEST_IPV6_OR_PORT;
  spider_addr_parse(&ipv6_dir_ap.addr, TEST_IPV6_ADDR_STR);
  ipv6_dir_ap.port = TEST_IPV6_DIR_PORT;

  spider_addr_make_null(&n_ipv4_ap.addr, AF_INET);
  n_ipv4_ap.port = 0;
  spider_addr_make_null(&n_ipv6_ap.addr, AF_INET6);
  n_ipv6_ap.port = 0;
  ...
  /* Prefer IPv4 but want IPv6 (contradicspidery) */
  tt_assert(fascist_firewall_choose_address(&ipv4_or_ap, &ipv6_or_ap, 0,
                                            FIREWALL_OR_CONNECTION, 0, 0)
            == &ipv4_or_ap);
  tt_assert(fascist_firewall_choose_address(&ipv4_or_ap, &ipv6_or_ap, 0,
  ...
  /* Prefer IPv6 but want IPv4 (contradicspidery) */
  tt_assert(fascist_firewall_choose_address(&ipv4_or_ap, &ipv6_or_ap, 1,
                                            FIREWALL_OR_CONNECTION, 0, 1)
            == &ipv6_or_ap);
  tt_assert(fascist_firewall_choose_address(&ipv4_or_ap, &ipv6_or_ap, 1,
  ...
  fake_rs.addr = spider_addr_to_ipv4h(&ipv4_or_ap.addr);
  fake_rs.or_port = ipv4_or_ap.port;
  fake_rs.dir_port = ipv4_dir_ap.port;

  spider_addr_copy(&fake_rs.ipv6_addr, &ipv6_or_ap.addr);
