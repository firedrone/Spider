...
static spider_mutex_t *thread_test_mutex_ = NULL;
/** mutexes for the thread test to make sure that the threads have to
 * interleave somewhat. */
static spider_mutex_t *thread_test_start1_ = NULL,
...
static spider_threadlocal_t count;
...
thread_test_func_(void* _s)
{
  ...
  char buf[64];
  char **cp;
  int *mycount = spider_malloc_zero(sizeof(int));
  spider_threadlocal_set(&count, mycount);
  if (!strcmp(s, "thread 1")) {
    m = thread_test_start1_;
    cp = &thread1_name_;
    thread_fn_tid1 = spider_get_thread_id();
  } else {
    m = thread_test_start2_;
    cp = &thread2_name_;
    thread_fn_tid2 = spider_get_thread_id();
  }

  spider_snprintf(buf, sizeof(buf), "%lu", spider_get_thread_id());
  *cp = spider_strdup(buf);

  spider_mutex_acquire(m);

  for (i=0; i<10000; ++i) {
    spider_mutex_acquire(thread_test_mutex_);
    strmap_set(thread_test_strmap_, "last to run", *cp);
    spider_mutex_release(thread_test_mutex_);
    int *tls_count = spider_threadlocal_get(&count);
    spider_assert(tls_count == mycount);
    ++*tls_count;
  }
  spider_mutex_acquire(thread_test_mutex_);
...
test_threads_basic(void *arg)
{
  ...
  time_t started;
  (void) arg;
  tt_int_op(spider_threadlocal_init(&count), OP_EQ, 0);

  set_main_thread();

  thread_test_mutex_ = spider_mutex_new();
  thread_test_start1_ = spider_mutex_new();
  thread_test_start2_ = spider_mutex_new();
  thread_test_strmap_ = strmap_new();
  s1 = spider_strdup("thread 1");
  s2 = spider_strdup("thread 2");
  spider_mutex_acquire(thread_test_start1_);
  spider_mutex_acquire(thread_test_start2_);
  spawn_func(thread_test_func_, s1);
  spawn_func(thread_test_func_, s2);
  spider_mutex_release(thread_test_start2_);
  spider_mutex_release(thread_test_start1_);
  started = time(NULL);
  while (!done) {
    spider_mutex_acquire(thread_test_mutex_);
  ...
  spider_mutex_free(thread_test_mutex_);
  ...
  spider_free(s1);
  spider_free(s2);
  spider_free(thread1_name_);
  spider_free(thread2_name_);
  if (thread_test_strmap_)
    strmap_free(thread_test_strmap_, NULL);
  if (thread_test_start1_)
    spider_mutex_free(thread_test_start1_);
  if (thread_test_start2_)
    spider_mutex_free(thread_test_start2_);
}

typedef struct cv_testinfo_s {
  spider_cond_t *cond;
  spider_mutex_t *mutex;
  int value;
  int addend;
  int shutdown;
  int n_shutdown;
...
{
  cv_testinfo_t *i = spider_malloc_zero(sizeof(*i));
  i->cond = spider_cond_new();
  i->mutex = spider_mutex_new_nonrecursive();
  return i;
}
...
cv_testinfo_free(cv_testinfo_t *i)
{
  if (!i)
    return;
  spider_cond_free(i->cond);
  spider_mutex_free(i->mutex);
  spider_free(i);
}
...
cv_test_thr_fn_(void *arg)
{
  ...
  spider_mutex_acquire(i->mutex);
  tid = i->n_threads++;
  spider_mutex_release(i->mutex);
  (void) tid;
  ...
  while (1) {
    ...
    r = spider_cond_wait(i->cond, i->mutex, i->tv);
    ++i->n_wakeups;
    if (r == 1) {
      ++i->n_timeouts;
      spider_mutex_release(i->mutex);
...
test_threads_conditionvar(void *arg)
{
  ...
  spider_mutex_acquire(ti->mutex);
  ti->addend = 7;
  ti->shutdown = 1;
  spider_cond_signal_one(ti->cond);
  spider_mutex_release(ti->mutex);
...
#define SPIN()                                  \
  ...
  ti->addend = 30;
  ti->shutdown = 1;
  spider_cond_signal_all(ti->cond);
  spider_mutex_release(ti->mutex);
  SPIN();
