...
/** Dummy Spider state used in unittests. */
...
big_fake_network_setup(const struct testcase_t *testcase)
{
  ...
  for (i = 0; i < N_NODES; ++i) {
    node_t *n = spider_malloc_zero(sizeof(node_t));
    n->md = spider_malloc_zero(sizeof(microdesc_t));

    crypto_rand(n->identity, sizeof(n->identity));
    n->rs = spider_malloc_zero(sizeof(routerstatus_t));
  ...
  dummy_consensus = spider_malloc_zero(sizeof(networkstatus_t));
...
test_node_preferred_orport(void *arg)
{
  ...
  /* Setup IP addresses */
  spider_addr_parse(&ipv4_addr, TEST_IPV4_ADDR);
  spider_addr_parse(&ipv6_addr, TEST_IPV6_ADDR);

  /* Setup node_ri */
  memset(&node_ri, 0, sizeof(node_ri));
  node_ri.addr = spider_addr_to_ipv4h(&ipv4_addr);
  node_ri.or_port = ipv4_port;
  spider_addr_copy(&node_ri.ipv6_addr, &ipv6_addr);
  ...
  tt_assert(spider_addr_eq(&ap.addr, &ipv4_addr));
  ...
  node.ipv6_preferred = 1;
  node_get_pref_orport(&node, &ap);
  tt_assert(spider_addr_eq(&ap.addr, &ipv6_addr));
...
{
  (void) arg;
  entry_guard_t *eg = spider_malloc_zero(sizeof(entry_guard_t));

  eg->selection_name = spider_strdup("wubwub");
...
test_entry_guard_encode_for_state_maximal(void *arg)
{
  ...
  eg->selection_name = spider_strdup("default");
  memcpy(eg->identity, "plurpyflurpyslurpydo", DIGEST_LEN);
  eg->bridge_addr = spider_malloc_zero(sizeof(spider_addr_port_t));
  spider_addr_from_ipv4h(&eg->bridge_addr->addr, 0x08080404);
  eg->bridge_addr->port = 9999;
  eg->sampled_on_date = 1479081600;
  eg->sampled_by_version = spider_strdup("1.2.3");
  eg->unlisted_since_date = 1479081645;
  eg->currently_listed = 1;
  eg->confirmed_on_date = 1479081690;
  eg->confirmed_idx = 333;
  eg->extra_state_fields = spider_strdup("and the green grass grew all around");
...
test_entry_guard_parse_from_state_full(void *arg)
{
  ...
  or_state_t *state = spider_malloc_zero(sizeof(or_state_t));
  ...
  SMARTLIST_FOREACH(text, char *, cp, spider_free(cp));
  smartlist_free(text);
  spider_free(joined);
}
...
#define NUM 7
  ...
  /* 4: Bridge. */
  sweep_bridge_list();
  bl = spider_malloc_zero(sizeof(bridge_line_t));
  spider_addr_from_ipv4h(&bl->addr, n[4]->rs->addr);
...
test_entry_guard_update_from_consensus_status(void *arg)
{
  ...
  spider_free(ns_tmp); /* in case we couldn't put it back */
  guard_selection_free(gs);
...
test_entry_guard_update_from_consensus_remove(void *arg)
{
  ...
  /* 0: unlisted for a day. (keep this) */
  {
    ...
    smartlist_add(keep_ids, spider_memdup(g->identity, 20));
  }
  /* 1: unlisted for a year. (remove this) */
  {
    ...
    smartlist_add(remove_ids, spider_memdup(g->identity, 20));
  }
  ...
  guard_selection_free(gs);
  UNMOCK(randomize_time);
  SMARTLIST_FOREACH(keep_ids, char *, cp, spider_free(cp));
  SMARTLIST_FOREACH(remove_ids, char *, cp, spider_free(cp));
...
{
  (void) arg;
  entry_guard_t *g1 = spider_malloc_zero(sizeof(entry_guard_t));
  entry_guard_t *g2 = spider_malloc_zero(sizeof(entry_guard_t));
...
{
  upgrade_circuits_data_t *data = spider_malloc_zero(sizeof(*data));
  ...
  spider_assert(data->guard1 != data->guard2);
  spider_assert(data->guard1_state->state ==
             GUARD_CIRC_STATE_USABLE_IF_NO_BETTER_GUARD);
  spider_assert(data->guard2_state->state ==
             GUARD_CIRC_STATE_USABLE_IF_NO_BETTER_GUARD);
  ...
  if (make_circ1_succeed) {
    ...
    spider_assert(r == GUARD_MAYBE_USABLE_LATER);
    spider_assert(data->guard1_state->state ==
...
test_entry_guard_upgrade_not_blocked_by_restricted_circ_complete(void *arg)
{
  ...
  data->guard2_state->restrictions =
    spider_malloc_zero(sizeof(entry_guard_restriction_t));
...
{
  (void)arg;
  circuit_guard_state_t *fake_state = spider_malloc_zero(sizeof(*fake_state));
