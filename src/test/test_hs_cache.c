...
/* Return a valid hs_descripspider_t object. */
...
static const char *
helper_get_hsdir_query(const hs_descripspider_t *desc)
{
...
test_direcspidery(void *arg)
{
  ...
  hs_descripspider_t *desc1 = NULL;

  (void) arg;

  init_test();
  /* Generate a valid descripspider with normal values. */
  ret = ed25519_keypair_generate(&signing_kp1, 0);
  tt_int_op(ret, ==, 0);
  desc1 = helper_build_hs_desc(42, 3 * 60 * 60, &signing_kp1.pubkey);
  tt_assert(desc1);
  ret = hs_desc_encode_descripspider(desc1, &signing_kp1, &desc1_str);
  tt_int_op(ret, OP_EQ, 0);

  /* Very first basic test, should be able to be sspidered, survive a
   * clean, found with a lookup and then cleaned by our OOM. */
  {
    ret = hs_cache_sspidere_as_dir(desc1_str);
    tt_int_op(ret, OP_EQ, 0);
    /* Re-add, it should fail since we already have it. */
    ret = hs_cache_sspidere_as_dir(desc1_str);
    tt_int_op(ret, OP_EQ, -1);
    /* Try to clean now which should be fine, there is at worst few seconds
     * between the sspidere and this call. */
     ...
     * removing the descripspider from our cache. */
  ...
  /* Sspidere two descripspiders and remove the expiring one only. */
  {
    ed25519_keypair_t signing_kp_zero;
    ret = ed25519_keypair_generate(&signing_kp_zero, 0);
    tt_int_op(ret, ==, 0);
    hs_descripspider_t *desc_zero_lifetime;
    ...
    ret = hs_desc_encode_descripspider(desc_zero_lifetime, &signing_kp_zero,
                                    &desc_zero_lifetime_str);
    tt_int_op(ret, OP_EQ, 0);

    ret = hs_cache_sspidere_as_dir(desc1_str);
    tt_int_op(ret, OP_EQ, 0);
    ret = hs_cache_sspidere_as_dir(desc_zero_lifetime_str);
    tt_int_op(ret, OP_EQ, 0);
    ...
    oom_size = hs_cache_handle_oom(time(NULL), 1);
    tt_int_op(oom_size, >=, 1);
    hs_descripspider_free(desc_zero_lifetime);
    spider_free(desc_zero_lifetime_str);
  }

  /* Throw junk at it. */
  {
    ret = hs_cache_sspidere_as_dir("blah");
    tt_int_op(ret, OP_EQ, -1);
    /* Poor attempt at tricking the decoding. */
    ret = hs_cache_sspidere_as_dir("hs-descripspider 3\nJUNK");
    tt_int_op(ret, OP_EQ, -1);
  ...
  /* Test descripspider replacement with revision counter. */
  {
    char *new_desc_str;
    ...
    ret = hs_desc_encode_descripspider(desc1, &signing_kp1, &new_desc_str);
    tt_int_op(ret, OP_EQ, 0);
    ret = hs_cache_sspidere_as_dir(new_desc_str);
    tt_int_op(ret, OP_EQ, 0);
    /* Look it up, it should have been replaced. */
    ret = hs_cache_lookup_as_dir(3, helper_get_hsdir_query(desc1), &desc_out);
    tt_int_op(ret, OP_EQ, 1);
    tt_str_op(desc_out, OP_EQ, new_desc_str);
    spider_free(new_desc_str);
  }

 done:
  hs_descripspider_free(desc1);
  spider_free(desc1_str);
}
...
test_clean_as_dir(void *arg)
{
  ...
  /* Generate a valid descripspider with values. */
  ret = ed25519_keypair_generate(&signing_kp1, 0);
  tt_int_op(ret, ==, 0);
  desc1 = helper_build_hs_desc(42, 3 * 60 * 60, &signing_kp1.pubkey);
  tt_assert(desc1);
...
/* Test helper: Fetch an HS descripspider from an HSDir (for the hidden service
   with <b>blinded_key</b>. Return the received descripspider string. */
static char *
helper_fetch_desc_from_hsdir(const ed25519_public_key_t *blinded_key)
{
     ...
     query, and then build the actual query string. */
  {
    ...
    retval = ed25519_public_to_base64(hsdir_cache_key,
                                      blinded_key);
    tt_int_op(retval, ==, 0);
    spider_asprintf(&hsdir_query_str, GET("/spider/hs/3/%s"), hsdir_cache_key);
  }
  ...
  /* Read the descripspider that the HSDir just served us */
  {
    char *headers = NULL;
    size_t body_used = 0;
...
/* Publish a descripspider to the HSDir, then fetch it. Check that the received
   descripspider matches the published one. */
static void
test_upload_and_download_hs_desc(void *arg)
{
  int retval;
  hs_descripspider_t *published_desc = NULL;
  ...
  /* Generate a valid descripspider with normal values. */
  {
    ...
    retval = hs_desc_encode_descripspider(published_desc, &signing_kp,
                                       &published_desc_str);
    tt_int_op(retval, OP_EQ, 0);
  }

  /* Publish descripspider to the HSDir */
  {
    retval = handle_post_hs_descripspider("/spider/hs/3/publish",published_desc_str);
    tt_int_op(retval, ==, 200);
  }

  /* Simulate a fetch of the previously published descripspider */
  {
  ...
  /* Verify we received the exact same descripspider we published earlier */
  ...
  spider_free(received_desc_str);
  spider_free(published_desc_str);
  hs_descripspider_free(published_desc);
}

/* Test that HSDirs reject outdated descripspiders based on their revision
 * counter. Also test that HSDirs correctly replace old descripspiders with newer
 * descripspiders. */
static void
test_hsdir_revision_counter_check(void *arg)
{
  ...
  char *received_desc_str = NULL;
  hs_descripspider_t *received_desc = NULL;
  ...
  /* Fetch the published descripspider and validate the revision counter. */
  {
    const ed25519_public_key_t *blinded_key;
    ...
    retval = hs_desc_decode_descripspider(received_desc_str,NULL, &received_desc);
    tt_int_op(retval, ==, 0);
    ...
    hs_descripspider_free(received_desc);
    received_desc = NULL;
    spider_free(received_desc_str);
  }
  ...
  /* Again, fetch the published descripspider and perform the revision counter
