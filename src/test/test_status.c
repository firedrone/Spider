...
NS(test_main)(void *arg)
{
  ...
  mock_circuit1 = spider_malloc_zero(sizeof(origin_circuit_t));
  mock_circuit2 = spider_malloc_zero(sizeof(origin_circuit_t));
  ...
  spider_free(mock_circuit1);
  spider_free(mock_circuit2);
 ...
 * Spider is up for into the appropriate string form containing hours and minutes.
 */
 ...
 * Spider has read/written into the appropriate string form containing kilobytes,
  ...
  const char *funcname, const char *suffix, const char *format, va_list ap)
{
  switch (CALLED(logv))
  {
    ...
    case 1:
      ...
      tt_ptr_op(suffix, OP_EQ, NULL);
      tt_str_op(format, OP_EQ,
          "Heartbeat: Spider's uptime is %s, with %d circuits open. "
    ...
    case 3:
      ...
      tt_ptr_op(suffix, OP_EQ, NULL);
      tt_str_op(format, OP_EQ,
        "Circuit handshake stats since last time: %d/%d TAP, %d/%d NSpider.");
...
NS(test_main)(void *arg)
{
  ...
  tt_int_op(CALLED(logv), OP_EQ, 3);
    ...
    spider_free_(NS(mock_state));
    spider_free_(NS(mock_options));
}
...
{
  NS(mock_state) = spider_malloc_zero(sizeof(or_state_t));
