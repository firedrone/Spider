...
static int tlschan_is_local_addr_mock(const spider_addr_t *addr);
...
/* Thing to cast to fake spider_tls_t * to appease assert_connection_ok() */
static int fake_spidertls = 0; /* Bleh... */
...
tlschan_connection_or_connect_mock(const spider_addr_t *addr,
                                   uint16_t port,
                                   const char *digest,
                                   const ed25519_public_key_t *ed_id,
                                   channel_tls_t *tlschan)
{
  ...
  /* Make a fake orconn */
  result = spider_malloc_zero(sizeof(*result));
  ...
  result->base_.address = spider_strdup("<fake>");
  memcpy(&(result->base_.addr), addr, sizeof(spider_addr_t));
  result->base_.port = port;
  memcpy(result->identity_digest, digest, DIGEST_LEN);
  result->chan = tlschan;
  memcpy(&(result->real_addr), addr, sizeof(spider_addr_t));
  result->tls = (spider_tls_t *)((void *)(&fake_spidertls));
...
tlschan_fake_close_method(channel_t *chan)
{
  ...
  spider_free(tlschan->conn->base_.address);
  spider_free(tlschan->conn);
...
static int
tlschan_is_local_addr_mock(const spider_addr_t *addr)
{
  tt_assert(addr != NULL);
