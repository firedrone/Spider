...
test_cfmt_connected_cells(void *arg)
{
  ...
  tt_int_op(r, OP_EQ, 0);
  tt_int_op(spider_addr_family(&addr), OP_EQ, AF_UNSPEC);
  ...
  r = connected_cell_parse(&rh, &cell, &addr, &ttl);
  tt_int_op(r, OP_EQ, 0);
  tt_int_op(spider_addr_family(&addr), OP_EQ, AF_INET);
  ...
  r = connected_cell_parse(&rh, &cell, &addr, &ttl);
  tt_int_op(r, OP_EQ, 0);
  tt_int_op(spider_addr_family(&addr), OP_EQ, AF_INET6);
  ...
  spider_addr_parse(&addr, "30.40.50.60");
  ...
  memset(&rh, 0, sizeof(rh));
  memset(&cell, 0, sizeof(cell));
  spider_addr_parse(&addr, "2620::6b0:b:1a1a:0:26e5:480e");
  rh.length = connected_cell_format_payload(cell.payload+RELAY_HEADER_SIZE,
                                            &addr, 3600);
...
test_cfmt_create_cells(void *arg)
{
  ...
  /* A valid create2 cell with an nspider payload */
  memset(&cell, 0, sizeof(cell));
  memset(b, 0, sizeof(b));
  crypto_rand((char*)b, NTOR_ONIONSKIN_LEN);
  cell.command = CELL_CREATE2;
  memcpy(cell.payload, "\x00\x02\x00\x54", 4); /* nspider, 84 bytes long */
  ...
  /* A valid create cell with an nspider payload, in legacy format. */
  memset(&cell, 0, sizeof(cell));
  memset(b, 0, sizeof(b));
  crypto_rand((char*)b, NTOR_ONIONSKIN_LEN);
  cell.command = CELL_CREATE;
  memcpy(cell.payload, "nspiderNTORnspiderNTOR", 16);
...
test_cfmt_extend_cells(void *arg)
{
  ...
  tt_int_op(AF_UNSPEC, OP_EQ, spider_addr_family(&ec.orport_ipv6.addr));
  ...
  /* Let's do an nspider stuffed in a legacy EXTEND cell */
  memset(p, 0, sizeof(p));
  memset(b, 0, sizeof(b));
  crypto_rand((char*)b, NTOR_ONIONSKIN_LEN);
  memcpy(p, "\x12\xf4\x00\x01\x01\x02", 6); /* 18 244 0 1 : 258 */
  memcpy(p+6,"nspiderNTORnspiderNTOR", 16);
  ...
  /* Now let's do a minimal nspider EXTEND2 cell. */
...
#define SET_CELL(s) do {                                                \
  ...
  tt_int_op(smartlist_len(addrs), OP_EQ, 1);
  a = smartlist_get(addrs, 0);
  tt_assert(spider_addr_is_null(&a->addr));
...
test_cfmt_is_destroy(void *arg)
{
  ...
  chan = spider_malloc_zero(sizeof(channel_t));
