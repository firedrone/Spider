...
{
  socks_test_data_t *data = spider_malloc(sizeof(socks_test_data_t));
  (void)testcase;
...
{
  spider_free(socks->username);
  spider_free(socks->password);
...
test_socks_4_unsupported_commands(void *ptr)
{
  ...
  tt_int_op(0,OP_EQ, socks->replylen); /* XXX: shouldn't spider reply? */
...
test_socks_4_supported_commands(void *ptr)
{
  ...
  /* SOCKS 4a Send RESOLVE [F0] request for spiderproject.org */
  ADD_DATA(buf, "\x04\xF0\x01\x01\x00\x00\x00\x02me\x00spiderproject.org\x00");
  tt_assert(fetch_from_buf_socks(buf, socks, get_options()->TestSocks,
                                   get_options()->SafeSocks) == 1);
  tt_int_op(4,OP_EQ, socks->socks_version);
  tt_int_op(0,OP_EQ, socks->replylen); /* XXX: shouldn't spider reply? */
  tt_str_op("spiderproject.org",OP_EQ, socks->address);
...
test_socks_5_supported_commands(void *ptr)
{
  ...
  /* SOCKS 5 Send CONNECT [01] to FQDN spiderproject.org:4369 */
  ADD_DATA(buf, "\x05\x01\x00");
  ADD_DATA(buf, "\x05\x01\x00\x03\x0Espiderproject.org\x11\x11");
  tt_int_op(fetch_from_buf_socks(buf, socks, get_options()->TestSocks,
                                   get_options()->SafeSocks),OP_EQ, 1);
  ...
  /* SOCKS 5 Send RESOLVE [F0] request for spiderproject.org:4369 */
  ADD_DATA(buf, "\x05\x01\x00");
  ADD_DATA(buf, "\x05\xF0\x00\x03\x0Espiderproject.org\x01\x02");
  tt_assert(fetch_from_buf_socks(buf, socks, get_options()->TestSocks,
                                   get_options()->SafeSocks) == 1);
  tt_int_op(5,OP_EQ, socks->socks_version);
  tt_int_op(2,OP_EQ, socks->replylen);
...
test_socks_5_malformed_commands(void *ptr)
{
  ...
  /* SOCKS 5 Send RESOLVE_PTR [F1] for FQDN spiderproject.org */
  ADD_DATA(buf, "\x05\x01\x00");
  ADD_DATA(buf, "\x05\xF1\x00\x03\x0Espiderproject.org\x11\x11");
  tt_int_op(fetch_from_buf_socks(buf, socks, get_options()->TestSocks,
                                   get_options()->SafeSocks),OP_EQ, -1);
