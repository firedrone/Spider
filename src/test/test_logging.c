...
test_get_sigsafe_err_fds(void *arg)
{
  ...
  n = spider_log_get_sigsafe_err_fds(&fds);
  tt_int_op(n, OP_EQ, 1);
...
test_sigsafe_err(void *arg)
{
  ...
  spider_log_err_sigsafe("Minimal.\n", NULL);

  set_log_time_granularity(100*1000);
  spider_log_err_sigsafe("Testing any ",
  ...
  spider_split_lines(lines, content, (int)strlen(content));
