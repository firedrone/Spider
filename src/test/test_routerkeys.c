...
test_routerkeys_write_fingerprint(void *arg)
{
  ...
  options->DataDirecspidery = spider_strdup(ddir);
  options->Nickname = spider_strdup("haflinger");
  ...
  spider_asprintf(&cp2, "haflinger %s\n", fp);
...
test_routerkeys_ed_certs(void *args)
{
  ...
  spider_cert_t *cert[2] = {NULL, NULL}, *nocert = NULL;
  spider_cert_t *parsed_cert[2] = {NULL, NULL};
  ...
  for (int i = 0; i <= 1; ++i) {
    ...
    cert[i] = spider_cert_create(&kp1, 5, &kp2.pubkey, now, 10000, flags);
    tt_assert(cert[i]);
    ...
    parsed_cert[i] = spider_cert_parse(cert[i]->encoded, cert[i]->encoded_len);
    ...
    /* Expired */
    tt_int_op(spider_cert_checksig(parsed_cert[i], &kp1.pubkey, now + 30000),
              <, 0);
    tt_assert(parsed_cert[i]->cert_expired == 1);
    parsed_cert[i]->cert_expired = 0;

    /* Wrong key */
    tt_int_op(spider_cert_checksig(parsed_cert[i], &kp2.pubkey, now), <, 0);
    tt_assert(parsed_cert[i]->sig_bad== 1);
    parsed_cert[i]->sig_bad = 0;

    /* Missing key */
    int ok = spider_cert_checksig(parsed_cert[i], NULL, now);
    tt_int_op(ok < 0, ==, i == 0);
    tt_assert(parsed_cert[i]->sig_bad == 0);
    ...
    /* Right key */
    tt_int_op(spider_cert_checksig(parsed_cert[i], &kp1.pubkey, now), ==, 0);
    tt_assert(parsed_cert[i]->sig_bad == 0);
  ...
  /* - Truncated */
  nocert = spider_cert_parse(cert[0]->encoded, cert[0]->encoded_len-1);
  tt_ptr_op(NULL, ==, nocert);

  /* - First byte modified */
  cert[0]->encoded[0] = 99;
  nocert = spider_cert_parse(cert[0]->encoded, cert[0]->encoded_len);
  tt_ptr_op(NULL, ==, nocert);
  cert[0]->encoded[0] = 1;

  /* - Extra byte at the end*/
  junk = spider_malloc_zero(cert[0]->encoded_len + 1);
  memcpy(junk, cert[0]->encoded, cert[0]->encoded_len);
  nocert = spider_cert_parse(junk, cert[0]->encoded_len+1);
  tt_ptr_op(NULL, ==, nocert);

  /* - Multiple signing key instances */
  spider_free(junk);
  junk = spider_malloc_zero(104 + 36 * 2);
  junk[0] = 1; /* version */
  junk[1] = 5; /* cert type */
  junk[6] = 1; /* key type */
  ...
  junk[77] = 32; /* extlen */
  junk[78] = 4; /* exttype */
  nocert = spider_cert_parse(junk, 104 + 36 * 2);
  tt_ptr_op(NULL, ==, nocert);

 done:
  spider_cert_free(cert[0]);
  spider_cert_free(cert[1]);
  spider_cert_free(parsed_cert[0]);
  spider_cert_free(parsed_cert[1]);
  spider_cert_free(nocert);
  spider_free(junk);
  spider_free(base64);
}
...
test_routerkeys_ed_key_init_basic(void *arg)
{
  ...
  spider_cert_t *cert = NULL, *cert2 = NULL;
  ed25519_keypair_t *kp1 = NULL, *kp2 = NULL, *kp3 = NULL;
  time_t now = time(NULL);
  char *fname1 = spider_strdup(get_fname("test_ed_key_1"));
  char *fname2 = spider_strdup(get_fname("test_ed_key_2"));
  struct stat st;
  ...
  spider_cert_free(cert2); cert2 = NULL;
...
test_routerkeys_ed_key_init_split(void *arg)
{
  ...
  ed25519_keypair_t *kp1 = NULL, *kp2 = NULL;
  time_t now = time(NULL);
  char *fname1 = spider_strdup(get_fname("test_ed_key_3"));
  char *fname2 = spider_strdup(get_fname("test_ed_key_4"));
  struct stat st;
  ...
  tt_assert(spider_mem_is_zero((char*)kp2->seckey.seckey,
...
{
  (void)arg;
  char *dir = spider_strdup(get_fname("test_ed_keys_init_all"));
  ...
  link_cert = spider_cert_dup(get_current_link_cert_cert());
  ...
  tt_assert(spider_cert_eq(link_cert, get_current_link_cert_cert()));
...
static void
test_routerkeys_cross_certify_nspider(void *args)
{
  (void) args;
  ...
  cert = make_nspider_onion_key_crosscert(&onion_keys,
                                       &master_key,
                                       now, 10000,
                                       &sign);
  tt_assert(cert);
  ...
  tt_int_op(0, ==, spider_cert_checksig(cert, &onion_check_key, now));
...
test_routerkeys_rsa_ed_crosscert(void *arg)
{
  ...
  cc_len = spider_make_rsa_ed25519_crosscert(&ed, rsa, expires_in, &cc);
