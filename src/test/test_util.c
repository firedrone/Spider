 ...
 * changed spider_timegm interface. */
static time_t
spider_timegm_wrapper(const struct tm *tm)
{
  time_t t;
  if (spider_timegm(tm, &t) < 0)
    return -1;
  return t;
}

#define spider_timegm spider_timegm_wrapper
                              ...
                              size_t read_limit)
{
  ...
  fifo_name = spider_strdup(get_fname(fname));
  test_str = spider_malloc(file_len);
  ...
  unlink(fifo_name);
  spider_free(fifo_name);
  spider_free(test_str);
  spider_free(str);
  if (fd >= 0)
    close(fd);
}
...
test_util_read_file_eof_tiny_limit(void *arg)
{
  ...
  test_util_read_until_eof_impl("spider_test_fifo_tiny", 5, 4);
}
...
{
  (void)arg;
  test_util_read_until_eof_impl("spider_test_fifo_1ka", 1024, 1023);
}
...
{
  (void)arg;
  test_util_read_until_eof_impl("spider_test_fifo_1kb", 1024, 1024);
}
...
test_util_read_file_eof_two_loops(void *arg)
{
  ...
  test_util_read_until_eof_impl("spider_test_fifo_2k", 2048, 10000);
}
...
test_util_read_file_eof_two_loops_b(void *arg)
{
  ...
  test_util_read_until_eof_impl("spider_test_fifo_2kb", 2048, 2048);
}
...
test_util_read_file_eof_zero_bytes(void *arg)
{
  (void)arg;
  // zero-byte fifo
  test_util_read_until_eof_impl("spider_test_fifo_empty", 0, 10000);
}
...
test_util_write_chunks_to_file(void *arg)
{
  ...
  char *data_str = spider_malloc(data_str_len);
  char *temp_str = spider_malloc(temp_str_len);
  ...
  */
  fname = spider_strdup(get_fname("write_chunks_with_tempfile"));
  spider_asprintf(&tempname, "%s.tmp", fname);
  ...
  tt_int_op(r, OP_EQ, 0);
  spider_free(fname);
  spider_free(tempname);

  /*
  *  Check if it skips using a tempfile with flags
  */
  fname = spider_strdup(get_fname("write_chunks_with_no_tempfile"));
  spider_asprintf(&tempname, "%s.tmp", fname);
...
#define TOR_USEC_PER_SEC 1000000
  ...
  /* Test spider_timegm & spider_gmtime_r */
   ...
   * Zeroing may not be needed for b_time, as long as spider_gmtime_r
  ...
  a_time.tm_sec = 55;
  t_res = 1062224095UL;
  tt_int_op(t_res, OP_EQ, spider_timegm(&a_time));
  spider_gmtime_r(&t_res, &b_time);
...
#if SIZEOF_TIME_T == 4
  tt_int_op((time_t) -1,OP_EQ, spider_timegm(&a_time));
  ...
  /* Test spider_timegm out of range */
...
#define CHECK_TIMEGM_ARG_OUT_OF_RANGE(msg) \
    CHECK_TIMEGM_WARNING("Out-of-range argument to spider_timegm")
  ...
  /* month */
  a_time.tm_year = 2007-1900;  /* resspidere valid year */
  ...
  /* hour */
  a_time.tm_mday = 3;           /* resspidere valid month day */
  ...
  /* minute */
  a_time.tm_hour = 22;          /* resspidere valid hour */
  ...
  /* second */
  a_time.tm_min = 37;           /* resspidere valid minute */
  ...
  /* Test spider_gmtime_r out of range */
  ...
  spider_gettimeofday(&start);
  /* now make sure time works. */
  spider_gettimeofday(&end);
...
#define T(s) do {                               \
    format_iso_time(b, spider_timegm(&a_time));    \
    tt_str_op(b, OP_EQ, (s));                      \
    b[0]='\0';                                  \
  } while (0)
  ...
  tt_int_op(0,OP_EQ,
            parse_http_time("Wednesday, 04 Aug 1994 00:48:22 GMT", &a_time));
  tt_int_op((time_t)775961302UL,OP_EQ, spider_timegm(&a_time));
...
#elif SIZEOF_TIME_T == 8
  tt_int_op(0,OP_EQ,parse_http_time("Wed, 17 Feb 2038 06:13:20 GMT", &a_time));
  tt_int_op((time_t)2150000000UL,OP_EQ, spider_timegm(&a_time));
...
test_util_config_line_comment_character(void *arg)
{
  ...
  strlcpy(buf, "k1 \"# in quotes\"\n"
          ...
          "k3 /home/user/mySpiderNetwork#2\n"    /* Testcase for #1323 */
          , sizeof(buf));
  str = buf;
  ...
  tt_str_op(str,OP_EQ, "k3 /home/user/mySpiderNetwork#2\n");

#if 0
  str = parse_config_line_from_str_verbose(str, &k, &v, NULL);
  test_streq(k, "k3");
  test_streq(v, "/home/user/mySpiderNetwork#2");
  spider_free(k); spider_free(v);
...
/** Test spider_escape_str_for_pt_args(). */
static void
test_util_escape_string_socks(void *arg)
{
  ...
  (void)arg;
  escaped_string = spider_escape_str_for_pt_args("This is a backslash: \\",";\\");
  tt_assert(escaped_string);
  tt_str_op(escaped_string,OP_EQ, "This is a backslash: \\\\");
  spider_free(escaped_string);

  /** Simple semicolon escape. */
  escaped_string = spider_escape_str_for_pt_args("First rule:Do not use ;",";\\");
  tt_assert(escaped_string);
  tt_str_op(escaped_string,OP_EQ, "First rule:Do not use \\;");
  spider_free(escaped_string);

  /** Empty string. */
  escaped_string = spider_escape_str_for_pt_args("", ";\\");
  tt_assert(escaped_string);
...
test_util_strmisc(void *arg)
{
  ...
  tt_int_op(-1,OP_EQ, spider_snprintf(buf, 0, "Foo"));
  tt_int_op(-1,OP_EQ, spider_snprintf(buf, 2, "Foo"));
  tt_int_op(-1,OP_EQ, spider_snprintf(buf, 3, "Foo"));
  tt_int_op(-1,OP_NE, spider_snprintf(buf, 4, "Foo"));
  /* Always NUL-terminate the output */
  spider_snprintf(buf, 5, "abcdef");
  tt_int_op(0,OP_EQ, buf[4]);
  spider_snprintf(buf, 10, "abcdef");
  tt_int_op(0,OP_EQ, buf[6]);
  /* uint64 */
  spider_snprintf(buf, sizeof(buf), "x!"U64_FORMAT"!x",
  ...
  memset(buf,0,20);
  buf[20] = 'x';
  tt_assert(spider_digest_is_zero(buf));
  buf[19] = 'x';
  tt_assert(!spider_digest_is_zero(buf));
  ...
  memset(buf,0,128);
  buf[128] = 'x';
  tt_assert(spider_mem_is_zero(buf, 10));
  tt_assert(spider_mem_is_zero(buf, 20));
  tt_assert(spider_mem_is_zero(buf, 128));
  tt_assert(!spider_mem_is_zero(buf, 129));
  buf[60] = (char)255;
  tt_assert(!spider_mem_is_zero(buf, 128));
  buf[0] = (char)1;
  tt_assert(!spider_mem_is_zero(buf, 10));
  ...
  cp_tmp = spider_strdup("abcdef");
  tt_assert(spider_strisnonupper(cp_tmp));
  cp_tmp[3] = 'D';
  tt_assert(!spider_strisnonupper(cp_tmp));
  spider_strupper(cp_tmp);
  tt_str_op(cp_tmp,OP_EQ, "ABCDEF");
  spider_strlower(cp_tmp);
  tt_str_op(cp_tmp,OP_EQ, "abcdef");
  tt_assert(spider_strisnonupper(cp_tmp));
  tt_assert(spider_strisprint(cp_tmp));
  cp_tmp[3] = 3;
  tt_assert(!spider_strisprint(cp_tmp));
  spider_free(cp_tmp);
  ...
  /* Test memmem and memstr */
  {
    ...
    tt_assert(!spider_memmem(haystack, 5, "ef", 2));
    tt_ptr_op(spider_memmem(haystack, 5, "cd", 2),OP_EQ, haystack + 2);
    tt_ptr_op(spider_memmem(haystack, 5, "cde", 3),OP_EQ, haystack + 2);
    tt_assert(!spider_memmem(haystack, 4, "cde", 3));
    haystack = "ababcad";
    tt_ptr_op(spider_memmem(haystack, 7, "abc", 3),OP_EQ, haystack + 2);
    tt_ptr_op(spider_memmem(haystack, 7, "ad", 2),OP_EQ, haystack + 5);
    tt_ptr_op(spider_memmem(haystack, 7, "cad", 3),OP_EQ, haystack + 4);
    tt_assert(!spider_memmem(haystack, 7, "dadad", 5));
    tt_assert(!spider_memmem(haystack, 7, "abcdefghij", 10));
    /* memstr */
    tt_ptr_op(spider_memstr(haystack, 7, "abc"),OP_EQ, haystack + 2);
    tt_ptr_op(spider_memstr(haystack, 7, "cad"),OP_EQ, haystack + 4);
    tt_assert(!spider_memstr(haystack, 6, "cad"));
    tt_assert(!spider_memstr(haystack, 7, "cadd"));
    tt_assert(!spider_memstr(haystack, 7, "fe"));
    tt_assert(!spider_memstr(haystack, 7, "ababcade"));
  }
...
test_util_parse_integer(void *arg)
{
  ...
  /* Test parse_long */
  /* Empty/zero input */
  tt_int_op(0L,OP_EQ, spider_parse_long("",10,0,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(0L,OP_EQ, spider_parse_long("0",10,0,100,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  /* Normal cases */
  tt_int_op(10L,OP_EQ, spider_parse_long("10",10,0,100,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  tt_int_op(10L,OP_EQ, spider_parse_long("10",10,0,10,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  tt_int_op(10L,OP_EQ, spider_parse_long("10",10,10,100,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  tt_int_op(-50L,OP_EQ, spider_parse_long("-50",10,-100,100,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  tt_int_op(-50L,OP_EQ, spider_parse_long("-50",10,-100,0,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  tt_int_op(-50L,OP_EQ, spider_parse_long("-50",10,-50,0,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  /* Extra garbage */
  tt_int_op(0L,OP_EQ, spider_parse_long("10m",10,0,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(0L,OP_EQ, spider_parse_long("-50 plus garbage",10,-100,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(10L,OP_EQ, spider_parse_long("10m",10,0,100,&i,&cp));
  tt_int_op(1,OP_EQ, i);
  tt_str_op(cp,OP_EQ, "m");
  tt_int_op(-50L,OP_EQ, spider_parse_long("-50 plus garbage",10,-100,100,&i,&cp));
  tt_int_op(1,OP_EQ, i);
  tt_str_op(cp,OP_EQ, " plus garbage");
  /* Illogical min max */
  spider_capture_bugs_(1);
  tt_int_op(0L,OP_EQ,  spider_parse_long("10",10,50,4,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(1, OP_EQ, smartlist_len(spider_get_captured_bug_log_()));
  tt_str_op("!(max < min)", OP_EQ,
            smartlist_get(spider_get_captured_bug_log_(), 0));
  spider_end_capture_bugs_();
  spider_capture_bugs_(1);
  tt_int_op(0L,OP_EQ,   spider_parse_long("-50",10,100,-100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(1, OP_EQ, smartlist_len(spider_get_captured_bug_log_()));
  tt_str_op("!(max < min)", OP_EQ,
            smartlist_get(spider_get_captured_bug_log_(), 0));
  spider_end_capture_bugs_();
  /* Out of bounds */
  tt_int_op(0L,OP_EQ,  spider_parse_long("10",10,50,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(0L,OP_EQ,   spider_parse_long("-50",10,0,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  /* Base different than 10 */
  tt_int_op(2L,OP_EQ,   spider_parse_long("10",2,0,100,NULL,NULL));
  tt_int_op(0L,OP_EQ,   spider_parse_long("2",2,0,100,NULL,NULL));
  tt_int_op(0L,OP_EQ,   spider_parse_long("10",-2,0,100,NULL,NULL));
  tt_int_op(68284L,OP_EQ, spider_parse_long("10abc",16,0,70000,NULL,NULL));
  tt_int_op(68284L,OP_EQ, spider_parse_long("10ABC",16,0,70000,NULL,NULL));
  tt_int_op(0,OP_EQ, spider_parse_long("10ABC",-1,0,70000,&i,NULL));
  tt_int_op(i,OP_EQ, 0);

  /* Test parse_ulong */
  tt_int_op(0UL,OP_EQ, spider_parse_ulong("",10,0,100,NULL,NULL));
  tt_int_op(0UL,OP_EQ, spider_parse_ulong("0",10,0,100,NULL,NULL));
  tt_int_op(10UL,OP_EQ, spider_parse_ulong("10",10,0,100,NULL,NULL));
  tt_int_op(0UL,OP_EQ, spider_parse_ulong("10",10,50,100,NULL,NULL));
  tt_int_op(10UL,OP_EQ, spider_parse_ulong("10",10,0,10,NULL,NULL));
  tt_int_op(10UL,OP_EQ, spider_parse_ulong("10",10,10,100,NULL,NULL));
  tt_int_op(0UL,OP_EQ, spider_parse_ulong("8",8,0,100,NULL,NULL));
  tt_int_op(50UL,OP_EQ, spider_parse_ulong("50",10,50,100,NULL,NULL));
  tt_int_op(0UL,OP_EQ, spider_parse_ulong("-50",10,0,100,NULL,NULL));
  tt_int_op(0UL,OP_EQ, spider_parse_ulong("50",-1,50,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(0UL,OP_EQ, spider_parse_ulong("-50",10,0,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);

  /* Test parse_uint64 */
  tt_assert(U64_LITERAL(10) == spider_parse_uint64("10 x",10,0,100, &i, &cp));
  tt_int_op(1,OP_EQ, i);
  tt_str_op(cp,OP_EQ, " x");
  tt_assert(U64_LITERAL(12345678901) ==
              spider_parse_uint64("12345678901",10,0,UINT64_MAX, &i, &cp));
  tt_int_op(1,OP_EQ, i);
  tt_str_op(cp,OP_EQ, "");
  tt_assert(U64_LITERAL(0) ==
              spider_parse_uint64("12345678901",10,500,INT32_MAX, &i, &cp));
  tt_int_op(0,OP_EQ, i);
  tt_assert(U64_LITERAL(0) ==
              spider_parse_uint64("123",-1,0,INT32_MAX, &i, &cp));
  tt_int_op(0,OP_EQ, i);

  {
  /* Test parse_double */
  double d = spider_parse_double("10", 0, (double)UINT64_MAX,&i,NULL);
  tt_int_op(1,OP_EQ, i);
  tt_assert(DBL_TO_U64(d) == 10);
  d = spider_parse_double("0", 0, (double)UINT64_MAX,&i,NULL);
  tt_int_op(1,OP_EQ, i);
  tt_assert(DBL_TO_U64(d) == 0);
  d = spider_parse_double(" ", 0, (double)UINT64_MAX,&i,NULL);
  tt_int_op(0,OP_EQ, i);
  d = spider_parse_double(".0a", 0, (double)UINT64_MAX,&i,NULL);
  tt_int_op(0,OP_EQ, i);
  d = spider_parse_double(".0a", 0, (double)UINT64_MAX,&i,&cp);
  tt_int_op(1,OP_EQ, i);
  d = spider_parse_double("-.0", 0, (double)UINT64_MAX,&i,NULL);
  tt_int_op(1,OP_EQ, i);
  tt_assert(DBL_TO_U64(d) == 0);
  d = spider_parse_double("-10", -100.0, 100.0,&i,NULL);
  tt_int_op(1,OP_EQ, i);
  tt_double_op(fabs(d - -10.0),OP_LT, 1E-12);
  }

  {
    /* Test spider_parse_* where we overflow/underflow the underlying type. */
    /* This string should overflow 64-bit ints. */
#define TOOBIG "100000000000000000000000000"
    tt_int_op(0L, OP_EQ,
              spider_parse_long(TOOBIG, 10, LONG_MIN, LONG_MAX, &i, NULL));
    tt_int_op(i,OP_EQ, 0);
    tt_int_op(0L,OP_EQ,
              spider_parse_long("-"TOOBIG, 10, LONG_MIN, LONG_MAX, &i, NULL));
    tt_int_op(i,OP_EQ, 0);
    tt_int_op(0UL,OP_EQ, spider_parse_ulong(TOOBIG, 10, 0, ULONG_MAX, &i, NULL));
    tt_int_op(i,OP_EQ, 0);
    tt_u64_op(U64_LITERAL(0), OP_EQ, spider_parse_uint64(TOOBIG, 10,
                                             0, UINT64_MAX, &i, NULL));
    tt_int_op(i,OP_EQ, 0);
  }
 done:
...
test_util_pow2(void *arg)
{
  /* Test spider_log2(). */
  (void)arg;
  tt_int_op(spider_log2(64),OP_EQ, 6);
  tt_int_op(spider_log2(65),OP_EQ, 6);
  tt_int_op(spider_log2(63),OP_EQ, 5);
  /* incorrect mathematically, but as specified: */
  tt_int_op(spider_log2(0),OP_EQ, 0);
  tt_int_op(spider_log2(1),OP_EQ, 0);
  tt_int_op(spider_log2(2),OP_EQ, 1);
  tt_int_op(spider_log2(3),OP_EQ, 1);
  tt_int_op(spider_log2(4),OP_EQ, 2);
  tt_int_op(spider_log2(5),OP_EQ, 2);
  tt_int_op(spider_log2(U64_LITERAL(40000000000000000)),OP_EQ, 55);
  tt_int_op(spider_log2(UINT64_MAX),OP_EQ, 63);
...
test_util_gzip(void *arg)
{
  ...
  const char *ccp2;
  size_t len1, len2;
  spider_zlib_state_t *state = NULL;

  (void)arg;
  buf1 = spider_strdup("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAZAAAAAAAAAAAAAAAAAAAZ");
  tt_assert(detect_compression_method(buf1, strlen(buf1)) == UNKNOWN_METHOD);

  tt_assert(!spider_gzip_compress(&buf2, &len1, buf1, strlen(buf1)+1,
                               GZIP_METHOD));
  tt_assert(buf2);
  tt_assert(len1 < strlen(buf1));
  tt_assert(detect_compression_method(buf2, len1) == GZIP_METHOD);

  tt_assert(!spider_gzip_uncompress(&buf3, &len2, buf2, len1,
  ...
  spider_free(buf3);
  buf2 = spider_reallocarray(buf2, len1, 2);
  memcpy(buf2+len1, buf2, len1);
  tt_assert(!spider_gzip_uncompress(&buf3, &len2, buf2, len1*2,
                                   ZLIB_METHOD, 1, LOG_INFO));
  ...
  buf1 =
    spider_strdup("String with low redundancy that won't be compressed much.");
  tt_assert(!spider_gzip_compress(&buf2, &len1, buf1, strlen(buf1)+1,
                                 ZLIB_METHOD));
  tt_assert(len1>16);
  /* when we allow an incomplete string, we should succeed.*/
  tt_assert(!spider_gzip_uncompress(&buf3, &len2, buf2, len1-16,
  ...
  spider_free(buf2);
  spider_free(buf3);
  state = spider_zlib_new(1, ZLIB_METHOD, HIGH_COMPRESSION);
  tt_assert(state);
  cp1 = buf1 = spider_malloc(1024);
  len1 = 1024;
  ccp2 = "ABCDEFGHIJABCDEFGHIJ";
  len2 = 21;
  tt_assert(spider_zlib_process(state, &cp1, &len1, &ccp2, &len2, 0)
              == TOR_ZLIB_OK);
  ...
  len2 = 0;
  cp2 = cp1;
  tt_assert(spider_zlib_process(state, &cp1, &len1, &ccp2, &len2, 1)
              == TOR_ZLIB_DONE);
  tt_int_op(0,OP_EQ, len2);
  tt_assert(cp1 > cp2); /* Make sure we really added something. */

  tt_assert(!spider_gzip_uncompress(&buf3, &len2, buf1, 1024-len1,
...
test_util_gzip_compression_bomb(void *arg)
{
   ...
   * one. Most compression formats support them, but they can be a DOS vecspider.
   * In Spider we try not to generate them, and we don't accept them.
   */
  (void) arg;
  size_t one_million = 1<<20;
  char *one_mb = spider_malloc_zero(one_million);
  char *result = NULL;
  size_t result_len = 0;
  spider_zlib_state_t *state = NULL;
  ...
  setup_full_capture_of_logs(LOG_WARN);
  tt_int_op(-1, OP_EQ, spider_gzip_compress(&result, &result_len,
                                         one_mb, one_million,
                                         ZLIB_METHOD));
  expect_single_log_msg_containing(
         "We compressed something and got an insanely high "
         "compression facspider; other Spiders would think this "
         "was a zlib bomb.");
  teardown_capture_of_logs();
  ...
  tt_int_op(-1, OP_EQ, spider_gzip_uncompress(&result, &result_len,
                                           compression_bomb, 1039,
                                           ZLIB_METHOD, 0, LOG_WARN));

  /* Now try streaming that. */
  state = spider_zlib_new(0, ZLIB_METHOD, HIGH_COMPRESSION);
  spider_zlib_output_t r;
  const char *inp = compression_bomb;
  size_t inlen = 1039;
  do {
    char *outp = one_mb;
    size_t outleft = 4096; /* small on purpose */
    r = spider_zlib_process(state, &outp, &outleft, &inp, &inlen, 0);
...
{
  char *fname1 = spider_strdup(get_fname("mapped_1"));
  char *fname2 = spider_strdup(get_fname("mapped_2"));
  char *fname3 = spider_strdup(get_fname("mapped_3"));
  const size_t buflen = 17000;
  char *buf = spider_malloc(17000);
  spider_mmap_t *mapping = NULL;

  (void)arg;
  crypto_rand(buf, buflen);

  mapping = spider_mmap_file(fname1);
  tt_assert(! mapping);
...
#ifdef _WIN32
  tt_int_op(0, OP_EQ, spider_munmap_file(mapping));
  mapping = NULL;
  ...
  mapping = spider_mmap_file(fname2);
  tt_assert(mapping);
  ...
  mapping = spider_mmap_file(fname3);
  tt_assert(mapping);
...
test_util_sscanf(void *arg)
{
  ...
  (void)arg;
  tt_int_op(-1,OP_EQ, spider_sscanf("123", "%i", &r)); /* %i is not supported */
  tt_int_op(-1,OP_EQ,
            spider_sscanf("wrong", "%5c", s1)); /* %c cannot have a number. */
  tt_int_op(-1,OP_EQ, spider_sscanf("hello", "%s", s1)); /* %s needs a number. */
  tt_int_op(-1,OP_EQ, spider_sscanf("prettylongstring", "%999999s", s1));
#if 0
  /* GCC thinks these two are illegal. */
  test_eq(-1, spider_sscanf("prettylongstring", "%0s", s1));
  test_eq(0, spider_sscanf("prettylongstring", "%10s", NULL));
#endif
  /* No '%'-strings: always "success" */
  tt_int_op(0,OP_EQ, spider_sscanf("hello world", "hello world"));
  tt_int_op(0,OP_EQ, spider_sscanf("hello world", "good bye"));
  /* Excess data */
  tt_int_op(0,OP_EQ,
            spider_sscanf("hello 3", "%u", &u1));  /* have to match the start */
  tt_int_op(0,OP_EQ, spider_sscanf(" 3 hello", "%u", &u1));
  tt_int_op(0,OP_EQ,
            spider_sscanf(" 3 hello", "%2u", &u1)); /* not even in this case */
  tt_int_op(1,OP_EQ,
            spider_sscanf("3 hello", "%u", &u1));  /* but trailing is alright */

  /* Numbers (ie. %u) */
  tt_int_op(0,OP_EQ,
            spider_sscanf("hello world 3", "hello worlb %u", &u1)); /* d vs b */
  tt_int_op(1,OP_EQ, spider_sscanf("12345", "%u", &u1));
  tt_int_op(12345u,OP_EQ, u1);
  tt_int_op(1,OP_EQ, spider_sscanf("12346 ", "%u", &u1));
  tt_int_op(12346u,OP_EQ, u1);
  tt_int_op(0,OP_EQ, spider_sscanf(" 12347", "%u", &u1));
  tt_int_op(1,OP_EQ, spider_sscanf(" 12348", " %u", &u1));
  tt_int_op(12348u,OP_EQ, u1);
  tt_int_op(1,OP_EQ, spider_sscanf("0", "%u", &u1));
  tt_int_op(0u,OP_EQ, u1);
  tt_int_op(1,OP_EQ, spider_sscanf("0000", "%u", &u2));
  tt_int_op(0u,OP_EQ, u2);
  tt_int_op(0,OP_EQ, spider_sscanf("", "%u", &u1)); /* absent number */
  tt_int_op(0,OP_EQ, spider_sscanf("A", "%u", &u1)); /* bogus number */
  tt_int_op(0,OP_EQ, spider_sscanf("-1", "%u", &u1)); /* negative number */

  /* Numbers with size (eg. %2u) */
  tt_int_op(0,OP_EQ, spider_sscanf("-1", "%2u", &u1));
  tt_int_op(2,OP_EQ, spider_sscanf("123456", "%2u%u", &u1, &u2));
  tt_int_op(12u,OP_EQ, u1);
  tt_int_op(3456u,OP_EQ, u2);
  tt_int_op(1,OP_EQ, spider_sscanf("123456", "%8u", &u1));
  tt_int_op(123456u,OP_EQ, u1);
  tt_int_op(1,OP_EQ, spider_sscanf("123457  ", "%8u", &u1));
  tt_int_op(123457u,OP_EQ, u1);
  tt_int_op(0,OP_EQ, spider_sscanf("  123456", "%8u", &u1));
  tt_int_op(3,OP_EQ, spider_sscanf("!12:3:456", "!%2u:%2u:%3u", &u1, &u2, &u3));
  tt_int_op(12u,OP_EQ, u1);
  tt_int_op(3u,OP_EQ, u2);
  tt_int_op(456u,OP_EQ, u3);
  tt_int_op(3,OP_EQ,
            spider_sscanf("67:8:099", "%2u:%2u:%3u", &u1, &u2, &u3)); /* 0s */
  tt_int_op(67u,OP_EQ, u1);
  tt_int_op(8u,OP_EQ, u2);
  tt_int_op(99u,OP_EQ, u3);
  /* %u does not match space.*/
  tt_int_op(2,OP_EQ, spider_sscanf("12:3: 45", "%2u:%2u:%3u", &u1, &u2, &u3));
  tt_int_op(12u,OP_EQ, u1);
  tt_int_op(3u,OP_EQ, u2);
  /* %u does not match negative numbers. */
  tt_int_op(2,OP_EQ, spider_sscanf("67:8:-9", "%2u:%2u:%3u", &u1, &u2, &u3));
  tt_int_op(67u,OP_EQ, u1);
  tt_int_op(8u,OP_EQ, u2);
  /* Arbitrary amounts of 0-padding are okay */
  tt_int_op(3,OP_EQ, spider_sscanf("12:03:000000000000000099", "%2u:%2u:%u",
                        &u1, &u2, &u3));
  tt_int_op(12u,OP_EQ, u1);
  tt_int_op(3u,OP_EQ, u2);
  tt_int_op(99u,OP_EQ, u3);
  ...
  /* Hex (ie. %x) */
  tt_int_op(3,OP_EQ,
            spider_sscanf("1234 02aBcdEf ff", "%x %x %x", &u1, &u2, &u3));
  tt_int_op(0x1234,OP_EQ, u1);
  tt_int_op(0x2ABCDEF,OP_EQ, u2);
  tt_int_op(0xFF,OP_EQ, u3);
  /* Width works on %x */
  tt_int_op(3,OP_EQ, spider_sscanf("f00dcafe444", "%4x%4x%u", &u1, &u2, &u3));
  tt_int_op(0xf00d,OP_EQ, u1);
  tt_int_op(0xcafe,OP_EQ, u2);
  tt_int_op(444,OP_EQ, u3);

  /* Literal '%' (ie. '%%') */
  tt_int_op(1,OP_EQ, spider_sscanf("99% fresh", "%3u%% fresh", &u1));
  tt_int_op(99,OP_EQ, u1);
  tt_int_op(0,OP_EQ, spider_sscanf("99 fresh", "%% %3u %s", &u1, s1));
  tt_int_op(1,OP_EQ, spider_sscanf("99 fresh", "%3u%% %s", &u1, s1));
  tt_int_op(2,OP_EQ, spider_sscanf("99 fresh", "%3u %5s %%", &u1, s1));
  tt_int_op(99,OP_EQ, u1);
  tt_str_op(s1,OP_EQ, "fresh");
  tt_int_op(1,OP_EQ, spider_sscanf("% boo", "%% %3s", s1));
  tt_str_op("boo",OP_EQ, s1);

  /* Strings (ie. %s) */
  tt_int_op(2,OP_EQ, spider_sscanf("hello", "%3s%7s", s1, s2));
  tt_str_op(s1,OP_EQ, "hel");
  tt_str_op(s2,OP_EQ, "lo");
  tt_int_op(2,OP_EQ, spider_sscanf("WD40", "%2s%u", s3, &u1)); /* %s%u */
  tt_str_op(s3,OP_EQ, "WD");
  tt_int_op(40,OP_EQ, u1);
  tt_int_op(2,OP_EQ, spider_sscanf("WD40", "%3s%u", s3, &u1)); /* %s%u */
  tt_str_op(s3,OP_EQ, "WD4");
  tt_int_op(0,OP_EQ, u1);
  tt_int_op(2,OP_EQ, spider_sscanf("76trombones", "%6u%9s", &u1, s1)); /* %u%s */
  tt_int_op(76,OP_EQ, u1);
  tt_str_op(s1,OP_EQ, "trombones");
  tt_int_op(1,OP_EQ, spider_sscanf("prettylongstring", "%999s", s1));
  tt_str_op(s1,OP_EQ, "prettylongstring");
  /* %s doesn't eat spaces */
  tt_int_op(2,OP_EQ, spider_sscanf("hello world", "%9s %9s", s1, s2));
  tt_str_op(s1,OP_EQ, "hello");
  tt_str_op(s2,OP_EQ, "world");
  tt_int_op(2,OP_EQ, spider_sscanf("bye   world?", "%9s %9s", s1, s2));
  tt_str_op(s1,OP_EQ, "bye");
  tt_str_op(s2,OP_EQ, "");
  tt_int_op(3,OP_EQ,
            spider_sscanf("hi", "%9s%9s%3s", s1, s2, s3)); /* %s can be empty. */
  tt_str_op(s1,OP_EQ, "hi");
  tt_str_op(s2,OP_EQ, "");
  tt_str_op(s3,OP_EQ, "");

  tt_int_op(3,OP_EQ, spider_sscanf("1.2.3", "%u.%u.%u%c", &u1, &u2, &u3, &ch));
  tt_int_op(4,OP_EQ,
            spider_sscanf("1.2.3 foobar", "%u.%u.%u%c", &u1, &u2, &u3, &ch));
  tt_int_op(' ',OP_EQ, ch);

  r = spider_sscanf("12345 -67890 -1", "%d %ld %d", &int1, &lng1, &int2);
  tt_int_op(r,OP_EQ, 3);
  ...
  tt_int_op(1,OP_EQ, spider_sscanf("4294967295", "%u", &u1));
  tt_int_op(4294967295U,OP_EQ, u1);

  /* But UINT32_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, spider_sscanf("4294967296", "%u", &u1));
  /* but parsing only 9... */
  tt_int_op(1,OP_EQ, spider_sscanf("4294967296", "%9u", &u1));
  tt_int_op(429496729U,OP_EQ, u1);

  /* %x */
  /* UINT32_MAX should work */
  tt_int_op(1,OP_EQ, spider_sscanf("FFFFFFFF", "%x", &u1));
  tt_int_op(0xFFFFFFFF,OP_EQ, u1);

  /* But UINT32_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, spider_sscanf("100000000", "%x", &u1));

  /* %d */
  /* INT32_MIN and INT32_MAX should work */
  r = spider_sscanf("-2147483648. 2147483647.", "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 2);
  tt_int_op(int1,OP_EQ, -2147483647 - 1);
  tt_int_op(int2,OP_EQ, 2147483647);

  /* But INT32_MIN - 1 and INT32_MAX + 1 shouldn't work */
  r = spider_sscanf("-2147483649.", "%d.", &int1);
  tt_int_op(r,OP_EQ, 0);

  r = spider_sscanf("2147483648.", "%d.", &int1);
  tt_int_op(r,OP_EQ, 0);

  /* and the first failure stops further processing */
  r = spider_sscanf("-2147483648. 2147483648.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 1);

  r = spider_sscanf("-2147483649. 2147483647.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 0);

  r = spider_sscanf("2147483648. -2147483649.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 0);
#elif SIZEOF_INT == 8
  /* %u */
  /* UINT64_MAX should work */
  tt_int_op(1,OP_EQ, spider_sscanf("18446744073709551615", "%u", &u1));
  tt_int_op(18446744073709551615U,OP_EQ, u1);

  /* But UINT64_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, spider_sscanf("18446744073709551616", "%u", &u1));
  /* but parsing only 19... */
  tt_int_op(1,OP_EQ, spider_sscanf("18446744073709551616", "%19u", &u1));
  tt_int_op(1844674407370955161U,OP_EQ, u1);

  /* %x */
  /* UINT64_MAX should work */
  tt_int_op(1,OP_EQ, spider_sscanf("FFFFFFFFFFFFFFFF", "%x", &u1));
  tt_int_op(0xFFFFFFFFFFFFFFFF,OP_EQ, u1);

  /* But UINT64_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, spider_sscanf("10000000000000000", "%x", &u1));

  /* %d */
  /* INT64_MIN and INT64_MAX should work */
  r = spider_sscanf("-9223372036854775808. 9223372036854775807.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 2);
  ...
  r = spider_sscanf("-9223372036854775809.", "%d.", &int1);
  tt_int_op(r,OP_EQ, 0);

  r = spider_sscanf("9223372036854775808.", "%d.", &int1);
  tt_int_op(r,OP_EQ, 0);

  /* and the first failure stops further processing */
  r = spider_sscanf("-9223372036854775808. 9223372036854775808.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 1);

  r = spider_sscanf("-9223372036854775809. 9223372036854775807.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 0);

  r = spider_sscanf("9223372036854775808. -9223372036854775809.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 0);
#endif
  ...
  /* %lu */
  /* UINT32_MAX should work */
  tt_int_op(1,OP_EQ, spider_sscanf("4294967295", "%lu", &ulng));
  tt_int_op(4294967295UL,OP_EQ, ulng);

  /* But UINT32_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, spider_sscanf("4294967296", "%lu", &ulng));
  /* but parsing only 9... */
  tt_int_op(1,OP_EQ, spider_sscanf("4294967296", "%9lu", &ulng));
  tt_int_op(429496729UL,OP_EQ, ulng);

  /* %lx */
  /* UINT32_MAX should work */
  tt_int_op(1,OP_EQ, spider_sscanf("FFFFFFFF", "%lx", &ulng));
  tt_int_op(0xFFFFFFFFUL,OP_EQ, ulng);

  /* But UINT32_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, spider_sscanf("100000000", "%lx", &ulng));

  /* %ld */
  /* INT32_MIN and INT32_MAX should work */
  r = spider_sscanf("-2147483648. 2147483647.", "%ld. %ld.", &lng1, &lng2);
  tt_int_op(r,OP_EQ, 2);
  tt_int_op(lng1,OP_EQ, -2147483647L - 1L);
  tt_int_op(lng2,OP_EQ, 2147483647L);

  /* But INT32_MIN - 1 and INT32_MAX + 1 shouldn't work */
  r = spider_sscanf("-2147483649.", "%ld.", &lng1);
  tt_int_op(r,OP_EQ, 0);

  r = spider_sscanf("2147483648.", "%ld.", &lng1);
  tt_int_op(r,OP_EQ, 0);
  ...
  /* %lu */
  /* UINT64_MAX should work */
  tt_int_op(1,OP_EQ, spider_sscanf("18446744073709551615", "%lu", &ulng));
  tt_int_op(18446744073709551615UL,OP_EQ, ulng);

  /* But UINT64_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, spider_sscanf("18446744073709551616", "%lu", &ulng));
  /* but parsing only 19... */
  tt_int_op(1,OP_EQ, spider_sscanf("18446744073709551616", "%19lu", &ulng));
  tt_int_op(1844674407370955161UL,OP_EQ, ulng);

  /* %lx */
  /* UINT64_MAX should work */
  tt_int_op(1,OP_EQ, spider_sscanf("FFFFFFFFFFFFFFFF", "%lx", &ulng));
  tt_int_op(0xFFFFFFFFFFFFFFFFUL,OP_EQ, ulng);

  /* But UINT64_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, spider_sscanf("10000000000000000", "%lx", &ulng));
  ...
  /* But INT64_MIN - 1 and INT64_MAX + 1 shouldn't work */
  r = spider_sscanf("-9223372036854775809.", "%ld.", &lng1);
  tt_int_op(r,OP_EQ, 0);

  r = spider_sscanf("9223372036854775808.", "%ld.", &lng1);
  tt_int_op(r,OP_EQ, 0);
  ...
  r = spider_sscanf("123.456 .000007 -900123123.2000787 00003.2",
...
{
  /* use the same sized buffer and integers as spider uses */
#define DBUF_SIZE 64
  char dbuf[DBUF_SIZE];
#define T_        "%ld"
...
#define TL_       T_ " " L_
  ...
  r = spider_sscanf(dbuf, TL_, &sec, label_s);
  tt_int_op(r,OP_EQ, 2);
  ...
  tt_int_op(strnlen(dbuf, DBUF_SIZE),OP_LE, DBUF_SIZE - 1);
  r = spider_sscanf(dbuf, TL_, &min, label_m);
  tt_int_op(r,OP_EQ, 2);
  ...
  tt_int_op(strnlen(dbuf, DBUF_SIZE),OP_LE, DBUF_SIZE - 1);
  r = spider_sscanf(dbuf, TL_, &hour, label_h);
  tt_int_op(r,OP_EQ, 2);
  ...
  tt_int_op(strnlen(dbuf, DBUF_SIZE),OP_LE, DBUF_SIZE - 1);
  r = spider_sscanf(dbuf, TL_, &day, label_d);
  tt_int_op(r,OP_EQ, 2);
  ...
  format_time_interval(dbuf, sizeof(dbuf), 24*60*60 + 60);
  tt_int_op(strnlen(dbuf, DBUF_SIZE),OP_LE, DBUF_SIZE - 1);
  r = spider_sscanf(dbuf, TL_ " " TL_ " " TL_,
...
/** Run unittests for memory area allocaspider */
static void
...
test_util_datadir(void *arg)
{
  ...
  spider_snprintf(buf, sizeof(buf), "%s"PATH_SEPARATOR"state", temp_dir);
  ...
  spider_snprintf(buf, sizeof(buf), "%s"PATH_SEPARATOR"cache.foo",
               temp_dir);
  tt_str_op(f,OP_EQ, buf);

 done:
  spider_free(f);
  spider_free(temp_dir);
}
...
test_util_strtok(void *arg)
{
  ...
  for (i = 0; i < 3; i++) {
    ...
    spider_snprintf(buf, sizeof(buf), "%s", pad1);
    spider_snprintf(buf2, sizeof(buf2), "%s", pad2);
    tt_assert(NULL == spider_strtok_r_impl(buf, " ", &cp1));
    tt_assert(NULL == spider_strtok_r_impl(buf2, ".!..;!", &cp2));

    spider_snprintf(buf, sizeof(buf),
                 "%sGraved on the dark  in gestures of descent%s", pad1, pad1);
    spider_snprintf(buf2, sizeof(buf2),
                "%sthey.seemed;;their!.own;most.perfect;monument%s",pad2,pad2);
    /*  -- "Year's End", Richard Wilbur */

    tt_str_op("Graved",OP_EQ, spider_strtok_r_impl(buf, " ", &cp1));
    tt_str_op("they",OP_EQ, spider_strtok_r_impl(buf2, ".!..;!", &cp2));
#define S1() spider_strtok_r_impl(NULL, " ", &cp1)
#define S2() spider_strtok_r_impl(NULL, ".!..;!", &cp2)
  ...
  buf[0] = 0;
  tt_ptr_op(NULL,OP_EQ, spider_strtok_r_impl(buf, " ", &cp1));
  tt_ptr_op(NULL,OP_EQ, spider_strtok_r_impl(buf, "!", &cp1));

  strlcpy(buf, "Howdy!", sizeof(buf));
  tt_str_op("Howdy",OP_EQ, spider_strtok_r_impl(buf, "!", &cp1));
  tt_ptr_op(NULL,OP_EQ, spider_strtok_r_impl(NULL, "!", &cp1));
  ...
  tt_str_op("something",OP_EQ, spider_strtok_r_impl(buf, " ", &cp1));
  tt_ptr_op(NULL,OP_EQ, spider_strtok_r_impl(NULL, ";", &cp1));
 done:
  ;
}
...
#define LOREMIPSUM                                              \
  ...
  /* simple string */
  r = spider_asprintf(&cp, "simple string 100%% safe");
  tt_assert(cp);
  ...
  /* numbers (%i) */
  r = spider_asprintf(&cp, "I like numbers-%2i, %i, etc.", -1, 2);
  tt_assert(cp);
  ...
  /* numbers (%d) */
  r = spider_asprintf(&cp2, "First=%d, Second=%d", 101, 202);
  tt_assert(cp2);
  ...
  r = spider_asprintf(&cp, "Lorem1: %sLorem2: %s", LOREMIPSUM, LOREMIPSUM);
  tt_assert(cp);
  ...
  r = spider_asprintf(&cp, "1: %s 2: %s 3: %s",
...
test_util_listdir(void *ptr)
{
  ...
  dir_contents = spider_listdir(dirname);
  ...
  spider_free(dirname);
  if (dir_contents) {
    SMARTLIST_FOREACH(dir_contents, char *, cp, spider_free(cp));
...
test_util_ftruncate(void *ptr)
{
  ...
  tt_int_op((int)spider_fd_getpos(fd), OP_EQ, strlen(message2));
  tt_int_op(0, OP_EQ, fstat(fd, &st));
...
test_util_join_win_cmdline(void *ptr)
{
   ...
   * MSDN but we don't exercise all quoting rules because spider_join_win_cmdline
  ...
  for (i=0; cmdlines[i]!=NULL; i++) {
    ...
    joined_argv = spider_join_win_cmdline(argvs[i]);
    tt_str_op(cmdlines[i],OP_EQ, joined_argv);
    spider_free(joined_argv);
  }
...
test_util_split_lines(void *ptr)
{
  ...
  for (i=0; tests[i].orig_line; i++) {
    ...
    orig_line = spider_memdup(tests[i].orig_line, tests[i].orig_length + 1);
    spider_split_lines(sl, orig_line, tests[i].orig_length);
...
#define EQ 0
  ...
  for (i = 0; examples[i].a; ++i) {
    ...
    eq1 = spider_memeq(examples[i].a, examples[i].b, len);
    eq2 = spider_memeq(examples[i].b, examples[i].a, len);
    neq1 = spider_memneq(examples[i].a, examples[i].b, len);
    neq2 = spider_memneq(examples[i].b, examples[i].a, len);
    cmp1 = spider_memcmp(examples[i].a, examples[i].b, len);
    cmp2 = spider_memcmp(examples[i].b, examples[i].a, len);
...

  {
    ...
    /* exhaustively test spider_memeq and spider_memcmp
    ...
    for (z = 0; z < 256; z++) {
      for (i = 0; i < 256; i++) {
        ii = (uint8_t)i;
        zz = (uint8_t)z;
        tt_int_op(spider_memeq(&zz, &ii, 1),OP_EQ, zz == ii);
        tt_int_op(spider_memcmp(&zz, &ii, 1) > 0 ? GT : EQ,OP_EQ,
                  zz > ii ? GT : EQ);
        tt_int_op(spider_memcmp(&ii, &zz, 1) < 0 ? LT : EQ,OP_EQ,
                  ii < zz ? LT : EQ);
      }
    }
  }
...
test_util_di_map(void *arg)
{
  ...
  char *str1 = spider_strdup("You are precisely as big as what you love"
                          " and precisely as small as what you allow"
                          " to annoy you.");
  char *str2 = spider_strdup("Let us hope that Lysenko's success in Russia will"
  ...
  char *str3 = spider_strdup("Don't write naughty words on walls "
                          "if you can't spell.");
...
test_util_sl_new_from_text_lines(void *ptr)
{
  ...
  { /* Normal usage */
    ...
    SMARTLIST_FOREACH(sl, void *, x, spider_free(x));
    smartlist_free(sl);
  }
...
test_util_make_environment(void *ptr)
{
  ...
  tt_want(spider_memeq(expected_windows_env_block, env->windows_environment_block,
  ...
  SMARTLIST_FOREACH(env_vars, char *, x, spider_free(x));
...
test_util_set_env_var_in_sl(void *ptr)
{
  ...
  /* Clean up. */
  SMARTLIST_FOREACH(merged_env_vars, char *, x, spider_free(x));
  smartlist_free(merged_env_vars);

  smartlist_free(new_env_vars);

  SMARTLIST_FOREACH(expected_resulting_env_vars, char *, x, spider_free(x));
...
{
  int i, j, n[16];
  spider_weak_rng_t rng;
  (void) arg;

  spider_init_weak_random(&rng, (unsigned)time(NULL));

  for (i = 1; i <= 256; ++i) {
    for (j=0;j<100;++j) {
      int r = spider_weak_random_range(&rng, i);
      tt_int_op(0, OP_LE, r);
      tt_int_op(r, OP_LT, i);
    }
  }
  ...
  memset(n,0,sizeof(n));
  for (j=0;j<8192;++j) {
    n[spider_weak_random_range(&rng, 16)]++;
  }
...
static int
fd_is_cloexec(spider_socket_t fd)
{
...
static int
fd_is_nonblocking(spider_socket_t fd)
{
...
#define SOCK_ERR_IS_EPROTO(s) ERRNO_IS_EPROTO(spider_socket_errno(s))

/* Test for spider_open_socket*, using IPv4 or IPv6 depending on arg. */
static void
test_util_socket(void *arg)
{
  const int domain = !strcmp(arg, "4") ? AF_INET : AF_INET6;
  spider_socket_t fd1 = TOR_INVALID_SOCKET;
  spider_socket_t fd2 = TOR_INVALID_SOCKET;
  spider_socket_t fd3 = TOR_INVALID_SOCKET;
  spider_socket_t fd4 = TOR_INVALID_SOCKET;
  ...
  fd1 = spider_open_socket_with_extensions(domain, SOCK_STREAM, 0, 0, 0);
  int err = spider_socket_errno(fd1);
  ...
  fd2 = spider_open_socket_with_extensions(domain, SOCK_STREAM, 0, 0, 1);
  tt_assert(SOCKET_OK(fd1));
  tt_assert(SOCKET_OK(fd2));
  tt_int_op(get_n_open_sockets(), OP_EQ, n + 2);
  //fd3 = spider_open_socket_with_extensions(domain, SOCK_STREAM, 0, 1, 0);
  //fd4 = spider_open_socket_with_extensions(domain, SOCK_STREAM, 0, 1, 1);
  fd3 = spider_open_socket(domain, SOCK_STREAM, 0);
  fd4 = spider_open_socket_nonblocking(domain, SOCK_STREAM, 0);
  ...
  spider_assert(spider_close_socket == spider_close_socket__real);

  /* we use close_socket__real here so that coverity can tell that we are
   * really closing these sockets. */
  spider_close_socket__real(fd1);
  spider_close_socket__real(fd2);
  fd1 = fd2 = TOR_INVALID_SOCKET;
  tt_int_op(get_n_open_sockets(), OP_EQ, n + 2);
  spider_close_socket__real(fd3);
  spider_close_socket__real(fd4);
...
{
  spider_socket_t s;
  s = spider_open_socket(family, SOCK_STREAM, IPPROTO_TCP);
  spider_assert(SOCKET_OK(s));
 ...
 * the latter is a tolerably good way to exersize spider_accept_socket(). */
static void
test_util_socketpair(void *arg)
{
  const int ersatz = !strcmp(arg, "1");
  int (*const spider_socketpair_fn)(int, int, int, spider_socket_t[2]) =
    ersatz ? spider_ersatz_socketpair : spider_socketpair;
  int n = get_n_open_sockets();
  spider_socket_t fds[2] = {TOR_INVALID_SOCKET, TOR_INVALID_SOCKET};
  const int family = AF_UNIX;
  int socketpair_result = 0;

  socketpair_result = spider_socketpair_fn(family, SOCK_STREAM, 0, fds);

#ifdef __FreeBSD__
  /* If there is no 127.0.0.1, spider_ersatz_socketpair will and must fail.
  ...
  if (SOCKET_OK(fds[0]))
    spider_close_socket(fds[0]);
  if (SOCKET_OK(fds[1]))
    spider_close_socket(fds[1]);
}
...
test_util_hostname_validation(void *arg)
{
  ...
  tt_assert(string_is_valid_hostname("spiderproject.org"));
...
test_util_writepid(void *arg)
{
  ...
  int n = spider_sscanf(contents, "%lu\n%c", &pid, &c);
  tt_int_op(n, OP_EQ, 1);
...
test_util_pwdb(void *arg)
{
  ...
  me = spider_getpwuid(getuid());
  tt_assert(me != NULL);
  name = spider_strdup(me->pw_name);

  /* Uncached case */
  me2 = spider_getpwnam(name);
  tt_assert(me2 != NULL);
  tt_int_op(me2->pw_uid, OP_EQ, getuid());

  /* Cached case */
  me3 = spider_getpwuid(getuid());
  tt_assert(me3 != NULL);
  tt_str_op(me3->pw_name, OP_EQ, name);

  me3 = spider_getpwnam(name);
  tt_assert(me3 != NULL);
  ...
  for (i = 0; i < 100; ++i) {
    ...
    if (spider_getpwnam(badname) == NULL) {
      found = 1;
      break;
    }
  }
...
test_util_monotonic_time_ratchet(void *arg)
{
  ...
  /* win32, GetTickCounts32 ratchet-and-rollover-detecspider. */
...
test_util_htonll(void *arg)
{
  ...
  tt_u64_op(0, OP_EQ, spider_htonll(0));
  tt_u64_op(0, OP_EQ, spider_ntohll(0));
  tt_u64_op(UINT64_MAX, OP_EQ, spider_htonll(UINT64_MAX));
  tt_u64_op(UINT64_MAX, OP_EQ, spider_ntohll(UINT64_MAX));

#ifdef WORDS_BIGENDIAN
  tt_u64_op(res_be, OP_EQ, spider_htonll(0x8877665544332211));
  tt_u64_op(res_be, OP_EQ, spider_ntohll(0x8877665544332211));
#else
  tt_u64_op(res_le, OP_EQ, spider_htonll(0x8877665544332211));
  tt_u64_op(res_le, OP_EQ, spider_ntohll(0x8877665544332211));
#endif
