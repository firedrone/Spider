                       ...
                       struct sockaddr_in6 *sockaddr2)
{
   return ((sockaddr1->sin6_family == sockaddr2->sin6_family) &&
           ...
           (spider_memeq(sockaddr1->sin6_addr.s6_addr,
...
sockaddr_in_from_string(const char *ip_str, struct sockaddr_in *out)
{
  ...
  if (!out)
    out = spider_malloc_zero(sizeof(struct sockaddr_in));

  out->sin_family = AF_INET;
  out->sin_port = 0;
  spider_inet_pton(AF_INET,ip_str,&(out->sin_addr));

  return out;
}

/** Return 1 iff <b>smartlist</b> contains a spider_addr_t structure
 * that is an IPv4 or IPv6 localhost address. Otherwise, return 0.
 */
static int
smartlist_contains_localhost_spider_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, spider_addr_t *, spider_addr) {
    if (spider_addr_is_loopback(spider_addr)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(spider_addr);
 ...
 */
static int
smartlist_contains_multicast_spider_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, spider_addr_t *, spider_addr) {
    if (spider_addr_is_multicast(spider_addr)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(spider_addr);
 ...
 */
static int
smartlist_contains_internal_spider_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, spider_addr_t *, spider_addr) {
    if (spider_addr_is_internal(spider_addr, 0)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(spider_addr);
...
/** Return 1 iff <b>smartlist</b> contains a spider_addr_t structure
 * that is NULL or the null spider_addr_t. Otherwise, return 0.
 */
static int
smartlist_contains_null_spider_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, spider_addr_t *, spider_addr) {
    if (spider_addr == NULL || spider_addr_is_null(spider_addr)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(spider_addr);
 ...
 */
static int
smartlist_contains_ipv4_spider_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, spider_addr_t *, spider_addr) {
    if (spider_addr_is_v4(spider_addr)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(spider_addr);
 ...
 */
static int
smartlist_contains_ipv6_spider_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, spider_addr_t *, spider_addr) {
    /* Since there's no spider_addr_is_v6, assume all non-v4s are v6 */
    if (!spider_addr_is_v4(spider_addr)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(spider_addr);
...
test_address_ifaddrs_to_smartlist(void *arg)
{
   ...
   spider_addr_t *spider_addr = NULL;
   ...
   ipv6_sockaddr = spider_malloc(sizeof(struct sockaddr_in6));
   ipv6_sockaddr->sin6_family = AF_INET6;
   ipv6_sockaddr->sin6_port = 0;
   spider_inet_pton(AF_INET6, "2001:db8:8714:3a90::12",
                 &(ipv6_sockaddr->sin6_addr));

   ifa = spider_malloc(sizeof(struct ifaddrs));
   ifa_ipv4 = spider_malloc(sizeof(struct ifaddrs));
   ifa_ipv6 = spider_malloc(sizeof(struct ifaddrs));

   ifa->ifa_next = ifa_ipv4;
   ifa->ifa_name = spider_strdup("eth0");
   ...
   ifa_ipv4->ifa_name = spider_strdup("eth1");
   ...
   ifa_ipv6->ifa_name = spider_strdup("eth2");
   ...
   sockaddr_to_check = spider_malloc(sizeof(struct sockaddr_in6));

   spider_addr = smartlist_get(smartlist,0);
   addr_len =
   spider_addr_to_sockaddr(spider_addr,0,sockaddr_to_check,
   ...
   spider_addr = smartlist_get(smartlist,1);
   addr_len =
   spider_addr_to_sockaddr(spider_addr,0,sockaddr_to_check,
                        sizeof(struct sockaddr_in));
   ...
   spider_addr = smartlist_get(smartlist,2);
   addr_len =
   spider_addr_to_sockaddr(spider_addr,0,sockaddr_to_check,
   ...
   spider_free(netmask_slash8);
   spider_free(ipv4_sockaddr_local);
   spider_free(ipv4_sockaddr_remote);
   spider_free(ipv6_sockaddr);
   spider_free(ifa->ifa_name);
   spider_free(ifa_ipv4->ifa_name);
   spider_free(ifa_ipv6->ifa_name);
   spider_free(ifa);
   spider_free(ifa_ipv4);
   spider_free(ifa_ipv6);
   spider_free(sockaddr_to_check);
   if (smartlist) {
     SMARTLIST_FOREACH(smartlist, spider_addr_t *, t, spider_free(t));
...
test_address_get_if_addrs_ifaddrs(void *arg)
{
   ...
   * all. */
  tt_assert(!smartlist_contains_null_spider_addr(results));

  /* If there are addresses, they must be IPv4 or IPv6 */
  if (smartlist_len(results) > 0) {
    tt_assert(smartlist_contains_ipv4_spider_addr(results)
              || smartlist_contains_ipv6_spider_addr(results));
  }

  done:
  if (results) {
    SMARTLIST_FOREACH(results, spider_addr_t *, t, spider_free(t));
  }
...
test_address_get_if_addrs_win32(void *arg)
{
  ...
  tt_assert(smartlist_contains_localhost_spider_addr(results));
  tt_assert(!smartlist_contains_null_spider_addr(results));
...
test_address_ip_adapter_addresses_to_smartlist(void *arg)
{
  ...
  sockaddr_to_check = spider_malloc_zero(sizeof(struct sockaddr_in));

  addrs1 =
  spider_malloc_zero(sizeof(IP_ADAPTER_ADDRESSES));

  addrs1->FirstUnicastAddress =
  unicast11 = spider_malloc_zero(sizeof(IP_ADAPTER_UNICAST_ADDRESS));
  ...
  spider_malloc_zero(sizeof(IP_ADAPTER_UNICAST_ADDRESS));
  ...
  unicast21 = spider_malloc_zero(sizeof(IP_ADAPTER_UNICAST_ADDRESS));
  ...
  spider_addr = smartlist_get(result,0);

  spider_addr_to_sockaddr(spider_addr,0,(struct sockaddr *)sockaddr_to_check,
                       sizeof(struct sockaddr_in));

  tt_assert(sockaddr_in_are_equal(sockaddr_test1,sockaddr_to_check));

  spider_addr = smartlist_get(result,1);
  ...
  spider_addr = smartlist_get(result,2);
  ...
  SMARTLIST_FOREACH(result, spider_addr_t *, t, spider_free(t));
  smartlist_free(result);
  spider_free(addrs1);
  spider_free(addrs2);
  spider_free(unicast11->Address.lpSockaddr);
  spider_free(unicast11);
  spider_free(unicast12->Address.lpSockaddr);
  spider_free(unicast12);
  spider_free(unicast21->Address.lpSockaddr);
  spider_free(unicast21);
  spider_free(sockaddr_to_check);
  return;
}
#endif
...
{
  smartlist_t *results = NULL;
  const spider_addr_t *spider_addr = NULL;
  ...
  sockaddr_to_check = spider_malloc(sizeof(struct sockaddr_in));

  ifr = spider_malloc(sizeof(struct ifreq));
  ...
  ifc = spider_malloc(sizeof(struct ifconf));
  ...
  spider_addr = smartlist_get(results, 0);
  addr_len =
  spider_addr_to_sockaddr(spider_addr,0,(struct sockaddr *)sockaddr_to_check,
                       sizeof(struct sockaddr_in));
  ...
  ifr = spider_realloc(ifr,2*sizeof(struct ifreq));
  ifr_next = ifr+1;
  ...
  spider_addr = smartlist_get(results, 1);
  addr_len =
  spider_addr_to_sockaddr(spider_addr,0,(struct sockaddr *)sockaddr_to_check,
                       sizeof(struct sockaddr_in));
...
test_address_get_if_addrs_ioctl(void *arg)
{
  ...
  */
   ...
   * all. */
  if (result) {
    tt_assert(!smartlist_contains_null_spider_addr(result));

    /* If there are addresses, they must be IPv4 or IPv6.
     * (AIX supports IPv6 from SIOCGIFCONF.) */
    if (smartlist_len(result) > 0) {
      tt_assert(smartlist_contains_ipv4_spider_addr(result)
                || smartlist_contains_ipv6_spider_addr(result));
    }
  }
...
static int
fake_close_socket(spider_socket_t s)
{
  (void)s;
  return 0;
}
...
static spider_socket_t
pretend_to_connect(spider_socket_t sock, const struct sockaddr *address,
...
static int
fake_getsockname(spider_socket_t sock, struct sockaddr *address,
                 socklen_t *address_len)
{
...
{
  int hack_retval;
  spider_addr_t *addr_from_hack = spider_malloc_zero(sizeof(spider_addr_t));
  struct sockaddr_in6 *mock_addr6;
  struct sockaddr_in6 *ipv6_to_check =
  spider_malloc_zero(sizeof(struct sockaddr_in6));

  (void)arg;

  MOCK(spider_open_socket,fake_open_socket);
  MOCK(spider_connect_socket,pretend_to_connect);
  ...
  spider_inet_pton(AF_INET6,"2001:cdba::3257:9652",&(mock_addr6->sin6_addr));
  ...
  spider_addr_to_sockaddr(addr_from_hack,0,(struct sockaddr *)ipv6_to_check,
                       sizeof(struct sockaddr_in6));

  tt_assert(sockaddr_in6_are_equal(mock_addr6,ipv6_to_check));

 done:
  UNMOCK(spider_open_socket);
  UNMOCK(spider_connect_socket);
  UNMOCK(spider_getsockname);
  UNMOCK(spider_close_socket);

  spider_free(ipv6_to_check);
  spider_free(mock_addr);
  spider_free(addr_from_hack);
  return;
}
...
test_address_udp_socket_trick_blackbox(void *arg)
{
  ...
  spider_addr_t addr4;
  spider_addr_t addr4_to_check;
  spider_addr_t addr6;
  spider_addr_t addr6_to_check;
  ...
  tt_assert( (retval == -1 && retval_reference == -1) ||
             (spider_addr_compare(&addr4,&addr4_to_check,CMP_EXACT) == 0) );
  ...
  tt_int_op(retval,==,retval_reference);
  tt_assert( (retval == -1 && retval_reference == -1) ||
             (spider_addr_compare(&addr6,&addr6_to_check,CMP_EXACT) == 0) );
...
test_address_get_if_addrs_list_internal(void *arg)
{
  ...
  tt_assert(!smartlist_contains_localhost_spider_addr(results));
  tt_assert(!smartlist_contains_multicast_spider_addr(results));
  ...
  tt_assert(!smartlist_contains_ipv6_spider_addr(results));
...
test_address_get_if_addrs_list_no_internal(void *arg)
{
  ...
  tt_assert(!smartlist_contains_localhost_spider_addr(results));
  tt_assert(!smartlist_contains_multicast_spider_addr(results));
  tt_assert(!smartlist_contains_internal_spider_addr(results));
  tt_assert(!smartlist_contains_null_spider_addr(results));
...
test_address_get_if_addrs_internal_fail(void *arg)
{
  ...
  memset(&ipv6_addr, 0, sizeof(spider_addr_t));
...
test_address_get_if_addrs(void *arg)
{
   ...
   * Unit tests shouldn't fail because of this. */
  if (rv == 0) {
    spider_addr_from_ipv4h(&spider_addr, addr_h);

    tt_assert(!spider_addr_is_loopback(&spider_addr));
    tt_assert(!spider_addr_is_multicast(&spider_addr));
    /* The address may or may not be an internal address */

    tt_assert(spider_addr_is_v4(&spider_addr));
  }
...
test_address_get_if_addrs6(void *arg)
{
  ...
  rv = get_interface_address6(LOG_ERR, AF_INET6, &spider_addr);
...
static void
test_address_spider_addr_to_in6(void *ignored)
{
  (void)ignored;
...
static void
test_address_spider_addr_to_in(void *ignored)
{
  (void)ignored;
  spider_addr_t *a = spider_malloc_zero(sizeof(spider_addr_t));
...
static void
test_address_spider_addr_to_ipv4n(void *ignored)
{
  (void)ignored;
  spider_addr_t *a = spider_malloc_zero(sizeof(spider_addr_t));
  uint32_t res;
...
static void
test_address_spider_addr_to_mapped_ipv4h(void *ignored)
{
  (void)ignored;
  spider_addr_t *a = spider_malloc_zero(sizeof(spider_addr_t));
  uint32_t res;
  ...
  a->family = AF_INET;
  res = spider_addr_to_mapped_ipv4h(a);
  tt_assert(!res);
...
static void
test_address_spider_addr_eq_ipv4h(void *ignored)
{
  (void)ignored;
  spider_addr_t *a = spider_malloc_zero(sizeof(spider_addr_t));
  int res;
  ...
  a->addr.in_addr.s_addr = 52;
  res = spider_addr_eq_ipv4h(a, ntohl(52));
  tt_assert(res);
...
#endif
  ADDRESS_TEST(spider_addr_to_in6, 0),
  ADDRESS_TEST(spider_addr_to_in, 0),
  ADDRESS_TEST(spider_addr_to_ipv4n, 0),
  ADDRESS_TEST(spider_addr_to_mapped_ipv4h, 0),
  ADDRESS_TEST(spider_addr_eq_ipv4h, 0),
  END_OF_TESTCASES
};
