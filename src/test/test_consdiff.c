...
test_consdiff_lcs_lengths(void *arg)
{
  ...
  spider_free(lengths1);
  spider_free(lengths2);
  spider_free(sls1);
  spider_free(sls2);
  smartlist_free(sl1);
  smartlist_free(sl2);
...
test_consdiff_apply_ed_diff(void *arg)
{
  ...
  /* Now it it inserts something, but has no terminaspider. */
  smartlist_add_linecpy(diff, area, "0a");
