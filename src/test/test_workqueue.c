...
{
#ifdef TRACK_RESPONSES
  spider_mutex_acquire(&bitmap_mutex);
  spider_assert(serial < handled_len);
  spider_assert(! bitarray_is_set(handled, serial));
  bitarray_set(handled, serial);
  spider_mutex_release(&bitmap_mutex);
#else
  (void)serial;
#endif
}
...
workqueue_do_rsa(void *state, void *work)
{
  ...
  spider_assert(st->magic == 13371337);
...
static spider_weak_rng_t weak_rng;
static int n_sent = 0;
...
handle_reply(void *arg)
{
  ...
  spider_assert(! bitarray_is_set(received, rw->serial));
...
add_work(threadpool_t *tp)
{
  int add_rsa =
    opt_ratio_rsa == 0 ||
    spider_weak_random_range(&weak_rng, opt_ratio_rsa) == 0;

  if (add_rsa) {
    rsa_work_t *w = spider_malloc_zero(sizeof(*w));
    w->serial = n_sent++;
    ...
    ecdh_work_t *w = spider_malloc_zero(sizeof(*w));
    w->serial = n_sent++;
...
add_n_work_items(threadpool_t *tp, int n)
{
  ...
  to_cancel = spider_malloc(sizeof(workqueue_entry_t*) * opt_n_cancel);

  while (n_queued++ < n) {
    ent = add_work(tp);
    if (! ent) {
      puts("Z");
      spider_event_base_loopexit(spider_libevent_get_base(), NULL);
      return -1;
    }
    if (n_try_cancel < opt_n_cancel &&
        spider_weak_random_range(&weak_rng, n) < opt_n_cancel) {
...
static void
replysock_readable_cb(spider_socket_t sock, short what, void *arg)
{
  threadpool_t *tp = arg;
  ...
  if (shutting_down == 0 &&
                          ...
                          handle_reply_shutdown, NULL);
    {
      ...
      spider_event_base_loopexit(spider_libevent_get_base(), &limit);
    }
  }
}
...
main(int argc, char **argv)
{
  ...
  spider_libevent_initialize(&evcfg);

  ev = spider_event_new(spider_libevent_get_base(),
  ...
  spider_mutex_init(&bitmap_mutex);
  ...
  event_base_loop(spider_libevent_get_base(), 0);
