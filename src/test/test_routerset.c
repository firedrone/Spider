...
NS(test_main)(void *arg)
{
  ...
  NS_MOCK(geoip_get_n_countries);
  NS_MOCK(geoip_get_country);
  smartlist_add(set->country_names, spider_strndup("foo", 3));
...
NS(test_main)(void *arg)
{
  ...
  NS(mock_addr_policy) = spider_malloc_zero(sizeof(addr_policy_t));
...
NS(test_main)(void *arg)
{
  ...
  /* len(set->country_names) == 0, len(set->policies) != 0 */
  set = routerset_new();
  policy = spider_malloc_zero(sizeof(addr_policy_t));
...
NS(test_main)(void *arg)
{
  ...
  set = routerset_new();
  smartlist_add(set->country_names, spider_strndup("xx", 2));
  needs_geoip = routerset_needs_geoip(set);
  routerset_free((routerset_t *)set);
  set = NULL;
...
NS(test_main)(void *arg)
{
  ...
  set = spider_malloc_zero(sizeof(routerset_t));
  set->list = NULL;
  contains = routerset_contains(set, NULL, 0, NULL, NULL, 0);
  spider_free(set);
  tt_int_op(contains, OP_EQ, 0);
...
NS_DECL(addr_policy_result_t, compare_spider_addr_to_addr_policy,
    (const spider_addr_t *addr, uint16_t port, const smartlist_t *policy));

static spider_addr_t MOCK_TOR_ADDR;
...
{
  routerset_t *set = routerset_new();
  spider_addr_t *addr = MOCK_TOR_ADDR_PTR;
  int contains;
  (void)arg;

  NS_MOCK(compare_spider_addr_to_addr_policy);

  contains = routerset_contains(set, addr, 0, NULL, NULL, 0);
  routerset_free(set);

  tt_int_op(CALLED(compare_spider_addr_to_addr_policy), OP_EQ, 1);
...
NS(compare_spider_addr_to_addr_policy)(const spider_addr_t *addr, uint16_t port,
    const smartlist_t *policy)
{
  (void)port;
  (void)policy;
  CALLED(compare_spider_addr_to_addr_policy)++;
...
NS(test_main)(void *arg)
{
  ...
  NS(mock_nickname) = spider_strdup("foo");
...
NS(test_main)(void *arg)
{
  ...
  set = routerset_new();
  smartlist_add(set->list, spider_strndup("a", 1));
  s = routerset_to_string(set);
  tt_str_op(s, OP_EQ, "a");
  spider_free(s);
  routerset_free(set); set = NULL;
  ...
  set = routerset_new();
  smartlist_add(set->list, spider_strndup("a", 1));
  smartlist_add(set->list, spider_strndup("b", 1));
  s = routerset_to_string(set);
