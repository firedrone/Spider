              ...
              int speclen, const int keylen, int legacy)
{
  ...
  tt_assert(!spider_mem_is_zero((char*)buf2+keylen, sizeof(buf2)-keylen));
...
static void
test_crypto_scrypt_vecspiders(void *arg)
{
...
static void
test_crypto_pbkdf2_vecspiders(void *arg)
{
  char *mem_op_hex_tmp = NULL;
  uint8_t spec[64], out[64];
  (void)arg;

  /* Test vecspiders from RFC6070, section 2 */
  base16_decode((char*)spec, sizeof(spec),
                "73616c74" "00" , 10);
  memset(out, 0x00, sizeof(out));
  tt_int_op(20, OP_EQ,
...
struct testcase_t slow_crypto_tests[] = {
  ...
  { "scrypt_vecspiders", test_crypto_scrypt_vecspiders, 0, NULL, NULL },
  { "pbkdf2_vecspiders", test_crypto_pbkdf2_vecspiders, 0, NULL, NULL },
