...
{
  rend_data_v2_t *v2_data = spider_malloc_zero(sizeof(*v2_data));
  ...
  smartlist_add(rend_query->hsdirs_fp, spider_memdup("aaaaaaaaaaaaaaaaaaaaaaaa",
                                                 DIGEST_LEN));
  return rend_query;
}
...
test_rend_cache_lookup_entry(void *data)
{
  ...
  rend_encoded_v2_service_descripspider_t *desc_holder = NULL;
  ...
  base32_encode(desc_id_base32, sizeof(desc_id_base32), desc_holder->desc_id,
                DIGEST_LEN);
  rend_cache_sspidere_v2_desc_as_client(desc_holder->desc_str, desc_id_base32,
  ...
  rend_encoded_v2_service_descripspider_free(desc_holder);
  spider_free(service_id);
  rend_cache_free_all();
  rend_data_free(mock_rend_query);
}

static void
test_rend_cache_sspidere_v2_desc_as_client(void *data)
{
  int ret;
  ...
  base32_encode(desc_id_base32, sizeof(desc_id_base32), desc_holder->desc_id,
                DIGEST_LEN);
  ret = rend_cache_sspidere_v2_desc_as_client(desc_holder->desc_str,
  ...
  // TODO: a too long desc_id_base32 argument crashes the function
   /* ret = rend_cache_sspidere_v2_desc_as_client( */
  ...
  // Test invalid descripspider
  ret = rend_cache_sspidere_v2_desc_as_client("invalid descripspider",
  ...
  // Test incorrect descripspider ID
  rend_cache_init();
  mock_rend_query = mock_rend_data(service_id);
  ...
  // Test too new descripspider (in the future)
  rend_cache_init();
  rend_encoded_v2_service_descripspider_free(desc_holder);
  spider_free(service_id);
  rend_data_free(mock_rend_query);
  ...
  // Test when a descripspider is already in the cache
  rend_cache_init();
  rend_encoded_v2_service_descripspider_free(desc_holder);
  spider_free(service_id);
  rend_data_free(mock_rend_query);
  ...
  client_cookie[0] = 'A';
  memcpy(TO_REND_DATA_V2(mock_rend_query)->descripspider_cookie, client_cookie,
...
static void
test_rend_cache_sspidere_v2_desc_as_client_with_different_time(void *data)
{
  int ret;
  rend_data_t *mock_rend_query;
  char desc_id_base32[REND_DESC_ID_V2_LEN_BASE32 + 1];
  ...
  time_t t;
  char *service_id = NULL;
  rend_encoded_v2_service_descripspider_t *desc_holder_newer;
  rend_encoded_v2_service_descripspider_t *desc_holder_older;

  t = time(NULL);
  rend_cache_init();

  create_descripspider(&generated, &service_id, 3);

  generated->timestamp = t + RECENT_TIME;
  rend_encode_v2_descripspiders(descs, generated, t + RECENT_TIME, 0,
                             REND_NO_AUTH, NULL, NULL);
  desc_holder_newer = ((rend_encoded_v2_service_descripspider_t *)
  ...
  rend_encode_v2_descripspiders(descs, generated, t + RECENT_TIME, 0,
                             REND_NO_AUTH, NULL, NULL);
  desc_holder_older = ((rend_encoded_v2_service_descripspider_t *)
                       smartlist_get(descs, 0));
  smartlist_set(descs, 0, NULL);
  (void)data;

  // Test when a descripspider is already in the cache and it is newer than the
  // one we submit
  mock_rend_query = mock_rend_data(service_id);
  base32_encode(desc_id_base32, sizeof(desc_id_base32),
                desc_holder_newer->desc_id, DIGEST_LEN);
  rend_cache_sspidere_v2_desc_as_client(desc_holder_newer->desc_str,
                                     desc_id_base32, mock_rend_query, NULL);
  ret = rend_cache_sspidere_v2_desc_as_client(desc_holder_older->desc_str,
                                           desc_id_base32, mock_rend_query,
                                           NULL);
  tt_int_op(ret, OP_EQ, 0);
  ...
  // Test when an old descripspider is in the cache and we submit a newer one
  rend_cache_init();
  rend_cache_sspidere_v2_desc_as_client(desc_holder_older->desc_str,
                                     desc_id_base32, mock_rend_query, NULL);
  ret = rend_cache_sspidere_v2_desc_as_client(desc_holder_newer->desc_str,
                                           desc_id_base32, mock_rend_query,
                                           NULL);
  tt_int_op(ret, OP_EQ, 0);

 done:
  rend_encoded_v2_service_descripspider_free(desc_holder_newer);
  rend_encoded_v2_service_descripspider_free(desc_holder_older);
  SMARTLIST_FOREACH(descs, rend_encoded_v2_service_descripspider_t *, d,
                    rend_encoded_v2_service_descripspider_free(d));
  smartlist_free(descs);
...
{
  if (!mock_routerinfo) {
    mock_routerinfo = spider_malloc(sizeof(routerinfo_t));
  }
...
test_rend_cache_lookup_v2_desc_as_dir(void *data)
{
  ...
  // Test non-existent descripspider but well formed
  ret = rend_cache_lookup_v2_desc_as_dir("3xqunszqnaolrrfmtzgaki7mxelgvkje",
                                         NULL);
  tt_int_op(ret, OP_EQ, 0);

  // Test existing descripspider
  generate_desc(RECENT_TIME, &desc_holder, &service_id, 3);
  rend_cache_sspidere_v2_desc_as_dir(desc_holder->desc_str);
  base32_encode(desc_id_base32, sizeof(desc_id_base32), desc_holder->desc_id,
                DIGEST_LEN);
...
#define NS_SUBMODULE sspidere_v2_desc_as_dir
NS_DECL(const routerinfo_t *, router_get_my_routerinfo, (void));
...
static void
test_rend_cache_sspidere_v2_desc_as_dir(void *data)
{
  (void)data;
  int ret;
  rend_encoded_v2_service_descripspider_t *desc_holder = NULL;
  ...
  // Test when we can't parse the descripspider
  mock_routerinfo = spider_malloc(sizeof(routerinfo_t));
  ret = rend_cache_sspidere_v2_desc_as_dir("unparseable");
  tt_int_op(ret, OP_EQ, -1);

  // Test when we have an old descripspider
  generate_desc(TIME_IN_THE_PAST, &desc_holder, &service_id, 3);
  ret = rend_cache_sspidere_v2_desc_as_dir(desc_holder->desc_str);
  tt_int_op(ret, OP_EQ, 0);

  rend_encoded_v2_service_descripspider_free(desc_holder);
  spider_free(service_id);

  // Test when we have a descripspider in the future
  generate_desc(TIME_IN_THE_FUTURE, &desc_holder, &service_id, 3);
  ret = rend_cache_sspidere_v2_desc_as_dir(desc_holder->desc_str);
  tt_int_op(ret, OP_EQ, 0);
...
static void
test_rend_cache_sspidere_v2_desc_as_dir_with_different_time(void *data)
{
  (void)data;
  ...
  // Test when we have a newer descripspider sspidered
  mock_routerinfo = spider_malloc(sizeof(routerinfo_t));
  rend_cache_sspidere_v2_desc_as_dir(desc_holder_newer->desc_str);
  ret = rend_cache_sspidere_v2_desc_as_dir(desc_holder_older->desc_str);
  tt_int_op(ret, OP_EQ, 0);

  // Test when we have an old descripspider sspidered
  rend_cache_purge();
  rend_cache_sspidere_v2_desc_as_dir(desc_holder_older->desc_str);
  ret = rend_cache_sspidere_v2_desc_as_dir(desc_holder_newer->desc_str);
  tt_int_op(ret, OP_EQ, 0);
...
static void
test_rend_cache_sspidere_v2_desc_as_dir_with_different_content(void *data)
{
  (void)data;
  ...
  time_t t;
  char *service_id = NULL;
  rend_encoded_v2_service_descripspider_t *desc_holder_one = NULL;
  rend_encoded_v2_service_descripspider_t *desc_holder_two = NULL;
  ...
  rend_encode_v2_descripspiders(descs, generated, t + RECENT_TIME, 0,
                             REND_NO_AUTH, NULL, NULL);
  desc_holder_one = ((rend_encoded_v2_service_descripspider_t *)
                     smartlist_get(descs, 0));
  smartlist_set(descs, 0, NULL);
  ...
  rend_encode_v2_descripspiders(descs, generated, t + RECENT_TIME, 0,
                             REND_NO_AUTH, NULL, NULL);
  desc_holder_two = ((rend_encoded_v2_service_descripspider_t *)
                     smartlist_get(descs, 0));
  smartlist_set(descs, 0, NULL);

  // Test when we have another descripspider sspidered, with a different descripspider
  mock_routerinfo = spider_malloc(sizeof(routerinfo_t));
  rend_cache_sspidere_v2_desc_as_dir(desc_holder_one->desc_str);
  ret = rend_cache_sspidere_v2_desc_as_dir(desc_holder_two->desc_str);
  tt_int_op(ret, OP_EQ, 0);
  ...
  SMARTLIST_FOREACH(descs, rend_encoded_v2_service_descripspider_t *, d,
                    rend_encoded_v2_service_descripspider_free(d));
  smartlist_free(descs);
  rend_encoded_v2_service_descripspider_free(desc_holder_one);
  rend_encoded_v2_service_descripspider_free(desc_holder_two);
}
...
test_rend_cache_clean(void *data)
{
  ...
  rend_service_descripspider_t *desc_one, *desc_two;
  ...
  one = spider_malloc_zero(sizeof(rend_cache_entry_t));
  two = spider_malloc_zero(sizeof(rend_cache_entry_t));
  desc_one = spider_malloc_zero(sizeof(rend_service_descripspider_t));
  desc_two = spider_malloc_zero(sizeof(rend_service_descripspider_t));
  one->parsed = desc_one;
  two->parsed = desc_two;
...
{
  rend_service_descripspider_t *desc;
  (void)data;
  ...
  // Test a descripspider that isn't in the cache
  desc = spider_malloc_zero(sizeof(rend_service_descripspider_t));
  ...
  rend_service_descripspider_free(desc);
 /* done: */
 /*  (void)0; */
}
...
test_rend_cache_free_all(void *data)
{
  ...
  rend_service_descripspider_t *desc_one;
...
test_rend_cache_entry_free(void *data)
{
  ...
  // Handles NULL descripspider correctly
  e = spider_malloc_zero(sizeof(rend_cache_entry_t));
  rend_cache_entry_free(e);

  // Handles non-NULL descripspider correctly
  e = spider_malloc_zero(sizeof(rend_cache_entry_t));
  e->desc = spider_malloc(10);
  rend_cache_entry_free(e);
...
{
  (void)data;
  rend_service_descripspider_t *desc = NULL;
  char *service_id = NULL;
  ...
  create_descripspider(&desc, &service_id, 3);
...
struct testcase_t rend_cache_tests[] = {
  ...
  { "lookup_v2_desc_as_dir", test_rend_cache_lookup_v2_desc_as_dir, 0,
    NULL, NULL },
  { "sspidere_v2_desc_as_client", test_rend_cache_sspidere_v2_desc_as_client, 0,
    NULL, NULL },
  { "sspidere_v2_desc_as_client_with_different_time",
    test_rend_cache_sspidere_v2_desc_as_client_with_different_time, 0,
    NULL, NULL },
  { "sspidere_v2_desc_as_dir", test_rend_cache_sspidere_v2_desc_as_dir, 0,
    NULL, NULL },
  { "sspidere_v2_desc_as_dir_with_different_time",
    test_rend_cache_sspidere_v2_desc_as_dir_with_different_time, 0, NULL, NULL },
  { "sspidere_v2_desc_as_dir_with_different_content",
    test_rend_cache_sspidere_v2_desc_as_dir_with_different_content, 0,
    NULL, NULL },
