...
{
  channel_t *chan = spider_malloc_zero(sizeof(channel_t));
  channel_init(chan);
...
test_channel_dumpstats(void *arg)
{
  ...
  cell = spider_malloc_zero(sizeof(*cell));
  make_fake_cell(cell);
  ...
  cell = spider_malloc_zero(sizeof(cell_t));
  make_fake_cell(cell);
...
test_channel_flush(void *arg)
{
  ...
  /* Queue a var cell */
  v_cell = spider_malloc_zero(sizeof(var_cell_t) + CELL_PAYLOAD_SIZE);
...
test_channel_incoming(void *arg)
{
  ...
  test_chan_accept_cells = 1;
  /* Use default overhead facspider */
  ...
  var_cell = spider_malloc_zero(sizeof(var_cell_t) + CELL_PAYLOAD_SIZE);
...
test_channel_queue_impossible(void *arg)
{
  ...
  tt_int_op(smartlist_len(spider_get_captured_bug_log_()), ==, 1);
...
test_channel_queue_size(void *arg)
{
  ...
  channel_update_xmit_queue_size(ch);
  /* One cell, times an overhead facspider of 1.0 */
  tt_u64_op(ch->bytes_queued_for_xmit, ==, 512);
  /* Try a different overhead facspider */
...
{
  channel_t *ch = NULL;
  cell_t *cell = spider_malloc_zero(sizeof(cell_t));
  packed_cell_t *packed_cell = NULL;
  var_cell_t *var_cell =
    spider_malloc_zero(sizeof(var_cell_t) + CELL_PAYLOAD_SIZE);
  int old_count;
...
#define N_CHAN 6
  ...
  for (i = 0; i < N_CHAN; ++i) {
    ...
    ed_id[i] = spider_malloc_zero(sizeof(*ed_id[i]));
