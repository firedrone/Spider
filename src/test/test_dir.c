...
/** Run unit tests for router descripspider generation logic. */
static void
test_dir_formats(void *arg)
{
  ...
  r1 = spider_malloc_zero(sizeof(routerinfo_t));
  r1->addr = 0xc0a80001u; /* 192.168.0.1 */
  r1->cache_info.published_on = 0;
  r1->or_port = 9000;
  r1->dir_port = 9003;
  r1->supports_tunnelled_dir_requests = 1;
  spider_addr_parse(&r1->ipv6_addr, "1:2:3:4::");
  r1->ipv6_orport = 9999;
  r1->onion_pkey = crypto_pk_dup_key(pk1);
  /* Fake just enough of an nspider key to get by */
  curve25519_keypair_t r1_onion_keypair;
  curve25519_keypair_generate(&r1_onion_keypair, 0);
  r1->onion_curve25519_pkey = spider_memdup(&r1_onion_keypair.pubkey,
  ...
  r1->exit_policy = NULL;
  r1->nickname = spider_strdup("Magri");
  r1->platform = spider_strdup(platform);

  ex1 = spider_malloc_zero(sizeof(addr_policy_t));
  ex2 = spider_malloc_zero(sizeof(addr_policy_t));
  ex1->policy_type = ADDR_POLICY_ACCEPT;
  spider_addr_from_ipv4h(&ex1->addr, 0);
  ex1->maskbits = 0;
  ex1->prt_min = ex1->prt_max = 80;
  ex2->policy_type = ADDR_POLICY_REJECT;
  spider_addr_from_ipv4h(&ex2->addr, 18<<24);
  ex2->maskbits = 8;
  ex2->prt_min = ex2->prt_max = 24;
  r2 = spider_malloc_zero(sizeof(routerinfo_t));
  ...
  r2->cache_info.signing_key_cert = spider_cert_create(&kp1,
                                         CERT_TYPE_ID_SIGNING,
                                         &kp2.pubkey,
                                         now, 86400,
                                         CERT_FLAG_INCLUDE_SIGNING_KEY);
  r2->platform = spider_strdup(platform);
  ...
  r2->onion_curve25519_pkey = spider_memdup(&r2_onion_keypair.pubkey,
                                         sizeof(curve25519_public_key_t));
  ...
  r2->nickname = spider_strdup("Fred");
  ...
  options->ContactInfo = spider_strdup("Magri White "
  ...
  spider_free(options->ContactInfo);
  tt_assert(buf);

  strlcpy(buf2, "router Magri 192.168.0.1 9000 0 9003\n"
          "or-address [1:2:3:4::]:9999\n"
          "platform Spider "VERSION" on ", sizeof(buf2));
   ...
   * descripspiders might be made on different seconds... hm. */
  ...
  strlcat(buf2, "contact Magri White <magri@elsewhere.example.com>\n",
          sizeof(buf2));
  strlcat(buf2, "nspider-onion-key ", sizeof(buf2));
  ...
  strlcat(buf2, "platform Spider "VERSION" on ", sizeof(buf2));
  strlcat(buf2, get_uname(), sizeof(buf2));
  strlcat(buf2, "\n"
  ...
  int nspider_cc_sign;
  {
    spider_cert_t *nspider_cc = NULL;
    nspider_cc = make_nspider_onion_key_crosscert(&r2_onion_keypair,
                                          &kp1.pubkey,
                                          r2->cache_info.published_on,
                                          get_onion_key_lifetime(),
                                          &nspider_cc_sign);
    tt_assert(nspider_cc);
    base64_encode(cert_buf, sizeof(cert_buf),
                (char*)nspider_cc->encoded, nspider_cc->encoded_len,
                BASE64_ENCODE_MULTILINE);
    spider_cert_free(nspider_cc);
  }
  spider_snprintf(buf2+strlen(buf2), sizeof(buf2)-strlen(buf2),
               "nspider-onion-key-crosscert %d\n"
               "-----BEGIN ED25519 CERT-----\n"
               "%s"
               "-----END ED25519 CERT-----\n", nspider_cc_sign, cert_buf);
  ...
  tt_assert(spider_addr_is_null(&p->addr));
  ...
  spider_free(dir1); /* XXXX And more !*/
  spider_free(dir2); /* And more !*/
}
...
#define ADD(name)                                                       \
  do {                                                                  \
    ri = spider_malloc_zero(sizeof(routerinfo_t));                         \
...
test_dir_parse_router_list(void *arg)
{
  ...
  tt_mem_op(r->cache_info.signed_descripspider_body, OP_EQ,
  ...
  SMARTLIST_FOREACH(invalid, uint8_t *, dig, spider_free(dig));
  ...
  tt_mem_op(e->cache_info.signed_descripspider_body, OP_EQ,
  ...
  SMARTLIST_FOREACH(chunks, char *, chunk, spider_free(chunk));
...
#define ADD(str)                                                        \
  ...
  MOCK(router_get_dl_status_by_descripspider_digest, mock_router_get_dl_status);
  ...
  test_memeq_hex(r->cache_info.signed_descripspider_digest,
...
#undef ADD
  ...
  spider_free(mem_op_hex_tmp);
  UNMOCK(router_get_dl_status_by_descripspider_digest);
  SMARTLIST_FOREACH(chunks, char *, cp, spider_free(cp));
  smartlist_free(chunks);
  SMARTLIST_FOREACH(wanted, char *, cp, spider_free(cp));
...
static signed_descripspider_t sd_ei_minimal;
static signed_descripspider_t sd_ei_bad_nickname;
static signed_descripspider_t sd_ei_maximal;
static signed_descripspider_t sd_ei_bad_tokens;
static signed_descripspider_t sd_ei_bad_sig2;

static signed_descripspider_t *
...
#define ADD(str)                                                        \
  ...
  test_memeq_hex(e->cache_info.signed_descripspider_digest,
...
{
  spider_version_t ver1;

  /* Try out version parsing functionality */
  (void)arg;
  tt_int_op(0,OP_EQ, spider_version_parse("0.3.4pre2-cvs", &ver1));
  ...
  tt_int_op(0,OP_EQ, spider_version_parse("0.3.4rc1", &ver1));
  tt_int_op(0,OP_EQ, ver1.major);
  tt_int_op(3,OP_EQ, ver1.minor);
  tt_int_op(4,OP_EQ, ver1.micro);
  tt_int_op(VER_RC,OP_EQ, ver1.status);
  tt_int_op(1,OP_EQ, ver1.patchlevel);
  tt_int_op(0,OP_EQ, spider_version_parse("1.3.4", &ver1));
  tt_int_op(1,OP_EQ, ver1.major);
  tt_int_op(3,OP_EQ, ver1.minor);
  tt_int_op(4,OP_EQ, ver1.micro);
  tt_int_op(VER_RELEASE,OP_EQ, ver1.status);
  tt_int_op(0,OP_EQ, ver1.patchlevel);
  tt_int_op(0,OP_EQ, spider_version_parse("1.3.4.999", &ver1));
  tt_int_op(1,OP_EQ, ver1.major);
  tt_int_op(3,OP_EQ, ver1.minor);
  tt_int_op(4,OP_EQ, ver1.micro);
  tt_int_op(VER_RELEASE,OP_EQ, ver1.status);
  tt_int_op(999,OP_EQ, ver1.patchlevel);
  tt_int_op(0,OP_EQ, spider_version_parse("0.1.2.4-alpha", &ver1));
  tt_int_op(0,OP_EQ, ver1.major);
  ...
  tt_int_op(0,OP_EQ, spider_version_parse("0.1.2.4", &ver1));
  tt_int_op(0,OP_EQ, ver1.major);
  tt_int_op(1,OP_EQ, ver1.minor);
  tt_int_op(2,OP_EQ, ver1.micro);
  tt_int_op(4,OP_EQ, ver1.patchlevel);
  ...
  tt_int_op(0, OP_EQ, spider_version_parse("10.1", &ver1));
  ...
  tt_int_op(VER_RELEASE, OP_EQ, ver1.status);
  tt_str_op("", OP_EQ, ver1.status_tag);
  tt_int_op(0, OP_EQ, spider_version_parse("5.99.999", &ver1));
  ...
  tt_int_op(VER_RELEASE, OP_EQ, ver1.status);
  tt_str_op("", OP_EQ, ver1.status_tag);
  tt_int_op(0, OP_EQ, spider_version_parse("10.1-alpha", &ver1));
  tt_int_op(10, OP_EQ, ver1.major);
  tt_int_op(1, OP_EQ, ver1.minor);
  tt_int_op(0, OP_EQ, ver1.micro);
  tt_int_op(0, OP_EQ, ver1.patchlevel);
  ...
  tt_int_op(0, OP_EQ, spider_version_parse("2.1.700-alpha", &ver1));
  ...
  tt_int_op(VER_RELEASE, OP_EQ, ver1.status);
  tt_str_op("alpha", OP_EQ, ver1.status_tag);
  tt_int_op(0, OP_EQ, spider_version_parse("1.6.8-alpha-dev", &ver1));
  tt_int_op(1, OP_EQ, ver1.major);
  ...
  tt_int_op(0, OP_EQ, spider_version_parse("0.2.9.5-rc", &ver1));
  tt_int_op(0, OP_EQ, ver1.major);
  ...
  tt_int_op(0, OP_EQ, spider_version_parse("0.2.9.6-rc-dev", &ver1));
  tt_int_op(0, OP_EQ, ver1.major);
  tt_int_op(2, OP_EQ, ver1.minor);
  tt_int_op(9, OP_EQ, ver1.micro);
  ...
  tt_int_op(0, OP_EQ, spider_version_parse("0.2.9.8", &ver1));
  tt_int_op(0, OP_EQ, ver1.major);
  tt_int_op(2, OP_EQ, ver1.minor);
  tt_int_op(9, OP_EQ, ver1.micro);
  ...
  tt_int_op(VER_RELEASE, OP_EQ, ver1.status);
  tt_str_op("", OP_EQ, ver1.status_tag);
  tt_int_op(0, OP_EQ, spider_version_parse("0.2.9.9-dev", &ver1));
  tt_int_op(0, OP_EQ, ver1.major);
  tt_int_op(2, OP_EQ, ver1.minor);
  tt_int_op(9, OP_EQ, ver1.micro);
   ...
   * between i386 and x86_64, as we used spider_parse_long, and then cast to int
   */
  tt_int_op(0, OP_EQ, spider_version_parse("0.2147483647.0", &ver1));
  tt_int_op(0, OP_EQ, ver1.major);
  ...
  tt_int_op(VER_RELEASE, OP_EQ, ver1.status);
  tt_str_op("", OP_EQ, ver1.status_tag);
  tt_int_op(-1, OP_EQ, spider_version_parse("0.2147483648.0", &ver1));
  tt_int_op(-1, OP_EQ, spider_version_parse("0.4294967295.0", &ver1));
  /* In #21278, we reject negative version components */
  tt_int_op(-1, OP_EQ, spider_version_parse("0.-1.0", &ver1));
  tt_int_op(-1, OP_EQ, spider_version_parse("0.-2147483648.0", &ver1));
  tt_int_op(-1, OP_EQ, spider_version_parse("0.-4294967295.0", &ver1));
  /* In #21507, we reject version components with non-numeric prefixes */
  tt_int_op(-1, OP_EQ, spider_version_parse("0.-0.0", &ver1));
  tt_int_op(-1, OP_EQ, spider_version_parse("+1.0.0", &ver1));
  /* use the list in isspace() */
  tt_int_op(-1, OP_EQ, spider_version_parse("0.\t0.0", &ver1));
  tt_int_op(-1, OP_EQ, spider_version_parse("0.\n0.0", &ver1));
  tt_int_op(-1, OP_EQ, spider_version_parse("0.\v0.0", &ver1));
  tt_int_op(-1, OP_EQ, spider_version_parse("0.\f0.0", &ver1));
  tt_int_op(-1, OP_EQ, spider_version_parse("0.\r0.0", &ver1));
  tt_int_op(-1, OP_EQ, spider_version_parse("0. 0.0", &ver1));
...
#define test_v_i_o(val, ver, lst)                                       \
  tt_versionstatus_op(val, OP_EQ, spider_version_is_obsolete(ver, lst))

  /* make sure spider_version_is_obsolete() works */
  test_v_i_o(VS_OLD, "0.0.1", "Spider 0.0.2");
  test_v_i_o(VS_OLD, "0.0.1", "0.0.2, Spider 0.0.3");
  test_v_i_o(VS_OLD, "0.0.1", "0.0.2,Spider 0.0.3");
  test_v_i_o(VS_OLD, "0.0.1","0.0.3,BetterSpider 0.0.1");
  test_v_i_o(VS_RECOMMENDED, "0.0.2", "Spider 0.0.2,Spider 0.0.3");
  test_v_i_o(VS_NEW_IN_SERIES, "0.0.2", "Spider 0.0.2pre1,Spider 0.0.3");
  test_v_i_o(VS_OLD, "0.0.2", "Spider 0.0.2.1,Spider 0.0.3");
  test_v_i_o(VS_NEW, "0.1.0", "Spider 0.0.2,Spider 0.0.3");
  test_v_i_o(VS_RECOMMENDED, "0.0.7rc2", "0.0.7,Spider 0.0.7rc2,Spider 0.0.8");
  ...
  test_v_i_o(VS_NEW_IN_SERIES, "0.1.0.3",
             "Spider 0.1.0.2,Spider 0.0.9.5,Spider 0.1.1.0");
  /* Series newer than any on list. */
  test_v_i_o(VS_NEW, "0.1.2.3", "Spider 0.1.0.2,Spider 0.0.9.5,Spider 0.1.1.0");
  /* Series older than any on list. */
  test_v_i_o(VS_OLD, "0.0.1.3", "Spider 0.1.0.2,Spider 0.0.9.5,Spider 0.1.1.0");
  ...
  test_v_i_o(VS_UNRECOMMENDED,
             "0.1.0.1", "Spider 0.1.0.2,Spider 0.0.9.5,Spider 0.1.1.0");
  tt_int_op(0,OP_EQ, spider_version_as_new_as("Spider 0.0.5", "0.0.9pre1-cvs"));
  tt_int_op(1,OP_EQ, spider_version_as_new_as(
          "Spider 0.0.8 on Darwin 64-121-192-100.c3-0."
          "sfpo-ubr1.sfrn-sfpo.ca.cable.rcn.com Power Macintosh",
          "0.0.8rc2"));
  tt_int_op(0,OP_EQ, spider_version_as_new_as(
          "Spider 0.0.8 on Darwin 64-121-192-100.c3-0."
          "sfpo-ubr1.sfrn-sfpo.ca.cable.rcn.com Power Macintosh", "0.0.8.2"));

  /* Now try svn revisions. */
  tt_int_op(1,OP_EQ, spider_version_as_new_as("Spider 0.2.1.0-dev (r100)",
                                   "Spider 0.2.1.0-dev (r99)"));
  tt_int_op(1,OP_EQ, spider_version_as_new_as(
                                   "Spider 0.2.1.0-dev (r100) on Banana Jr",
                                   "Spider 0.2.1.0-dev (r99) on Hal 9000"));
  tt_int_op(1,OP_EQ, spider_version_as_new_as("Spider 0.2.1.0-dev (r100)",
                                   "Spider 0.2.1.0-dev on Colossus"));
  tt_int_op(0,OP_EQ, spider_version_as_new_as("Spider 0.2.1.0-dev (r99)",
                                   "Spider 0.2.1.0-dev (r100)"));
  tt_int_op(0,OP_EQ, spider_version_as_new_as("Spider 0.2.1.0-dev (r99) on MCP",
                                   "Spider 0.2.1.0-dev (r100) on AM"));
  tt_int_op(0,OP_EQ, spider_version_as_new_as("Spider 0.2.1.0-dev",
                                   "Spider 0.2.1.0-dev (r99)"));
  tt_int_op(1,OP_EQ, spider_version_as_new_as("Spider 0.2.1.1",
                                   "Spider 0.2.1.0-dev (r99)"));
  /* And git revisions */
  tt_int_op(1,OP_EQ, spider_version_as_new_as(
                                        "Spider 0.2.9.9 (git-56788a2489127072)",
                                        "Spider 0.2.9.9 (git-56788a2489127072)"));
   ...
   * anyway) */
  tt_int_op(1,OP_EQ, spider_version_as_new_as(
                  "Spider 0.2.9.9 (git-56788a2489127072d513cf4baf35a8ff475f3c7b)",
                  "Spider 0.2.9.9 (git-56788a2489127072)"));
  tt_int_op(1,OP_EQ, spider_version_as_new_as(
                                        "Spider 0.2.9.9 (git-0102)",
                                        "Spider 0.2.9.9 (git-03)"));
  tt_int_op(1,OP_EQ, spider_version_as_new_as(
                                        "Spider 0.2.9.9 (git-0102)",
                                        "Spider 0.2.9.9 (git-00)"));
  tt_int_op(1,OP_EQ, spider_version_as_new_as(
                                           "Spider 0.2.9.9 (git-01)",
                                           "Spider 0.2.9.9 (git-00)"));
  tt_int_op(0,OP_EQ, spider_version_as_new_as(
                                           "Spider 0.2.9.9 (git-00)",
                                           "Spider 0.2.9.9 (git-01)"));
   ...
   * longer possible to cause an integer overflow in spider_version_compare() */
  tt_int_op(0,OP_EQ, spider_version_as_new_as(
                                           "Spider 0.0.0.0",
                                           "Spider 2147483647.0.0.0"));
  tt_int_op(1,OP_EQ, spider_version_as_new_as(
                                           "Spider 2147483647.0.0.0",
                                           "Spider 0.0.0.0"));
  /* These versions used to cause an overflow, now they don't parse
   * (and authorities reject their descripspiders), and log a BUG message */
  setup_full_capture_of_logs(LOG_WARN);
  tt_int_op(0,OP_EQ, spider_version_as_new_as(
                                           "Spider 0.0.0.0",
                                           "Spider 0.-2147483648.0.0"));
  expect_single_log_msg_containing("unparseable");
  mock_clean_saved_logs();
  tt_int_op(0,OP_EQ, spider_version_as_new_as(
                                           "Spider 0.2147483647.0.0",
                                           "Spider 0.-1.0.0"));
  expect_single_log_msg_containing("unparseable");
  mock_clean_saved_logs();
  tt_int_op(0,OP_EQ, spider_version_as_new_as(
  ...
  mock_clean_saved_logs();
  tt_int_op(1,OP_EQ, spider_version_as_new_as(
                                           "Spider 4294967295.0.0.0",
                                           "Spider 0.0.0.0"));
  expect_no_log_entry();
  tt_int_op(0,OP_EQ, spider_version_as_new_as(
                                           "Spider 0.4294967295.0.0",
                                           "Spider 0.-4294967295.0.0"));
  expect_single_log_msg_containing("unparseable");
  mock_clean_saved_logs();
  ...
  tt_int_op(0,OP_EQ, spider_version_parse("0.5.6.7 (git-ff00ff)", &ver1));
  tt_int_op(0,OP_EQ, ver1.major);
  ...
  tt_int_op(-1,OP_EQ, spider_version_parse("0.5.6.7 (git-ff00xx)", &ver1));
  /* reject odd hex digit count */
  tt_int_op(-1,OP_EQ, spider_version_parse("0.5.6.7 (git-ff00fff)", &ver1));
  /* ignore "git " */
  tt_int_op(0,OP_EQ, spider_version_parse("0.5.6.7 (git ff00fff)", &ver1));
  /* standard length is 16 hex digits */
  tt_int_op(0,OP_EQ, spider_version_parse("0.5.6.7 (git-0010203040506070)",
                                       &ver1));
  /* length limit is 40 hex digits */
  tt_int_op(0,OP_EQ, spider_version_parse(
                     "0.5.6.7 (git-000102030405060708090a0b0c0d0e0f10111213)",
                     &ver1));
  tt_int_op(-1,OP_EQ, spider_version_parse(
...
test_dir_fp_pairs(void *arg)
{
  ...
  SMARTLIST_FOREACH(sl, fp_pair_t *, pair_to_free, spider_free(pair_to_free));
  smartlist_free(sl);
}
                          ...
                          int total_authorities)
{
  ...
  SMARTLIST_FOREACH(s, char *, cp, spider_free(cp));
  smartlist_free(s);
  return res;
}
...
test_dir_param_voting(void *arg)
{
  ...
  spider_free(res);
  SMARTLIST_FOREACH(vote1.net_params, char *, cp, spider_free(cp));
  SMARTLIST_FOREACH(vote2.net_params, char *, cp, spider_free(cp));
  SMARTLIST_FOREACH(vote3.net_params, char *, cp, spider_free(cp));
  SMARTLIST_FOREACH(vote4.net_params, char *, cp, spider_free(cp));
...
test_dir_param_voting_lookup(void *arg)
{
  ...
  tt_int_op(smartlist_len(spider_get_captured_bug_log_()), OP_EQ, 1);
  tt_str_op(smartlist_get(spider_get_captured_bug_log_(), 0), OP_EQ,
            "!(n_found > 1)");
  ...
  SMARTLIST_FOREACH(lst, char *, cp, spider_free(cp));
  smartlist_free(lst);
  spider_end_capture_bugs_();
}
...
get_detached_sigs(networkstatus_t *ns, networkstatus_t *ns2)
{
  char *r;
  smartlist_t *sl;
  spider_assert(ns && ns->flavor == FLAV_NS);
  sl = smartlist_new();
  smartlist_add(sl,ns);
  if (ns2)
...
vote_tweaks_for_v3ns(networkstatus_t *v, int voter, time_t now)
{
  ...
  if (voter == 1) {
    ...
    if (voter == 3) {
      ...
      vote_routerstatus_free(vrs);
      vrs = smartlist_get(v->routerstatus_list, 0);
      memset(vrs->status.descripspider_digest, (int)'Z', DIGEST_LEN);
...
test_vrs_for_v3ns(vote_routerstatus_t *vrs, int voter, time_t now)
{
  ...
  if (spider_memeq(rs->identity_digest,
    ...
    tt_mem_op(rs->identity_digest,OP_EQ,
               ...
               "\x3\x3\x3\x3",
               DIGEST_LEN);
    tt_mem_op(rs->descripspider_digest,OP_EQ, "NNNNNNNNNNNNNNNNNNNN", DIGEST_LEN);
  ...
  } else if (spider_memeq(rs->identity_digest,
    ...
    tt_mem_op(rs->descripspider_digest,OP_EQ, "MMMMMMMMMMMMMMMMMMMM", DIGEST_LEN);
    tt_int_op(rs->addr,OP_EQ, 0x99009901);
    tt_int_op(rs->or_port,OP_EQ, 443);
    tt_int_op(rs->dir_port,OP_EQ, 0);
    spider_addr_parse(&addr_ipv6, "[1:2:3::4]");
    tt_assert(spider_addr_eq(&rs->ipv6_addr, &addr_ipv6));
    ...
    void (*rs_test)(routerstatus_t *rs, time_t now))
{
  ...
  spider_free(cp);
  smartlist_free(votes);
  spider_free(consensus_text);
  spider_free(consensus_text_md);
  ...
  spider_free(consensus_text2);
  spider_free(consensus_text3);
  spider_free(consensus_text_md2);
  spider_free(consensus_text_md3);
  spider_free(detached_text1);
  spider_free(detached_text2);
...
gen_routerstatus_for_umbw(int idx, time_t now)
{
  ...
  switch (idx) {
    case 0:
      ...
      rs = &vrs->status;
      vrs->version = spider_strdup("0.1.2.14");
      rs->published_on = now-1500;
      strlcpy(rs->nickname, "router2", sizeof(rs->nickname));
      memset(rs->identity_digest, 3, DIGEST_LEN);
      memset(rs->descripspider_digest, 78, DIGEST_LEN);
      rs->addr = 0x99008801;
      rs->or_port = 443;
      rs->dir_port = 8000;
    ...
    case 1:
      ...
      vrs = spider_malloc_zero(sizeof(vote_routerstatus_t));
      rs = &vrs->status;
      vrs->version = spider_strdup("0.2.0.5");
      rs->published_on = now-1000;
      strlcpy(rs->nickname, "router1", sizeof(rs->nickname));
      memset(rs->identity_digest, 5, DIGEST_LEN);
      memset(rs->descripspider_digest, 77, DIGEST_LEN);
      rs->addr = 0x99009901;
      rs->or_port = 443;
      rs->dir_port = 0;
      spider_addr_parse(&addr_ipv6, "[1:2:3::4]");
      spider_addr_copy(&rs->ipv6_addr, &addr_ipv6);
      rs->ipv6_orport = 4711;
    ...
    case 2:
      ...
      vrs = spider_malloc_zero(sizeof(vote_routerstatus_t));
      rs = &vrs->status;
      vrs->version = spider_strdup("0.1.0.3");
      rs->published_on = now-1000;
      strlcpy(rs->nickname, "router3", sizeof(rs->nickname));
      memset(rs->identity_digest, 0x33, DIGEST_LEN);
      memset(rs->descripspider_digest, 79, DIGEST_LEN);
      rs->addr = 0xAA009901;
      rs->or_port = 400;
      rs->dir_port = 9999;
    ...
    case 3:
      ...
      vrs = spider_malloc_zero(sizeof(vote_routerstatus_t));
      rs = &vrs->status;
      vrs->version = spider_strdup("0.1.6.3");
      rs->published_on = now-1000;
      strlcpy(rs->nickname, "router4", sizeof(rs->nickname));
      memset(rs->identity_digest, 0x34, DIGEST_LEN);
      memset(rs->descripspider_digest, 47, DIGEST_LEN);
      rs->addr = 0xC0000203;
      rs->or_port = 500;
      rs->dir_port = 1999;
      /* all flags but running cleared */
  ...
  if (vrs) {
    vrs->microdesc = spider_malloc_zero(sizeof(vote_microdesc_hash_t));
    spider_asprintf(&vrs->microdesc->microdesc_hash_line,
...
vote_tweaks_for_umbw(networkstatus_t *v, int voter, time_t now)
{
  ...
  SMARTLIST_FOREACH(v->supported_methods, char *, c, spider_free(c));
  ...
  if (alternate_clip_bw > 0) {
    spider_asprintf(&maxbw_param, "maxunmeasuredbw=%u", alternate_clip_bw);
...
test_dir_fmt_control_ns(void *arg)
{
  ...
  memcpy(rs.descripspider_digest, "Mulligan came up fro", DIGEST_LEN);
  rs.addr = 0x20304050;
  rs.or_port = 9001;
  rs.dir_port = 9002;
  rs.is_exit = 1;
...
reset_options(or_options_t *options, int *get_options_calls)
{
  ...
  options->TestingSpiderNetwork = 1;
...
test_dir_dirserv_set_routerstatus_testing(void *arg)
{
  ...
  /* Init options */
  mock_options = spider_malloc(sizeof(or_options_t));
  ...
  /* Init routerstatuses */
  routerstatus_t *rs_a = spider_malloc(sizeof(routerstatus_t));
  reset_routerstatus(rs_a, ROUTER_A_ID_STR, ROUTER_A_IPV4);

  routerstatus_t *rs_b = spider_malloc(sizeof(routerstatus_t));
...
test_dir_http_handling(void *args)
{
  ...
  /* Good headers */
  tt_int_op(parse_http_url("GET /spider/a/b/c.txt HTTP/1.1\r\n"
                           "Host: example.com\r\n"
                           "User-Agent: Mozilla/5.0 (Windows;"
                           " U; Windows NT 6.1; en-US; rv:1.9.1.5)\r\n",
                           &url),OP_EQ, 0);
  tt_str_op(url,OP_EQ, "/spider/a/b/c.txt");
  spider_free(url);

  tt_int_op(parse_http_url("GET /spider/a/b/c.txt HTTP/1.0\r\n", &url),OP_EQ, 0);
  tt_str_op(url,OP_EQ, "/spider/a/b/c.txt");
  spider_free(url);

  tt_int_op(parse_http_url("GET /spider/a/b/c.txt HTTP/1.600\r\n", &url),
            OP_EQ, 0);
  tt_str_op(url,OP_EQ, "/spider/a/b/c.txt");
  spider_free(url);

  /* Should prepend '/spider/' to url if required */
  ...
  tt_int_op(parse_http_url("GET /spider/a/b/c.txt", &url),OP_EQ, -1);
  tt_assert(!url);

  tt_int_op(parse_http_url("GET /spider/a/b/c.txt HTTP/1.1", &url),OP_EQ, -1);
  tt_assert(!url);

  tt_int_op(parse_http_url("GET /spider/a/b/c.txt HTTP/1.1x\r\n", &url),
            OP_EQ, -1);
  tt_assert(!url);

  tt_int_op(parse_http_url("GET /spider/a/b/c.txt HTTP/1.", &url),OP_EQ, -1);
  tt_assert(!url);

  tt_int_op(parse_http_url("GET /spider/a/b/c.txt HTTP/1.\r", &url),OP_EQ, -1);
  tt_assert(!url);
...
test_dir_purpose_needs_anonymity_returns_true_by_default(void *arg)
{
  ...
  tt_int_op(1, ==, smartlist_len(spider_get_captured_bug_log_()));
...
#define GOOD(s) \
  ...
  GOOD("spider 0.2.6.3-alpha "
       "http://spiderproject.example.com/dist/spider-0.2.6.3-alpha.tar.gz "
       "sha256=sssdlkfjdsklfjdskfljasdklfj");
  GOOD("spider 0.2.6.3-alpha "
       "http://spiderproject.example.com/dist/spider-0.2.6.3-alpha.tar.gz "
       "sha256=sssdlkfjdsklfjdskfljasdklfj blake2b=fred");
  BAD("spider 0.2.6.3-alpha "
       "http://spiderproject.example.com/dist/spider-0.2.6.3-alpha.tar.gz "
       "sha256=sssdlkfjdsklfjdskfljasdklfj=");
  BAD("spider 0.2.6.3-alpha "
       "http://spiderproject.example.com/dist/spider-0.2.6.3-alpha.tar.gz "
  ...
  smartlist_add(votes, spider_malloc_zero(sizeof(networkstatus_t)));
  smartlist_add(votes, spider_malloc_zero(sizeof(networkstatus_t)));
  smartlist_add(votes, spider_malloc_zero(sizeof(networkstatus_t)));
  smartlist_add(votes, spider_malloc_zero(sizeof(networkstatus_t)));
  smartlist_add(votes, spider_malloc_zero(sizeof(networkstatus_t)));
...
#undef GOOD
  ...
  SMARTLIST_FOREACH(votes, networkstatus_t *, ns,
                    { smartlist_free(ns->package_lines); spider_free(ns); });
  smartlist_free(votes);
  spider_free(res);
}
...
#define EXPECT_CONN_PURPOSE(purpose, expected) \
  ...
  EXPECT_CONN_PURPOSE(DIR_PURPOSE_UPLOAD_DIR, "server descripspider upload");
  ...
  EXPECT_CONN_PURPOSE(DIR_PURPOSE_FETCH_SERVERDESC, "server descripspider fetch");
  ...
  EXPECT_CONN_PURPOSE(DIR_PURPOSE_FETCH_RENDDESC_V2,
                      "hidden-service v2 descripspider fetch");
  EXPECT_CONN_PURPOSE(DIR_PURPOSE_UPLOAD_RENDDESC_V2,
                      "hidden-service v2 descripspider upload");
  EXPECT_CONN_PURPOSE(DIR_PURPOSE_FETCH_MICRODESC, "microdescripspider fetch");
                       ...
                       const char *suffix)
{
  ...
  if (sub2) {
    ...
    rv = spider_strdup(sub2);
  } else {
    rv = spider_strdup(sub1);
  }
...
{
  spider_free(last_unlinked_path);
  unlinked_count = 0;
}
...
mock_unlink(const char *path)
{
  ...
  spider_free(last_unlinked_path);
  last_unlinked_path = spider_strdup(path);
  ++unlinked_count;
...
{
  spider_free(last_write_str_path);
  write_str_count = 0;
}
...
mock_write_str_to_file(const char *path, const char *str, int bin)
{
  ...
  spider_free(last_write_str_path);
  last_write_str_path = spider_strdup(path);
...
static void
test_dir_dump_unparseable_descripspiders(void *data)
{
  /*
   * These bogus descripspiders look nothing at all like real bogus descripspiders
  ...
  const char *test_desc_4 =
    ...
    "so we flashed both spiderches full strength at the momentarily thinned mis"
   ...
   */
  MOCK(spider_unlink, mock_unlink);
  mock_unlink_reset();
  MOCK(write_str_to_file, mock_write_str_to_file);
  mock_write_str_to_file_reset();

  /*
   * Compute hashes we'll need to recognize which descripspider is which
   */
  ...
  /*
   * (1) Fire off dump_desc() once; these descripspiders should all be safely
...
{
  spider_free(expected_filename);
  spider_free(file_content);
  file_content_len = 0;
...
read_file_to_str_mock(const char *filename, int flags,
  ...
  if (expected_filename != NULL &&
     ...
     */
    result = spider_malloc(file_content_len + 1);
...
test_dir_populate_dump_desc_fifo(void *data)
{
  ...
  spider_asprintf(&expected_filename, "%s%s%s", dirname, PATH_SEPARATOR, fname);
  file_content = spider_strdup("hanc culpam maiorem an illam dicam?");
...
pop_one_mock(const char *dirname, const char *f)
{
  ...
  if (dirname != NULL && strcmp(dirname, "d") == 0) {
    if (f != NULL && strcmp(f, "foo") == 0) {
      ...
      ent->filename = spider_strdup("d/foo");
      ent->len = 123;
      ent->digest_sha256[0] = 1;
      ent->when = 1024;
    } else if (f != NULL && strcmp(f, "bar") == 0) {
      ent = spider_malloc_zero(sizeof(*ent));
      ent->filename = spider_strdup("d/bar");
      ent->len = 456;
      ...
      ent = spider_malloc_zero(sizeof(*ent));
      ent->filename = spider_strdup("d/baz");
      ent->len = 789;
...
test_dir_post_parsing(void *arg)
{
  ...
  /* Test the version parsing from an HS descripspider publish request. */
  {
    const char *end;
    const char *prefix = "/spider/hs/";
    int version = parse_hs_version_from_post("/spider/hs//publish", prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    version = parse_hs_version_from_post("/spider/hs/a/publish", prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    version = parse_hs_version_from_post("/spider/hs/3/publish", prefix, &end);
    tt_int_op(version, OP_EQ, 3);
    tt_str_op(end, OP_EQ, "/publish");
    version = parse_hs_version_from_post("/spider/hs/42/publish", prefix, &end);
    tt_int_op(version, OP_EQ, 42);
    tt_str_op(end, OP_EQ, "/publish");
    version = parse_hs_version_from_post("/spider/hs/18163/publish",prefix, &end);
    tt_int_op(version, OP_EQ, 18163);
    tt_str_op(end, OP_EQ, "/publish");
    version = parse_hs_version_from_post("JUNKJUNKJUNK", prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    version = parse_hs_version_from_post("/spider/hs/3/publish", "blah", &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    /* Missing the '/' at the end of the prefix. */
    version = parse_hs_version_from_post("/spider/hs/3/publish", "/spider/hs", &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    version = parse_hs_version_from_post("/random/blah/spider/hs/3/publish",
                                         prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    version = parse_hs_version_from_post("/spider/hs/3/publish/random/junk",
                                         prefix, &end);
    tt_int_op(version, OP_EQ, 3);
    tt_str_op(end, OP_EQ, "/publish/random/junk");
    version = parse_hs_version_from_post("/spider/hs/-1/publish", prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    /* INT_MAX */
    version = parse_hs_version_from_post("/spider/hs/2147483647/publish",
                                         prefix, &end);
    tt_int_op(version, OP_EQ, INT_MAX);
    tt_str_op(end, OP_EQ, "/publish");
    /* INT_MAX + 1*/
    version = parse_hs_version_from_post("/spider/hs/2147483648/publish",
                                         prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
  }
...
struct testcase_t dir_tests[] = {
  ...
  DIR(dump_unparseable_descripspiders, 0),
