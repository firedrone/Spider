...
/** Helper function for testing spider_spawn_background */
static void
                          ...
                          int expected_status)
{
  ...
  /* Start the program */
#ifdef _WIN32
  status = spider_spawn_background(NULL, argv, NULL, &process_handle);
#else
  status = spider_spawn_background(argv[0], argv, NULL, &process_handle);
#endif
  ...
  /* Check stdout */
  pos = spider_read_all_from_process_stdout(process_handle, stdout_buf,
                                         sizeof(stdout_buf) - 1);
  tt_assert(pos >= 0);
  stdout_buf[pos] = '\0';
  ...
  retval = spider_get_exit_code(process_handle, 1, &exit_code);
  ...
  /* Check stderr */
  pos = spider_read_all_from_process_stderr(process_handle, stderr_buf,
                                         sizeof(stderr_buf) - 1);
  tt_assert(pos >= 0);
  stderr_buf[pos] = '\0';
  ...
  if (process_handle)
    spider_process_handle_destroy(process_handle, 1);
}
...
test_util_spawn_background_fail(void *ptr)
{
  ...
  spider_snprintf(code, sizeof(code), "%x/%x",
    9 /* CHILD_STATE_FAILEXEC */ , ENOENT);
  spider_snprintf(expected_out, sizeof(expected_out),
...
#ifdef _WIN32
    pos = spider_read_all_handle(process_handle->stdout_pipe, stdout_buf,
...
#endif
    log_info(LD_GENERAL, "spider_read_all_handle() returned %d", (int)pos);
...
test_util_spawn_background_waitpid_notify(void *arg)
{
   ...
   * process exit (on unix) and/or whether spider_get_exit_code() can notice it
   * (on windows) */
  ...
  ms_timer = 30*1000;
  while (((retval = spider_get_exit_code(process_handle, 0, &exit_code))
