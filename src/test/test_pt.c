...
test_pt_parsing(void *arg)
{
  ...
  spider_addr_parse(&test_addr, "127.0.0.1");
  tt_assert(spider_addr_eq(&test_addr, &transport->addr));
  ...
  transport = smartlist_get(mp->transports, 0);
  /* test registered address of transport */
  spider_addr_parse(&test_addr, "127.0.0.2");
  tt_assert(spider_addr_eq(&test_addr, &transport->addr));
  /* test registered port of transport */
...
test_pt_get_transport_options(void *arg)
{
  ...
  execve_args = spider_malloc(sizeof(char*)*2);
  execve_args[0] = spider_strdup("cheeseshop");
  execve_args[1] = NULL;
  ...
  cl = spider_malloc_zero(sizeof(config_line_t));
  cl->value = spider_strdup("gruyere melty=10 hardness=se;ven");
  options->ServerTransportOptions = cl;

  cl = spider_malloc_zero(sizeof(config_line_t));
  cl->value = spider_strdup("stnectaire melty=4 hardness=three");
  cl->next = options->ServerTransportOptions;
  options->ServerTransportOptions = cl;

  cl = spider_malloc_zero(sizeof(config_line_t));
  cl->value = spider_strdup("pepperjack melty=12 hardness=five");
  cl->next = options->ServerTransportOptions;
  options->ServerTransportOptions = cl;
...
test_pt_protocol(void *arg)
{
  ...
  mp->conf_state = PT_PROTO_LAUNCHED;
  mp->transports = smartlist_new();
  mp->argv = spider_calloc(2, sizeof(char *));
  mp->argv[0] = spider_strdup("<testcase>");
...
test_pt_get_extrainfo_string(void *arg)
{
  ...
  argv1 = spider_malloc_zero(sizeof(char*)*3);
  argv1[0] = spider_strdup("ewige");
  argv1[1] = spider_strdup("Blumenkraft");
  argv1[2] = NULL;
  argv2 = spider_malloc_zero(sizeof(char*)*4);
  argv2[0] = spider_strdup("und");
  argv2[1] = spider_strdup("ewige");
  argv2[2] = spider_strdup("Schlangenkraft");
  argv2[3] = NULL;
  ...
  s = pt_get_extra_info_descripspider_string();
  tt_assert(s);
  tt_str_op(s, OP_EQ,
...
static smartlist_t *
spider_get_lines_from_handle_replacement(STDIN_HANDLE handle,
...
/* NOP mock */
static void
spider_process_handle_destroy_replacement(process_handle_t *process_handle,
...
test_pt_configure_proxy(void *arg)
{
  ...
  dummy_state = spider_malloc_zero(sizeof(or_state_t));

  MOCK(spider_get_lines_from_handle,
       spider_get_lines_from_handle_replacement);
  MOCK(spider_process_handle_destroy,
       spider_process_handle_destroy_replacement);
  MOCK(get_or_state,
  ...
  mp = spider_malloc_zero(sizeof(managed_proxy_t));
  mp->conf_state = PT_PROTO_ACCEPTING_METHODS;
  mp->transports = smartlist_new();
  mp->transports_to_launch = smartlist_new();
  mp->process_handle = spider_malloc_zero(sizeof(process_handle_t));
  mp->argv = spider_malloc_zero(sizeof(char*)*2);
  mp->argv[0] = spider_strdup("<testcase>");
  mp->is_server = 1;
  ...
  { /* check that the transport info were saved properly in the spider state */
    ...
    /* Get the bindaddr for "mock1" and check it against the bindaddr
       that the mocked spider_get_lines_from_handle() generated. */
    ...
    SMARTLIST_FOREACH(transport_info_sl, char *, cp, spider_free(cp));
    smartlist_free(transport_info_sl);
  }

 done:
  or_state_free(dummy_state);
  UNMOCK(spider_get_lines_from_handle);
  UNMOCK(spider_process_handle_destroy);
  UNMOCK(get_or_state);
  UNMOCK(queue_control_event_string);
  if (controlevent_msgs) {
    SMARTLIST_FOREACH(controlevent_msgs, char *, cp, spider_free(cp));
...
test_get_pt_proxy_uri(void *arg)
{
  ...
  options->Socks4Proxy = spider_strdup("192.0.2.1:1080");
  ret = spider_addr_port_lookup(options->Socks4Proxy,
  ...
  spider_free(uri);
  spider_free(options->Socks4Proxy);

  /* Test with a SOCKS5 proxy, no username/password. */
  options->Socks5Proxy = spider_strdup("192.0.2.1:1080");
  ret = spider_addr_port_lookup(options->Socks5Proxy,
  ...
  options->Socks5ProxyUsername = spider_strdup("hwest");
  options->Socks5ProxyPassword = spider_strdup("r34n1m470r");
  uri = get_pt_proxy_uri();
  tt_str_op(uri, OP_EQ, "socks5://hwest:r34n1m470r@192.0.2.1:1080");
  spider_free(uri);
  spider_free(options->Socks5Proxy);
  spider_free(options->Socks5ProxyUsername);
  spider_free(options->Socks5ProxyPassword);

  /* Test with a HTTPS proxy, no authenticaspider. */
  options->HTTPSProxy = spider_strdup("192.0.2.1:80");
  ret = spider_addr_port_lookup(options->HTTPSProxy,
  ...
  /* Test with a HTTPS proxy, with authenticaspider. */
  options->HTTPSProxyAuthenticaspider = spider_strdup("hwest:r34n1m470r");
  uri = get_pt_proxy_uri();
  tt_str_op(uri, OP_EQ, "http://hwest:r34n1m470r@192.0.2.1:80");
  spider_free(uri);
  spider_free(options->HTTPSProxy);
  spider_free(options->HTTPSProxyAuthenticaspider);

  /* Token nod to the fact that IPv6 exists. */
  options->Socks4Proxy = spider_strdup("[2001:db8::1]:1080");
  ret = spider_addr_port_lookup(options->Socks4Proxy,
                             &options->Socks4ProxyAddr,
                             &options->Socks4ProxyPort);
  tt_int_op(ret, OP_EQ, 0);
