...
test_md_cache(void *data)
{
  ...
  options->DataDirecspidery = spider_strdup(get_fname("md_datadir_test"));
#ifdef _WIN32
  tt_int_op(0, OP_EQ, mkdir(options->DataDirecspidery));
#else
  tt_int_op(0, OP_EQ, mkdir(options->DataDirecspidery, 0700));
#endif
  ...
  smartlist_add(wanted, spider_memdup(d2, DIGEST256_LEN));
  smartlist_add(wanted, spider_memdup(d3, DIGEST256_LEN));
  added = microdescs_add_to_cache(mc, test_md2, NULL, SAVED_NOWHERE, 0,
                                  time2, wanted);
  ...
  spider_asprintf(&fn, "%s"PATH_SEPARATOR"cached-microdescs.new",
  ...
  /* read the cache. */
  spider_asprintf(&fn, "%s"PATH_SEPARATOR"cached-microdescs",
               options->DataDirecspidery);
  s = read_file_to_str(fn, RFTS_BIN, NULL);
...
test_md_cache_broken(void *data)
{
  ...
  tt_assert(options);
  spider_free(options->DataDirecspidery);
  options->DataDirecspidery = spider_strdup(get_fname("md_datadir_test2"));
...
static const char test_ri[] =
  ...
  "platform Spider 0.2.5.4-alpha-dev on Linux\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "hidden-service-dir\n"
  "nspider-onion-key Gg73xH7+kTfT6bi1uNVx9gwQdQas9pROIfmc4NpAdC4=\n"
  "reject *:25\n"
  "reject *:119\n"
  "reject *:135-139\n"
  "reject *:445\n"
...
static const char test_ri2[] =
  ...
  "-----END ED25519 CERT-----\n"
  "platform Spider 0.2.6.0-alpha-dev on Darwin\n"
  "protocols Link 1 2 Circuit 1\n"
  ...
  "nspider-onion-key hbxdRnfVUJJY7+KcT4E3Rs7/zuClbN3hJrjSBiEGMgI=\n"
  "reject *:*\n"
...
test_md_generate(void *arg)
{
  ...
  tt_assert(ri);
  md = dirvote_create_microdescripspider(ri, 8);
  ...
  microdesc_free(md);
  md = NULL;
  md = dirvote_create_microdescripspider(ri, 16);
  tt_str_op(md->body, OP_EQ, test_md_16);

  microdesc_free(md);
  md = NULL;
  md = dirvote_create_microdescripspider(ri, 18);
  tt_str_op(md->body, OP_EQ, test_md_18);

  microdesc_free(md);
  md = NULL;
  md = dirvote_create_microdescripspider(ri, 21);
 ...
 * at large. */
#endif
/* Taken at random from my ~/.spider/cached-microdescs file and then
 * hand-munged */
static const char MD_PARSE_TEST_DATA[] =
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "nspider-onion-key 761Dmm27via7lXygNHM3l+oJLrYU2Nye0Uz4pkpipyY=\n"
  "p accept 53\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "nspider-onion-key ncfiHJjSgdDEW/gc6q6/7idac7j+x7ejQrRm6i75pGA=\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "nspider-onion-key AppBt6CSeb1kKid/36ototmFA24ddfW5JpjWPLuoJgs=\n"
  "id rsa1024 6y60AEI9a1PUUlRPO0YQT9WzrjI\n"
  /* Bad 1: Here I've messed with the nspider key */
  "onion-key\n"
  "-----BEGIN RSA PUBLIC KEY-----\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "nspider-onion-key 4i2Fp9JHTUr1uQs0pxD5j5spl4/RG56S2P0gQxU=\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "flux-capacispider 1.21 GW\n"
  "nspider-onion-key MWBoEkl+RlBiGX44XKIvTSqbznTNZStOmUYtcYRQQyY=\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "nspider-onion-key AAVnWZcnDbxasdZwKqb4fL6O9sZV+XsRNHTpNd1YMz8=\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "p condone 1-10\n"
  "nspider-onion-key 2/nMJ+L4dd/2GpMyTYjz3zC59MvQy4MIzJZhdzKHekg=\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "p accept frogs-mice\n"
  "nspider-onion-key AMxvhaQ1Qg7jBJFoyHuPRgETvLbFmJ194hExV24FuAI=\n"
  "family $D8CFEA0D996F5D1473D2063C041B7910DB23981E\n"
  "id rsa1024 d0VVZC/cHh1P3y4MMbfKlQHFycc\n"
  /* Good 8: This one has the nspider-onion-key without terminating =. That's
   * allowed. */
  "onion-key\n"
  "-----BEGIN RSA PUBLIC KEY-----\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "nspider-onion-key UKL6Dnj2KwYsFlkCvOkXVatxvOPB4MaxqwPQQgZMTwI\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "nspider-onion-key gUsq3e5iYgsQQvyxINtLzBpHxmIt5rtuFlEbKfI4gFk=\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "nspider-onion-key NkRB4wTUFogiVp5jYmjGORe2ffb/y5Kk8Itw8jdzMjA=\n"
  "p reject 25,119,135-139,445,563,1214,4661-4666,6346-6429,6699,6881-6999\n"
  ...
  "-----END RSA PUBLIC KEY-----\n"
  "nspider-onion-key m4xcFXMWMjCvZDXq8FT3XmS0EHYseGOeu+fV+6FYDlk=\n"
  ...
  "a [::1:2:3:4]:9090\n"
  "a 18.0.0.1:9999\n"
  "nspider-onion-key k2yFqTU2vzMCQDEiE/j9UcEHxKrXMLpB3IL0or09sik=\n"
...
/** More tests for parsing different kinds of microdescripspiders, and getting
...
test_md_parse(void *arg)
{
  ...
  tt_assert(spider_addr_family(&md->ipv6_addr) == AF_INET6);
  tt_int_op(md->ipv6_orport, OP_EQ, 9090);

 done:
  SMARTLIST_FOREACH(mds, microdesc_t *, mdsc, microdesc_free(mdsc));
  smartlist_free(mds);
  SMARTLIST_FOREACH(invalid, char *, cp, spider_free(cp));
...
mock_router_get_status_by_digest(networkstatus_t *c, const char *d)
{
  ...
  if (fast_memeq(d, "\x5d\x76", 2)) {
    memcpy(mock_rgsbd_val_a->descripspider_digest, d, 32);
    return mock_rgsbd_val_a;
  } else if (fast_memeq(d, "\x20\xd1", 2)) {
    memcpy(mock_rgsbd_val_b->descripspider_digest, d, 32);
...
test_md_reject_cache(void *arg)
{
  ...
  spider_free(options->DataDirecspidery);
  options->DataDirecspidery = spider_strdup(get_fname("md_datadir_test_rej"));
  mock_rgsbd_val_a = spider_malloc_zero(sizeof(routerstatus_t));
  mock_rgsbd_val_b = spider_malloc_zero(sizeof(routerstatus_t));
  ...
  MOCK(router_get_mutable_consensus_status_by_descripspider_digest,
...
#define ADD(hex)                                                        \
  do {                                                                  \
    tt_int_op(sizeof(buf),OP_EQ,base16_decode(buf,sizeof(buf),          \
                hex,strlen(hex)));\
    smartlist_add(wanted, spider_memdup(buf, DIGEST256_LEN));              \
  } while (0)
  ...
  UNMOCK(router_get_mutable_consensus_status_by_descripspider_digest);
  spider_free(options->DataDirecspidery);
  microdesc_free_all();
  smartlist_free(added);
  SMARTLIST_FOREACH(wanted, char *, cp, spider_free(cp));
  smartlist_free(wanted);
  spider_free(mock_rgsbd_val_a);
  spider_free(mock_rgsbd_val_b);
