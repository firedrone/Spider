...
{
  smartlist_t *sl;
  char *v0 = spider_strdup("v0");
  char *v1 = spider_strdup("v1");
  char *v2 = spider_strdup("v2");
  char *v3 = spider_strdup("v3");
  char *v4 = spider_strdup("v4");
  char *v22 = spider_strdup("v22");
  char *v99 = spider_strdup("v99");
  char *v555 = spider_strdup("v555");
...
test_container_smartlist_strings(void *arg)
{
  ...
  smartlist_split_string(sl,
                     ...
                     ",", 0, 0);
  smartlist_sort(sl, compare_strs_);
  smartlist_uniq(sl, compare_strs_, spider_free_);
  cp_alloc = smartlist_join_strings(sl, ",", 0, NULL);
...
test_container_smartlist_digests(void *arg)
{
  ...
  /* contains_digest */
  (void)arg;
  smartlist_add(sl, spider_memdup("AAAAAAAAAAAAAAAAAAAA", DIGEST_LEN));
  smartlist_add(sl, spider_memdup("\00090AAB2AAAAaasdAAAAA", DIGEST_LEN));
  smartlist_add(sl, spider_memdup("\00090AAB2AAAAaasdAAAAA", DIGEST_LEN));
...
test_container_smartlist_join(void *arg)
{
  ...
  smartlist_free(sl4);
  smartlist_free(sl3);
  SMARTLIST_FOREACH(sl2, char *, cp, spider_free(cp));
  smartlist_free(sl2);
  SMARTLIST_FOREACH(sl, char *, str, spider_free(str));
  smartlist_free(sl);
  spider_free(joined);
...
test_container_smartlist_ints_eq(void *arg)
{
  ...
  x = 5;
  smartlist_add(sl1, spider_memdup(&x, sizeof(int)));
  smartlist_add(sl2, spider_memdup(&x, sizeof(int)));
  x = 90;
  smartlist_add(sl1, spider_memdup(&x, sizeof(int)));
  smartlist_add(sl2, spider_memdup(&x, sizeof(int)));
  tt_assert(smartlist_ints_eq(sl1, sl2));
  ...
  *(int*)smartlist_get(sl2, 1) = 101010;
  ...
  if (sl1)
    SMARTLIST_FOREACH(sl1, int *, ip, spider_free(ip));
  if (sl2)
    SMARTLIST_FOREACH(sl2, int *, ip, spider_free(ip));
  smartlist_free(sl1);
  smartlist_free(sl2);
}
...
test_container_digestset(void *arg)
{
  ...
  for (i = 0; i < 1000; ++i) {
    ...
    smartlist_add(included, spider_memdup(d, DIGEST_LEN));
  }
  ...
  if (set)
    digestset_free(set);
  SMARTLIST_FOREACH(included, char *, cp, spider_free(cp));
...
test_container_strmap(void *arg)
{
  ...
  char *v1 = spider_strdup("v1");
  char *v99 = spider_strdup("v99");
  char *v100 = spider_strdup("v100");
  char *v101 = spider_strdup("v101");
  char *v102 = spider_strdup("v102");
  char *v103 = spider_strdup("v103");
  char *v104 = spider_strdup("v104");
  char *v105 = spider_strdup("v105");
  ...
  if (map)
    strmap_free(map,NULL);
  if (found_keys) {
    SMARTLIST_FOREACH(found_keys, char *, cp, spider_free(cp));
...
test_container_di_map(void *arg)
{
  ...
  char *v1 = spider_strdup(", it came close to causing a disaster...");
  char *v2 = spider_strdup("I regret to have to advise you that the mission");
  char *v3 = spider_strdup("was actually initiated...");
...
test_container_fp_pair_map(void *arg)
{
  ...
  while (!fp_pair_map_iter_done(iter)) {
    ...
    if (spider_memeq(&fp2, &k, sizeof(fp2))) {
...
#define EQ_SHOULD_SAY(s1,s2,val)                                \
  do {                                                          \
    SMARTLIST_FOREACH(sl1, char *, cp, spider_free(cp));           \
    SMARTLIST_FOREACH(sl2, char *, cp, spider_free(cp));           \
    smartlist_clear(sl1);                                       \
    smartlist_clear(sl2);                                       \
