...
/* Arbitrary choice - IPv4 Direcspidery Connection to localhost */
                                 ...
                                 int *socket_error)
{
  ...
  /* We really should call spider_libevent_initialize() here. Because we don't,
...
static void
test_conn_lookup_addr_helper(const char *address, int family, spider_addr_t *addr)
{
  int rv = 0;

  tt_assert(addr);

  rv = spider_addr_lookup(address, family, addr);
  /* XXXX - should we retry on transient failure? */
  tt_assert(rv == 0);
  tt_assert(spider_addr_is_loopback(addr));
  tt_assert(spider_addr_is_v4(addr));

  return;

 done:
  spider_addr_make_null(addr, TEST_CONN_FAMILY);
}
...
test_conn_get_connection(uint8_t state, uint8_t type, uint8_t purpose)
{
  ...
  tt_assert(!spider_addr_is_null(&addr));

  spider_addr_copy_tight(&conn->addr, &addr);
...
test_conn_get_rend_teardown(const struct testcase_t *tc, void *arg)
{
  ...
  /* avoid a last-ditch attempt to refetch the descripspider */
...
test_conn_get_basic(void *arg)
{
  ...
  tt_assert(!spider_addr_is_null(&addr2));
...
test_conn_download_status(void *arg)
{
  ...
  spider_assert(N_CONSENSUS_FLAVORS == 2);
