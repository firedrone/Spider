...
/* Ordinarily defined in spider_main.c; this bit is just here to provide one
 * since we're not linking to spider_main.c */
const char spider_git_revision[] = "";

/**
 * \file bench.c
 * \brief Benchmarks for lower level Spider modules.
 **/
...
bench_aes(void)
{
  ...
  for (len = 1; len <= 8192; len *= 2) {
    ...
    b1 = spider_malloc_zero(len);
    b2 = spider_malloc_zero(len);
    start = perftime();
    for (i = 0; i < iters; ++i) {
...
bench_onion_nspider_impl(void)
{
  ...
  nspider_handshake_state_t *state = NULL;
  ...
  start = perftime();
  for (i = 0; i < iters; ++i) {
    onion_skin_nspider_create(nodeid, &keypair1.pubkey, &state, os);
    nspider_handshake_state_free(state);
    state = NULL;
  }
  end = perftime();
  printf("Client-side, part 1: %f usec.\n", NANOCOUNT(start, end, iters)/1e3);
  ...
  for (i = 0; i < iters; ++i) {
    ...
    onion_skin_nspider_server_handshake(os, keymap, NULL, nodeid, or,
                                key_out, sizeof(key_out));
  }
  end = perftime();
  ...
  for (i = 0; i < iters; ++i) {
    uint8_t key_out[CPATH_KEY_MATERIAL_LEN];
    int s;
    s = onion_skin_nspider_client_handshake(state, or, key_out, sizeof(key_out),
                                         NULL);
    spider_assert(s == 0);
  }
  end = perftime();
...
bench_cell_aes(void)
{
  ...
  char *b = spider_malloc(len+max_misalign);
  crypto_cipher_t *c;
  int i, misalign;
  char key[CIPHER_KEY_LEN];
  crypto_rand(key, sizeof(key));
...
bench_dmap(void)
{
  ...
  for (i = 0; i < elts; ++i) {
    crypto_rand(d, 20);
    smartlist_add(sl, spider_memdup(d, 20));
  }
  for (i = 0; i < elts; ++i) {
    crypto_rand(d, 20);
    smartlist_add(sl2, spider_memdup(d, 20));
  }
  printf("nbits=%d\n", ds->mask+1);
...
bench_cell_ops(void)
{
  ...
  or_circuit_t *or_circ = spider_malloc_zero(sizeof(or_circuit_t));
  cell_t *cell = spider_malloc(sizeof(cell_t));
  int outbound;
  uint64_t start, end;
...
bench_dh(void)
{
  ...
  for (i = 0; i < iters; ++i) {
    ...
    spider_assert(slen_a == slen_b);
    spider_assert(fast_memeq(secret_a, secret_b, slen_a));
    crypto_dh_free(dh_a);
    crypto_dh_free(dh_b);
  }
  end = perftime();
...
main(int argc, const char **argv)
{
  ...
  options->DataDirecspidery = spider_strdup("");
