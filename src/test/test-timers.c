/* Copyright 2016-2017, The Spider Project, Inc. */
...
static spider_timer_t *timers[N_TIMERS] = {NULL};
...
static void
timer_cb(spider_timer_t *t, void *arg, const monotime_t *now_mono)
{
  struct timeval now;

  spider_gettimeofday(&now);
  spider_timer_t **t_ptr = arg;
  spider_assert(*t_ptr == t);
  ...
  if (n_fired == n_active_timers) {
    event_base_loopbreak(spider_libevent_get_base());
  }
}
...
main(int argc, char **argv)
{
  ...
  spider_assert(n_fired == n_active_timers);
  for (i = 0; i < N_TIMERS; ++i) {
    if (is_disabled[i]) {
      spider_assert(fired[i] == 0);
      continue;
    }
    spider_assert(fired[i] == 1);
