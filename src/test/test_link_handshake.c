...
static spider_tls_t *mock_peer_cert_expect_spidertls = NULL;
static spider_x509_cert_t *mock_peer_cert = NULL;
static spider_x509_cert_t *
mock_get_peer_cert(spider_tls_t *tls)
{
  if (mock_peer_cert_expect_spidertls &&
      mock_peer_cert_expect_spidertls != tls)
    return NULL;
...
static int
mock_export_key_material(spider_tls_t *tls, uint8_t *secrets_out,
...
test_link_handshake_certs_ok(void *arg)
{
  ...
  spider_addr_from_ipv4h(&c1->base_.addr, 0x7f000001);
  spider_addr_from_ipv4h(&c2->base_.addr, 0x7f000001);
  ...
  MOCK(spider_tls_get_peer_cert, mock_get_peer_cert);
  ...
  chan1 = spider_malloc_zero(sizeof(*chan1));
  channel_tls_common_init(chan1);
  c1->chan = chan1;
  chan1->conn = c1;
  c1->base_.address = spider_strdup("C1");
  c1->tls = spider_tls_new(-1, 0);
  c1->link_proto = 4;
  c1->base_.conn_array_index = -1;
  crypto_pk_get_digest(key2, c1->identity_digest);

  if (with_ed) {
    const spider_x509_cert_t *linkc, *idc;
    spider_tls_get_my_certs(1, &linkc, &idc);
    mock_peer_cert_expect_spidertls = c1->tls; /* We should see this tls... */
    mock_peer_cert = spider_x509_cert_dup(linkc); /* and when we do, the peer's
                                                *  cert is this... */
  }
  channel_tls_process_certs_cell(cell2, chan1);
  mock_peer_cert_expect_spidertls = NULL;
  mock_peer_cert = NULL;

  spider_assert(c1->handshake_state->authenticated);
  ...
  tt_assert(! spider_mem_is_zero(
                (char*)c1->handshake_state->authenticated_rsa_peer_id, 20));

  chan2 = spider_malloc_zero(sizeof(*chan2));
  channel_tls_common_init(chan2);
  c2->chan = chan2;
  chan2->conn = c2;
  c2->base_.address = spider_strdup("C2");
  c2->tls = spider_tls_new(-1, 1);
  c2->link_proto = 4;
  ...
  UNMOCK(spider_tls_get_peer_cert);
...
{
  (void)test;
  certs_data_t *d = spider_malloc_zero(sizeof(*d));
  ...
  spider_assert(is_ed != is_rsa);
  spider_assert(is_link != is_auth);

  d->c = or_connection_new(CONN_TYPE_OR, AF_INET);
  d->chan = spider_malloc_zero(sizeof(*d->chan));
  d->c->chan = d->chan;
  d->c->base_.address = spider_strdup("HaveAnAddress");
  spider_addr_from_ipv4h(&d->c->base_.addr, 0x801f0127);
  ...
  const spider_x509_cert_t *a,*b;
  const uint8_t *enca, *encb;
  size_t lena, lenb;
  spider_tls_get_my_certs(is_link ? 1 : 0, &a, &b);
  spider_x509_cert_get_der(a, &enca, &lena);
  spider_x509_cert_get_der(b, &encb, &lenb);
  ...
  if (is_ed) {
    ...
    const spider_cert_t *id_sign = get_master_signing_key_cert();
    const spider_cert_t *secondary =
  ...
  if (is_link) {
    ...
    mock_peer_cert = spider_x509_cert_dup(a);
  }
...
CERTS_FAIL(missing_signing_key, /* ed25519 */
  {
    ...
    spider_cert_t *bad_cert = spider_cert_create(mk, CERT_TYPE_ID_SIGNING,
...
CERTS_FAIL(link_mismatch, /* ed25519 */
  {
    require_failure_message = "Link certificate does not match "
      "TLS certificate";
    const spider_x509_cert_t *idc;
    spider_tls_get_my_certs(1, NULL, &idc);
    spider_x509_cert_free(mock_peer_cert);
    /* Pretend that the peer cert was something else. */
    mock_peer_cert = spider_x509_cert_dup(idc);
    /* No reencode needed. */
  })
...
CERTS_FAIL(wrong_labels_2,
  {
    const spider_x509_cert_t *a;
    const spider_x509_cert_t *b;
    const uint8_t *enca;
    size_t lena;
    require_failure_message = "The link certificate was not valid";
    spider_tls_get_my_certs(1, &a, &b);
    spider_x509_cert_get_der(a, &enca, &lena);
...
{
  (void)test;
  authchallenge_data_t *d = spider_malloc_zero(sizeof(*d));
  d->c = or_connection_new(CONN_TYPE_OR, AF_INET);
  d->chan = spider_malloc_zero(sizeof(*d->chan));
  d->c->chan = d->chan;
  d->c->base_.address = spider_strdup("HaveAnAddress");
...
static int
mock_get_tlssecrets(spider_tls_t *tls, uint8_t *secrets_out)
{
  (void)tls;
  memcpy(secrets_out, "int getRandomNumber(){return 4;}", 32);
  return 0;
...
authenticate_data_cleanup(const struct testcase_t *test, void *arg)
{
  ...
  UNMOCK(connection_or_write_var_cell_to_buf);
  UNMOCK(spider_tls_get_peer_cert);
  UNMOCK(spider_tls_get_tlssecrets);
  UNMOCK(connection_or_close_for_error);
  UNMOCK(channel_set_circid_type);
  UNMOCK(spider_tls_export_key_material);
...
{
  authenticate_data_t *d = spider_malloc_zero(sizeof(*d));
  ...
  MOCK(connection_or_write_var_cell_to_buf, mock_write_var_cell);
  MOCK(spider_tls_get_peer_cert, mock_get_peer_cert);
  MOCK(spider_tls_get_tlssecrets, mock_get_tlssecrets);
  MOCK(connection_or_close_for_error, mock_close_for_err);
  MOCK(channel_set_circid_type, mock_set_circid_type);
  MOCK(spider_tls_export_key_material, mock_export_key_material);
  d->c1 = or_connection_new(CONN_TYPE_OR, AF_INET);
  d->c2 = or_connection_new(CONN_TYPE_OR, AF_INET);
  spider_addr_from_ipv4h(&d->c1->base_.addr, 0x01020304);
  spider_addr_from_ipv4h(&d->c2->base_.addr, 0x05060708);
  ...
  d->chan2 = spider_malloc_zero(sizeof(*d->chan2));
  channel_tls_common_init(d->chan2);
  d->c2->chan = d->chan2;
  d->chan2->conn = d->c2;
  d->c2->base_.address = spider_strdup("C2");
  d->c2->tls = spider_tls_new(-1, 1);
  d->c2->handshake_state->received_certs_cell = 1;

  const spider_x509_cert_t *id_cert=NULL, *link_cert=NULL, *auth_cert=NULL;
  tt_assert(! spider_tls_get_my_certs(1, &link_cert, &id_cert));

  const uint8_t *der;
  size_t sz;
  spider_x509_cert_get_der(id_cert, &der, &sz);
  d->c1->handshake_state->certs->id_cert = spider_x509_cert_decode(der, sz);
  d->c2->handshake_state->certs->id_cert = spider_x509_cert_decode(der, sz);

  if (is_ed) {
    d->c1->handshake_state->certs->ed_id_sign =
      spider_cert_dup(get_master_signing_key_cert());
    d->c2->handshake_state->certs->ed_id_sign =
      spider_cert_dup(get_master_signing_key_cert());
    d->c2->handshake_state->certs->ed_sign_auth =
      spider_cert_dup(get_current_auth_key_cert());
  } else {
    tt_assert(! spider_tls_get_my_certs(0, &auth_cert, &id_cert));
    spider_x509_cert_get_der(auth_cert, &der, &sz);
    d->c2->handshake_state->certs->auth_cert = spider_x509_cert_decode(der, sz);
  }

  spider_x509_cert_get_der(link_cert, &der, &sz);
  mock_peer_cert = spider_x509_cert_decode(der, sz);
...
test_link_handshake_auth_cell(void *arg)
{
  ...
  if (d->is_ed) {
    ...
    auth_pubkey = spider_tls_cert_get_key(
...
AUTHENTICATE_FAIL(badtype,
                  require_failure_message = "Authenticaspider type was not "
                    "recognized";
                  d->cell->payload[0] = 0xff)
AUTHENTICATE_FAIL(truncated_1,
                  require_failure_message = "Authenticaspider was truncated";
                  d->cell->payload[2]++)
AUTHENTICATE_FAIL(truncated_2,
                  require_failure_message = "Authenticaspider was truncated";
                  d->cell->payload[3]++)
AUTHENTICATE_FAIL(tooshort_1,
                  require_failure_message = "Authenticaspider was too short";
...
AUTHENTICATE_FAIL(missing_ed_id,
                {
                  spider_cert_free(d->c2->handshake_state->certs->ed_id_sign);
...
AUTHENTICATE_FAIL(missing_ed_auth,
                {
                  spider_cert_free(d->c2->handshake_state->certs->ed_sign_auth);
