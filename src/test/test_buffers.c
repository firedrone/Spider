...
test_buffers_basic(void *arg)
{
  ...
  {
    char *mem = spider_malloc_zero(65536);
    buf = buf_new();
...
test_buffer_pullup(void *arg)
{
  ...
  size_t sz;
  (void)arg;
  stuff = spider_malloc(16384);
  tmp = spider_malloc(16384);
...
test_buffers_zlib_impl(int finalize_with_nil)
{
  ...
  char *expanded = NULL;
  buf_t *buf = NULL;
  spider_zlib_state_t *zlib_state = NULL;
  size_t out_len, in_len;
  int done;

  buf = buf_new_with_capacity(128); /* will round up */
  zlib_state = spider_zlib_new(1, ZLIB_METHOD, HIGH_COMPRESSION);

  msg = spider_malloc(512);
  crypto_rand(msg, 512);
  ...
  contents = spider_malloc(in_len);

  tt_int_op(fetch_from_buf(contents, in_len, buf), OP_EQ, 0);

  tt_int_op(0, OP_EQ, spider_gzip_uncompress(&expanded, &out_len,
                                       contents, in_len,
                                       ZLIB_METHOD, 1,
                                       LOG_WARN));
  ...
  buf_free(buf);
  spider_zlib_free(zlib_state);
  spider_free(contents);
  spider_free(expanded);
  spider_free(msg);
}
...
static int
mock_tls_read(spider_tls_t *tls, char *cp, size_t len)
{
  (void)tls;
...
test_buffers_tls_read_mocked(void *arg)
{
  ...
  mem = spider_malloc(64*1024);
  crypto_rand((char*)mem, 64*1024);
  tls_read_ptr = mem;
  n_remaining = 64*1024;

  MOCK(spider_tls_read, mock_tls_read);
...
test_buffers_find_contentlen(void *arg)
{
  ...
  for (i = 0; results[i].headers; ++i) {
    ...
    char * tmp = spider_memdup(results[i].headers, headerlen);/* ensure no eos */
