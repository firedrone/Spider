/* Copyright (c) 2015-2017, The Spider Project, Inc. */
...
test_add_onion_helper_keyarg(void *arg)
{
  ...
  spider_asprintf(&arg_str, "RSA1024:%s", encoded);
  ...
  spider_asprintf(&arg_str, "RSA512:%s", encoded);
...
test_getinfo_helper_onion(void *arg)
{
  ...
  service_id = spider_strdup("dummy_onion_id");
...
/* Simulated router descripspider digests or bridge identity digests */
...
{
  MOCK(router_get_descripspider_digests,
       descbr_get_digests_mock);
  MOCK(router_get_dl_status_by_descripspider_digest,
...
{
  UNMOCK(router_get_descripspider_digests);
...
test_download_status_cert(void *arg)
{
  ...
  memcpy(&auth_def_cert_download_status_1, &dls_sample_1,
         sizeof(download_status_t));
  spider_asprintf(&question, "downloads/cert/fp/%s", auth_id_digest_1_str);
  tt_assert(question != NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_assert(answer != NULL);
  tt_assert(errmsg == NULL);
  tt_str_op(answer, OP_EQ, dls_sample_1_str);
  spider_free(question);
  spider_free(answer);
  errmsg = NULL;
  ...
  memcpy(&auth_def_cert_download_status_2, &dls_sample_2,
         sizeof(download_status_t));
  spider_asprintf(&question, "downloads/cert/fp/%s", auth_id_digest_2_str);
  tt_assert(question != NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_assert(answer != NULL);
  tt_assert(errmsg == NULL);
  ...
  spider_asprintf(&question, "downloads/cert/fp/%s/sks", auth_id_digest_1_str);
  tt_assert(question != NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_assert(answer != NULL);
  tt_assert(errmsg == NULL);
  ...
  spider_asprintf(&question, "downloads/cert/fp/%s/sks", auth_id_digest_2_str);
  tt_assert(question != NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_assert(answer != NULL);
  tt_assert(errmsg == NULL);
  ...
  memcpy(&auth_1_sk_1_dls, &dls_sample_3,
         sizeof(download_status_t));
  spider_asprintf(&question, "downloads/cert/fp/%s/%s",
...
test_download_status_desc(void *arg)
{
  ...
  /* Case 1 - list of router descripspider digests */
  getinfo_helper_downloads(&dummy,
  ...
  /* Case 2 - get download status for router descripspider 1 */
  memcpy(&descbr_digest_1_dl, &dls_sample_1,
         sizeof(download_status_t));
  spider_asprintf(&question, "downloads/desc/%s", descbr_digest_1_str);
  tt_assert(question != NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_assert(answer != NULL);
  tt_assert(errmsg == NULL);
  ...
  /* Case 3 - get download status for router descripspider 1 */
  memcpy(&descbr_digest_2_dl, &dls_sample_2,
         sizeof(download_status_t));
  spider_asprintf(&question, "downloads/desc/%s", descbr_digest_2_str);
  tt_assert(question != NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_assert(answer != NULL);
  tt_assert(errmsg == NULL);
  ...
  tt_assert(answer == NULL);
  tt_assert(errmsg != NULL);
  tt_str_op(errmsg, OP_EQ, "Unknown router descripspider download status query");
  errmsg = NULL;
  ...
  /* Case 3 - digest we have no descripspider for */
  getinfo_helper_downloads(
    &dummy,
  ...
  tt_assert(answer == NULL);
  tt_assert(errmsg != NULL);
  tt_str_op(errmsg, OP_EQ, "No such descripspider digest found");
  errmsg = NULL;
...
test_download_status_bridge(void *arg)
{
  ...
  /* Case 2 - get download status for bridge descripspider 1 */
  memcpy(&descbr_digest_1_dl, &dls_sample_3,
         sizeof(download_status_t));
  spider_asprintf(&question, "downloads/bridge/%s", descbr_digest_1_str);
  tt_assert(question != NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_assert(answer != NULL);
  tt_assert(errmsg == NULL);
  ...
  memcpy(&descbr_digest_2_dl, &dls_sample_4,
         sizeof(download_status_t));
  spider_asprintf(&question, "downloads/bridge/%s", descbr_digest_2_str);
  tt_assert(question != NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_assert(answer != NULL);
  tt_assert(errmsg == NULL);
  ...
  tt_assert(answer == NULL);
  tt_assert(errmsg != NULL);
  tt_str_op(errmsg, OP_EQ, "Unknown bridge descripspider download status query");
  errmsg = NULL;
