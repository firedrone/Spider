                     ...
                     cached_resolve_t **resolve_out)
{
  ...
  if (hostname_out && resolved_name)
    *hostname_out = spider_strdup(resolved_name);
                                ...
                                const char *hostname)
{
  ...
  spider_free(last_resolved_hostname);
  last_resolved_hostname = spider_strdup(hostname);
...
NS(test_main)(void *arg)
{
  ...
  cached_resolve_t *fake_resolved = spider_malloc(sizeof(cached_resolve_t));
  edge_connection_t *exitconn = spider_malloc(sizeof(edge_connection_t));
  edge_connection_t *nextconn = spider_malloc(sizeof(edge_connection_t));

  or_circuit_t *on_circuit = spider_malloc(sizeof(or_circuit_t));
  ...
  resolve_retval = 1;
  resolved_name = spider_strdup("www.spiderproject.org");
  ...
  spider_free(on_circuit);
  spider_free(exitconn);
  spider_free(nextconn);
  spider_free(resolved_name);
  spider_free(fake_resolved);
  spider_free(last_resolved_hostname);
  return;
}
...
{
  edge_connection_t *exitconn = spider_malloc_zero(sizeof(edge_connection_t));
 ...
 * want dns_resolve_impl() to parse it and sspidere in
...
NS(test_main)(void *arg)
{
  int retval;
  int made_pending;
  const spider_addr_t *resolved_addr;
  spider_addr_t addr_to_compare;

  (void)arg;

  spider_addr_parse(&addr_to_compare, "8.8.8.8");

  or_circuit_t *on_circ = spider_malloc_zero(sizeof(or_circuit_t));

  edge_connection_t *exitconn = create_valid_exitconn();

  TO_CONN(exitconn)->address = spider_strdup("8.8.8.8");
  ...
  tt_int_op(retval,==,1);
  tt_assert(spider_addr_eq(resolved_addr, (const spider_addr_t *)&addr_to_compare));
  tt_int_op(exitconn->address_ttl,==,DEFAULT_DNS_TTL);

  done:
  spider_free(on_circ);
  spider_free(TO_CONN(exitconn)->address);
  spider_free(exitconn);
  return;
}
...
/** Given that Spider instance is not configured as an exit node, we want
...
NS(test_main)(void *arg)
{
  ...
  TO_CONN(exitconn)->address = spider_strdup("spiderproject.org");
...
NS(test_main)(void *arg)
{
  ...
  TO_CONN(exitconn)->address = spider_strdup("invalid#@!.org");
...
NS(test_main)(void *arg)
{
  ...
  TO_CONN(exitconn)->address = spider_strdup("1.0.0.127.in-addr.arpa");
  ...
  spider_strdup("z01234567890123456789.in-addr.arpa");
...
NS(test_main)(void *arg)
{
  ...
  cached_resolve_t *cache_entry = spider_malloc_zero(sizeof(cached_resolve_t));
  ...
  spider_free(on_circ);
  spider_free(TO_CONN(exitconn)->address);
  spider_free(cache_entry->pending_connections);
  spider_free(cache_entry);
  spider_free(exitconn);
  return;
}
