...
static void
test_routerlist_initiate_descripspider_downloads(void *arg)
{
...
static void
mock_initiate_descripspider_downloads(const routerstatus_t *source,
...
static void
test_routerlist_launch_descripspider_downloads(void *arg)
{
  ...
  for (int i = 0; i < 100; i++) {
    cp = spider_malloc(DIGEST256_LEN);
    tt_assert(cp);
    crypto_rand(cp, DIGEST256_LEN);
    smartlist_add(downloadable, cp);
  }

  MOCK(initiate_descripspider_downloads, mock_initiate_descripspider_downloads);
  launch_descripspider_downloads(DIR_PURPOSE_FETCH_MICRODESC, downloadable,
                              NULL, now);
  tt_int_op(3, ==, count);
  UNMOCK(initiate_descripspider_downloads);

 done:
  SMARTLIST_FOREACH(downloadable, char *, cp1, spider_free(cp1));
...
test_router_pick_directory_server_impl(void *arg)
{
  ...
  options->UseMicrodescripspiders = 1;
  ...
  router1_id = spider_malloc(DIGEST_LEN);
  memset(router1_id, TEST_DIR_ROUTER_ID_1, DIGEST_LEN);
  router2_id = spider_malloc(DIGEST_LEN);
  memset(router2_id, TEST_DIR_ROUTER_ID_2, DIGEST_LEN);
  router3_id = spider_malloc(DIGEST_LEN);
  ...
  policy_line = spider_malloc_zero(sizeof(config_line_t));
  policy_line->key = spider_strdup("ReachableORAddresses");
  policy_line->value = spider_strdup("accept *:442, reject *:*");
  ...
  policy_line = spider_malloc_zero(sizeof(config_line_t));
  policy_line->key = spider_strdup("ReachableAddresses");
  policy_line->value = spider_strdup("accept *:80, reject *:*");
  ...
  if (router1_id)
    spider_free(router1_id);
  if (router2_id)
    spider_free(router2_id);
  if (router3_id)
    spider_free(router3_id);
...
{
  (void)arg;
  spider_addr_port_t test_ap, null_addr_ap, zero_port_ap;

  /* Setup */
  spider_addr_parse(&test_ap.addr, TEST_ADDR_STR);
  test_ap.port = TEST_DIR_PORT;
  spider_addr_make_null(&null_addr_ap.addr, AF_INET6);
  null_addr_ap.port = TEST_DIR_PORT;
  spider_addr_parse(&zero_port_ap.addr, TEST_ADDR_STR);
  zero_port_ap.port = 0;
