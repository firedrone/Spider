...
client1(int argc, char **argv)
{
  ...
  if (onion_skin_nspider_create(node_id, &B, &state, msg)<0) {
...
server1(int argc, char **argv)
{
  ...
  keys = spider_malloc(keybytes);
  hexkeys = spider_malloc(keybytes*2+1);
...
{
  struct nspider_handshake_state_t state;
  ...
  keys = spider_malloc(keybytes);
  hexkeys = spider_malloc(keybytes*2+1);
  if (onion_skin_nspider_client_handshake(&state, msg, keys, keybytes, NULL)<0) {
    fprintf(stderr, "handshake failed");
    result = 2;
    goto done;
  }
