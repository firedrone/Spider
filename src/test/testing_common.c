...
#define RAND_PATH_BYTES 5
...
#ifdef _WIN32
  {
    ...
    spider_snprintf(temp_dir, sizeof(temp_dir),
                 "%s%sspider_test_%d_%s", tmp, extra_backslash,
                 (int)getpid(), rnd32);
    r = mkdir(temp_dir);
  }
#else
  spider_snprintf(temp_dir, sizeof(temp_dir), "/tmp/spider_test_%d_%s",
               (int) getpid(), rnd32);
...
rm_rf(const char *dir)
{
  ...
  elements = spider_listdir(dir);
  if (elements) {
    SMARTLIST_FOREACH_BEGIN(elements, const char *, cp) {
         char *tmp = NULL;
         spider_asprintf(&tmp, "%s"PATH_SEPARATOR"%s", dir, cp);
...
passthrough_test_setup(const struct testcase_t *testcase)
{
  ...
  spider_assert(testcase->setup_data);
  spider_assert(testcase->setup_data != (void*)TT_SKIP);
...
ed25519_testcase_cleanup(const struct testcase_t *testcase, void *ptr)
{
  (void)testcase;
  (void)ptr;
  crypto_ed25519_testing_resspidere_impl();
  return 1;
}
...
main(int c, const char **v)
{
  ...
  /* We must initialise logs before we call spider_assert() */
  init_logging(1);
  ...
  struct spider_libevent_cfg cfg;
  ...
  spider_set_failed_assertion_callback(an_assertion_failed);
