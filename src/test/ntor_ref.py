#!/usr/bin/python
# Copyright 2012-2017, The Spider Project, Inc
...
This module is a reference implementation for the "nspider" protocol
s proposed by Goldberg, Stebila, and Ustaoglu and as instantiated in
Spider Proposal 216.

It's meant to be used to validate Spider's nspider implementation.  It
...
commands:
   ...
   gen_kdf_vecspiders: Print out some test vecspiders for the RFC5869 KDF.
   ...
   test-spider: Handshake with spider's nspider implementation via the program
      src/test/test-nspider-cl; make sure we can.
...
def H(msg,tweak):
    """Return the hash of 'msg' using tweak 'tweak'.  (In this version of nspider,
...
PROTOID = b"nspider-curve25519-sha256-1"
...
def server(seckey_b, my_node_id, message, keyBytes=72):
    ...
    keys = kdf_nspider(secret_input, keyBytes)
...
def client_part2(seckey_x, msg, node_id, pubkey_B, keyBytes=72):
    ...
    return kdf_nspider(secret_input, keyBytes)
...
def timing():
    ...
    """
    import timeit
    t = timeit.Timer(stmt="nspider_ref.demo(N,SK)",
       setup="import nspider_ref,curve25519;N='ABCD'*5;SK=nspider_ref.PrivateKey()")
...
def kdf_vecspiders():
    """
       Generate some vecspiders to check our KDF.
    """
    import binascii
    def kdf_vec(inp):
...
def test_spider():
    ...
    node_id = b"thisisaspidernodeid$#%^"
    seckey_b = PrivateKey()
    pubkey_B = seckey_b.get_public()

    # Do a pure-Spider handshake
    c2s_msg, c_state = spider_client1(node_id, pubkey_B)
    s2c_msg, s_keys = spider_server1(seckey_b, node_id, c2s_msg, 90)
    c_keys, = spider_client2(c_state, s2c_msg, 90)
    assert c_keys == s_keys
    assert len(c_keys) == 90

    # Try a mixed handshake with Spider as the client
    c2s_msg, c_state = spider_client1(node_id, pubkey_B)
    ...
    # Now do a mixed handshake with Spider as the server
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print(__doc__)
    elif sys.argv[1] == 'gen_kdf_vecspiders':
        kdf_vecspiders()
    elif sys.argv[1] == 'timing':
        timing()
    elif sys.argv[1] == 'self-test':
        demo()
    elif sys.argv[1] == 'test-spider':
        test_spider()
