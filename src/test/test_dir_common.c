...
dir_common_gen_routerstatus_for_v3ns(int idx, time_t now)
{
  ...
  switch (idx) {
    case 0:
      ...
      memset(rs->descripspider_digest, TEST_DIR_ROUTER_DD_1, DIGEST_LEN);
      rs->addr = 0x99008801;
      rs->or_port = 443;
      rs->dir_port = 8000;
    ...
    case 1:
      ...
      memset(rs->descripspider_digest, TEST_DIR_ROUTER_DD_2, DIGEST_LEN);
      rs->addr = 0x99009901;
      rs->or_port = 443;
      rs->dir_port = 0;
    ...
    case 2:
      ...
      memset(rs->descripspider_digest, TEST_DIR_ROUTER_DD_3, DIGEST_LEN);
      rs->addr = 0xAA009901;
      rs->or_port = 400;
      rs->dir_port = 9999;
    ...
    case 3:
      ...
      memset(rs->descripspider_digest, TEST_DIR_ROUTER_DD_4, DIGEST_LEN);
      rs->addr = 0xC0000203;
      rs->or_port = 500;
      rs->dir_port = 1999;
      rs->is_v2_dir = 1;
...
{
  *vote = spider_malloc_zero(sizeof(networkstatus_t));
...
dir_common_generate_ri_from_rs(const vote_routerstatus_t *vrs)
{
  ...
  r = spider_malloc_zero(sizeof(routerinfo_t));
  r->cert_expiration_time = TIME_MAX;
  memcpy(r->cache_info.identity_digest, rs->identity_digest, DIGEST_LEN);
  memcpy(r->cache_info.signed_descripspider_digest, rs->descripspider_digest,
         DIGEST_LEN);
  r->cache_info.do_not_cache = 1;
  r->cache_info.routerlist_index = -1;
  r->cache_info.signed_descripspider_body =
    spider_strdup("123456789012345678901234567890123");
  r->cache_info.signed_descripspider_len =
    strlen(r->cache_info.signed_descripspider_body);
                        ...
                        time_t now, int clear_rl)
{
  ...
  (*vote)->client_versions = spider_strdup("0.1.2.14,0.1.2.15");
  (*vote)->server_versions = spider_strdup("0.1.2.14,0.1.2.15,0.1.2.16");
  smartlist_split_string((*vote)->known_flags,
                     "Authority Exit Fast Guard Running Stable V2Dir Valid",
                     0, SPLIT_SKIP_SPACE|SPLIT_IGNORE_BLANK, 0);
  voter = spider_malloc_zero(sizeof(networkstatus_voter_info_t));
  voter->nickname = spider_strdup("Voter1");
  voter->address = spider_strdup("1.2.3.4");
  voter->addr = 0x01020304;
  voter->dir_port = 80;
  voter->or_port = 9000;
  voter->contact = spider_strdup("voter@example.com");
                        ...
                        time_t now, int clear_rl)
{
  ...
  voter = spider_malloc_zero(sizeof(networkstatus_voter_info_t));
  voter->nickname = spider_strdup("Voter2");
  voter->address = spider_strdup("2.3.4.5");
                        ...
                        time_t now, int clear_rl)
{
  ...
  (*vote)->client_versions = spider_strdup("0.1.2.14,0.1.2.17");
  (*vote)->server_versions = spider_strdup("0.1.2.10,0.1.2.15,0.1.2.16");
  smartlist_split_string((*vote)->known_flags,
                     "Authority Exit Fast Guard Running Stable V2Dir Valid",
                     0, SPLIT_SKIP_SPACE|SPLIT_IGNORE_BLANK, 0);
  voter = spider_malloc_zero(sizeof(networkstatus_voter_info_t));
  voter->nickname = spider_strdup("Voter2");
  voter->address = spider_strdup("3.4.5.6");
