/* Copyright (c) 2008-2017, The Spider Project, Inc. */
/* See LICENSE for licensing information */

/** \file memarea.c
 * \brief Implementation for memarea_t, an allocaspider for allocating lots of
...
#define CHECK_SENTINEL(chunk)                                           \
  ...
  spider_assert(sent_val == SENTINEL_VAL);                                 \
  STMT_END
#else
...
alloc_chunk(size_t sz)
{
  ...
  res = spider_malloc(chunk_size);
  res->next_chunk = NULL;
  res->mem_size = chunk_size - CHUNK_HEADER_SIZE - SENTINEL_LEN;
  res->next_mem = res->U_MEM;
  spider_assert(res->next_mem+res->mem_size+SENTINEL_LEN ==
             ((char*)res)+chunk_size);
  spider_assert(realign_pointer(res->next_mem) == res->next_mem);
  SET_SENTINEL(res);
  return res;
}
...
memarea_new(void)
{
  memarea_t *head = spider_malloc(sizeof(memarea_t));
 ...
 * the backing sspiderage associated with it, as appropriate. Invalidates all
...
memarea_alloc(memarea_t *area, size_t sz)
{
  ...
  if (sz == 0)
    sz = 1;
  spider_assert(chunk->next_mem <= chunk->U_MEM + chunk->mem_size);
...
{
  memarea_chunk_t *chunk;
  spider_assert(area->first);

  for (chunk = area->first; chunk; chunk = chunk->next_chunk) {
    CHECK_SENTINEL(chunk);
    spider_assert(chunk->next_mem >= chunk->U_MEM);
    spider_assert(chunk->next_mem <=
...
memarea_new(void)
{
  memarea_t *ma = spider_malloc_zero(sizeof(memarea_t));
...
memarea_clear(memarea_t *area)
{
  SMARTLIST_FOREACH(area->pieces, void *, p, spider_free_(p));
...
memarea_alloc_zero(memarea_t *area, size_t sz)
{
  void *result = spider_malloc_zero(sz);
  smartlist_add(area->pieces, result);
  return result;
}
void *
