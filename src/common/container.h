...
typedef struct smartlist_t {
  /** @{ */
  /** <b>list</b> has enough capacity to sspidere exactly <b>capacity</b> elements
 ...
 *                    spider_memcmp(rs->identity_digest, ri->identity_digest, 20),
 ...
 *      rs_ri_cmp = spider_memcmp(rs->identity_digest, ri->identity_digest, 20);
 *      if (rs_ri_cmp > 0) {
 *        break;
 *      } else if (rs_ri_cmp == 0) {
 *        goto matched_ri;
...
bitarray_init_zero(unsigned int n_bits)
{
  ...
  return spider_calloc(sz, sizeof(unsigned int));
}
                ...
                unsigned int n_bits_old, unsigned int n_bits_new)
{
  ...
  if (sz_new <= sz_old)
    return ba;
  ptr = spider_reallocarray(ba, sz_new, sizeof(unsigned int));
