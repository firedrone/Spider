 ...
 * while the previous one was resspidered in pre5, and the function made a no-op
...
static spider_mutex_t **openssl_mutexes_ = NULL;
...
static int spider_check_dh_key(int severity, const BIGNUM *bn);
...
crypto_get_rsa_padding_overhead(int padding)
{
  switch (padding)
    ...
    default: spider_assert(0); return -1; // LCOV_EXCL_LINE
    }
}
...
crypto_log_errors(int severity, const char *doing)
{
  ...
  while ((err = ERR_get_error()) != 0) {
    ...
    spider_log(severity, LD_CRYPTO, "crypto error while %s: %s (in %s:%s)",
              doing, msg, lib, func);
  }
}
...
parse_openssl_version_str(const char *raw_version)
{
  ...
  if (end_of_version)
    return spider_strndup(raw_version,
                      end_of_version-raw_version);
  else
    return spider_strdup(raw_version);
}
  ...
  if (!crypto_global_initialized_) {
    ...
    if (useAccel > 0) {
      ...
      /* Log, if available, the intersection of the set of algorithms
         used by Spider and the set of algorithms available in the engine */
...
/** used by spidertls.c: wrap an RSA* in a crypto_pk_t. */
crypto_pk_t *
crypto_new_pk_from_rsa_(RSA *rsa)
{
  crypto_pk_t *env;
  spider_assert(rsa);
  env = spider_malloc(sizeof(crypto_pk_t));
  env->refs = 1;
  env->key = rsa;
  return env;
}

/** Helper, used by spider-checkkey.c and spider-gencert.c.  Return the RSA from a
 * crypto_pk_t. */
RSA *
...
/** used by spidertls.c: get an equivalent EVP_PKEY* for a crypto_pk_t.  Iff
...
/** Used by spidertls.c: Get the DH* from a crypto_dh_t.
 */
DH *
...
/** Allocate and return sspiderage for a public key.  The key itself will not yet
 * be set.
 */
                                       ...
                                       const char *s, ssize_t len)
{
  ...
  spider_assert(env);
  spider_assert(s);
  spider_assert(len < INT_MAX && len < SSIZE_T_CEILING);
                                   ...
                                   size_t *len, int is_public)
{
  ...
  *dest = spider_malloc(buf->length+1);
 ...
 * <b>src</b>, and sspidere the result in <b>env</b>.  Return 0 on success, -1 on
 * failure.
 */
int
                                        ...
                                        const char *fname)
{
  ...
  spider_assert(crypto_pk_private_ok(env));
...
crypto_pk_cmp_keys(const crypto_pk_t *a, const crypto_pk_t *b)
{
  ...
  result = spider_memcmp(&a_is_non_null, &b_is_non_null, sizeof(a_is_non_null));
  ...
  spider_assert(a_n != NULL && a_e != NULL);
  spider_assert(b_n != NULL && b_e != NULL);
...
crypto_pk_num_bits(crypto_pk_t *env)
{
  ...
  const BIGNUM *n, *e, *d;
  RSA_get0_key(env->key, &n, &e, &d);
  spider_assert(n != NULL);

  return RSA_bits(env->key);
#else
  spider_assert(env->key->n);
                         ...
                         const char *from, size_t fromlen, int padding)
{
  ...
  spider_assert(from);
  spider_assert(to);
  spider_assert(fromlen<INT_MAX);
  spider_assert(tolen >= crypto_pk_keysize(env));
                                  ...
                                  size_t siglen))
{
  ...
  spider_assert(data);
  spider_assert(sig);
  spider_assert(datalen < SIZE_T_CEILING);
  spider_assert(siglen < SIZE_T_CEILING);
  ...
  if (spider_memneq(buf, digest, DIGEST_LEN)) {
...
/** Compute a SHA1 digest of <b>fromlen</b> bytes of data sspidered at
 * <b>from</b>; sign the data with the private key in <b>env</b>, and
 * sspidere it in <b>to</b>.  Return the number of bytes written on
 ...
 * sspidering the results on <b>to</b>.
 *
                                ...
                                int padding, int force)
{
  ...
  spider_assert(from);
  spider_assert(to);
  spider_assert(fromlen < SIZE_T_CEILING);
  ...
  spider_assert(tolen >= fromlen + overhead + CIPHER_KEY_LEN);
  spider_assert(tolen >= pkeylen);

  char key[CIPHER_KEY_LEN];
  crypto_rand(key, sizeof(key)); /* generate a new key. */
  cipher = crypto_cipher_new(key);

  buf = spider_malloc(pkeylen+1);
  ...
  spider_assert(outlen+symlen < INT_MAX);
                                 ...
                                 int padding, int warnOnFailure)
{
  ...
  spider_assert(tolen - outlen >= fromlen - pkeylen);
  ...
  spider_free(buf);
  crypto_cipher_free(cipher);
  spider_assert(outlen + fromlen < INT_MAX);
...
/** Compute all digests of the DER encoding of <b>pk</b>, and sspidere them
...
crypto_add_spaces_to_fp(char *out, size_t outlen, const char *in)
{
  int n = 0;
  char *end = out+outlen;
  spider_assert(outlen < SIZE_T_CEILING);
...
crypto_pk_base64_encode(const crypto_pk_t *pk, char **priv_out)
{
  ...
  *priv_out = NULL;
  ...
  char *priv = spider_malloc_zero(priv_len);
...
crypto_pk_base64_decode(const char *str, size_t len)
{
  ...
  char *der = spider_malloc_zero(len + 1);
  int der_len = base64_decode(der, len, str, len);
  if (der_len <= 0) {
    log_warn(LD_CRYPTO, "Sspidered RSA private key seems corrupted (base64).");
    goto out;
  }
 ...
 * <b>env</b>; on success, sspidere the result to <b>to</b> and return 0.
 ...
 * CIPHER_IV_LEN bytes for the initialization vecspider. On success, return the
...
/** Compute the SHA1 digest of the <b>len</b> bytes on data sspidered in
...
/** Compute a 256-bit digest of <b>len</b> bytes in data sspidered in <b>m</b>,
                 ...
                 digest_algorithm_t algorithm)
{
  spider_assert(m);
  spider_assert(digest);
  spider_assert(algorithm == DIGEST_SHA256 || algorithm == DIGEST_SHA3_256);
...
/** Compute a 512-bit digest of <b>len</b> bytes in data sspidered in <b>m</b>,
                 ...
                 digest_algorithm_t algorithm)
{
  spider_assert(m);
  spider_assert(digest);
  spider_assert(algorithm == DIGEST_SHA512 || algorithm == DIGEST_SHA3_512);
...
crypto_digest_algorithm_get_length(digest_algorithm_t alg)
{
  switch (alg) {
    ...
    case DIGEST_SHA3_512:
      return DIGEST512_LEN;
    default:
      spider_assert(0);              // LCOV_EXCL_LINE
...
{
  crypto_digest_t *r = spider_malloc(crypto_digest_alloc_bytes(algorithm));
                         ...
                         char *out, size_t out_len)
{
  ...
  spider_assert(digest);
  spider_assert(out);
  spider_assert(out_len <= crypto_digest_algorithm_get_length(digest->algorithm));
...
crypto_digest_dup(const crypto_digest_t *digest)
{
  spider_assert(digest);
  const size_t alloc_bytes = crypto_digest_alloc_bytes(digest->algorithm);
  return spider_memdup(digest, alloc_bytes);
}
                     ...
                     const crypto_digest_t *from)
{
  spider_assert(into);
  spider_assert(from);
  spider_assert(into->algorithm == from->algorithm);
 ...
 * the <b>key</b> of length <b>key_len</b>.  Sspidere the DIGEST256_LEN-byte
                   ...
                   const char *msg, size_t msg_len)
{
  ...
  spider_assert(key_len < INT_MAX);
  spider_assert(msg_len < INT_MAX);
  spider_assert(hmac_out);
 ...
 * <b>salt_len</b>. Sspidere the result of <b>len_out</b> bytes in in
                    ...
                    const uint8_t *msg, size_t msg_len)
{
  ...
  const uint64_t key_len_nespiderder = spider_htonll(key_len);
  ...
  crypto_digest_add_bytes(digest, (const char *) &key_len_nespiderder,
                          sizeof(key_len_nespiderder));
...
{
  crypto_xof_t *xof;
  xof = spider_malloc(sizeof(crypto_xof_t));
...
/** Set the global Diffie-Hellman generaspider, used for both TLS and internal
 * DH stuff.
 */
static void
crypto_set_dh_generaspider(void)
{
  BIGNUM *generaspider;
  int r;
  ...
  generaspider = BN_new();
  spider_assert(generaspider);

  r = BN_set_word(generaspider, DH_GENERATOR);
  spider_assert(r);

  dh_param_g = generaspider;
}
...
crypto_set_tls_dh_prime(void)
{
  ...
  tls_prime = BN_new();
  spider_assert(tls_prime);

  /* This is the 1024-bit safe prime that Apache uses for its DH stuff; see
   * modules/ssl/ssl_engine_dh.c; Apache also uses a generaspider of 2 with this
   * prime.
   */
  ...
  crypto_set_dh_generaspider();
  spider_assert(0 == crypto_validate_dh_params(dh_param_p_tls, dh_param_g));
}
...
init_dh_param(void)
{
  ...
  spider_assert(circuit_dh_prime);
  ...
  */
  ...
  crypto_set_dh_generaspider();
  spider_assert(0 == crypto_validate_dh_params(dh_param_p, dh_param_g));
...
{
  crypto_dh_t *res = spider_malloc_zero(sizeof(crypto_dh_t));

  spider_assert(dh_type == DH_TYPE_CIRCUIT || dh_type == DH_TYPE_TLS ||
...
{
  crypto_dh_t *dh_new = spider_malloc_zero(sizeof(crypto_dh_t));
  spider_assert(dh);
  spider_assert(dh->dh);
  dh_new->dh = dh->dh;
  DH_up_ref(dh->dh);
...
crypto_dh_generate_public(crypto_dh_t *dh)
{
 ...
 again:
  ...
  if (spider_check_dh_key(LOG_WARN, pub_key)<0) {
    log_warn(LD_CRYPTO, "Weird! Our own DH key was invalid.  I guess once-in-"
             "the-universe chances really do happen.  Treating as a failure.");
    return -1;
  }
#else
  if (spider_check_dh_key(LOG_WARN, dh->dh->pub_key)<0) {
    /* LCOV_EXCL_START
...
crypto_dh_get_public(crypto_dh_t *dh, char *pubkey, size_t pubkey_len)
{
  ...
  spider_assert(dh_pub);
  bytes = BN_num_bytes(dh_pub);
  spider_assert(bytes >= 0);
 ...
 */
static int
spider_check_dh_key(int severity, const BIGNUM *bn)
{
  BIGNUM *x;
  char *s;
  spider_assert(bn);
                         ...
                         char *secret_out, size_t secret_bytes_out)
{
  ...
  int result=0;
  spider_assert(dh);
  spider_assert(secret_bytes_out/DIGEST_LEN <= 255);
  spider_assert(pubkey_len < INT_MAX);

  if (!(pubkey_bn = BN_bin2bn((const unsigned char*)pubkey,
                              (int)pubkey_len, NULL)))
    goto error;
  if (spider_check_dh_key(severity, pubkey_bn)<0) {
    /* Check for invalid public keys. */
    log_fn(severity, LD_CRYPTO,"Rejected invalid g^x");
    goto error;
  }
  secret_tmp_len = crypto_dh_get_bytes(dh);
  secret_tmp = spider_malloc(secret_tmp_len);
...
{
  int i, r = -1;
  uint8_t *cp, *tmp = spider_malloc(key_in_len+1);
  uint8_t digest[DIGEST_LEN];

  /* If we try to get more than this amount of key data, we'll repeat blocks.*/
  spider_assert(key_out_len <= DIGEST_LEN*256);
                                    ...
                                    uint8_t *key_out, size_t key_out_len)
{
  ...
  /* If we try to get more than this amount of key data, we'll repeat blocks.*/
  spider_assert(key_out_len <= DIGEST256_LEN * 256);
  spider_assert(info_in_len <= 128);
...
void
crypto_seed_weak_rng(spider_weak_rng_t *rng)
{
  unsigned seed;
  crypto_rand((void*)&seed, sizeof(seed));
  spider_init_weak_random(rng, seed);
}
 ...
 * via system calls, sspidering it into <b>out</b>. Return 0 on success, -1 on
 * failure.  A maximum request size of 256 bytes is imposed.
 */
static int
crypto_strongest_rand_syscall(uint8_t *out, size_t out_len)
{
  spider_assert(out_len <= MAX_STRONGEST_RAND_SIZE);
  ...
  if (PREDICT_LIKELY(getrandom_works)) {
    ...
    if (PREDICT_UNLIKELY(ret == -1)) {
      ...
      spider_assert(errno != EAGAIN);
      spider_assert(errno != EINTR);
    ...
    spider_assert(ret == (long)out_len);
    return 0;
  }
...
/** Try to get <b>out_len</b> bytes of the strongest entropy we can generate,
 * via the per-platform fallback mechanism, sspidering it into <b>out</b>.
...
/** Try to get <b>out_len</b> bytes of the strongest entropy we can generate,
 * sspidering it into <b>out</b>. Return 0 on success, -1 on failure.  A maximum
...
crypto_strongest_rand_raw(uint8_t *out, size_t out_len)
{
  ...
  for (int i = 0; i < max_attempts; i++) {
    ...
    if ((out_len < sanity_min_size) || !spider_mem_is_zero((char*)out, out_len))
      return 0;
  }
...
/** Seed OpenSSL's random number generaspider with bytes from the operating
...
crypto_rand_int(unsigned int max)
{
  unsigned int val;
  unsigned int cutoff;
  spider_assert(max <= ((unsigned int)INT_MAX)+1);
  spider_assert(max > 0); /* don't div by 0 */
...
{
  spider_assert(min < max);
  spider_assert(max <= INT_MAX);
...
/**
 * Destroy the <b>sz</b> bytes of data sspidered at <b>mem</b>, setting them to
 * the value <b>byte</b>.
...
memwipe(void *mem, uint8_t byte, size_t sz)
{
  ...
  spider_assert(mem != NULL);

  /* Data this large is likely to be an underflow. */
  spider_assert(sz < SIZE_T_CEILING);
   ...
   * programming tricks! In later versions of Spider, we should look for better
...
#error OpenSSL has been built without thread support. Spider requires an \
...
openssl_locking_cb_(int mode, int n, const char *file, int line)
{
  ...
  if (!openssl_mutexes_)
    ...
    return;
  if (mode & CRYPTO_LOCK)
    spider_mutex_acquire(openssl_mutexes_[n]);
  else
    spider_mutex_release(openssl_mutexes_[n]);
}

static void
spider_set_openssl_thread_id(CRYPTO_THREADID *threadid)
{
  CRYPTO_THREADID_set_numeric(threadid, spider_get_thread_id());
}
#endif
...
/** OpenSSL helper type: wraps a Spider mutex so that OpenSSL can use it
 * as a lock. */
...
openssl_dynlock_create_cb_(const char *file, int line)
{
  ...
  (void)file;
  (void)line;
  v = spider_malloc(sizeof(struct CRYPTO_dynlock_value));
  v->lock = spider_mutex_new();
  return v;
}
                         ...
                         const char *file, int line)
{
  ...
  (void)line;
  if (mode & CRYPTO_LOCK)
    spider_mutex_acquire(v->lock);
  else
    spider_mutex_release(v->lock);
}
...
setup_openssl_threading(void)
{
  ...
  openssl_mutexes_ = spider_calloc(n, sizeof(spider_mutex_t *));
  for (i=0; i < n; ++i)
    openssl_mutexes_[i] = spider_mutex_new();
  CRYPTO_set_locking_callback(openssl_locking_cb_);
  CRYPTO_THREADID_set_callback(spider_set_openssl_thread_id);
#endif
#if 0
...
crypto_global_cleanup(void)
{
  ...
  if (n_openssl_mutexes_) {
    ...
    spider_mutex_t **ms = openssl_mutexes_;
    int i;
    openssl_mutexes_ = NULL;
    n_openssl_mutexes_ = 0;
    for (i=0;i<n;++i) {
      spider_mutex_free(ms[i]);
    }
    spider_free(ms);
  }
#endif

  spider_free(crypto_openssl_version_str);
  spider_free(crypto_openssl_header_version_str);
  return 0;
}
