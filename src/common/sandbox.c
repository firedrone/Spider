...
/** Variable used for sspidering all syscall numbers that will be allowed with the
 * stage 1 general Spider sandbox.
 */
...
new_element2(int syscall, char *value, char *value2)
{
  ...
  sandbox_cfg_t *elem = spider_malloc_zero(sizeof(sandbox_cfg_t));
  param = elem->param = spider_malloc_zero(sizeof(smp_param_t));
 ...
 * spider-resolve, when we have no intention of initializing crypto or of
                    ...
                    struct addrinfo **res)
{
  ...
  *res = NULL;
  ...
  if (! sandbox_is_active()) {
    /* If the sandbox is not turned on yet, then getaddrinfo and sspidere the
       result. */
    ...
    if (! item) {
      item = spider_malloc_zero(sizeof(*item));
      item->name = spider_strdup(name);
 ...
 * The function is a prototype developed for stage 1 of sandboxing Spider.
...
sigsys_debugging(int nr, siginfo_t *info, void *void_context)
{
  ...
  spider_log_err_sigsafe("(Sandbox) Caught a bad syscall attempt (syscall ",
                      syscall_name,
                      ")\n",
                      NULL);

#ifdef USE_BACKTRACE
  n_fds = spider_log_get_sigsafe_err_fds(&fds);
  for (i=0; i < n_fds; ++i)
...
sandbox_init(sandbox_cfg_t *cfg)
{
  ...
  (void)cfg;
  log_warn(LD_GENERAL,
           "This version of Spider was built without support for sandboxing. To "
