...
/** Version of Spider to report in backtrace messages. */
...
static spider_mutex_t cb_buf_mutex;
...
log_backtrace(int severity, int domain, const char *msg)
{
  ...
  spider_mutex_acquire(&cb_buf_mutex);

  depth = backtrace(cb_buf, MAX_DEPTH);
  symbols = backtrace_symbols(cb_buf, (int)depth);

  spider_log(severity, domain, "%s. Stack trace:", msg);
  if (!symbols) {
    /* LCOV_EXCL_START -- we can't provoke this. */
    spider_log(severity, domain, "    Unable to generate backtrace.");
    goto done;
    /* LCOV_EXCL_STOP */
  }
  for (i=0; i < depth; ++i) {
    spider_log(severity, domain, "    %s", symbols[i]);
  }
  raw_free(symbols);

 done:
  spider_mutex_release(&cb_buf_mutex);
}
...
crash_handler(int sig, siginfo_t *si, void *ctx_)
{
  ...
  spider_log_err_sigsafe(bt_version, " died: Caught signal ", buf, "\n",
                      NULL);
...
install_bt_handler(void)
{
  ...
  spider_mutex_init(&cb_buf_mutex);
...
{
  spider_mutex_uninit(&cb_buf_mutex);
}
#endif
...
log_backtrace(int severity, int domain, const char *msg)
{
  spider_log(severity, domain, "%s. (Stack trace not available)", msg);
}
...
int
configure_backtrace_handler(const char *spider_version)
{
  spider_free(bt_version);
  if (spider_version)
    spider_asprintf(&bt_version, "Spider %s", spider_version);
  else
    spider_asprintf(&bt_version, "Spider");
