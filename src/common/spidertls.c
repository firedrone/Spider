...
/* (Unlike other spider functions, these
 * are prefixed with spider_ in order to avoid conflicting with OpenSSL
...
/** Return values for spider_tls_classify_client_ciphers.
 *
 * @{
 */
...
/** The ex_data index in which we sspidere a pointer to an SSL object's
 * corresponding spider_tls_t object. */
STATIC int spider_tls_object_ex_data_index = -1;

/** Helper: Allocate spider_tls_object_ex_data_index. */
STATIC void
spider_tls_allocate_spider_tls_object_ex_data_index(void)
{
  if (spider_tls_object_ex_data_index == -1) {
    spider_tls_object_ex_data_index =
      SSL_get_ex_new_index(0, NULL, NULL, NULL, NULL);
    spider_assert(spider_tls_object_ex_data_index != -1);
  }
}

/** Helper: given a SSL* pointer, return the spider_tls_t object using that
 * pointer. */
STATIC spider_tls_t *
spider_tls_get_by_ssl(const SSL *ssl)
{
  spider_tls_t *result = SSL_get_ex_data(ssl, spider_tls_object_ex_data_index);
  if (result)
    spider_assert(result->magic == TOR_TLS_MAGIC);
  return result;
}

static void spider_tls_context_decref(spider_tls_context_t *ctx);
static void spider_tls_context_incref(spider_tls_context_t *ctx);
 ...
 *
 * @{ */
STATIC spider_tls_context_t *server_tls_context = NULL;
STATIC spider_tls_context_t *client_tls_context = NULL;
/**@}*/

/** True iff spider_tls_init() has been called. */
...
void
spider_tls_get_state_description(spider_tls_t *tls, char *buf, size_t sz)
{
  const char *ssl_state;
  const char *spidertls_state;
  ...
  switch (tls->state) {
#define CASE(st) case TOR_TLS_ST_##st: spidertls_state = " in "#st ; break
    CASE(HANDSHAKE);
    CASE(OPEN);
    CASE(GOTCLOSE);
    CASE(SENTCLOSE);
...
#undef CASE
  case TOR_TLS_ST_BUFFEREVENT:
    ...
    break;
  default:
    spidertls_state = " in unknown TLS state";
    break;
  }

  spider_snprintf(buf, sz, "%s%s", ssl_state, spidertls_state);
}
...
void
spider_tls_log_one_error(spider_tls_t *tls, unsigned long err,
                  int severity, int domain, const char *doing)
{
  ...
  if (!func) func = "(null)";
  if (doing) {
    spider_log(severity, domain, "TLS error while %s%s%s: %s (in %s:%s:%s)",
        doing, addr?" with ":"", addr?addr:"",
        msg, lib, func, state);
  } else {
    spider_log(severity, domain, "TLS error%s%s: %s (in %s:%s:%s)",
        addr?" with ":"", addr?addr:"",
        msg, lib, func, state);
  }
}
 ...
 */
STATIC void
tls_log_errors(spider_tls_t *tls, int severity, int domain, const char *doing)
{
  unsigned long err;

  while ((err = ERR_get_error()) != 0) {
    spider_tls_log_one_error(tls, err, severity, domain, doing);
  }
}
 ...
 * code. */
STATIC int
spider_errno_to_tls_error(int e)
{
  switch (e) {
...
const char *
spider_tls_err_to_string(int err)
{
  if (err >= 0)
    return "[Not an error.]";
  switch (err) {
 ...
 */
STATIC int
spider_tls_get_error(spider_tls_t *tls, int r, int extra,
                  const char *doing, int severity, int domain)
{
  int err = SSL_get_error(tls->ssl, r);
  int spider_error = TOR_TLS_ERROR_MISC;
  switch (err) {
    case SSL_ERROR_NONE:
      return TOR_TLS_DONE;
    ...
    case SSL_ERROR_SYSCALL:
      ...
      if (r == 0) {
        spider_log(severity, LD_NET, "TLS error: unexpected close while %s (%s)",
            doing, SSL_state_string_long(tls->ssl));
        spider_error = TOR_TLS_ERROR_IO;
      } else {
        int e = spider_socket_errno(tls->socket);
        spider_log(severity, LD_NET,
            "TLS error: <syscall error while %s> (errno=%d: %s; state=%s)",
            doing, e, spider_socket_strerror(e),
            SSL_state_string_long(tls->ssl));
        spider_error = spider_errno_to_tls_error(e);
      }
    ...
    case SSL_ERROR_ZERO_RETURN:
      ...
      spider_log(severity, LD_NET, "TLS connection closed while %s in state %s",
          doing, SSL_state_string_long(tls->ssl));
      tls_log_errors(tls, severity, domain, doing);
...
void
spider_tls_free_all(void)
{
  check_no_tls_errors();

  if (server_tls_context) {
    spider_tls_context_t *ctx = server_tls_context;
    server_tls_context = NULL;
    spider_tls_context_decref(ctx);
  }
  if (client_tls_context) {
    spider_tls_context_t *ctx = client_tls_context;
...
static X509_NAME *
spider_x509_name_new(const char *cname)
{
  int nid;
  X509_NAME *name;
...
MOCK_IMPL(STATIC X509 *,
          spider_tls_create_certificate,(crypto_pk_t *rsa,
...
#define SERIAL_NUMBER_SIZE 8
  ...
  spider_assert(rsa);
  spider_assert(cname);
  spider_assert(rsa_sign);
  spider_assert(cname_sign);
  ...
  if (!(name = spider_x509_name_new(cname)))
    goto error;
  if (!(X509_set_subject_name(x509, name)))
    goto error;
  if (!(name_issuer = spider_x509_name_new(cname_sign)))
    goto error;
 ...
 * disabled, set your Spiders' cipher list to
 * (SSL3_TXT_RSA_NULL_SHA).  If you do this, you won't be able to communicate
 * with any of the "real" Spiders, though. */
...
/** Free all sspiderage held in <b>cert</b> */
void
spider_x509_cert_free(spider_x509_cert_t *cert)
{
  if (! cert)
    return;
  if (cert->cert)
    X509_free(cert->cert);
  spider_free(cert->encoded);
...
/**
 * Allocate a new spider_x509_cert_t to hold the certificate "x509_cert".
 *
 * Steals a reference to x509_cert.
 */
MOCK_IMPL(STATIC spider_x509_cert_t *,
          spider_x509_cert_new,(X509 *x509_cert))
{
  ...
  cert->encoded = spider_malloc(length);
...
spider_x509_cert_t *
spider_x509_cert_dup(const spider_x509_cert_t *cert)
{
  return spider_x509_cert_new(X509_dup(cert->cert));
}

/** Read a DER-encoded X509 cert, of length exactly <b>certificate_len</b>,
 * from a <b>certificate</b>.  Return a newly allocated spider_x509_cert_t on
 * success and NULL on failure. */
spider_x509_cert_t *
spider_x509_cert_decode(const uint8_t *certificate, size_t certificate_len)
{
  X509 *x509;
  const unsigned char *cp = (const unsigned char *)certificate;
  spider_x509_cert_t *newcert;
  spider_assert(certificate);
  check_no_tls_errors();
  ...
  newcert = spider_x509_cert_new(x509);
  if (!newcert) {
    goto err;
  }
...
void
spider_x509_cert_get_der(const spider_x509_cert_t *cert,
                 const uint8_t **encoded_out, size_t *size_out)
{
  spider_assert(cert);
  spider_assert(encoded_out);
  spider_assert(size_out);
...
spider_x509_cert_get_id_digests(const spider_x509_cert_t *cert)
{
...
const common_digests_t *
spider_x509_cert_get_cert_digests(const spider_x509_cert_t *cert)
{
 ...
 * references. */
static void
spider_tls_context_decref(spider_tls_context_t *ctx)
{
  spider_assert(ctx);
  if (--ctx->refcnt == 0) {
    SSL_CTX_free(ctx->ctx);
    spider_x509_cert_free(ctx->my_link_cert);
    spider_x509_cert_free(ctx->my_id_cert);
    spider_x509_cert_free(ctx->my_auth_cert);
...
int
spider_tls_get_my_certs(int server,
                     const spider_x509_cert_t **link_cert_out,
                     const spider_x509_cert_t **id_cert_out)
{
  spider_tls_context_t *ctx = server ? server_tls_context : client_tls_context;
  if (! ctx)
    return -1;
  if (link_cert_out)
 ...
 */
crypto_pk_t *
spider_tls_get_my_client_auth_key(void)
{
  if (! client_tls_context)
    return NULL;
 ...
 */
crypto_pk_t *
spider_tls_cert_get_key(spider_x509_cert_t *cert)
{
 ...
 */
MOCK_IMPL(int,
spider_tls_cert_matches_key,(const spider_tls_t *tls, const spider_x509_cert_t *cert))
{
...
int
spider_tls_cert_is_valid(int severity,
...
static void
spider_tls_context_incref(spider_tls_context_t *ctx)
{
  ++ctx->refcnt;
}
...
int
spider_tls_context_init(unsigned flags,
                     ...
                     unsigned int key_lifetime)
{
  ...
  if (is_public_server) {
    spider_tls_context_t *new_ctx;
    spider_tls_context_t *old_ctx;

    spider_assert(server_identity != NULL);

    rv1 = spider_tls_context_init_one(&server_tls_context,
                                   server_identity,
                                   key_lifetime, flags, 0);

    if (rv1 >= 0) {
      new_ctx = server_tls_context;
      spider_tls_context_incref(new_ctx);
      old_ctx = client_tls_context;
      client_tls_context = new_ctx;

      if (old_ctx != NULL) {
        spider_tls_context_decref(old_ctx);
      }
    }
  } else {
    if (server_identity != NULL) {
      ...
      spider_tls_context_t *old_ctx = server_tls_context;
      server_tls_context = NULL;
    ...
    rv2 = spider_tls_context_init_one(&client_tls_context,
                                   client_identity,
                                   key_lifetime,
                                   flags,
                                   1);
 ...
 */
STATIC int
spider_tls_context_init_one(spider_tls_context_t **ppcontext,
                         crypto_pk_t *identity,
                         unsigned int key_lifetime,
                         unsigned int flags,
                         int is_client)
{
  spider_tls_context_t *new_ctx = spider_tls_context_new(identity,
                                                   key_lifetime,
                                                   flags,
                                                   is_client);
  spider_tls_context_t *old_ctx = *ppcontext;
  ...
  if (new_ctx != NULL) {
    *ppcontext = new_ctx;
    ...
    if (old_ctx != NULL) {
       ...
       * count spider_tls_context_t objects. */
      spider_tls_context_decref(old_ctx);
    }
  }
...
#define  NID_spider_default_ecdhe_group NID_X9_62_prime256v1

#define RSA_LINK_KEY_BITS 2048

/** Create a new TLS context for use with Spider TLS handshakes.
 * <b>identity</b> should be set to the identity key used to sign the
 * certificate.
 */
STATIC spider_tls_context_t *
spider_tls_context_new(crypto_pk_t *identity, unsigned int key_lifetime,
                    unsigned flags, int is_client)
{
  crypto_pk_t *rsa = NULL, *rsa_auth = NULL;
  EVP_PKEY *pkey = NULL;
  spider_tls_context_t *result = NULL;
  ...
  if (!is_client) {
    ...
    cert = spider_tls_create_certificate(rsa, identity, nickname, nn2,
                                      key_lifetime);
    /* Create self-signed certificate for identity key. */
    idcert = spider_tls_create_certificate(identity, identity, nn2, nn2,
                                        IDENTITY_CERT_LIFETIME);
    /* Create an authentication certificate signed by identity key. */
    authcert = spider_tls_create_certificate(rsa_auth, identity, nickname, nn2,
                                          key_lifetime);
  ...
  result = spider_malloc_zero(sizeof(spider_tls_context_t));
  result->refcnt = 1;
  if (!is_client) {
    result->my_link_cert = spider_x509_cert_new(X509_dup(cert));
    result->my_id_cert = spider_x509_cert_new(X509_dup(idcert));
    result->my_auth_cert = spider_x509_cert_new(X509_dup(authcert));
  ...
  * hisspiderically been chosen for fingerprinting resistance. */
   ...
   * distinguishability vecspider.  This can give us worse PFS, though, if we
  ...
  if (! is_client) {
    ...
    cert=NULL;
    if (idcert) {
      X509_STORE *s = SSL_CTX_get_cert_sspidere(result->ctx);
      spider_assert(s);
  ...
  if (! is_client) {
    ...
    if (flags & TOR_TLS_CTX_USE_ECDHE_P224)
      ...
      nid = NID_spider_default_ecdhe_group;
    /* Use P-256 for ECDHE. */
 ...
 error:
  ...
  if (rsa_auth)
    crypto_pk_free(rsa_auth);
  if (result)
    spider_tls_context_decref(result);
  if (cert)
    X509_free(cert);
  if (idcert)
    X509_free(idcert);
...
STATIC void
spider_tls_debug_state_callback(const SSL *ssl, int type, int val)
{
...
const char *
spider_tls_get_ciphersuite_name(spider_tls_t *tls)
{
...
find_cipher_by_id(const SSL *ssl, const SSL_METHOD *m, uint16_t cipher)
{
  ...
  (void) m;
  {
    ...
    if (c)
      spider_assert((SSL_CIPHER_get_id(c) & 0xffff) == cipher);
    return c != NULL;
  }
#else
...
# if defined(HAVE_STRUCT_SSL_METHOD_ST_GET_CIPHER_BY_CHAR)
  if (m && m->get_cipher_by_char) {
    ...
    if (c)
      spider_assert((c->id & 0xffff) == cipher);
    return c != NULL;
  }
# endif
# ifndef OPENSSL_1_1_API
 ...
 **/
STATIC int
spider_tls_classify_client_ciphers(const SSL *ssl,
                                STACK_OF(SSL_CIPHER) *peer_ciphers)
{
  int i, res;
  spider_tls_t *spider_tls;
  if (PREDICT_UNLIKELY(!v2_cipher_list_pruned))
    prune_v2_cipher_list(ssl);

  spider_tls = spider_tls_get_by_ssl(ssl);
  if (spider_tls && spider_tls->client_cipher_list_type)
    return spider_tls->client_cipher_list_type;
   ...
   * dealing with an updated Spider. */
 ...
 v2_or_higher:
  {
    ...
    for (i = 0; i < sk_SSL_CIPHER_num(peer_ciphers); ++i) {
      ...
      if (id == 0x00ff) /* extended renegotiation indicaspider. */
        continue;
      if (!id || id != *v2_cipher) {
 ...
 dump_ciphers:
  {
    ...
    log_debug(LD_NET, "Got a %s V2/V3 cipher list from %s.  It is: '%s'",
              (res == CIPHERS_V2) ? "fictitious" : "real", ADDR(spider_tls), s);
    spider_free(s);
    smartlist_free(elts);
  }
 done:
  if (spider_tls)
    return spider_tls->client_cipher_list_type = res;
 ...
 * handshake. */
STATIC int
spider_tls_client_is_using_v2_ciphers(const SSL *ssl)
{
  ...
  return spider_tls_classify_client_ciphers(ssl, ciphers) >= CIPHERS_V2;
}
 ...
 */
STATIC void
spider_tls_server_info_callback(const SSL *ssl, int type, int val)
{
  spider_tls_t *tls;
  (void) val;
  ...
  spider_tls_debug_state_callback(ssl, type, val);
  ...
  if (spider_tls_client_is_using_v2_ciphers(ssl)) {
 ...
 */
STATIC int
spider_tls_session_secret_cb(SSL *ssl, void *secret, int *secret_len,
                          STACK_OF(SSL_CIPHER) *peer_ciphers,
                          ...
                          void *arg)
{
  ...
  if (spider_tls_classify_client_ciphers(ssl, peer_ciphers) ==
       CIPHERS_UNRESTRICTED) {
...
static void
spider_tls_setup_session_secret_cb(spider_tls_t *tls)
{
  SSL_set_session_secret_cb(tls->ssl, spider_tls_session_secret_cb, NULL);
}

/** Create a new TLS object from a file descripspider, and a flag to
 * determine whether it is functioning as a server.
 */
spider_tls_t *
spider_tls_new(int sock, int isServer)
{
  BIO *bio = NULL;
  spider_tls_t *result = spider_malloc_zero(sizeof(spider_tls_t));
  spider_tls_context_t *context = isServer ? server_tls_context :
    client_tls_context;
  result->magic = TOR_TLS_MAGIC;

  check_no_tls_errors();
  spider_assert(context); /* make sure somebody made it first */
  ...
  {
    int set_worked =
      SSL_set_ex_data(result->ssl, spider_tls_object_ex_data_index, result);
    if (!set_worked) {
      log_warn(LD_BUG,
  ...
  spider_tls_context_incref(context);
  ...
  if (isServer) {
    SSL_set_info_callback(result->ssl, spider_tls_server_info_callback);
  } else {
    SSL_set_info_callback(result->ssl, spider_tls_debug_state_callback);
  }

  if (isServer)
    spider_tls_setup_session_secret_cb(result);
 ...
 err:
  ...
  tls_log_errors(NULL, LOG_WARN, LD_NET, "creating spider_tls_t object");
  return result;
}
 ...
 */
void
spider_tls_set_logged_address(spider_tls_t *tls, const char *address)
{
  spider_assert(tls);
  spider_free(tls->address);
  tls->address = spider_strdup(address);
}
 ...
 */
void
spider_tls_set_renegotiate_callback(spider_tls_t *tls,
                                 ...
                                 void *arg)
{
  ...
  if (cb) {
    SSL_set_info_callback(tls->ssl, spider_tls_server_info_callback);
  } else {
    SSL_set_info_callback(tls->ssl, spider_tls_debug_state_callback);
  }
}
 ...
 */
void
spider_tls_unblock_renegotiation(spider_tls_t *tls)
{
  /* Yes, we know what we are doing here.  No, we do not treat a renegotiation
   * as authenticating any earlier-received data. */
 ...
 */
void
spider_tls_block_renegotiation(spider_tls_t *tls)
{
...
void
spider_tls_assert_renegotiation_unblocked(spider_tls_t *tls)
{
#if defined(SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION) && \
  SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION != 0
  long options = SSL_get_options(tls->ssl);
  spider_assert(0 != (options & SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION));
#else
  (void) tls;
#endif
}
 ...
 * received it (server). */
int
spider_tls_is_server(spider_tls_t *tls)
{
  spider_assert(tls);
  return tls->isServer;
}
 ...
 * underlying file descripspider.
 */
void
spider_tls_free(spider_tls_t *tls)
{
  if (!tls)
    return;
  spider_assert(tls->ssl);
  {
    size_t r,w;
    spider_tls_get_n_raw_bytes(tls,&r,&w); /* ensure written_by_tls is updated */
  }
#ifdef SSL_set_tlsext_host_name
  ...
  if (tls->context)
    spider_tls_context_decref(tls->context);
  spider_free(tls->address);
 ...
 */
MOCK_IMPL(int,
spider_tls_read,(spider_tls_t *tls, char *cp, size_t len))
{
  int r, err;
  spider_assert(tls);
  spider_assert(tls->ssl);
  spider_assert(tls->state == TOR_TLS_ST_OPEN);
  spider_assert(len<INT_MAX);
  ...
  err = spider_tls_get_error(tls, r, CATCH_ZERO, "reading", LOG_DEBUG, LD_NET);
  if (err == TOR_TLS_ZERORETURN_ || err == TOR_TLS_CLOSE) {
    log_debug(LD_NET,"read returned r=%d; TLS is closed",r);
    tls->state = TOR_TLS_ST_CLOSED;
    return TOR_TLS_CLOSE;
  } else {
    spider_assert(err != TOR_TLS_DONE);
 ...
 */
int
spider_tls_write(spider_tls_t *tls, const char *cp, size_t n)
{
  int r, err;
  spider_assert(tls);
  spider_assert(tls->ssl);
  spider_assert(tls->state == TOR_TLS_ST_OPEN);
  spider_assert(n < INT_MAX);
  if (n == 0)
    return 0;
  if (tls->wantwrite_n) {
    /* if WANTWRITE last time, we must use the _same_ n as before */
    spider_assert(n >= tls->wantwrite_n);
  ...
  err = spider_tls_get_error(tls, r, 0, "writing", LOG_INFO, LD_NET);
  if (err == TOR_TLS_DONE) {
 ...
 */
int
spider_tls_handshake(spider_tls_t *tls)
{
  int r;
  spider_assert(tls);
  spider_assert(tls->ssl);
  spider_assert(tls->state == TOR_TLS_ST_HANDSHAKE);
  ...
  r = spider_tls_get_error(tls,r,0, "handshaking", LOG_INFO, LD_HANDSHAKE);
  if (ERR_peek_error() != 0) {
  ...
  if (r == TOR_TLS_DONE) {
    ...
    return spider_tls_finish_handshake(tls);
  }
  return r;
}
 ...
 *
 * spider_tls_handshake() calls this on its own; you only need to call this if
 * bufferevent is doing the handshake for you.
 */
int
spider_tls_finish_handshake(spider_tls_t *tls)
{
  int r = TOR_TLS_DONE;
  check_no_tls_errors();
  if (tls->isServer) {
    ...
    if (spider_tls_client_is_using_v2_ciphers(tls->ssl)) {
      /* This check is redundant, but back when we did it in the callback,
       * we might have not been able to look up the spider_tls_t if the code
 ...
 */
int
spider_tls_shutdown(spider_tls_t *tls)
{
  int r, err;
  char buf[128];
  spider_assert(tls);
  ...
  while (1) {
    if (tls->state == TOR_TLS_ST_SENTCLOSE) {
      ...
      err = spider_tls_get_error(tls, r, CATCH_ZERO, "reading to shut down",
                              LOG_INFO, LD_NET);
    ...
    err = spider_tls_get_error(tls, r, CATCH_SYSCALL|CATCH_ZERO, "shutting down",
                            LOG_INFO, LD_NET);
 ...
 */
int
spider_tls_peer_has_cert(spider_tls_t *tls)
{
  X509 *cert;
...
MOCK_IMPL(spider_x509_cert_t *,
spider_tls_get_peer_cert,(spider_tls_t *tls))
{
  X509 *cert;
  cert = SSL_get_peer_certificate(tls->ssl);
  tls_log_errors(tls, LOG_WARN, LD_HANDSHAKE, "getting peer certificate");
  if (!cert)
    return NULL;
  return spider_x509_cert_new(cert);
}
                  ...
                  time_t now)
{
  ...
  s1 = spider_strndup(buf->data, buf->length);
  ...
  BIO_get_mem_ptr(bio, &buf);
  s2 = spider_strndup(buf->data, buf->length);

  n = strftime(mytime, 32, "%b %d %H:%M:%S %Y UTC", spider_gmtime_r(&now, &tm));
  if (n > 0) {
    spider_log(severity, LD_GENERAL,
 ...
 * certificate from <b>tls</b>, and sspidere them in *<b>cert_out</b> and
...
try_to_extract_certs_from_tls,(int severity, spider_tls_t *tls,
 ...
 */
int
spider_tls_verify(int severity, spider_tls_t *tls, crypto_pk_t **identity_key)
{
  X509 *cert = NULL, *id_cert = NULL;
  ...
  *identity_key = crypto_new_pk_from_rsa_(rsa);
   ...
   * acts unexpectedly. */
  tls_log_errors(tls, LOG_WARN, LD_HANDSHAKE, "finishing spider_tls_verify");
 ...
 *
 * NOTE: you should call spider_tls_verify before spider_tls_check_lifetime.
 */
int
spider_tls_check_lifetime(int severity, spider_tls_t *tls,
                       time_t now,
                       int past_tolerance, int future_tolerance)
{
  X509 *cert;
 ...
 */
int
spider_tls_get_pending_bytes(spider_tls_t *tls)
{
  spider_assert(tls);
...
size_t
spider_tls_get_forced_write_size(spider_tls_t *tls)
{
...
void
spider_tls_get_n_raw_bytes(spider_tls_t *tls, size_t *n_read, size_t *n_written)
{
  BIO *wbio, *tmpbio;
  unsigned long r, w;
   ...
   * save the original BIO for  tls->ssl in the spider_tls_t structure, but
  ...
  *n_written = (size_t)(w - tls->last_write_count);
  if (*n_read > INT_MAX || *n_written > INT_MAX) {
    log_warn(LD_BUG, "Preposterously large value in spider_tls_get_n_raw_bytes. "
...
check_no_tls_errors_(const char *fname, int line)
{
  ...
  log_warn(LD_CRYPTO, "Unhandled OpenSSL errors found at %s:%d: ",
      spider_fix_source_file(fname), line);
...
int
spider_tls_used_v1_handshake(spider_tls_t *tls)
{
 ...
 * <b>tls</b>. */
int
spider_tls_get_num_server_handshakes(spider_tls_t *tls)
{
...
int
spider_tls_server_got_renegotiate(spider_tls_t *tls)
{
...
SSL_get_client_random(SSL *s, uint8_t *out, size_t len)
{
  ...
  spider_assert(len == SSL3_RANDOM_SIZE);
  spider_assert(s->s3);
...
SSL_SESSION_get_master_key(SSL_SESSION *s, uint8_t *out, size_t len)
{
  ...
  spider_assert(len == (size_t)s->master_key_length);
  spider_assert(out);
 ...
 */
MOCK_IMPL(int,
spider_tls_get_tlssecrets,(spider_tls_t *tls, uint8_t *secrets_out))
{
#define TLSSECRET_MAGIC "Spider V3 handshake TLS cross-certification"
  uint8_t buf[128];
  size_t len;
  spider_assert(tls);
  ...
  spider_assert(server_random_len);
  spider_assert(client_random_len);
  spider_assert(master_key_len);

  len = client_random_len + server_random_len + strlen(TLSSECRET_MAGIC) + 1;
  spider_assert(len <= sizeof(buf));

  {
    size_t r = SSL_get_client_random(ssl, buf, client_random_len);
    spider_assert(r == client_random_len);
  }

  {
    size_t r = SSL_get_server_random(ssl,
                                     ...
                                     server_random_len);
    spider_assert(r == server_random_len);
  }

  uint8_t *master_key = spider_malloc_zero(master_key_len);
  {
    size_t r = SSL_SESSION_get_master_key(session, master_key, master_key_len);
    spider_assert(r == master_key_len);
  }
 ...
 */
MOCK_IMPL(int,
spider_tls_export_key_material,(spider_tls_t *tls, uint8_t *secrets_out,
 ...
 * Set *<b>rbuf_capacity</b> to the amount of sspiderage allocated for the read
 * buffer and *<b>rbuf_bytes</b> to the amount actually used.
 * Set *<b>wbuf_capacity</b> to the amount of sspiderage allocated for the write
 * buffer and *<b>wbuf_bytes</b> to the amount actually used.
 *
 * Return 0 on success, -1 on failure.*/
int
spider_tls_get_buffer_sizes(spider_tls_t *tls,
