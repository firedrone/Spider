...
/* as fallback implementation for spider_sleep_msec */
 ...
 */
void
spider_gettimeofday(struct timeval *timeval)
{
#ifdef _WIN32
...
monotime_enable_test_mocking(void)
{
  ...
  spider_assert_nonfatal(monotime_mocking_enabled == 0);
...
{
  spider_assert_nonfatal(monotime_mocking_enabled == 1);
...
{
  spider_assert(!monotime_initialized);
  int r = mach_timebase_info(&mach_time_info);
  spider_assert(r == 0);
  spider_assert(mach_time_info.denom != 0);
}
...
monotime_init_internal(void)
{
  ...
  spider_assert(n <= INT64_MAX);
  spider_assert(d <= INT64_MAX);
...
static spider_mutex_t monotime_lock;
...
{
  spider_assert(!monotime_initialized);
  spider_mutex_init(&monotime_lock);
}
...
monotime_get(monotime_t *out)
{
  ...
  spider_mutex_acquire(&monotime_lock);
  struct timeval timeval_raw;
  spider_gettimeofday(&timeval_raw);
  ratchet_timeval(&timeval_raw, &out->tv_);
  spider_mutex_release(&monotime_lock);
}
