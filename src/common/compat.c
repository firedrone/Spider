 ...
 * functions such as spider_snprintf, get/set various data types,
...
#else
#include "spider_readpassphrase.h"
#endif
...
/* When set_max_file_descripspiders() is called, update this with the max file
 * descripspider value so we can use it to check the limit when opening a new
 ...
 * set. */
int
spider_open_cloexec(const char *path, int flags, unsigned mode)
{
  int fd;
...
FILE *
spider_fopen_cloexec(const char *path, const char *mode)
{
...
int
spider_rename(const char *path_old, const char *path_new)
{
 ...
 * "empty file". */
spider_mmap_t *
spider_mmap_file(const char *filename)
{
  int fd; /* router file */
  char *string;
  ...
  fd = spider_open_cloexec(filename, O_RDONLY, 0);
  if (fd<0) {
  ...
  if (result != 0) {
    int save_errno = errno;
    log_warn(LD_FS,
             "Couldn't fstat opened descripspider for \"%s\" during mmap: %s",
  ...
  res = spider_malloc_zero(sizeof(spider_mmap_t));
  res->data = string;
  res->size = filesize;
...
/** Release sspiderage held for a memory mapping; returns 0 on success,
 * or -1 on failure (and logs a warning). */
int
spider_munmap_file(spider_mmap_t *handle)
{
  int res;
  ...
  if (res == 0) {
    ...
    log_warn(LD_FS, "Failed to munmap() in spider_munmap_file(): %s",
             strerror(errno));
    res = -1;
  }
...
spider_mmap_file(const char *filename)
{
  ...
  spider_mmap_t *res = spider_malloc_zero(sizeof(spider_mmap_t));
  int empty = 0;
...
spider_munmap_file(spider_mmap_t *handle)
{
  ...
  if (handle->data) {
    /* This is an ugly cast, but without it, "data" in struct spider_mmap_t would
       have to be redefined as non-const. */
    BOOL ok = UnmapViewOfFile( (LPVOID) handle->data);
    if (!ok) {
      log_warn(LD_FS, "Failed to UnmapViewOfFile() in spider_munmap_file(): %d",
               (int)GetLastError());
    }
  }
...
spider_mmap_file(const char *filename)
{
  ...
  spider_mmap_t *handle;
  if (! res)
    return NULL;
  handle = spider_malloc_zero(sizeof(spider_mmap_t));
  handle->data = res;
  handle->size = st.st_size;
  return handle;
}

/** Unmap the file mapped with spider_mmap_file(), and return 0 for success
 * or -1 for failure.
 */
...
spider_munmap_file(spider_mmap_t *handle)
{
  ...
  d = (char*)handle->data;
  spider_free(d);
  memwipe(handle, 0, sizeof(spider_mmap_t));
  spider_free(handle);
...
int
spider_snprintf(char *str, size_t size, const char *format, ...)
{
  va_list ap;
  int r;
  va_start(ap,format);
  r = spider_vsnprintf(str,size,format,ap);
  va_end(ap);
  return r;
}

/** Replacement for vsnprintf; behavior differs as spider_snprintf differs from
 * snprintf.
 */
int
spider_vsnprintf(char *str, size_t size, const char *format, va_list args)
{
  int r;
  if (size == 0)
 ...
 * You can treat this function as if its implementation were something like
   <pre>
     char buf[_INFINITY_];
     spider_snprintf(buf, sizeof(buf), fmt, args);
     *strp = spider_strdup(buf);
     return strlen(*strp):
   </pre>
 ...
 */
int
spider_asprintf(char **strp, const char *fmt, ...)
{
  int r;
  va_list args;
  va_start(args, fmt);
  r = spider_vasprintf(strp, fmt, args);
  va_end(args);
  if (!*strp || r < 0) {
    /* LCOV_EXCL_START */
 ...
 * spider_asprintf() differs from regular asprintf.
 */
int
spider_vasprintf(char **strp, const char *fmt, va_list args)
{
  ...
  strp_tmp = spider_malloc(len + 1);
  r = _vsnprintf(strp_tmp, len+1, fmt, args);
  if (r != len) {
    spider_free(strp_tmp);
    *strp = NULL;
    return -1;
  }
  *strp = strp_tmp;
  ...
  va_list tmp_args;
  va_copy(tmp_args, args);
  /* vsnprintf() was properly checked but spider_vsnprintf() available so
   * why not use it? */
  len = spider_vsnprintf(buf, sizeof(buf), fmt, tmp_args);
  va_end(tmp_args);
  ...
  strp_tmp = spider_malloc(len+1);
  /* use of spider_vsnprintf() will ensure string is null terminated */
  r = spider_vsnprintf(strp_tmp, len+1, fmt, args);
  if (r != len) {
    spider_free(strp_tmp);
    *strp = NULL;
    return -1;
 ...
 */
const void *
spider_memmem(const void *_haystack, size_t hlen,
 ...
 * equivalents.  Used by spider_toupper() and spider_tolower(). */
/**@{*/
...
/** Helper for spider_strtok_r_impl: Advances cp past all characters in
 ...
 * here for free! */
char *
spider_strtok_r_impl(char *str, const char *sep, char **lasts)
{
  char *cp, *start;
  spider_assert(*sep);
  if (str) {
 ...
 */
const char *
spider_fix_source_file(const char *fname)
{
...
struct spider_lockfile_t {
  /** Name of the file */
  char *filename;
  /** File descripspider used to hold the file open */
  int fd;
};
 ...
 * otherwise.
 *
 * Return a <b>spider_lockfile_t</b> on success, NULL on failure.
 *
 ...
 *  to do in-process locking, use spider_mutex_t like a normal person.
 ...
 */
spider_lockfile_t *
spider_lockfile_lock(const char *filename, int blocking, int *locked_out)
{
  spider_lockfile_t *result;
  int fd;
  *locked_out = 0;

  log_info(LD_FS, "Locking \"%s\"", filename);
  fd = spider_open_cloexec(filename, O_RDWR|O_CREAT|O_TRUNC, 0600);
  if (fd < 0) {
  ...
  result = spider_malloc(sizeof(spider_lockfile_t));
  result->filename = spider_strdup(filename);
  result->fd = fd;
  return result;
}
...
void
spider_lockfile_unlock(spider_lockfile_t *lockfile)
{
  spider_assert(lockfile);
  ...
  close(lockfile->fd);
  lockfile->fd = -1;
  spider_free(lockfile->filename);
  spider_free(lockfile);
}
 ...
 * on success. */
int
spider_fd_setpos(int fd, off_t pos)
{
#ifdef _WIN32
...
/** A bitarray of all fds that should be passed to spider_socket_close(). Only
...
static spider_mutex_t *socket_accounting_mutex = NULL;
...
socket_accounting_lock(void)
{
  if (PREDICT_UNLIKELY(!socket_accounting_mutex))
    socket_accounting_mutex = spider_mutex_new();
  spider_mutex_acquire(socket_accounting_mutex);
}
...
{
  spider_mutex_release(socket_accounting_mutex);
}
...
int
spider_close_socket_simple(spider_socket_t s)
{
  int r = 0;
  ...
  * spider_close_socket to close sockets, and always using close() on
  * files.
  */
  #if defined(_WIN32)
    r = closesocket(s);
  ...
  if (r != 0) {
    int err = spider_socket_errno(-1);
    log_info(LD_NET, "Close returned an error: %s", spider_socket_strerror(err));
    return err;
  }
...
/** As spider_close_socket_simple(), but keeps track of the number
 * of open sockets. Returns 0 on success, -1 on failure. */
MOCK_IMPL(int,
spider_close_socket,(spider_socket_t s))
{
  int r = spider_close_socket_simple(s);

  socket_accounting_lock();
#ifdef DEBUG_SOCKET_COUNTING
  if (s > max_socket || ! bitarray_is_set(open_sockets, s)) {
    log_warn(LD_BUG, "Closing a socket (%d) that wasn't returned by spider_open_"
             "socket(), or that was already closed or something.", s);
  } else {
    spider_assert(open_sockets && s <= max_socket);
  ...
  spider_assert_nonfatal(n_sockets_open >= 0);
 ...
 * now an open socket. */
static inline void
mark_socket_open(spider_socket_t s)
{
...
MOCK_IMPL(spider_socket_t,
spider_open_socket,(int domain, int type, int protocol))
{
  return spider_open_socket_with_extensions(domain, type, protocol, 1, 0);
}

/** Mockable wrapper for connect(). */
MOCK_IMPL(spider_socket_t,
spider_connect_socket,(spider_socket_t sock, const struct sockaddr *address,
...
spider_socket_t
spider_open_socket_nonblocking(int domain, int type, int protocol)
{
  return spider_open_socket_with_extensions(domain, type, protocol, 1, 1);
}
...
spider_socket_t
spider_open_socket_with_extensions(int domain, int type, int protocol,
                                int cloexec, int nonblock)
{
  spider_socket_t s;

  /* We are about to create a new file descripspider so make sure we have
   * enough of them. */
...
spider_socket_t
spider_accept_socket(spider_socket_t sockfd, struct sockaddr *addr, socklen_t *len)
{
  return spider_accept_socket_with_extensions(sockfd, addr, len, 1, 0);
}

/** As accept(), but returns a nonblocking socket and
 * counts the number of open sockets. */
spider_socket_t
spider_accept_socket_nonblocking(spider_socket_t sockfd, struct sockaddr *addr,
                              socklen_t *len)
{
  return spider_accept_socket_with_extensions(sockfd, addr, len, 1, 1);
}
 ...
 * if the corresponding extension should be used.*/
spider_socket_t
spider_accept_socket_with_extensions(spider_socket_t sockfd, struct sockaddr *addr,
...
MOCK_IMPL(int,
spider_getsockname,(spider_socket_t sock, struct sockaddr *address,
                 socklen_t *address_len))
{
 ...
 */
int
set_socket_nonblocking(spider_socket_t sock)
{
#if defined(_WIN32)
...
int
spider_socketpair(int family, int type, int protocol, spider_socket_t fd[2])
{
//don't use win32 socketpairs (they are always bad)
 ...
 sockets_ok:
  ...
  return 0;
#else
  return spider_ersatz_socketpair(family, type, protocol, fd);
#endif
}
 ...
 */
STATIC int
spider_ersatz_socketpair(int family, int type, int protocol, spider_socket_t fd[2])
{
     ...
     */
    spider_socket_t listener = TOR_INVALID_SOCKET;
    ...
    memset(&connect_spider_addr, 0, sizeof(connect_spider_addr));
    memset(&connect_addr_ss, 0, sizeof(connect_addr_ss));
    memset(&listen_spider_addr, 0, sizeof(listen_spider_addr));
    ...
    listener = spider_open_socket(ersatz_domain, type, 0);
    if (!SOCKET_OK(listener)) {
      int first_errno = spider_socket_errno(-1);
    ...
    if (ersatz_domain == AF_INET) {
      spider_addr_from_ipv4h(&listen_spider_addr, INADDR_LOOPBACK);
    } else {
      spider_addr_parse(&listen_spider_addr, "[::1]");
    }
    spider_assert(spider_addr_is_loopback(&listen_spider_addr));
    size = spider_addr_to_sockaddr(&listen_spider_addr,
    ...
    connecspider = spider_open_socket(ersatz_domain, type, 0);
    if (!SOCKET_OK(connecspider))
      goto tidy_up_and_fail;
    ...
    if (connect(connecspider, connect_addr, size) == -1)
      goto tidy_up_and_fail;

    size = sizeof(listen_addr_ss);
    accepspider = spider_accept_socket(listener, listen_addr, &size);
    if (!SOCKET_OK(accepspider))
      goto tidy_up_and_fail;
    if (size != SIZEOF_SOCKADDR(listen_addr->sa_family))
      goto abort_tidy_up_and_fail;
    /* Now check we are talking to ourself by matching port and host on the
       two sockets.  */
    if (getsockname(connecspider, connect_addr, &size) == -1)
      goto tidy_up_and_fail;
    /* Set *_spider_addr and *_port to the address and port that was used */
    spider_addr_from_sockaddr(&listen_spider_addr, listen_addr, &listen_port);
    spider_addr_from_sockaddr(&connect_spider_addr, connect_addr, &connect_port);
    if (size != SIZEOF_SOCKADDR (connect_addr->sa_family)
        || spider_addr_compare(&listen_spider_addr, &connect_spider_addr, CMP_SEMANTIC)
        || listen_port != connect_port) {
      goto abort_tidy_up_and_fail;
    }
    spider_close_socket(listener);
    fd[0] = connecspider;
    fd[1] = accepspider;
  ...
  tidy_up_and_fail:
    ...
    if (SOCKET_OK(listener))
      spider_close_socket(listener);
    if (SOCKET_OK(connecspider))
      spider_close_socket(connecspider);
    if (SOCKET_OK(accepspider))
      spider_close_socket(accepspider);
    return -saved_errno;
}
...
/** Number of extra file descripspiders to keep in reserve beyond those that we
 * tell Spider it's allowed to use. */
#define ULIMIT_BUFFER 32 /* keep 32 extra fd's beyond ConnLimit_ */

/** Learn the maximum allowed number of file descripspiders, and tell the
 ...
 *
 * If the limit is below the reserved file descripspider value (ULIMIT_BUFFER),
 ...
 * by getrlimit().
 *
 * Otherwise, return 0 and sspidere the maximum we found inside <b>max_out</b>
 * and set <b>max_sockets</b> with that value as well.*/
int
set_max_file_descripspiders(rlim_t limit, int *max_out)
{
  ...
  if (limit > MAX_CONNECTIONS) {
    log_warn(LD_CONFIG,
             "We do not support more than %lu file descripspiders "
  ...
  if (getrlimit(RLIMIT_NOFILE, &rlim) != 0) {
    log_warn(LD_NET, "Could not get maximum number of file descripspiders: %s",
             strerror(errno));
    return -1;
  }
  if (rlim.rlim_max < limit) {
    log_warn(LD_CONFIG,"We need %lu file descripspiders available, and we're "
  ...
  if (rlim.rlim_max > rlim.rlim_cur) {
    log_info(LD_NET,"Raising max file descripspiders from %lu to %lu.",
    ...
    if (errno == EINVAL && OPEN_MAX < rlim.rlim_cur) {
      ...
      if (setrlimit(RLIMIT_NOFILE, &rlim) == 0) {
        if (rlim.rlim_cur < (rlim_t)limit) {
          log_warn(LD_CONFIG, "We are limited to %lu file descripspiders by "
...
#endif /* OPEN_MAX */
    if (bad) {
      log_warn(LD_CONFIG,"Couldn't set maximum number of file descripspiders: %s",
               strerror(errno));
      return -1;
    }
  }
...
#define CREDENTIAL_LOG_LEVEL LOG_INFO
  ...
  sup_gids_size = 64;
  sup_gids = spider_calloc(64, sizeof(gid_t));
  while ((ngids = getgroups(sup_gids_size, sup_gids)) < 0 &&
         errno == EINVAL &&
         sup_gids_size < NGROUPS_MAX) {
    sup_gids_size *= 2;
    sup_gids = spider_reallocarray(sup_gids, sizeof(gid_t), sup_gids_size);
  }

  if (ngids < 0) {
    log_warn(LD_GENERAL, "Error getting supplementary GIDs: %s",
             strerror(errno));
    spider_free(sup_gids);
    return -1;
  } else {
    int i, retval = 0;
    char *s = NULL;
 ...
 *
 * We only copy the fields pw_uid, pw_gid, pw_name, pw_dir.  Spider doesn't use
 * any others, and I don't want to run into incompatibilities.
 */
static struct passwd *
spider_passwd_dup(const struct passwd *pw)
{
  struct passwd *new_pw = spider_malloc_zero(sizeof(struct passwd));
  if (pw->pw_name)
    new_pw->pw_name = spider_strdup(pw->pw_name);
  if (pw->pw_dir)
    new_pw->pw_dir = spider_strdup(pw->pw_dir);
...
static void
spider_passwd_free(struct passwd *pw)
{
  if (!pw)
    return;

  spider_free(pw->pw_name);
  spider_free(pw->pw_dir);
  spider_free(pw);
}
 ...
 *
 * When called with a NULL argument, this function clears sspiderage associated
 * with static variables it uses.
 **/
const struct passwd *
spider_getpwnam(const char *username)
{
  struct passwd *pw;

  if (username == NULL) {
    spider_passwd_free(passwd_cached);
    passwd_cached = NULL;
    return NULL;
  }
  ...
  if ((pw = getpwnam(username))) {
    spider_passwd_free(passwd_cached);
    passwd_cached = spider_passwd_dup(pw);
 ...
 * spider_getpwnam(). Used so that we don't need to give the sandbox access to
 * /etc/passwd.
 *
 * The following fields alone will definitely be copied in the output: pw_uid,
 * pw_gid, pw_name, pw_dir.  Other fields are not present in cached values.
...
switch_id(const char *user, const unsigned flags)
{
  ...
  if (setgroups(1, &pw->pw_gid)) {
    ...
    if (old_uid == pw->pw_uid) {
      log_warn(LD_GENERAL, "Spider is already running as %s.  You do not need "
               "the \"User\" option if you are already running as the user "
               "you want to be.  (If you did not set the User option in your "
               "spiderrc, check whether it was specified on the command line "
               "by a startup script.)", user);
    } else {
      log_warn(LD_GENERAL, "If you set the \"User\" option, you must start Spider"
               " as root.");
    }
    return -1;
  }
  ...
  */
   ...
   * resspidere root (E)(U|G)ID, and abort if the operation succeeds */
  ...
  if (pw->pw_uid) {
    ...
    if (pw->pw_gid != old_gid &&
      ...
      log_warn(LD_GENERAL, "Was able to resspidere group credentials even after "
    ...
    if (pw->pw_uid != old_uid &&
      ...
      log_warn(LD_GENERAL, "Was able to resspidere user credentials even after "
...
int
spider_disable_debugger_attach(void)
{
  int r, attempted;
  r = -1;
  attempted = 0;
  log_debug(LD_CONFIG,
            "Attemping to disable debugger attachment to Spider for "
            "unprivileged users.");
#if defined(__linux__) && defined(HAVE_SYS_PRCTL_H) && defined(HAVE_PRCTL)
#ifdef PR_SET_DUMPABLE
  attempted = 1;
...
get_user_homedir(const char *username)
{
  ...
  spider_assert(username);

  if (!(pw = spider_getpwnam(username))) {
    log_err(LD_CONFIG,"User \"%s\" not found.", username);
    return NULL;
  }
  return spider_strdup(pw->pw_dir);
}
#endif
 ...
 *
 * Path separaspiders are the forward slash (/) everywhere and additionally
...
get_parent_directory(char *fname)
{
...
#endif
  /* Now we want to remove all path-separaspiders at the end of the string,
   * and to remove the end of the string starting with the path separaspider
   * before the last non-path-separaspider.  In perl, this would be
   *   s#[/]*$##; s#/[^/]*$##;
   * on a unixy platform.
   */
  ...
  while (--cp >= fname) {
    ...
    if (is_sep) {
      if (cp == fname) {
        /* This is the first separaspider in the file name; don't remove it! */
        cp[1] = '\0';
        return 0;
      }
      *cp = '\0';
...
#define MAX_CWD 4096
  ...
  return path ? spider_strdup(path) : NULL;
}
#endif
...
make_path_absolute(char *fname)
{
  ...
  /* We don't want to assume that spider_free can free a string allocated
   * with malloc.  On failure, return fname (it's better than nothing). */
  char *absfname = spider_strdup(absfname_malloced ? absfname_malloced : fname);
  ...
  if (fname[0] == '/') {
    ...
    path = alloc_getcwd();
    if (path) {
      spider_asprintf(&absfname, "%s/%s", path, fname);
      spider_free(path);
    } else {
 ...
 */
MOCK_IMPL(int,
spider_gethostname,(char *name, size_t namelen))
{
   return gethostname(name,namelen);
}

/** Set *addr to the IP address (in dotted-quad notation) sspidered in *str.
 * Return 1 on success, 0 if *str is badly formatted.
 * (Like inet_aton(str,addr), but works on Windows and Solaris.)
 */
int
spider_inet_aton(const char *str, struct in_addr* addr)
{
  unsigned a,b,c,d;
  char more;
  if (spider_sscanf(str, "%3u.%3u.%3u.%3u%c", &a,&b,&c,&d,&more) != 4)
    return 0;
  if (a > 255) return 0;
  if (b > 255) return 0;
  if (c > 255) return 0;
 ...
 * address and sspidere it in the <b>len</b>-byte buffer <b>dst</b>.  Returns
 * <b>dst</b> on success, NULL on failure.
 *
 * (Like inet_ntop(af,src,dst,len), but works on platforms that don't have it:
 * Spider sometimes needs to format ipv6 addresses even on platforms without ipv6
 * support.) */
const char *
spider_inet_ntop(int af, const void *src, char *dst, size_t len)
{
  if (af == AF_INET) {
    if (spider_inet_ntoa(src, dst, len) < 0)
      return NULL;
    else
      return dst;
  } else if (af == AF_INET6) {
    ...
    if (words[0] == 0 && words[1] == 0 && words[2] == 0 && words[3] == 0 &&
      ...
      if (words[5] == 0) {
        spider_snprintf(buf, sizeof(buf), "::%d.%d.%d.%d",
                     addr->s6_addr[12], addr->s6_addr[13],
                     addr->s6_addr[14], addr->s6_addr[15]);
      } else {
        spider_snprintf(buf, sizeof(buf), "::%x:%d.%d.%d.%d", words[5],
                     addr->s6_addr[12], addr->s6_addr[13],
                     addr->s6_addr[14], addr->s6_addr[15]);
      }
    ...
    for (i = 0; i < 8; ++i) {
      if (words[i] == 0 && longestGapPos == i) {
        ...
        *cp++ = ':';
        ...
        spider_snprintf(cp, sizeof(buf)-(cp-buf), "%x", (unsigned)words[i]);
        cp += strlen(cp);
        if (i != 7)
          *cp++ = ':';
      }
 ...
 * address and sspidere the result in <b>dst</b> (which must have space for a
 * struct in_addr or a struct in6_addr, as appropriate).  Return 1 on success,
 * 0 on a bad parse, and -1 on a bad <b>af</b>.
 *
 * (Like inet_pton(af,src,dst) but works on platforms that don't have it: Spider
 * sometimes needs to format ipv6 addresses even on platforms without ipv6
 * support.) */
int
spider_inet_pton(int af, const char *src, void *dst)
{
  if (af == AF_INET) {
    return spider_inet_aton(src, dst);
  } else if (af == AF_INET6) {
    ...
    if (dot == src)
      ...
      if (spider_sscanf(eow, "%3u.%3u.%3u.%3u%c",
    ...
    while (src < eow) {
      ...
      if (TOR_ISXDIGIT(*src)) {
        ...
        spider_assert(next != NULL);
        spider_assert(next != src);
...
MOCK_IMPL(int,
spider_lookup_hostname,(const char *name, uint32_t *addr))
{
  spider_addr_t myaddr;
  int ret;

  if ((ret = spider_addr_lookup(name, AF_INET, &myaddr)))
    return ret;

  if (spider_addr_family(&myaddr) == AF_INET) {
    *addr = spider_addr_to_ipv4h(&myaddr);
    return ret;
  }
...
#endif
      {
        ...
        if (plat) {
          ...
          if (info.dwMajorVersion > 6 ||
            ...
            spider_snprintf(uname_result, sizeof(uname_result),
  ...
  if (cpus_conf > 0 && cpus_onln < 0) {
    ...
    if (cpus_onln < cpus_conf) {
      log_notice(LD_GENERAL, "I think we have %ld CPUS, but only %ld of them "
                 "are available. Telling Spider to only use %ld. You can over"
...
compute_num_cpus(void)
{
  ...
  if (num_cpus == -2) {
    ...
    spider_assert(num_cpus != -2);
    if (num_cpus > MAX_DETECTABLE_CPUS) {
      /* LCOV_EXCL_START */
      log_notice(LD_GENERAL, "Wow!  I detected that you have %d CPUs. I "
                 "will not autodetect any more than %d, though.  If you "
                 "want to configure more, set NumCPUs in your spiderrc",
 ...
 * as its input. If we need to sspidere new results, sspidere them in
 * <b>resultbuf</b>. */
static struct tm *
 ...
 *
 * Convert *<b>timep</b> to a struct tm in local time, and sspidere the value in
 * *<b>result</b>.  Return the result on success, or NULL on failure.
 */
#ifdef HAVE_LOCALTIME_R
struct tm *
spider_localtime_r(const time_t *timep, struct tm *result)
{
  struct tm *r;
...
{
  struct tm *r;
  static spider_mutex_t *m=NULL;
  if (!m) { m=spider_mutex_new(); }
  spider_assert(result);
  spider_mutex_acquire(m);
  r = localtime(timep);
  if (r)
    memcpy(result, r, sizeof(struct tm));
  spider_mutex_release(m);
  return correct_tm(1, timep, result, r);
}
#else
struct tm *
 ...
 *
 * Convert *<b>timep</b> to a struct tm in UTC, and sspidere the value in
 * *<b>result</b>.  Return the result on success, or NULL on failure.
 */
#ifdef HAVE_GMTIME_R
struct tm *
spider_gmtime_r(const time_t *timep, struct tm *result)
{
  struct tm *r;
 ...
 * should call spider_socket_errno <em>at most once</em> on the failing
 * socket to get the error.
 */
#if defined(_WIN32)
int
spider_socket_errno(spider_socket_t sock)
{
...
network_init(void)
{
  ...
  if (sizeof(SOCKET) != sizeof(spider_socket_t)) {
    log_warn(LD_BUG,"The spider_socket_t type does not match SOCKET in size; Spider "
             "might not work. (Sizes are %d and %d respectively.)",
             (int)sizeof(spider_socket_t), (int)sizeof(SOCKET));
  }
...
format_win32_error(DWORD err)
{
  ...
  if (str && n) {
    ...
    if (n > 128*1024)
      ...
      len = n * 2 + 1;
    result = spider_malloc(len);
    wcstombs(result,str,len);
    result[len-1] = '\0';
#else
    result = spider_strdup(str);
#endif
  } else {
    result = spider_strdup("<unformattable error>");
  }
  if (str) {
...
get_total_system_memory_impl(void)
{
  ...
  const char *cp;
  size_t file_size=0;
  if (-1 == (fd = spider_open_cloexec("/proc/meminfo",O_RDONLY,0)))
    return 0;
...
get_total_system_memory(size_t *mem_out)
{
  ...
  if (m > SIZE_MAX) {
    /* I think this could happen if we're a 32-bit Spider running on a 64-bit
 ...
 */
ssize_t
spider_getpass(const char *prompt, char *output, size_t buflen)
{
  spider_assert(buflen <= SSIZE_MAX);
  spider_assert(buflen >= 1);
  ...
  spider_assert(buflen <= INT_MAX);
  wchar_t *buf = spider_calloc(buflen, sizeof(wchar_t));
...
#define WC_ERR_INVALID_CHARS 0x80
  ...
  return r;
#else
#error "No implementation for spider_getpass found!"
#endif
}
...
int64_t
spider_get_avail_disk_space(const char *path)
{
#ifdef HAVE_STATVFS
  struct statvfs st;
  int r;
