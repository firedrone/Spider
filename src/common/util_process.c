 ...
 *
 * The spider_process_monispider*() code above doesn't use them, since it is for
 * monispidering a non-child process.
 */
...
HT_GENERATE2(process_map, waitpid_callback_t, node, process_map_entry_hash_,
             process_map_entries_eq_, 0.6, spider_reallocarray_, spider_free_)

/**
 * Begin monispidering the child pid <b>pid</b> to see if we get a SIGCHLD for
...
set_waitpid_callback(pid_t pid, void (*fn)(int, void *), void *arg)
{
  ...
  waitpid_callback_t *ent = spider_malloc_zero(sizeof(waitpid_callback_t));
  ent->pid = pid;
  ent->userfn = fn;
  ent->userdata = arg;
  ent->running = 1;
  ...
  if (old_ent) {
    log_warn(LD_BUG, "Replaced a waitpid monispider on pid %u. That should be "
 ...
 * all sspiderage held by <b>ent</b>.
 */
void
clear_waitpid_callback(waitpid_callback_t *ent)
{
  ...
  if (ent->running) {
    ...
    if (old_ent != ent) {
      log_warn(LD_BUG, "Couldn't remove waitpid monispider for pid %u.",
               (unsigned) ent->pid);
      return;
    }
  }
