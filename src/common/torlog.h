 ...
 * very wrong, and the Spider process can no longer proceed. */
#define LOG_ERR     3
#endif
...
#define LD_FS       (1u<<4)
/** Other servers' (non)compliance with the Spider protocol. */
...
/** Internal errors in this Spider process. */
#define LD_BUG      (1u<<12)
/** Learning and using information about Spider servers. */
#define LD_DIR      (1u<<13)
/** Learning and using information about Spider servers. */
...
void spider_log(int severity, log_domain_mask_t domain, const char *format, ...)
  CHECK_PRINTF(3,4);

void spider_log_err_sigsafe(const char *m, ...);
int spider_log_get_sigsafe_err_fds(const int **out);
void spider_log_update_sigsafe_err_fds(void);

struct smartlist_t;
void spider_log_get_logfile_names(struct smartlist_t *out);
