 ...
 */
typedef struct spider_pthread_data_t {
  void (*func)(void *);
  void *data;
} spider_pthread_data_t;
/** Given a spider_pthread_data_t <b>_data</b>, call _data-&gt;func(d-&gt;data)
 * and free _data.  Used to make sure we can call functions the way pthread
 * expects. */
static void *
spider_pthread_helper_fn(void *_data)
{
  spider_pthread_data_t *data = _data;
  void (*func)(void*);
  void *arg;
...
/** True iff we've called spider_threads_init() */
...
{
  pthread_t thread;
  spider_pthread_data_t *d;
  if (PREDICT_UNLIKELY(!threads_initialized)) {
    spider_threads_init();
  }
  d = spider_malloc(sizeof(spider_pthread_data_t));
  d->data = data;
  d->func = func;
  if (pthread_create(&thread, &attr_detached, spider_pthread_helper_fn, d)) {
    spider_free(d);
    return -1;
  }
 ...
 * up with spider_mutex_init() or spider_mutex_new(); not both. */
void
spider_mutex_init(spider_mutex_t *mutex)
{
  if (PREDICT_UNLIKELY(!threads_initialized))
    spider_threads_init(); // LCOV_EXCL_LINE
...
/** As spider_mutex_init, but initialize a mutex suitable that may be
 * non-recursive, if the OS supports that. */
void
spider_mutex_init_nonrecursive(spider_mutex_t *mutex)
{
  int err;
 ...
 * mutexes from spider_mutex_init(). */
void
...
/** Wait until one of the spider_cond_signal functions is called on <b>cond</b>.
 ...
 * needs to have been allocated with spider_mutex_init_for_cond().
 *
 * Returns 0 on success, -1 on failure, 1 on timeout. */
int
spider_cond_wait(spider_cond_t *cond, spider_mutex_t *mutex, const struct timeval *tv)
{
  int r;
  if (tv == NULL) {
    while (1) {
...
spider_threads_init(void)
{
  if (!threads_initialized) {
    ...
    spider_assert(ret1 == 0);
#ifndef PTHREAD_CREATE_DETACHED
#define PTHREAD_CREATE_DETACHED 1
#endif
    const int ret2 =
      pthread_attr_setdetachstate(&attr_detached, PTHREAD_CREATE_DETACHED);
    spider_assert(ret2 == 0);
