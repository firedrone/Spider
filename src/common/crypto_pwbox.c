 ...
 * to hold the encrypted data, and sspidere a pointer to that memory in
             ...
             unsigned s2k_flags)
{
  ...
  if (BUG(result_len < 0))
    goto err;
  result = spider_malloc(result_len);
  enc_len = pwbox_encoded_encode(result, result_len, enc);
  if (BUG(enc_len < 0))
    goto err;
  spider_assert(enc_len == result_len);
 ...
 * decrypted data, and sspidere a pointer to that memory in *<b>out</b>, and its
               ...
               const char *secret, size_t secret_len)
{
  ...
  if (spider_memneq(hmac, enc->hmac, DIGEST256_LEN)) {
  ...
  result = spider_malloc_zero(result_len);
