...
pubsub_subscribe_(pubsub_topic_t *topic,
                  ...
                  unsigned priority)
{
  spider_assert(! topic->locked);
  if (subscribe_flags & SUBSCRIBE_ATSTART) {
    spider_assert(topic->n_events_fired == 0);
  }
  pubsub_subscriber_t *r = spider_malloc_zero(sizeof(*r));
  r->priority = priority;
...
/**
 * Release all sspiderage held by <b>topic</b>.
 */
void
