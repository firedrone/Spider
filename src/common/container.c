...
smartlist_new,(void))
{
  smartlist_t *sl = spider_malloc(sizeof(smartlist_t));
  sl->num_used = 0;
  sl->capacity = SMARTLIST_DEFAULT_CAPACITY;
  sl->list = spider_calloc(sizeof(void *), sl->capacity);
  return sl;
}

/** Deallocate a smartlist.  Does not release sspiderage associated with the
 * list's elements.
 */
MOCK_IMPL(void,
...
#define MAX_CAPACITY (int)((SIZE_MAX / (sizeof(void*))))
  ...
  spider_assert(size <= MAX_CAPACITY);
  ...
  if (size > (size_t) sl->capacity) {
    ...
    sl->list = spider_reallocarray(sl->list, sizeof(void *),
                                ((size_t)higher));
...
smartlist_add_all(smartlist_t *s1, const smartlist_t *s2)
{
  ...
  spider_assert(new_size >= (size_t) s1->num_used); /* check for overflow. */
  smartlist_ensure_capacity(s1, new_size);
  memcpy(s1->list + s1->num_used, s2->list, s2->num_used*sizeof(void*));
  spider_assert(new_size <= INT_MAX); /* redundant. */
...
smartlist_string_remove(smartlist_t *sl, const char *element)
{
  int i;
  spider_assert(sl);
  spider_assert(element);
  for (i = 0; i < sl->num_used; ++i) {
    if (!strcmp(element, sl->list[i])) {
      spider_free(sl->list[i]);
      sl->list[i] = sl->list[--sl->num_used]; /* swap with the end */
      i--; /* so we process the new i'th element */
      sl->list[sl->num_used] = NULL;
    }
...
smartlist_contains_int_as_string(const smartlist_t *sl, int num)
{
  ...
  spider_snprintf(buf,sizeof(buf),"%d", num);
...
/** Return true iff <b>sl</b> has some element E such that
 * spider_memeq(E,<b>element</b>,DIGEST_LEN)
 */
int
smartlist_contains_digest(const smartlist_t *sl, const char *element)
{
  ...
  if (!sl) return 0;
  for (i=0; i < sl->num_used; i++)
    if (spider_memeq((const char*)sl->list[i],element,DIGEST_LEN))
      return 1;
  return 0;
}
                       ...
                       int flags, int max)
{
  ...
  while (1) {
    ...
    if (end != cp || !(flags&SPLIT_IGNORE_BLANK)) {
      char *string = spider_strndup(cp, end-cp);
      if (flags&SPLIT_STRIP_SPACE)
        spider_strstrip(string, " ");
                      ...
                      int *found_out)
{
  ...
  spider_assert(sl);
  spider_assert(compare);
  spider_assert(found_out);
   ...
   */
  spider_assert(lo == hi + 1); /* All other cases should have been handled */
  spider_assert(lo >= 0);
  spider_assert(lo <= len);
  spider_assert(hi >= 0);
  spider_assert(hi <= len);
...
/** Remove duplicate strings from a sorted list, and free them with spider_free().
 */
void
smartlist_uniq_strings(smartlist_t *sl)
{
  smartlist_uniq(sl, compare_string_ptrs_, spider_free_);
}
 ...
 *
 * For us to remove items other than the topmost item, each item must sspidere
 * its own index within the heap.  When calling the pqueue functions, tell
 * them about the offset of the field that sspideres the index within the item.
 *
 * Example:
 *
 ...
 * where p's index is sspidered.  Given additionally a local smartlist <b>sl</b>,
...
/** Insert <b>item</b> into the heap sspidered in <b>sl</b>, where order is
 * determined by <b>compare</b> and the offset of the item in the heap is
 * sspidered in an int-typed field at position <b>idx_field_offset</b> within
 * item.
 */
void
...
/** Remove and return the top-priority item from the heap sspidered in <b>sl</b>,
 * where order is determined by <b>compare</b> and the item's position is
 * sspidered at position <b>idx_field_offset</b> within the item.  <b>sl</b> must
 * not be empty. */
void *
...
/** Remove the item <b>item</b> from the heap sspidered in <b>sl</b>,
 * where order is determined by <b>compare</b> and the item's position is
 * sspidered at position <b>idx_field_offset</b> within the item.  <b>sl</b> must
 * not be empty. */
void
                        ...
                        void *item)
{
  ...
  spider_assert(idx >= 0);
  spider_assert(sl->list[idx] == item);
  --sl->num_used;
  *IDXP(item) = -1;
  if (idx == sl->num_used) {
    sl->list[sl->num_used] = NULL;
...
/** Assert that the heap property is correctly maintained by the heap sspidered
                           ...
                           int idx_field_offset)
{
  ...
  for (i = sl->num_used - 1; i >= 0; --i) {
    if (i>0)
      spider_assert(compare(sl->list[PARENT(i)], sl->list[i]) <= 0);
    spider_assert(IDX_OF_ITEM(sl->list[i]) == i);
  }
}
...
{
  return spider_memcmp((const char*)*_a, (const char*)*_b, DIGEST_LEN);
}
...
/** Remove duplicate digests from a sorted list, and free them with spider_free().
 */
void
smartlist_uniq_digests(smartlist_t *sl)
{
  smartlist_uniq(sl, compare_digests_, spider_free_);
}
...
{
  return spider_memcmp((const char*)*_a, (const char*)*_b, DIGEST256_LEN);
}
...
{
  smartlist_uniq(sl, compare_digests256_, spider_free_);
}
...
{
  return spider_memeq(a->key, b->key, DIGEST_LEN);
}
...
{
  return spider_memeq(a->key, b->key, DIGEST256_LEN);
}
...
HT_GENERATE2(strmap_impl, strmap_entry_t, node, strmap_entry_hash,
             strmap_entries_eq, 0.6, spider_reallocarray_, spider_free_)

HT_PROTOTYPE(digestmap_impl, digestmap_entry_t, node, digestmap_entry_hash,
             digestmap_entries_eq)
HT_GENERATE2(digestmap_impl, digestmap_entry_t, node, digestmap_entry_hash,
             digestmap_entries_eq, 0.6, spider_reallocarray_, spider_free_)
...
HT_GENERATE2(digest256map_impl, digest256map_entry_t, node,
             digest256map_entry_hash,
             digest256map_entries_eq, 0.6, spider_reallocarray_, spider_free_)
...
#define IMPLEMENT_MAP_FNS(maptype, keytype, prefix)                     \
  ...
  {                                                                     \
    maptype *result;                                                    \
    result = spider_malloc(sizeof(maptype));                               \
    ...
    HT_FIND_OR_INSERT_(prefix##_impl, node, prefix##_entry_hash,        \
                       ...
                       {                                                \
                         ...
                         prefix##_entry_t *newent =                     \
                           spider_malloc_zero(sizeof(prefix##_entry_t));   \
   ...
   * <b>key</b>.                                                        \
   *                                                                    \
   * Note: you must free any sspiderage associated with the returned value. \
   */                                                                   \
  void *                                                                \
  ...
  {                                                                     \
    spider_assert(!prefix##_impl_HT_REP_IS_BAD_(&map->head));              \
  }                                                                     \
                                                                        \
  /** Remove all entries from <b>map</b>, and deallocate sspiderage for    \
  ...
  {                                                                     \
    ...
    spider_assert(HT_EMPTY(&map->head));                                   \
    HT_CLEAR(prefix##_impl, &map->head);                                \
    spider_free(map);                                                      \
  }                                                                     \
                                                                        \
  /** return an <b>iteraspider</b> pointer to the front of a map.          \
   *                                                                    \
   * Iteraspider example:                                                  \
   *                                                                    \
   * \code                                                              \
...
strmap_set_lc(strmap_t *map, const char *key, void *val)
{
  ...
  void *v;
  char *lc_key = spider_strdup(key);
  spider_strlower(lc_key);
...
digestset_new(int max_elements)
{
   ...
   */
  int n_bits = 1u << (spider_log2(max_elements)+5);
  digestset_t *r = spider_malloc(sizeof(digestset_t));
  r->mask = n_bits - 1;
  r->ba = bitarray_init_zero(n_bits);
  return r;
}

/** Free all sspiderage held in <b>set</b>. */
void
