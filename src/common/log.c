...
sev_to_string(int severity)
{
  switch (severity) {
    ...
    default:          /* Call assert, not spider_assert, since spider_assert
...
should_log_function_name(log_domain_mask_t domain, int severity)
{
  switch (severity) {
    ...
    default:
      /* Call assert, not spider_assert, since spider_assert calls log on failure. */
      raw_assert(0); return 0; // LCOV_EXCL_LINE
  }
}

/** A mutex to guard changes to logfiles and logging. */
static spider_mutex_t log_mutex;
 ...
 * the length of the messages, not the number of bytes used to sspidere
 * them.) */
static size_t pending_startup_messages_len;

/** True iff we should sspidere messages while waiting for the logs to get
 * configured. */
...
/** Don't sspidere more than this many bytes of messages while waiting for the
...
#define LOCK_LOGS() STMT_BEGIN                                          \
  spider_assert(log_mutex_initialized);                                    \
  spider_mutex_acquire(&log_mutex);                                        \
  STMT_END
/** Unlock the log_mutex */
#define UNLOCK_LOGS() STMT_BEGIN                                        \
  spider_assert(log_mutex_initialized);                                    \
  spider_mutex_release(&log_mutex);                                        \
  STMT_END
 ...
 * log.
 *
 * Spider uses this string to write the version number to the log file. */
void
log_set_application_name(const char *name)
{
  spider_free(appname);
  appname = name ? spider_strdup(name) : NULL;
}
...
log_prefix_(char *buf, size_t buf_len, int severity)
{
  ...
  n = strftime(buf, buf_len, "%b %d %H:%M:%S", spider_localtime_r(&t, &tm));
  r = spider_snprintf(buf+n, buf_len-n, ".%.3i [%s] ", ms,
 ...
 */
static int
log_spider_version(logfile_t *lf, int reset)
{
  char buf[256];
  size_t n;
  int is_new;
  ...
  is_new = lf->fd >= 0 && spider_fd_getpos(lf->fd) == 0;
  ...
  if (appname) {
    spider_snprintf(buf+n, sizeof(buf)-n,
                 "%s opening %slog file.\n", appname, is_new?"new ":"");
  } else {
    spider_snprintf(buf+n, sizeof(buf)-n,
                 "Spider %s opening %slog file.\n", VERSION, is_new?"new ":"");
  }
...
static const char bug_suffix[] = " (on Spider " VERSION
#ifndef _MSC_VER
  " "
 ...
 * facspidered out of <b>logv</b> so that we never format a message more
           ...
           const char *format, va_list ap, size_t *msg_len_out)
{
  ...
  if (funcname && should_log_function_name(domain, severity)) {
    r = spider_snprintf(buf+n, buf_len-n,
  ...
  r = spider_vsnprintf(buf+n,buf_len-n,format,ap);
  if (r < 0) {
...
{
  pending_log_message_t *m = spider_malloc(sizeof(pending_log_message_t));
  m->severity = severity;
  m->domain = domain;
  m->fullmsg = fullmsg ? spider_strdup(fullmsg) : NULL;
  m->msg = spider_strdup(shortmsg);
  return m;
}

/** Release all sspiderage held by <b>msg</b>. */
static void
...
#warn "MAXLINE is a very low number; it might not be from syslog.h after all"
    ...
    if (msg_len >= MAXLINE)
      m = spider_strndup(msg_after_prefix, MAXLINE-1);
    syslog(severity, "%s", m);
 ...
 * message.  The actual message is derived as from spider_snprintf(format,ap).
 */
 ...
 * */
void
spider_log(int severity, log_domain_mask_t domain, const char *format, ...)
{
  va_list ap;
 ...
 * on failure. */
static int
spider_log_err_sigsafe_write(const char *s)
{
  int i;
  ssize_t r;
  size_t len = strlen(s);
...
void
spider_log_err_sigsafe(const char *m, ...)
{
  va_list ap;
  const char *x;
  char timebuf[33];
  ...
  spider_log_err_sigsafe_write("\n=========================================="
                             "================== T=");
  spider_log_err_sigsafe_write(timebuf);
  spider_log_err_sigsafe_write("\n");
  spider_log_err_sigsafe_write(m);
  va_start(ap, m);
  while ((x = va_arg(ap, const char*))) {
    spider_log_err_sigsafe_write(x);
  }
  va_end(ap);
}
...
int
spider_log_get_sigsafe_err_fds(const int **out)
{
  *out = sigsafe_log_fds;
...
void
spider_log_update_sigsafe_err_fds(void)
{
  const logfile_t *lf;
  ...
  if (!found_real_stderr &&
    ...
    raw_assert(n_sigsafe_log_fds >= 2); /* Don't spider_assert inside log fns */
...
void
spider_log_get_logfile_names(smartlist_t *out)
{
  logfile_t *lf;
  spider_assert(out);
...
/** Free all sspiderage held by <b>victim</b>. */
static void
log_free(logfile_t *victim)
{
  if (!victim)
    return;
  spider_free(victim->severities);
  spider_free(victim->filename);
  spider_free(victim);
}
...
//    spider_assert(tmpl);
//    spider_assert(tmpl->next == victim);
    if (!tmpl)
      return;
...
{
  int i;
  spider_assert(loglevelMin >= loglevelMax);
  spider_assert(loglevelMin >= LOG_ERR && loglevelMin <= LOG_DEBUG);
  spider_assert(loglevelMax >= LOG_ERR && loglevelMax <= LOG_DEBUG);
...
{
  logfile_t *lf;
  lf = spider_malloc_zero(sizeof(logfile_t));
  lf->fd = fd;
  lf->filename = spider_strdup(name);
  lf->severities = spider_memdup(severity, sizeof(log_severity_list_t));
  lf->next = logfiles;
...
{
  log_severity_list_t *s = spider_malloc_zero(sizeof(log_severity_list_t));
...
add_callback_log(const log_severity_list_t *severity, log_callback cb)
{
  ...
  lf->fd = -1;
  lf->severities = spider_memdup(severity, sizeof(log_severity_list_t));
  lf->filename = spider_strdup("<callback>");
  lf->callback = cb;
  lf->next = logfiles;
...
/** Flush all the messages we sspidered from startup while waiting for log
 * initialization.
 */
void
             ...
             const int truncate_log)
{
  ...
  fd = spider_open_cloexec(filename, open_flags, 0640);
  if (fd<0)
    return -1;
  if (spider_fd_seekend(fd)<0) {
    close(fd);
    return -1;
  }
 ...
 *
 * If this is the first log handler, opens syslog with ident Spider or
 * Spider-<syslog_identity_tag> if that is not NULL.
 */
int
               ...
               const char* syslog_identity_tag)
{
  ...
  if (syslog_count++ == 0) {
    ...
    if (syslog_identity_tag) {
      spider_snprintf(buf, sizeof(buf), "Spider-%s", syslog_identity_tag);
    } else {
      spider_snprintf(buf, sizeof(buf), "Spider");
    }
  ...
  lf->fd = -1;
  lf->severities = spider_memdup(severity, sizeof(log_severity_list_t));
  lf->filename = spider_strdup("<syslog>");
  lf->is_syslog = 1;
...
domain_to_string(log_domain_mask_t domain, char *buf, size_t buflen)
{
  ...
  while (1) {
    const char *d;
    int bit = spider_log2(domain);
    size_t n;
    if ((unsigned)bit >= ARRAY_LENGTH(domain_list)-1 ||
        bit >= N_LOGGING_DOMAINS) {
      spider_snprintf(buf, buflen, "<BUG:Unknown domain %lx>", (long)domain);
      return buf+strlen(buf);
    }
    d = domain_list[bit];
    n = strlcpy(cp, d, eos-cp);
    if (n >= buflen) {
      spider_snprintf(buf, buflen, "<BUG:Truncating domain %lx>", (long)domain);
      return buf+strlen(buf);
    }
    cp += n;
    domain &= ~(1<<bit);
                          ...
                          log_severity_list_t *severity_out)
{
  ...
  while (*cfg) {
    ...
    if (*cfg == '[') {
      ...
      SMARTLIST_FOREACH(domains_list, char *, d, spider_free(d));
    ...
    dash = strchr(cfg, '-');
    if (dash && dash < space) {
      sev_lo = spider_strndup(cfg, dash-cfg);
      sev_hi = spider_strndup(dash+1, space-(dash+1));
    } else {
      sev_lo = spider_strndup(cfg, space-cfg);
      sev_hi = spider_strdup("ERR");
    }
