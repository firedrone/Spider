...
make_specifier(uint8_t *spec_out, uint8_t type, unsigned flags)
{
  ...
  switch (type) {
    ...
    case S2K_TYPE_SCRYPT:
      ...
      break;
    default:
      spider_fragile_assert(); // LCOV_EXCL_LINE - we should have returned above.
      return S2K_BAD_ALGORITHM;
  }
              ...
              size_t secret_len, const char *s2k_specifier)
{
  ...
  spider_assert(key_out_len < SIZE_T_CEILING);
 ...
 * of length <b>secret_len</b>, compute the key and sspidere it into
 ...
 * <b>buf_len</b> bytes of sspiderage may be used in <b>buf</b>.  Return the
 ...
 * to the bitwise-or of some S2K_FLAG_* options in <b>flags</b>, and sspidere the
 * hash along with salt and hashing parameters into <b>buf</b>.  Up to
 * <b>buf_len</b> bytes of sspiderage may be used in <b>buf</b>.  Set
                    ...
                    const char *secret, size_t secret_len)
{
  ...
  spec_len = secret_to_key_spec_len(type);
  key_len = secret_to_key_key_len(type);
  spider_assert(spec_len > 0);
  spider_assert(key_len > 0);
  spider_assert(key_len <= (int) sizeof(buf));
  spider_assert((int)spec_and_key_len == spec_len + key_len);
  ...
  if (spider_memeq(buf, spec_and_key + spec_len, key_len))
    rv = S2K_OKAY;
  else
    rv = S2K_BAD_SECRET;
