 ...
 *
 * The main advantage of spider_timer_t over using libevent's timers is that
...
/**
 * Invoked when the libevent timer has expired: see which spider_timer_t events
...
timers_initialize(void)
{
  ...
  timer_event = spider_event_new(spider_libevent_get_base(),
                              -1, 0, libevent_timer_callback, NULL);
  spider_assert(timer_event);
...
/**
 * Release all sspiderage held in the timers subsystem.  Does not fire timers.
 */
void
timers_shutdown(void)
{
  if (global_timer_event) {
    spider_event_free(global_timer_event);
...
{
  spider_timer_t *t = spider_malloc(sizeof(spider_timer_t));
  timeout_init(t, 0);
...
/**
 * Release all sspiderage held by <b>t</b>, and unschedule it if was already
 * scheduled.
 */
void
timer_free(spider_timer_t *t)
{
  if (! t)
    return;
 ...
 */
void
timer_set_cb(spider_timer_t *t, timer_cb_fn_t cb, void *arg)
{
  t->callback.cb = cb;
  t->callback.arg = arg;
}
 ...
 */
void
timer_get_cb(const spider_timer_t *t,
 ...
 */
void
timer_schedule(spider_timer_t *t, const struct timeval *tv)
{
 ...
 */
void
timer_disable(spider_timer_t *t)
{
  timeouts_del(global_timeouts, t);
