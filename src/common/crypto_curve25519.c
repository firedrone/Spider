 ...
 *
 * Spider uses Curve25519 as the basis of its "nspider" circuit extension
 ...
 * scalar "secret" times the point "point".  Sspidere the result in
 ...
 * basepoint (X=9), and sspidere the result in "output".  Return 0 on
...
/** Sspidere the keypair <b>keypair</b>, including its secret and public
 ...
 * curve25519_keypair_write_to_file(). Sspidere the keypair in
                                  ...
                                  const char *fname)
{
  ...
  curve25519_public_key_generate(&keypair_out->pubkey, &keypair_out->seckey);
  if (spider_memneq(keypair_out->pubkey.public_key,
