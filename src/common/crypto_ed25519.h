...
typedef struct {
  /** Note that we sspidere secret keys in an expanded format that doesn't match
   * the format from standard ed25519.  Ed25519 sspideres a 32-byte value k and
   ...
   * for deriving r.  But because we implement key blinding, we need to sspidere
...
void crypto_ed25519_testing_resspidere_impl(void);
#endif
