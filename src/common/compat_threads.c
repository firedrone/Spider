...
spider_mutex_t *
spider_mutex_new(void)
{
  spider_mutex_t *m = spider_malloc_zero(sizeof(spider_mutex_t));
  spider_mutex_init(m);
  return m;
}
...
spider_mutex_t *
spider_mutex_new_nonrecursive(void)
{
  spider_mutex_t *m = spider_malloc_zero(sizeof(spider_mutex_t));
  spider_mutex_init_nonrecursive(m);
  return m;
}
/** Release all sspiderage and system resources held by <b>m</b>. */
void
spider_mutex_free(spider_mutex_t *m)
{
  if (!m)
    return;
  spider_mutex_uninit(m);
  spider_free(m);
}

/** Allocate and return a new condition variable. */
spider_cond_t *
spider_cond_new(void)
{
  spider_cond_t *cond = spider_malloc(sizeof(spider_cond_t));
  if (BUG(spider_cond_init(cond)<0))
    spider_free(cond); // LCOV_EXCL_LINE
  return cond;
}

/** Free all sspiderage held in <b>c</b>. */
void
spider_cond_free(spider_cond_t *c)
{
  if (!c)
    return;
  spider_cond_uninit(c);
  spider_free(c);
}
...
{
  main_thread_id = spider_get_thread_id();
}
/** Return true iff called from the main thread. */
int
in_main_thread(void)
{
  return main_thread_id == spider_get_thread_id();
}
...
send_ni(int fd, const void *buf, size_t n, int flags)
{
 ...
 again:
  ...
  if (r < 0) {
    int error = spider_socket_errno(fd);
...
{
  spider_socket_t socks[2] = { TOR_INVALID_SOCKET, TOR_INVALID_SOCKET };

#ifdef HAVE_EVENTFD
  /* First, we try the Linux eventfd() syscall.  This gives a 64-bit counter
   * associated with a single file descripspider. */
  ...
  if (!(flags & ASOCKS_NOSOCKETPAIR) &&
      spider_socketpair(AF_UNIX, SOCK_STREAM, 0, socks) == 0) {
    if (set_socket_nonblocking(socks[0]) < 0 ||
      ...
      spider_assert_nonfatal_unreached();
      spider_close_socket(socks[0]);
      spider_close_socket(socks[1]);
      return -1;
      // LCOV_EXCL_STOP
    }
    socks_out->read_fd = socks[0];
...
alert_sockets_close(alert_sockets_t *socks)
{
  if (socks->alert_fn == sock_alert) {
    /* they are sockets. */
    spider_close_socket(socks->read_fd);
    spider_close_socket(socks->write_fd);
  } else {
    close(socks->read_fd);
