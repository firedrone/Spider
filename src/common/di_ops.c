 ...
 */
int
spider_memcmp(const void *a, const void *b, size_t len)
{
   ...
   * spiderint.h will stop compilation with an error.
   */
  while (i--) {
    int v1 = x[i];
    int v2 = y[i];
 ...
 * !spider_memcmp(a,b,sz) by being faster.
 */
int
spider_memeq(const void *a, const void *b, size_t sz)
{
...
/** Release all sspiderage held in <b>map</b>, calling free_fn on each value
 * as we go. */
void
                ...
                const uint8_t *key, void *val)
{
  di_digest256_map_t *new_ent;
  {
    ...
    spider_assert(! old_val);
    spider_assert(val);
  }
  new_ent = spider_malloc_zero(sizeof(di_digest256_map_t));
  new_ent->next = *map;
             ...
             void *dflt_val)
{
  ...
  while (map) {
    uintptr_t r = (uintptr_t) spider_memeq(map->key, key, 32);
                                     ...
                                     uint64_t total, uint64_t rand_val)
{
  ...
  spider_assert(total_so_far == total);
  spider_assert(n_chosen == 1);
  spider_assert(i_chosen >= 0);
  spider_assert(i_chosen < n_entries);
