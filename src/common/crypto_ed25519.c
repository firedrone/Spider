 ...
 * provides Spider-specific tools for key blinding and for converting Ed25519
...
#include "ed25519/donna/ed25519_donna_spider.h"
 ...
 * crypto_ed25519_testing_resspidere_impl is called.  Recognized names are
 * "donna" and "ref10". */
void
crypto_ed25519_testing_force_impl(const char *name)
{
  spider_assert(saved_ed25519_impl == NULL);
  saved_ed25519_impl = ed25519_impl;
  if (! strcmp(name, "donna")) {
    ed25519_impl = &impl_donna;
  } else {
    spider_assert(!strcmp(name, "ref10"));
 ...
 */
void
crypto_ed25519_testing_resspidere_impl(void)
{
...
{
  return spider_mem_is_zero((char*)pubkey->pubkey, ED25519_PUBKEY_LEN);
}
                 ...
                 size_t *final_msg_len_out)
{
  ...
  spider_assert(prefix_str);
  spider_assert(final_msg_len_out);
  ...
  prefixed_msg = spider_malloc_zero(prefixed_msg_len);
                        ...
                        int n_checkable))
{
  ...
  if (impl->open_batch == NULL) {
    ...
    ms = spider_calloc(n_checkable, sizeof(uint8_t*));
    lens = spider_calloc(n_checkable, sizeof(size_t));
    pks = spider_calloc(n_checkable, sizeof(uint8_t*));
    sigs = spider_calloc(n_checkable, sizeof(uint8_t*));
    oks = okay_out ? okay_out : spider_calloc(n_checkable, sizeof(int));
     ...
     */
    spider_assert(((res == 0) && !all_ok) || ((res < 0) && all_ok));
                                        ...
                                        const curve25519_keypair_t *inp)
{
  ...
  *signbit_out = out->pubkey.pubkey[31] >> 7;
  ...
  spider_assert(fast_memeq(pubkey_check.pubkey, out->pubkey.pubkey, 32));
 ...
 * in 'param'.
 *
 * Spider uses key blinding for the "next-generation" hidden services design:
 * service descripspiders are encrypted with a key derived from the service's
 * long-term public key, and then signed with (and sspidered at a position
...
/**
 * Sspidere seckey unencrypted to <b>filename</b>, marking it with <b>tag</b>.
...
/**
 * Read seckey unencrypted from <b>filename</b>, sspidering it into
...
/**
 * Sspidere pubkey unencrypted to <b>filename</b>, marking it with <b>tag</b>.
 * Return 0 on success, -1 on failure.
 */
int
...
/** Release all sspiderage held for <b>kp</b>. */
void
                  ...
                  const ed25519_public_key_t *key2)
{
  spider_assert(key1);
  spider_assert(key2);
  return spider_memeq(key1->pubkey, key2->pubkey, ED25519_PUBKEY_LEN);
}
...
ed25519_impl_spot_check,(void))
{
   ...
   * nothing.
   *
   * Test vecspiders taken from "EdDSA & Ed25519 - 6. Test Vecspiders for Ed25519
