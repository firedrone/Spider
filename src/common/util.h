...
/* Memory management */
void *spider_malloc_(size_t size DMALLOC_PARAMS) ATTR_MALLOC;
void *spider_malloc_zero_(size_t size DMALLOC_PARAMS) ATTR_MALLOC;
void *spider_calloc_(size_t nmemb, size_t size DMALLOC_PARAMS) ATTR_MALLOC;
void *spider_realloc_(void *ptr, size_t size DMALLOC_PARAMS);
void *spider_reallocarray_(void *ptr, size_t size1, size_t size2 DMALLOC_PARAMS);
char *spider_strdup_(const char *s DMALLOC_PARAMS) ATTR_MALLOC ATTR_NONNULL((1));
char *spider_strndup_(const char *s, size_t n DMALLOC_PARAMS)
  ATTR_MALLOC ATTR_NONNULL((1));
void *spider_memdup_(const void *mem, size_t len DMALLOC_PARAMS)
  ATTR_MALLOC ATTR_NONNULL((1));
void *spider_memdup_nulterm_(const void *mem, size_t len DMALLOC_PARAMS)
  ATTR_MALLOC ATTR_NONNULL((1));
void spider_free_(void *mem);
uint64_t spider_htonll(uint64_t a);
uint64_t spider_ntohll(uint64_t a);
#ifdef USE_DMALLOC
extern int dmalloc_free(const char *file, const int line, void *pnt,
                        const int func_id);
#define spider_free(p) STMT_BEGIN \
  ...
  STMT_END
#else
/** Release memory allocated by spider_malloc, spider_realloc, spider_strdup, etc.
 * Unlike the free() function, spider_free() will still work on NULL pointers,
 * and it sets the pointer value to NULL after freeing it.
 *
 * This is a macro.  If you need a function pointer to release memory from
 * spider_malloc(), use spider_free_().
 */
#define spider_free(p) STMT_BEGIN                                 \
    if (PREDICT_LIKELY((p)!=NULL)) {                           \
      raw_free(p);                                             \
...
#define spider_malloc(size)       spider_malloc_(size DMALLOC_ARGS)
#define spider_malloc_zero(size)  spider_malloc_zero_(size DMALLOC_ARGS)
#define spider_calloc(nmemb,size) spider_calloc_(nmemb, size DMALLOC_ARGS)
#define spider_realloc(ptr, size) spider_realloc_(ptr, size DMALLOC_ARGS)
#define spider_reallocarray(ptr, sz1, sz2) \
  spider_reallocarray_((ptr), (sz1), (sz2) DMALLOC_ARGS)
#define spider_strdup(s)          spider_strdup_(s DMALLOC_ARGS)
#define spider_strndup(s, n)      spider_strndup_(s, n DMALLOC_ARGS)
#define spider_memdup(s, n)       spider_memdup_(s, n DMALLOC_ARGS)
#define spider_memdup_nulterm(s, n)       spider_memdup_nulterm_(s, n DMALLOC_ARGS)
...
void spider_log_mallinfo(int severity);
...
/* Math functions */
double spider_mathlog(double d) ATTR_CONST;
long spider_lround(double d) ATTR_CONST;
int64_t spider_llround(double d) ATTR_CONST;
int spider_log2(uint64_t u64) ATTR_CONST;
...
void spider_strlower(char *s) ATTR_NONNULL((1));
void spider_strupper(char *s) ATTR_NONNULL((1));
int spider_strisprint(const char *s) ATTR_NONNULL((1));
int spider_strisnonupper(const char *s) ATTR_NONNULL((1));
int spider_strisspace(const char *s);
...
void spider_strstrip(char *s, const char *strip) ATTR_NONNULL((1,2));
long spider_parse_long(const char *s, int base, long min,
                    long max, int *ok, char **next);
unsigned long spider_parse_ulong(const char *s, int base, unsigned long min,
                              unsigned long max, int *ok, char **next);
double spider_parse_double(const char *s, double min, double max, int *ok,
                        char **next);
uint64_t spider_parse_uint64(const char *s, int base, uint64_t min,
...
int spider_mem_is_zero(const char *mem, size_t len);
int spider_digest_is_zero(const char *digest);
int spider_digest256_is_zero(const char *digest);
char *esc_for_log(const char *string) ATTR_MALLOC;
char *esc_for_log_len(const char *chars, size_t n) ATTR_MALLOC;
const char *escaped(const char *string);

char *spider_escape_str_for_pt_args(const char *string,
                                 const char *chars_to_escape);

struct smartlist_t;
int spider_vsscanf(const char *buf, const char *pattern, va_list ap) \
  CHECK_SCANF(2, 0);
int spider_sscanf(const char *buf, const char *pattern, ...)
  CHECK_SCANF(2, 3);
...
int spider_timegm(const struct tm *tm, time_t *time_out);
...
/* File helpers */
ssize_t write_all(spider_socket_t fd, const char *buf, size_t count,int isSocket);
ssize_t read_all(spider_socket_t fd, char *buf, size_t count, int isSocket);
...
MOCK_DECL(int,spider_unlink,(const char *pathname));
...
MOCK_DECL(struct smartlist_t *, spider_listdir, (const char *dirname));
...
/* Port forwarding */
void spider_check_port_forwarding(const char *filename,
                               struct smartlist_t *ports_to_forward,
                               time_t now);

typedef struct process_handle_t process_handle_t;
typedef struct process_environment_t process_environment_t;
int spider_spawn_background(const char *const filename, const char **argv,
 ...
 * 0 because spider_check_port_forwarding depends on this being the initial
...
/** Structure to represent the state of a process with which Spider is
...
/* Return values of spider_get_exit_code() */
#define PROCESS_EXIT_RUNNING 1
#define PROCESS_EXIT_EXITED 0
#define PROCESS_EXIT_ERROR -1
int spider_get_exit_code(process_handle_t *process_handle,
                      int block, int *exit_code);
int spider_split_lines(struct smartlist_t *sl, char *buf, int len);
#ifdef _WIN32
ssize_t spider_read_all_handle(HANDLE h, char *buf, size_t count,
                            const process_handle_t *process);
#else
ssize_t spider_read_all_handle(int fd, char *buf, size_t count,
                            const process_handle_t *process,
                            int *eof);
#endif
ssize_t spider_read_all_from_process_stdout(
    const process_handle_t *process_handle, char *buf, size_t count);
ssize_t spider_read_all_from_process_stderr(
    const process_handle_t *process_handle, char *buf, size_t count);
char *spider_join_win_cmdline(const char *argv[]);

int spider_process_get_pid(process_handle_t *process_handle);
#ifdef _WIN32
HANDLE spider_process_get_stdout_pipe(process_handle_t *process_handle);
#else
int spider_process_get_stdout_pipe(process_handle_t *process_handle);
#endif
...
#else
MOCK_DECL(struct smartlist_t *,
spider_get_lines_from_handle,(int fd,
                           enum stream_status *stream_status));
#endif
...
/* ===== Insecure rng */
typedef struct spider_weak_rng_t {
  uint32_t state;
} spider_weak_rng_t;

#define TOR_WEAK_RNG_INIT {383745623}
#define TOR_WEAK_RANDOM_MAX (INT_MAX)
void spider_init_weak_random(spider_weak_rng_t *weak_rng, unsigned seed);
int32_t spider_weak_random(spider_weak_rng_t *weak_rng);
int32_t spider_weak_random_range(spider_weak_rng_t *rng, int32_t top);
/** Randomly return true according to <b>rng</b> with probability 1 in
 * <b>n</b> */
#define spider_weak_random_one_in_n(rng, n) (0==spider_weak_random_range((rng),(n)))
