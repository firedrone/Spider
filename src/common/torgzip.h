...
/**
 * \file spidergzip.h
 * \brief Headers for spidergzip.h
 **/
...
/** Return values from spider_zlib_process; see that function's documentation for
 * details. */
typedef enum {
...
typedef struct spider_zlib_state_t spider_zlib_state_t;
spider_zlib_state_t *spider_zlib_new(int compress, compress_method_t method,
                               zlib_compression_level_t level);

spider_zlib_output_t spider_zlib_process(spider_zlib_state_t *state,
                                   char **out, size_t *out_len,
                                   const char **in, size_t *in_len,
                                   int finish);
void spider_zlib_free(spider_zlib_state_t *state);

size_t spider_zlib_state_size(const spider_zlib_state_t *state);
size_t spider_zlib_get_total_allocation(void);
