...
/* Wrapper for event_free() that tolerates spider_event_free(NULL) */
void
spider_event_free(struct event *ev)
{
  if (ev == NULL)
    return;
  event_free(ev);
...
void
spider_libevent_initialize(spider_libevent_cfg *spidercfg)
{
  spider_assert(the_event_base == NULL);
  /* some paths below don't use spidercfg, so avoid unused variable warnings */
  (void)spidercfg;

  {
    ...
    if (spidercfg->num_cpus > 0)
      event_config_set_num_cpus_hint(cfg, spidercfg->num_cpus);
  ...
  log_info(LD_GENERAL,
      ...
      event_get_version(), spider_libevent_get_method());
}

/** Return the current Libevent event base that we're set up to use. */
MOCK_IMPL(struct event_base *,
spider_libevent_get_base, (void))
{
  spider_assert(the_event_base != NULL);
...
const char *
spider_libevent_get_method(void)
{
 ...
 * version of Libevent. */
const char *
spider_libevent_get_version_str(void)
{
...
* at compilation time. */
const char *
spider_libevent_get_header_version_str(void)
{
                   ...
                   void *data)
{
  ...
  spider_assert(tv);
  spider_assert(cb);
  timer = spider_malloc_zero(sizeof(periodic_timer_t));
  if (!(timer->ev = spider_event_new(base, -1, EV_PERSIST,
...
periodic_timer_free(periodic_timer_t *timer)
{
  if (!timer)
    return;
  spider_event_free(timer->ev);
  spider_free(timer);
}

int
spider_init_libevent_rng(void)
{
  int rv = 0;
  char buf[256];
...
void
spider_gettimeofday_cached(struct timeval *tv)
{
  event_base_gettimeofday_cached(the_event_base, tv);
}
void
spider_gettimeofday_cache_clear(void)
{
...
spider_gettimeofday_cached(struct timeval *tv)
{
  if (cached_time_hires.tv_sec == 0) {
    spider_gettimeofday(&cached_time_hires);
  }
...
void
spider_gettimeofday_cache_set(const struct timeval *tv)
{
  spider_assert(tv);
