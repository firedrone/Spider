...
/** Write a usage message for spider-gencert to stderr. */
static void
show_help(void)
{
  fprintf(stderr, "Syntax:\n"
          "spider-gencert [-h|--help] [-v] [-r|--reuse] [--create-identity-key]\n"
...
crypto_log_errors(int severity, const char *doing)
{
  ...
  while ((err = ERR_get_error()) != 0) {
    ...
    if (doing) {
      ...
      spider_log(severity, LD_CRYPTO, "crypto error: %s (in %s:%s)",
              msg, lib, func);
    }
  }
}
...
load_passphrase(void)
{
  ...
  passphrase = spider_strndup(buf, passphrase_len);
...
parse_commandline(int argc, char **argv)
{
  ...
  for (i = 1; i < argc; ++i) {
    if (!strcmp(argv[i], "--help") || !strcmp(argv[i], "-h")) {
      ...
      identity_key_file = spider_strdup(argv[++i]);
      ...
      signing_key_file = spider_strdup(argv[++i]);
      ...
      certificate_file = spider_strdup(argv[++i]);
      ...
      spider_inet_ntoa(&in, b, sizeof(b));
      spider_asprintf(&address, "%s:%d", b, (int)port);
  ...
  if (!identity_key_file) {
    identity_key_file = spider_strdup("./authority_identity_key");
    log_info(LD_GENERAL, "No identity key file given; defaulting to %s",
             identity_key_file);
  }
  if (!signing_key_file) {
    signing_key_file = spider_strdup("./authority_signing_key");
    log_info(LD_GENERAL, "No signing key file given; defaulting to %s",
             signing_key_file);
  }
  if (!certificate_file) {
    certificate_file = spider_strdup("./authority_certificate");
    log_info(LD_GENERAL, "No signing key file given; defaulting to %s",
             certificate_file);
  }
  if (passphrase_fd >= 0) {
 ...
 * sspidere it.  Return 0 on success, nonzero on failure.
 */
static int
...
key_to_string(EVP_PKEY *key)
{
  ...
  result = spider_malloc(buf->length + 1);
...
main(int argc, char **argv)
{
  ...
  spider_free(address);
  spider_free(identity_key_file);
  spider_free(signing_key_file);
  spider_free(certificate_file);
  spider_free(address);
