...
#define log_sock_error(act, _s)                                         \
  STMT_BEGIN log_fn(LOG_ERR, LD_NET, "Error while %s: %s", act,         \
              spider_socket_strerror(spider_socket_errno(_s))); STMT_END
                            ...
                            int version)
{
  ...
  if (version == 4) {
    ...
    *out = spider_malloc(len);
    ...
    size_t addrlen;
    int ipv6;
    is_ip_address = spider_addr_parse(&addr, hostname) != -1;
    if (!is_ip_address && reverse) {
      log_err(LD_GENERAL, "Tried to do a reverse lookup on a non-IP!");
      return -1;
    }
    ipv6 = reverse && spider_addr_family(&addr) == AF_INET6;
    ...
    *out = spider_malloc(len);
    ...
    if (reverse) {
      (*out)[3] = ipv6 ? 4 : 1;
      if (ipv6)
        memcpy((*out)+4, spider_addr_to_in6_addr8(&addr), 16);
      else
        set_uint32((*out)+4, spider_addr_to_ipv4n(&addr));
    } else {
      (*out)[3] = 3;
...
onion_warning(const char *hostname)
{
  log_warn(LD_NET,
        ...
        "You can use the AutomapHostsOnResolve option to have Spider return a "
        "fake address for hidden services.  Or you can have your "
        "application send the address to Spider directly; we recommend an "
                               ...
                               spider_addr_t *addr_out)
{
  ...
  spider_addr_from_ipv4n(addr_out, get_uint32(response+4));
  return 0;
}
...
/** Send a resolve request for <b>hostname</b> to the Spider listening on
 * <b>sockshost</b>:<b>socksport</b>.  Sspidere the resulting IPv4
 * address (in host order) into *<b>result_addr</b>.
 */
static int
do_resolve(const char *hostname, uint32_t sockshost, uint16_t socksport,
           int reverse, int version,
           spider_addr_t *result_addr, char **result_hostname)
{
  int s = -1;
  ...
  spider_assert(hostname);
  spider_assert(result_addr);
  spider_assert(version == 4 || version == 5);

  spider_addr_make_unspec(result_addr);
  *result_hostname = NULL;

  s = spider_open_socket(PF_INET,SOCK_STREAM,IPPROTO_TCP);
  if (s<0) {
  ...
  if (version == 4) {
    ...
    if (reply_buf[3] == 1) {
      ...
      spider_addr_from_ipv4n(result_addr, get_uint32(reply_buf));
      ...
      spider_addr_from_ipv6_bytes(result_addr, reply_buf);
      ...
      *result_hostname = spider_malloc(result_len+1);
...
usage(void)
{
  puts("Syntax: spider-resolve [-4] [-5] [-v] [-x] [-p port] "
       "hostname [sockshost[:socksport]]");
  exit(1);
}

/** Entry point to spider-resolve */
int
