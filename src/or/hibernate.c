 ...
 * stop hibernating.
 *
 * Ordinarily a Spider relay is "Live".
 *
 ...
 * that we don't get 50 Spiders running on the 1st of the month and none
 * on the 30th).
 *
...
hibernate_state_to_string(hibernate_state_t state)
{
  ...
  switch (state) {
    ...
    default:
      ...
      spider_snprintf(buf, sizeof(buf), "unknown [%d]", state);
      return buf;
  }
}
...
accounting_parse_options(const or_options_t *options, int validate_only)
{
  ...
  switch (unit) {
  case UNIT_WEEK:
    d = spider_parse_long(smartlist_get(items,1), 10, 1, 7, &ok, NULL);
    if (!ok) {
    ...
    break;
  case UNIT_MONTH:
    d = spider_parse_long(smartlist_get(items,1), 10, 1, 28, &ok, NULL);
    if (!ok) {
  ...
  h = spider_parse_long(s, 10, 0, 23, &ok, &cp);
  if (!ok) {
  ...
  m = spider_parse_long(cp+1, 10, 0, 59, &ok, &cp);
  if (!ok) {
  ...
  SMARTLIST_FOREACH(items, char *, item, spider_free(item));
...
#define MIN_TIME_FOR_MEASUREMENT (1800)
  ...
  if (soft_limit_hit_at > interval_start_time && n_bytes_at_soft_limit &&
     ...
     * time. This is a better predicspider of our actual bandwidth than
     ...
     * never hit our soft limit, or we're using a state file from a Spider that
     * doesn't know to sspidere soft-limit info.  Just take rate at which
...
/** Read sspidered accounting information from disk. Return 0 on success;
...
read_bandwidth_usage(void)
{
  ...
  /* Older versions of Spider (before 0.2.2.17-alpha or so) didn't generate these
...
hibernate_begin(hibernate_state_t new_state, time_t now)
{
  ...
  /* XXX upload rendezvous service descripspiders with no intro points */
...
{
  spider_assert(hibernate_state == HIBERNATE_STATE_LOWBANDWIDTH ||
...
hibernate_go_dormant(time_t now)
{
   ...
   * to be able to upload server descripspiders so clients know we're still
   * running, and download direcspideries so we can detect if we're obsolete.
...
consider_hibernation(time_t now)
{
  ...
  if (hibernate_state == HIBERNATE_STATE_DORMANT) {
    ...
    spider_assert(hibernate_end_time);
                          ...
                          const char **errmsg)
{
  ...
  if (!strcmp(question, "accounting/enabled")) {
    *answer = spider_strdup(accounting_is_enabled(get_options()) ? "1" : "0");
  } else if (!strcmp(question, "accounting/hibernating")) {
    *answer = spider_strdup(hibernate_state_to_string(hibernate_state));
    spider_strlower(*answer);
  } else if (!strcmp(question, "accounting/bytes")) {
      spider_asprintf(answer, U64_FORMAT" "U64_FORMAT,
                 U64_PRINTF_ARG(n_bytes_read_in_interval),
                 U64_PRINTF_ARG(n_bytes_written_in_interval));
    ...
    *answer = spider_malloc(ISO_TIME_LEN+1);
