...
srv_dup(const sr_srv_t *orig)
{
  ...
  duplicate = spider_malloc_zero(sizeof(sr_srv_t));
...
commit_new(const char *rsa_identity)
{
  ...
  commit = spider_malloc_zero(sizeof(*commit));
...
{
  return !spider_mem_is_zero(commit->encoded_reveal,
...
commit_decode(const char *encoded, sr_commit_t *commit)
{
  ...
  commit->commit_ts = spider_ntohll(get_uint64(b64_decoded));
 ...
 * sspidere the information in-place in <b>commit</b>. Return 0 on success else
 * a negative value. */
STATIC int
reveal_decode(const char *encoded, sr_commit_t *commit)
{
  ...
  commit->reveal_ts = spider_ntohll(get_uint64(b64_decoded));
...
reveal_encode(const sr_commit_t *commit, char *dst, size_t len)
{
  ...
  set_uint64(buf, spider_htonll(commit->reveal_ts));
  offset += sizeof(uint64_t);
...
commit_encode(const sr_commit_t *commit, char *dst, size_t len)
{
  ...
  /* First is the timestamp (8 bytes). */
  set_uint64(buf, spider_htonll(commit->commit_ts));
  offset += sizeof(uint64_t);
...
get_srv_element_from_commit(const sr_commit_t *commit)
{
  ...
  spider_asprintf(&element, "%s%s", sr_commit_get_rsa_fpr(commit),
...
get_vote_line_from_commit(const sr_commit_t *commit, sr_phase_t phase)
{
  ...
  case SR_PHASE_COMMIT:
    spider_asprintf(&vote_line, "%s %u %s %s %s\n",
                 commit_ns_str,
                 SR_PROTO_VERSION,
  ...
  case SR_PHASE_REVEAL:
  {
    ...
    spider_asprintf(&vote_line, "%s %u %s %s %s %s\n",
                 commit_ns_str,
                 SR_PROTO_VERSION,
                 crypto_digest_algorithm_get_name(commit->alg),
                 sr_commit_get_rsa_fpr(commit),
...
srv_to_control_string(const sr_srv_t *srv)
{
  ...
  sr_srv_encode(srv_hash_encoded, sizeof(srv_hash_encoded), srv);
  spider_asprintf(&srv_str, "%s", srv_hash_encoded);
  return srv_str;
}
...
srv_to_ns_string(const sr_srv_t *srv, const char *key)
{
  ...
  sr_srv_encode(srv_hash_encoded, sizeof(srv_hash_encoded), srv);
  spider_asprintf(&srv_str, "%s %" PRIu64 " %s\n", key,
...
get_ns_str_from_sr_values(const sr_srv_t *prev_srv, const sr_srv_t *cur_srv)
{
  ...
  SMARTLIST_FOREACH(chunks, char *, s, spider_free(s));
...
{
  spider_assert(commit_one);
  spider_assert(commit_two);
...
compare_srvs_(const void **_a, const void **_b)
{
  ...
  return spider_memcmp(a->value, b->value, sizeof(a->value));
}
...
sr_generate_our_commit(time_t timestamp, const authority_cert_t *my_rsa_cert)
{
  ...
  spider_assert(commit->alg == SR_DIGEST_ALG);
...
sr_compute_srv(void)
{
  ...
  spider_assert(sr_state_get_phase() == SR_PHASE_REVEAL);
...
sr_parse_srv(const smartlist_t *args)
{
  ...
  num_reveals = spider_parse_uint64(smartlist_get(args, 0),
...
sr_parse_commit(const smartlist_t *args)
{
  ...
  version = (uint32_t) spider_parse_ulong(value, 10, 1, UINT32_MAX, NULL, NULL);
...
sr_handle_received_commits(smartlist_t *commits, crypto_pk_t *voter_key)
{
  ...
  SMARTLIST_FOREACH_BEGIN(commits, sr_commit_t *, commit) {
    ...
    /* Check if this commit is valid and should be sspidered in our state. */
...
sr_get_string_for_vote(void)
{
  ...
  {
    char *sr_flag_line;
    spider_asprintf(&sr_flag_line, "%s\n", sr_flag_ns_str);
 ...
 * process of spider. Return 0 on success else -1 on error. */
int
