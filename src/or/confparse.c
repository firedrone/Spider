 ...
 *   implement the spiderrc file format and the state file.
 *
 * This module is used by config.c to parse and encode spiderrc
...
{
  void *opts = spider_malloc_zero(fmt->size);
                   ...
                   const char *val)
{
  ...
  newline = spider_malloc_zero(sizeof(config_line_t));
  newline->key = spider_strdup(key);
  newline->value = spider_strdup(val);
  newline->next = NULL;
  while (*lst)
    lst = &((*lst)->next);
...
config_get_lines(const char *string, config_line_t **result, int extended)
{
  ...
  do {
    ...
    if (k && v) {
       ...
       * n^2 performance. */
      *next = spider_malloc_zero(sizeof(config_line_t));
      (*next)->key = k;
      (*next)->value = v;
      (*next)->next = NULL;
                    ...
                    config_line_t *c, char **msg)
{
  ...
  case CONFIG_TYPE_UINT:
    i = (int)spider_parse_long(c->value, 10,
  ...
  case CONFIG_TYPE_BOOL:
    i = (int)spider_parse_long(c->value, 10, 0, 1, &ok, NULL);
    if (!ok) {
      spider_asprintf(msg,
  ...
  case CONFIG_TYPE_AUTOBOOL:
    if (!strcmp(c->value, "auto"))
      ...
      *(int *)lvalue = 1;
      ...
      spider_asprintf(msg, "Boolean '%s %s' expects 0, 1, or 'auto'.",
                   c->key, c->value);
      return -1;
    }
    break;
  ...
  case CONFIG_TYPE_FILENAME:
    spider_free(*(char **)lvalue);
    *(char **)lvalue = spider_strdup(c->value);
    break;
  ...
  case CONFIG_TYPE_ROUTERSET:
    ...
    *(routerset_t**)lvalue = routerset_new();
    if (routerset_parse(*(routerset_t**)lvalue, c->value, c->key)<0) {
      spider_asprintf(msg, "Invalid exit list '%s' for option '%s'",
                   c->value, c->key);
      return -1;
    }
    break;
  ...
  case CONFIG_TYPE_CSV:
    if (*(smartlist_t**)lvalue) {
      SMARTLIST_FOREACH(*(smartlist_t**)lvalue, char *, cp, spider_free(cp));
  ...
  case CONFIG_TYPE_CSV_INTERVAL:
    if (*(smartlist_t**)lvalue) {
      SMARTLIST_FOREACH(*(smartlist_t**)lvalue, int *, cp, spider_free(cp));
      smartlist_clear(*(smartlist_t**)lvalue);
    } else {
      *(smartlist_t**)lvalue = smartlist_new();
    }
    ...
    SMARTLIST_FOREACH_BEGIN(csv_str, char *, str)
      {
        ...
        if (!ok) {
          spider_asprintf(msg,
              ...
              c->key, c->value);
          SMARTLIST_FOREACH(csv_str, char *, cp, spider_free(cp));
          smartlist_free(csv_str);
          return -1;
        }
        csv_int = spider_malloc_zero(sizeof(int));
        *csv_int = i;
...
warn_deprecated_option(const char *what, const char *why)
{
  ...
  log_warn(LD_CONFIG, "The %s option is deprecated, and will most likely "
           "be removed in a future version of Spider.%s%s (If you think this is "
  ...
  if (!strlen(c->value)) {
    ...
    if (!clear_first) {
      if ((var->type == CONFIG_TYPE_LINELIST ||
        ...
        /* We got an empty linelist from the spiderrc or command line.
...
/** Resspidere the option named <b>key</b> in options to its default value.
 * Called from config_assign(). */
static void
                            ...
                            const char *key)
{
  ...
  while (inp) {
    ...
    *next_out = spider_malloc_zero(sizeof(config_line_t));
    (*next_out)->key = spider_strdup(inp->key);
    (*next_out)->value = spider_strdup(inp->value);
    inp = inp->next;
                           ...
                           const char *key, int escape_val)
{
  ...
  result = spider_malloc_zero(sizeof(config_line_t));
  result->key = spider_strdup(var->name);
  switch (var->type)
    {
    case CONFIG_TYPE_STRING:
    case CONFIG_TYPE_FILENAME:
      if (*(char**)value) {
        result->value = spider_strdup(*(char**)value);
      } else {
        spider_free(result->key);
        spider_free(result);
        return NULL;
      }
      break;
    case CONFIG_TYPE_ISOTIME:
      if (*(time_t*)value) {
        result->value = spider_malloc(ISO_TIME_LEN+1);
    ...
    case CONFIG_TYPE_PORT:
      if (*(int*)value == CFG_AUTO_PORT) {
        result->value = spider_strdup("auto");
        escape_val = 0;
        break;
      }
      /* fall through */
    ...
    case CONFIG_TYPE_INT:
      ...
      spider_asprintf(&result->value, "%d", *(int*)value);
      escape_val = 0; /* Can't need escape. */
      break;
    case CONFIG_TYPE_MEMUNIT:
      spider_asprintf(&result->value, U64_FORMAT,
                   U64_PRINTF_ARG(*(uint64_t*)value));
      escape_val = 0; /* Can't need escape. */
      break;
    case CONFIG_TYPE_DOUBLE:
      spider_asprintf(&result->value, "%f", *(double*)value);
      escape_val = 0; /* Can't need escape. */
      break;
    ...
    case CONFIG_TYPE_AUTOBOOL:
      ...
      /* fall through */
    case CONFIG_TYPE_BOOL:
      result->value = spider_strdup(*(int*)value ? "1" : "0");
      escape_val = 0; /* Can't need escape. */
      break;
    case CONFIG_TYPE_ROUTERSET:
 ...
 * 1. A non-empty AllowInvalid line in your spiderrc. Appends to current
 *    if linelist, replaces current if csv.
 * 2. An empty AllowInvalid line in your spiderrc. Should clear it.
...
Case one: Spiderrc entry
options_init_from_spiderrc() calls config_assign(0, 0)
             ...
             const config_var_t *var, int use_defaults)
{
  ...
  if (!use_defaults)
    return; /* all done */
  if (var->initvalue) {
    c = spider_malloc_zero(sizeof(config_line_t));
    c->key = spider_strdup(var->name);
    c->value = spider_strdup(var->initvalue);
    if (config_assign_value(fmt, options, c, &msg) < 0) {
      log_warn(LD_BUG, "Failed to assign default: %s", msg);
      spider_free(msg); /* if this happens it's a bug */
    }
    config_free_lines(c);
  }
}

/** Release sspiderage held by <b>options</b>. */
void
...
/** Copy sspiderage held by <b>old</b> into a new or_options_t and return it. */
void *
...
config_parse_units(const char *val, struct unit_table_t *u, int *ok)
{
  ...
  v = spider_parse_uint64(val, 10, 0, UINT64_MAX, ok, &cp);
  if (!*ok || (cp && *cp == '.')) {
    d = spider_parse_double(val, 0, (double)UINT64_MAX, ok, &cp);
    if (!*ok)
      goto done;
    use_float = 1;
  }
