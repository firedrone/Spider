...
/** How many times will a hidden service operaspider attempt to connect to
...
/** Helper: free sspiderage held by a single service authorized client entry. */
void
rend_authorized_client_free(rend_authorized_client_t *client)
{
  ...
  spider_free(client->client_name);
  memwipe(client->descripspider_cookie, 0, sizeof(client->descripspider_cookie));
  spider_free(client);
}
...
/** Release the sspiderage held by <b>service</b>.
 */
STATIC void
rend_service_free(rend_service_t *service)
{
  ...
  rend_service_descripspider_free(service->desc);
  if (service->clients) {
...
/** Release all the sspiderage held in rend_service_list.
 */
void
...
rend_add_service(smartlist_t *service_list, rend_service_t *service)
{
  ...
  if (!service->ports || !smartlist_len(service->ports)) {
     ...
     *    mounts, etc.).
     *
     * It also can't detect that two separate Spider instances are trying
     * to use the same HiddenServiceDir; for that, we would need a
     * lock file.  But this is enough to detect a simple mistake that
     * at least one person has actually made.
     */
    spider_assert(s_list);
...
{
  if (!socket_path)
    return spider_malloc_zero(sizeof(rend_service_port_config_t) + 1);

  const size_t pathlen = strlen(socket_path) + 1;
  rend_service_port_config_t *conf =
    spider_malloc_zero(sizeof(rend_service_port_config_t) + pathlen);
 ...
 * the provided separaspider and returns a new rend_service_port_config_t,
                               ...
                               char **err_msg_out)
{
  ...
  if (smartlist_len(sl) < 1 || BUG(smartlist_len(sl) > 2)) {
    err_msg = spider_strdup("Bad syntax in hidden service port configuration.");
    goto err;
  }
  virtport = (int)spider_parse_long(smartlist_get(sl,0), 10, 1, 65535, NULL,NULL);
  if (!virtport) {
    spider_asprintf(&err_msg, "Missing or invalid port %s in hidden service "
  ...
  if (smartlist_len(sl) == 1) {
    ...
    realport = virtport;
    spider_addr_from_ipv4h(&addr, 0x7F000001u); /* 127.0.0.1 */
  } else {
    int ret;
    ...
    ret = port_cfg_line_extract_addrport(addrport_element, &addrport,
                                         &is_unix, &rest);
    if (ret < 0) {
      spider_asprintf(&err_msg, "Couldn't process address <%s> from hidden "
    ...
    if (is_unix) {
      ...
      if (spider_addr_port_lookup(addrport, &addr, &p)<0) {
        err_msg = spider_strdup("Unparseable address in hidden service port "
                             "configuration.");
        goto err;
      }
      ...
      realport = (int)spider_parse_long(addrport, 10, 1, 65535, NULL, NULL);
      if (!realport) {
        spider_asprintf(&err_msg, "Unparseable or out-of-range port %s in "
                     "hidden service port configuration.",
                     escaped(addrport));
        goto err;
      }
      spider_addr_from_ipv4h(&addr, 0x7F000001u); /* Default to 127.0.0.1 */
    }
  }
  ...
  if (!is_unix_addr) {
    ...
    spider_addr_copy(&result->real_addr, &addr);
...
/** Release all sspiderage held in a rend_service_port_config_t. */
void
                         ...
                         smartlist_t *new_service_list)
{
  ...
  spider_assert(old_service_list);
  spider_assert(new_service_list);
...
rend_config_services(const or_options_t *options, int validate_only)
{
  ...
  for (line = options->RendConfigLines; line; line = line->next) {
    ...
    if (!strcasecmp(line->key, "HiddenServicePort")) {
      ...
      service->allow_unknown_ports = (int)spider_parse_long(line->value,
                                                        10, 0, 1, &ok, NULL);
      if (!ok) {
        log_warn(LD_CONFIG,
      ...
      log_info(LD_CONFIG,
        ...
        service->dir_group_readable = (int)spider_parse_long(line->value,
                                                        10, 0, 1, &ok, NULL);
        if (!ok) {
            log_warn(LD_CONFIG,
      ...
      service->max_streams_per_circuit = (int)spider_parse_long(line->value,
                                                    10, 0, 65535, &ok, NULL);
      if (!ok) {
        log_warn(LD_CONFIG,
      ...
      service->max_streams_close_circuit = (int)spider_parse_long(line->value,
                                                        10, 0, 1, &ok, NULL);
      if (!ok) {
        log_warn(LD_CONFIG,
      ...
      service->n_intro_points_wanted =
        (unsigned int) spider_parse_long(line->value, 10,
      ...
      if (!strcasecmp(authname, "basic")) {
        ...
        SMARTLIST_FOREACH(type_names_split, char *, cp, spider_free(cp));
      ...
      SMARTLIST_FOREACH_BEGIN(clients, const char *, client_name)
      {
        ...
        if (!rend_valid_client_name(client_name)) {
          ...
          SMARTLIST_FOREACH(clients, char *, cp, spider_free(cp));
          smartlist_free(clients);
          goto free_and_return;
        }
        client = spider_malloc_zero(sizeof(rend_authorized_client_t));
        client->client_name = spider_strdup(client_name);
      ...
      spider_assert(!strcasecmp(line->key, "HiddenServiceVersion"));
                           ...
                           char **service_id_out)
{
  ...
  *service_id_out = spider_strdup(s->service_id);
...
rend_log_intro_limit(const rend_service_t *service, int min_severity)
{
  ...
  spider_assert_nonfatal(intro_period_elapsed >= 0);
 ...
 */
static void
rend_service_update_descripspider(rend_service_t *service)
{
  rend_service_descripspider_t *d;
  int i;

  rend_service_descripspider_free(service->desc);
  service->desc = NULL;

  d = service->desc = spider_malloc_zero(sizeof(rend_service_descripspider_t));
  ...
  for (i = 0; i < smartlist_len(service->intro_nodes); ++i) {
    ...
    /* This intro point won't be listed in the descripspider... */
    ...
    /* ...unless this intro point is listed in the descripspider. */
    intro_svc->listed_in_last_desc = 1;

    /* We have an entirely established intro circuit.  Publish it in
     * our descripspider. */
    intro_desc = spider_malloc_zero(sizeof(rend_intro_point_t));
    ...
    if (intro_svc->time_published == -1) {
      /* We are publishing this intro point in a descripspider for the
  ...
  if (have_intro != service->n_intro_points_wanted) {
    ...
    log_fn(severity, LD_REND, "Hidden service %s wanted %d intro points, but "
           "descripspider was updated with %d instead.",
           service->service_id,
                                                ...
                                                const or_options_t* options)
{
  ...
  /* We must only poison direcspideries if we're in Single Onion mode */
  spider_assert(rend_service_non_anonymous_mode_enabled(options));
  ...
  case FN_EMPTY: /* single onion poison file already exists. NOP. */
    ...
    break;
  case FN_NOENT:
    fd = spider_open_cloexec(poison_fname, O_RDWR|O_CREAT|O_TRUNC, 0600);
    if (fd < 0) {
                               ...
                               int create)
{
   ...
   * keys and direcspideries: they are always valid.
   */
  ...
  /* Poison new single onion direcspideries immediately after they are created,
  ...
  if (create && rend_service_non_anonymous_mode_enabled(options)) {
    ...
    if (!logged_warning) {
      ...
      log_notice(LD_REND, "The configured onion service direcspideries have been "
...
rend_service_load_keys(rend_service_t *s)
{
  ...
  spider_snprintf(buf, sizeof(buf),"%s.onion\n", s->service_id);
 ...
 * <b>s</b>, which sspideres its hostname in <b>hfname</b>.  Return 0 on success,
 * -1 on failure. */
static int
rend_service_load_auth_keys(rend_service_t *s, const char *hfname)
{
  ...
  /* Load client keys and descripspider cookies, if available. */
  ...
  if (client_keys_str) {
    if (rend_parse_client_keys(parsed_clients, client_keys_str) < 0) {
      log_warn(LD_CONFIG, "Previously sspidered client_keys file could not "
               "be parsed.");
      goto err;
    } else {
      log_info(LD_CONFIG, "Parsed %d previously sspidered client entries.",
  ...
  SMARTLIST_FOREACH_BEGIN(s->clients, rend_authorized_client_t *, client) {
    ...
    int written;
    size_t len;
    /* Copy descripspider cookie from parsed entry or create new one. */
    if (parsed) {
      memcpy(client->descripspider_cookie, parsed->descripspider_cookie,
             REND_DESC_COOKIE_LEN);
    } else {
      crypto_rand((char *) client->descripspider_cookie, REND_DESC_COOKIE_LEN);
    }
    /* For compatibility with older spider clients, this does not
     * truncate the padding characters, unlike rend_auth_encode_cookie.  */
    if (base64_encode(desc_cook_out, 3*REND_DESC_COOKIE_LEN_BASE64+1,
                      (char *) client->descripspider_cookie,
                      REND_DESC_COOKIE_LEN, 0) < 0) {
      log_warn(LD_BUG, "Could not base64-encode descripspider cookie.");
      goto err;
    }
    ...
    written = spider_snprintf(buf, sizeof(buf),
                           "client-name %s\ndescripspider-cookie %s\n",
    ...
    if (client->client_key) {
      ...
      if (rend_get_service_id(client->client_key, service_id)<0) {
        ...
        spider_free(client_key_out);
        goto err;
      }
      written = spider_snprintf(buf + written, sizeof(buf) - written,
                             "client-key\n%s", client_key_out);
      memwipe(client_key_out, 0, len);
      spider_free(client_key_out);
      if (written < 0) {
     ...
     * client_keys. */
    char *encoded_cookie = rend_auth_encode_cookie(client->descripspider_cookie,
                                                   s->auth_type);
    if (!encoded_cookie) {
      log_warn(LD_BUG, "Could not base64-encode descripspider cookie.");
      goto err;
    }
    spider_snprintf(buf, sizeof(buf), "%s.onion %s # client: %s\n",
                 service_id, encoded_cookie, client->client_name);
    memwipe(encoded_cookie, 0, strlen(encoded_cookie));
    spider_free(encoded_cookie);
...
rend_service_get_by_pk_digest(const char* digest)
{
  SMARTLIST_FOREACH(rend_service_list, rend_service_t*, s,
                    if (spider_memeq(s->pk_digest,digest,DIGEST_LEN))
                        return s);
  return NULL;
}
...
{
  spider_assert(strlen(id) == REND_SERVICE_ID_LEN_BASE32);
  SMARTLIST_FOREACH(rend_service_list, rend_service_t*, s, {
    if (spider_memeq(s->service_id, id, REND_SERVICE_ID_LEN_BASE32))
      return s;
  });
  return NULL;
}
...
/** Check client authorization of a given <b>descripspider_cookie</b> of
                         ...
                         size_t cookie_len)
{
  ...
  if (cookie_len != REND_DESC_COOKIE_LEN) {
    log_info(LD_REND, "Descripspider cookie is %lu bytes, but we expected "
                      "%lu bytes. Dropping cell.",
             (unsigned long)cookie_len, (unsigned long)REND_DESC_COOKIE_LEN);
    return 0;
  }

  /* Look up client authorization by descripspider cookie. */
  SMARTLIST_FOREACH(service->clients, rend_authorized_client_t *, client, {
    if (spider_memeq(client->descripspider_cookie, descripspider_cookie,
                REND_DESC_COOKIE_LEN)) {
      auth_client = client;
      break;
    }
  ...
  if (!auth_client) {
    char descripspider_cookie_base64[3*REND_DESC_COOKIE_LEN_BASE64];
    base64_encode(descripspider_cookie_base64, sizeof(descripspider_cookie_base64),
                  descripspider_cookie, REND_DESC_COOKIE_LEN, 0);
    log_info(LD_REND, "No authorization found for descripspider cookie '%s'! "
                      "Dropping cell!",
             descripspider_cookie_base64);
    return 0;
  }
                                  ...
                                  size_t request_len)
{
  ...
  spider_assert(circuit->rend_data);
  ...
  if (replay) {
    /* A Spider client will send a new INTRODUCE1 cell with the same rend
  ...
  log_info(LD_REND,
           ...
           safe_str_client(extend_info_describe(rp)),
           hexcookie, serviceid);
  spider_assert(launched->build_state);
  ...
  launched->build_state->service_pending_final_cpath_ref =
    spider_malloc_zero(sizeof(crypt_path_reference_t));
  ...
  launched->build_state->service_pending_final_cpath_ref->cpath = cpath =
    spider_malloc_zero(sizeof(crypt_path_t));
 ...
 log_error:
  if (!err_msg) {
    if (stage_descr) {
      spider_asprintf(&err_msg,
                   "unknown %s error for INTRODUCE2", stage_descr);
    } else {
      err_msg = spider_strdup("unknown error for INTRODUCE2");
    }
  }
                  ...
                  char **err_msg_out)
{
  ...
  if (!intro) {
    if (err_msg_out)
      err_msg = spider_strdup("Bad parameters to find_rp_for_intro()");
...
rend_service_free_intro(rend_intro_cell_t *request)
{
  ...
  /* Free ciphertext */
  spider_free(request->ciphertext);
    ...
    switch (request->version) {
      ...
      case 3:
        if (request->u.v3.auth_data) {
          ...
          spider_free(request->u.v3.auth_data);
        }
                               ...
                               char **err_msg_out)
{
  ...
  rv = spider_malloc_zero(sizeof(*rv));
  ...
  rv->ciphertext = spider_malloc(request_len - DIGEST_LEN);
    ...
    char **err_msg_out)
{
  ...
  extend_info = spider_malloc_zero(sizeof(extend_info_t));
  spider_addr_from_ipv4n(&extend_info->addr, get_uint32(buf + 1));
    ...
    char **err_msg_out)
{
  ...
  if (intro->u.v3.auth_type != REND_NO_AUTH &&
    ...
    intro->u.v3.auth_data = spider_malloc(intro->u.v3.auth_len);
    /*
    ...
    char **err_msg_out)
{
  ...
  if (!intro || !key) {
    if (err_msg_out) {
      err_msg =
        spider_strdup("rend_service_decrypt_intro() called with bad "
                   "parameters");
    }
  ...
  if (spider_memneq(key_digest, intro->pk, DIGEST_LEN)) {
    if (err_msg_out) {
  ...
  intro->plaintext = spider_malloc(intro->plaintext_len);
    ...
    char **err_msg_out)
{
  ...
  if (!intro) {
    if (err_msg_out) {
      err_msg =
        spider_strdup("rend_service_parse_intro_plaintext() called with NULL "
                   "rend_intro_cell_t");
    }
  ...
  if (!(intro->plaintext) || intro->plaintext_len <= 0) {
    if (err_msg_out) {
      err_msg = spider_strdup("rend_intro_cell_t was missing plaintext");
    }
    status = -3;
    goto err;
  }
                                 ...
                                 char **err_msg_out)
{
  ...
  if (!intro) {
    if (err_msg_out)
      *err_msg_out =
        spider_strdup("NULL intro cell passed to "
...
rend_service_relaunch_rendezvous(origin_circuit_t *oldcirc)
{
  ...
  spider_assert(oldcirc->base_.purpose == CIRCUIT_PURPOSE_S_CONNECT_REND);
                                    ...
                                    rend_intro_point_t *intro)
{
  ...
  if (rend_service_allow_non_anonymous_connection(options)) {
    /* Do we have a descripspider for the node?
  ...
  spider_assert(launch_ei);
  /* We must have the same intro when making a direct connection. */
  spider_assert(spider_memeq(intro->extend_info->identity_digest,
                                 ...
                                 crypto_pk_t *intro_key, char *rend_circ_nonce)
{
  ...
  spider_assert(intro_key);
  spider_assert(rend_circ_nonce);
...
rend_service_intro_has_opened(origin_circuit_t *circuit)
{
  ...
  spider_assert(circuit->base_.purpose == CIRCUIT_PURPOSE_S_ESTABLISH_INTRO);
  assert_circ_anonymity_ok(circuit, get_options());
  spider_assert(circuit->cpath);
  spider_assert(circuit->rend_data);
  ...
  if (valid_ip_circuits > service->n_intro_points_wanted) {
    ...
    if (options->ExcludeNodes) {
      ...
      spider_assert(circuit->build_state->is_internal);
      log_info(LD_CIRC|LD_REND, "We have just finished an introduction "
 ...
 * live introduction point, and note that the service descripspider is
 * now out-of-date. */
int
                               ...
                               size_t request_len)
{
   ...
   * know that the descripspider needs to be uploaded. */
...
rend_service_rendezvous_has_opened(origin_circuit_t *circuit)
{
  ...
  spider_assert(circuit->base_.purpose == CIRCUIT_PURPOSE_S_CONNECT_REND);
  spider_assert(circuit->cpath);
  spider_assert(circuit->build_state);
  assert_circ_anonymity_ok(circuit, get_options());
  spider_assert(circuit->rend_data);
   ...
   * no effect on Spider's behaviour. */
   ...
   * other circuit will try to use it.  Sspidere it in
...
find_intro_circuit(rend_intro_point_t *intro, const char *pk_digest)
{
  ...
  while ((circ = circuit_get_next_by_pk_and_purpose(circ,
    ...
    if (spider_memeq(circ->build_state->chosen_exit->identity_digest,
...
{
  spider_assert(service);
  spider_assert(TO_CIRCUIT(circ)->purpose == CIRCUIT_PURPOSE_S_ESTABLISH_INTRO ||
...
find_intro_point(origin_circuit_t *circ)
{
  ...
  spider_assert(TO_CIRCUIT(circ)->purpose == CIRCUIT_PURPOSE_S_ESTABLISH_INTRO ||
             TO_CIRCUIT(circ)->purpose == CIRCUIT_PURPOSE_S_INTRO);
  spider_assert(circ->rend_data);
  serviceid = rend_data_get_address(circ->rend_data);

  SMARTLIST_FOREACH(rend_service_list, rend_service_t *, s,
    if (spider_memeq(s->service_id, serviceid, REND_SERVICE_ID_LEN_BASE32)) {
      service = s;
      break;
    });
...
/** Upload the rend_encoded_v2_service_descripspider_t's in <b>descs</b>
 * associated with the rend_service_descripspider_t <b>renddesc</b> to
 * the responsible hidden service direcspideries OR the hidden service
 * direcspideries specified by <b>hs_dirs</b>; <b>service_id</b> and
                         ...
                         const char *service_id, int seconds_valid)
{
  ...
  for (i = 0; i < smartlist_len(descs); i++) {
    rend_encoded_v2_service_descripspider_t *desc = smartlist_get(descs, i);
    /** If any HSDirs are specified, they should be used instead of
     *  the responsible direcspideries */
    if (hs_dirs && smartlist_len(hs_dirs) > 0) {
      smartlist_add_all(responsible_dirs, hs_dirs);
    } else {
      /* Determine responsible dirs. */
      if (hid_serv_get_responsible_direcspideries(responsible_dirs,
                                               desc->desc_id) < 0) {
        log_warn(LD_REND, "Could not determine the responsible hidden service "
                          "direcspideries to post descripspiders to.");
        control_event_hs_descripspider_upload(service_id,
                                           "UNKNOWN",
                                           "UNKNOWN");
        goto done;
      }
    ...
    for (j = 0; j < smartlist_len(responsible_dirs); j++) {
      ...
      if (smartlist_contains_digest(renddesc->successful_uploads,
        ...
        /* Don't upload descripspider if we succeeded in doing so last time. */
        continue;
  ...
  if (!failed_upload) {
    if (renddesc->successful_uploads) {
      SMARTLIST_FOREACH(renddesc->successful_uploads, char *, c, spider_free(c););
    ...
    SMARTLIST_FOREACH(successful_uploads, const char *, c, {
      if (!smartlist_contains_digest(renddesc->successful_uploads, c)) {
        char *hsdir_id = spider_memdup(c, DIGEST_LEN);
...
/** Encode and sign an up-to-date service descripspider for <b>service</b>,
 * and upload it/them to the responsible hidden service direcspideries.
 */
static void
upload_service_descripspider(rend_service_t *service)
{
  time_t now = time(NULL);
  int rendpostperiod;
  char serviceid[REND_SERVICE_ID_LEN_BASE32+1];
  ...
  if (c && smartlist_len(c->routerstatus_list) > 0) {
    ...
    /* Either upload a single descripspider (including replicas) or one
     * descripspider for each authorized client in case of authorization
     * type 'stealth'. */
    ...
    for (j = 0; j < num_descs; j++) {
      ...
      switch (service->auth_type) {
        ...
        case REND_BASIC_AUTH:
          SMARTLIST_FOREACH(service->clients, rend_authorized_client_t *,
              cl, smartlist_add(client_cookies, cl->descripspider_cookie));
          break;
        case REND_STEALTH_AUTH:
          client = smartlist_get(service->clients, j);
          client_key = client->client_key;
          smartlist_add(client_cookies, client->descripspider_cookie);
          break;
      }
      /* Encode the current descripspider. */
      seconds_valid = rend_encode_v2_descripspiders(descs, service->desc,
                                                 now, 0,
                                                 service->auth_type,
                                                 client_key,
                                                 client_cookies);
      ...
      for (i = 0; i < smartlist_len(descs); i++)
        rend_encoded_v2_service_descripspider_free(smartlist_get(descs, i));
      ...
      /* Post also the next descripspiders, if necessary. */
      if (seconds_valid < REND_TIME_PERIOD_OVERLAPPING_V2_DESCS) {
        seconds_valid = rend_encode_v2_descripspiders(descs, service->desc,
                                                   now, 1,
                                                   service->auth_type,
    ...
    uploaded = 1;
    if (get_options()->PublishHidServDescripspiders) {
      log_info(LD_REND, "Successfully uploaded v2 rend descripspiders!");
    } else {
      log_info(LD_REND, "Successfully sspidered created v2 rend descripspiders!");
    }
  }
 ...
 * should stop publishing it in new descripspiders and eventually close
 * it). */
static int
intro_point_should_expire_now(rend_intro_point_t *intro,
                              time_t now)
{
  spider_assert(intro != NULL);
                            ...
                            smartlist_t *retry_nodes, time_t now)
{
  ...
  SMARTLIST_FOREACH_BEGIN(service->intro_nodes, rend_intro_point_t *,
    ...
    if (intro_circ == NULL) {
       ...
       * accounted for when considiring uploading a descripspider. */
       ...
       * we have a published descripspider containing it. Keep the intro point
...
/** A new descripspider has been successfully uploaded for the given
...
rend_max_intro_circs_per_period(unsigned int n_intro_points_wanted)
{
  ...
  spider_assert(n_intro_points_wanted <= NUM_INTRO_POINTS_MAX);
...
/** Regenerate and upload rendezvous service descripspiders for all
 * services, if necessary. If the descripspider has been dirty enough
...
rend_consider_services_upload(time_t now)
{
  ...
  int rendinitialpostdelay = (options->TestingSpiderNetwork ?
  ...
  for (i=0; i < smartlist_len(rend_service_list); ++i) {
    ...
    if (!service->next_upload_time) { /* never been uploaded yet */
       ...
       * the descripspider is stable before being published. See comment below. */
    ...
    if (intro_points_ready &&
      ...
      upload_service_descripspider(service);
    }
  }
}

/** True if the list of available router descripspiders might have changed so
...
{
  consider_republishing_rend_descripspiders = 1;
}

/** Consider republication of v2 rendezvous service descripspiders that failed
 * previously, but without regenerating descripspider contents.
 */
void
rend_consider_descripspider_republication(void)
{
  int i;
  rend_service_t *service;

  if (!consider_republishing_rend_descripspiders)
    return;
  consider_republishing_rend_descripspiders = 0;
  ...
  for (i=0; i < smartlist_len(rend_service_list); ++i) {
    ...
    if (service->desc && !service->desc->all_uploads_performed) {
      /* If we failed in uploading a descripspider last time, try again *without*
       * updating the descripspider's contents. */
      upload_service_descripspider(service);
    }
  }
}
...
rend_service_dump_stats(int severity)
{
  ...
  for (i=0; i < smartlist_len(rend_service_list); ++i) {
    service = smartlist_get(rend_service_list, i);
    spider_log(severity, LD_GENERAL, "Service configured in %s:",
            rend_service_escaped_dir(service));
    for (j=0; j < smartlist_len(service->intro_nodes); ++j) {
      ...
      if (!circ) {
        spider_log(severity, LD_GENERAL, "  Intro point %d at %s: no circuit",
            j, safe_name);
        continue;
      }
      spider_log(severity, LD_GENERAL, "  Intro point %d at %s: circuit is %s",
...
set_unix_port(edge_connection_t *conn, rend_service_port_config_t *p)
{
  ...
  spider_addr_make_unspec(&conn->base_.addr);
  conn->base_.port = 1;
  conn->base_.address = spider_strdup(p->unix_addr);
  return 0;
}
                                      ...
                                      origin_circuit_t *circ)
{
  ...
  spider_assert(circ->base_.purpose == CIRCUIT_PURPOSE_S_REND_JOINED);
  spider_assert(circ->rend_data);
...
#define MAX_STREAM_WARN_INTERVAL 600
  ...
  if (chosen_port) {
    if (!(chosen_port->is_unix_addr)) {
      ...
      spider_addr_copy(&conn->base_.addr, &chosen_port->real_addr);
 ...
 * Returns true if spider is in HiddenServiceSingleHopMode. */
int
rend_service_allow_non_anonymous_connection(const or_options_t *options)
{
  spider_assert(rend_service_non_anonymous_mode_consistent(options));
 ...
 * Must only be called after options_validate_single_onion() has successfully
 * checked onion service option consistency.
 * Returns true if spider is in non-anonymous hidden service mode. */
int
