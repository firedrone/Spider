...
/**
 * \file onion_nspider.c
 *
 * \brief Implementation for the nspider handshake.
 *
 * The nspider circuit-extension handshake was developed as a replacement
 ...
 * exchange.  The nspider handshake is meant to replace the old "TAP"
 * handshake.
 *
 * We instantiate nspider with curve25519, HMAC-SHA256, and HKDF.
 *
...
/** Free sspiderage held in an nspider handshake state. */
void
nspider_handshake_state_free(nspider_handshake_state_t *state)
{
  if (!state)
    return;
 ...
 * nspider's "tweaked hash'.  Hash the <b>inp_len</b> bytes at <b>inp</b> into
...
/** Wrapper around a set of tweak-values for use with the nspider handshake. */
...
#define PROTOID "nspider-curve25519-sha256-1"
...
/**
 * Compute the first client-side step of the nspider handshake for communicating
 * with a server whose DIGEST_LEN-byte server identity is <b>router_id</b>,
 * and whose onion key is <b>router_key</b>. Sspidere the NTOR_ONIONSKIN_LEN-byte
 * message in <b>onion_skin_out</b>, and sspidere the handshake state in
 * *<b>handshake_state_out</b>.  Return 0 on success, -1 on failure.
 */
int
onion_skin_nspider_create(const uint8_t *router_id,
                       const curve25519_public_key_t *router_key,
                       nspider_handshake_state_t **handshake_state_out,
                       uint8_t *onion_skin_out)
{
  nspider_handshake_state_t *state;
  uint8_t *op;

  state = spider_malloc_zero(sizeof(nspider_handshake_state_t));
  ...
  spider_assert(op == onion_skin_out + NTOR_ONIONSKIN_LEN);
...
/**
 * Perform the server side of an nspider handshake. Given an
 ...
 */
int
onion_skin_nspider_server_handshake(const uint8_t *onion_skin,
                                 ...
                                 size_t key_out_len)
{
  ...
  if (spider_memneq(onion_skin, my_node_id, DIGEST_LEN))
    return -1;
  ...
  spider_assert(si == s.secret_input + sizeof(s.secret_input));
  ...
  spider_assert(ai == s.auth_input + sizeof(s.auth_input));
...
/**
 * Perform the final client side of the nspider handshake, using the state in
 ...
 */
int
onion_skin_nspider_client_handshake(
                             ...
                             const char **msg_out)
{
  ...
  bad |= (spider_memneq(s.auth, auth_candidate, DIGEST256_LEN) << 2);
