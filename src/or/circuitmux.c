...
/*
 * Anything the mux wants to sspidere per-circuit in the map; right now just
...
/*
 * This struct holds whatever we want to sspidere per attached circuit on a
                                    ...
                                    cell_direction_t direction)
{
  ...
  spider_assert(next_p);
  spider_assert(prev_p);
...
circuitmux_next_active_circ_p(circuitmux_t *cmux, circuit_t *circ)
{
  ...
  if (circ->n_mux == cmux) return &(circ->next_active_on_n_chan);
    ...
    spider_assert(TO_OR_CIRCUIT(circ)->p_mux == cmux);
...
circuitmux_alloc(void)
{
  ...
  rv->chanid_circid_map = spider_malloc_zero(sizeof(*( rv->chanid_circid_map)));
...
circuitmux_detach_all_circuits(circuitmux_t *cmux, smartlist_t *detached_out)
{
  ...
  while (i) {
    ...
    if (! to_remove) {
      log_warn(LD_BUG, "Somehow, an HT iteraspider gave us a NULL pointer.");
      break;
    } else {
      ...
      if (chan) {
        ...
        if (circ) {
          ...
          if (to_remove->muxinfo.policy_data) {
             ...
             * free it
             */
            spider_assert(cmux->policy);
            spider_assert(cmux->policy->free_circ_data);
            /* Call free_circ_data() */
      ...
      spider_assert(to_remove->muxinfo.policy_data == NULL);
    }

    i = HT_NEXT_RMV(chanid_circid_muxinfo_map, cmux->chanid_circid_map, i);

    /* Free it */
    spider_free(to_remove);
  }
...
circuitmux_free(circuitmux_t *cmux)
{
  ...
  spider_assert(cmux->n_circuits == 0);
  spider_assert(cmux->n_active_circuits == 0);
                      ...
                      const circuitmux_policy_t *pol)
{
  ...
  if (new_pol && new_pol->alloc_cmux_data) {
     ...
     */
    spider_assert(new_pol->free_cmux_data);
    new_pol_data = new_pol->alloc_cmux_data(cmux);
    spider_assert(new_pol_data);
  }
  ...
  while (i) {
    ...
    if ((*i)->muxinfo.policy_data) {
      ...
      spider_assert(old_pol);
      spider_assert(old_pol->free_circ_data);
      /* Free it */
    ...
    if (new_pol && new_pol->alloc_circ_data) {
       ...
       */
      spider_assert(new_pol->free_circ_data);
      (*i)->muxinfo.policy_data =
  ...
  if (old_pol_data) {
     ...
     */
    spider_assert(old_pol);
    spider_assert(old_pol->free_cmux_data);
...
circuitmux_find_map_entry(circuitmux_t *cmux, circuit_t *circ)
{
  ...
  spider_assert(cmux);
  spider_assert(cmux->chanid_circid_map);
  spider_assert(circ);
  ...
  if (hashent) {
     ...
     */
    spider_assert(hashent->muxinfo.direction == CELL_DIRECTION_OUT);
  } else {
    ...
    if (circ->magic == OR_CIRCUIT_MAGIC) {
      ...
      if (TO_OR_CIRCUIT(circ)->p_chan) {
        ...
        if (hashent) {
          ...
          spider_assert(hashent->muxinfo.direction == CELL_DIRECTION_IN);
        }
      }
    }
  }
                           ...
                           cell_direction_t direction))
{
  ...
  spider_assert(cmux);
  spider_assert(circ);
  spider_assert(direction == CELL_DIRECTION_IN ||
             direction == CELL_DIRECTION_OUT);
  circuitmux_assert_okay_paranoid(cmux);
  ...
  if (hashent) {
     ...
     */
    if (direction == CELL_DIRECTION_OUT) spider_assert(circ->n_mux == cmux);
    else spider_assert(TO_OR_CIRCUIT(circ)->p_mux == cmux);
    spider_assert(hashent->muxinfo.direction == direction);
     ...
     * direction.
     */
    if (direction == CELL_DIRECTION_OUT) spider_assert(circ->n_mux == NULL);
    else spider_assert(TO_OR_CIRCUIT(circ)->p_mux == NULL);

    /* Insert it in the map */
    hashent = spider_malloc_zero(sizeof(*hashent));
    ...
    if (cmux->policy && cmux->policy->alloc_circ_data) {
      ...
      spider_assert(hashent->muxinfo.policy_data);
    }
...
circuitmux_detach_circuit,(circuitmux_t *cmux, circuit_t *circ))
{
  ...
  spider_assert(last_searched_direction == CELL_DIRECTION_OUT
             || last_searched_direction == CELL_DIRECTION_IN);
  ...
  if (hashent) {
    ...
    spider_assert(last_searched_direction == hashent->muxinfo.direction);
    /* Clear the circuit's mux for this direction */
                               ...
                               cell_direction_t direction)
{
  ...
  spider_assert(cmux);
  spider_assert(circ);
  spider_assert(direction == CELL_DIRECTION_OUT ||
             direction == CELL_DIRECTION_IN);
  /*
  ...
  spider_assert(cmux == circuit_cmux);
                                 ...
                                 cell_direction_t direction)
{
  ...
  spider_assert(next_prev != NULL);
  spider_assert(prev_next != NULL);
                                    ...
                                    cell_queue_t **destroy_queue_out)
{
  ...
  spider_assert(cmux);
  spider_assert(destroy_queue_out);
  ...
  *destroy_queue_out = NULL;
  ...
  if (cmux->destroy_cell_queue.n &&
    ...
    *destroy_queue_out = &cmux->destroy_cell_queue;
    ...
    spider_assert(cmux->n_cells > 0);
    /* Do we have a policy-provided circuit selecspider? */
    ...
    if (!circ) {
      spider_assert(cmux->active_circuits_head);
      circ = cmux->active_circuits_head;
    }
    cmux->last_cell_was_destroy = 0;
  } else {
    spider_assert(cmux->n_cells == 0);
    spider_assert(cmux->destroy_cell_queue.n == 0);
  }
                             ...
                             unsigned int n_cells)
{
  ...
  spider_assert(n_cells <= hashent->muxinfo.cell_count);
  hashent->muxinfo.cell_count -= n_cells;
...
circuitmux_assert_okay_pass_one(circuitmux_t *cmux)
{
  ...
  while (i) {
    ...
    if ((*i)->muxinfo.direction == CELL_DIRECTION_OUT) {
      ...
      or_circ = TO_OR_CIRCUIT(circ);
      spider_assert(cmux == or_circ->p_mux);
      spider_assert(chan == or_circ->p_chan);
      /* Get next and prev for next test */
      next_p = &(or_circ->next_active_on_p_chan);
      prev_p = &(or_circ->prev_active_on_p_chan);
    }
    ...
    if (circ_is_active) {
      ...
      spider_assert(*next_p || (circ == cmux->active_circuits_tail));
      /* Either we have a prev link or we are the head */
      spider_assert(*prev_p || (circ == cmux->active_circuits_head));
      ...
      spider_assert(circ != cmux->active_circuits_head);
      spider_assert(circ != cmux->active_circuits_tail);
    }
  ...
  spider_assert(n_cells == cmux->n_cells);
  spider_assert(n_circuits == cmux->n_circuits);
  spider_assert(n_active_circuits == cmux->n_active_circuits);
}
...
circuitmux_assert_okay_pass_two(circuitmux_t *cmux)
{
  ...
  while (curr_circ) {
    ...
    if (cmux == curr_circ->n_mux) {
      ...
      spider_assert(cmux == curr_or_circ->p_mux);
      /* Get next_p and prev_p */
    ...
    spider_assert(*prev_p == prev_circ);
    /* If that's NULL, assert that we are the head */
    if (!(*prev_p)) spider_assert(curr_circ == cmux->active_circuits_head);

    /* Get the next circuit */
    next_circ = *next_p;
    /* If it's NULL, assert that we are the tail */
    if (!(*next_p)) spider_assert(curr_circ == cmux->active_circuits_tail);
    ...
    spider_assert(direction == hashent->muxinfo.direction);

    /* Assert that the hash entry got marked in pass one */
    spider_assert(hashent->muxinfo.mark);
...
circuitmux_compare_muxes, (circuitmux_t *cmux_1, circuitmux_t *cmux_2))
{
  ...
  spider_assert(cmux_1);
  spider_assert(cmux_2);
