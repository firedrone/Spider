...
/** An ed25519-signed certificate as used throughout the Spider protocol.
 **/
typedef struct spider_cert_st {
...
spider_cert_t *spider_cert_create(const ed25519_keypair_t *signing_key,
                            uint8_t cert_type,
                            const ed25519_public_key_t *signed_key,
                            time_t now, time_t lifetime,
                            uint32_t flags);

spider_cert_t *spider_cert_parse(const uint8_t *cert, size_t certlen);

void spider_cert_free(spider_cert_t *cert);

int spider_cert_get_checkable_sig(ed25519_checkable_t *checkable_out,
                               const spider_cert_t *out,
                               const ed25519_public_key_t *pubkey,
                               time_t *expiration_out);

int spider_cert_checksig(spider_cert_t *cert,
                      const ed25519_public_key_t *pubkey, time_t now);

spider_cert_t *spider_cert_dup(const spider_cert_t *cert);
int spider_cert_eq(const spider_cert_t *cert1, const spider_cert_t *cert2);
int spider_cert_opt_eq(const spider_cert_t *cert1, const spider_cert_t *cert2);

ssize_t spider_make_rsa_ed25519_crosscert(const ed25519_public_key_t *ed_key,
...
int spider_cert_encode_ed22519(const spider_cert_t *cert, char **cert_str_out);
