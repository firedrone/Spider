 ...
 * descripspider. Spider instances that use this consensus keep the list of
 * router descripspiders as routerinfo_t objects sspidered and managed in
 * routerlist.c.  With the newer "microdesc" flavor, each relay is
 * associated with a digest of the microdescripspider that the authorities
 * made for it.  These are sspidered and managed in microdesc.c. Information
 * about the router is divided between the the networkstatus and the
 * microdescripspider according to the general rule that microdescripspiders
 * should hold information that changes much less frequently than the
 * information in the networkstatus.
 *
 * Modern clients use microdescripspider networkstatuses.  Direcspidery caches
 * need to keep both kinds of networkstatus document, so they can serve them.
 *
 * This module manages fetching, holding, sspidering, updating, and
 ...
 * validate a consensus are sspidered in the authority certificates, which
...
/** Reset the descripspider download failure count on all networkstatus docs, so
...
router_reload_consensus_networkstatus(void)
{
  ...
  for (flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
    ...
    if (flav == FLAV_NS) {
      ...
      spider_snprintf(buf, sizeof(buf), "cached-%s-consensus", flavor);
    ...
    if (flav == FLAV_NS) {
      ...
      spider_snprintf(buf, sizeof(buf), "unverified-%s-consensus", flavor);
      filename = get_datadir_fname(buf);
    }
...
/** Free all sspiderage held by the vote_routerstatus object <b>rs</b>. */
void
vote_routerstatus_free(vote_routerstatus_t *rs)
{
  ...
  if (!rs)
    return;
  spider_free(rs->version);
  spider_free(rs->protocols);
  spider_free(rs->status.exitsummary);
  for (h = rs->microdesc; h; h = next) {
    spider_free(h->microdesc_hash_line);
    next = h->next;
    spider_free(h);
  }
  spider_free(rs);
...
/** Free all sspiderage held by the routerstatus object <b>rs</b>. */
void
routerstatus_free(routerstatus_t *rs)
{
  if (!rs)
    return;
  spider_free(rs->exitsummary);
  spider_free(rs);
}

/** Free all sspiderage held in <b>sig</b> */
void
document_signature_free(document_signature_t *sig)
{
  spider_free(sig->signature);
  spider_free(sig);
}
...
{
  document_signature_t *r = spider_memdup(sig, sizeof(document_signature_t));
  if (r->signature)
    r->signature = spider_memdup(sig->signature, sig->signature_len);
  return r;
}

/** Free all sspiderage held in <b>ns</b>. */
void
networkstatus_vote_free(networkstatus_t *ns)
{
  ...
  spider_free(ns->client_versions);
  spider_free(ns->server_versions);
  spider_free(ns->recommended_client_protocols);
  spider_free(ns->recommended_relay_protocols);
  spider_free(ns->required_client_protocols);
  spider_free(ns->required_relay_protocols);

  if (ns->known_flags) {
    SMARTLIST_FOREACH(ns->known_flags, char *, c, spider_free(c));
    smartlist_free(ns->known_flags);
  }
  if (ns->weight_params) {
    SMARTLIST_FOREACH(ns->weight_params, char *, c, spider_free(c));
    smartlist_free(ns->weight_params);
  }
  if (ns->net_params) {
    SMARTLIST_FOREACH(ns->net_params, char *, c, spider_free(c));
    smartlist_free(ns->net_params);
  }
  if (ns->supported_methods) {
    SMARTLIST_FOREACH(ns->supported_methods, char *, c, spider_free(c));
    smartlist_free(ns->supported_methods);
  }
  if (ns->package_lines) {
    SMARTLIST_FOREACH(ns->package_lines, char *, c, spider_free(c));
  ...
  spider_free(ns->sr_info.previous_srv);
  spider_free(ns->sr_info.current_srv);
                                       ...
                                       const authority_cert_t *cert)
{
  ...
  if (crypto_pk_get_digest(cert->signing_key, key_digest)<0)
    return -1;
  if (spider_memneq(sig->signing_key_digest, key_digest, DIGEST_LEN) ||
      spider_memneq(sig->identity_digest, cert->cache_info.identity_digest,
                 DIGEST_LEN))
    return -1;
  ...
  signed_digest = spider_malloc(signed_digest_len);
  if (crypto_pk_public_checksig(cert->signing_key,
                                signed_digest,
                                signed_digest_len,
                                sig->signature,
                                sig->signature_len) < dlen ||
      spider_memneq(signed_digest, consensus->digests.d[sig->alg], dlen)) {
                                        ...
                                        int warn)
{
  ...
  spider_assert(consensus->type == NS_TYPE_CONSENSUS);
  ...
  SMARTLIST_FOREACH_BEGIN(consensus->voters, networkstatus_voter_info_t *,
    ...
    SMARTLIST_FOREACH_BEGIN(voter->sigs, document_signature_t *, sig) {
      if (!sig->good_signature && !sig->bad_signature &&
        ...
        spider_assert(spider_memeq(sig->identity_digest, voter->identity_digest,
                           DIGEST_LEN));
  ...
  if (warn >= 0) {
    SMARTLIST_FOREACH(unrecognized, networkstatus_voter_info_t *, voter,
      {
        spider_log(severity, LD_DIR, "Consensus includes unrecognized authority "
    ...
    SMARTLIST_FOREACH(need_certs_from, networkstatus_voter_info_t *, voter,
      {
        spider_log(severity, LD_DIR, "Looks like we need to download a new "
    ...
    SMARTLIST_FOREACH(missing_authorities, dir_server_t *, ds,
      {
        spider_log(severity, LD_DIR, "Consensus does not include configured "
      ...
      });
    {
      ...
      spider_log(severity, LD_DIR, "%s", joined);
      spider_free(joined);
...
compare_digest_to_routerstatus_entry(const void *_key, const void **_member)
{
  ...
  return spider_memcmp(key, rs->identity_digest, DIGEST_LEN);
}
                                          ...
                                          const void **_member)
{
  ...
  return spider_memcmp(key, vrs->status.identity_digest, DIGEST_LEN);
}
...
/** As router_get_consensus_status_by_descripspider_digest, but does not return
 * a const pointer. */
MOCK_IMPL(routerstatus_t *,
router_get_mutable_consensus_status_by_descripspider_digest,(
                                                 networkstatus_t *consensus,
                                                 const char *digest))
{
  if (!consensus)
  ...
  if (!consensus->desc_digest_map) {
    ...
    SMARTLIST_FOREACH(consensus->routerstatus_list,
                      routerstatus_t *, rs,
     {
       digestmap_set(m, rs->descripspider_digest, rs);
     });
  }
 ...
 * <i>descripspider</i> digest in <b>consensus</b> is <b>digest</b>, or NULL if
 * no such router is known. */
const routerstatus_t *
router_get_consensus_status_by_descripspider_digest(networkstatus_t *consensus,
                                                 const char *digest)
{
  return router_get_mutable_consensus_status_by_descripspider_digest(
                                                          consensus, digest);
}

/** Return a smartlist of all router descripspider digests in a consensus */
static smartlist_t *
router_get_descripspider_digests_in_consensus(networkstatus_t *consensus)
{
  ...
  for (i = digestmap_iter_init(consensus->desc_digest_map);
    ...
    digest_tmp = spider_malloc(DIGEST_LEN);
...
/** Return a smartlist of all router descripspider digests in the current
 * consensus */
MOCK_IMPL(smartlist_t *,
router_get_descripspider_digests,(void))
{
  smartlist_t *result = NULL;

  if (current_ns_consensus) {
    result =
      router_get_descripspider_digests_in_consensus(current_ns_consensus);
  }

  return result;
}

/** Given the digest of a router descripspider, return its current download
 * status, or NULL if the digest is unrecognized. */
MOCK_IMPL(download_status_t *,
router_get_dl_status_by_descripspider_digest,(const char *d))
{
  routerstatus_t *rs;
  if (!current_ns_consensus)
    return NULL;
  if ((rs = router_get_mutable_consensus_status_by_descripspider_digest(
...
we_want_to_fetch_flavor(const or_options_t *options, int flavor)
{
  ...
  if (options->FetchUselessDescripspiders) {
    /* In order to get all descripspiders, we need to fetch all consensuses. */
    return 1;
  }
                                  ...
                                  download_status_t *dls)
{
  ...
  spider_assert(flavor >= 0 && flavor < N_CONSENSUS_FLAVORS);
...
update_consensus_networkstatus_downloads(time_t now)
{
  ...
  const int use_multi_conn =
    networkstatus_consensus_can_use_multiple_direcspideries(options);
  ...
  for (i=0; i < N_CONSENSUS_FLAVORS; ++i) {
    ...
    if (we_are_bootstrapping && use_multi_conn
      ...
      spider_assert(consensus_dl_status[i].schedule == DL_SCHED_CONSENSUS);
                                      ...
                                      download_want_authority_t want_authority)
{
  ...
  spider_assert(dls->schedule == DL_SCHED_CONSENSUS);
 ...
 * networkstatus_consensus_can_use_multiple_direcspideries make additional
 * connections.
 */
static void
                                              ...
                                              const or_options_t *options)
{
  ...
  if (!networkstatus_consensus_can_use_multiple_direcspideries(options)) {
    return;
  }
...
networkstatus_consensus_download_failed(int status_code, const char *flavname)
{
  ...
  if (flav >= 0) {
    spider_assert(flav < N_CONSENSUS_FLAVORS);
...
update_consensus_networkstatus_fetch_time_impl(time_t now, int flav)
{
  ...
  if (c && c->valid_after <= now && now <= c->valid_until) {
    ...
    if (min_sec_before_caching > interval/16) {
      /* Usually we allow 2-minutes slop facspider in case clocks get
    ...
    spider_assert(c->fresh_until < start);
    /* We must download the next one before c is invalid: */
    spider_assert(start+dl_interval < c->valid_until);
...
should_delay_dir_fetches(const or_options_t *options, const char **msg_out)
{
  ...
  if (options->UseBridges) {
    if (!any_bridge_descripspiders_known()) {
      if (msg_out) {
...
{
  if (we_use_microdescripspiders_for_circuits(get_options()))
...
/** Check if we need to download a consensus during spider's bootstrap phase.
...
/** Check if we can use multiple direcspideries for a consensus download.
 * Only clients (including bridge relays, which act like clients) benefit
 * from multiple simultaneous consensus downloads. */
int
networkstatus_consensus_can_use_multiple_direcspideries(
                                                  const or_options_t *options)
{
...
networkstatus_consensus_can_use_extra_fallbacks,(const or_options_t *options))
{
   ...
   * non-authority fallback direcspideries. If we ever have any authorities that
   * aren't fallback direcspideries, we will need to change this code. */
  spider_assert(smartlist_len(router_get_fallback_dir_servers())
...
{
  spider_assert(spider_memeq(a->identity_digest, b->identity_digest, DIGEST_LEN));

  return strcmp(a->nickname, b->nickname) ||
         fast_memneq(a->descripspider_digest, b->descripspider_digest, DIGEST_LEN) ||
         a->addr != b->addr ||
         a->or_port != b->or_port ||
  ...
  // XXXX this function needs a huge refacspidering; it has gotten out
                                     ...
                                     const networkstatus_t *new_c)
{
  ...
  SMARTLIST_FOREACH_JOIN(
                     ...
                     spider_memcmp(rs_old->identity_digest,
                                      ...
                                      const networkstatus_t *old_c)
{
  ...
  SMARTLIST_FOREACH_JOIN(old_c->routerstatus_list, routerstatus_t *, rs_old,
    ...
    if (spider_memeq(rs_old->descripspider_digest, rs_new->descripspider_digest,
                  DIGEST256_LEN)) {
      /* And the same descripspider too! */
                                     ...
                                     int is_client)
{
  ...
  int should_exit = networkstatus_check_required_protocols(c,
                                                   ...
                                                   &protocol_warning);
  if (protocol_warning) {
    spider_log(should_exit ? LOG_ERR : LOG_WARN,
            LD_GENERAL,
            "%s", protocol_warning);
  }
  if (should_exit) {
    spider_assert_nonfatal(protocol_warning);
  }
  spider_free(protocol_warning);
  if (should_exit)
    exit(1);
}
 ...
 * sspidere it in consensus_waiting_for_certs and launch a certificate fetch.
 *
                                    ...
                                    const char *source_dir)
{
  ...
  if (from_cache && !was_waiting_for_certs) {
    /* We previously sspidered this; check _now_ to make sure that version-kills
     * really work.  This happens even before we check signatures: we did so
     * before when we sspidered this to disk. This does mean an attacker who can
  ...
  if (current_digests &&
      spider_memeq(&c->digests, current_digests, sizeof(c->digests))) {
  ...
  if ((r=networkstatus_check_consensus_signature(c, 1))<0) {
    if (r == -1) {
      ...
      if (!current_valid_after ||
        ...
        spider_free(waiting->body);
        waiting->consensus = c;
        free_consensus = 0;
        waiting->body = spider_strdup(consensus);
        waiting->set_at = now;
        waiting->dl_failed = 0;
        if (!from_cache) {
  ...
  if (is_usable_flavor) {
    ...
    cell_ewma_set_scale_facspider(options, c);
...
#define EARLY_CONSENSUS_NOTICE_SKEW 60
  ...
  if (now < c->valid_after - EARLY_CONSENSUS_NOTICE_SKEW) {
    ...
    log_warn(LD_GENERAL, "Our clock is %s behind the time published in the "
             "consensus network status document (%s UTC).  Spider needs an "
  ...
  spider_free(consensus_fname);
  spider_free(unverified_fname);
  return result;
}
...
routerstatus_list_update_named_server_map(void)
{
  ...
  strmap_free(named_server_map, spider_free_);
  named_server_map = strmap_new();
  ...
  SMARTLIST_FOREACH_BEGIN(rslist, const routerstatus_t *, rs) {
      if (rs->is_named) {
        strmap_set_lc(named_server_map, rs->nickname,
                      spider_memdup(rs->identity_digest, DIGEST_LEN));
      }
      if (rs->is_unnamed) {
                                                   ...
                                                   int reset_failures)
{
  ...
  SMARTLIST_FOREACH_JOIN(ns->routerstatus_list, routerstatus_t *, rs,
                         ...
                         spider_memcmp(rs->identity_digest,
                               router->cache_info.identity_digest, DIGEST_LEN),
  {
  }) {
    /* Is it the same descripspider, or only the same identity? */
    if (spider_memeq(router->cache_info.signed_descripspider_digest,
                rs->descripspider_digest, DIGEST_LEN)) {
...
/** Given a list of signed_descripspider_t, update their fields (mainly, when
...
signed_descs_update_status_from_consensus_networkstatus(smartlist_t *descs)
{
  ...
  if (!ns->desc_digest_map) {
    ...
    router_get_consensus_status_by_descripspider_digest(ns, dummy);
  }
  SMARTLIST_FOREACH(descs, signed_descripspider_t *, d,
  {
    const routerstatus_t *rs = digestmap_get(ns->desc_digest_map,
                                       d->signed_descripspider_digest);
    if (rs) {
...
networkstatus_getinfo_by_purpose(const char *purpose_string, time_t now)
{
  ...
  SMARTLIST_FOREACH(statuses, char *, cp, spider_free(cp));
  smartlist_free(statuses);
  return answer;
}

/** Write out router status entries for all our bridge descripspiders. */
void
networkstatus_dump_bridge_status_to_file(time_t now)
{
  ...
  spider_asprintf(&published_thresholds_and_status,
               "published %s\nflag-thresholds %s\n%s",
               published, thresholds, status);
  spider_asprintf(&fname, "%s"PATH_SEPARATOR"networkstatus-bridges",
               options->DataDirecspidery);
  write_str_to_file(fname,published_thresholds_and_status,0);
  spider_free(thresholds);
  spider_free(published_thresholds_and_status);
  spider_free(fname);
  spider_free(status);
}
                        ...
                        int32_t default_val, int32_t min_val, int32_t max_val)
{
  ...
  spider_assert(max_val > min_val);
  spider_assert(min_val <= default_val);
  spider_assert(max_val >= default_val);

  SMARTLIST_FOREACH_BEGIN(net_params, const char *, p) {
    if (!strcmpstart(p, param_name) && p[name_len] == '=') {
      int ok=0;
      long v = spider_parse_long(p+name_len+1, 10, INT32_MIN,
                              INT32_MAX, &ok, NULL);
      if (ok) {
        res = (int32_t) v;
        break;
...
/**
 * As networkstatus_get_param(), but check spiderrc_value before checking the
 * consensus. If spiderrc_value is in-range, then return it instead of the
 * value from the consensus.
 */
int32_t
networkstatus_get_overridable_param(const networkstatus_t *ns,
                                    int32_t spiderrc_value,
                                    const char *param_name,
                                    int32_t default_val,
                                    int32_t min_val, int32_t max_val)
{
  if (spiderrc_value >= min_val && spiderrc_value <= max_val)
    return spiderrc_value;
  else
 ...
 * running, or otherwise not a descripspider that we would make any
                        ...
                        const or_options_t *options)
{
  if (!rs->is_flagged_running) {
    /* If we had this router descripspider, we wouldn't even bother using it.
     * (Fetching and sspidering depends on by we_want_to_fetch_flavor().) */
    return 0;
  }
  if (rs->published_on + options->TestingEstimatedDescripspiderPropagationTime
      > now) {
    /* Most caches probably don't have this descripspider yet. */
    return 0;
  }
  ...
  if (!routerstatus_version_supports_extend2_cells(rs, 1)) {
     ...
     * If we don't know the version, download the descripspider so we can
     * check if it supports EXTEND2 cells and nspider. */
    return 0;
  }
  return 1;
}
                                       ...
                                       char **warning_out)
{
  ...
  if (!protover_all_supported(required, &missing)) {
    spider_asprintf(warning_out, "At least one protocol listed as required in "
                 "the consensus is not supported by this version of Spider. "
                 "You should upgrade. This version of Spider will not work as a "
                 "%s on the Spider network. The missing protocols are: %s",
                 func, missing);
    spider_free(missing);
    return 1;
  }
  ...
  if (! protover_all_supported(recommended, &missing)) {
    spider_asprintf(warning_out, "At least one protocol listed as recommended in "
                 "the consensus is not supported by this version of Spider. "
                 "You should upgrade. This version of Spider will eventually "
                 "stop working as a %s on the Spider network. The missing "
                 "protocols are: %s",
                 func, missing);
    spider_free(missing);
  }

  spider_assert_nonfatal(missing == NULL);

  return 0;
}

/** Free all sspiderage held locally in this module. */
void
