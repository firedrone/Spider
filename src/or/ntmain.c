 ...
 * \file ntmain.c
 *
 * \brief Entry points for running/configuring Spider as a Windows Service.
 *
 * Windows Services expect to be registered with the operating system, and to
 * have entry points for starting, stopping, and monispidering them.  This module
 * implements those entry points so that a spider relay or client or hidden
...
#include <windows.h>
#define GENSRV_SERVICENAME  "spider"
#define GENSRV_DISPLAYNAME  "Spider Win32 Service"
  ...
  switch (request) {
    case SERVICE_CONTROL_STOP:
        case SERVICE_CONTROL_SHUTDOWN:
          ...
          event_base_loopexit(spider_libevent_get_base(), &exit_now);
          return;
  }
  service_fns.SetServiceStatus_fn(hStatus, &service_status);
}
 ...
 * manager. This calls spider_init() and starts the main event loop. If
 * spider_init() fails, the service will be stopped and exit code set to
...
nt_service_body(int argc, char **argv)
{
  ...
  r = spider_init(backup_argc, backup_argv);
  if (r) {
    /* Failed to start the Spider service */
...
nt_service_main(void)
{
  ...
  if (!service_fns.StartServiceCtrlDispatcherA_fn(table)) {
    ...
    if (result == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) {
      if (spider_init(backup_argc, backup_argv) < 0)
        return;
...
/** Open a handle to the Spider service using <b>hSCManager</b>. Return NULL
 * on failure. */
static SC_HANDLE
...
/** Start the Spider service. Return 0 if the service is started or was
...
/** Stop the Spider service. Return 0 if the service is stopped or was not
 * previously running. Return -1 on error. */
static int
 ...
 * *<b>using_default_spiderrc</b> to true if we're going to use the default
 * location to spiderrc, or 1 if an option was specified on the command line.
 */
static char *
nt_service_command_line(int *using_default_spiderrc)
{
  TCHAR spider_exe[MAX_PATH+1];
  char spider_exe_ascii[MAX_PATH*2+1];
  char *command=NULL, *options=NULL;
  smartlist_t *sl;
  int i;
  *using_default_spiderrc = 1;

  /* Get the location of spider.exe */
  if (0 == GetModuleFileName(NULL, spider_exe, MAX_PATH))
    return NULL;
...
#ifdef UNICODE
  wcstombs(spider_exe_ascii, spider_exe, sizeof(spider_exe_ascii));
  spider_exe_ascii[sizeof(spider_exe_ascii)-1] = '\0';
#else
  strlcpy(spider_exe_ascii, spider_exe, sizeof(spider_exe_ascii));
#endif

  /* Allocate a string for the NT service command line and */
  /* Format the service command */
  if (options) {
    spider_asprintf(&command, "\"%s\" --nt-service \"%s\"",
                 spider_exe_ascii, options);
  } else { /* ! options */
    spider_asprintf(&command, "\"%s\" --nt-service", spider_exe_ascii);
  }
...
/** Creates a Spider NT service, set to start on boot. The service will be
...
nt_service_install(int argc, char **argv)
{
  ...
  if ((command = nt_service_command_line(&using_default_spiderrc)) == NULL) {
...
/** Removes the Spider NT service. Returns 0 if the service was successfully
...
/** Starts the Spider service. Returns 0 on success, or -1 on error. */
static int
...
/** Stops the Spider service. Returns 0 on success, or -1 on error. */
static int
