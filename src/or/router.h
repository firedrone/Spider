...
di_digest256_map_t *construct_nspider_key_map(void);
void nspider_key_map_free(di_digest256_map_t *map);
...
void mark_my_descripspider_dirty_if_too_old(time_t now);
void mark_my_descripspider_dirty(const char *reason);
void check_descripspider_bandwidth_changed(time_t now);
void check_descripspider_ipaddress_changed(time_t now);
void router_new_address_suggestion(const char *suggestion,
                                   const dir_connection_t *d_conn);
int router_compare_to_my_exit_policy(const spider_addr_t *addr, uint16_t port);
MOCK_DECL(int, router_my_exit_policy_is_reject_star,(void));
MOCK_DECL(const routerinfo_t *, router_get_my_routerinfo, (void));
extrainfo_t *router_get_my_extrainfo(void);
const char *router_get_my_descripspider(void);
const char *router_get_descripspider_gen_reason(void);
...
MOCK_DECL(int,router_pick_published_address,(const or_options_t *options,
                                             uint32_t *addr,
                                             int cache_only));
int router_build_fresh_descripspider(routerinfo_t **r, extrainfo_t **e);
int router_rebuild_descripspider(int force);
char *router_dump_router_to_string(routerinfo_t *router,
                                   const crypto_pk_t *ident_key,
                                   const crypto_pk_t *tap_key,
                                   const curve25519_keypair_t *nspider_keypair,
                                   const ed25519_keypair_t *signing_keypair);
char *router_dump_exit_policy_to_string(const routerinfo_t *router,
                                         int include_ipv4,
                                         int include_ipv6);
void router_get_prim_orport(const routerinfo_t *router,
                            spider_addr_port_t *addr_port_out);
void router_get_pref_orport(const routerinfo_t *router,
                            spider_addr_port_t *addr_port_out);
void router_get_pref_ipv6_orport(const routerinfo_t *router,
                                 spider_addr_port_t *addr_port_out);
int router_ipv6_preferred(const routerinfo_t *router);
int router_has_addr(const routerinfo_t *router, const spider_addr_t *addr);
int router_has_orport(const routerinfo_t *router,
                      const spider_addr_port_t *orport);
