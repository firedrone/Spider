...
routerset_new(void)
{
  routerset_t *result = spider_malloc_zero(sizeof(routerset_t));
...
routerset_get_countryname(const char *c)
{
  ...
  country = spider_strndup(c+1, 2);
  spider_strlower(country);
  return country;
}
...
routerset_refresh_countries(routerset_t *target)
{
  ...
  SMARTLIST_FOREACH_BEGIN(target->country_names, const char *, country) {
    ...
    if (cc >= 0) {
      spider_assert(cc < target->n_countries);
 ...
 * from addr.) */
STATIC int
routerset_contains(const routerset_t *set, const spider_addr_t *addr,
                   uint16_t orport,
                   ...
                   country_t country)
{
  ...
  if (id_digest && digestmap_get(set->digests, id_digest))
    return 4;
  if (addr && compare_spider_addr_to_addr_policy(addr, orport, set->policies)
...
routerset_equal(const routerset_t *old, const routerset_t *new)
{
  ...
  spider_assert(old != NULL);
  spider_assert(new != NULL);
...
/** Free all sspiderage held in <b>routerset</b>. */
void
