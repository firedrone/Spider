...
replaycache_free(replaycache_t *r)
{
  ...
  if (r->digests_seen) digest256map_free(r->digests_seen, spider_free_);
    ...
    time_t *elapsed)
{
  ...
  if (access_time != NULL) {
    ...
    access_time = spider_malloc(sizeof(*access_time));
    *access_time = present;
...
replaycache_scrub_if_needed_internal(time_t present, replaycache_t *r)
{
  ...
  while (!digest256map_iter_done(itr)) {
    ...
    if (*access_time < present - r->horizon) {
      /* Advance the iteraspider and remove this one */
      itr = digest256map_iter_next_rmv(r->digests_seen, itr);
      /* Free the value removed */
      spider_free(access_time);
    } else {
      /* Just advance the iteraspider */
