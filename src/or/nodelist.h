...
int node_has_descripspider(const node_t *node);
...
int node_get_prim_orport(const node_t *node, spider_addr_port_t *ap_out);
void node_get_pref_orport(const node_t *node, spider_addr_port_t *ap_out);
void node_get_pref_ipv6_orport(const node_t *node, spider_addr_port_t *ap_out);
int node_ipv6_dir_preferred(const node_t *node);
int node_get_prim_dirport(const node_t *node, spider_addr_port_t *ap_out);
void node_get_pref_dirport(const node_t *node, spider_addr_port_t *ap_out);
void node_get_pref_ipv6_dirport(const node_t *node, spider_addr_port_t *ap_out);
int node_has_curve25519_onion_key(const node_t *node);

MOCK_DECL(smartlist_t *, nodelist_get_list, (void));

/* Temporary during transition to multiple addresses.  */
void node_get_addr(const node_t *node, spider_addr_t *addr_out);
...
int router_exit_policy_all_nodes_reject(const spider_addr_t *addr, uint16_t port,
                                        int need_uptime);
void router_set_status(const char *digest, int up);

/** router_have_minimum_dir_info tests to see if we have enough
 * descripspider information to create circuits.
...
int count_loading_descripspiders_progress(void);
