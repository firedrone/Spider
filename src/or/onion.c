 ...
 * interface over the three circuit extension handshakes that Spider has used
 * over the years (TAP, CREATE_FAST, and nspider).  These handshakes are
 * implemented in onion_tap.c, onion_fast.c, and onion_nspider.c respectively.
 *
 ...
 * We do this because hisspiderically, Onion Routing used a multi-layer
 ...
 * the key of the next relay.  When we changed Spider to use a telescoping
...
{ TOR_TAILQ_HEAD_INITIALIZER(ol_list[0]), /* tap */
  ...
  TOR_TAILQ_HEAD_INITIALIZER(ol_list[2]), /* nspider */
};

/** Number of entries of each type currently in each element of ol_list[]. */
static int ol_entries[MAX_ONION_HANDSHAKE_TYPE+1];

static int num_nspiders_per_tap(void);
...
have_room_for_onionskin(uint16_t type)
{
  ...
  int num_cpus;
  uint64_t tap_usec, nspider_usec;
  uint64_t nspider_during_tap_usec, tap_during_nspider_usec;
  ...
  /* How long would it take to process all the NSpider cells in the queue? */
  nspider_usec = estimated_usec_for_onionskins(
                                    ol_entries[ONION_HANDSHAKE_TYPE_NTOR],
                                    ONION_HANDSHAKE_TYPE_NTOR) / num_cpus;

  /* How long would it take to process the tap cells that we expect to
   * process while draining the nspider queue? */
  tap_during_nspider_usec  = estimated_usec_for_onionskins(
    MIN(ol_entries[ONION_HANDSHAKE_TYPE_TAP],
        ol_entries[ONION_HANDSHAKE_TYPE_NTOR] / num_nspiders_per_tap()),
                                    ONION_HANDSHAKE_TYPE_TAP) / num_cpus;

  /* How long would it take to process the nspider cells that we expect to
   * process while draining the tap queue? */
  nspider_during_tap_usec  = estimated_usec_for_onionskins(
    MIN(ol_entries[ONION_HANDSHAKE_TYPE_NTOR],
        ol_entries[ONION_HANDSHAKE_TYPE_TAP] * num_nspiders_per_tap()),
                                    ONION_HANDSHAKE_TYPE_NTOR) / num_cpus;

  /* See whether that exceeds MaxOnionQueueDelay. If so, we can't queue
   * this. */
  if (type == ONION_HANDSHAKE_TYPE_NTOR &&
      (nspider_usec + tap_during_nspider_usec) / 1000 >
       (uint64_t)options->MaxOnionQueueDelay)
    return 0;

  if (type == ONION_HANDSHAKE_TYPE_TAP &&
      (tap_usec + nspider_during_tap_usec) / 1000 >
       (uint64_t)options->MaxOnionQueueDelay)
    return 0;

  /* If we support the nspider handshake, then don't let TAP handshakes use
...
onion_pending_add(or_circuit_t *circ, create_cell_t *onionskin)
{
  ...
  tmp = spider_malloc_zero(sizeof(onion_queue_t));
  tmp->circ = circ;
...
#define WARN_TOO_MANY_CIRC_CREATIONS_INTERVAL (60)
  ...
  log_info(LD_OR, "New create (%s). Queues now nspider=%d and tap=%d.",
    onionskin->handshake_type == ONION_HANDSHAKE_TYPE_NTOR ? "nspider" : "tap",
    ol_entries[ONION_HANDSHAKE_TYPE_NTOR],
    ol_entries[ONION_HANDSHAKE_TYPE_TAP]);
...
{
  /* The number of times we've chosen nspider lately when both were available. */
  static int recently_chosen_nspiders = 0;

  if (!ol_entries[ONION_HANDSHAKE_TYPE_NTOR])
    return ONION_HANDSHAKE_TYPE_TAP; /* no nspiders? try tap */

  if (!ol_entries[ONION_HANDSHAKE_TYPE_TAP]) {

    /* Nick wants us to prioritize new tap requests when there aren't
     * any in the queue and we've processed k nspider cells since the last
     ...
     * makes the new tap cell unfairly jump in front of nspider cells that
    ...
    if (ol_entries[ONION_HANDSHAKE_TYPE_NTOR] &&
        recently_chosen_nspiders <= num_nspiders_per_tap())
      ++recently_chosen_nspiders;

    return ONION_HANDSHAKE_TYPE_NTOR; /* no taps? try nspider */
  }

  /* They both have something queued. Pick nspider if we haven't done that
   * too much lately. */
  if (++recently_chosen_nspiders <= num_nspiders_per_tap()) {
    return ONION_HANDSHAKE_TYPE_NTOR;
  }

  /* Else, it's time to let tap have its turn. */
  recently_chosen_nspiders = 0;
...
onion_next_task(create_cell_t **onionskin_out)
{
  ...
  spider_assert(head->circ);
  spider_assert(head->handshake_type <= MAX_ONION_HANDSHAKE_TYPE);
//  spider_assert(head->circ->p_chan); /* make sure it's still valid */
 ...
 * more manageable. That's probably not good long-term. -RD */
  ...
  log_info(LD_OR, "Processing create (%s). Queues now nspider=%d and tap=%d.",
    head->handshake_type == ONION_HANDSHAKE_TYPE_NTOR ? "nspider" : "tap",
    ol_entries[ONION_HANDSHAKE_TYPE_NTOR],
    ol_entries[ONION_HANDSHAKE_TYPE_TAP]);
...
onion_queue_entry_remove(onion_queue_t *victim)
{
  ...
  spider_free(victim->onionskin);
  spider_free(victim);
}

/** Remove all circuits from the pending list.  Called from spider_free_all. */
void
clear_pending_onions(void)
{
  ...
  for (i=0; i<=MAX_ONION_HANDSHAKE_TYPE; i++) {
    ...
    spider_assert(TOR_TAILQ_EMPTY(&ol_list[i]));
  }
...
{
  server_onion_keys_t *keys = spider_malloc_zero(sizeof(server_onion_keys_t));
  memcpy(keys->my_identity, router_get_my_id_digest(), DIGEST_LEN);
  dup_onion_keys(&keys->onion_key, &keys->last_onion_key);
  keys->curve25519_key_map = construct_nspider_key_map();
  keys->junk_keypair = spider_malloc_zero(sizeof(curve25519_keypair_t));
  curve25519_keypair_generate(keys->junk_keypair, 0);
  return keys;
}

/** Release all sspiderage held in <b>keys</b>. */
void
server_onion_keys_free(server_onion_keys_t *keys)
{
  ...
  nspider_key_map_free(keys->curve25519_key_map);
  spider_free(keys->junk_keypair);
  memwipe(keys, 0, sizeof(server_onion_keys_t));
  spider_free(keys);
}

/** Release whatever sspiderage is held in <b>state</b>, depending on its
...
onion_handshake_state_release(onion_handshake_state_t *state)
{
  ...
  case ONION_HANDSHAKE_TYPE_NTOR:
    nspider_handshake_state_free(state->u.nspider);
    state->u.nspider = NULL;
    break;
  default:
    /* LCOV_EXCL_START
 ...
 * <b>onion_skin_out</b>, and sspidere any state information in <b>state_out</b>.
                  ...
                  uint8_t *onion_skin_out)
{
  ...
  case ONION_HANDSHAKE_TYPE_FAST:
    ...
    break;
  case ONION_HANDSHAKE_TYPE_NTOR:
    if (!extend_info_supports_nspider(node))
      return -1;
    if (onion_skin_nspider_create((const uint8_t*)node->identity_digest,
                      ...
                      uint8_t *rend_nonce_out)
{
  ...
  case ONION_HANDSHAKE_TYPE_NTOR:
      ...
      return -1;
    {
      ...
      uint8_t *keys_tmp = spider_malloc(keys_out_len + DIGEST_LEN);

      if (onion_skin_nspider_server_handshake(
                                   onion_skin, keys->curve25519_key_map,
                                   keys->junk_keypair,
                                   keys->my_identity,
                                   reply_out, keys_tmp, keys_tmp_len)<0) {
        spider_free(keys_tmp);
        return -1;
      }
 ...
 * <b>rend_authenticaspider_out</b> to the "KH" field that can be used to
...
onion_skin_client_handshake(int type,
                      ...
                      uint8_t *keys_out, size_t keys_out_len,
                      uint8_t *rend_authenticaspider_out,
                      const char **msg_out)
{
  ...
  case ONION_HANDSHAKE_TYPE_TAP:
    ...
    memcpy(rend_authenticaspider_out, reply+DH_KEY_LEN, DIGEST_LEN);
  ...
  case ONION_HANDSHAKE_TYPE_FAST:
    ...
    memcpy(rend_authenticaspider_out, reply+DIGEST_LEN, DIGEST_LEN);
    return 0;
  case ONION_HANDSHAKE_TYPE_NTOR:
    if (reply_len < NTOR_REPLY_LEN) {
      if (msg_out)
        *msg_out = "nspider reply was not of the correct length.";
      return -1;
    }
    {
      size_t keys_tmp_len = keys_out_len + DIGEST_LEN;
      uint8_t *keys_tmp = spider_malloc(keys_tmp_len);
      if (onion_skin_nspider_client_handshake(handshake_state->u.nspider,
                                        reply,
      ...
      memcpy(keys_out, keys_tmp, keys_out_len);
      memcpy(rend_authenticaspider_out, keys_tmp + keys_out_len, DIGEST_LEN);
      memwipe(keys_tmp, 0, keys_tmp_len);
      spider_free(keys_tmp);
    }
    return 0;
 ...
 * TAP payload is really an nspider payload.  We'd do away with this if every
 * relay supported EXTEND2, but we want to be able to extend from A to B with
 * nspider even when A doesn't understand EXTEND2 and so can't generate a
 * CREATE2 cell.
 **/
#define NTOR_CREATE_MAGIC "nspiderNTORnspiderNTOR"
...
create_cell_parse(create_cell_t *cell_out, const cell_t *cell_in)
{
  ...
  case CELL_CREATE:
    if (spider_memeq(cell_in->payload, NTOR_CREATE_MAGIC, 16)) {
...
{
  if (spider_digest_is_zero((const char*)cell->node_id))
    return -1;
  /* We don't currently allow EXTEND2 cells without an IPv4 address */
  if (spider_addr_family(&cell->orport_ipv4.addr) == AF_UNSPEC)
    return -1;
...
{
  spider_assert(cell_out);
  spider_assert(cell);
  memset(cell_out, 0, sizeof(*cell_out));
  spider_addr_make_unspec(&cell_out->orport_ipv4.addr);
  spider_addr_make_unspec(&cell_out->orport_ipv6.addr);

  cell_out->cell_type = RELAY_COMMAND_EXTEND;
  spider_addr_from_ipv4h(&cell_out->orport_ipv4.addr, cell->ipv4addr);
  cell_out->orport_ipv4.port = cell->port;
  if (spider_memeq(cell->onionskin, NTOR_CREATE_MAGIC, 16)) {
                                   ...
                                   const extend2_cell_body_t *cell)
{
    ...
    switch (ls->ls_type) {
      case LS_IPV4:
        if (found_ipv4)
          continue;
        found_ipv4 = 1;
        spider_addr_from_ipv4h(&cell_out->orport_ipv4.addr, ls->un_ipv4_addr);
      ...
      case LS_IPV6:
        if (found_ipv6)
          continue;
        found_ipv6 = 1;
        spider_addr_from_ipv6_bytes(&cell_out->orport_ipv6.addr,
  ...
  if (!found_rsa_id || !found_ipv4) /* These are mandaspidery */
    return -1;
                        ...
                        int relayed)
{
  ...
  case CELL_CREATE:
    ...
    /* Fall through */
  case CELL_CREATE_FAST:
    spider_assert(cell_in->handshake_len <= space);
    memcpy(p, cell_in->onionskin, cell_in->handshake_len);
    break;
  case CELL_CREATE2:
    spider_assert(cell_in->handshake_len <= sizeof(cell_out->payload)-4);
...
created_cell_format(cell_t *cell_out, const created_cell_t *cell_in)
{
  ...
  case CELL_CREATED:
  case CELL_CREATED_FAST:
    spider_assert(cell_in->handshake_len <= sizeof(cell_out->payload));
    memcpy(cell_out->payload, cell_in->reply, cell_in->handshake_len);
    break;
  case CELL_CREATED2:
    spider_assert(cell_in->handshake_len <= sizeof(cell_out->payload)-2);
...
/** Return true iff we are configured (by spiderrc or by the networkstatus
...
/** Format the EXTEND{,2} cell in <b>cell_in</b>, sspidering its relay payload in
  ...
  case RELAY_COMMAND_EXTEND:
    {
      ...
      *len_out = 6 + TAP_ONIONSKIN_CHALLENGE_LEN + DIGEST_LEN;
      set_uint32(p, spider_addr_to_ipv4n(&cell_in->orport_ipv4.addr));
  ...
  case RELAY_COMMAND_EXTEND2:
    {
      ...
      *command_out = RELAY_COMMAND_EXTEND2;
      ...
      link_specifier_t *ls;
      {
        ...
        ls->ls_type = LS_IPV4;
        ls->ls_len = 6;
        ls->un_ipv4_addr = spider_addr_to_ipv4h(&cell_in->orport_ipv4.addr);
...
/** Format the EXTENDED{,2} cell in <b>cell_in</b>, sspidering its relay payload
