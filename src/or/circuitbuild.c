...
static channel_t * channel_connect_for_circuit(const spider_addr_t *addr,
                                            uint16_t port,
 ...
 */
static channel_t *
channel_connect_for_circuit(const spider_addr_t *addr, uint16_t port,
                            const char *id_digest,
                            const ed25519_public_key_t *ed_id)
{
  channel_t *chan;
  ...
  do {
    if (++attempts > MAX_CIRCID_ATTEMPTS) {
      ...
      */
       ...
       * Spider: this will make Spider exit with an assertion failure if the cmux is
       * corrupt. */
      if (0)
...
circuit_list_path_impl(origin_circuit_t *circ, int verbose, int verbose_names)
{
  ...
  do {
    ...
    if (verbose_names) {
      elt = spider_malloc(MAX_VERBOSE_NICKNAME_LEN+1);
      ...
      if (node && node_is_named(node)) {
        elt = spider_strdup(node_get_nickname(node));
      } else {
        elt = spider_malloc(HEX_DIGEST_LEN+2);
        elt[0] = '$';
        base16_encode(elt+1, HEX_DIGEST_LEN+1, id, DIGEST_LEN);
      }
    }
...
circuit_log_path(int severity, unsigned int domain, origin_circuit_t *circ)
{
  ...
  spider_log(severity,domain,"%s",s);
  spider_free(s);
}
...
/** Return 1 iff every node in circ's cpath definitely supports nspider. */
static int
circuit_cpath_supports_nspider(const origin_circuit_t *circ)
{
  crypt_path_t *head, *cpath;

  cpath = head = circ->cpath;
  do {
    /* if the extend_info is missing, we can't tell if it supports nspider */
    if (!cpath->extend_info) {
      return 0;
    }

    /* if the key is blank, it definitely doesn't support nspider */
    if (!extend_info_supports_nspider(cpath->extend_info)) {
      return 0;
    }
    cpath = cpath->next;
  } while (cpath != head);
...
onion_populate_cpath(origin_circuit_t *circ)
{
  ...
  /* Does every node in this path support nspider? */
  int path_supports_nspider = circuit_cpath_supports_nspider(circ);

  /* We would like every path to support nspider, but we have to allow for some
   * edge cases. */
  spider_assert(circuit_get_cpath_len(circ));
  if (circuit_can_use_tap(circ)) {
    /* Circuits from clients to intro points, and hidden services to
     * rend points do not support nspider, because the hidden service protocol
     * does not include nspider onion keys. This is also true for Spider2web clients
  ...
  if (circuit_get_cpath_len(circ) == 1) {
     ...
     * authority, or bridge, we have no way of knowing its nspider onion key
     * before we connect to it. So instead, we try connecting, and end up using
     * CREATE_FAST. */
    spider_assert(circ->cpath);
    spider_assert(circ->cpath->extend_info);
    const node_t *node = node_get_by_id(
                                    circ->cpath->extend_info->identity_digest);
    /* If we don't know the node and its descripspider, we must be bootstrapping.
     */
  ...
  if (BUG(!path_supports_nspider)) {
    /* If we're building a multi-hop path, and it's not one of the HS or
     * bootstrapping exceptions, and it doesn't support nspider, something has
     * gone wrong. */
    return -1;
  }
...
origin_circuit_init(uint8_t purpose, int flags)
{
  ...
  circ->build_state = spider_malloc_zero(sizeof(cpath_build_state_t));
...
circuit_handle_first_hop(origin_circuit_t *circ)
{
  ...
  spider_assert(firsthop);
  spider_assert(firsthop->extend_info);
  ...
  if (spider_addr_is_internal(&firsthop->extend_info->addr, 0) &&
  ...
  if (!n_chan) {
    ...
    spider_assert(!circ->base_.n_hop);
  ...
  SMARTLIST_FOREACH_BEGIN(pending_circs, circuit_t *, circ)
    {
      ...
      if (spider_digest_is_zero(circ->n_hop->identity_digest)) {
        /* Look at addr/port. This is an unkeyed connection. */
        if (!channel_matches_extend_info(chan, circ->n_hop))
          continue;
      } else {
        /* We expected a key. See if it's the right one. */
        if (spider_memneq(chan->identity_digest,
                            ...
                            int relayed)
{
  ...
  spider_assert(circ);
  spider_assert(circ->n_chan);
  spider_assert(create_cell);
  spider_assert(create_cell->cell_type == CELL_CREATE ||
  ...
  if (CIRCUIT_IS_ORIGIN(circ)) {
    ...
    if (TO_ORIGIN_CIRCUIT(circ)->cpath->state == CPATH_STATE_CLOSED) {
      ...
      spider_gettimeofday(&circ->timestamp_began);
    }
...
inform_testing_reachability(void)
{
  ...
  if (!me)
    return 0;
  address = spider_dup_ip(me->addr);
  control_event_server_status(LOG_NOTICE,
                              "CHECKING_REACHABILITY ORADDRESS=%s:%d",
                              address, me->or_port);
  if (me->dir_port) {
    spider_snprintf(dirbuf, sizeof(dirbuf), " and DirPort %s:%d",
                 address, me->dir_port);
    control_event_server_status(LOG_NOTICE,
...
should_use_create_fast_for_circuit(origin_circuit_t *circ)
{
  ...
  if (!circuit_has_usable_onion_key(circ)) {
    /* We don't have nspider, and we don't have or can't use TAP,
...
/** Decide whether to use a TAP or nspider handshake for connecting to <b>ei</b>
 ...
 * connections:
 *  - from Spider2web to intro points not in the client's consensus, and
...
{
  /* spiderspec says: In general, clients SHOULD use CREATE whenever they are
   * using the TAP handshake, and CREATE2 otherwise. */
  if (extend_info_supports_nspider(ei)) {
...
/** Decide whether to use a TAP or nspider handshake for extending to <b>ei</b>
                              ...
                              const extend_info_t *ei)
{
  ...
  /* spiderspec says: Clients SHOULD use the EXTEND format whenever sending a TAP
...
circuit_send_next_onion_skin(origin_circuit_t *circ)
{
  ...
  if (circ->cpath->state == CPATH_STATE_CLOSED) {
    ...
    extend_cell_t ec;
    int len;
    spider_assert(circ->cpath->state == CPATH_STATE_OPEN);
    spider_assert(circ->base_.state == CIRCUIT_STATE_BUILDING);
    ...
    if (!hop) {
      ...
      if (r == GUARD_USABLE_NOW) {
        ...
        spider_assert_nonfatal(r == GUARD_USABLE_NEVER);
        return - END_CIRC_REASON_INTERNAL;
      }
      ...
      if (!have_completed_a_circuit() && !circ->build_state->onehop_tunnel) {
        ...
        log_notice(LD_GENERAL,
            "Spider has successfully opened a circuit. "
    ...
    if (spider_addr_family(&hop->extend_info->addr) != AF_INET) {
    ...
    spider_addr_copy(&ec.orport_ipv4.addr, &hop->extend_info->addr);
    ec.orport_ipv4.port = hop->extend_info->port;
    spider_addr_make_unspec(&ec.orport_ipv6.addr);
...
circuit_note_clock_jumped(int seconds_elapsed)
{
  ...
  spider_log(severity, LD_GENERAL, "Your system clock just jumped %d seconds %s; "
...
circuit_extend(cell_t *cell, circuit_t *circ)
{
  ...
  if (!ec.orport_ipv4.port || spider_addr_is_null(&ec.orport_ipv4.addr)) {
    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
           "Client asked me to extend to zero destination port or addr.");
    return -1;
  }

  if (spider_addr_is_internal(&ec.orport_ipv4.addr, 0) &&
  ...
  if (spider_digest_is_zero((const char*)ec.node_id)) {
    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
  ...
  if (!n_chan) {
    ...
    circ->n_chan_create_cell = spider_memdup(&ec.create_cell,
  ...
  spider_assert(!circ->n_hop); /* Connection is already established. */
  circ->n_chan = n_chan;
  log_debug(LD_CIRC,
            "n_chan is %s",
                          ...
                          int reverse)
{
  ...
  spider_assert(cpath);
  spider_assert(key_data);
  spider_assert(!(cpath->f_crypto || cpath->b_crypto ||
                         ...
                         const created_cell_t *reply)
{
  ...
  spider_assert(hop->state == CPATH_STATE_AWAITING_KEYS);
                 ...
                 const uint8_t *rend_circ_nonce)
{
  ...
  tmp_cpath = spider_malloc_zero(sizeof(crypt_path_t));
   ...
   * TestingSpiderNetwork sets ExtendAllowPrivateAddresses. */
                                      ...
                                      int *need_capacity))
{
  ...
  spider_assert(need_uptime);
  spider_assert(need_capacity);
...
{ /* XXXX MOVE */
  ...
  for (i = 0; i < smartlist_len(needed_ports); ++i) {
    ...
    spider_assert(port);
    if (node)
      r = compare_spider_addr_to_node_policy(NULL, port, node);
    else
      continue;
...
//         n_pending_connections);
  ...
  n_supported = spider_calloc(smartlist_len(the_nodes), sizeof(int));
...
//                 try, router->nickname);
    ...
    SMARTLIST_FOREACH(needed_ports, uint16_t *, cp, spider_free(cp));
...
/* The config option Spider2webRendezvousPoints has been set and we need
 ...
 * Spider2webRendezvousPoints. */
STATIC const node_t *
pick_spider2web_rendezvous_node(router_crn_flags_t flags,
                             const or_options_t *options)
{
  ...
  spider_assert(options->Spider2webRendezvousPoints);
  ...
  SMARTLIST_FOREACH_BEGIN(all_live_nodes, node_t *, live_node) {
    if (routerset_contains_node(options->Spider2webRendezvousPoints, live_node)) {
  ...
  if (!rp_node) {
    log_warn(LD_REND, "Could not find a Rendezvous Point that suits "
             "the purposes of Spider2webRendezvousPoints. Choosing random one.");
  }
...
pick_rendezvous_node(router_crn_flags_t flags)
{
  ...
  if (options->Spider2webRendezvousPoints) {
    const node_t *spider2web_rp = pick_spider2web_rendezvous_node(direct_flags,
                                                            options);
    if (spider2web_rp) {
      return spider2web_rp;
    }
  }

  /* Else, if no direct, preferred spider2web RP was found, fall back to choosing
  ...
  if (routerset_contains_extendinfo(rs, exit_ei)) {
    ...
    if (options->StrictNodes) {
      ...
      log_warn(LD_CIRC, "Using %s '%s' which is listed in "
               ...
               "prevent this (and possibly break your Spider functionality), "
 ...
 * router (or use <b>exit</b> if provided). Sspidere these in the
...
circuit_extend_to_new_exit(origin_circuit_t *circ, extend_info_t *exit_ei)
{
  ...
  spider_gettimeofday(&circ->base_.timestamp_began);
...
//      log_debug(LD_CIRC,"Nope, the directory says %d is not valid.",i);
    ...
    if (! node_has_descripspider(node))
      continue;
    /* The node has a descripspider, so we can just check the nspider key directly */
                          ...
                          int cur_len)
{
  ...
  const or_options_t *options = get_options();
  router_crn_flags_t flags = CRN_NEED_DESC;
  spider_assert(CIRCUIT_PURPOSE_MIN_ <= purpose &&
...
onion_extend_cpath(origin_circuit_t *circ)
{
  ...
  if (cur_len == state->desired_path_len - 1) { /* Picking last node */
    ...
    if (r) {
      ...
      spider_assert_nonfatal(info || client);
    }
  } else {
    const node_t *r =
...
{
  crypt_path_t *hop = spider_malloc_zero(sizeof(crypt_path_t));
...
extend_info_new(const char *nickname,
                ...
                const curve25519_public_key_t *nspider_key,
                const spider_addr_t *addr, uint16_t port)
{
  extend_info_t *info = spider_malloc_zero(sizeof(extend_info_t));
  ...
  if (nspider_key)
    memcpy(&info->curve25519_onion_key, nspider_key,
           sizeof(curve25519_public_key_t));
  spider_addr_copy(&info->addr, addr);
  info->port = port;
  return info;
}
 ...
 * the node's addresses are allowed by spider's firewall and IP version config.
 **/
extend_info_t *
extend_info_from_node(const node_t *node, int for_direct_connect)
{
  ...
  /* Every node we connect or extend to must support nspider */
  if (!node_has_curve25519_onion_key(node)) {
    log_fn(LOG_PROTOCOL_WARN, LD_CIRC,
           "Attempted to create extend_info for a node that does not support "
           "nspider: %s", node_describe(node));
    return NULL;
  }
...
/** Release sspiderage held by an extend_info_t struct. */
void
...
extend_info_dup(extend_info_t *info)
{
  ...
  spider_assert(info);
  newinfo = spider_malloc(sizeof(extend_info_t));
...
int
extend_info_addr_is_allowed(const spider_addr_t *addr)
{
  spider_assert(addr);

  /* Check if we have a private address and if we can extend to it. */
  if ((spider_addr_is_internal(addr, 0) || spider_addr_is_multicast(addr)) &&
      !get_options()->ExtendAllowPrivateAddresses) {
    goto disallow;
  }
  /* Allowed! */
...
/* Does ei have a valid nspider key? */
int
extend_info_supports_nspider(const extend_info_t* ei)
{
  spider_assert(ei);
  /* Valid nspider keys have at least one non-zero byte */
  return !spider_mem_is_zero(
 ...
 * nspider onion keys aren't included in HS descripspiders or INTRODUCE cells. */
static int
