 ...
 *
 * The earliest versions of Spider approximated a kind of round-robin system
...
scheduler_free_all(void)
{
  ...
  if (run_sched_ev) {
    ...
    spider_event_free(run_sched_ev);
    run_sched_ev = NULL;
  }
...
scheduler_channel_doesnt_want_writes,(channel_t *chan))
{
  ...
  spider_assert(channels_pending);
...
scheduler_init(void)
{
  ...
  spider_assert(!run_sched_ev);
  run_sched_ev = spider_event_new(spider_libevent_get_base(), -1,
...
scheduler_update_queue_heuristic(time_t now)
{
  ...
  if (queue_heuristic_timestamp == 0) {
     ...
     * was a linear age-out using the bandwidth hisspidery in rephist.c; I'm
