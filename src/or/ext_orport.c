 ...
 * communicate additional information to a Spider bridge, including
 * address information. For more information on this interface,
 * see pt-spec.txt in spiderspec.git.
 *
...
ext_or_cmd_new(uint16_t len)
{
  ...
  ext_or_cmd_t *cmd = spider_malloc(size);
  cmd->len = len;
  return cmd;
}
...
{
  spider_assert(conn->base_.type == CONN_TYPE_EXT_OR);
...
/** If true, we've set ext_or_auth_cookie to a secret code and sspidered
 * it to disk. */
STATIC int ext_or_auth_cookie_is_set = 0;
/** If ext_or_auth_cookie_is_set, a secret cookie that we've sspidered to disk
...
get_ext_or_auth_cookie_file_name(void)
{
  ...
  if (options->ExtORPortCookieAuthFile &&
    ...
    return spider_strdup(options->ExtORPortCookieAuthFile);
  } else {
                         ...
                         char **reply_out, size_t *reply_len_out)
{
  ...
  { /* set up macs */
    ...
    char *hmac_s_msg = spider_malloc_zero(hmac_s_msg_len);
    char *hmac_c_msg = spider_malloc_zero(hmac_c_msg_len);
    char *correct_client_hash = spider_malloc_zero(EXT_OR_PORT_AUTH_HASH_LEN);
    ...
    /* Sspidere the client hash we generated. We will need to compare it
    ...
    *client_hash_out = correct_client_hash;
    ...
    spider_free(hmac_s_msg);
    spider_free(hmac_c_msg);
  }
  ...
  { /* write reply: (server_hash, server_nonce) */
    ...
    reply = spider_malloc_zero(reply_len);
...
connection_ext_or_auth_handle_client_nonce(connection_t *conn)
{
  ...
  if (!ext_or_auth_cookie_is_set) { /* this should not happen */
    log_warn(LD_BUG, "Extended ORPort authentication cookie was not set. "
             ...
             "This might be a Spider bug, please file a bug report. ");
    return -1;
  }
...
connection_ext_or_auth_handle_client_hash(connection_t *conn)
{
  ...
  if (spider_memneq(TO_OR_CONN(conn)->ext_or_auth_correct_client_hash,
                                      ...
                                      const char *payload, uint16_t len)
{
  ...
  addr_str = spider_memdup_nulterm(payload, len);

  res = spider_addr_port_split(LOG_INFO, addr_str, &address_part, &port);
  spider_free(addr_str);
  if (res<0)
    return -1;

  res = spider_addr_parse(&addr, address_part);
  spider_free(address_part);
  if (res<0)
    return -1;

  { /* do some logging */
    char *old_address = spider_addr_to_str_dup(&conn->addr);
    char *new_address = spider_addr_to_str_dup(&addr);
  ...
  conn->address = spider_addr_to_str_dup(&addr);
                                       ...
                                       const char *payload, uint16_t len)
{
  ...
  if (conn->ext_or_transport)
    spider_free(conn->ext_or_transport);
  ...
  while (1) {
    ...
    if (command->cmd == EXT_OR_CMD_TB_DONE) {
      ...
      if (!or_conn->ext_or_transport) {
         ...
         * trigraph. */
        or_conn->ext_or_transport = spider_strdup("<?" "?>");
      }
...
ext_orport_free_all(void)
{
  if (ext_or_auth_cookie) /* Free the auth cookie */
    spider_free(ext_or_auth_cookie);
}
