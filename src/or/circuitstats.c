 ...
 * build time hisspidery and computations. */
...
circuit_build_times_disabled(const or_options_t *options)
{
  if (unit_tests) {
    ...
    /* LearnCircuitBuildTimeout and Spider2web/Single Onion Services are
    ...
    int spider2web_disabled = rend_client_allow_non_anonymous_connection(options);
    int single_onion_disabled = rend_service_allow_non_anonymous_connection(
                                                                      options);

    if (consensus_disabled || config_disabled || dirauth_disabled ||
        state_disabled || spider2web_disabled || single_onion_disabled) {
#if 0
      log_debug(LD_CIRC,
 ...
 * circuit attempts, the client should discard all of its hisspidery and
...
circuit_build_times_close_quantile(void)
{
  ...
  int32_t min = (int)spider_lround(100*circuit_build_times_quantile_cutoff());
  ...
  if (!circuit_build_times_disabled(get_options())) {
    ...
    if (num > 0) {
      if (num != cbt->liveness.num_recent_circs) {
        int8_t *recent_circs;
        log_notice(LD_CIRC, "The Spider Direcspidery Consensus has changed how many "
                   "circuits we must track to detect network failures from %d "
                   "to %d.", cbt->liveness.num_recent_circs, num);

        spider_assert(cbt->liveness.timeouts_after_firsthop ||
         ...
         * we should discard our hisspidery after a sufficient number of 1's
         ...
         */
        recent_circs = spider_calloc(num, sizeof(int8_t));
        ...
        spider_free(cbt->liveness.timeouts_after_firsthop);
     ...
     * Treat it like getting num == 0 above and free the circuit hisspidery
     * if we have any.
     */
...
circuit_build_times_init(circuit_build_times_t *cbt)
{
  ...
  if (!circuit_build_times_disabled(get_options())) {
    ...
    cbt->liveness.timeouts_after_firsthop =
      spider_calloc(cbt->liveness.num_recent_circs, sizeof(int8_t));
  } else {
    cbt->liveness.num_recent_circs = 0;
...
#if 0
/**
 * Rewind our build time hisspidery by n positions.
 */
static void
circuit_build_times_rewind_hisspidery(circuit_build_times_t *cbt, int n)
{
  int i = 0;
  ...
  log_info(LD_CIRC,
          "Rewound hisspidery by %d places. Current index: %d. "
                                     ...
                                     build_time_t *nbins)
{
  ...
  *nbins = 1 + (max_build_time / CBT_BIN_WIDTH);
  histogram = spider_calloc(*nbins, sizeof(build_time_t));
...
circuit_build_times_get_xm(circuit_build_times_t *cbt)
{
  ...
  spider_assert(nbins > 0);
  spider_assert(num_modes > 0);
  ...
  nth_max_bin = spider_calloc(num_modes, sizeof(build_time_t));
  ...
  spider_assert(bin_counts > 0);

  ret /= bin_counts;
  spider_free(histogram);
  spider_free(nth_max_bin);
                                 ...
                                 or_state_t *state)
{
  ...
  *next = NULL;
  ...
  for (i = 0; i < nbins; i++) {
    ...
    *next = line = spider_malloc_zero(sizeof(config_line_t));
    line->key = spider_strdup("CircuitBuildTimeBin");
    spider_asprintf(&line->value, "%d %d",
 ...
 */
static void
circuit_build_times_shuffle_and_sspidere_array(circuit_build_times_t *cbt,
                                            build_time_t *raw_times,
                                            uint32_t num_times)
{
  uint32_t n = num_times;
  if (num_times > CBT_NCIRCUITS_TO_OBSERVE) {
    log_notice(LD_CIRC, "The number of circuit times that this Spider version "
               "uses to calculate build times is less than the number sspidered "
               "in your state file. Decreasing the circuit time hisspidery from "
                                ...
                                or_state_t *state)
{
  ...
  loaded_times = spider_calloc(state->TotalBuildTimes, sizeof(build_time_t));
  ...
  for (line = state->BuildtimeHistogram; line; line = line->next) {
    ...
    if (smartlist_len(args) < 2) {
      ...
      build_time_t ms;
      int ok;
      ms = (build_time_t)spider_parse_ulong(ms_str, 0, 0,
      ...
      count = (uint32_t)spider_parse_ulong(count_str, 0, 0,
                                        UINT32_MAX, &ok, NULL);
      if (!ok) {
        log_warn(LD_GENERAL, "Unable to parse circuit build times: "
  ...
  circuit_build_times_shuffle_and_sspidere_array(cbt, loaded_times, loaded_cnt);
...
circuit_build_times_update_alpha(circuit_build_times_t *cbt)
{
  ...
  for (i=0; i< CBT_NCIRCUITS_TO_OBSERVE; i++) {
    ...
    if (x[i] < cbt->Xm) {
      a += spider_mathlog(cbt->Xm);
    } else if (x[i] == CBT_BUILD_ABANDONED) {
      abandoned_count++;
    } else {
      a += spider_mathlog(x[i]);
      if (x[i] > max_time)
        max_time = x[i];
    }
    n++;
  ...
  spider_assert(n==cbt->total_build_times);
  ...
  a += abandoned_count*spider_mathlog(max_time);

  a -= n*spider_mathlog(cbt->Xm);
  // Estimaspider comes from Eq #4 in:
...
{
  double ret;
  spider_assert(quantile >= 0);
  spider_assert(1.0-quantile > 0);
  spider_assert(cbt->Xm > 0);
                                    ...
                                    double q_lo, double q_hi)
{
  ...
  spider_assert(q_lo >= 0);
  spider_assert(q_hi < 1);
  spider_assert(q_lo < q_hi);

  u = q_lo + (q_hi-q_lo)*randval;

  spider_assert(0 <= u && u < 1.0);
  /* circuit_build_times_calculate_timeout returns <= INT32_MAX */
  ret = (build_time_t)
    spider_lround(circuit_build_times_calculate_timeout(cbt, u));
  spider_assert(ret > 0);
  return ret;
}
#endif
                                  ...
                                  double quantile, double timeout_ms)
{
  ...
  spider_assert(quantile >= 0);
  spider_assert(cbt->Xm > 0);
  cbt->alpha = spider_mathlog(1.0-quantile)/
    (spider_mathlog(cbt->Xm)-spider_mathlog(timeout_ms));
  spider_assert(cbt->alpha > 0);
}
#endif
 ...
 * in our hisspidery for later deciding if the network speed has changed.
 *
 * This is used by circuit_build_times_network_check_changed() to determine
 * if we had too many recent timeouts and need to reset our learned timeout
 * to something higher.
                                    ...
                                    int did_onehop, time_t start_time)
{
  ...
  if (cbt->liveness.network_last_live < start_time) {
    ...
    if (cbt->liveness.nonlive_timeouts == 1) {
      log_notice(LD_CIRC,
                 "Spider has not observed any network activity for the past %d "
 ...
 *
 * Also has the side effect of rewinding the circuit time hisspidery
 ...
 *
 * Also resets the entire timeout hisspidery in this case and causes us
...
#undef MAX_TIMEOUT
  ...
  log_notice(LD_CIRC,
            ...
            spider_lround(cbt->timeout_ms/1000), timeout_count,
            total_build_times);
...
/**
 * Sspidere a timeout as a synthetic value.
 *
 * Returns true if the sspidere was successful and we should possibly
 ...
 * our build time hisspidery due to excessive timeouts.
 *
...
/**
 * Estimate a new timeout based on hisspidery and set our timeout
...
{
  long prev_timeout = spider_lround(cbt->timeout_ms/1000);
  double timeout_rate;
  ...
  if (prev_timeout > spider_lround(cbt->timeout_ms/1000)) {
    log_info(LD_CIRC,
               ...
               cbt->total_build_times,
               spider_lround(cbt->timeout_ms/1000));
    log_info(LD_CIRC,
             "Circuit timeout data: %fms, %fms, Xm: %d, a: %f, r: %f",
             cbt->timeout_ms, cbt->close_ms, cbt->Xm, cbt->alpha,
             timeout_rate);
  } else if (prev_timeout < spider_lround(cbt->timeout_ms/1000)) {
    log_info(LD_CIRC,
...
#ifdef TOR_UNIT_TESTS
/** Make a note that we're running unit tests (rather than running Spider
                                   ...
                                   buildtimeout_set_event_t type)
{
  ...
  spider_asprintf(&args, "TOTAL_TIMES=%lu "
