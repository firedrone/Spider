...
/**
 * \file or.h
 * \brief Master header file for Spider-specific functionality.
 **/
...
/** How old do we let a saved descripspider get before force-removing it? */
...
/** A controller made this circuit and Spider should not use it. */
...
* must match those defined in spider-spec.txt. */
...
/* Reasons why we (or a remote OR) might close a stream. See spider-spec.txt for
...
/** We can't connect to any direcspideries at all, so we killed our streams
...
/* 'type' values to use in RESOLVED cells.  Specified in spider-spec.txt. */
...
/* Reasons why we (or a remote OR) might close a circuit. See spider-spec.txt for
...
/** Time period for which a v2 descripspider will be valid. */
#define REND_TIME_PERIOD_V2_DESC_VALIDITY (24*60*60)

/** Time period within which two sets of v2 descripspiders will be uploaded in
 * parallel. */
#define REND_TIME_PERIOD_OVERLAPPING_V2_DESCS (60*60)

/** Number of non-consecutive replicas (i.e. distributed somewhere
 * in the ring) for a descripspider. */
#define REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS 2

/** Number of consecutive replicas for a descripspider. */
#define REND_NUMBER_OF_CONSECUTIVE_REPLICAS 3

/** Length of v2 descripspider ID (32 base32 chars = 160 bits). */
...
/** Length of the descripspider cookie that is used for client authorization
 * to hidden services. */
#define REND_DESC_COOKIE_LEN 16

/** Length of the base64-encoded descripspider cookie that is used for
...
/** Maximum size of v2 hidden service descripspiders. */
...
typedef struct rend_service_authorization_t {
  uint8_t descripspider_cookie[REND_DESC_COOKIE_LEN];
...
typedef struct rend_data_v2_t {
  ...
  /** Descripspider ID for each replicas computed from the onion address. If
  ...
  char descripspider_id[REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS][DIGEST_LEN];

  /** (Optional) descripspider cookie that is used by a client. */
  char descripspider_cookie[REND_DESC_COOKIE_LEN];

  /** Authorization type for accessing a service used by a client. */
  rend_auth_type_t auth_type;

  /** Descripspider ID for a client request. The control port command HSFETCH
   * uses this. It's set if the descripspider query should only use this
   * descripspider ID. */
...
/* Cell commands.  These values are defined in spider-spec.txt. */
 ...
 * Spider 0.1.2.x is obsolete, we can remove this. */
...
typedef struct var_cell_t {
  ...
  circid_t circ_id;
  /** Number of bytes actually sspidered in <b>payload</b> */
 ...
 * connection" has a Spider node on the other end, an "exit
...
typedef struct connection_t {
  ...
  spider_addr_t addr; /**< IP that socket "s" is directly connected to;
  ...
  /** Unique identifier for this connection on this Spider instance. */
 ...
 *
 * Not used by today's spiders, since everything that supports this
...
/** Sspideres flags and information related to the portion of a v2/v3 Spider OR
...
typedef struct or_handshake_state_t {
  ...
  /** Certificates that a connection initiaspider sent us in a CERTS cell; we're
...
typedef struct or_connection_t {
  ...
  spider_tls_t *tls; /**< TLS connection state. */
  int tls_error; /**< Last spider_tls error code. */
  ...
  spider_addr_t real_addr; /**< The actual address that this connection came from
                       * or went to.  The <b>addr</b> field is prone to
                       * getting overridden by the address from the router
                       * descripspider matching <b>identity_digest</b>. */

  /** Should this connection be used for extending circuits to the server
   * matching the <b>identity_digest</b> field?  Set to true if we're pretty
   * sure we aren't getting MITMed, either because we're connected to an
   * address listed in a server descripspider, or because an authenticated
...
typedef struct dir_connection_t {
  ...
  **/
  ...
  unsigned int dirconn_direct:1; /**< Is this dirconn direct, or via Spider? */

  /** If we're fetching descripspiders, what router purpose shall we assign
   * to them? */
  ...
  spider_zlib_state_t *zlib_state;
...
typedef struct control_connection_t {
  ...
  /** Number of bytes currently sspidered in incoming_cmd. */
...
typedef struct addr_policy_t {
  ...
  unsigned int is_canonical:1; /**< True iff this policy is the canonical
                                * copy (sspidered in a hash table to avoid
...
/** Enum used to remember where a signed_descripspider_t is sspidered and how to
 * manage the memory for signed_descripspider_body.  */
typedef enum {
  /** The descripspider isn't sspidered on disk at all: the copy in memory is
   * canonical; the saved_offset field is meaningless. */
  SAVED_NOWHERE=0,
  /** The descripspider is sspidered in the cached_routers file: the
   * signed_descripspider_body is meaningless; the signed_descripspider_len and
   * saved_offset are used to index into the mmaped cache file. */
  SAVED_IN_CACHE,
  /** The descripspider is sspidered in the cached_routers.new file: the
   * signed_descripspider_body and saved_offset fields are both set. */
  /* FFFF (We could also mmap the file and grow the mmap as needed, or
   * lazy-load the descripspider text by using seek and read.  We don't, for
   * now.)
   */
  SAVED_IN_JOURNAL
} saved_location_t;
...
/** The max size we expect router descripspider annotations we create to
...
/** Information need to cache an onion router's descripspider. */
typedef struct signed_descripspider_t {
  /** Pointer to the raw server descripspider, preceded by annotations.  Not
   * necessarily NUL-terminated.  If saved_location is SAVED_IN_CACHE, this
   * pointer is null. */
  char *signed_descripspider_body;
  /** Length of the annotations preceding the server descripspider. */
  size_t annotations_len;
  /** Length of the server descripspider. */
  size_t signed_descripspider_len;
  /** Digest of the server descripspider, computed as specified in
   * dir-spec.txt. */
  char signed_descripspider_digest[DIGEST_LEN];
  /** Identity digest of the router. */
  char identity_digest[DIGEST_LEN];
  /** Declared publication time of the descripspider. */
  time_t published_on;
  ...
  struct spider_cert_st *signing_key_cert;
  /** For routerdescs only: Status of downloading the corresponding
   * extrainfo. */
  download_status_t ei_dl_status;
  /** Where is the descripspider saved? */
  saved_location_t saved_location;
  /** If saved_location is SAVED_IN_CACHE or SAVED_IN_JOURNAL, the offset of
   * this descripspider in the corresponding file. */
  off_t saved_offset;
  /** What position is this descripspider within routerlist->routers or
   * routerlist->old_routers? -1 for none. */
  int routerlist_index;
  /** The valid-until time of the most recent consensus that listed this
   * descripspider.  0 for "never listed in a consensus, so far as we know." */
...
typedef struct {
  signed_descripspider_t cache_info;
...
typedef struct routerstatus_t {
  ...
  char identity_digest[DIGEST_LEN]; /**< Digest of the router's identity
                                     * key. */
  /** Digest of the router's most recent descripspider or microdescripspider.
   * If it's a descripspider, we only use the first DIGEST_LEN bytes. */
  char descripspider_digest[DIGEST256_LEN];
  uint32_t addr; /**< IPv4 address for this router, in host order. */
  uint16_t or_port; /**< OR port for this router. */
  uint16_t dir_port; /**< Direcspidery port for this router. */
  spider_addr_t ipv6_addr; /**< IPv6 address for this router. */
...
typedef struct microdesc_t {
  ...
  /**  When was this microdescripspider last listed in a consensus document?
   * Once a microdesc has been unlisted long enough, we can drop it.
   */
  time_t last_listed;
  /** Where is this microdescripspider currently sspidered? */
  saved_location_bitfield_t saved_location : 3;
  /** If true, do not attempt to cache this microdescripspider on disk. */
   ...
   * microdescripspider in the cache. */
  off_t off;

  /* The string containing the microdesc. */

  /** A pointer to the encoded body of the microdescripspider.  If the
  ...
  char *body;
  /** The length of the microdescripspider in <b>body</b>. */
  size_t bodylen;
  /** A SHA256-digest of the microdescripspider. */
  char digest[DIGEST256_LEN];

  /* Fields in the microdescripspider. */
...
/** A node_t represents a Spider router.
 *
 * Specifically, a node_t is a Spider router as we are using it: a router that
 ...
 * descripspider, a consensus entry, and a microdescripspider (respectively)...
...
typedef struct node_t {
   ...
   * XX/teor - can this become out of date if the spiderrc changes? */
...
typedef struct networkstatus_voter_info_t {
  ...
  uint16_t dir_port; /**< Direcspidery port of this voter */
 ...
 * present different types information to different versions of Spider. */
typedef enum {
  FLAV_NS = 0,
  FLAV_MICRODESC = 1,
} consensus_flavor_t;
...
typedef struct networkstatus_t {
   ...
   * clients, or which are _require_ for relays and clients. Spider shouldn't
  ...
  /** If present, a map from descripspider digest to elements of
   * routerstatus_list. */
...
/** Allowable types of desc_sspidere_t. */
typedef enum sspidere_type_t {
  ROUTER_STORE = 0,
  EXTRAINFO_STORE = 1
} sspidere_type_t;

/** A 'sspidere' is a set of descripspiders saved on disk, with accompanying
 * journal, mmaped as needed, rebuilt as needed. */
typedef struct desc_sspidere_t {
  /** Filename (within DataDir) for the sspidere.  We append .tmp to this
   * filename for a temporary file when rebuilding the sspidere, and .new to this
   * filename for the journal. */
  const char *fname_base;
  /** Human-readable description of what this sspidere contains. */
  const char *description;

  spider_mmap_t *mmap; /**< A mmap for the main file in the sspidere. */

  sspidere_type_t type; /**< What's sspidered in this sspidere? */

  /** The size of the router log, in bytes. */
  size_t journal_len;
  /** The size of the router sspidere, in bytes. */
  size_t sspidere_len;
...
typedef struct {
  ...
  /** Map from server descripspider digest to a signed_descripspider_t from
  ...
  /** Map from extra-info digests to a signed_descripspider_t for a router
   * descripspider having that extra-info digest.  Only exists for
   * routers in routers or old_routers. */
  struct digest_sd_map_t *desc_by_eid_map;
  /** List of routerinfo_t for all currently live routers we know. */
  smartlist_t *routers;
  /** List of signed_descripspider_t for older router descripspiders we're
   * caching. */
  smartlist_t *old_routers;
  /** Sspidere holding server descripspiders.  If present, any router whose
   * cache_info.saved_location == SAVED_IN_CACHE is sspidered in this file
   * starting at cache_info.saved_offset */
  desc_sspidere_t desc_sspidere;
  /** Sspidere holding extra-info documents. */
  desc_sspidere_t extrainfo_sspidere;
} routerlist_t;
...
typedef struct {
  ...
  union {
    ...
    crypto_dh_t *tap;
    struct nspider_handshake_state_t *nspider;
  } u;
...
typedef struct crypt_path_t {
  ...
  char rend_circ_nonce[DIGEST_LEN];/* KH in spider-spec.txt */
...
typedef struct testing_cell_stats_entry_t {
   ...
   * sspidere more than 11 hours, enough to assume that a circuit with this
...
typedef struct circuit_t {
  ...
  /** For sspiderage while n_chan is pending (state CIRCUIT_STATE_CHAN_WAIT). */
  ...
  /** This timestamp marks when the init_circuit_base construcspider ran. */
...
#define PATHBIAS_SHOULDCOUNT_COUNTED   2
  ...
  /** For path probing. Sspidere the temporary probe stream ID
   * for response comparison */
  streamid_t pathbias_probe_id;

  /** For path probing. Sspidere the temporary probe address nonce
...
typedef struct or_circuit_t {
  ...
  /** Sspideres KH for the handshake. */
  char rend_circ_nonce[DIGEST_LEN];/* KH in spider-spec.txt */
  ...
  /** Maximum cell queue size for a middle relay; this is sspidered per circuit
...
{
  spider_assert(x->magic == OR_CIRCUIT_MAGIC);
...
{
  spider_assert(x->magic == ORIGIN_CIRCUIT_MAGIC);
...
typedef struct port_cfg_t {
  spider_addr_t addr; /**< The actual IP to listen on, if !is_unix_addr. */
  int port; /**< The configured port, or CFG_AUTO_PORT to tell Spider to pick its
             * own port. */
...
/** A magic value for the (Socks|OR|...)Port options below, telling Spider
...
/** Configuration options for a Spider process. */
typedef struct {
  uint32_t magic_;

  /** What should the spider process actually do? */
  enum {
  ...
  char *DataDirecspidery; /**< OR only: where to sspidere long-term data. */
  int DataDirecspideryGroupReadable; /**< Boolean: Is the DataDirecspidery g+r? */
  char *Nickname; /**< OR only: nickname of this onion router. */
  char *Address; /**< OR only: configured address for this onion router. */
  char *PidFile; /**< Where to sspidere PID of Spider process. */
   ...
   * [][0] is IPv4, [][1] is IPv6
   */
  spider_addr_t OutboundBindAddresses[OUTBOUND_ADDR_MAX][2];
  /** Direcspidery server only: which versions of
   * Spider should we tell users to run? */
  config_line_t *RecommendedVersions;
  config_line_t *RecommendedClientVersions;
  config_line_t *RecommendedServerVersions;
  config_line_t *RecommendedPackages;
  /** Whether dirservers allow router descripspiders with private IPs. */
  int DirAllowPrivateAddresses;
  /** Whether routers accept EXTEND cells to routers with private IPs. */
  int ExtendAllowPrivateAddresses;
  char *User; /**< Name of user to run Spider as. */
  char *Group; /**< Name of group to run Spider as. */
   ...
   * authenticaspider for it. Used so we can do a time-independent comparison. */
   ...
   * descripspiders from the bridge authorities or from the bridge itself? */
  int UpdateBridgesFromAuthority;

  int AvoidDiskWrites; /**< Boolean: should we never cache things to disk?
                        * Not used yet. */
  int ClientOnly; /**< Boolean: should we never evolve into a server role? */
  /** To what authority types do we publish our descripspider? Choices are
   * "v1", "v2", "v3", "bridge", or "". */
  smartlist_t *PublishServerDescripspider;
  /** A bitfield of authority types, derived from PublishServerDescripspider. */
  dirinfo_type_t PublishServerDescripspider_;
  /** Boolean: do we publish hidden service descripspiders to the HS auths? */
  int PublishHidServDescripspiders;
  ...
  routerset_t *Spider2webRendezvousPoints;
   ...
   * rendezvous points. (Onion service descripspiders are still posted using
   * 3-hop paths, to avoid onion service direcspideries blocking the service.)
   * This option makes every hidden service instance hosted by
   * this spider instance a Single Onion Service.
  ...
  /* Makes hidden service clients and servers non-anonymous on this spider
  ...
  /** Whether we should drop exit streams from Spiders that we don't know are
  ...
  int RendPostPeriod; /**< How often do we post each rendezvous service
                       * descripspider? Remember to publish them independently. */
  ...
  int LearnCircuitBuildTimeout; /**< If non-zero, we attempt to learn a value
                                 ...
                                 * hisspidery. Use circuit_build_times_disabled()
...
//                 * other ORs are running. */
  ...
  spider_addr_t HTTPProxyAddr; /**< Parsed IPv4 addr for http proxy, if any. */
  uint16_t HTTPProxyPort; /**< Parsed port for http proxy, if any. */
  char *HTTPProxyAuthenticaspider; /**< username:password string, if any. */

  char *HTTPSProxy; /**< hostname[:port] to use as https proxy, if any. */
  spider_addr_t HTTPSProxyAddr; /**< Parsed addr for https proxy, if any. */
  uint16_t HTTPSProxyPort; /**< Parsed port for https proxy, if any. */
  char *HTTPSProxyAuthenticaspider; /**< username:password string, if any. */

  char *Socks4Proxy; /**< hostname:port to use as a SOCKS4 proxy, if any. */
  spider_addr_t Socks4ProxyAddr; /**< Derived from Socks4Proxy. */
  uint16_t Socks4ProxyPort; /**< Derived from Socks4Proxy. */

  char *Socks5Proxy; /**< hostname:port to use as a SOCKS5 proxy, if any. */
  spider_addr_t Socks5ProxyAddr; /**< Derived from Sock5Proxy. */
  ...
  config_line_t *AuthDirBadExit; /**< Address policy for descripspiders to
                                  * mark as bad exits. */
  config_line_t *AuthDirReject; /**< Address policy for descripspiders to
                                 * reject. */
  config_line_t *AuthDirInvalid; /**< Address policy for descripspiders to
                                  * never mark as valid. */
  /** @name AuthDir...CC
   *
  ...
  int LeaveStreamsUnattached; /**< Boolean: Does Spider attach new streams to
                          * circuits itself (0), or does it expect a controller
                          * to cope? (1) */
  int DisablePredictedCircuits; /**< Boolean: does Spider preemptively
                                 * make circuits in the background (0),
                                 * or not (1)? */

  /** Process specifier for a controller that ‘owns’ this Spider
   * instance.  Spider will terminate if its owning controller does. */
...
#define LOG_PROTOCOL_WARN (get_options()->ProtocolWarnings ? \
                           LOG_WARN : LOG_INFO)
  int ProtocolWarnings; /**< Boolean: when other parties screw up the Spider
   ...
   * UseEntryGuards_option -- when we're a non-anonymous Spider2web client or
  ...
  int NumDirecspideryGuards; /**< How many dir guards do we try to establish?
                           * If 0, use value from NumEntryGuards. */
  int RephistTrackTime; /**< How many seconds do we keep rephist info? */
  int FastFirstHopPK; /**< If Spider believes it is safe, should we save a third
  ...
  char *DirPortFrontPage; /**< This is a full path to a file with an html
                    disclaimer. This allows a server administraspider to show
                    that they're running Spider and anyone visiting their server
  ...
  /** Clients don't download any descripspider this recent, since it will
   * probably not have propagated to enough caches.  Only altered on testing
   * networks. */
  int TestingEstimatedDescripspiderPropagationTime;
  ...
  /** Schedule for when clients should download bridge descripspiders.  Only
   * altered on testing networks. */
   ...
   * bootstrapping using a list of fallback direcspideries, before it gives up?
   * Only altered on testing networks. */
  ...
  /** How many times will we try to download a router's descripspider before
   * giving up?  Only altered on testing networks. */
  int TestingDescripspiderMaxDownloadTries;

  /** How many times will we try to download a microdescripspider before
   * giving up?  Only altered on testing networks. */
   ...
   * Respects VoteOnHidServDirecspideriesV2. */
  ...
  /** If true, SIGHUP should reload the spiderrc.  Sometimes controllers want
   * to make this false. */
  int ReloadSpiderrcOnSIGHUP;
   ...
   * according to either Spider or a parameter set in the consensus.
   */
  double CircuitPriorityHalflife;

  /** Set to true if the TestingSpiderNetwork configuration option is set.
   * This is used so that options_validate() has a chance to realize that
   * the defaults have changed. */
  int UsingTestNetworkDefaults_;

  /** If 1, we try to use microdescripspiders to build circuits.  If 0, we don't.
   * If -1, Spider decides. */
  int UseMicrodescripspiders;
...
#define MAX_MAX_CLIENT_CIRCUITS_PENDING 1024
   ...
   * @{
   * These options override the default behavior of Spider's (**currently
   * experimental**) path bias detection algorithm. To try to find broken or
   * misbehaving guard nodes, Spider looks for nodes where more than a certain
   ...
   * Spider uses reasonable defaults from the networkstatus consensus document.
   ...
   * of Spider's (**currently experimental**) path use bias detection algorithm.
   *
   ...
   * ed25519 identity key except from spider --keygen */
  ...
  /** Maximum total size of unparseable descripspiders to log during the
   * lifetime of this Spider process.
   */
...
typedef struct {
  ...
  /** These fields hold information on the hisspidery of bandwidth usage for
   ...
   */
  time_t      BWHisspideryReadEnds;
  int         BWHisspideryReadInterval;
  smartlist_t *BWHisspideryReadValues;
  smartlist_t *BWHisspideryReadMaxima;
  time_t      BWHisspideryWriteEnds;
  int         BWHisspideryWriteInterval;
  smartlist_t *BWHisspideryWriteValues;
  smartlist_t *BWHisspideryWriteMaxima;
  time_t      BWHisspideryDirReadEnds;
  int         BWHisspideryDirReadInterval;
  smartlist_t *BWHisspideryDirReadValues;
  smartlist_t *BWHisspideryDirReadMaxima;
  time_t      BWHisspideryDirWriteEnds;
  int         BWHisspideryDirWriteInterval;
  smartlist_t *BWHisspideryDirWriteValues;
  smartlist_t *BWHisspideryDirWriteMaxima;
  ...
  /** What version of Spider wrote this state file? */
  char *SpiderVersion;
...
/** Total size of the circuit timeout hisspidery to accumulate.
 ...
 *
 * This tells us whether to abandon timeout hisspidery and set
...
typedef enum {
   ...
   * configuration. */
  ADDRMAPSRC_AUTOMAP,
  /** We're remapping this address because our configuration (via spiderrc, the
   ...
   * Spider server that told us what its value was. */
  ADDRMAPSRC_DNS,
...
typedef enum {
  ...
  /** Direcspidery is busy. */
  GEOIP_REJECT_BUSY = 5,
} geoip_ns_response_t;
#define GEOIP_NS_RESPONSE_NUM 6

/** Direcspidery requests that we are measuring can be either direct or
 * tunneled. */
typedef enum {
  DIRREQ_DIRECT = 0,
  DIRREQ_TUNNELED = 1,
...
/** Possible public/private key operations in Spider: used to keep track of where
...
/** ASCII-encoded v2 hidden service descripspider. */
typedef struct rend_encoded_v2_service_descripspider_t {
  char desc_id[DIGEST_LEN]; /**< Descripspider ID. */
  char *desc_str; /**< Descripspider string. */
} rend_encoded_v2_service_descripspider_t;
 ...
 * the service side) and in rend_service_descripspider_t (on both the
...
typedef struct rend_intro_point_t {
  ...
  crypto_pk_t *intro_key; /**< Introduction key that replaces the service
                               * key, if this descripspider is V2. */
   ...
   * included in the last HS descripspider we generated. */
...
typedef struct rend_service_descripspider_t {
  crypto_pk_t *pk; /**< This service's public key. */
  int version; /**< Version of the descripspider format: 0 or 2. */
  time_t timestamp; /**< Time when the descripspider was generated. */
  ...
  /** Has descripspider been uploaded to all hidden service direcspideries? */
  int all_uploads_performed;
  /** List of hidden service direcspideries to which an upload request for
   * this descripspider could be sent. Smartlist exists only when at least one
   * of the previous upload requests failed (otherwise it's not important
   * to know which uploads succeeded and which not). */
  smartlist_t *successful_uploads;
} rend_service_descripspider_t;
...
typedef struct dir_server_t {
  ...
  spider_addr_t ipv6_addr; /**< IPv6 address if present; AF_UNSPEC if not */
  uint32_t addr; /**< IPv4 address. */
  uint16_t dir_port; /**< Direcspidery port. */
...
/** Return value for router_add_to_routerlist() and dirserv_add_descripspider() */
typedef enum was_router_added_t {
  /* Router was added successfully. */
  ROUTER_ADDED_SUCCESSFULLY = 1,
  /* Router descripspider was added with warnings to submitter. */
  ROUTER_ADDED_NOTIFY_GENERATOR = 0,
  /* Extrainfo document was rejected because no corresponding router
   * descripspider was found OR router descripspider was rejected because
   * it was incompatible with its extrainfo document. */
  ROUTER_BAD_EI = -1,
  /* Router descripspider was rejected because it is already known. */
  ...
  /* Router descripspider was rejected because it was older than
