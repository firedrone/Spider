 ...
 * ones in the spiderrc file, but one day we may be able to learn about new
 * bridges on our own, and remember them in the state file. */
struct bridge_info_t {
  /** Address and port of the bridge, as configured by the user.*/
  spider_addr_port_t addrport_configured;
  ...
  /** When should we next try to fetch a descripspider for this bridge? */
...
/** A list of configured bridges. Whenever we actually get a descripspider
...
bridge_free(bridge_info_t *bridge)
{
  ...
  spider_free(bridge->transport_name);
  if (bridge->socks_args) {
    SMARTLIST_FOREACH(bridge->socks_args, char*, s, spider_free(s));
...
bridge_get_rsa_id_digest(const bridge_info_t *bridge)
{
  ...
  if (spider_digest_is_zero(bridge->identity))
    return NULL;
  else
 ...
 * spider_addr_port_t's in <b>orports</b>, return that bridge.  Else return
 * NULL. */
                                        ...
                                        const smartlist_t *orports)
{
  ...
  SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, bridge)
    {
      if (spider_digest_is_zero(bridge->identity)) {
        SMARTLIST_FOREACH_BEGIN(orports, spider_addr_port_t *, ap)
          {
            if (spider_addr_compare(&bridge->addr, &ap->addr, CMP_EXACT) == 0 &&
                bridge->port == ap->port)
              return bridge;
          }
        SMARTLIST_FOREACH_END(ap);
      }
      if (digest && spider_memeq(bridge->identity, digest, DIGEST_LEN))
        return bridge;
    }
...
bridge_info_t *
get_configured_bridge_by_addr_port_digest(const spider_addr_t *addr,
                                          uint16_t port,
                                          const char *digest)
{
  if (!bridge_list)
  ...
  SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, bridge)
    {
      if ((spider_digest_is_zero(bridge->identity) || digest == NULL) &&
          !spider_addr_compare(&bridge->addr, addr, CMP_EXACT) &&
          bridge->port == port)
        return bridge;
      if (digest && spider_memeq(bridge->identity, digest, DIGEST_LEN))
        return bridge;
 ...
 */
bridge_info_t *
get_configured_bridge_by_exact_addr_port_digest(const spider_addr_t *addr,
                                                uint16_t port,
                                                const char *digest)
{
  if (!bridge_list)
    return NULL;
  SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, bridge) {
    if (!spider_addr_compare(&bridge->addr, addr, CMP_EXACT) &&
        bridge->port == port) {

      if (digest && spider_memeq(bridge->identity, digest, DIGEST_LEN))
        return bridge;
      else if (!digest || spider_digest_is_zero(bridge->identity))
        return bridge;
    }
 ...
 * address/port matches only. */
int
addr_is_a_configured_bridge(const spider_addr_t *addr,
                                uint16_t port,
                                const char *digest)
{
  spider_assert(addr);
 ...
 * it up via router descripspider <b>ri</b>. */
static bridge_info_t *
get_configured_bridge_by_routerinfo(const routerinfo_t *ri)
{
  ...
  bi = get_configured_bridge_by_orports_digest(ri->cache_info.identity_digest,
                                               orports);
  SMARTLIST_FOREACH(orports, spider_addr_port_t *, p, spider_free(p));
 ...
 */
void
learned_router_identity(const spider_addr_t *addr, uint16_t port,
                        const char *digest,
                        const ed25519_public_key_t *ed_id)
{
  ...
  if (bridge && spider_digest_is_zero(bridge->identity)) {
  ...
  if (learned) {
    ...
    if (transport_name)
      spider_asprintf(&transport_info, " (with transport '%s')", transport_name);
...
{
  if (digest)
    return spider_memeq(digest, bridge->identity, DIGEST_LEN);
  else
    return spider_digest_is_zero(bridge->identity);
}
 ...
 */
static void
bridge_resolve_conflicts(const spider_addr_t *addr, uint16_t port,
                         const char *digest, const char *transport_name)
{
  ...
  */
  SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, bridge) {
    ...
    if (spider_addr_eq(&bridge->addr, addr) && (bridge->port == port)) {
      ...
      if (!bridge_has_digest(bridge, digest) ||
        ...
        spider_asprintf(&bridge_description_new, "%s:%s:%s",
                     fmt_addrport(addr, port),
                     digest ? hex_str(digest, DIGEST_LEN) : "",
                     transport_name ? transport_name : "");
        spider_asprintf(&bridge_description_old, "%s:%s:%s",
                     fmt_addrport(&bridge->addr, bridge->port),
                     spider_digest_is_zero(bridge->identity) ?
        ...
        spider_free(bridge_description_new);
        spider_free(bridge_description_old);
      }
    }
  } SMARTLIST_FOREACH_END(bridge);
}
...
bridge_add_from_config(bridge_line_t *bridge_line)
{
  ...
  { /* Log the bridge we are about to register: */
    log_debug(LD_GENERAL, "Registering bridge at %s (transport: %s) (%s)",
              ...
              spider_digest_is_zero(bridge_line->digest) ?
  ...
  b = spider_malloc_zero(sizeof(bridge_info_t));
  spider_addr_copy(&b->addrport_configured.addr, &bridge_line->addr);
  b->addrport_configured.port = bridge_line->port;
  spider_addr_copy(&b->addr, &bridge_line->addr);
  ...
  spider_free(bridge_line); /* Deallocate bridge_line now. */
...
find_bridge_by_digest(const char *digest)
{
  ...
  SMARTLIST_FOREACH(bridge_list, bridge_info_t *, bridge,
    {
      if (spider_memeq(bridge->identity, digest, DIGEST_LEN))
        return bridge;
    });
  return NULL;
}
 ...
 *  NULL. */
const char *
find_transport_name_by_bridge_addrport(const spider_addr_t *addr, uint16_t port)
{
  if (!bridge_list)
    return NULL;

  SMARTLIST_FOREACH_BEGIN(bridge_list, const bridge_info_t *, bridge) {
    if (spider_addr_eq(&bridge->addr, addr) &&
        (bridge->port == port))
 ...
 */
int
get_transport_by_bridge_addrport(const spider_addr_t *addr, uint16_t port,
...
const smartlist_t *
get_socks_args_by_bridge_addrport(const spider_addr_t *addr, uint16_t port)
{
...
/** We need to ask <b>bridge</b> for its server descripspider. */
static void
launch_direct_bridge_descripspider_fetch(bridge_info_t *bridge)
{
  ...
  /* Until we get a descripspider for the bridge, we only know one address for
   * it. */
  if (!fascist_firewall_allows_address_addr(&bridge->addr, bridge->port,
                                            FIREWALL_OR_CONNECTION, 0, 0)) {
    log_notice(LD_CONFIG, "Tried to fetch a descripspider directly from a "
...
/** Fetching the bridge descripspider from the bridge authority returned a
 * "not found". Fall back to trying a direct fetch. */
void
retry_bridge_descripspider_fetch_directly(const char *digest)
{
  bridge_info_t *bridge = find_bridge_by_digest(digest);
  if (!bridge)
    return; /* not found? oh well. */

  launch_direct_bridge_descripspider_fetch(bridge);
}

/** For each bridge in our list for which we don't currently have a
 * descripspider, fetch a new copy of its descripspider -- either directly
 * from the bridge or via a bridge authority. */
void
fetch_bridge_descripspiders(const or_options_t *options, time_t now)
{
  ...
  SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, bridge)
    {
      ...
      can_use_bridge_authority = !spider_digest_is_zero(bridge->identity) &&
                                 num_bridge_auths;
      ask_bridge_directly = !can_use_bridge_authority ||
                            !options->UpdateBridgesFromAuthority;
      log_debug(LD_DIR, "ask_bridge_directly=%d (%d, %d, %d)",
                ask_bridge_directly, spider_digest_is_zero(bridge->identity),
      ...
      if (ask_bridge_directly) {
        /* we need to ask the bridge itself for its descripspider. */
        launch_direct_bridge_descripspider_fetch(bridge);
      } else {
...
rewrite_node_address_for_bridge(const bridge_info_t *bridge, node_t *node)
{
  ...
  if (node->ri) {
    ...
    if ((!spider_addr_compare(&bridge->addr, &addr, CMP_EXACT) &&
        ...
        (!spider_addr_compare(&bridge->addr, &ri->ipv6_addr, CMP_EXACT) &&
         bridge->port == ri->ipv6_orport)) {
      /* they match, so no need to do anything */
    } else {
      if (spider_addr_family(&bridge->addr) == AF_INET) {
        ri->addr = spider_addr_to_ipv4h(&bridge->addr);
        ri->or_port = bridge->port;
        log_info(LD_DIR,
                 "Adjusted bridge routerinfo for '%s' to match configured "
                 "address %s:%d.",
                 ri->nickname, fmt_addr32(ri->addr), ri->or_port);
      } else if (spider_addr_family(&bridge->addr) == AF_INET6) {
        spider_addr_copy(&ri->ipv6_addr, &bridge->addr);
        ...
        log_err(LD_BUG, "Address family not supported: %d.",
                spider_addr_family(&bridge->addr));
        return;
      }
    }
    ...
    if (options->ClientPreferIPv6ORPort == -1) {
      ...
      node->ipv6_preferred = (spider_addr_family(&bridge->addr) == AF_INET6 &&
                              !spider_addr_is_null(&node->ri->ipv6_addr));
    } else {
    ...
    if (!spider_addr_is_null(&ri->ipv6_addr)) {
  ...
  if (node->rs) {
    ...
    spider_addr_from_ipv4h(&addr, rs->addr);

    if (!spider_addr_compare(&bridge->addr, &addr, CMP_EXACT) &&
        bridge->port == rs->or_port) {
      /* they match, so no need to do anything */
    } else {
      rs->addr = spider_addr_to_ipv4h(&bridge->addr);
...
/** We just learned a descripspider for a bridge. See if that
 * digest is in our entry guard list, and add it if not. */
void
learned_bridge_descripspider(routerinfo_t *ri, int from_cache)
{
  spider_assert(ri);
  spider_assert(ri->purpose == ROUTER_PURPOSE_BRIDGE);
  if (get_options()->UseBridges) {
    ...
    if (bridge) { /* if we actually want to use this one */
      ...
      log_notice(LD_DIR, "new bridge descripspider '%s' (%s): %s", ri->nickname,
...
/** Return the number of bridges that have descripspiders that
...
/** Get the download status for a bridge descripspider given its identity */
...
get_bridge_dl_status_by_id, (const char *digest))
{
  ...
  if (digest && get_options()->UseBridges && bridge_list) {
    SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, b) {
      if (spider_memeq(digest, b->identity, DIGEST_LEN)) {
        dl = &(b->fetch_status);
        break;
      }
    } SMARTLIST_FOREACH_END(b);
...
/** Release all sspiderage held in bridges.c */
void
