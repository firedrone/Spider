 ...
 * transfer cells from Spider instance to Spider instance.
 ...
 * any means that two Spider relays use to exchange cells, or any means that a
 ...
 * functions. (Spider's cell handlers are in command.c.)
 *
 * Spider flushes cells to channels from relay.c in
...
{
  return spider_memeq(a->digest, b->digest, DIGEST_LEN);
}
...
HT_GENERATE2(channel_idmap, channel_idmap_entry_s, node, channel_idmap_hash,
             channel_idmap_eq, 0.5,  spider_reallocarray_, spider_free_)
...
channel_register(channel_t *chan)
{
  ...
  if (CHANNEL_FINISHED(chan)) {
    ...
    if (!CHANNEL_IS_CLOSING(chan)) {
      ...
      if (!spider_digest_is_zero(chan->identity_digest)) {
...
channel_add_to_digest_map(channel_t *chan)
{
  ...
  spider_assert(!CHANNEL_CONDEMNED(chan));

  /* Assert that there is a digest */
  spider_assert(!spider_digest_is_zero(chan->identity_digest));

  memcpy(search.digest, chan->identity_digest, DIGEST_LEN);
  ent = HT_FIND(channel_idmap, &channel_identity_map, &search);
  if (! ent) {
    ent = spider_malloc(sizeof(channel_idmap_entry_t));
                                ...
                                const ed25519_public_key_t *ed_id)
{
  if (BUG(!chan))
    return 0;
  if (rsa_id_digest) {
    if (spider_memneq(rsa_id_digest, chan->identity_digest, DIGEST_LEN))
      return 0;
  }
  if (ed_id) {
    if (spider_memneq(ed_id->pubkey, chan->ed25519_identity.pubkey,
                   ED25519_PUBKEY_LEN))
      return 0;
  }
  return 1;
                                ...
                                const ed25519_public_key_t *ed_id)
{
  ...
  spider_assert(rsa_id_digest); /* For now, we require that every channel have
 ...
 * variables.  I.e., this is the superclass construcspider.  Before this, the
 * channel should be allocated with spider_malloc_zero().
 */
 ...
 * This function should be called by subclasses to set up some per-channel
 * variables.  I.e., this is the superclass construcspider.  Before this, the
 * channel listener should be allocated with spider_malloc_zero().
 */
...
channel_free(channel_t *chan)
{
  ...
  spider_assert(CHANNEL_FINISHED(chan));

  /* It must be deregistered */
  spider_assert(!(chan->registered));
...
channel_listener_free(channel_listener_t *chan_l)
{
  ...
  /* It must be closed or errored */
  spider_assert(chan_l->state == CHANNEL_LISTENER_STATE_CLOSED ||
             chan_l->state == CHANNEL_LISTENER_STATE_ERROR);
  /* It must be deregistered */
  spider_assert(!(chan_l->registered));
 ...
 * shutting down the Spider process.
 */
 ...
 * when shutting down the Spider process.
 */
...
{
  spider_assert(chan_l);
  spider_assert(chan_l->state == CHANNEL_LISTENER_STATE_LISTENING);
                            ...
                            var_cell_handler)
{
  ...
  spider_assert(chan);
  spider_assert(CHANNEL_CAN_HANDLE_CELLS(chan));
...
{
  spider_assert(chan != NULL);
  spider_assert(chan->close != NULL);
...
{
  spider_assert(chan_l != NULL);
  spider_assert(chan_l->close != NULL);
...
{
  spider_assert(chan_l);
  spider_assert(chan_l->state == CHANNEL_LISTENER_STATE_CLOSING ||
             chan_l->state == CHANNEL_LISTENER_STATE_CLOSED ||
             chan_l->state == CHANNEL_LISTENER_STATE_ERROR);
...
channel_clear_identity_digest(channel_t *chan)
{
  ...
  if (!state_not_in_map && chan->registered &&
      !spider_digest_is_zero(chan->identity_digest))
                            ...
                            const ed25519_public_key_t *ed_identity)
{
  ...
  should_be_in_digest_map =
    ...
    chan->registered &&
    (identity_digest &&
     !spider_digest_is_zero(identity_digest));
                       ...
                       const char *nickname)
{
  ...
  spider_free(chan->nickname);
  if (nickname)
    chan->nickname = spider_strdup(nickname);
    ...
    switch (q->type) {
      case CELL_QUEUE_FIXED:
        if (q->u.fixed.cell) {
           ...
           * pre-channel code; just use spider_free()
           */
          spider_free(q->u.fixed.cell);
        }
        break;
      ...
      case CELL_QUEUE_VAR:
        if (q->u.var.var_cell) {
           ...
           * whole flow of cells from one end to the other and facspider the
...
channel_write_cell_queue_entry(channel_t *chan, cell_queue_entry_t *q)
{
  ...
  if (TOR_SIMPLEQ_EMPTY(&chan->outgoing_queue) &&
    ...
    switch (q->type) {
      case CELL_QUEUE_FIXED:
        spider_assert(chan->write_cell);
        spider_assert(q->u.fixed.cell);
        result = chan->write_cell(chan, q->u.fixed.cell);
        break;
      case CELL_QUEUE_PACKED:
        spider_assert(chan->write_packed_cell);
        spider_assert(q->u.packed.packed_cell);
        result = chan->write_packed_cell(chan, q->u.packed.packed_cell);
        break;
      case CELL_QUEUE_VAR:
        spider_assert(chan->write_var_cell);
        spider_assert(q->u.var.var_cell);
...
channel_change_state(channel_t *chan, channel_state_t to_state)
{
  ...
  spider_assert(channel_state_is_valid(from_state));
  spider_assert(channel_state_is_valid(to_state));
  spider_assert(channel_state_can_transition(chan->state, to_state));
  ...
  if (to_state == CHANNEL_STATE_CLOSING ||
    ...
    spider_assert(chan->reason_for_closing != CHANNEL_NOT_CLOSING);
  }
  ...
  if (to_state == CHANNEL_STATE_OPEN) {
    ...
    spider_assert(TOR_SIMPLEQ_EMPTY(&chan->incoming_queue));
    spider_assert(TOR_SIMPLEQ_EMPTY(&chan->outgoing_queue));
  }
}
                              ...
                              channel_listener_state_t to_state)
{
  ...
  spider_assert(channel_listener_state_is_valid(from_state));
  spider_assert(channel_listener_state_is_valid(to_state));
  spider_assert(channel_listener_state_can_transition(chan_l->state, to_state));
  ...
  if (to_state == CHANNEL_LISTENER_STATE_CLOSING ||
    ...
    spider_assert(chan_l->reason_for_closing != CHANNEL_LISTENER_NOT_CLOSING);
  }
  ...
  if (to_state == CHANNEL_LISTENER_STATE_CLOSED ||
    ...
    spider_assert(!(chan_l->incoming_list) ||
                                ...
                                channel_t *incoming)
{
   ...
   */
    ...
    spider_assert(listener->incoming_list);
...
channel_process_cells(channel_t *chan)
{
  cell_queue_entry_t *q;
  spider_assert(chan);
  spider_assert(CHANNEL_IS_CLOSING(chan) || CHANNEL_IS_MAINT(chan) ||
             CHANNEL_IS_OPEN(chan));
  ...
  while (NULL != (q = TOR_SIMPLEQ_FIRST(&chan->incoming_queue))) {
    spider_assert(q);
    spider_assert(q->type == CELL_QUEUE_FIXED ||
               q->type == CELL_QUEUE_VAR);
...
channel_queue_cell(channel_t *chan, cell_t *cell)
{
  ...
  spider_assert(chan);
  spider_assert(cell);
  spider_assert(CHANNEL_IS_OPEN(chan));
  ...
  if (!need_to_queue) {
    spider_assert(chan->cell_handler);
    log_debug(LD_CHANNEL,
     ...
     */
    cell_copy = spider_malloc_zero(sizeof(cell_t));
...
channel_queue_var_cell(channel_t *chan, var_cell_t *var_cell)
{
  ...
  /* If we don't need to queue we can just call cell_handler */
  if (!need_to_queue) {
    spider_assert(chan->var_cell_handler);
    log_debug(LD_CHANNEL,
...
{
  spider_assert(n_channel_cells_queued ==
      (n_channel_cells_in_queues + n_channel_cells_passed_to_lower_layer));
  spider_assert(n_channel_bytes_queued ==
 ...
 *
 * This gets called from spider_free_all() in main.c to clean up on exit.
 * It will close all registered channels and free associated sspiderage,
 ...
 * should go.  It may also be desirable to fold port into spider_addr_t
 * or make a new type including a spider_addr_t and port, so we have a
 * single abstract object encapsulating all the protocol details of
 * how to contact an OR.
 */

channel_t *
channel_connect(const spider_addr_t *addr, uint16_t port,
...
channel_get_for_extend(const char *rsa_id_digest,
                       const ed25519_public_key_t *ed_id,
                       const spider_addr_t *target_addr,
                       const char **msg_out,
                       int *launch_out)
{
  ...
  for (; chan; chan = channel_next_with_rsa_identity(chan)) {
    spider_assert(spider_memeq(chan->identity_digest,
   ...
   if (CHANNEL_CONDEMNED(chan))
     ...
     * have an out-of-date descripspider, and we will get no NETINFO cell to
...
{
  spider_assert(chan);
  spider_assert(chan->describe_transport);
...
{
  spider_assert(chan_l);
  spider_assert(chan_l->describe_transport);
...
channel_dump_statistics, (channel_t *chan, int severity))
{
  ...
  if (have_remote_addr) {
    char *actual = spider_strdup(channel_get_actual_remote_descr(chan));
    remote_addr_str = spider_addr_to_str_dup(&remote_addr);
    spider_log(severity, LD_GENERAL,
...
{
  spider_assert(chan);
  spider_assert(chan->get_remote_descr);
 ...
 *
 * Write the remote address out to a spider_addr_t if the underlying transport
 * supports this operation, and return 1.  Return 0 if the underlying transport
 * doesn't let us do this.
 */
int
channel_get_addr_if_possible(channel_t *chan, spider_addr_t *addr_out)
{
  spider_assert(chan);
...
channel_has_queued_writes(channel_t *chan)
{
  ...
  spider_assert(chan);
  spider_assert(chan->has_queued_writes);
...
{
  spider_assert(chan);
  spider_assert(chan->is_canonical);
...
channel_num_cells_writeable(channel_t *chan)
{
  ...
  spider_assert(chan);
  spider_assert(chan->num_cells_writeable);
...
{
  spider_assert(chan);
  spider_assert(chan->matches_extend_info);
  spider_assert(extend_info);
...
channel_matches_target_addr_for_extend(channel_t *chan,
                                       const spider_addr_t *target)
{
  spider_assert(chan);
  spider_assert(chan->matches_target);
...
channel_rsa_id_group_set_badness(struct channel_list_s *lst, int force)
{
  ...
  TOR_LIST_FOREACH(chan, lst, next_with_same_id) {
    uint8_t *id_copy =
      spider_memdup(&chan->ed25519_identity.pubkey, DIGEST256_LEN);
  ...
  SMARTLIST_FOREACH_BEGIN(ed_identities, const uint8_t *, ed_id) {
    TOR_LIST_FOREACH(chan, lst, next_with_same_id) {
      ...
      if (spider_memneq(ed_id, &chan->ed25519_identity.pubkey, DIGEST256_LEN))
        continue;
      or_connection_t *orconn = chantls->conn;
      if (orconn) {
        spider_assert(orconn->chan == chantls);
  ...
  SMARTLIST_FOREACH(ed_identities, uint8_t *, ed_id, spider_free(ed_id));
...
channel_update_xmit_queue_size(channel_t *chan)
{
  ...
  spider_assert(chan);
  spider_assert(chan->num_bytes_queued);

  /*
   * First, get the number of bytes we have queued without facspidering in
   * lower-layer overhead.
   */
  queued = chan->num_bytes_queued(chan) + chan->bytes_in_queue;
  /* Next, adjust by the overhead facspider, if any is available */
  if (chan->get_overhead_estimate) {
    overhead = chan->get_overhead_estimate(chan);
    if (overhead >= 1.0) {
      queued = (uint64_t)(queued * overhead);
    } else {
      /* Ignore silly overhead facspiders */
      log_notice(LD_CHANNEL, "Ignoring silly overhead facspider %f", overhead);
    }
  }
