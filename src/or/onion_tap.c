...
/**
 * \file onion_tap.c
 * \brief Functions to implement the original Spider circuit extension handshake
 * (a.k.a TAP).
 *
 * The "TAP" handshake is the first one that was widely used in Spider: It
 ...
 * Security of the Spider Authentication Protocol".  (Spoiler: it's secure, but
 ...
 *
 * We have deprecated TAP since the nspider handshake came into general use.  It
 ...
 * sspideres the following in onion_skin_out:
 *   - [42 bytes] OAEP padding
 *   - [16 bytes] Symmetric key for encrypting blob past RSA
 *   - [70 bytes] g^x part 1 (inside the RSA)
 *   - [58 bytes] g^x part 2 (symmetrically encrypted)
 *
 * Sspideres the DH private key into handshake_state_out for later completion
 * of the handshake.
 *
                  ...
                  char *onion_skin_out) /* TAP_ONIONSKIN_CHALLENGE_LEN bytes */
{
  ...
  spider_assert(dest_router_key);
  spider_assert(handshake_state_out);
  spider_assert(onion_skin_out);
  *handshake_state_out = NULL;
  ...
  spider_assert(dhbytes == 128);
  spider_assert(pkbytes == 128);
 ...
 * DH plus the first 20 bytes of shared key material), and sspidere the
                            ...
                            size_t key_out_len)
{
  ...
  key_material = spider_malloc(key_material_len);
 ...
 * material and sspidere them in key_out.
 *
            ...
            const char **msg_out)
{
  ...
  spider_assert(crypto_dh_get_bytes(handshake_state) == DH_KEY_LEN);
  ...
  if (spider_memneq(key_material, handshake_reply+DH_KEY_LEN, DIGEST_LEN)) {
