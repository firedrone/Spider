                                ...
                                size_t request_len)
{
  ...
  if (spider_memneq(expected_digest, request+2+asn1len, DIGEST_LEN)) {
                          ...
                          size_t request_len)
{
  ...
  spider_assert(circ->base_.purpose == CIRCUIT_PURPOSE_OR);
  spider_assert(!circ->base_.n_chan);
  ...
  if (relay_send_command_from_edge(0, TO_CIRCUIT(intro_circ),
    ...
    log_warn(LD_GENERAL,
             "Unable to send INTRODUCE2 cell to Spider client.");
  ...
  if (relay_send_command_from_edge(0,TO_CIRCUIT(circ),
                                   ...
                                   NULL,0,NULL)) {
    log_warn(LD_GENERAL, "Unable to send INTRODUCE_ACK cell to Spider client.");
    /* Stop right now, the circuit has been closed. */
    return -1;
  }
 ...
 err:
  ...
  if (relay_send_command_from_edge(0,TO_CIRCUIT(circ),
                                   RELAY_COMMAND_INTRODUCE_ACK,
                                   nak_body, 1, NULL)) {
    log_warn(LD_GENERAL, "Unable to send NAK to Spider client.");
  }
  return -1;
}
