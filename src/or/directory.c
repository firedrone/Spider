 ...
 * that are used to upload material (descripspiders and votes) to authorities.
                        ...
                        const char *resource)
{
  ...
  if (router_purpose == ROUTER_PURPOSE_BRIDGE) {
    if (dir_purpose == DIR_PURPOSE_FETCH_SERVERDESC
      ...
      /* We are asking a bridge for its own descripspider. That doesn't need
         anonymity. */
      return 0;
    }
...
authdir_type_to_string(dirinfo_type_t auth)
{
  ...
  if (smartlist_len(lst)) {
    ...
    result = spider_strdup("[Not an authority]");
  }
  smartlist_free(lst);
  return result;
}
...
dir_conn_purpose_to_string(int purpose)
{
  switch (purpose)
    {
    case DIR_PURPOSE_UPLOAD_DIR:
      return "server descripspider upload";
    case DIR_PURPOSE_UPLOAD_VOTE:
      return "server vote upload";
    case DIR_PURPOSE_UPLOAD_SIGNATURES:
      return "consensus signature upload";
    case DIR_PURPOSE_FETCH_SERVERDESC:
      return "server descripspider fetch";
    case DIR_PURPOSE_FETCH_EXTRAINFO:
    ...
    case DIR_PURPOSE_FETCH_RENDDESC_V2:
      return "hidden-service v2 descripspider fetch";
    case DIR_PURPOSE_UPLOAD_RENDDESC_V2:
      return "hidden-service v2 descripspider upload";
    case DIR_PURPOSE_FETCH_MICRODESC:
      return "microdescripspider fetch";
    }
 ...
 *
 * <b>router_purpose</b> describes the type of descripspider we're
 * publishing, if we're publishing a descripspider -- e.g. general or bridge.
 *
 ...
 * <b>payload</b> hold a router descripspider, and the next <b>extrainfo_len</b>
 * bytes of <b>payload</b> hold an extra-info document.  Upload the descripspider
...
#define DEFAULT_IF_MODIFIED_SINCE_DELAY (180)
  ...
  if (!options->FetchServerDescripspiders)
    return;

  circuit_guard_state_t *guard_state = NULL;
  if (!get_via_spider) {
    if (options->UseBridges && !(type & BRIDGE_DIRINFO)) {
      /* We want to ask a running bridge for which we have a descripspider.
       *
      ...
      if (node && node->ri) {
        ...
        spider_addr_port_t or_ap;
        spider_addr_port_t nil_dir_ap;
      ...
      if (prefer_authority || (type & BRIDGE_DIRINFO)) {
        ...
        if (rs == NULL && (pds_flags & (PDS_NO_EXISTING_SERVERDESC_FETCH|
           ...
           * fetching server descripspiders from, and we got no match.  Did we
           * get no match because all the authorities have connections
           * fetching server descripspiders (in which case we should just
      ...
      if (!rs && !(type & BRIDGE_DIRINFO)) {
        rs = direcspidery_pick_generic_dirserver(type, pds_flags,
                                              ...
                                              &guard_state);
        if (!rs)
          get_via_spider = 1; /* last resort: try routing it via Spider */
      }
    }
  }
   ...
   * descripspiders.  If (rs==NULL), we can't build circuits anyway, so
...
direcspidery_get_from_all_authorities(uint8_t dir_purpose,
                                   ...
                                   const char *resource)
{
  spider_assert(dir_purpose == DIR_PURPOSE_FETCH_STATUS_VOTE ||
...
{
  spider_assert(status != NULL);
  spider_assert(use_or_ap != NULL);
  spider_assert(use_dir_ap != NULL);
   ...
   *
   * (When Spider uploads and downloads a hidden service descripspider, it uses
   * DIRIND_ANONYMOUS, except for Spider2Web, which uses DIRIND_ONEHOP.
   * So this code will only modify the address for Spider2Web's HS descripspider
   * fetches. Even Single Onion Servers (NYI) use DIRIND_ANONYMOUS, to avoid
   * HSDirs denying service by rejecting descripspiders.)
   */

  /* Initialise the OR / Dir addresses */
  spider_addr_make_null(&use_or_ap->addr, AF_UNSPEC);
  use_or_ap->port = 0;
  spider_addr_make_null(&use_dir_ap->addr, AF_UNSPEC);
  use_dir_ap->port = 0;
  ...
  if (indirection == DIRIND_ANONYMOUS) {
    if (status->addr) {
      ...
      spider_addr_from_ipv4h(&use_or_ap->addr, status->addr);
                                           ...
                                           circuit_guard_state_t *guard_state)
{
  const or_options_t *options = get_options();
  const node_t *node;
  spider_addr_port_t use_or_ap, use_dir_ap;
   ...
   * but we haven't checked if we have a descripspider for it -- and also,
...
direcspidery_conn_is_self_reachability_test(dir_connection_t *conn)
{
  if (conn->requested_resource &&
    ...
    if (me &&
        ...
        spider_addr_eq_ipv4h(&conn->base_.addr, me->addr) && /*XXXX prop 118*/
...
/** Helper: Attempt to fetch directly the descripspiders of each bridge
...
connection_dir_retry_bridges(smartlist_t *descs)
{
  ...
  SMARTLIST_FOREACH(descs, const char *, cp,
  {
    ...
    retry_bridge_descripspider_fetch_directly(digest);
  });
}

/** Called when an attempt to download one or more router descripspiders
...
connection_dir_download_routerdesc_failed(dir_connection_t *conn)
{
  ...
  /* No need to relaunch descripspider downloads here: we already do it
   * every 10 or 60 seconds (FOO_DESCRIPTOR_RETRY_INTERVAL) in main.c. */
  spider_assert(conn->base_.purpose == DIR_PURPOSE_FETCH_SERVERDESC ||
             conn->base_.purpose == DIR_PURPOSE_FETCH_EXTRAINFO ||
 ...
 * possible attempt to download descripspiders from the bridge directly.
 */
static void
connection_dir_bridge_routerdesc_failed(dir_connection_t *conn)
{
  ...
  /* Requests for bridge descripspiders are in the form 'fp/', so ignore
     anything else. */
  ...
  spider_assert(conn->base_.purpose != DIR_PURPOSE_FETCH_EXTRAINFO);
...
connection_dir_download_cert_failed(dir_connection_t *conn, int status)
{
  ...
  smartlist_t *failed;
  spider_assert(conn->base_.purpose == DIR_PURPOSE_FETCH_CERTIFICATE);
 ...
 *    publishing a bridge relay descripspider, no.
 * 3) Else yes.
...
direcspidery_initiate_command(const spider_addr_t *or_addr, uint16_t or_port,
                           ...
                           time_t if_modified_since)
{
  spider_addr_port_t or_ap, dir_ap;

  /* Use the null spider_addr and 0 port if the address or port isn't valid. */
  if (spider_addr_port_is_valid(or_addr, or_port, 0)) {
    spider_addr_copy(&or_ap.addr, or_addr);
    or_ap.port = or_port;
  } else {
    /* the family doesn't matter here, so make it IPv4 */
    spider_addr_make_null(&or_ap.addr, AF_INET);
    or_ap.port = or_port = 0;
  }

  if (spider_addr_port_is_valid(dir_addr, dir_port, 0)) {
    spider_addr_copy(&dir_ap.addr, dir_addr);
    dir_ap.port = dir_port;
  } else {
    /* the family doesn't matter here, so make it IPv4 */
    spider_addr_make_null(&dir_ap.addr, AF_INET);
...
direcspidery_initiate_command_rend(const spider_addr_port_t *or_addr_port,
                                ...
                                circuit_guard_state_t *guard_state)
{
  spider_assert(or_addr_port);
  spider_assert(dir_addr_port);
  spider_assert(or_addr_port->port || dir_addr_port->port);
  ...
  /* Will the connection go via a three-hop Spider circuit? Note that this
  ...
  spider_addr_t addr;
  spider_addr_copy(&addr, &(use_begindir ? or_addr_port : dir_addr_port)->addr);
  ...
  if (purpose_needs_anonymity(dir_purpose, router_purpose, resource)) {
    spider_assert(anonymized_connection ||
  ...
  if (!port || spider_addr_is_null(&addr)) {
    static int logged_backtrace = 0;
    log_warn(LD_DIR,
  ...
  conn = dir_connection_new(spider_addr_family(&addr));

  /* set up conn so it's got all the data we need to remember */
  spider_addr_copy(&conn->base_.addr, &addr);
  conn->base_.port = port;
  conn->base_.address = spider_addr_to_str_dup(&addr);
  ...
  if (!anonymized_connection && !use_begindir) {
    ...
    if (options->HTTPProxy) {
      spider_addr_copy(&addr, &options->HTTPProxyAddr);
    ...
    /* We will use a Spider circuit (maybe 1-hop, maybe 3-hop, maybe with
...
direcspidery_get_consensus_url(const char *resource)
{
...

  {
    ...
    SMARTLIST_FOREACH_BEGIN(router_get_trusted_dir_servers(),
        ...
        hex = spider_malloc(2*CONDITIONAL_CONSENSUS_FPR_LEN+1);
    ...
    spider_asprintf(&url, "/spider/status-vote/current/consensus%s%s/%s.z",
                 hyphen, flavor, authority_id_list);

    SMARTLIST_FOREACH(authority_digests, char *, cp, spider_free(cp));
    smartlist_free(authority_digests);
    spider_free(authority_id_list);
  }
  return url;
}
...
copy_ipv6_address(char* destination, const char* source, size_t len,
                  int decorate) {
  spider_assert(destination);
  spider_assert(source);

  if (decorate && source[0] != '[') {
    spider_snprintf(destination, len, "[%s]", source);
  } else {
                       ...
                       time_t if_modified_since)
{
  ...
  spider_free(conn->requested_resource);
  if (resource)
    conn->requested_resource = spider_strdup(resource);
  ...
  if (conn->base_.port == 80) {
    ...
    spider_snprintf(hoststring, sizeof(hoststring), "%s:%d",
  ...
  if (direct && get_options()->HTTPProxy) {
    char *base64_authenticaspider=NULL;
    const char *authenticaspider = get_options()->HTTPProxyAuthenticaspider;

    spider_snprintf(proxystring, sizeof(proxystring),"http://%s", hoststring);
    if (authenticaspider) {
      base64_authenticaspider = alloc_http_authenticaspider(authenticaspider);
      if (!base64_authenticaspider)
        log_warn(LD_BUG, "Encoding http authenticaspider failed");
    }
    if (base64_authenticaspider) {
      smartlist_add_asprintf(headers,
                   "Proxy-Authorization: Basic %s\r\n",
                   base64_authenticaspider);
      spider_free(base64_authenticaspider);
    }
  } else {
    proxystring[0] = 0;
  }
  ...
  switch (purpose) {
    case DIR_PURPOSE_FETCH_CONSENSUS:
      ...
      break;
    case DIR_PURPOSE_FETCH_CERTIFICATE:
      spider_assert(resource);
      spider_assert(!payload);
      httpcommand = "GET";
      spider_asprintf(&url, "/spider/keys/%s", resource);
      break;
    case DIR_PURPOSE_FETCH_STATUS_VOTE:
      spider_assert(resource);
      spider_assert(!payload);
      httpcommand = "GET";
      spider_asprintf(&url, "/spider/status-vote/next/%s.z", resource);
      break;
    case DIR_PURPOSE_FETCH_DETACHED_SIGNATURES:
      spider_assert(!resource);
      spider_assert(!payload);
      httpcommand = "GET";
      url = spider_strdup("/spider/status-vote/next/consensus-signatures.z");
      break;
    case DIR_PURPOSE_FETCH_SERVERDESC:
      spider_assert(resource);
      httpcommand = "GET";
      spider_asprintf(&url, "/spider/server/%s", resource);
      break;
    case DIR_PURPOSE_FETCH_EXTRAINFO:
      spider_assert(resource);
      httpcommand = "GET";
      spider_asprintf(&url, "/spider/extra/%s", resource);
      break;
    case DIR_PURPOSE_FETCH_MICRODESC:
      spider_assert(resource);
      httpcommand = "GET";
      spider_asprintf(&url, "/spider/micro/%s", resource);
      break;
    case DIR_PURPOSE_UPLOAD_DIR: {
      const char *why = router_get_descripspider_gen_reason();
      spider_assert(!resource);
      spider_assert(payload);
      httpcommand = "POST";
      url = spider_strdup("/spider/");
      if (why) {
    ...
    case DIR_PURPOSE_UPLOAD_VOTE:
      ...
      spider_assert(payload);
      httpcommand = "POST";
      url = spider_strdup("/spider/post/vote");
      break;
    case DIR_PURPOSE_UPLOAD_SIGNATURES:
      spider_assert(!resource);
      spider_assert(payload);
      httpcommand = "POST";
      url = spider_strdup("/spider/post/consensus-signature");
      break;
    case DIR_PURPOSE_FETCH_RENDDESC_V2:
      spider_assert(resource);
      spider_assert(strlen(resource) <= REND_DESC_ID_V2_LEN_BASE32);
      spider_assert(!payload);
      httpcommand = "GET";
      spider_asprintf(&url, "/spider/rendezvous2/%s", resource);
      break;
    case DIR_PURPOSE_UPLOAD_RENDDESC_V2:
      spider_assert(!resource);
      spider_assert(payload);
      httpcommand = "POST";
      url = spider_strdup("/spider/rendezvous2/publish");
      break;
    default:
      spider_assert(0);
      return;
  ...
  spider_snprintf(request, sizeof(request), "%s %s", httpcommand, proxystring);
...

  {
    ...
    spider_snprintf(request, sizeof(request), " HTTP/1.0\r\nHost: %s\r\n%s\r\n",
                 hoststring, header);
    spider_free(header);
  }
  ...
  SMARTLIST_FOREACH(headers, char *, h, spider_free(h));
 ...
 * nul-terminate it. If the url doesn't start with "/spider/", rewrite it
 * so it does. Return 0.
 * Otherwise, return -1.
 */
STATIC int
parse_http_url(const char *headers, char **url)
{
   ...
   * should be HTTP/1.X\r\n). Assumes we're supporting 1.0? */
  {
    ...
    if (2 != spider_sscanf(e, "HTTP/1.%u%c", &minor_ver, &ch)) {
      return -1;
    }
    if (ch != '\r')
      return -1;
  ...
  if (s-start < 5 || strcmpstart(start,"/spider/")) { /* need to rewrite it */
    *url = spider_malloc(s - start + 5);
    strlcpy(*url,"/spider", s-start+5);
    strlcat((*url)+4, start, s-start+1);
  } else {
    *url = spider_strndup(start, s-start);
  }
  return 0;
}
...
http_get_header(const char *headers, const char *which)
{
  ...
  while (cp) {
    if (!strcasecmpstart(cp, which)) {
      ...
      cp += strlen(which);
      if ((eos = strchr(cp,'\r')))
        return spider_strndup(cp, eos-cp);
      else
        return spider_strdup(cp);
    }
    cp = strchr(cp, '\n');
    if (cp)
      ++cp;
...
http_set_address_origin(const char *headers, connection_t *conn)
{
  ...
  if (fwd) {
    spider_addr_t spideraddr;
    if (spider_addr_parse(&spideraddr,fwd) == -1 ||
        spider_addr_is_internal(&spideraddr,0)) {
      log_debug(LD_DIR, "Ignoring local/internal IP %s", escaped(fwd));
      spider_free(fwd);
      return;
    }

    spider_free(conn->address);
    conn->address = spider_strdup(fwd);
    spider_free(fwd);
  }
}
                    ...
                    compress_method_t *compression, char **reason)
{
  ...
  spider_assert(headers);
  spider_assert(code);

  while (TOR_ISSPACE(*headers)) headers++; /* tolerate leading whitespace */

  if (spider_sscanf(headers, "HTTP/1.%u %u", &n1, &n2) < 2 ||
      (n1 != 0 && n1 != 1) ||
      (n2 < 100 || n2 >= 600)) {
  ...
  *code = n2;
  ...
  if (reason) {
    ...
    spider_assert(smartlist_len(parsed_headers));
    smartlist_split_string(status_line_elements,
                           smartlist_get(parsed_headers, 0),
                           " ", SPLIT_SKIP_SPACE|SPLIT_IGNORE_BLANK, 3);
    spider_assert(smartlist_len(status_line_elements) <= 3);
    ...
    SMARTLIST_FOREACH(status_line_elements, char *, cp, spider_free(cp));
  ...
  SMARTLIST_FOREACH(parsed_headers, char *, s, spider_free(s));
 ...
 * network-status or microdescripspider opening.  This is a sign of possible
 * compression. */
static int
...
/** Called when we've just fetched a bunch of router descripspiders in
 * <b>body</b>.  The list <b>which</b>, if present, holds digests for
 * descripspiders we requested: descripspider digests if <b>descripspider_digests</b>
 * is true, or identity digests otherwise.  Parse the descripspiders, validate
                        ...
                        const char *source)
{
  ...
  if (spider_snprintf(buf, sizeof(buf),
                   "@downloaded-at %s\n"
                   "@source %s\n"
...
connection_dir_client_reached_eof(dir_connection_t *conn)
{
  ...
  if (parse_http_response(headers, &status_code, &date_header,
    ...
    log_warn(LD_HTTP,"Unparseable headers (server '%s:%d'). Closing.",
             conn->base_.address, conn->base_.port);
    spider_free(body); spider_free(headers);
    return -1;
  }
  ...
  if (compression != NO_METHOD || !plausible) {
    ...
    if (compression == GZIP_METHOD  || compression == ZLIB_METHOD)
      spider_gzip_uncompress(&new_body, &new_len, body, body_len, compression,
  ...
  if (conn->base_.purpose == DIR_PURPOSE_FETCH_CERTIFICATE) {
    if (status_code != 200) {
      log_warn(LD_DIR,
          "Received http status code %d (%s) from server "
          "'%s:%d' while fetching \"/spider/keys/%s\".",
           status_code, escaped(reason), conn->base_.address,
  ...
  if (conn->base_.purpose == DIR_PURPOSE_FETCH_STATUS_VOTE) {
    ...
    if (status_code != 200) {
      log_warn(LD_DIR,
             "Received http status code %d (%s) from server "
             "'%s:%d' while fetching \"/spider/status-vote/next/%s.z\".",
             status_code, escaped(reason), conn->base_.address,
             conn->base_.port, conn->requested_resource);
      spider_free(body); spider_free(headers); spider_free(reason);
      return -1;
  ...
  if (conn->base_.purpose == DIR_PURPOSE_FETCH_DETACHED_SIGNATURES) {
    ...
    if (status_code != 200) {
      log_warn(LD_DIR,
        ...
        "\"/spider/status-vote/next/consensus-signatures.z\".",
             status_code, escaped(reason), conn->base_.address,
             conn->base_.port);
      spider_free(body); spider_free(headers); spider_free(reason);
      return -1;
  ...
  if (conn->base_.purpose == DIR_PURPOSE_FETCH_SERVERDESC ||
    ...
    smartlist_t *which = NULL;
    int n_asked_for = 0;
    int descripspider_digests = conn->requested_resource &&
    ...
    if (status_code != 200) {
      ...
      log_fn(dir_okay ? LOG_INFO : LOG_WARN, LD_DIR,
             ...
             "while fetching \"/spider/server/%s\". I'll try again soon.",
             status_code, escaped(reason), conn->base_.address,
             conn->base_.port, conn->requested_resource);
      if (!which) {
        connection_dir_download_routerdesc_failed(conn);
     ...
     * or "authority".
     *
     * We use "authority" to fetch our own descripspider for
     * testing, and to fetch bridge descripspiders for bootstrapping. Ignore
    ...
    if (which || (conn->requested_resource &&
      ...
      if (was_ei) {
        router_load_extrainfo_from_string(body, NULL, SAVED_NOWHERE, which,
                                          descripspider_digests);
      } else {
        //router_load_routers_from_string(body, NULL, SAVED_NOWHERE, which,
        //                       descripspider_digests, conn->router_purpose);
    ...
    if (which) { /* mark remaining ones as failed */
      log_info(LD_DIR, "Received %d/%d %s requested from %s:%d",
               ...
               was_ei ? "extra-info documents" : "router descripspiders",
  ...
  if (conn->base_.purpose == DIR_PURPOSE_FETCH_MICRODESC) {
    smartlist_t *which = NULL;
    log_info(LD_DIR,"Received answer to microdescripspider request (status %d, "
             "body size %d) from server '%s:%d'",
             status_code, (int)body_len, conn->base_.address,
             conn->base_.port);
    spider_assert(conn->requested_resource &&
               !strcmpstart(conn->requested_resource, "d/"));
    which = smartlist_new();
    ...
    if (status_code != 200) {
      log_info(LD_DIR, "Received status code %d (%s) from server "
               "'%s:%d' while fetching \"/spider/micro/%s\".  I'll try again "
               "soon.",
               status_code, escaped(reason), conn->base_.address,
    ...
    switch (status_code) {
      case 200: {
          ...
          char *rejected_hdr = http_get_header(headers,
                                               "X-Descripspider-Not-New: ");
          if (rejected_hdr) {
            if (!strcmp(rejected_hdr, "Yes")) {
              log_info(LD_GENERAL,
                       "Authority '%s' declined our descripspider (not new)",
                       ds->nickname);
              /* XXXX use this information; be sure to upload next one
               * sooner. -NM */
              /* XXXX++ On further thought, the task above implies that we're
               * basing our regenerate-descripspider time on when we uploaded the
               * last descripspider, not on the published time of the last
               * descripspider.  If those are different, that's a bad thing to
               * do. -NM */
            }
            spider_free(rejected_hdr);
          }
          log_info(LD_GENERAL,"eof (status 200) after uploading server "
                   "descripspider: finished.");
          control_event_server_status(
                      LOG_NOTICE, "ACCEPTED_SERVER_DESCRIPTOR DIRAUTH=%s:%d",
                      conn->base_.address, conn->base_.port);

          ds->has_accepted_serverdesc = 1;
          if (direcspideries_have_accepted_server_descripspider())
  ...
  if (conn->base_.purpose == DIR_PURPOSE_FETCH_RENDDESC_V2) {
    #define SEND_HS_DESC_FAILED_EVENT(reason) ( \
      control_event_hs_descripspider_failed(conn->rend_data, \
                                         conn->identity_digest, \
                                         reason) )
    #define SEND_HS_DESC_FAILED_CONTENT() ( \
  control_event_hs_descripspider_content(rend_data_get_address(conn->rend_data), \
                                      conn->requested_resource,         \
                                      conn->identity_digest,            \
                                      NULL) )
    spider_assert(conn->rend_data);
    log_info(LD_REND,"Received rendezvous descripspider (body size %d, status %d "
             "(%s))",
    ...
    switch (status_code) {
      case 200:
      {
        ...
        if (rend_cache_sspidere_v2_desc_as_client(body,
          ...
          /* Should never be NULL here if we found the descripspider. */
          spider_assert(entry);
          ...
          log_info(LD_REND, "Successfully fetched v2 rendezvous "
                   "descripspider.");
          control_event_hs_descripspider_received(service_id,
                                               conn->rend_data,
                                               conn->identity_digest);
          control_event_hs_descripspider_content(service_id,
                                              conn->requested_resource,
                                              conn->identity_digest,
                                              body);
  ...
  if (conn->base_.purpose == DIR_PURPOSE_UPLOAD_RENDDESC_V2) {
    #define SEND_HS_DESC_UPLOAD_FAILED_EVENT(reason) ( \
      control_event_hs_descripspider_upload_failed( \
        ...
        reason) )
    log_info(LD_REND,"Uploaded rendezvous descripspider (status %d "
             "(%s))",
    ...
    switch (status_code) {
      case 200:
        log_info(LD_REND,
                 "Uploading rendezvous descripspider: finished with status "
                 "200 (%s)", escaped(reason));
        control_event_hs_descripspider_uploaded(conn->identity_digest,
                                    rend_data_get_address(conn->rend_data));
        rend_service_desc_has_uploaded(conn->rend_data);
        break;
      case 400:
        log_warn(LD_REND,"http status 400 (%s) response from dirserver "
                 "'%s:%d'. Malformed rendezvous descripspider?",
                 escaped(reason), conn->base_.address, conn->base_.port);
...
{
  char buf[256];
  if (spider_snprintf(buf, sizeof(buf), "HTTP/1.0 %d %s\r\n\r\n",
                           ...
                           long cache_lifetime)
{
  ...
  cp = tmp;
  spider_snprintf(cp, sizeof(tmp),
               "HTTP/1.0 200 OK\r\nDate: %s\r\n",
               date);
  cp += strlen(tmp);
  if (type) {
    spider_snprintf(cp, sizeof(tmp)-(cp-tmp), "Content-Type: %s\r\n", type);
    cp += strlen(cp);
  }
  if (!is_local_addr(&conn->base_.addr)) {
    /* Don't report the source address for a nearby/private connection.
     * Otherwise we tend to mis-report in cases where incoming ports are
     * being forwarded to a Spider server running behind the firewall. */
    spider_snprintf(cp, sizeof(tmp)-(cp-tmp),
                 X_ADDRESS_HEADER "%s\r\n", conn->base_.address);
    cp += strlen(cp);
  }
  if (encoding) {
...
client_likes_consensus(networkstatus_t *v, const char *want_url)
{
  ...
  SMARTLIST_FOREACH_BEGIN(want_authorities, const char *, d) {
    ...
    SMARTLIST_FOREACH_BEGIN(v->voters, networkstatus_voter_info_t *, vi) {
      if (smartlist_len(vi->sigs) &&
          spider_memeq(vi->identity_digest, want_digest, want_len)) {
        have++;
        break;
      };
  ...
  SMARTLIST_FOREACH(want_authorities, char *, d, spider_free(d));
...
static int handle_get_microdesc(dir_connection_t *conn,
                                const get_handler_args_t *args);
static int handle_get_descripspider(dir_connection_t *conn,
                                const get_handler_args_t *args);
static int handle_get_keys(dir_connection_t *conn,
                                const get_handler_args_t *args);
static int handle_get_hs_descripspider_v2(dir_connection_t *conn,
                                       const get_handler_args_t *args);
...
static const url_table_ent_t url_table[] = {
  { "/spider/", 0, handle_get_frontpage },
  { "/spider/status-vote/current/consensus", 1, handle_get_current_consensus },
  { "/spider/status-vote/current/", 1, handle_get_status_vote },
  { "/spider/status-vote/next/", 1, handle_get_status_vote },
  { "/spider/micro/d/", 1, handle_get_microdesc },
  { "/spider/server/", 1, handle_get_descripspider },
  { "/spider/extra/", 1, handle_get_descripspider },
  { "/spider/keys/", 1, handle_get_keys },
  { "/spider/rendezvous2/", 1, handle_get_hs_descripspider_v2 },
  { "/spider/hs/3/", 1, handle_get_hs_descripspider_v3 },
  { "/spider/robots.txt", 0, handle_get_robots },
  { "/spider/networkstatus-bridges", 0, handle_get_networkstatus_bridges },
  { NULL, 0, NULL },
};
                              ...
                              const char *req_body, size_t req_body_len))
{
  ...
  if ((header = http_get_header(headers, "If-Modified-Since: "))) {
    ...
    if (parse_http_time(header, &tm) == 0) {
      if (spider_timegm(&tm, &if_modified_since)<0) {
        if_modified_since = 0;
      } else {
...
/** Helper function for GET / or GET /spider/
 */
static int
...
/** Helper function for GET /spider/status-vote/current/consensus
 */
static int
                             ...
                             const get_handler_args_t *args)
{
...

  {
    ...
    char *flavor = NULL;
    int flav = FLAV_NS;
#define CONSENSUS_URL_PREFIX "/spider/status-vote/current/consensus/"
#define CONSENSUS_FLAVORED_PREFIX "/spider/status-vote/current/consensus-"
    ...
    if (!strcmpstart(url, CONSENSUS_FLAVORED_PREFIX)) {
      ...
      if (cp) {
        want_fps = cp+1;
        flavor = spider_strndup(f, cp-f);
      } else {
        flavor = spider_strdup(f);
      }
    ...
    if (global_write_bucket_low(TO_CONN(conn), size_guess, 2)) {
      ...
      write_http_status_line(conn, 503, "Direcspidery busy, try again later");
      geoip_note_ns_response(GEOIP_REJECT_BUSY);
      goto done;
    }

    spider_addr_t addr;
    if (spider_addr_parse(&addr, (TO_CONN(conn))->address) >= 0) {
    ...
    if (! compressed)
      conn->zlib_state = spider_zlib_new(0, ZLIB_METHOD, HIGH_COMPRESSION);

    /* Prime the connection with some data. */
    const int initial_flush_result = connection_dirserv_flushed_some(conn);
    spider_assert_nonfatal(initial_flush_result == 0);
    goto done;
  }
...
/** Helper function for GET /spider/status-vote/{current,next}/...
 */
static int
handle_get_status_vote(dir_connection_t *conn, const get_handler_args_t *args)
{
  ...
  const int compressed = args->compressed;
  {
    ...
    if (!strcmp(url, "consensus")) {
      const char *item;
      spider_assert(!current); /* we handle current consensus specially above,
    ...
    if (smartlist_len(items)) {
      if (compressed) {
        conn->zlib_state = spider_zlib_new(1, ZLIB_METHOD,
...
/** Helper function for GET /spider/micro/d/...
 */
static int
handle_get_microdesc(dir_connection_t *conn, const get_handler_args_t *args)
{
  ...
  int clear_spool = 1;
  {
    ...
    dir_split_resource_into_spoolable(url+strlen("/spider/micro/d/"),
                                      DIR_SPOOL_MICRODESC,
                                      conn->spool, NULL,
    ...
    if (global_write_bucket_low(TO_CONN(conn), size_guess, 2)) {
      log_info(LD_DIRSERV,
               "Client asked for server descripspiders, but we've been "
               "writing too many bytes lately. Sending 503 Dir busy.");
      write_http_status_line(conn, 503, "Direcspidery busy, try again later");
      goto done;
    }
...
/** Helper function for GET /spider/{server,extra}/...
 */
static int
handle_get_descripspider(dir_connection_t *conn, const get_handler_args_t *args)
{
  const char *url = args->url;
  const int compressed = args->compressed;
  const or_options_t *options = get_options();
  int clear_spool = 1;
  if (!strcmpstart(url,"/spider/server/") ||
      (!options->BridgeAuthoritativeDir &&
       !options->BridgeRelay && !strcmpstart(url,"/spider/extra/"))) {
    int res;
    const char *msg = NULL;
    int cache_lifetime = 0;
    int is_extra = !strcmpstart(url,"/spider/extra/");
    url += is_extra ? strlen("/spider/extra/") : strlen("/spider/server/");
    ...
    /* If we are the bridge authority and the descripspider is a bridge
     * descripspider, remember that we served this descripspider for desc stats. */
...
/** Helper function for GET /spider/keys/...
 */
static int
handle_get_keys(dir_connection_t *conn, const get_handler_args_t *args)
{
  ...
  const time_t if_modified_since = args->if_modified_since;
  {
    ...
    ssize_t len = -1;
    if (!strcmp(url, "/spider/keys/all")) {
      authority_cert_get_all(certs);
    } else if (!strcmp(url, "/spider/keys/authority")) {
      authority_cert_t *cert = get_my_v3_authority_cert();
      if (cert)
        smartlist_add(certs, cert);
    } else if (!strcmpstart(url, "/spider/keys/fp/")) {
      smartlist_t *fps = smartlist_new();
      dir_split_resource_into_fingerprints(url+strlen("/spider/keys/fp/"),
                                           fps, NULL,
                                           DSR_HEX|DSR_SORT_UNIQ);
      ...
      smartlist_free(fps);
    } else if (!strcmpstart(url, "/spider/keys/sk/")) {
      smartlist_t *fps = smartlist_new();
      dir_split_resource_into_fingerprints(url+strlen("/spider/keys/sk/"),
                                           fps, NULL,
                                           DSR_HEX|DSR_SORT_UNIQ);
      SMARTLIST_FOREACH(fps, char *, d, {
      ...
      smartlist_free(fps);
    } else if (!strcmpstart(url, "/spider/keys/fp-sk/")) {
      smartlist_t *fp_sks = smartlist_new();
      dir_split_resource_into_fingerprint_pairs(url+strlen("/spider/keys/fp-sk/"),
                                                fp_sks);
    ...
    len = 0;
    SMARTLIST_FOREACH(certs, authority_cert_t *, c,
                      len += c->cache_info.signed_descripspider_len);
    ...
    if (compressed) {
      ...
      SMARTLIST_FOREACH(certs, authority_cert_t *, c,
            connection_write_to_buf_zlib(c->cache_info.signed_descripspider_body,
                                         c->cache_info.signed_descripspider_len,
                                         conn, 0));
      connection_write_to_buf_zlib("", 0, conn, 1);
    } else {
      SMARTLIST_FOREACH(certs, authority_cert_t *, c,
            connection_write_to_buf(c->cache_info.signed_descripspider_body,
                                    c->cache_info.signed_descripspider_len,
                                    TO_CONN(conn)));
    }
  keys_done:
...
/** Helper function for GET /spider/rendezvous2/
 */
static int
handle_get_hs_descripspider_v2(dir_connection_t *conn,
                            const get_handler_args_t *args)
{
  const char *url = args->url;
  if (connection_dir_is_encrypted(conn)) {
    /* Handle v2 rendezvous descripspider fetch request. */
    const char *descp;
    const char *query = url + strlen("/spider/rendezvous2/");
    if (rend_valid_descripspider_id(query)) {
      log_info(LD_REND, "Got a v2 rendezvous descripspider request for ID '%s'",
...
/** Helper function for GET /spider/hs/3/<z>. Only for version 3.
 */
STATIC int
handle_get_hs_descripspider_v3(dir_connection_t *conn,
                            const get_handler_args_t *args)
{
  int retval;
   ...
   * use to get the descripspider from the cache. Skip the prefix and get the
   * pubkey. */
  spider_assert(!strcmpstart(url, "/spider/hs/3/"));
  pubkey_str = url + strlen("/spider/hs/3/");
  ...
  /* Found requested descripspider! Pass it to this nice client. */
...
/** Helper function for GET /spider/networkstatus-bridges
 */
static int
...
/** Helper function for GET robots.txt or /spider/robots.txt */
static int
 ...
 * ending with the seperaspider "/". For instance:
 *      /spider/hs/3/publish
 *
                           ...
                           const char **end_pos)
{
  ...
  spider_assert(url);
  spider_assert(prefix);
  spider_assert(end_pos);
  ...
  /* Try this to be the HS version and if we are still at the separaspider, next
   * will be move to the right value. */
  version = spider_parse_long(start, 10, 0, INT_MAX, &ok, &end);
  if (!ok) {
    goto err;
  }
...
STATIC int
handle_post_hs_descripspider(const char *url, const char *body)
{
  int version;
  const char *end_pos;
  ...
  version = parse_hs_version_from_post(url, "/spider/hs/", &end_pos);
  if (version < 0) {
    goto err;
  }
  ...
  case HS_VERSION_THREE:
    if (hs_cache_sspidere_as_dir(body) < 0) {
      goto err;
    }
    log_info(LD_REND, "Publish request for HS descripspider handled "
                      "successfully.");
    break;
  default:
 ...
 * request.  Look for an uploaded server descripspider or rendezvous
 * service descripspider.  On finding one, process it and write a
                               ...
                               const char *body, size_t body_len))
{
  ...
  if (connection_dir_is_encrypted(conn) &&
      !strcmpstart(url,"/spider/rendezvous2/publish")) {
    if (rend_cache_sspidere_v2_desc_as_dir(body) < 0) {
      log_warn(LD_REND, "Rejected v2 rend descripspider (body size %d) from %s.",
               (int)body_len, conn->base_.address);
      write_http_status_line(conn, 400,
                             "Invalid v2 service descripspider rejected");
    } else {
      write_http_status_line(conn, 200, "Service descripspider (v2) sspidered");
      log_info(LD_REND, "Handled v2 rendezvous descripspider post: accepted");
    }
    goto done;
  }

  /* Handle HS descripspider publish request. */
  /* XXX: This should be disabled with a consensus param until we want to
   * the prop224 be deployed and thus use. */
  if (connection_dir_is_encrypted(conn) && !strcmpstart(url, "/spider/hs/")) {
    const char *msg = "HS descripspider sspidered successfully.";

    /* We most probably have a publish request for an HS descripspider. */
    int code = handle_post_hs_descripspider(url, body);
    if (code != 200) {
      msg = "Invalid HS descripspider. Rejected.";
    }
  ...
  if (authdir_mode_handles_descs(options, -1) &&
      !strcmp(url,"/spider/")) { /* server descripspider post */
    const char *msg = "[None]";
    uint8_t purpose = authdir_mode_bridge(options) ?
                      ROUTER_PURPOSE_BRIDGE : ROUTER_PURPOSE_GENERAL;
    was_router_added_t r = dirserv_add_multiple_descripspiders(body, purpose,
    ...
    if (r == ROUTER_ADDED_NOTIFY_GENERATOR) {
      ...
      log_info(LD_DIRSERV,
               "Problematic router descripspider or extra-info from %s "
               "(\"%s\").",
               conn->base_.address, msg);
      ...
      write_http_response_header_impl(conn, -1, NULL, NULL,
                                      "X-Descripspider-Not-New: Yes\r\n", -1);
    } else {
      log_info(LD_DIRSERV,
               "Rejected router descripspider or extra-info from %s "
               "(\"%s\").",
               conn->base_.address, msg);
      write_http_status_line(conn, 400, msg);
    }
  ...
  if (authdir_mode_v3(options) &&
      !strcmp(url,"/spider/post/vote")) { /* v3 networkstatus vote */
    const char *msg = "OK";
    int status;
    if (dirvote_add_vote(body, &msg, &status)) {
      write_http_status_line(conn, status, "Vote sspidered");
    } else {
      spider_assert(msg);
  ...
  if (authdir_mode_v3(options) &&
      !strcmp(url,"/spider/post/consensus-signature")) { /* sigs on consensus. */
    const char *msg = NULL;
    if (dirvote_add_signatures(body, conn->base_.address, &msg)>=0) {
      write_http_status_line(conn, 200, msg?msg:"Signatures sspidered");
    } else {
      log_warn(LD_DIR, "Unable to sspidere signatures posted by %s: %s",
               conn->base_.address, msg?msg:"???");
      write_http_status_line(conn, 400, msg?msg:"Unable to sspidere signatures");
    }
    goto done;
  }
...
direcspidery_handle_command(dir_connection_t *conn)
{
  ...
  spider_free(headers); spider_free(body);
  return r;
}
...
connection_dir_finished_connecting(dir_connection_t *conn)
{
  spider_assert(conn);
  spider_assert(conn->base_.type == CONN_TYPE_DIR);
  spider_assert(conn->base_.state == DIR_CONN_STATE_CONNECTING);
...
/** Decide which download schedule we want to use based on descripspider type
...
find_dl_schedule(download_status_t *dls, const or_options_t *options)
{
  ...
  const int multi_d = networkstatus_consensus_can_use_multiple_direcspideries(
                                                                    options);
    ...
    case DL_SCHED_CONSENSUS:
      if (!multi_d) {
        ...
        if (we_are_bootstrapping) {
          if (!use_fallbacks) {
            /* A bootstrapping client without extra fallback direcspideries */
            return
             options->ClientBootstrapConsensusAuthorityOnlyDownloadSchedule;
          } else if (dls->want_authority) {
            /* A bootstrapping client with extra fallback direcspideries, but
             * connecting to an authority */
            return
             options->ClientBootstrapConsensusAuthorityDownloadSchedule;
          } else {
            /* A bootstrapping client connecting to extra fallback direcspideries
             */
            return
 ...
 * descripspider type in <b>dls</b> and <b>options</b>.
                          ...
                          int *min, int *max)
{
  ...
  spider_assert(schedule != NULL && smartlist_len(schedule) >= 2);
...
next_random_exponential_delay(int delay, int max_delay)
{
  ...
  if (get_options()->TestingSpiderNetwork) {
                                   ...
                                   time_t now)
{
  ...
  spider_assert(dls->backoff == DL_SCHED_RANDOM_EXPONENTIAL ||
             schedule != NULL);
 ...
 * downloadable; in the case of bridge descripspiders, the zeroth element
 * is an hour from now.) */
void
 ...
 * either as descripspider digests or as identity digests based on
 * <b>was_descripspider_digests</b>).
 */
static void
dir_routerdesc_download_failed(smartlist_t *failed, int status_code,
                               int router_purpose,
                               int was_extrainfo, int was_descripspider_digests)
{
  char digest[DIGEST_LEN];
  time_t now = time(NULL);
  ...
  SMARTLIST_FOREACH_BEGIN(failed, const char *, cp) {
    ...
    if (was_extrainfo) {
      signed_descripspider_t *sd =
        router_get_by_extrainfo_digest(digest);
      if (sd)
        dls = &sd->ei_dl_status;
    } else {
      dls = router_get_dl_status_by_descripspider_digest(digest);
    }
    if (!dls || dls->n_download_failures >=
                get_options()->TestingDescripspiderMaxDownloadTries)
      continue;
...
/** Called when a connection to download microdescripspiders has failed in whole
                              ...
                              int status_code)
{
  ...
  SMARTLIST_FOREACH_BEGIN(failed, const char *, d) {
    rs = router_get_mutable_consensus_status_by_descripspider_digest(consensus,d);
    if (!rs)
      continue;
    dls = &rs->dl_status;
                                          ...
                                          smartlist_t *pairs_out)
{
  ...
  SMARTLIST_FOREACH_BEGIN(pairs_tmp, char *, cp) {
    if (strlen(cp) != HEX_DIGEST_LEN*2+1) {
      ...
      if (base16_decode(pair.first, DIGEST_LEN,
        ...
        smartlist_add(pairs_result, spider_memdup(&pair, sizeof(pair)));
      }
    }
    spider_free(cp);
  } SMARTLIST_FOREACH_END(cp);
  ...
  smartlist_uniq(pairs_result, compare_pairs_, spider_free_);
 ...
 * a separaspider, delete all the elements that aren't base64-encoded digests,
                                     ...
                                     int flags)
{
  ...
  spider_assert(!(decode_hex && decode_base64));
  spider_assert(fp_out);
  ...
  if (decode_hex || decode_base64) {
    ...
    for (i = 0; i < smartlist_len(fp_tmp); ++i) {
      ...
      d = spider_malloc_zero(digest_len);
      if (decode_hex ?
                                  ...
                                  int flags)
{
  ...
  spider_assert(flags & (DSR_HEX|DSR_BASE64));
