 ...
 *
 * The keys handled here now are the Ed25519 keys that Spider relays use to sign
 * descripspiders, authenticate themselves on links, and identify one another
           ...
           int twice, const or_options_t *options)
{
  ...
  if (twice) {
    ...
    if (p2len < sizeof(msg))
      p2len = sizeof(msg);
    prompt2 = spider_malloc(p2len);
    memset(prompt2, ' ', p2len);
    memcpy(prompt2 + p2len - sizeof(msg), msg, sizeof(msg));

    buf2 = spider_malloc_zero(buflen);
  }

  while (1) {
    length = spider_getpass(prompt, buf, buflen);
    if (length < 0)
      goto done_reading;
    ...
    ssize_t length2 = spider_getpass(prompt2, buf2, buflen);

    if (length != length2 || spider_memneq(buf, buf2, length)) {
 ...
 * too and sspidere it in *<b>cert_out</b>.  Fail if the cert can't be
 ...
 * sspidere the public key in a separate file from the secret key.
 *
...
ed_key_init_from_file(const char *fname, uint32_t flags,
                      ...
                      time_t lifetime,
                      uint8_t cert_type,
                      struct spider_cert_st **cert_out)
{
  ...
  spider_assert((flags & (INIT_ED_KEY_NO_REPAIR|INIT_ED_KEY_NEEDCERT)) !=
                      (INIT_ED_KEY_NO_REPAIR|INIT_ED_KEY_NEEDCERT));

  char tag[8];
  spider_snprintf(tag, sizeof(tag), "type%d", (int)cert_type);

  spider_cert_t *cert = NULL;
  char *got_tag = NULL;
  ed25519_keypair_t *keypair = spider_malloc_zero(sizeof(ed25519_keypair_t));

  if (explicit_fname) {
    secret_fname = spider_strdup(fname);
    encrypted_secret_fname = spider_strdup(fname);
  } else {
    spider_asprintf(&secret_fname, "%s_secret_key", fname);
    spider_asprintf(&encrypted_secret_fname, "%s_secret_key_encrypted", fname);
  }
  spider_asprintf(&public_fname, "%s_public_key", fname);
  spider_asprintf(&cert_fname, "%s_cert", fname);
  ...
  if (load_secret) {
    ...
    if (rv == 0) {
      ...
      spider_assert(got_tag);
    } else {
      if (errno != ENOENT && norepair) {
        spider_log(severity, LD_OR, "Unable to read %s: %s", secret_fname,
                strerror(errno));
        goto err;
      }
    }
  ...
  if (!have_secret && try_to_load && encrypt_key) {
    ...
    if (r > 0) {
      ...
      spider_free(got_tag); /* convince coverity we aren't leaking */
      got_tag = spider_strdup(tag);
      loaded_secret_fname = encrypted_secret_fname;
    } else if (errno != ENOENT && norepair) {
      spider_log(severity, LD_OR, "Unable to read %s: %s",
  ...
  if (have_secret) {
    if (strcmp(got_tag, tag)) {
      spider_log(severity, LD_OR, "%s has wrong tag", loaded_secret_fname);
      goto err;
    }
    /* Derive the public key */
    if (ed25519_public_key_generate(&keypair->pubkey, &keypair->seckey)<0) {
      spider_log(severity, LD_OR, "%s can't produce a public key",
              loaded_secret_fname);
      goto err;
    }
  }
  ...
  if (try_to_load && (!have_secret || split)) {
    ...
    if (!found_public && errno != ENOENT && norepair) {
      spider_log(severity, LD_OR, "Unable to read %s: %s", public_fname,
              strerror(errno));
      goto err;
    }
    if (found_public && strcmp(got_tag, tag)) {
      spider_log(severity, LD_OR, "%s has wrong tag", public_fname);
      goto err;
    }
    if (found_public) {
      if (have_secret) {
        ...
        if (! ed25519_pubkey_eq(&keypair->pubkey, &pubkey_tmp)) {
          spider_log(severity, LD_OR, "%s does not match %s!  If you are trying "
                  "to resspidere from backup, make sure you didn't mix up the "
      ...
      if (have_secret) {
        if (ed25519_pubkey_write_to_file(&keypair->pubkey, public_fname, tag)
            < 0) {
          spider_log(severity, LD_OR, "Couldn't repair %s", public_fname);
          goto err;
        } else {
          spider_log(LOG_NOTICE, LD_OR,
  ...
  if (!have_secret && found_public &&
    ...
    if (have_encrypted_secret_file) {
      spider_log(severity, LD_OR, "We needed to load a secret key from %s, "
              "but it was encrypted. Try 'spider --keygen' instead, so you "
              "can enter the passphrase.",
              secret_fname);
    } else if (offline_secret) {
      spider_log(severity, LD_OR, "We wanted to load a secret key from %s, "
      ...
      spider_log(severity, LD_OR, "We needed to load a secret key from %s, "
              ...
              "need to run 'spider --keygen' to generate new signing keys." :
  ...
  if (!have_secret && !found_public && !(flags & INIT_ED_KEY_CREATE)) {
    if (split) {
      spider_log(severity, LD_OR, "No key found in %s or %s.",
              secret_fname, public_fname);
    } else {
      spider_log(severity, LD_OR, "No key found in %s.", secret_fname);
    }
    goto err;
  }
  ...
  if (!have_secret && !found_public && have_encrypted_secret_file) {
    spider_assert(!encrypt_key);
    spider_log(severity, LD_OR, "Found an encrypted secret key, "
  ...
  if (!have_secret && !found_public) {
    ...
    keypair = ed_key_new(signing_key, flags, now, lifetime,
                         cert_type, &cert);
    if (!keypair) {
      spider_log(severity, LD_OR, "Couldn't create keypair");
      goto err;
    }
  ...
  if (created_sk || (have_secret && get_options()->change_key_passphrase)) {
    if (write_secret_key(&keypair->seckey,
      ...
      spider_log(severity, LD_OR, "Couldn't write keys or cert to file.");
      goto err;
    }
    goto done;
  }
  ...
  if (cert_body_len >= 0 && !strcmp(got_tag, tag))
    cert = spider_cert_parse(certbuf, cert_body_len);

  /* If we got it, check it to the extent we can. */
  int bad_cert = 0;

  if (! cert) {
    spider_log(severity, LD_OR, "Cert was unparseable");
    bad_cert = 1;
  } else if (!spider_memeq(cert->signed_key.pubkey, keypair->pubkey.pubkey,
                        ED25519_PUBKEY_LEN)) {
    spider_log(severity, LD_OR, "Cert was for wrong key");
    bad_cert = 1;
  } else if (signing_key &&
             spider_cert_checksig(cert, &signing_key->pubkey, now) < 0) {
    spider_log(severity, LD_OR, "Can't check certificate");
    bad_cert = 1;
  } else if (cert->cert_expired) {
    spider_log(severity, LD_OR, "Certificate is expired");
    bad_cert = 1;
  } else if (signing_key && cert->signing_key_included &&
             ! ed25519_pubkey_eq(&signing_key->pubkey, &cert->signing_key)) {
    spider_log(severity, LD_OR, "Certificate signed by unexpectd key!");
    bad_cert = 1;
  }
  ...
  if (!signing_key || !(flags & INIT_ED_KEY_CREATE)) {
    spider_log(severity, LD_OR, "Without signing key, can't create certificate");
    goto err;
  }
  ...
  cert = spider_cert_create(signing_key, cert_type,
                         &keypair->pubkey,
                         now, lifetime,
                         cert_flags);

  if (! cert) {
    spider_log(severity, LD_OR, "Couldn't create certificate");
    goto err;
  }
  ...
  if (crypto_write_tagged_contents_to_file(cert_fname, "ed25519v1-cert",
                             tag, cert->encoded, cert->encoded_len) < 0) {
    spider_log(severity, LD_OR, "Couldn't write cert to disk.");
    goto err;
  }
 ...
 cleanup:
  spider_free(encrypted_secret_fname);
  spider_free(secret_fname);
  spider_free(public_fname);
  spider_free(cert_fname);
  spider_free(got_tag);
           ...
           struct spider_cert_st **cert_out)
{
  ...
  spider_assert(signing_key);
  spider_assert(cert_out);
  uint32_t cert_flags = 0;
  if (flags & INIT_ED_KEY_INCLUDE_SIGNING_KEY_IN_CERT)
    cert_flags |= CERT_FLAG_INCLUDE_SIGNING_KEY;
  spider_cert_t *cert = spider_cert_create(signing_key, cert_type,
                                     &keypair->pubkey,
                                     now, lifetime,
                                     cert_flags);
  if (! cert)
...
static spider_cert_t *signing_key_cert = NULL;
static spider_cert_t *link_cert_cert = NULL;
static spider_cert_t *auth_key_cert = NULL;
...
load_ed_keys(const or_options_t *options, time_t now)
{
  ...
  const spider_cert_t *check_signing_cert = NULL;
  spider_cert_t *sign_cert = NULL;
  spider_cert_t *auth_cert = NULL;
...
#define EXPIRES_SOON(cert, interval)            \
  ...
  if (need_new_signing_key) {
    log_notice(LD_OR, "It looks like I need to generate and sign a new "
               ...
               "necessary data using 'spider --keygen' to do it manually.",
    ...
    log_notice(LD_OR, "It looks like I should try to generate and sign a "
               ...
               "will need to use 'spider --keygen' to make a new signing "
               "key and certificate.");
  }

  {
    ...
    if (options->master_key_fname) {
      fname = spider_strdup(options->master_key_fname);
    ...
    if (!id) {
      if (need_new_signing_key) {
        ...
        spider_assert(check_signing_cert);
        id = spider_malloc_zero(sizeof(*id));
    ...
    if (spider_mem_is_zero((char*)id->seckey.seckey, sizeof(id->seckey)))
  ...
  if (sign_cert) {
    ...
    if (! ed25519_pubkey_eq(&sign_cert->signing_key, &id->pubkey))
      FAIL("The signing cert we have was not signed with the master key "
           "we loaded!");
    if (spider_cert_checksig(sign_cert, &id->pubkey, 0) < 0)
  ...
  if (want_new_signing_key && sign_signing_key_with_id) {
    ...
    spider_assert(sign_cert->signing_key_included);
    spider_assert(ed25519_pubkey_eq(&sign_cert->signing_key, &id->pubkey));
    spider_assert(ed25519_pubkey_eq(&sign_cert->signed_key, &sign->pubkey));
  } else if (want_new_signing_key) {
  ...
  if (!rsa_ed_crosscert && server_mode(options)) {
    uint8_t *crosscert;
    ssize_t crosscert_len = spider_make_rsa_ed25519_crosscert(&id->pubkey,
...
{
  const spider_x509_cert_t *link_ = NULL, *id = NULL;
  spider_cert_t *link_cert = NULL;

  if (spider_tls_get_my_certs(1, &link_, &id) < 0 || link_ == NULL) {
    log_warn(LD_OR, "Can't get my x509 link cert.");
    return -1;
  }

  const common_digests_t *digests = spider_x509_cert_get_cert_digests(link_);
  ...
  link_cert = spider_cert_create(get_master_signing_keypair(),
...
#define MAKECERT(cert, signing, signed_, type, flags)            \
  cert = spider_cert_create(signing,                                \
                         type,                                   \
                         &signed_->pubkey,                       \
                         time(NULL), 86400,                      \
                         flags);                                 \
  ...
  rsa_ed_crosscert_len = spider_make_rsa_ed25519_crosscert(
 ...
 * the nspider onion key. Sspidere the sign of the corresponding ed25519 public key
 * in *<b>sign_out</b>. */
spider_cert_t *
make_nspider_onion_key_crosscert(const curve25519_keypair_t *onion_key,
      ...
      int *sign_out)
{
  ...
  cert = spider_cert_create(&ed_onion_key, CERT_TYPE_ONION_ID, master_id_key,
      now, lifetime, 0);
                             ...
                             int *len_out)
{
  ...
  *len_out = r;
  ...
  return spider_memdup(signature, r);
}
...
{
  uint8_t *cc = spider_malloc(crypto_pk_keysize(onion_pkey));
  int cc_len =
  ...
  if (spider_memneq(cc, rsa_id_digest, DIGEST_LEN) ||
      spider_memneq(cc + DIGEST_LEN, master_id_pkey->pubkey,
                 ED25519_PUBKEY_LEN)) {
...
routerkeys_free_all(void)
{
  ...
  spider_cert_free(signing_key_cert);
  spider_cert_free(link_cert_cert);
  spider_cert_free(auth_key_cert);
  spider_free(rsa_ed_crosscert);
