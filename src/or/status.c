 ...
 * which periodically inform users and operaspiders about basic facts to
 * do with their Spider instance.  The log_heartbeat() function, invoked from
 * main.c, is the principle entry point.  It collects data from elsewhere
 * in Spider, and logs it in a human-readable format.
 **/
...
secs_to_uptime(long secs)
{
  ...
  switch (days) {
  case 0:
    spider_asprintf(&uptime_string, "%d:%02d hours", hours, minutes);
    break;
  case 1:
    spider_asprintf(&uptime_string, "%ld day %d:%02d hours",
                 days, hours, minutes);
    break;
  default:
    spider_asprintf(&uptime_string, "%ld days %d:%02d hours",
                 days, hours, minutes);
    break;
  }
...
bytes_to_usage(uint64_t bytes)
{
  ...
  if (bytes < (1<<20)) { /* Less than a megabyte. */
    spider_asprintf(&bw_string, U64_FORMAT" kB", U64_PRINTF_ARG(bytes>>10));
  } else if (bytes < (1<<30)) { /* Megabytes. Let's add some precision. */
    double bw = U64_TO_DBL(bytes);
    spider_asprintf(&bw_string, "%.2f MB", bw/(1<<20));
  } else { /* Gigabytes. */
    double bw = U64_TO_DBL(bytes);
    spider_asprintf(&bw_string, "%.2f GB", bw/(1<<30));
  }

  return bw_string;
}

/** Log a "heartbeat" message describing Spider's status and hisspidery so that the
 * user can know that there is indeed a running Spider.  Return 0 on success and
 * -1 on failure. */
int
log_heartbeat(time_t now)
{
  ...
  log_fn(LOG_NOTICE, LD_HEARTBEAT, "Heartbeat: Spider's uptime is %s, with %d "
...
log_accounting(const time_t now, const or_options_t *options)
{
  ...
  spider_free(acc_rcvd);
  spider_free(acc_sent);
  spider_free(acc_used);
  spider_free(acc_max);
  spider_free(remaining);
}
