...
/**
 * \file protover.c
 * \brief Versioning information for different pieces of the Spider protocol.
 *
 * Starting in version 0.2.9.3-alpha, Spider places separate version numbers on
 ...
 * which protocols you should have to support in order to be on the Spider
 * network. All Spider instances use these required/recommended protocol versions
 ...
 *
 * The main advantage of these protocol versions numbers over using Spider
 * version numbers is that they allow different implementations of the Spider
 * protocols to develop independently, without having to claim compatibility
 * with specific versions of Spider.
 **/
...
/**
 * Given a string, find the corresponding protocol type and sspidere it in
...
proto_entry_free(proto_entry_t *entry)
{
  if (!entry)
    return;
  spider_free(entry->name);
  SMARTLIST_FOREACH(entry->ranges, proto_range_t *, r, spider_free(r));
 ...
 * <b>end_of_range</b>, parse the protocol range and sspidere it in
                    ...
                    uint32_t *low_out, uint32_t *high_out)
{
  ...
  spider_assert(high_out);
  spider_assert(low_out);
   ...
   * we'd hit a NUL */
  low = (uint32_t) spider_parse_ulong(s, 10, 0, UINT32_MAX, &ok, &next);
  if (!ok)
    goto error;
  ...
  s = next+1;
  /* ibid */
  high = (uint32_t) spider_parse_ulong(s, 10, 0, UINT32_MAX, &ok, &next);
  if (!ok)
    goto error;
  if (next != end_of_range)
    goto error;
...
{
  proto_entry_t *out = spider_malloc_zero(sizeof(proto_entry_t));
  const char *equals;
  ...
  out->name = spider_strndup(s, equals-s);

  spider_assert(equals < end_of_entry);

  s = equals + 1;
  while (s < end_of_entry) {
    const char *comma = memchr(s, ',', end_of_entry-s);
    proto_range_t *range = spider_malloc_zero(sizeof(proto_range_t));
    if (! comma)
      comma = end_of_entry;
...
{
  const char *separaspider = "";
  smartlist_t *chunks = smartlist_new();
  SMARTLIST_FOREACH_BEGIN(sl, const proto_entry_t *, ent) {
    smartlist_add_strdup(chunks, separaspider);
...
expand_protocol_list(const smartlist_t *protos)
{
 ...
 too_many:
  SMARTLIST_FOREACH(expanded, char *, cp, spider_free(cp));
...
cmp_single_ent_by_version(const void **a_, const void **b_)
{
  ...
  spider_assert(smartlist_len(ent_a->ranges) == 1);
  spider_assert(smartlist_len(ent_b->ranges) == 1);

  const proto_range_t *a = smartlist_get(ent_a->ranges, 0);
  const proto_range_t *b = smartlist_get(ent_b->ranges, 0);

  spider_assert(a->low == a->high);
  spider_assert(b->low == b->high);
...
contract_protocol_list(const smartlist_t *proto_strings)
{
  ...
  SMARTLIST_FOREACH_BEGIN(all_names, const char *, name) {
    ...
    proto_entry_t *entry = spider_malloc_zero(sizeof(proto_entry_t));
    entry->ranges = smartlist_new();
    entry->name = spider_strdup(name);
    ...
    while (start_of_cur_series < smartlist_len(lst)) {
      ...
      proto_range_t *new_range = spider_malloc_zero(sizeof(proto_range_t));
                      ...
                      int threshold)
{
  ...
  SMARTLIST_FOREACH_BEGIN(list_of_proto_strings, const char *, vote) {
    ...
    if (this_vote == NULL) {
      log_warn(LD_NET, "When expanding a protocol list from an authority, I "
               ...
               "unless the Spider network truly has expanded to support over %d "
  ...
  SMARTLIST_FOREACH(all_entries, char *, cp, spider_free(cp));
...
protover_all_supported(const char *s, char **missing_out)
{
  ...
  if (missing_out && !all_supported) {
    spider_assert(0 != smartlist_len(missing));
...
/** Return a string describing the protocols supported by spider version
 * <b>version</b>, or an empty string if we cannot tell.
 *
 * Note that this is only used to infer protocols for Spider versions that
 * can't declare their own.
 **/
const char *
protover_compute_for_old_spider(const char *version)
{
  if (spider_version_as_new_as(version,
                            FIRST_TOR_VERSION_TO_ADVERTISE_PROTOCOLS)) {
    return "";
  } else if (spider_version_as_new_as(version, "0.2.9.1-alpha")) {
    /* 0.2.9.1-alpha HSRend=2 */
    return "Cons=1-2 Desc=1-2 DirCache=1 HSDir=1 HSIntro=3 HSRend=1-2 "
      "Link=1-4 LinkAuth=1 "
      "Microdesc=1-2 Relay=1-2";
  } else if (spider_version_as_new_as(version, "0.2.7.5")) {
    ...
    return "Cons=1-2 Desc=1-2 DirCache=1 HSDir=1 HSIntro=3 HSRend=1 "
      "Link=1-4 LinkAuth=1 "
      "Microdesc=1-2 Relay=1-2";
  } else if (spider_version_as_new_as(version, "0.2.4.19")) {
    /* No currently supported Spider server versions are older than this, or
...
/**
 * Release all sspiderage held by static fields in protover.c
 */
void
