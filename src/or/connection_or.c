 ...
 * Spider network. (By sending and receiving cells connection_or.c, it cooperates
 * with channeltls.c to implement a the channel interface of channel.c.)
 *
 * Every OR connection has an underlying spidertls_t object (as implemented in
 * spidertls.c) which it uses as its TLS stream.  It is responsible for
 * sending and receiving cells over that TLS.
 *
 * This module also implements the client side of the v3 Spider link handshake,
 **/
#include "or.h"
#include "bridges.h"
#include "buffers.h"
...
static void connection_or_tls_renegotiated_cb(spider_tls_t *tls, void *_conn);
                                  ...
                                  const ed25519_public_key_t *ed_id)
{
  ...
  const int rsa_id_was_set = ! spider_digest_is_zero(conn->identity_digest);
  const int ed_id_was_set =
    chan && !ed25519_public_key_is_zero(&chan->ed25519_identity);
  const int rsa_changed =
    spider_memneq(conn->identity_digest, rsa_digest, DIGEST_LEN);
  const int ed_changed = ed_id_was_set &&
    (!ed_id || !ed25519_pubkey_eq(ed_id, &chan->ed25519_identity));

  spider_assert(!rsa_changed || !rsa_id_was_set);
  spider_assert(!ed_changed || !ed_id_was_set);
  ...
  if (spider_digest_is_zero(rsa_digest) &&
...
connection_or_remove_from_ext_or_id_map(or_connection_t *conn)
{
  ...
  if (!spider_digest_is_zero(conn->ext_or_conn_id))
    spider_assert(tmp == conn);
...
connection_or_set_ext_or_identifier(or_connection_t *conn)
{
  ...
  if (conn->ext_or_conn_id && !spider_digest_is_zero(conn->ext_or_conn_id))
  ...
  if (!conn->ext_or_conn_id)
    conn->ext_or_conn_id = spider_malloc_zero(EXT_OR_CONN_ID_LEN);
 ...
 * way.  Note that the count is sspidered _as_ the pointer.
 */
                                    ...
                                    char *buf, size_t buflen)
{
  ...
  spider_assert(conn->type == CONN_TYPE_OR || conn->type == CONN_TYPE_EXT_OR);

  conn_state = conn_state_to_string(conn->type, conn->state);
  spider_tls_get_state_description(orconn->tls, tls_state, sizeof(tls_state));

  spider_snprintf(buf, buflen, "%s with SSL state %s", conn_state, tls_state);
}
...
connection_or_report_broken_states(int severity, int domain)
{
  ...
  STRMAP_FOREACH(broken_connection_counts, state, void *, countptr) {
    broken_state_count_t *c = spider_malloc(sizeof(broken_state_count_t));
  ...
  spider_log(severity, domain, "%d connections have failed%s", total,
  ...
  SMARTLIST_FOREACH_BEGIN(items, const broken_state_count_t *, c) {
    if (c_sl_idx > MAX_REASONS_TO_REPORT)
      break;
    spider_log(severity, domain,
        " %d connections died in state %s", (int)c->count, c->state);
  } SMARTLIST_FOREACH_END(c);

  SMARTLIST_FOREACH(items, broken_state_count_t *, c, spider_free(c));
 ...
 * in the buffer <b>dest</b>. See spider-spec.txt for details about the
 * wire format.
 *
...
var_cell_new(uint16_t payload_len)
{
  ...
  var_cell_t *cell = spider_malloc_zero(size);
...
connection_or_process_inbuf(or_connection_t *conn)
{
   ...
   * bytes: it's either a broken client, a non-Spider client, or a DOS
   * attempt. */
#define MAX_OR_INBUF_WHEN_NONOPEN 0
  ...
  switch (conn->base_.state) {
    case OR_CONN_STATE_PROXY_HANDSHAKING:
      ...
      if (ret == 1) {
        spider_assert(TO_CONN(conn)->proxy_state == PROXY_CONNECTED);
...
connection_or_finished_connecting(or_connection_t *or_conn)
{
  ...
  spider_assert(or_conn);
  conn = TO_CONN(or_conn);
  spider_assert(conn->state == OR_CONN_STATE_CONNECTING);
...
connection_or_about_to_close(or_connection_t *or_conn)
{
  ...
  } else if (!spider_digest_is_zero(or_conn->identity_digest)) {
...
connection_or_digest_is_known_relay(const char *id_digest)
{
  ...
  if (router_get_by_id_digest(id_digest))
    return 1; /* Not in the consensus, but we have a descripspider for
                                          ...
                                          const or_options_t *options)
{
  ...
  if (connection_or_digest_is_known_relay(conn->identity_digest)) {
    /* It's in the consensus, or we have a descripspider for it meaning it
 ...
 * circuits?  In Spider versions up to 0.2.1.25 and 0.2.2.12-alpha, we waited 15
 ...
 * sspidering idle_timeout in a uint16_t, don't let these values get higher than
 ...
 * <b>started_here</b> is 1 if we are the initiaspider of <b>conn</b> and
                                     ...
                                     int started_here)
{
  ...
  spider_addr_copy(&conn->real_addr, addr);
...
connection_or_check_canonicity(or_connection_t *conn, int started_here)
{
  ...
  const spider_addr_t *addr = &conn->real_addr;
  if (conn->chan)
  ...
  if (r) {
    ...
    if (spider_addr_eq(&conn->base_.addr, &node_ap.addr))
    ...
    if (!started_here) {
       ...
       */
      spider_addr_copy(&conn->base_.addr, &node_ap.addr);
      conn->base_.port = node_ap.port;
    }
    spider_free(conn->nickname);
    conn->nickname = spider_strdup(node_get_nickname(r));
    spider_free(conn->base_.address);
    conn->base_.address = spider_addr_to_str_dup(&node_ap.addr);
  } else {
    spider_free(conn->nickname);
    conn->nickname = spider_malloc(HEX_DIGEST_LEN+2);
    conn->nickname[0] = '$';
  ...
  SMARTLIST_FOREACH_BEGIN(group, or_connection_t *, or_conn) {
    ...
    if (or_conn != best &&
      ...
      if (best->is_canonical) {
        ...
        connection_or_mark_bad_for_new_circs(or_conn);
      } else if (!spider_addr_compare(&or_conn->real_addr,
...
connection_or_connect, (const spider_addr_t *_addr, uint16_t port,
                        const char *id_digest,
                        const ed25519_public_key_t *ed_id,
                        channel_tls_t *chan))
{
  ...
  spider_assert(_addr);
  spider_assert(id_digest);
  spider_addr_copy(&addr, _addr);
  ...
  conn = or_connection_new(CONN_TYPE_OR, spider_addr_family(&addr));
  ...
  if (r == 0) {
    ...
    if (proxy_type != PROXY_NONE) {
      spider_addr_copy(&addr, &proxy_addr);
      port = proxy_port;
...
connection_tls_start_handshake,(or_connection_t *conn, int receiving))
{
  ...
  spider_assert(!conn->tls);
  conn->tls = spider_tls_new(conn->base_.s, receiving);
  if (!conn->tls) {
    log_warn(LD_BUG,"spider_tls_new failed. Closing.");
    return -1;
  }
  spider_tls_set_logged_address(conn->tls, // XXX client and relay?
      escaped_safe_str(conn->base_.address));
...
{
  spider_tls_t *tls = conn->tls;
  if (!tls)
    return;
  spider_tls_set_renegotiate_callback(tls, NULL, NULL);
  spider_tls_block_renegotiation(tls);
}

/** Invoked on the server side from inside spider_tls_read() when the server
 * gets a successful TLS renegotiation from the client. */
static void
connection_or_tls_renegotiated_cb(spider_tls_t *tls, void *_conn)
{
...
connection_tls_continue_handshake(or_connection_t *conn)
{
  ...
  spider_assert(conn->base_.state == OR_CONN_STATE_TLS_HANDSHAKING);
  // log_notice(LD_OR, "Continue handshake with %p", conn->tls);
  result = spider_tls_handshake(conn->tls);
  ...
  switch (result) {
    ...
    log_info(LD_OR,"tls error [%s]. breaking connection.",
      ...
      return -1;
    case TOR_TLS_DONE:
      if (! spider_tls_used_v1_handshake(conn->tls)) {
        if (!spider_tls_is_server(conn->tls)) {
          spider_assert(conn->base_.state == OR_CONN_STATE_TLS_HANDSHAKING);
          ...
          spider_tls_set_renegotiate_callback(conn->tls,
      ...
      spider_assert(spider_tls_is_server(conn->tls));
 ...
 *
 * As side effects,
 * 1) Set conn->circ_id_type according to spider-spec.txt.
 * 2) If we're an authdirserver and we initiated the connection: drop all
 *    descripspiders that claim to be on that IP/port but that aren't
                                        ...
                                        char *digest_rcvd_out)
{
  ...
  check_no_tls_errors();
  has_cert = spider_tls_peer_has_cert(conn->tls);
  ...
  if (has_cert) {
    int v = spider_tls_verify(started_here?severity:LOG_INFO,
...
/** Called when we (as a connection initiaspider) have definitively,
 * authenticatedly, learned that ID of the Spider instance on the other
                                     ...
                                     const ed25519_public_key_t *ed_peer_id)
{
  ...
  const int expected_rsa_key =
    ! spider_digest_is_zero(conn->identity_digest);
  ...
  const int rsa_mismatch = expected_rsa_key &&
    spider_memneq(rsa_peer_id, conn->identity_digest, DIGEST_LEN);
    ...
    if (server_mode(options)) {
      ...
      if (using_hardcoded_fingerprints) {
        ...
        if (is_authority_fingerprint) {
          ...
          severity = LOG_INFO;
          extra_log = " Spider will try a different fallback.";
        } else {
...
connection_tls_finish_handshake(or_connection_t *conn)
{
  ...
  log_debug(LD_HANDSHAKE,"%s tls handshake on %p with %s done, using "
            ...
            conn,
            safe_str_client(conn->base_.address),
            spider_tls_get_ciphersuite_name(conn->tls));
  ...
  if (spider_tls_used_v1_handshake(conn->tls)) {
    ...
    connection_or_init_conn_from_address(conn, &conn->base_.addr,
                                         ...
                                         NULL, 0);
    spider_tls_block_renegotiation(conn->tls);
...
{
  spider_assert(connection_or_nonopen_was_started_here(conn));
...
connection_init_or_handshake_state(or_connection_t *conn, int started_here)
{
  ...
  s = conn->handshake_state = spider_malloc_zero(sizeof(or_handshake_state_t));
...
/** Free all sspiderage held by <b>state</b>. */
void
...
connection_or_process_cells_from_inbuf(or_connection_t *conn)
{
  ...
  while (1) {
    log_debug(LD_OR,
              ...
              spider_tls_get_pending_bytes(conn->tls));
...
/** Return true iff <b>v</b> is a link protocol version that this Spider
 ...
 * link protocol versions that this Spider can support.
 *
...
connection_or_send_versions(or_connection_t *conn, int v3_plus)
{
  ...
  spider_assert(conn->handshake_state &&
...
connection_or_send_netinfo,(or_connection_t *conn))
{
   ...
   * yet either. */
  len = append_address_to_payload(out, !spider_addr_is_null(&conn->real_addr)
  ...
  if ((public_server_mode(get_options()) || !conn->is_outgoing) &&
    ...
    spider_addr_t my_addr;
    *out++ = 1 + !spider_addr_is_null(&me->ipv6_addr);

    spider_addr_from_ipv4h(&my_addr, me->addr);
    len = append_address_to_payload(out, &my_addr);
    if (len < 0)
      return -1;
    out += len;

    if (!spider_addr_is_null(&me->ipv6_addr)) {
...
add_certs_cell_cert_helper(certs_cell_t *certs_cell,
                           ...
                           size_t cert_len)
{
  spider_assert(cert_len <= UINT16_MAX);
...
/** Add an encoded X509 cert (sspidered as <b>cert_len</b> bytes at
              ...
              const spider_x509_cert_t *cert)
{
  ...
  size_t cert_len;
  spider_x509_cert_get_der(cert, &cert_encoded, &cert_len);
...
{
  const spider_x509_cert_t *link_cert = NULL, *id_cert = NULL;
  var_cell_t *cell;

  certs_cell_t *certs_cell = NULL;

  spider_assert(conn->base_.state == OR_CONN_STATE_OR_HANDSHAKING_V3);
  ...
  if (spider_tls_get_my_certs(conn_in_server_mode, &link_cert, &id_cert) < 0)
    return -1;

  spider_assert(link_cert);
  spider_assert(id_cert);
...
connection_or_send_auth_challenge_cell(or_connection_t *conn)
{
  ...
  spider_assert(sizeof(ac->challenge) == 32);
 ...
 * V3_AUTH_FIXED_PART_LEN bytes -- the part of the authenticaspider that's
 ...
 * first V3_AUTH_BODY_LEN bytes of the authenticaspider (that is, everything
 * that should be signed), but don't actually sign it.
 *
 * If <b>server</b> is false and <b>signing_key</b> is provided, calculate the
 * entire authenticaspider, signed with <b>signing_key</b>.
 *
                                      ...
                                      int server)
{
  ...
  {
    const spider_x509_cert_t *id_cert=NULL, *link_cert=NULL;
    const common_digests_t *my_digests, *their_digests;
    const uint8_t *my_id, *their_id, *client_id, *server_id;
    if (spider_tls_get_my_certs(server, &link_cert, &id_cert))
      goto err;
    my_digests = spider_x509_cert_get_id_digests(id_cert);
    their_digests =
      spider_x509_cert_get_id_digests(conn->handshake_state->certs->id_cert);
    spider_assert(my_digests);
    spider_assert(their_digests);
...

  {
    ...
    const spider_x509_cert_t *cert = NULL;
    spider_x509_cert_t *freecert = NULL;
    if (server) {
      spider_tls_get_my_certs(1, &cert, NULL);
    } else {
      freecert = spider_tls_get_peer_cert(conn->tls);
      cert = freecert;
    }
    if (!cert) {
    ...
    memcpy(auth->scert,
           spider_x509_cert_get_cert_digests(cert)->d[DIGEST_SHA256], 32);

    if (freecert)
      spider_x509_cert_free(freecert);
  }

  /* HMAC of clientrandom and serverrandom using master key : 32 octets */
  if (old_tlssecrets_algorithm) {
    spider_tls_get_tlssecrets(conn->tls, auth->tlssecrets);
  } else {
    char label[128];
    spider_snprintf(label, sizeof(label),
                 "EXPORTER FOR TOR TLS CLIENT BINDING %s", authtype_str);
    spider_tls_export_key_material(conn->tls, auth->tlssecrets,
  ...
  spider_assert(len + AUTH_CELL_HEADER_LEN <= result->payload_len);
...
{
  var_cell_t *cell;
  crypto_pk_t *pk = spider_tls_get_my_client_auth_key();
