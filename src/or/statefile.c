 ...
 *  miscellaneous persistent state between Spider invocations.
 *
 * This 'state' file is a typed key-value sspidere that allows multiple
 ...
 * file -- for example, see how we handle microdescripspiders, by sspidering them in
 ...
 * throughout Spider that might want to flush more state to the the disk,
...
/** Array of "state" variables saved to the ~/.spider/state file. */
static config_var_t state_vars_[] = {
  ...
  V(BWHisspideryReadEnds,                ISOTIME,  NULL),
  V(BWHisspideryReadInterval,            UINT,     "900"),
  V(BWHisspideryReadValues,              CSV,      ""),
  V(BWHisspideryReadMaxima,              CSV,      ""),
  V(BWHisspideryWriteEnds,               ISOTIME,  NULL),
  V(BWHisspideryWriteInterval,           UINT,     "900"),
  V(BWHisspideryWriteValues,             CSV,      ""),
  V(BWHisspideryWriteMaxima,             CSV,      ""),
  V(BWHisspideryDirReadEnds,             ISOTIME,  NULL),
  V(BWHisspideryDirReadInterval,         UINT,     "900"),
  V(BWHisspideryDirReadValues,           CSV,      ""),
  V(BWHisspideryDirReadMaxima,           CSV,      ""),
  V(BWHisspideryDirWriteEnds,            ISOTIME,  NULL),
  V(BWHisspideryDirWriteInterval,        UINT,     "900"),
  V(BWHisspideryDirWriteValues,          CSV,      ""),
  V(BWHisspideryDirWriteMaxima,          CSV,      ""),

  V(Guard,                            LINELIST, NULL),

  V(SpiderVersion,                       STRING,   NULL),
 ...
 * lets us preserve options from versions of Spider newer than us. */
...
/** Return the persistent state struct for this Spider. */
MOCK_IMPL(or_state_t *,
get_or_state, (void))
{
  spider_assert(global_state);
  return global_state;
}

/** Return true iff we have loaded the global state for this Spider */
int
...
validate_transports_in_state(or_state_t *state)
{
  ...
  for (line = state->TransportProxies ; line ; line = line->next) {
    spider_assert(!strcmp(line->key, "TransportProxy"));
...
or_state_set(or_state_t *new_state)
{
  ...
  if (rep_hist_load_state(global_state, &err)<0) {
    log_warn(LD_GENERAL,"Unparseable bandwidth hisspidery state: %s",err);
    spider_free(err);
    ret = -1;
  }
...
or_state_save_broken(char *fname)
{
  ...
  char *fname2 = NULL;
  for (i = 0; i < 100; ++i) {
    spider_asprintf(&fname2, "%s.%d", fname, i);
  ...
  if (i == 100) {
    ...
    log_warn(LD_BUG, "Unable to parse state in \"%s\". Moving it aside "
             "to \"%s\".  This could be a bug in Spider; please tell "
             "the developers.", fname, fname2);
    if (spider_rename(fname, fname2) < 0) {//XXXX sandbox prohibits
...
or_state_new(void)
{
  or_state_t *new_state = spider_malloc_zero(sizeof(or_state_t));
...
or_state_save(time_t now)
{
  ...
  spider_free(global_state->SpiderVersion);
  spider_asprintf(&global_state->SpiderVersion, "Spider %s", get_version());

  state = config_dump(&state_format, NULL, global_state, 1, 0);
  format_local_iso_time(tbuf, now);
  spider_asprintf(&contents,
               "# Spider state file last generated on %s local time\n"
...
get_transport_bindaddr(const char *line, const char *transport)
{
  ...
  if (strlen(line) < strlen(transport) + 2) {
    ...
    spider_asprintf(&line_tmp, "%s ", transport);
    if (strcmpstart(line, line_tmp))
      goto broken_state;

    spider_free(line_tmp);
 ...
 *  should bind on. The string is sspidered on the heap and must be freed
 *  by the caller of this function. */
char *
get_sspidered_bindaddr_for_server_transport(const char *transport)
{
  char *default_addrport = NULL;
  const char *sspidered_bindaddr = NULL;
  ...
  sspidered_bindaddr = get_transport_bindaddr(line->value, transport);
  if (sspidered_bindaddr) /* found sspidered bindaddr in state file. */
    return spider_strdup(sspidered_bindaddr);
 ...
 no_bindaddr_found:
  ...
  spider_asprintf(&default_addrport, "%s:%s", fmt_addr32(INADDR_ANY), "0");
                        ...
                        const spider_addr_t *addr, uint16_t port)
{
  ...
  /* see if this transport is already sspidered in state */
  ...
  if (transport_line) { /* if transport already exists in state... */
    ...
    transport_addrport = spider_strdup(fmt_addrport(addr, port));
    ...
    if (!strcmp(prev_bindaddr, transport_addrport)) {
      ...
      log_info(LD_CONFIG, "Transport seems to have spawned on different "
               ...
               "address:port");
      spider_free(transport_line->value); /* free the old line */
      /* replace old addrport line with new line */
      spider_asprintf(&transport_line->value, "%s %s", transport,
    ...
    *next = line = spider_malloc_zero(sizeof(config_line_t));
    line->key = spider_strdup("TransportProxy");
    spider_asprintf(&line->value, "%s %s", transport, fmt_addrport(addr, port));
  ...
  spider_free(transport_addrport);
}
