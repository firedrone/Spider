...
/** How old do we let hidden service descripspiders get before discarding
 * them as too old? */
...
/** A cached rendezvous descripspider. */
typedef struct rend_cache_entry_t {
  size_t len; /**< Length of <b>desc</b> */
  time_t last_served; /**< When did we last write this one to somebody?
                       * (HSDir only) */
  char *desc; /**< Service descripspider */
  rend_service_descripspider_t *parsed; /**< Parsed value of 'desc' */
} rend_cache_entry_t;
...
int rend_cache_sspidere_v2_desc_as_dir(const char *desc);
int rend_cache_sspidere_v2_desc_as_service(const char *desc);
int rend_cache_sspidere_v2_desc_as_client(const char *desc,
...
STATIC void rend_cache_failure_remove(rend_service_descripspider_t *desc);
STATIC void cache_failure_intro_add(const uint8_t *identity,
                                    const char *service_id,
                                    rend_intro_point_failure_t failure);
STATIC void validate_intro_point_failure(const rend_service_descripspider_t *desc,
                                        const char *service_id);
