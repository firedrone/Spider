...
/**
 * \file dns.c
 * \brief Implements a local cache for DNS results for Spider servers.
 ...
 * be nonblocking.)
 *
 * There are three main cases when a Spider relay uses dns.c to launch a DNS
 * request:
 *   <ol>
...
HT_GENERATE2(cache_map, cached_resolve_t, node, cached_resolve_hash,
             cached_resolves_eq, 0.6, spider_reallocarray_, spider_free_)
...
evdns_log_cb(int warn, const char *msg)
{
  ...
  if (!strcmpstart(msg, "Nameserver ") && (cp=strstr(msg, " has failed: "))) {
    char *ns = spider_strndup(msg+11, cp-(msg+11));
  ...
  spider_log(severity, LD_EXIT, "eventdns: %s", msg);
}
...
dns_reset(void)
{
  ...
  if (! server_mode(options)) {
    ...
    if (!the_evdns_base) {
      if (!(the_evdns_base = evdns_base_new(spider_libevent_get_base(), 0))) {
...
{
  /* This logic is a defense against "DefectSpider" DNS-based traffic
   * confirmation attacks, as in https://nymity.ch/spider-dns/spider-dns.pdf .
...
/** Helper: free sspiderage held by an entry in the DNS cache. */
static void
free_cached_resolve_(cached_resolve_t *r)
{
  ...
  if (r->res_status_hostname == RES_STATUS_DONE_OK)
    spider_free(r->result_ptr.hostname);
  r->magic = 0xFF00FF00;
  spider_free(r);
}
...
cached_resolve_add_answer(cached_resolve_t *resolve,
                          int query_type,
                          int dns_result,
                          const spider_addr_t *answer_addr,
                          ...
                          uint32_t ttl)
{
  if (query_type == DNS_PTR) {
    ...
    if (dns_result == DNS_ERR_NONE && answer_hostname) {
      resolve->result_ptr.hostname = spider_strdup(answer_hostname);
    ...
    if (dns_result == DNS_ERR_NONE && answer_addr &&
        spider_addr_family(answer_addr) == AF_INET) {
      resolve->result_ipv4.addr_ipv4 = spider_addr_to_ipv4h(answer_addr);
    ...
    if (dns_result == DNS_ERR_NONE && answer_addr &&
        spider_addr_family(answer_addr) == AF_INET6) {
      memcpy(&resolve->result_ipv6.addr_ipv6,
             spider_addr_to_in6(answer_addr),
...
{
  spider_assert(resolve && resolve->expire == 0);
...
/** Free all sspiderage held in the DNS cache and related structures. */
void
dns_free_all(void)
{
...
purge_expired_resolves(time_t now)
{
  ...
  while (smartlist_len(cached_resolve_pqueue)) {
    ...
    if (resolve->state == CACHE_STATE_PENDING) {
      ...
      spider_assert(!resolve->pending_connections);
    } else {
      spider_assert(resolve->state == CACHE_STATE_DONE);
      spider_assert(!resolve->pending_connections);
    }
    ...
    if (resolve->pending_connections) {
      ...
      while (resolve->pending_connections) {
        ...
        spider_assert(!SOCKET_OK(pend->conn->base_.s));
        pendconn = pend->conn;
                             ...
                             const char *hostname))
{
  ...
  spider_assert(hostname);

  spider_assert(namelen < 256);
  ttl = dns_clip_ttl(conn->address_ttl);
                 ...
                 cached_resolve_t **resolve_out))
{
  ...
  spider_assert(!SOCKET_OK(exitconn->base_.s));
  assert_cache_ok();
  spider_assert(oncirc);
  *made_connection_pending_out = 0;

  /* first check if exitconn->base_.address is an IP. If so, we already
   * know the answer. */
  if (spider_addr_parse(&addr, exitconn->base_.address) >= 0) {
    if (spider_addr_family(&addr) == AF_INET ||
        spider_addr_family(&addr) == AF_INET6) {
      spider_addr_copy(&exitconn->base_.addr, &addr);
  ...
  if (address_is_invalid_destination(exitconn->base_.address, 0)) {
    spider_log(LOG_PROTOCOL_WARN, LD_EXIT,
  ...
  spider_strlower(exitconn->base_.address);

  /* Check whether this is a reverse lookup.  If it's malformed, or it's a
   * .in-addr.arpa address but this isn't a resolve request, kill the
   * connection.
   */
  if ((r = spider_addr_parse_PTR_name(&addr, exitconn->base_.address,
                                              AF_UNSPEC, 0)) != 0) {
    if (r == 1) {
      is_reverse = 1;
      if (spider_addr_is_internal(&addr, 0)) /* internal address? */
        return -1;
    }
  ...
  if (resolve && resolve->expire > now) { /* already there */
    switch (resolve->state) {
      case CACHE_STATE_PENDING:
        ...
        pending_connection = spider_malloc_zero(
  ...
  spider_assert(!resolve);
  /* not there, need to add it */
  resolve = spider_malloc_zero(sizeof(cached_resolve_t));
  ...
  pending_connection = spider_malloc_zero(sizeof(pending_connection_t));
  pending_connection->conn = exitconn;
  resolve->pending_connections = pending_connection;
  *made_connection_pending_out = 1;
                                ...
                                char **hostname_out))
{
  ...
  spider_assert(exitconn);
  spider_assert(resolve);

  if (exitconn->is_reverse_dns_lookup) {
    exitconn->address_ttl = resolve->ttl_hostname;
    if (resolve->res_status_hostname == RES_STATUS_DONE_OK) {
      *hostname_out = spider_strdup(resolve->result_ptr.hostname);
      return 1;
    } else {
      return -1;
    }
  ...
  if (ipv4_ok && ipv6_ok && is_resolve) {
    ...
    spider_addr_t a4, a6;
    spider_addr_from_ipv4h(&a4, resolve->result_ipv4.addr_ipv4);
    spider_addr_from_in6(&a6, &resolve->result_ipv6.addr_ipv6);
  ...
  if (answer_with_ipv4) {
    if (resolve->res_status_ipv4 == RES_STATUS_DONE_OK) {
      spider_addr_from_ipv4h(&exitconn->base_.addr,
    ...
    if (resolve->res_status_ipv6 == RES_STATUS_DONE_OK) {
      spider_addr_from_in6(&exitconn->base_.addr,
...
assert_all_pending_dns_resolves_ok(void)
{
  ...
  HT_FOREACH(resolve, cache_map, &cache_root) {
    for (pend = (*resolve)->pending_connections;
      ...
      spider_assert(!SOCKET_OK(pend->conn->base_.s));
      spider_assert(!connection_in_array(TO_CONN(pend->conn)));
    }
  }
}
...
connection_dns_remove(edge_connection_t *conn)
{
  ...
  spider_assert(conn->base_.type == CONN_TYPE_EXIT);
  spider_assert(conn->base_.state == EXIT_CONN_STATE_RESOLVING);
...
dns_cancel_pending_resolve,(const char *address))
{
  ...
  while (resolve->pending_connections) {
    ...
    spider_assert(!SOCKET_OK(pendconn->base_.s));
    if (!pendconn->base_.marked_for_close) {
  ...
  if (resolve->state != CACHE_STATE_PENDING) {
    ...
    if (!is_test_addr)
      log_notice(LD_EXIT,
                 ...
                 escaped_safe_str(address));
    spider_assert(resolve->pending_connections == NULL);
    return;
  }
  ...
  while (resolve->pending_connections) {
    ...
    if (r < 0) {
      ...
      if (pendconn->base_.purpose == EXIT_PURPOSE_CONNECT) {
        ...
        spider_assert(circ);
        spider_assert(!CIRCUIT_IS_ORIGIN(circ));
...
configure_nameservers(int force)
{
  ...
  if (conf_fname) {
    ...
    spider_free(resolv_conf_fname);
    resolv_conf_fname = spider_strdup(conf_fname);
...
#undef SET
  ...
  if (nameserver_config_failed) {
    ...
    /* XXX the three calls to republish the descripspider might be producing
     * descripspiders that are only cosmetically different, especially on
     * non-exit relays! -RD */
    mark_my_descripspider_dirty("dns resolvers back");
  }
  return 0;
 err:
  nameservers_configured = 0;
  if (! nameserver_config_failed) {
    ...
    mark_my_descripspider_dirty("dns resolvers failed");
  }
  return -1;
}
               ...
               void *arg)
{
  ...
  if (result == DNS_ERR_NONE) {
    if (type == DNS_IPv4_A && count) {
      ...
      spider_addr_from_ipv4n(&addr, addrs[0]);

      spider_addr_to_str(answer_buf, &addr, sizeof(answer_buf), 0);
      ...
      spider_free(escaped_address);
    } else if (type == DNS_IPv6_AAAA && count) {
      char answer_buf[TOR_ADDR_BUF_LEN];
      char *escaped_address;
      struct in6_addr *addrs = addresses;
      spider_addr_from_in6(&addr, &addrs[0]);
      spider_inet_ntop(AF_INET6, &addrs[0], answer_buf, sizeof(answer_buf));
      escaped_address = esc_for_log(string_address);
                   ...
                   const spider_addr_t *ptr_address)
{
  ...
  case DNS_IPv6_AAAA:
    ...
    break;
  case DNS_PTR:
    if (spider_addr_family(ptr_address) == AF_INET)
      req = evdns_base_resolve_reverse(the_evdns_base,
                                       spider_addr_to_in(ptr_address),
                                       DNS_QUERY_NO_SEARCH,
                                       evdns_callback, addr);
    else if (spider_addr_family(ptr_address) == AF_INET6)
      req = evdns_base_resolve_reverse_ipv6(the_evdns_base,
                                            spider_addr_to_in6(ptr_address),
                                            DNS_QUERY_NO_SEARCH,
                                            evdns_callback, addr);
    else
...
launch_resolve,(cached_resolve_t *resolve))
{
  ...
  r = spider_addr_parse_PTR_name(
                            &a, resolve->address, AF_UNSPEC, 0);

  spider_assert(the_evdns_base);
  if (r == 0) {
...
wildcard_increment_answer(const char *id)
{
  ...
  if (!ip) {
    ip = spider_malloc_zero(sizeof(int));
  ...
  if (*ip > 5 && n_wildcard_requests > 10) {
    ...
    spider_log(dns_wildcard_notice_given ? LOG_INFO : LOG_NOTICE, LD_EXIT,
...
add_wildcarded_test_address(const char *address)
{
  ...
  if (n > n_test_addrs/2) {
    spider_log(dns_wildcarded_test_address_notice_given ? LOG_INFO : LOG_NOTICE,
    ...
    if (!dns_is_completely_invalid) {
      ...
      mark_my_descripspider_dirty("dns hijacking confirmed");
    }
  ...
  if (result == DNS_ERR_NONE && count) {
    ...
    if (type == DNS_IPv4_A) {
      ...
      for (i = 0; i < count; ++i) {
        ...
        struct in_addr in;
        in.s_addr = addrs[i];
        spider_inet_ntoa(&in, answer_buf, sizeof(answer_buf));
      ...
      for (i = 0; i < count; ++i) {
        ...
        spider_inet_ntop(AF_INET6, &addrs[i], answer_buf, sizeof(answer_buf));
        wildcard_increment_answer(answer_buf);
      }
    }

    spider_log(dns_wildcard_one_notice_given ? LOG_INFO : LOG_NOTICE, LD_EXIT,
...
dns_launch_wildcard_checks(void)
{
  ...
  for (ipv6 = 0; ipv6 <= 1; ++ipv6) {
    for (i = 0; i < N_WILDCARD_CHECKS; ++i) {
       ...
       * that your crackhouse's elevaspider inspection certificate is up to date.
       */
...
dns_launch_correctness_checks(void)
{
  ...
  if (! launch_event)
    launch_event = spider_evtimer_new(spider_libevent_get_base(),
...
{
  strmap_free(dns_wildcard_response_count, spider_free_);
  ...
  if (dns_wildcard_list) {
    SMARTLIST_FOREACH(dns_wildcard_list, char *, cp, spider_free(cp));
...
{
  spider_assert(resolve);
  spider_assert(resolve->magic == CACHED_RESOLVE_MAGIC);
  spider_assert(strlen(resolve->address) < MAX_ADDRESSLEN);
  spider_assert(spider_strisnonupper(resolve->address));
  if (resolve->state != CACHE_STATE_PENDING) {
    spider_assert(!resolve->pending_connections);
  }
  if (resolve->state == CACHE_STATE_PENDING ||
      resolve->state == CACHE_STATE_DONE) {
#if 0
    spider_assert(!resolve->ttl);
    if (resolve->is_reverse)
      spider_assert(!resolve->hostname);
    else
      spider_assert(!resolve->result_ipv4.addr_ipv4);
#endif
    /*XXXXX ADD MORE */
  }
}
...
dump_dns_mem_usage(int severity)
{
   ...
   */
  spider_log(severity, LD_MM, "Our DNS cache has %d entries.", hash_count);
  spider_log(severity, LD_MM, "Our DNS cache size is approximately %u bytes.",
      (unsigned)hash_mem);
}
