                      ...
                      time_t now)
{
  ...
  if (purpose == CIRCUIT_PURPOSE_C_GENERAL) {
    spider_addr_t addr;
    const int family = spider_addr_parse(&addr, conn->socks_request->address);
    ...
    if (build_state->onehop_tunnel) {
      ...
      spider_assert(conn->chosen_exit_name);
      if (build_state->chosen_exit) {
        ...
        if (spider_memneq(digest, build_state->chosen_exit->identity_digest,
                          DIGEST_LEN))
    ...
    if (origin_circ->prepend_policy && family != -1) {
      int r = compare_spider_addr_to_addr_policy(&addr,
                 ...
                 int need_uptime, int need_internal)
{
  ...
  spider_assert(purpose == CIRCUIT_PURPOSE_C_GENERAL ||
 ...
 * Ignores Single Onion Service intro and Spider2web redezvous circuits, they are
...
#define MAX_ANCIENT_ONEHOP_CIRCUITS_TO_LOG 10
  ...
  SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ) {
    ...
    if (rend_service_allow_non_anonymous_connection(options) &&
      ...
      continue;
    /* Spider2web deliberately makes long term one-hop rend connections,
     * particularly when Spider2webRendezvousPoints is used. We only ignore
  ...
  SMARTLIST_FOREACH_BEGIN(log_these, const origin_circuit_t *, ocirc) {
    ...
    if (circ->timestamp_dirty) {
      ...
      spider_asprintf(&dirty, "Dirty since %s (%ld seconds vs %ld-second cutoff)",
                   dirty_since, (long)(now - circ->timestamp_dirty),
                   (long) options->MaxCircuitDirtiness);
    } else {
      dirty = spider_strdup("Not marked dirty");
    }
    ...
    if (CIRCUIT_IS_ORIGIN(circ) &&
      ...
      if (exitnode && (!need_uptime || build_state->need_uptime)) {
        ...
        if (conn) {
          ...
          r = compare_spider_addr_to_node_policy(NULL, port, exitnode);
  ...
  if (CIRCUIT_IS_ORIGIN(circ)) {
    ...
    if (removed) {
      ...
      if (circ->purpose == CIRCUIT_PURPOSE_S_REND_JOINED) {
        spider_assert(origin_circ->rend_data);
...
/* Do we have enough descripspiders to build paths?
                              ...
                              int flags)
{
  ...
  /* If Spider2webRendezvousPoints is enabled and we are dealing with an
     RP circuit, we want a specific RP node so we shouldn't canibalize
     an already existing circuit. */
  if (get_options()->Spider2webRendezvousPoints &&
                                ...
                                origin_circuit_t **circp)
{
  ...
  spider_assert(ENTRY_TO_CONN(conn)->state == AP_CONN_STATE_CIRCUIT_WAIT);
  ...
  if (check_exit_policy) {
    if (!conn->chosen_exit_name) {
      ...
      if (spider_inet_aton(conn->socks_request->address, &in)) {
        spider_addr_from_in(&addr, &in);
        addrp = &addr;
      }
      if (router_exit_policy_all_nodes_reject(addrp,
                                              ...
                                              need_uptime)) {
        log_notice(LD_APP,
                   "No Spider server allows exit to %s:%d. Rejecting.",
       ...
       * refacspider into a single function. */
      ...
      if (node && !connection_ap_can_use_exit(conn, node)) {
        ...
        if (opt) {
          ...
          spider_free(conn->chosen_exit_name);
          /* Try again. */
  ...
  if (!circ) {
    ...
    if (desired_circuit_purpose == CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT) {
      ...
      if (!extend_info) {
        log_info(LD_REND,
                 "No intro points for '%s': re-fetching service descripspider.",
    ...
    if (desired_circuit_purpose == CIRCUIT_PURPOSE_C_GENERAL) {
      if (conn->chosen_exit_name) {
        ...
        if (r && node_has_descripspider(r)) {
          /* We might want to connect to an IPv6 bridge for loading
             descripspiders so we use the preferred address rather than
             the primary. */
        ...
        } else  { /* ! (r && node_has_descripspider(r)) */
          ...
          if (want_onehop && conn->chosen_exit_name[0] == '$') {
            ...
            if (spider_addr_parse(&addr, conn->socks_request->address) < 0) {
    ...
    /* If spider2Web is on, then hidden service requests should be one-hop.
     */
                    ...
                    crypt_path_t *cpath)
{
  ...
  if (cpath) { /* we were given one; use it */
    spider_assert(cpath_is_on_circuit(circ, cpath));
  } else {
    /* use the last hop in the circuit */
    spider_assert(circ->cpath);
    spider_assert(circ->cpath->prev);
    spider_assert(circ->cpath->prev->state == CPATH_STATE_OPEN);
                             ...
                             const origin_circuit_t *circ)
{
  ...
  spider_asprintf(&new_address, "%s.%s.exit",
                                              ...
                                              crypt_path_t *cpath)
{
  ...
  spider_assert(conn->socks_request);
  spider_assert(circ);
  spider_assert(circ->base_.state == CIRCUIT_STATE_OPEN);
...
connection_ap_handshake_attach_circuit(entry_connection_t *conn)
{
  ...
  if (conn_age >= get_options()->SocksTimeout) {
    int severity = (spider_addr_is_null(&base_conn->addr) && !base_conn->port) ?
      LOG_INFO : LOG_NOTICE;
  ...
  if (!connection_edge_is_rendezvous_stream(ENTRY_TO_EDGE_CONN(conn))) {
    ...
    spider_assert(!ENTRY_TO_EDGE_CONN(conn)->cpath_layer);
     ...
     * re-fetch the descripspider.
     */
    if (ENTRY_TO_CONN(conn)->state != AP_CONN_STATE_CIRCUIT_WAIT) {
      log_info(LD_REND, "This connection is no longer ready to attach; its "
               "state changed."
               "(We probably have to re-fetch its descripspider.)");
      return 0;
    }
...
mark_circuit_unusable_for_new_conns(origin_circuit_t *circ)
{
   ...
   * deeper refacspidering of our expiration logic. */
  if (! circ->base_.timestamp_dirty)
    circ->base_.timestamp_dirty = approx_time();
