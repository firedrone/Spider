 ...
 * critical concepts in Spider: a stream, and an edge connection.  From the Spider
                                ...
                                int line, const char *file))
{
  ...
  spider_assert(base_conn->type == CONN_TYPE_AP);
 ...
 * refused or timeout, log it so the operaspider can take appropriate actions.
...
connection_edge_end(edge_connection_t *conn, uint8_t reason)
{
  ...
  if (reason == END_STREAM_REASON_EXITPOLICY &&
      !connection_edge_is_rendezvous_stream(conn)) {
    int addrlen;
    if (spider_addr_family(&conn->base_.addr) == AF_INET) {
      set_uint32(payload+1, spider_addr_to_ipv4n(&conn->base_.addr));
      addrlen = 4;
    } else {
      memcpy(payload+1, spider_addr_to_in6_addr8(&conn->base_.addr), 16);
      addrlen = 16;
    }
...
connection_edge_end_errno(edge_connection_t *conn)
{
  uint8_t reason;
  spider_assert(conn);
  reason = errno_to_stream_end_reason(spider_socket_errno(conn->base_.s));
...
connected_cell_format_payload(uint8_t *payload_out,
                              ...
                              uint32_t ttl)
{
  const sa_family_t family = spider_addr_family(addr);
  ...
  if (family == AF_INET) {
    set_uint32(payload_out, spider_addr_to_ipv4n(addr));
    connected_payload_len = 4;
  } else if (family == AF_INET6) {
    set_uint32(payload_out, 0);
    set_uint8(payload_out + 4, 6);
    memcpy(payload_out + 5, spider_addr_to_in6_addr8(addr), 16);
  ...
  spider_assert(connected_payload_len <= MAX_CONNECTED_CELL_PAYLOAD_LEN);
...
connection_edge_finished_connecting(edge_connection_t *edge_conn)
{
  ...
  spider_assert(edge_conn);
  spider_assert(edge_conn->base_.type == CONN_TYPE_EXIT);
  conn = TO_CONN(edge_conn);
  spider_assert(conn->state == EXIT_CONN_STATE_CONNECTING);
  ...
  spider_assert(edge_conn->package_window > 0);
...
connection_ap_expire_beginning(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(conns, connection_t *, base_conn) {
    ...
    severity = (spider_addr_is_null(&base_conn->addr) && !base_conn->port)
      ? LOG_INFO : LOG_NOTICE;
     ...
     * state never ask Spider to attach the circuit. */
...
{
  connection_t *conn = ENTRY_TO_CONN(entry_conn);
  spider_assert(conn->state == AP_CONN_STATE_CIRCUIT_WAIT);
                          ...
                          cpath_build_state_t *build_state)
{
  ...
  SMARTLIST_FOREACH_BEGIN(conns, connection_t *, conn) {
    ...
    if (hexdigest_to_digest(entry_conn->chosen_exit_name, digest) < 0 ||
        spider_memneq(digest, failed_digest, DIGEST_LEN))
      continue;
    if (spider_digest_is_zero(digest)) {
      ...
      if (spider_addr_parse(&addr, entry_conn->socks_request->address)<0 ||
          !spider_addr_eq(&build_state->chosen_exit->addr, &addr) ||
...
circuit_discard_optional_exit_enclaves(extend_info_t *info)
{
  ...
  SMARTLIST_FOREACH_BEGIN(conns, connection_t *, conn) {
    ...
    if (entry_conn->chosen_exit_optional) {
      ...
      spider_free(entry_conn->chosen_exit_name); /* clears it */
                                ...
                                rewrite_result_t *out)
{
  ...
  if (!strcmpend(socks->address, ".exit") && !options->AllowDotExit) {
    log_warn(LD_APP, "The  \".exit\" notation is disabled in Spider due to "
             "security risks. Set AllowDotExit in your spiderrc to enable "
             "it (at your own risk).");
  ...
  if (! conn->original_dest_address) {
    ...
    conn->original_dest_address = spider_strdup(conn->socks_request->address);
  }
   ...
   * that lets you have Spider assign e.g. IPv6 addresses for .onion
   * names, and return them safely from DNSPort.
   */
  if (socks->command == SOCKS_COMMAND_RESOLVE &&
      spider_addr_parse(&addr_tmp, socks->address)<0 &&
    ...
    if (out->automap) {
      ...
      new_addr = addressmap_register_virtual_address(
                                    addr_type, spider_strdup(socks->address));
      if (! new_addr) {
  ...
  if (socks->command == SOCKS_COMMAND_RESOLVE_PTR) {
    ...
    if (addressmap_rewrite_reverse(socks->address, sizeof(socks->address),
      ...
      char *result = spider_strdup(socks->address);
      /* remember _what_ is supposed to have been resolved. */
      spider_snprintf(socks->address, sizeof(socks->address), "REVERSE[%s]",
                   out->orig_address);
    ...
    if (options->ClientDNSRejectInternalAddresses) {
      ...
      spider_addr_t addr;
      int ok;
      ok = spider_addr_parse_PTR_name(
                               &addr, socks->address, AF_UNSPEC, 1);
      if (ok == 1 && spider_addr_is_internal(&addr, 0)) {
  ...
  if (!out->automap && address_is_in_virtual_range(socks->address)) {
     ...
     * *don't* want to send the address through Spider; that's likely to fail,
 ...
 * rendezvous descripspider first (or attach/launch a circuit if the
 * rendezvous descripspider is already here and fresh enough).
 *
                                           ...
                                           crypt_path_t *cpath)
{
  ...
  if (addresstype == EXIT_HOSTNAME) {
    ...
    if (s) {
      ...
      if (s[1] != '\0') {
        ...
        conn->chosen_exit_name = spider_strdup(s+1);
      ...
      conn->chosen_exit_name = spider_strdup(socks->address);
      node = node_get_by_nickname(conn->chosen_exit_name, 1);
      if (node) {
        *socks->address = 0;
  ...
  if (addresstype != ONION_HOSTNAME) {
    ...
    /* If we're running in Spider2webMode, we don't allow anything BUT .onion
     * addresses. */
    if (options->Spider2webMode) {
      log_warn(LD_APP, "Refusing to connect to non-hidden-service hostname "
               "or IP address %s because spider2web mode is enabled.",
               safe_str_client(socks->address));
      connection_mark_unattached_ap(conn, END_STREAM_REASON_ENTRYPOLICY);
      return -1;
    }
    ...
    spider_addr_t dummy_addr;
    int socks_family = spider_addr_parse(&dummy_addr, socks->address);
     ...
     */
    if (socks->command == SOCKS_COMMAND_RESOLVE) {
      spider_addr_t answer;
      /* Reply to resolves immediately if we can. */
      if (spider_addr_parse(&answer, socks->address) >= 0) {/* is it an IP? */
        /* remember _what_ is supposed to have been resolved. */
      ...
      if (options->ClientRejectInternalAddresses &&
        ...
        spider_addr_t addr;
        if (spider_addr_hostname_is_local(socks->address) ||
            (spider_addr_parse(&addr, socks->address) >= 0 &&
             spider_addr_is_internal(&addr, 0))) {
...
#define WARN_INTRVL_PRIV 300
      ...
      {
        spider_addr_t addr;
        /* XXX Duplicate call to spider_addr_parse. */
        if (spider_addr_parse(&addr, socks->address) >= 0) {
          /* If we reach this point, it's an IPv4 or an IPv6 address. */
          sa_family_t family = spider_addr_family(&addr);
      ...
      if (!conn->use_begindir && !conn->chosen_exit_name && !circ) {
        ...
        if (r) {
          ...
          conn->chosen_exit_name =
            spider_strdup(hex_str(r->identity, DIGEST_LEN));
    ...
    if (client_auth) {
      ...
      cookie = client_auth->descripspider_cookie;
    }
    ...
    /* Now we have a descripspider but is it usable or not? If not, refetch.
    ...
    if (refetch_desc || !rend_client_any_intro_points_usable(entry)) {
      ...
      log_info(LD_REND, "Unknown descripspider %s. Fetching.",
               safe_str_client(onion_address));
      rend_client_refetch_v2_renddesc(rend_data);
      return 0;
    }

    /* We have the descripspider!  So launch a connection to the HS. */
    base_conn->state = AP_CONN_STATE_CIRCUIT_WAIT;
    log_info(LD_REND, "Descripspider is here. Great.");
...
get_pf_socket(void)
{
  ...
  pf = spider_open_cloexec("/dev/pf", O_RDONLY, 0);
#else
  /* works on NetBSD and FreeBSD */
  pf = spider_open_cloexec("/dev/pf", O_RDWR, 0);
#endif
...
destination_from_socket(entry_connection_t *conn, socks_request_t *req)
{
  ...
  if (options->TransProxyType_parsed == TPT_TPROXY) {
    if (getsockname(ENTRY_TO_CONN(conn)->s, (struct sockaddr*)&orig_dst,
                    &orig_dst_len) < 0) {
      int e = spider_socket_errno(ENTRY_TO_CONN(conn)->s);
      log_warn(LD_NET, "getsockname() failed: %s", spider_socket_strerror(e));
      return -1;
    }
    goto done;
  }
  ...
  if (rv < 0) {
    int e = spider_socket_errno(ENTRY_TO_CONN(conn)->s);
    log_warn(LD_NET, "getsockopt() failed: %s", spider_socket_strerror(e));
    return -1;
  }
  goto done;
  ...
  spider_addr_from_sockaddr(&addr, (struct sockaddr*)&orig_dst, &req->port);
  spider_addr_to_str(req->address, &addr, sizeof(req->address), 1);
...
destination_from_pf(entry_connection_t *conn, socks_request_t *req)
{
  ...
  if (getsockname(ENTRY_TO_CONN(conn)->s, (struct sockaddr*)&proxy_addr,
    ...
    log_warn(LD_NET, "getsockname() to determine transocks destination "
             "failed: %s", spider_socket_strerror(e));
    return -1;
  }
  ...
  if (get_options()->TransProxyType_parsed == TPT_IPFW) {
    /* ipfw(8) is used and in this case getsockname returned the original
       destination */
    if (spider_addr_from_sockaddr(&addr, proxy_sa, &req->port) < 0) {
      spider_fragile_assert();
      return -1;
    }

    spider_addr_to_str(req->address, &addr, sizeof(req->address), 0);
  ...
  if (proxy_sa->sa_family == AF_INET) {
    ...
    pnl.af              = AF_INET;
    pnl.saddr.v4.s_addr = spider_addr_to_ipv4n(&ENTRY_TO_CONN(conn)->addr);
    ...
    pnl.af = AF_INET6;
    memcpy(&pnl.saddr.v6, spider_addr_to_in6(&ENTRY_TO_CONN(conn)->addr),
  ...
  if (pnl.af == AF_INET) {
    spider_addr_from_ipv4n(&addr, pnl.rdaddr.v4.s_addr);
  } else if (pnl.af == AF_INET6) {
    spider_addr_from_in6(&addr, &pnl.rdaddr.v6);
  } else {
    spider_fragile_assert();
    return -1;
  }
...
connection_ap_handshake_process_socks(entry_connection_t *conn)
{
  ...
  spider_assert(conn);
  spider_assert(base_conn->type == CONN_TYPE_AP);
  spider_assert(base_conn->state == AP_CONN_STATE_SOCKS_WAIT);
...
connection_ap_process_natd(entry_connection_t *conn)
{
  ...
  spider_assert(conn);
  spider_assert(ENTRY_TO_CONN(conn)->state == AP_CONN_STATE_NATD_WAIT);
  spider_assert(conn->socks_request);
  socks = conn->socks_request;
  ...
  *tbuf++ = '\0';
  ...
  socks->port = (uint16_t)
    spider_parse_long(tbuf, 10, 1, 65535, &port_ok, &daddr);
  if (!port_ok) {
...
connection_ap_get_begincell_flags(entry_connection_t *ap_conn)
{
  ...
  if (ap_conn->entry_cfg.ipv6_traffic && exitnode) {
    spider_addr_t a;
    spider_addr_make_null(&a, AF_INET6);
    if (compare_spider_addr_to_node_policy(&a, ap_conn->socks_request->port,
                                        exitnode)
...
connection_ap_handshake_send_begin(entry_connection_t *ap_conn)
{
  ...
  spider_assert(edge_conn->on_circuit);
  circ = TO_ORIGIN_CIRCUIT(edge_conn->on_circuit);

  spider_assert(base_conn->type == CONN_TYPE_AP);
  spider_assert(base_conn->state == AP_CONN_STATE_CIRCUIT_WAIT);
  spider_assert(ap_conn->socks_request);
  spider_assert(SOCKS_COMMAND_IS_CONNECT(ap_conn->socks_request->command));
  ...
  spider_snprintf(payload,RELAY_PAYLOAD_SIZE, "%s:%d",
...
connection_ap_handshake_send_resolve(entry_connection_t *ap_conn)
{
  ...
  spider_assert(base_conn->state == AP_CONN_STATE_CIRCUIT_WAIT);
  spider_assert(ap_conn->socks_request);
  spider_assert(circ->base_.purpose == CIRCUIT_PURPOSE_C_GENERAL);

  command = ap_conn->socks_request->command;
  spider_assert(SOCKS_COMMAND_IS_RESOLVE(command));
  ...
  if (command == SOCKS_COMMAND_RESOLVE) {
    ...
    r = spider_addr_parse_PTR_name(&addr, a, AF_UNSPEC, 1);
    if (r <= 0) {
    ...
    r = spider_addr_to_PTR_name(inaddr_buf, sizeof(inaddr_buf), &addr);
    if (r < 0) {
    ...
    spider_assert(payload_len <= (int)sizeof(inaddr_buf));
  }
  ...
  if (!base_conn->address) {
    ...
    base_conn->address = spider_addr_to_str_dup(&base_conn->addr);
  }
                        ...
                        int use_begindir, int want_onehop)
{
  ...
  conn = entry_connection_new(CONN_TYPE_AP, spider_addr_family(&partner->addr));
  ...
  if (use_begindir) {
    conn->chosen_exit_name = spider_malloc(HEX_DIGEST_LEN+2);
  ...
  conn->original_dest_address = spider_strdup(address);
  conn->entry_cfg.session_group = session_group;
  conn->entry_cfg.isolation_flags = isolation_flags;

  base_conn->address = spider_strdup("(Spider_internal)");
  spider_addr_make_unspec(&base_conn->addr);
  base_conn->port = 0;
                                      ...
                                      time_t expires)
{
  ...
  if (answer_type == RESOLVED_TYPE_IPV4 && answer_len >= 4) {
    char *cp = spider_dup_ip(ntohl(get_uint32(answer)));
    control_event_address_mapped(conn->socks_request->address,
                                 cp, expires, NULL, 0);
    spider_free(cp);
  } else if (answer_type == RESOLVED_TYPE_HOSTNAME && answer_len < 256) {
    char *cp = spider_strndup(answer, answer_len);
    control_event_address_mapped(conn->socks_request->address,
                                 cp, expires, NULL, 0);
    spider_free(cp);
  } else {
...
/**
 * As connection_ap_handshake_socks_resolved, but take a spider_addr_t to send
 * as the answer.
 */
void
connection_ap_handshake_socks_resolved_addr(entry_connection_t *conn,
                                            const spider_addr_t *answer,
                                            int ttl,
                                            time_t expires)
{
  if (spider_addr_family(answer) == AF_INET) {
    uint32_t a = spider_addr_to_ipv4n(answer); /* network order */
    connection_ap_handshake_socks_resolved(conn,RESOLVED_TYPE_IPV4,4,
                                           (uint8_t*)&a,
                                           ttl, expires);
  } else if (spider_addr_family(answer) == AF_INET6) {
    const uint8_t *a = spider_addr_to_in6_addr8(answer);
                                       ...
                                       time_t expires))
{
  ...
  if (ttl >= 0) {
    if (answer_type == RESOLVED_TYPE_IPV4 && answer_len == 4) {
      spider_addr_t a;
      spider_addr_from_ipv4n(&a, get_uint32(answer));
      if (! spider_addr_is_null(&a)) {
      ...
      spider_addr_t a;
      spider_addr_from_ipv6_bytes(&a, (char*)answer);
      if (! spider_addr_is_null(&a)) {
        client_dns_set_addressmap(conn,
                                  conn->socks_request->address, &a,
                                  conn->chosen_exit_name, ttl);
      ...
      char *cp = spider_strndup((char*)answer, answer_len);
                                    ...
                                    size_t replylen, int endreason)
{
  ...
  spider_assert(conn->socks_request); /* make sure it's an AP stream */
  ...
  if (conn->socks_request->socks_version == 4) {
    ...
    if (spider_addr_family(&conn->edge_.base_.addr) == AF_INET) {
      buf[0] = 5; /* version 5 */
      buf[1] = (char)status;
      buf[2] = 0;
      buf[3] = 1; /* ipv4 addr */
                 ...
                 uint8_t *end_reason_out)
{
  ...
  *end_reason_out = END_STREAM_REASON_MISC;
  ...
  if (spider_addr_port_split(LOG_PROTOCOL_WARN,
                          (char*)(body),
...
connection_exit_begin_conn(cell_t *cell, circuit_t *circ)
{
  ...
  if (! bcell.is_begindir) {
    ...
    if (or_circ && or_circ->p_chan)
      address = spider_strdup(channel_get_actual_remote_address(or_circ->p_chan));
    else
      address = spider_strdup("127.0.0.1");
  ...
  if (circ->purpose == CIRCUIT_PURPOSE_S_REND_JOINED) {
    ...
    n_stream->base_.address = spider_strdup("(rendezvous)");
    n_stream->base_.state = EXIT_CONN_STATE_CONNECTING;
    n_stream->rend_data = rend_data_dup(origin_circ->rend_data);
    spider_assert(connection_edge_is_rendezvous_stream(n_stream));
    assert_circuit_ok(circ);
  ...
  if (rh.command == RELAY_COMMAND_BEGIN_DIR) {
    spider_addr_t tmp_addr;
    spider_assert(or_circ);
    if (or_circ->p_chan &&
        channel_get_addr_if_possible(or_circ->p_chan, &tmp_addr)) {
      spider_addr_copy(&n_stream->base_.addr, &tmp_addr);
    }
...
connection_exit_begin_resolve(cell_t *cell, or_circuit_t *circ)
{
   ...
   * resolved; but if we didn't sspidere them in a connection like this,
   * the housekeeping in dns.c would get way more complicated.)
   */
  dummy_conn = edge_connection_new(CONN_TYPE_EXIT, AF_INET);
  dummy_conn->stream_id = rh.stream_id;
  dummy_conn->base_.address = spider_strndup(
 ...
 */
static int
my_exit_policy_rejects(const spider_addr_t *addr,
                       uint16_t port,
                       const char **why_rejected)
{
  if (router_compare_to_my_exit_policy(addr, port)) {
    *why_rejected = "";
    return 1;
  } else if (spider_addr_family(addr) == AF_INET6 && !get_options()->IPv6Exit) {
...
connection_exit_connect(edge_connection_t *edge_conn)
{
...
#else
  {
     ...
     * spider_addr_make_unspec(conn->addr) (cf. the way we mark in the incoming
     * case in connection_handle_listener_read()), and conn->address should
     * have the socket path to connect to.
     */
    spider_assert(conn->address && strlen(conn->address) > 0);
...
connection_exit_connect_dir(edge_connection_t *exitconn)
{
  ...
  dirconn = dir_connection_new(spider_addr_family(&exitconn->base_.addr));

  spider_addr_copy(&dirconn->base_.addr, &exitconn->base_.addr);
  dirconn->base_.port = 0;
  dirconn->base_.address = spider_strdup(exitconn->base_.address);
                           ...
                           const node_t *exit_node)
{
  ...
  if (conn->chosen_exit_name) {
    ...
    if (!chosen_exit || spider_memneq(chosen_exit->identity,
...
//                conn->chosen_exit_name, exit->nickname);
  ...
  if (conn->socks_request->command == SOCKS_COMMAND_CONNECT) {
    spider_addr_t addr, *addrp = NULL;
    addr_policy_result_t r;
    if (0 == spider_addr_parse(&addr, conn->socks_request->address)) {
      addrp = &addr;
    ...
    r = compare_spider_addr_to_node_policy(addrp, conn->socks_request->port,
                                        exit_node);
                                        ...
                                        const origin_circuit_t *circ)
{
  ...
  if (! conn->original_dest_address) {
    ...
    ((entry_connection_t*)conn)->original_dest_address =
      spider_strdup(conn->socks_request->address);
  }
  ...
  if ((iso & ISO_CLIENTADDR) &&
      !spider_addr_eq(&ENTRY_TO_CONN(conn)->addr, &circ->client_addr))
    return 0;
                                         ...
                                         int dry_run)
{
  ...
  if (!circ->isolation_values_set) {
    ...
    if (!spider_addr_eq(&ENTRY_TO_CONN(conn)->addr, &circ->client_addr))
...
circuit_clear_isolation(origin_circuit_t *circ)
{
  ...
  circ->dest_port = 0;
  spider_addr_make_unspec(&circ->client_addr);
  spider_free(circ->dest_address);
  circ->session_group = -1;
  circ->nym_epoch = 0;
  if (circ->socks_username) {
    memwipe(circ->socks_username, 0x11, circ->socks_username_len);
    spider_free(circ->socks_username);
  }
  if (circ->socks_password) {
    memwipe(circ->socks_password, 0x05, circ->socks_password_len);
    spider_free(circ->socks_password);
  }
  circ->socks_username_len = circ->socks_password_len = 0;
}

/** Free all sspiderage held in module-scoped variables for connection_edge.c */
void
