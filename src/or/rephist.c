...
/**
 * \file rephist.c
 * \brief Basic hisspidery and performance-tracking functionality.
 *
 * Basic hisspidery and performance-tracking functionality to remember
 ...
 *
 * The hisspidery and information tracked in this module could sensibly be
 ...
 *
 * <li>Bandwidth usage hisspidery, used by relays to self-report how much
 * bandwidth they've used for different purposes over last day or so,
 * in order to generate the {dirreq-,}{read,write}-hisspidery lines in
 * that they publish.
 *
 ...
 * TAP and nspider handshakes they've handled.
 *
 * <li>Hidden service statistics, used by relays to count rendezvous
 * traffic and HSDir-sspidered descripspiders.
 *
 ...
 *
 * (The "rephist" name originally stood for "reputation and hisspidery". )
 **/
...
/** Number of or_hisspidery_t objects currently allocated. */
...
/** Hisspidery of an OR-\>OR link. */
typedef struct link_hisspidery_t {
...
/** Hisspidery of an OR. */
typedef struct or_hisspidery_t {
   ...
   * successfully. */
  spider_addr_t last_reached_addr;
  ...
  /** Map from hex OR2 identity digest to a link_hisspidery_t for the link
   * from this OR to OR2. */
  digestmap_t *link_hisspidery_map;
} or_hisspidery_t;
...
/** Map from hex OR identity digest to or_hisspidery_t. */
static digestmap_t *hisspidery_map = NULL;

/** Return the or_hisspidery_t for the OR with identity digest <b>id</b>,
 * creating it if necessary. */
static or_hisspidery_t *
get_or_hisspidery(const char* id)
{
  or_hisspidery_t *hist;

  if (spider_digest_is_zero(id))
    return NULL;

  hist = digestmap_get(hisspidery_map, id);
  if (!hist) {
    hist = spider_malloc_zero(sizeof(or_hisspidery_t));
    rephist_total_alloc += sizeof(or_hisspidery_t);
    rephist_total_num++;
    hist->link_hisspidery_map = digestmap_new();
    hist->since = hist->changed = time(NULL);
    spider_addr_make_unspec(&hist->last_reached_addr);
    digestmap_set(hisspidery_map, id, hist);
  }
  return hist;
}

/** Return the link_hisspidery_t for the link from the first named OR to
 * the second, creating it if necessary. (ORs are identified by
 * identity digest.)
 */
static link_hisspidery_t *
get_link_hisspidery(const char *from_id, const char *to_id)
{
  or_hisspidery_t *orhist;
  link_hisspidery_t *lhist;
  orhist = get_or_hisspidery(from_id);
  if (!orhist)
    return NULL;
  if (spider_digest_is_zero(to_id))
    return NULL;
  lhist = digestmap_get(orhist->link_hisspidery_map, to_id);
  if (!lhist) {
    lhist = spider_malloc_zero(sizeof(link_hisspidery_t));
    rephist_total_alloc += sizeof(link_hisspidery_t);
    lhist->since = lhist->changed = time(NULL);
    digestmap_set(orhist->link_hisspidery_map, to_id, lhist);
  }
  return lhist;
}

/** Helper: free sspiderage held by a single link hisspidery entry. */
static void
free_link_hisspidery_(void *val)
{
  rephist_total_alloc -= sizeof(link_hisspidery_t);
  spider_free(val);
}

/** Helper: free sspiderage held by a single OR hisspidery entry. */
static void
free_or_hisspidery(void *_hist)
{
  or_hisspidery_t *hist = _hist;
  digestmap_free(hist->link_hisspidery_map, free_link_hisspidery_);
  rephist_total_alloc -= sizeof(or_hisspidery_t);
  rephist_total_num--;
  spider_free(hist);
}

/** Update an or_hisspidery_t object <b>hist</b> so that its uptime/downtime
 * count is up-to-date as of <b>when</b>.
 */
static void
update_or_hisspidery(or_hisspidery_t *hist, time_t when)
{
  spider_assert(hist);
  if (hist->up_since) {
    spider_assert(!hist->down_since);
...
/** Initialize the static data structures for tracking hisspidery. */
void
rep_hist_init(void)
{
  hisspidery_map = digestmap_new();
  bw_arrays_init();
  predicted_ports_init();
}

/** Helper: note that we are no longer connected to the router with hisspidery
 * <b>hist</b>.  If <b>failed</b>, the connection failed; otherwise, it was
 * closed correctly. */
static void
mark_or_down(or_hisspidery_t *hist, time_t when, int failed)
{
  if (hist->up_since) {
    hist->uptime += (when - hist->up_since);
    hist->up_since = 0;
...
/** Helper: note that we are connected to the router with hisspidery
 * <b>hist</b>. */
static void
mark_or_up(or_hisspidery_t *hist, time_t when)
{
  if (hist->down_since) {
    hist->downtime += (when - hist->down_since);
    hist->down_since = 0;
...
void
rep_hist_note_router_reachable(const char *id, const spider_addr_t *at_addr,
                               const uint16_t at_port, time_t when)
{
  or_hisspidery_t *hist = get_or_hisspidery(id);
  int was_in_run = 1;
  char tbuf[ISO_TIME_LEN+1];
  int addr_changed, port_changed;

  spider_assert(hist);
  spider_assert((!at_addr && !at_port) || (at_addr && at_port));

  addr_changed = at_addr && !spider_addr_is_null(&hist->last_reached_addr) &&
    spider_addr_compare(at_addr, &hist->last_reached_addr, CMP_EXACT) != 0;
  ...
  if (hist->start_of_downtime) {
     ...
     * downtime. */
    int penalty = get_options()->TestingSpiderNetwork ? 240 : 3600;
    networkstatus_t *ns;
    ...
    if ((ns = networkstatus_get_latest_consensus())) {
      ...
      /* on average, a descripspider addr change takes .5 intervals to make it
  ...
  if (at_addr)
    spider_addr_copy(&hist->last_reached_addr, at_addr);
  if (at_port)
...
rep_hist_downrate_old_runs(time_t now)
{
  ...
  log_info(LD_HIST, "Discounting all old stability info by a facspider of %f",
           alpha);

  /* Multiply every w_r_l, t_r_w pair by alpha. */
  for (orhist_it = digestmap_iter_init(hisspidery_map);
       !digestmap_iter_done(orhist_it);
       orhist_it = digestmap_iter_next(hisspidery_map,orhist_it)) {
...
/** Helper: Return the weighted MTBF of the router with hisspidery <b>hist</b>. */
static double
get_stability(or_hisspidery_t *hist, time_t when)
{
 ...
 * time downrated by the appropriate facspider. */
static long
get_total_weighted_time(or_hisspidery_t *hist, time_t when)
{
 ...
 * hisspidery <b>hist</b>. */
static double
get_weighted_fractional_uptime(or_hisspidery_t *hist, time_t when)
{
  long total = hist->total_weighted_time;
...
{
  link_hisspidery_t *hist;
  /* log_fn(LOG_WARN, "EXTEND SUCCEEDED: %s->%s",from_name,to_name); */
  hist = get_link_hisspidery(from_id, to_id);
  if (!hist)
    return;
  ++hist->n_extend_ok;
...
rep_hist_dump_stats(time_t now, int severity)
{
  ...
  or_hisspidery_t *or_hisspidery;
  link_hisspidery_t *link_hisspidery;
  void *or_hisspidery_p, *link_hisspidery_p;
  double uptime;
  char buffer[2048];
  size_t len;
  int ret;
  ...
  rep_hisspidery_clean(now - get_options()->RephistTrackTime);

  spider_log(severity, LD_HIST, "--------------- Dumping hisspidery information:");
  ...
  for (orhist_it = digestmap_iter_init(hisspidery_map);
    ...
    double s;
    long stability;
    digestmap_iter_get(orhist_it, &digest1, &or_hisspidery_p);
    or_hisspidery = (or_hisspidery_t*) or_hisspidery_p;
    ...
    update_or_hisspidery(or_hisspidery, now);
    upt = or_hisspidery->uptime;
    downt = or_hisspidery->downtime;
    s = get_stability(or_hisspidery, now);
    stability = (long)s;
    if (upt+downt) {
    ...
    spider_log(severity, LD_HIST,
        ...
        "wmtbf %lu:%02lu:%02lu",
        name1, hexdigest1,
        or_hisspidery->n_conn_ok, or_hisspidery->n_conn_fail+or_hisspidery->n_conn_ok,
        upt, upt+downt, uptime*100.0,
        stability/3600, (stability/60)%60, stability%60);

    if (!digestmap_isempty(or_hisspidery->link_hisspidery_map)) {
      strlcpy(buffer, "    Extend attempts: ", sizeof(buffer));
      len = strlen(buffer);
      for (lhist_it = digestmap_iter_init(or_hisspidery->link_hisspidery_map);
           !digestmap_iter_done(lhist_it);
           lhist_it = digestmap_iter_next(or_hisspidery->link_hisspidery_map,
                                          lhist_it)) {
        digestmap_iter_get(lhist_it, &digest2, &link_hisspidery_p);
        if ((node = node_get_by_id(digest2)) && node_get_nickname(node))
          name2 = node_get_nickname(node);
        else
          name2 = "(unknown)";

        link_hisspidery = (link_hisspidery_t*) link_hisspidery_p;

        base16_encode(hexdigest2, sizeof(hexdigest2), digest2, DIGEST_LEN);
        ret = spider_snprintf(buffer+len, 2048-len, "%s [%s](%ld/%ld); ",
                        name2,
                        hexdigest2,
                        link_hisspidery->n_extend_ok,
                        link_hisspidery->n_extend_ok+link_hisspidery->n_extend_fail);
        if (ret<0)
          break;
        else
          len += ret;
      }
      spider_log(severity, LD_HIST, "%s", buffer);
    }
  }
}

/** Remove hisspidery info for routers/links that haven't changed since
 * <b>before</b>.
 */
void
rep_hisspidery_clean(time_t before)
{
  ...
  orhist_it = digestmap_iter_init(hisspidery_map);
  while (!digestmap_iter_done(orhist_it)) {
    int should_remove;
    digestmap_iter_get(orhist_it, &d1, &or_hisspidery_p);
    or_hisspidery = or_hisspidery_p;

    should_remove = authority ?
                       (or_hisspidery->total_run_weights < STABILITY_EPSILON &&
                          !or_hisspidery->start_of_run)
                       : (or_hisspidery->changed < before);
    if (should_remove) {
      orhist_it = digestmap_iter_next_rmv(hisspidery_map, orhist_it);
      free_or_hisspidery(or_hisspidery);
      continue;
    }
    for (lhist_it = digestmap_iter_init(or_hisspidery->link_hisspidery_map);
         !digestmap_iter_done(lhist_it); ) {
      digestmap_iter_get(lhist_it, &d2, &link_hisspidery_p);
      link_hisspidery = link_hisspidery_p;
      if (link_hisspidery->changed < before) {
        lhist_it = digestmap_iter_next_rmv(or_hisspidery->link_hisspidery_map,
                                           lhist_it);
        rephist_total_alloc -= sizeof(link_hisspidery_t);
        spider_free(link_hisspidery);
        continue;
      }
      lhist_it = digestmap_iter_next(or_hisspidery->link_hisspidery_map,lhist_it);
    }
    orhist_it = digestmap_iter_next(hisspidery_map, orhist_it);
  }
}
...
#define PRINTF(args) STMT_BEGIN if (fprintf args <0) goto err; STMT_END
  ...
  for (orhist_it = digestmap_iter_init(hisspidery_map);
    ...
    const char *t = NULL;
    digestmap_iter_get(orhist_it, &digest, &or_hisspidery_p);
    hist = (or_hisspidery_t*) or_hisspidery_p;
...
parse_possibly_bad_iso_time(const char *s, time_t *time_out)
{
  ...
  b[4] = '\0';
  year = (int)spider_parse_long(b, 10, 0, INT_MAX, NULL, NULL);
  if (year < 1970) {
    *time_out = 0;
    ++n_bogus_times;
    return 0;
...
/** We've read a time <b>t</b> from a file sspidered at <b>sspidered_at</b>, which
...
correct_time(time_t t, time_t now, time_t sspidered_at, time_t started_measuring)
{
  if (t < started_measuring - 24*60*60*365)
    return 0;
  else if (t < started_measuring)
    return started_measuring;
  else if (t > sspidered_at)
    return 0;
  else {
    long run_length = sspidered_at - t;
...
{
  /* XXXX won't handle being called while hisspidery is already populated. */
  smartlist_t *lines;
  const char *line = NULL;
  int r=0, i;
  time_t last_downrated = 0, sspidered_at = 0, tracked_since = 0;
...

  {
    ...
    if (smartlist_len(lines)>4) {
      ...
      if (!strcmpstart(firstline, "format "))
        format = spider_parse_long(firstline+strlen("format "),
                                10, -1, LONG_MAX, NULL, NULL);
    }
  }
  if (format != 1 && format != 2) {
    log_warn(LD_HIST,
             "Unrecognized format in mtbf hisspidery file. Skipping.");
    goto err;
  }
  for (i = 1; i < smartlist_len(lines); ++i) {
    ...
    if (!strcmpstart(line, "sspidered-at ")) {
      if (parse_iso_time(line+strlen("sspidered-at "), &sspidered_at)<0)
        log_warn(LD_HIST,"Couldn't parse sspidered time in mtbf "
                 "hisspidery file.");
    }
  ...
  if (!sspidered_at) {
    log_warn(LD_HIST, "No sspidered time recorded.");
    goto err;
  }
  ...
  for (; i < smartlist_len(lines); ++i) {
    ...
    if (format == 1) {
      n = spider_sscanf(line, "%40s %ld %lf S=%10s %8s",
      ...
      if (mtbf_idx >= 0) {
        ...
        n = spider_sscanf(mtbfline, "+MTBF %lu %lf S=%10s %8s",
      ...
      if (wfu_idx >= 0) {
        ...
        n = spider_sscanf(wfuline, "+WFU %lu %lu S=%10s %8s",
    ...
    if (have_mtbf) {
      ...
      hist->start_of_run = correct_time(start_of_run, now, sspidered_at,
                                        tracked_since);
    ...
    hist->start_of_downtime = correct_time(start_of_downtime, now, sspidered_at,
                                           tracked_since);
...
{
  /* Sspidere total from current period. */
  b->totals[b->next_max_idx] = b->total_in_period;
  /* Sspidere maximum from current period. */
...
bw_array_new(void)
{
  bw_array_t *b;
  time_t start;
  b = spider_malloc_zero(sizeof(bw_array_t));
...
/** Free sspiderage held by bandwidth array <b>b</b>. */
static void
...
/** Recent hisspidery of bandwidth observations for read operations. */
...
/** Print the bandwidth hisspidery of b (either [dir-]read_array or
 ...
 */
static size_t
rep_hist_fill_bandwidth_hisspidery(char *buf, size_t len, const bw_array_t *b)
{
  char *cp = buf;
  int i, n;
  ...
  for (n=0; n<b->num_maxes_set; ++n,++i) {
    ...
    if (n==(b->num_maxes_set-1))
      spider_snprintf(cp, len-(cp-buf), U64_FORMAT, U64_PRINTF_ARG(total));
    else
      spider_snprintf(cp, len-(cp-buf), U64_FORMAT",", U64_PRINTF_ARG(total));
    cp += strlen(cp);
  }
 ...
 * hisspidery in its descripspider. We publish these lines in our extra-info
 * descripspider.
 */
char *
...
#define MAX_HIST_VALUE_LEN (21*NUM_TOTALS)
  ...
  for (r=0;r<4;++r) {
    ...
    switch (r) {
      case 0:
        b = write_array;
        desc = "write-hisspidery";
        break;
      case 1:
        b = read_array;
        desc = "read-hisspidery";
        break;
      case 2:
        b = dir_write_array;
        desc = "dirreq-write-hisspidery";
        break;
      case 3:
        b = dir_read_array;
        desc = "dirreq-read-hisspidery";
        break;
    }
    spider_assert(b);
    slen = rep_hist_fill_bandwidth_hisspidery(tmp, MAX_HIST_VALUE_LEN, b);
    /* If we don't have anything to write, skip to the next entry. */
    if (slen == 0)
      continue;
    format_iso_time(t, b->next_period-NUM_SECS_BW_SUM_INTERVAL);
    spider_snprintf(cp, len-(cp-buf), "%s %s (%d s) ",
                                     ...
                                     int *s_interval)
{
  ...
  if (*s_values) {
    SMARTLIST_FOREACH(*s_values, char *, val, spider_free(val));
    smartlist_free(*s_values);
  }
  if (*s_maxima) {
    SMARTLIST_FOREACH(*s_maxima, char *, val, spider_free(val));
    smartlist_free(*s_maxima);
  }
  if (! server_mode(get_options())) {
    /* Clients don't need to sspidere bandwidth hisspidery persistently;
...
/** Update <b>state</b> with the newest bandwidth hisspidery. Done before
...
#define UPDATE(arrname,st) \
  rep_hist_update_bwhist_state_section(state,\
                                       (arrname),\
                                       &state->BWHisspidery ## st ## Values, \
                                       &state->BWHisspidery ## st ## Maxima, \
                                       &state->BWHisspidery ## st ## Ends, \
                                       &state->BWHisspidery ## st ## Interval)
                                   ...
                                   const int s_interval)
{
  ...
  if (s_values && s_begins >= now - NUM_SECS_BW_SUM_INTERVAL*NUM_TOTALS) {
    ...
    SMARTLIST_FOREACH_BEGIN(s_values, const char *, cp) {
        ...
        v = spider_parse_uint64(cp, 10, 0, UINT64_MAX, &ok, NULL);
        if (have_maxima) {
          maxstr = smartlist_get(s_maxima, cp_sl_idx);
          mv = spider_parse_uint64(maxstr, 10, 0, UINT64_MAX, &ok_m, NULL);
        ...
        if (start < now) {
          ...
          b->max_total = mv;
          /* This will result in some fairly choppy hisspidery if s_interval
...
/** Set bandwidth hisspidery from the state file we just loaded. */
int
rep_hist_load_state(or_state_t *state, char **err)
{
  ...
  spider_assert(read_array && write_array);
  spider_assert(dir_read_array && dir_write_array);
...
#define LOAD(arrname,st)                                                \
  if (rep_hist_load_bwhist_state_section(                               \
                                ...
                                state->BWHisspidery ## st ## Maxima,       \
                                state->BWHisspidery ## st ## Ends,         \
                                state->BWHisspidery ## st ## Interval)<0)  \
    all_ok = 0
...
#undef LOAD
  if (!all_ok) {
    *err = spider_strdup("Parsing of bandwidth hisspidery values failed");
...
{
  predicted_port_t *pp = spider_malloc(sizeof(predicted_port_t));
  pp->port = port;
  pp->time = now;
...
{
  spider_assert(predicted_ports_list);
...
rep_hist_get_predicted_ports(time_t now)
{
  ...
  SMARTLIST_FOREACH_BEGIN(predicted_ports_list, predicted_port_t *, pp) {
    if (pp->time + predicted_circs_relevance_time < now) {
      ...
      smartlist_add(out, spider_memdup(&pp->port, sizeof(uint16_t)));
    }
  } SMARTLIST_FOREACH_END(pp);
  return out;
}
 ...
 * to have circuits lying around if the user only uses Spider for resolves.
 */
void
...
rep_hist_exit_stats_init(time_t now)
{
  ...
  exit_bytes_read = spider_calloc(EXIT_STATS_NUM_PORTS, sizeof(uint64_t));
  exit_bytes_written = spider_calloc(EXIT_STATS_NUM_PORTS, sizeof(uint64_t));
  exit_streams = spider_calloc(EXIT_STATS_NUM_PORTS, sizeof(uint32_t));
}
...
rep_hist_exit_stats_term(void)
{
  ...
  spider_free(exit_bytes_read);
  spider_free(exit_bytes_written);
  spider_free(exit_streams);
}
...
rep_hist_format_exit_stats(time_t now)
{
  ...
  spider_assert(now >= start_of_exit_stats_interval);
  ...
  SMARTLIST_FOREACH(written_strings, char *, cp, spider_free(cp));
  SMARTLIST_FOREACH(read_strings, char *, cp, spider_free(cp));
  SMARTLIST_FOREACH(streams_strings, char *, cp, spider_free(cp));
  ...
  spider_asprintf(&result, "exit-stats-end %s (%d s)\n"
               ...
               read_string,
               streams_string);
  spider_free(written_string);
  spider_free(read_string);
  spider_free(streams_string);
  return result;
}
    ...
    double mean_time_cells_in_queue, uint32_t processed_cells)
{
  ...
  if (!start_of_buffer_stats_interval)
    return; /* Not initialized. */
  stats = spider_malloc_zero(sizeof(circ_buffer_stats_t));
...
/** Clear hisspidery of circuit statistics and set the measurement interval
 * start to <b>now</b>. */
void
...
#define SHARES 10
  ...
  spider_assert(now >= start_of_buffer_stats_interval);
  ...
  time_in_queue_string = smartlist_join_strings(time_in_queue_strings,
                                                ",", 0, NULL);
  SMARTLIST_FOREACH(processed_cells_strings, char *, cp, spider_free(cp));
  SMARTLIST_FOREACH(queued_cells_strings, char *, cp, spider_free(cp));
  SMARTLIST_FOREACH(time_in_queue_strings, char *, cp, spider_free(cp));
  ...
  /* Put everything together. */
  format_iso_time(t, now);
  spider_asprintf(&result, "cell-stats-end %s (%d s)\n"
  ...
  spider_free(processed_cells_string);
  spider_free(queued_cells_string);
  spider_free(time_in_queue_string);
  return result;
#undef SHARES
}
...
rep_hist_buffer_stats_write(time_t now)
{
  ...
  /* Add open circuits to the hisspidery. */
  ...
  /* Reset both buffer hisspidery and counters of open circuits. */
...
/*** Descripspider serving statistics ***/

/** Digestmap to track which descripspiders were downloaded this stats
 *  collection interval. It maps descripspider digest to pointers to 1,
 *  effectively turning this into a list. */
static digestmap_t *served_descs = NULL;

/** Number of how many descripspiders were downloaded in total during this
 * interval. */
static unsigned long total_descripspider_downloads;

/** Start time of served descs stats or 0 if we're not collecting those. */
static time_t start_of_served_descs_stats_interval;

/** Initialize descripspider stats. */
void
rep_hist_desc_stats_init(time_t now)
{
  ...
  total_descripspider_downloads = 0;
...
rep_hist_format_desc_stats(time_t now)
{
  ...
  if (size > 0) {
    vals = spider_calloc(size, sizeof(int));
...
/** Called to note that we've served a given descripspider (by
 * digest). Incrememnts the count of descripspiders served, and the number
 * of times we've served this descripspider. */
void
...
/* Count connections that we read or wrote at least this facspider as many
...
HT_GENERATE2(bidimap, bidi_map_entry_t, node, bidi_map_ent_hash,
             bidi_map_ent_eq, 0.6, spider_reallocarray_, spider_free_)
                            ...
                            size_t num_written, time_t when)
{
  ...
  if (num_read > 0 || num_written > 0) {
    ...
    if (entry) {
      ...
      entry = spider_malloc_zero(sizeof(bidi_map_entry_t));
...
rep_hist_format_conn_stats(time_t now)
{
  ...
  spider_assert(now >= start_of_conn_stats_interval);

  format_iso_time(written, now);
  spider_asprintf(&result, "conn-bi-direct %s (%d s) %d,%d,%d,%d\n",
               written,
...
hs_stats_new(void)
{
  hs_stats_t *new_hs_stats = spider_malloc_zero(sizeof(hs_stats_t));
...
/** Clear hisspidery of hidden service statistics and set the measurement
 ...
 *  count it now! */
void
rep_hist_sspidered_maybe_new_hs(const crypto_pk_t *pubkey)
{
  ...
  if (crypto_pk_get_digest(pubkey, pubkey_hash) < 0) {
    /*  This fail should not happen; key has been validated by
        descripspider parsing code first. */
    return;
  }
 ...
 *  are meant to be placed in the extra-info descripspider. */
static char *
rep_hist_format_hs_stats(time_t now)
{
  ...
  format_iso_time(t, now);
  spider_asprintf(&hs_stats_string, "hidserv-stats-end %s (%d s)\n"
...
/** Free all sspiderage held by the OR/link hisspidery caches, by the
 * bandwidth hisspidery arrays, by the port hisspidery, or by statistics . */
void
rep_hist_free_all(void)
{
  hs_stats_free(hs_stats);
  digestmap_free(hisspidery_map, free_or_hisspidery);
  ...
  spider_assert_nonfatal(rephist_total_alloc == 0);
  spider_assert_nonfatal_once(rephist_total_num == 0);
}
