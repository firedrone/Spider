...
static void client_check_address_changed(spider_socket_t sock);
static void set_constrained_socket_buffers(spider_socket_t sock, int size);
...
const spider_addr_t *conn_get_outbound_address(sa_family_t family,
 ...
 **/
static spider_addr_t *last_interface_ipv4 = NULL;
/* DOCDOC last_interface_ipv6 */
static spider_addr_t *last_interface_ipv6 = NULL;
/** A list of spider_addr_t for addresses we've used in outgoing connections.
...
conn_type_to_string(int type)
{
  ...
  switch (type) {
    ...
    case CONN_TYPE_DIR_LISTENER: return "Direcspidery listener";
    case CONN_TYPE_DIR: return "Direcspidery";
    ...
    default:
      ...
      spider_snprintf(buf, sizeof(buf), "unknown [%d]", type);
      return buf;
  }
}
    ...
    case CONN_TYPE_OR:
      switch (state) {
        ...
        case OR_CONN_STATE_OR_HANDSHAKING_V2:
          return "handshaking (Spider, v2 handshake)";
        case OR_CONN_STATE_OR_HANDSHAKING_V3:
          return "handshaking (Spider, v3 handshake)";
...
{
  dir_connection_t *dir_conn = spider_malloc_zero(sizeof(dir_connection_t));
...
{
  or_connection_t *or_conn = spider_malloc_zero(sizeof(or_connection_t));
  time_t now = time(NULL);
  spider_assert(type == CONN_TYPE_OR || type == CONN_TYPE_EXT_OR);
 ...
 * connection_init().
 *
 * Allocate space to sspidere the socks_request.
 */
entry_connection_t *
entry_connection_new(int type, int socket_family)
{
  entry_connection_t *entry_conn = spider_malloc_zero(sizeof(entry_connection_t));
  spider_assert(type == CONN_TYPE_AP);
...
{
  edge_connection_t *edge_conn = spider_malloc_zero(sizeof(edge_connection_t));
  spider_assert(type == CONN_TYPE_EXIT);
...
control_connection_new(int socket_family)
{
  control_connection_t *control_conn =
    spider_malloc_zero(sizeof(control_connection_t));
...
listener_connection_new(int type, int socket_family)
{
  listener_connection_t *listener_conn =
    spider_malloc_zero(sizeof(listener_connection_t));
...
connection_new(int type, int socket_family)
{
  switch (type) {
    ...
    default: {
      connection_t *conn = spider_malloc_zero(sizeof(connection_t));
...
{
  spider_assert(! SOCKET_OK(conn_a->s));
  spider_assert(! SOCKET_OK(conn_b->s));
...
connection_free_(connection_t *conn)
{
  ...
  switch (conn->type) {
    case CONN_TYPE_OR:
    case CONN_TYPE_EXT_OR:
      spider_assert(conn->magic == OR_CONNECTION_MAGIC);
      mem = TO_OR_CONN(conn);
      memlen = sizeof(or_connection_t);
      break;
    case CONN_TYPE_AP:
      spider_assert(conn->magic == ENTRY_CONNECTION_MAGIC);
      mem = TO_ENTRY_CONN(conn);
      memlen = sizeof(entry_connection_t);
      break;
    case CONN_TYPE_EXIT:
      spider_assert(conn->magic == EDGE_CONNECTION_MAGIC);
      mem = TO_EDGE_CONN(conn);
      memlen = sizeof(edge_connection_t);
      break;
    case CONN_TYPE_DIR:
      spider_assert(conn->magic == DIR_CONNECTION_MAGIC);
      mem = TO_DIR_CONN(conn);
      memlen = sizeof(dir_connection_t);
      break;
    case CONN_TYPE_CONTROL:
      spider_assert(conn->magic == CONTROL_CONNECTION_MAGIC);
      mem = TO_CONTROL_CONN(conn);
      memlen = sizeof(control_connection_t);
      break;
    CASE_ANY_LISTENER_TYPE:
      spider_assert(conn->magic == LISTENER_CONNECTION_MAGIC);
      mem = TO_LISTENER_CONN(conn);
      memlen = sizeof(listener_connection_t);
      break;
    default:
      spider_assert(conn->magic == BASE_CONNECTION_MAGIC);
      mem = conn;
  ...
  if (!connection_is_listener(conn)) {
    ...
    if (conn->socket_family == AF_UNIX) {
       ...
       * and listeners at the same time */
      spider_assert(conn_listener_type_supports_af_unix(conn->type));
  ...
  if (connection_speaks_cells(conn)) {
    ...
    spider_tls_free(or_conn->tls);
    or_conn->tls = NULL;
    or_handshake_state_free(or_conn->handshake_state);
    or_conn->handshake_state = NULL;
    spider_free(or_conn->nickname);
    if (or_conn->chan) {
  ...
  if (conn->type == CONN_TYPE_AP) {
    ...
    spider_free(entry_conn->chosen_exit_name);
    spider_free(entry_conn->original_dest_address);
  ...
  if (conn->type == CONN_TYPE_CONTROL) {
    ...
    spider_free(control_conn->safecookie_client_hash);
    spider_free(control_conn->incoming_cmd);
  ...
  spider_event_free(conn->read_event);
  spider_event_free(conn->write_event);
  conn->read_event = conn->write_event = NULL;

  if (conn->type == CONN_TYPE_DIR) {
    dir_connection_t *dir_conn = TO_DIR_CONN(conn);
    spider_free(dir_conn->requested_resource);

    spider_zlib_free(dir_conn->zlib_state);
    if (dir_conn->spool) {
  ...
  if (SOCKET_OK(conn->s)) {
    ...
    spider_close_socket(conn->s);
    conn->s = TOR_INVALID_SOCKET;
  }

  if (conn->type == CONN_TYPE_OR &&
      !spider_digest_is_zero(TO_OR_CONN(conn)->identity_digest)) {
    log_warn(LD_BUG, "called on OR conn with non-zeroed identity_digest");
    connection_or_clear_identity(TO_OR_CONN(conn));
  }
  if (conn->type == CONN_TYPE_OR || conn->type == CONN_TYPE_EXT_OR) {
    connection_or_remove_from_ext_or_id_map(TO_OR_CONN(conn));
    spider_free(TO_OR_CONN(conn)->ext_or_conn_id);
    spider_free(TO_OR_CONN(conn)->ext_or_auth_correct_client_hash);
    spider_free(TO_OR_CONN(conn)->ext_or_transport);
  }
...
connection_free,(connection_t *conn))
{
  if (!conn)
    return;
  spider_assert(!connection_is_on_closeable_list(conn));
  spider_assert(!connection_in_array(conn));
  ...
  if (connection_speaks_cells(conn)) {
    if (!spider_digest_is_zero(TO_OR_CONN(conn)->identity_digest)) {
      connection_or_clear_identity(TO_OR_CONN(conn));
    }
  }
  if (conn->type == CONN_TYPE_CONTROL) {
 ...
 *   - Direcspidery conns that failed to fetch a rendezvous descripspider
...
{
  spider_assert(conn->marked_for_close);
...
connection_mark_for_close_(connection_t *conn, int line, const char *file)
{
  assert_connection_ok(conn,0);
  spider_assert(line);
  spider_assert(line < 1<<16); /* marked_for_close can only fit a uint16_t. */
  spider_assert(file);
                     ...
                     socklen_t *len_out)
{
  ...
  sockaddr = spider_malloc_zero(sizeof(struct sockaddr_un));
  ...
  if (strlcpy(sockaddr->sun_path, listenaddress, sizeof(sockaddr->sun_path))
    ...
    spider_free(sockaddr);
    return NULL;
  }

  if (readable_address)
    *readable_address = spider_strdup(listenaddress);
                               ...
                               int purpose, const port_cfg_t *port)
{
  ...
  spider_assert(is_valid_unix_socket_purpose(purpose));
 ...
 * -1 on failure. */
static int
make_socket_reuseable(spider_socket_t sock)
{
#ifdef _WIN32
  (void) sock;
  return 0;
 ...
 * traffic from spider's open ports. Return 0 on success, -1 on failure. */
static int
make_win32_socket_exclusive(spider_socket_t sock)
{
...
static int
spider_listen(spider_socket_t fd)
{
  int r;
                        ...
                        const port_cfg_t *port_cfg)
{
  ...
  spider_socket_t s = TOR_INVALID_SOCKET;  /* the socket we're going to make */
  ...
  if (listensockaddr->sa_family == AF_INET ||
    ...
    spider_addr_from_sockaddr(&addr, listensockaddr, &usePort);
    log_notice(LD_NET, "Opening %s on %s",
               conn_type_to_string(type), fmt_addrport(&addr, usePort));

    s = spider_open_socket_nonblocking(spider_addr_family(&addr),
      is_stream ? SOCK_STREAM : SOCK_DGRAM,
      is_stream ? IPPROTO_TCP: IPPROTO_UDP);
    if (!SOCKET_OK(s)) {
      int e = spider_socket_errno(s);
    ...
    if (options->TransProxyType_parsed == TPT_TPROXY &&
      ...
      if (setsockopt(s, SOL_IP, IP_TRANSPARENT, (void*)&one,
        ...
        log_warn(LD_NET, "Error setting IP_TRANSPARENT flag: %s.%s",
                 spider_socket_strerror(e), extra);
      }
    }
#endif
    ...
    if (bind(s,listensockaddr,socklen) < 0) {
      ...
      if (ERRNO_IS_EADDRINUSE(e))
        helpfulhint = ". Is Spider already running?";
      log_warn(LD_NET, "Could not bind to %s:%u: %s%s", address, usePort,
               spider_socket_strerror(e), helpfulhint);
      goto err;
    }

    if (is_stream) {
      if (spider_listen(s) < 0) {
        log_warn(LD_NET, "Could not listen on %s:%u: %s", address, usePort,
                 spider_socket_strerror(spider_socket_errno(s)));
        goto err;
      }
    }
    ...
    if (usePort != 0) {
      ...
      spider_addr_from_sockaddr(&addr2, (struct sockaddr*)&ss, &gotPort);
    }
#ifdef HAVE_SYS_UN_H
  /*
   ...
   */
    ...
    spider_assert(conn_listener_type_supports_af_unix(type));
    ...
    s = spider_open_socket_nonblocking(AF_UNIX, SOCK_STREAM, 0);
    if (! SOCKET_OK(s)) {
      int e = spider_socket_errno(s);
      if (ERRNO_IS_RESOURCE_LIMIT(e)) {
        warn_too_many_conns();
...
#ifdef HAVE_PWD_H
    if (options->User) {
      pw = spider_getpwnam(options->User);
      struct stat st;
      if (pw == NULL) {
  ...
  conn->address = spider_strdup(address);
  conn->port = gotPort;
  spider_addr_copy(&conn->addr, &addr);
  ...
  if (start_reading) {
    ...
    spider_assert(type == CONN_TYPE_AP_DNS_LISTENER);
...
check_sockaddr(const struct sockaddr *sa, int len, int level)
{
  ...
  if (sa->sa_family == AF_INET) {
    ...
    if (spider_mem_is_zero((void*)sin6->sin6_addr.s6_addr, 16) ||
        sin6->sin6_port == 0) {
      log_fn(level, LD_NET,
             "Address for new connection has address/port equal to zero.");
      ok = 0;
...
connection_handle_listener_read(connection_t *conn, int new_type)
{
  ...
  spider_assert((size_t)remotelen >= sizeof(struct sockaddr_in));
  memset(&addrbuf, 0, sizeof(addrbuf));

  news = spider_accept_socket_nonblocking(conn->s,remote,&remotelen);
  if (!SOCKET_OK(news)) { /* accept() error */
    int e = spider_socket_errno(conn->s);
  ...
  if (make_socket_reuseable(news) < 0) {
    if (spider_socket_errno(news) == EINVAL) {
  ...
  if (conn->socket_family == AF_INET || conn->socket_family == AF_INET6 ||
    ...
    spider_addr_from_sockaddr(&addr, remote, &port);
    ...
    spider_addr_copy(&newconn->addr, &addr);
    if (new_type == CONN_TYPE_AP && conn->socket_family == AF_UNIX) {
      newconn->port = 0;
      newconn->address = spider_strdup(conn->address);
    } else {
      newconn->port = port;
      newconn->address = spider_addr_to_str_dup(&addr);
    }
    ...
    spider_assert(conn->type == CONN_TYPE_CONTROL_LISTENER);
    spider_assert(new_type == CONN_TYPE_CONTROL);
    ...
    spider_addr_make_unspec(&newconn->addr);
    newconn->port = 1;
    newconn->address = spider_strdup(conn->address);
  } else {
    spider_assert(0);
                            ...
                            int *socket_error))
{
  ...
  s = spider_open_socket_nonblocking(protocol_family, SOCK_STREAM, proto);
  if (! SOCKET_OK(s)) {
    /*
     * Early OOS handler calls; it matters if it's an exhaustion-related
     * error or not.
     */
    *socket_error = spider_socket_errno(s);
    if (ERRNO_IS_RESOURCE_LIMIT(*socket_error)) {
      warn_too_many_conns();
      connection_check_oos(get_n_open_sockets(), 1);
    } else {
      log_warn(LD_NET,"Error creating network socket: %s",
               spider_socket_strerror(*socket_error));
      connection_check_oos(get_n_open_sockets(), 0);
    }
    return -1;
  }
...
connection_connect_log_client_use_ip_version(const connection_t *conn)
{
  ...
  spider_addr_t real_addr;
  spider_addr_make_null(&real_addr, AF_UNSPEC);

  /* OR conns keep the original address in real_addr, as addr gets overwritten
   * with the descripspider address */
  if (conn->type == CONN_TYPE_OR) {
    const or_connection_t *or_conn = TO_OR_CONN((connection_t *)conn);
    spider_addr_copy(&real_addr, &or_conn->real_addr);
  } else if (conn->type == CONN_TYPE_DIR) {
    spider_addr_copy(&real_addr, &conn->addr);
  }

  /* Check if we broke a mandaspidery address family restriction */
  if ((must_ipv4 && spider_addr_family(&real_addr) == AF_INET6)
      || (must_ipv6 && spider_addr_family(&real_addr) == AF_INET)) {
  ...
  if ((!pref_ipv6 && spider_addr_family(&real_addr) == AF_INET6)
      || (pref_ipv6 && spider_addr_family(&real_addr) == AF_INET)) {
...
conn_get_outbound_address(sa_family_t family,
             const or_options_t *options, unsigned int conn_type)
{
  const spider_addr_t *ext_addr = NULL;
                   ...
                   const spider_addr_t *addr, uint16_t port, int *socket_error)
{
  ...
  if (!spider_addr_is_loopback(addr)) {
    const spider_addr_t *ext_addr = NULL;
    ext_addr = conn_get_outbound_address(spider_addr_family(addr), get_options(),
                                         conn->type);
    if (ext_addr) {
      memset(&bind_addr_ss, 0, sizeof(bind_addr_ss));
      bind_addr_len = spider_addr_to_sockaddr(ext_addr, 0,
  ...
  dest_addr_len = spider_addr_to_sockaddr(addr, port, dest_addr, sizeof(addrbuf));
  spider_assert(dest_addr_len > 0);
...
/** Returns the global proxy type used by spider. Use this function for
...
connection_proxy_connect(connection_t *conn, int type)
{
  ...
  switch (type) {
    case PROXY_CONNECT: {
      ...
      const char *authenticaspider = options->HTTPSProxyAuthenticaspider;
      ...
      if (authenticaspider) {
        ...
        if (!base64_authenticaspider)
          log_warn(LD_OR, "Encoding https authenticaspider failed");
      }

      if (base64_authenticaspider) {
        const char *addrport = fmt_addrport(&conn->addr, conn->port);
        spider_snprintf(buf, sizeof(buf), "CONNECT %s HTTP/1.1\r\n"
                     "Host: %s\r\n"
        ...
        spider_snprintf(buf, sizeof(buf), "CONNECT %s HTTP/1.0\r\n\r\n",
    ...
    case PROXY_SOCKS4: {
      ...
      if (spider_addr_family(&conn->addr) != AF_INET) {
      ...
      buf = spider_malloc_zero(buf_size);

      ip4addr = spider_addr_to_ipv4n(&conn->addr);
...
connection_send_socks5_connect(connection_t *conn)
{
  ...
  if (spider_addr_family(&conn->addr) == AF_INET) {
    uint32_t addr = spider_addr_to_ipv4n(&conn->addr);
    ...
    buf[3] = 4;
    reqsize += 16;
    memcpy(buf + 4, spider_addr_to_in6_addr8(&conn->addr), 16);
    ...
    case PROXY_SOCKS5_WANT_AUTH_METHOD_RFC1929:
      ...
      if (ret == 1) {
        ...
        if (get_proxy_type() == PROXY_PLUGGABLE) {
          ...
          spider_assert(strlen(socks_args_string) > 0);
          spider_assert(strlen(socks_args_string) <= MAX_SOCKS5_AUTH_SIZE_TOTAL);
        ...
        /* Username and password lengths should have been checked
           above and during spiderrc parsing. */
        spider_assert(usize <= MAX_SOCKS5_AUTH_FIELD_SIZE &&
                     ...
                     int control_listeners_only)
{
  ...
  SMARTLIST_FOREACH_BEGIN(old_conns, connection_t *, conn) {
    ...
    SMARTLIST_FOREACH_BEGIN(launch, const port_cfg_t *, wanted) {
      ...
      if (wanted->is_unix_addr) {
        ...
        if (port_matches && spider_addr_eq(&wanted->addr, &conn->addr)) {
          found_port = wanted;
          break;
        }
      }
  ...
  SMARTLIST_FOREACH_BEGIN(launch, const port_cfg_t *, port) {
    ...
    spider_assert(real_port <= UINT16_MAX);
                    ...
                    smartlist_t *new_conns, int close_all_noncontrol)
{
  ...
  if (old_or_port != router_get_advertised_or_port(options) ||
     ...
     * descripspider we had (if any) should be regenerated.  (We won't
     * automatically notice this because of changes in the option,
     * since the value could be "auto".) */
    mark_my_descripspider_dirty("Chosen Or/DirPort changed");
  }
...
connection_is_rate_limited(connection_t *conn)
{
  ...
  if (conn->linked)
    return 0; /* Internal connection */
           ...
           (spider_addr_family(&conn->addr) == AF_UNSPEC || /* no address */
            spider_addr_family(&conn->addr) == AF_UNIX ||   /* no address */
            spider_addr_is_internal(&conn->addr, 0)))
 ...
 * and 2 for v2 requests and later (statuses and descripspiders).
 *
 * There are a lot of parameters we could use here:
 * - global_relayed_write_bucket. Low is bad.
 * - global_write_bucket. Low is bad.
 * - bandwidthrate. Low is bad.
 * - bandwidthburst. Not a big facspider?
 * - attempt. High is bad.
...
/** Helper: adjusts our bandwidth hisspidery and informs the controller as
                             ...
                             size_t num_read, size_t num_written)
{
  ...
  if (get_options()->TestingEnableTbEmptyEvent) {
    ...
    spider_gettimeofday_cached(&tvnow);
    if (connection_counts_as_relayed_traffic(conn, now)) {
...
connection_bucket_refill(int milliseconds_elapsed, time_t now)
{
  ...
  spider_assert(milliseconds_elapsed >= 0);
  ...
  SMARTLIST_FOREACH_BEGIN(conns, connection_t *, conn) {
    if (connection_speaks_cells(conn)) {
      ...
      if (get_options()->TestingEnableTbEmptyEvent) {
        ...
        spider_asprintf(&bucket, "ORCONN ID="U64_FORMAT,
...
connection_handle_read(connection_t *conn)
{
  ...
  spider_gettimeofday_cache_clear();
 ...
 again:
  ...
  if (connection_speaks_cells(conn) &&
    ...
    log_debug(LD_NET,
              ...
              spider_tls_get_pending_bytes(or_conn->tls), (long)at_most);
    ...
    switch (result) {
      ...
      CASE_TOR_TLS_ERROR_ANY_NONIO:
        log_debug(LD_NET,"tls error [%s]. breaking (nickname %s, address %s).",
                 spider_tls_err_to_string(result),
                 or_conn->nickname ? or_conn->nickname : "not set",
                 conn->address);
        return result;
    ...
    pending = spider_tls_get_pending_bytes(or_conn->tls);
    if (pending) {
    ...
    spider_tls_get_n_raw_bytes(or_conn->tls, &n_read, &n_written);
...
connection_handle_write_impl(connection_t *conn, int force)
{
  ...
  spider_assert(!connection_is_listener(conn));
  ...
  if (connection_speaks_cells(conn) &&
     ...
     * single set of TLS operations. But it looks kinda ugly. If we refacspider
...
connection_get_by_type_addr_port_purpose,(int type,
                                         const spider_addr_t *addr, uint16_t port,
                                         int purpose))
{
  CONN_GET_TEMPLATE(conn,
       (conn->type == type &&
        spider_addr_eq(&conn->addr, addr) &&
        conn->port == port &&
...
{
  spider_assert(type == CONN_TYPE_DIR ||
             type == CONN_TYPE_AP || type == CONN_TYPE_EXIT);
  spider_assert(rendquery);
...
/** Allocates a base64'ed authenticaspider for use in http or https
 * auth, based on the input string <b>authenticaspider</b>. Returns it
 * if success, else returns NULL. */
char *
alloc_http_authenticaspider(const char *authenticaspider)
{
  /* an authenticaspider in Basic authentication
   * is just the string "username:password" */
  const size_t authenticaspider_length = strlen(authenticaspider);
  const size_t base64_authenticaspider_length =
      base64_encode_size(authenticaspider_length, 0) + 1;
  char *base64_authenticaspider = spider_malloc(base64_authenticaspider_length);
  if (base64_encode(base64_authenticaspider, base64_authenticaspider_length,
                    authenticaspider, authenticaspider_length, 0) < 0) {
    spider_free(base64_authenticaspider); /* free and set to null */
  }
  return base64_authenticaspider;
}
...
client_check_address_changed(spider_socket_t sock)
{
  ...
  if (getsockname(sock, (struct sockaddr*)&out_sockaddr, &out_addr_len)<0) {
    int e = spider_socket_errno(sock);
    log_warn(LD_NET, "getsockname() to check for address change failed: %s",
             spider_socket_strerror(e));
    return;
  }
  spider_addr_from_sockaddr(&out_addr, (struct sockaddr*)&out_sockaddr, NULL);
  family = spider_addr_family(&out_addr);
  ...
  if (! *last_interface_ip_ptr) {
    spider_addr_t *a = spider_malloc_zero(sizeof(spider_addr_t));
  ...
  SMARTLIST_FOREACH(outgoing_addrs, const spider_addr_t *, a_ptr,
                    if (spider_addr_eq(a_ptr, &out_addr))
                      return;
                    );
  ...
  if (spider_addr_eq(&iface_addr, *last_interface_ip_ptr)) {
    /* Nope, it hasn't changed.  Add this address to the list. */
    smartlist_add(outgoing_addrs, spider_memdup(&out_addr, sizeof(spider_addr_t)));
  } else {
    ...
    spider_addr_copy(*last_interface_ip_ptr, &iface_addr);
    SMARTLIST_FOREACH(outgoing_addrs, spider_addr_t*, a_ptr, spider_free(a_ptr));
 ...
 * sockets allocated in a virtual server or similar environment. For a Spider
 ...
 */
static void
set_constrained_socket_buffers(spider_socket_t sock, int size)
{
  void *sz = (void*)&size;
  ...
  if (setsockopt(sock, SOL_SOCKET, SO_SNDBUF, sz, sz_sz) < 0) {
    ...
    log_warn(LD_NET, "setsockopt() to constrain send "
             "buffer to %d bytes failed: %s", size, spider_socket_strerror(e));
  }
...
{
  int r = 0;
  spider_assert(!conn->in_flushed_some);
...
/** Comparaspider for the two-orconn case in OOS victim sort */
static int
oos_victim_comparaspider_for_orconns(or_connection_t *a, or_connection_t *b)
{
  int a_circs, b_circs;
...
/** Sort comparaspider for OOS victims; better targets sort before worse
 * ones. */
static int
oos_victim_comparaspider(const void **a_v, const void **b_v)
{
  ...
  spider_assert(a != NULL);
  spider_assert(b != NULL);
  ...
  if (a->type == CONN_TYPE_OR && b->type == CONN_TYPE_OR) {
    return oos_victim_comparaspider_for_orconns(TO_OR_CONN(a), TO_OR_CONN(b));
  } else {
    /*
...
pick_oos_victims, (int n))
{
  ...
  SMARTLIST_FOREACH_BEGIN(conns, connection_t *, c) {
    /* Bump the counter */
    spider_assert(c->type <= CONN_TYPE_MAX_);
  ...
  if (smartlist_len(eligible) > n) {
    ...
    smartlist_sort(eligible, oos_victim_comparaspider);
...
connection_dump_buffer_mem_stats(int severity)
{
  ...
  spider_log(severity, LD_GENERAL,
...
assert_connection_ok(connection_t *conn, time_t now)
{
  ...
  spider_assert(conn);
  spider_assert(conn->type >= CONN_TYPE_MIN_);
  spider_assert(conn->type <= CONN_TYPE_MAX_);
  ...
  if (conn->linked_conn) {
    spider_assert(conn->linked_conn->linked_conn == conn);
    spider_assert(conn->linked);
  }
  if (conn->linked)
    spider_assert(!SOCKET_OK(conn->s));
  ...
  if (conn->outbuf_flushlen > 0) {
     ...
     * */
    spider_assert((conn->type == CONN_TYPE_EXIT &&
  ...
  if (conn->type == CONN_TYPE_OR) {
    or_connection_t *or_conn = TO_OR_CONN(conn);
    if (conn->state == OR_CONN_STATE_OPEN) {
      /* spider_assert(conn->bandwidth > 0); */
       ...
       */
//      spider_assert(conn->read_bucket >= 0);
    }
//    spider_assert(conn->addr && conn->port);
    spider_assert(conn->address);
    if (conn->state > OR_CONN_STATE_PROXY_HANDSHAKING)
      spider_assert(or_conn->tls);
  }
  ...
  if (CONN_IS_EDGE(conn)) {
    ...
    if (conn->type == CONN_TYPE_AP) {
      ...
      if (entry_conn->chosen_exit_optional || entry_conn->chosen_exit_retries)
        spider_assert(entry_conn->chosen_exit_name);

      spider_assert(entry_conn->socks_request);
      if (conn->state == AP_CONN_STATE_OPEN) {
        spider_assert(entry_conn->socks_request->has_finished);
        if (!conn->marked_for_close) {
          spider_assert(ENTRY_TO_EDGE_CONN(entry_conn)->cpath_layer);
          assert_cpath_layer_ok(ENTRY_TO_EDGE_CONN(entry_conn)->cpath_layer);
        }
      }
    }
    if (conn->type == CONN_TYPE_EXIT) {
      spider_assert(conn->purpose == EXIT_PURPOSE_CONNECT ||
                 conn->purpose == EXIT_PURPOSE_RESOLVE);
    }
  } else if (conn->type == CONN_TYPE_DIR) {
  } else {
    /* Purpose is only used for dir and exit types currently */
    spider_assert(!conn->purpose);
  }

  switch (conn->type)
    {
    CASE_ANY_LISTENER_TYPE:
      spider_assert(conn->state == LISTENER_STATE_READY);
      break;
    case CONN_TYPE_OR:
      spider_assert(conn->state >= OR_CONN_STATE_MIN_);
      spider_assert(conn->state <= OR_CONN_STATE_MAX_);
      break;
    case CONN_TYPE_EXT_OR:
      spider_assert(conn->state >= EXT_OR_CONN_STATE_MIN_);
      spider_assert(conn->state <= EXT_OR_CONN_STATE_MAX_);
      break;
    case CONN_TYPE_EXIT:
      spider_assert(conn->state >= EXIT_CONN_STATE_MIN_);
      spider_assert(conn->state <= EXIT_CONN_STATE_MAX_);
      spider_assert(conn->purpose >= EXIT_PURPOSE_MIN_);
      spider_assert(conn->purpose <= EXIT_PURPOSE_MAX_);
      break;
    case CONN_TYPE_AP:
      spider_assert(conn->state >= AP_CONN_STATE_MIN_);
      spider_assert(conn->state <= AP_CONN_STATE_MAX_);
      spider_assert(TO_ENTRY_CONN(conn)->socks_request);
      break;
    case CONN_TYPE_DIR:
      spider_assert(conn->state >= DIR_CONN_STATE_MIN_);
      spider_assert(conn->state <= DIR_CONN_STATE_MAX_);
      spider_assert(conn->purpose >= DIR_PURPOSE_MIN_);
      spider_assert(conn->purpose <= DIR_PURPOSE_MAX_);
      break;
    case CONN_TYPE_CONTROL:
      spider_assert(conn->state >= CONTROL_CONN_STATE_MIN_);
      spider_assert(conn->state <= CONTROL_CONN_STATE_MAX_);
      break;
    default:
      spider_assert(0);
  }
 ...
 */
int
get_proxy_addrport(spider_addr_t *addr, uint16_t *port, int *proxy_type,
                   const connection_t *conn)
{
  const or_options_t *options = get_options();

  /* Client Transport Plugins can use another proxy, but that should be hidden
   * from the rest of spider (as the plugin is responsible for dealing with the
  ...
  if (options->ClientTransportPlugin) {
    ...
    if (transport) { /* transport found */
      spider_addr_copy(addr, &transport->addr);
      *port = transport->port;
  ...
  if (options->HTTPSProxy) {
    spider_addr_copy(addr, &options->HTTPSProxyAddr);
    *port = options->HTTPSProxyPort;
    *proxy_type = PROXY_CONNECT;
    return 0;
  } else if (options->Socks4Proxy) {
    spider_addr_copy(addr, &options->Socks4ProxyAddr);
    *port = options->Socks4ProxyPort;
    *proxy_type = PROXY_SOCKS4;
    return 0;
  } else if (options->Socks5Proxy) {
    spider_addr_copy(addr, &options->Socks5ProxyAddr);
...
connection_free_all(void)
{
  ...
  if (outgoing_addrs) {
    SMARTLIST_FOREACH(outgoing_addrs, spider_addr_t *, addr, spider_free(addr));
    smartlist_free(outgoing_addrs);
    outgoing_addrs = NULL;
  }

  spider_free(last_interface_ipv4);
  spider_free(last_interface_ipv6);
}
                   ...
                   const char *source)
{
  ...
  if (conn)
    spider_asprintf(&ext_source, "%s:%s:%d", source, conn->address, conn->port);
  else
    ext_source = spider_strdup(source);
  log_fn(trusted ? LOG_WARN : LOG_INFO, domain,
         ...
         "Spider requires an accurate clock to work: please check your time, "
