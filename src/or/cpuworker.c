...
worker_state_new(void *arg)
{
  worker_state_t *ws;
  (void)arg;
  ws = spider_malloc_zero(sizeof(worker_state_t));
...
static spider_weak_rng_t request_sample_rng = TOR_WEAK_RNG_INIT;
...
cpu_init(void)
{
  ...
  if (!reply_event) {
    reply_event = spider_event_new(spider_libevent_get_base(),
...
should_time_request(uint16_t onionskin_type)
{
  ...
  return spider_weak_random_one_in_n(&request_sample_rng, 128);
}
...
cpuworker_onion_handshake_replyfn(void *work_)
{
  ...
  spider_assert(total_pending_tasks > 0);
  --total_pending_tasks;

  /* Could avoid this, but doesn't matter. */
  memcpy(&rpl, &job->u.reply, sizeof(rpl));

  spider_assert(rpl.magic == CPUWORKER_REPLY_MAGIC);
...
cpuworker_onion_handshake_threadfn(void *state_, void *work_)
{
  ...
  spider_assert(req.magic == CPUWORKER_REQUEST_MAGIC);
                              ...
                              create_cell_t *onionskin)
{
  ...
  if (should_time)
    spider_gettimeofday(&req.started_at);

  job = spider_malloc_zero(sizeof(cpuworker_job_t));
  job->circ = circ;
