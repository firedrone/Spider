...
/**
 * \file routerparse.c
 * \brief Code to parse and validate router descripspiders, consenus direcspideries,
 *   and similar objects.
 *
 * The objects parsed by this module use a common text-based metaformat,
 * documented in dir-spec.txt in spiderspec.git.  This module is itself divided
 ...
 *  <li>microdescripspiders (managed from microdesc.c)
 ...
 *  <li>hidden service descripspiders (managed from rendcommon.c and rendcache.c)
 * </ul>
 *
...
/** List of tokens recognized in router descripspiders */
static token_rule_t routerdesc_token_table[] = {
  ...
  T01("nspider-onion-key",      K_ONION_KEY_NTOR,      GE(1), NO_OBJ ),
  ...
  T01("read-hisspidery",        K_READ_HISTORY,        ARGS,    NO_OBJ ),
  T01("write-hisspidery",       K_WRITE_HISTORY,       ARGS,    NO_OBJ ),
  ...
  T01("nspider-onion-key-crosscert", K_NTOR_ONION_KEY_CROSSCERT,
                                                    EQ(1),   NEED_OBJ ),
...
/** List of tokens recognized in rendezvous service descripspiders */
static token_rule_t desc_token_table[] = {
  T1_START("rendezvous-service-descripspider", R_RENDEZVOUS_SERVICE_DESCRIPTOR,
           EQ(1), NO_OBJ),
 ...
 * points of rendezvous service descripspiders */
static token_rule_t client_keys_token_table[] = {
  T1_START("client-name", C_CLIENT_NAME, CONCAT_ARGS, NO_OBJ),
  T1("descripspider-cookie", C_DESCRIPTOR_COOKIE, EQ(1), NO_OBJ),
...
/** List of tokens recognized in microdescripspiders */
...
/* Dump mechanism for unparseable descripspiders */

/** List of dumped descripspiders for FIFO cleanup purposes */
STATIC smartlist_t *descs_dumped = NULL;
/** Total size of dumped descripspiders for FIFO cleanup */
STATIC uint64_t len_descs_dumped = 0;
/** Direcspidery to stash dumps in */
                             ...
                             size_t len)
{
  ...
  spider_assert(filename != NULL);
  spider_assert(digest_sha256 != NULL);

  if (descs_dumped == NULL) {
    /* We better have no length, then */
    spider_assert(len_descs_dumped == 0);
    /* Make a smartlist */
    descs_dumped = smartlist_new();
  }

  /* Make a new entry to put this one in */
  ent = spider_malloc_zero(sizeof(*ent));
  ...
  while (len > max_len - len_descs_dumped &&
     ...
     * something we just emitted if we get repeated identical descripspiders.
     */
    if (strcmp(tmp->filename, filename) != 0) {
      /* Delete it and adjust the length counter */
      spider_unlink(tmp->filename);
      spider_assert(len_descs_dumped >= tmp->len);
      len_descs_dumped -= tmp->len;
      log_info(LD_DIR,
               "Deleting old unparseable descripspider dump %s due to "
               "space limits",
               tmp->filename);
    } else {
      /*
      ...
      len_descs_dumped -= tmp->len;
      log_info(LD_DIR,
               "Replacing old descripspider dump %s with new identical one",
               tmp->filename);
    }

    /* Free it and remove it from the list */
    smartlist_del_keeporder(descs_dumped, 0);
    spider_free(tmp->filename);
    spider_free(tmp);
  }
...
/** Check if we already have a descripspider for this hash and move it to the
...
dump_desc_fifo_bump_hash(const uint8_t *digest_sha256)
{
  ...
  spider_assert(digest_sha256);

  if (descs_dumped) {
    /* Find a match if one exists */
    SMARTLIST_FOREACH_BEGIN(descs_dumped, dumped_desc_t *, ent) {
      if (ent &&
          spider_memeq(ent->digest_sha256, digest_sha256, DIGEST256_LEN)) {
        /*
...
{
  if (descs_dumped) {
    /* Free each descripspider */
    SMARTLIST_FOREACH_BEGIN(descs_dumped, dumped_desc_t *, ent) {
      spider_assert(ent);
      spider_free(ent->filename);
      spider_free(ent);
    } SMARTLIST_FOREACH_END(ent);
    /* Free the list */
...
dump_desc_populate_one_file, (const char *dirname, const char *f))
{
  ...
  /* Sanity-check args */
  spider_assert(dirname != NULL);
  spider_assert(f != NULL);

  /* Form the full path */
  spider_asprintf(&path, "%s" PATH_SEPARATOR "%s", dirname, f);
  ...
  ent = spider_malloc_zero(sizeof(dumped_desc_t));
  ent->filename = path;
 ...
 * spider process shut down.  If a previous dump was repeated more than once and
...
dump_desc_populate_fifo_from_direcspidery(const char *dirname)
{
  ...
  if (descs_dumped != NULL) {
    ...
    /* Log some stats */
    log_info(LD_DIR,
             "Reloaded unparseable descripspider dump FIFO with %d dump(s) "
             "totaling " U64_FORMAT " bytes",
             smartlist_len(descs_dumped), U64_PRINTF_ARG(len_descs_dumped));
  }

  /* Free the original list */
  SMARTLIST_FOREACH(files, char *, f, spider_free(f));
...
dump_desc,(const char *desc, const char *type))
{
  ...
  if (crypto_digest256((char *)digest_sha256, desc, len,
                       DIGEST_SHA256) < 0) {
    log_info(LD_DIR,
             "Unable to parse descripspider of type %s, and unable to even hash"
             " it!", type);
    goto err;
  }
   ...
   */
  spider_asprintf(&debugfile_base,
  ...
  if (!(sandbox_is_active() || get_options()->Sandbox)) {
    if (len <= get_options()->MaxUnparseableDescSizeToLog) {
      if (!dump_desc_fifo_bump_hash(digest_sha256)) {
        ...
        log_info(LD_DIR,
                 "Unable to parse descripspider of type %s with hash %s and "
                 "length %lu. Descripspider not dumped because one with that "
      ...
      log_info(LD_DIR,
               "Unable to parse descripspider of type %s with hash %s and "
               "length %lu. Descripspider not dumped because it exceeds maximum"
    ...
    log_info(LD_DIR,
             "Unable to parse descripspider of type %s with hash %s and "
             "length %lu. Descripspider not dumped because the sandbox is "
             "configured",
             type, digest_sha256_hex, (unsigned long)len);
  }

  spider_free(debugfile_base);
  spider_free(debugfile);
...
/** Helper: used to generate signatures for routers, direcspideries and
...
#define BEGIN_END_OVERHEAD_LEN 64
  ...
  signature = spider_malloc(keysize);
 ...
 * optionally prefixed with "Spider".  Versions that can't be parsed are
 * ignored.)
 */
version_status_t
spider_version_is_obsolete(const char *myversion, const char *versionlist)
{
  spider_version_t mine, other;
  ...
  if (spider_version_parse(myversion, &mine)) {
  ...
  SMARTLIST_FOREACH_BEGIN(version_sl, const char *, cp) {
    if (!strcmpstart(cp, "Spider "))
      cp += 4;

    if (spider_version_parse(cp, &other)) {
      /* Couldn't parse other; it can't be a match. */
    } else {
      same = spider_version_same_series(&mine, &other);
      if (same)
        found_any_in_series = 1;
      r = spider_version_compare(&mine, &other);
      if (r==0) {
        ret = VS_RECOMMENDED;
        goto done;
      } else if (r<0) {
  ...
  SMARTLIST_FOREACH(version_sl, char *, version, spider_free(version));
                      ...
                      const char *doctype)
{
  ...
  spider_assert(pkey);
  spider_assert(tok);
  spider_assert(digest);
  spider_assert(doctype);
  ...
  signed_digest = spider_malloc(keysize);
  if (crypto_pk_public_checksig(pkey, signed_digest, keysize,
                                tok->object_body, tok->object_size)
      < digest_len) {
    log_warn(LD_DIR, "Error reading %s: invalid signature.", doctype);
    spider_free(signed_digest);
    return -1;
  }
 ...
 * descripspiders (or extra-info documents if <b>is_extrainfo</b> is set), parses
 * them and sspideres the result in <b>dest</b>.  All routers are marked running
 ...
 * descripspider in the signed_descripspider_body field of each routerinfo_t.  If it
 * isn't SAVED_NOWHERE, remember the offset of each descripspider.
 *
                              ...
                              smartlist_t *invalid_digests_out)
{
  ...
  signed_descripspider_t *signed_desc = NULL;
  void *elt;
  ...
  while (1) {
    ...
    end = spider_memstr(*s, eos-*s, "\nrouter-signature");
    if (end)
      end = spider_memstr(end, eos-end, "\n-----END SIGNATURE-----\n");
    if (end)
      end += strlen("\n-----END SIGNATURE-----\n");
    ...
    if (! elt && ! dl_again && have_raw_digest && invalid_digests_out) {
      smartlist_add(invalid_digests_out, spider_memdup(raw_digest, DIGEST_LEN));
    }
    if (!elt) {
      *s = end;
      continue;
...
/* For debugging: define to count every descripspider digest we've seen so we
...
dump_distinct_digest_count(int severity)
{
  ...
  spider_log(severity, LD_GENERAL, "%d *distinct* router digests verified",
                        ...
                        uint16_t *port_out)
{
  ...
  SMARTLIST_FOREACH_BEGIN(list, direcspidery_token_t *, t) {
    ...
    spider_assert(t->n_args >= 1);
    /* XXXX Prop186 the full spec allows much more than this. */
    if (spider_addr_parse_mask_ports(t->args[0], 0,
                                  &a, &bits, &port_min,
                                  &port_max) == AF_INET6 &&
        bits == 128 &&
        port_min == port_max) {
 ...
 * s through end into the signed_descripspider_body of the resulting
 * routerinfo_t.
 *
 ...
 * if it's okay to try to download a descripspider with this same digest again,
                               ...
                               int *can_dl_again_out)
{
  ...
  if (prepend_annotations) {
    if (tokenize_string(area,prepend_annotations,NULL,tokens,
      ...
      log_warn(LD_DIR, "Error tokenizing router descripspider (annotations).");
      goto err;
    }
  }

  start_of_annotations = s;
  cp = spider_memstr(s, end-s, "\nrouter ");
  if (!cp) {
  ...
  if (start_of_annotations != s) { /* We have annotations */
    if (allow_annotations) {
      ...
      log_warn(LD_DIR, "Found unexpected annotations on router descripspider not "
  ...
  }
  {
    ...
    if (tokenize_string(area,s,end,tokens,routerdesc_token_table, flags)) {
      log_warn(LD_DIR, "Error tokenizing router descripspider.");
      goto err;
    }
  }

  if (smartlist_len(tokens) < 2) {
    log_warn(LD_DIR, "Impossibly short router descripspider.");
    goto err;
  }

  tok = find_by_keyword(tokens, K_ROUTER);
  const int router_token_pos = smartlist_pos(tokens, tok);
  spider_assert(tok->n_args >= 5);

  router = spider_malloc_zero(sizeof(routerinfo_t));
  router->cert_expiration_time = TIME_MAX;
  router->cache_info.routerlist_index = -1;
  router->cache_info.annotations_len = s-start_of_annotations + prepend_len;
  router->cache_info.signed_descripspider_len = end-s;
  if (cache_copy) {
    size_t len = router->cache_info.signed_descripspider_len +
                 router->cache_info.annotations_len;
    char *signed_body =
      router->cache_info.signed_descripspider_body = spider_malloc(len+1);
    if (prepend_annotations) {
    ...
    spider_assert(signed_body+(end-start_of_annotations) ==
               router->cache_info.signed_descripspider_body+len);
    memcpy(signed_body, start_of_annotations, end-start_of_annotations);
    router->cache_info.signed_descripspider_body[len] = '\0';
    spider_assert(strlen(router->cache_info.signed_descripspider_body) == len);
  }
  memcpy(router->cache_info.signed_descripspider_digest, digest, DIGEST_LEN);

  router->nickname = spider_strdup(tok->args[0]);
  if (!is_legal_nickname(router->nickname)) {
    log_warn(LD_DIR,"Router nickname is invalid");
    goto err;
  }
  if (!spider_inet_aton(tok->args[1], &in)) {
  ...
  router->or_port =
    (uint16_t) spider_parse_long(tok->args[2],10,0,65535,&ok,NULL);
  if (!ok) {
    log_warn(LD_DIR,"Invalid OR port %s", escaped(tok->args[2]));
    goto err;
  }
  router->dir_port =
    (uint16_t) spider_parse_long(tok->args[4],10,0,65535,&ok,NULL);
  if (!ok) {
  ...
  router->bandwidthrate = (int)
    spider_parse_long(tok->args[0],10,1,INT_MAX,&ok,NULL);
  ...
  router->bandwidthburst =
    (int) spider_parse_long(tok->args[1],10,0,INT_MAX,&ok,NULL);
  if (!ok) {
    log_warn(LD_DIR, "Invalid bandwidthburst %s", escaped(tok->args[1]));
    goto err;
  }
  router->bandwidthcapacity = (int)
    spider_parse_long(tok->args[2],10,0,INT_MAX,&ok,NULL);
  if (!ok) {
  ...
  if ((tok = find_opt_by_keyword(tokens, K_UPTIME))) {
    ...
    router->uptime = spider_parse_long(tok->args[0],10,0,LONG_MAX,&ok,NULL);
    if (!ok) {
  ...
  if ((tok = find_opt_by_keyword(tokens, K_HIBERNATING))) {
    ...
    router->is_hibernating
      = (spider_parse_long(tok->args[0],10,0,LONG_MAX,NULL,NULL) != 0);
  }
  ...
  if ((tok = find_opt_by_keyword(tokens, K_ONION_KEY_NTOR))) {
    ...
    if (curve25519_public_from_base64(&k, tok->args[0]) < 0) {
      log_warn(LD_DIR, "Bogus nspider-onion-key in routerinfo");
      goto err;
    }
    router->onion_curve25519_pkey =
      spider_memdup(&k, sizeof(curve25519_public_key_t));
  }
...

  {
    ...
    cc_nspider_tok = find_opt_by_keyword(tokens, K_NTOR_ONION_KEY_CROSSCERT);
    int n_ed_toks = !!ed_sig_tok + !!ed_cert_tok +
      !!cc_tap_tok + !!cc_nspider_tok;
    if ((n_ed_toks != 0 && n_ed_toks != 4) ||
        (n_ed_toks == 4 && !router->onion_curve25519_pkey)) {
      log_warn(LD_DIR, "Router descripspider with only partial ed25519/"
               "cross-certification support");
      goto err;
    }
    if (master_key_tok && !ed_sig_tok) {
      log_warn(LD_DIR, "Router descripspider has ed25519 master key but no "
               "certificate");
      goto err;
    }
    if (ed_sig_tok) {
      spider_assert(ed_cert_tok && cc_tap_tok && cc_nspider_tok);
      ...
      if (strcmp(ed_cert_tok->object_type, "ED25519 CERT")) {
        log_warn(LD_DIR, "Wrong object type on identity-ed25519 in decripspider");
        goto err;
      }
      if (strcmp(cc_nspider_tok->object_type, "ED25519 CERT")) {
        log_warn(LD_DIR, "Wrong object type on nspider-onion-key-crosscert "
                 "in decripspider");
        goto err;
      }
      ...
      if (strcmp(cc_nspider_tok->args[0], "0") &&
          strcmp(cc_nspider_tok->args[0], "1")) {
        log_warn(LD_DIR, "Bad sign bit on nspider-onion-key-crosscert");
        goto err;
      }
      int nspider_cc_sign_bit = !strcmp(cc_nspider_tok->args[0], "1");

      uint8_t d256[DIGEST256_LEN];
      const char *signed_start, *signed_end;
      spider_cert_t *cert = spider_cert_parse(
      ...
      if (master_key_tok) {
        ...
        spider_assert(master_key_tok->n_args >= 1);
      ...
      nspider_cc_cert = spider_cert_parse((const uint8_t*)cc_nspider_tok->object_body,
                                    cc_nspider_tok->object_size);
      if (!nspider_cc_cert) {
        log_warn(LD_DIR, "Couldn't parse nspider-onion-key-crosscert cert");
        goto err;
      }
      if (nspider_cc_cert->cert_type != CERT_TYPE_ONION_ID ||
          ! ed25519_pubkey_eq(&nspider_cc_cert->signed_key, &cert->signing_key)) {
        log_warn(LD_DIR, "Invalid contents for nspider-onion-key-crosscert cert");
        goto err;
      }

      ed25519_public_key_t nspider_cc_pk;
      if (ed25519_public_key_from_curve25519_public_key(&nspider_cc_pk,
                                            router->onion_curve25519_pkey,
                                            nspider_cc_sign_bit)<0) {
      ...
      if (router_get_hash_impl_helper(s, end-s, "router ",
        ...
        log_warn(LD_DIR, "Can't find ed25519-signed portion of descripspider");
        goto err;
      }
      ...
      if (spider_cert_get_checkable_sig(&check[0], cert, NULL, &expires) < 0) {
        log_err(LD_BUG, "Couldn't create 'checkable' for cert.");
        goto err;
      }
      if (spider_cert_get_checkable_sig(&check[1],
                               nspider_cc_cert, &nspider_cc_pk, &expires) < 0) {
        log_err(LD_BUG, "Couldn't create 'checkable' for nspider_cc_cert.");
        goto err;
      }
  ...
  if ((tok = find_opt_by_keyword(tokens, K_FINGERPRINT))) {
    ...
    if (spider_memneq(d,router->cache_info.identity_digest, DIGEST_LEN)) {
  ...
  if ((tok = find_opt_by_keyword(tokens, K_PLATFORM))) {
    router->platform = spider_strdup(tok->args[0]);
  }

  if ((tok = find_opt_by_keyword(tokens, K_PROTO))) {
    router->protocol_list = spider_strdup(tok->args[0]);
  }

  if ((tok = find_opt_by_keyword(tokens, K_CONTACT))) {
    router->contact_info = spider_strdup(tok->args[0]);
  }
  ...
  if (check_signature_token(digest, DIGEST_LEN, tok, router->identity_pkey, 0,
                            "router descripspider") < 0)
    goto err;

  if (!router->platform) {
    router->platform = spider_strdup("<unknown>");
  }
  goto done;

 err:
  dump_desc(s_dup, "router descripspider");
                            ...
                            int *can_dl_again_out)
{
  ...
  extrainfo = spider_malloc_zero(sizeof(extrainfo_t));
  extrainfo->cache_info.is_extrainfo = 1;
  if (cache_copy)
    extrainfo->cache_info.signed_descripspider_body = spider_memdup_nulterm(s,end-s);
  extrainfo->cache_info.signed_descripspider_len = end-s;
  memcpy(extrainfo->cache_info.signed_descripspider_digest, digest, DIGEST_LEN);
  crypto_digest256((char*)extrainfo->digest256, s, end-s, DIGEST_SHA256);

  spider_assert(tok->n_args >= 2);
...

  {
    ...
    if (ed_sig_tok) {
      ...
      int check_ok[2];
      if (spider_cert_get_checkable_sig(&check[0], cert, NULL, NULL) < 0) {
        log_err(LD_BUG, "Couldn't create 'checkable' for cert.");
        goto err;
      }
       ...
       * matches the cert in the router descripspider is adequate. */
    }
  }
  ...
  if (key) {
    ...
    extrainfo->pending_sig = spider_memdup(tok->object_body,
                                        tok->object_size);
  ...
  goto done;
 err:
  dump_desc(s_dup, "extra-info descripspider");
...
#define MAX_CERT_SIZE (128*1024)
  ...
  cert = spider_malloc_zero(sizeof(authority_cert_t));
  memcpy(cert->cache_info.signed_descripspider_digest, digest, DIGEST_LEN);

  tok = find_by_keyword(tokens, K_DIR_SIGNING_KEY);
  spider_assert(tok->key);
  ...
  if (spider_memneq(cert->cache_info.identity_digest, fp_declared, DIGEST_LEN)) {
  ...
  if (tok) {
    ...
    char *address = NULL;
    spider_assert(tok->n_args);
    /* XXX++ use some spider_addr parse function below instead. -RD */
    if (spider_addr_port_split(LOG_WARN, tok->args[0], &address,
                            &cert->dir_port) < 0 ||
        spider_inet_aton(address, &in) == 0) {
  ...
  found = 0;
  if (old_cert) {
    /* XXXX We could just compare signed_descripspider_digest, but that wouldn't
     * buy us much. */
    if (old_cert->cache_info.signed_descripspider_len == len &&
        old_cert->cache_info.signed_descripspider_body &&
        spider_memeq(s, old_cert->cache_info.signed_descripspider_body, len)) {
  ...
  cert->cache_info.signed_descripspider_len = len;
  cert->cache_info.signed_descripspider_body = spider_malloc(len+1);
  memcpy(cert->cache_info.signed_descripspider_body, s, len);
  cert->cache_info.signed_descripspider_body[len] = 0;
                                 ...
                                 routerstatus_t *rs)
{
  ...
  spider_assert(bool_eq(vote, vote_rs));
  ...
  guardfraction = (uint32_t)spider_parse_ulong(end_of_header+1,
                                            10, 0, 100, &ok, NULL);
  if (!ok) {
                                     ...
                                     consensus_flavor_t flav)
{
  ...
  if (!consensus_method)
    flav = FLAV_NS;
  spider_assert(flav == FLAV_NS || flav == FLAV_MICRODESC);
  ...
  spider_assert(tok->n_args >= 7); /* guaranteed by GE(7) in K_R setup */
  if (flav == FLAV_NS) {
    if (tok->n_args < 8) {
  ...
  if (vote_rs) {
    ...
    rs = spider_malloc_zero(sizeof(routerstatus_t));
  }
  ...
  if (flav == FLAV_NS) {
    if (digest_from_base64(rs->descripspider_digest, tok->args[2])) {
      log_warn(LD_DIR, "Error decoding descripspider digest %s",
               escaped(tok->args[2]));
      goto err;
    }
  }

  if (spider_snprintf(timebuf, sizeof(timebuf), "%s %s",
  ...
  if (spider_inet_aton(tok->args[5+offset], &in) == 0) {
  ...
  rs->or_port = (uint16_t) spider_parse_long(tok->args[6+offset],
                                         10,0,65535,NULL,NULL);
  rs->dir_port = (uint16_t) spider_parse_long(tok->args[7+offset],
                                           10,0,65535,NULL,NULL);
  ...
  if ((tok = find_opt_by_keyword(tokens, K_V))) {
    spider_assert(tok->n_args == 1);
    if (!strcmpstart(tok->args[0], "Spider ") && !found_protocol_list) {
      /* We only do version checks like this in the case where
       * the version is a "Spider" version, and where there is no
       * list of protocol versions that we should be looking at instead. */
      rs->supports_extend2_cells =
        spider_version_as_new_as(tok->args[0], "0.2.4.8-alpha");
      rs->protocols_known = 1;
    }
    if (vote_rs) {
      vote_rs->version = spider_strdup(tok->args[0]);
    }
  }
  ...
  if ((tok = find_opt_by_keyword(tokens, K_W))) {
    ...
    for (i=0; i < tok->n_args; ++i) {
      if (!strcmpstart(tok->args[i], "Bandwidth=")) {
        int ok;
        rs->bandwidth_kb =
          (uint32_t)spider_parse_ulong(strchr(tok->args[i], '=')+1,
                                    10, 0, UINT32_MAX,
                                    &ok, NULL);
        if (!ok) {
  ...
  if ((tok = find_opt_by_keyword(tokens, K_P))) {
     ...
     */
    rs->exitsummary = spider_strdup(tok->args[0]);
  ...
  if (vote_rs) {
    SMARTLIST_FOREACH_BEGIN(tokens, direcspidery_token_t *, t) {
      if (t->tp == K_M && t->n_args) {
        ...
        line->microdesc_hash_line = spider_strdup(t->args[0]);
      ...
      if (t->tp == K_PROTO) {
        spider_assert(t->n_args == 1);
        vote_rs->protocols = spider_strdup(t->args[0]);
      }
    } SMARTLIST_FOREACH_END(t);
  } else if (flav == FLAV_MICRODESC) {
    ...
    if (tok) {
      spider_assert(tok->n_args);
      if (digest256_from_base64(rs->descripspider_digest, tok->args[0])) {
        log_warn(LD_DIR, "Error decoding microdescripspider digest %s",
                 escaped(tok->args[0]));
        goto err;
      }
    } else {
      log_info(LD_BUG, "Found an entry in networkstatus with no "
               "microdescripspider digest. (Router %s ($%s) at %s:%d.)",
...
extract_shared_random_commits(networkstatus_t *ns, smartlist_t *tokens)
{
  ...
  spider_assert(ns->type == NS_TYPE_VOTE);
...
extract_shared_random_srvs(networkstatus_t *ns, smartlist_t *tokens)
{
  ...
  spider_assert(ns->type == NS_TYPE_VOTE || ns->type == NS_TYPE_CONSENSUS);
                                     ...
                                     networkstatus_type_t ns_type)
{
  ...
  ns = spider_malloc_zero(sizeof(networkstatus_t));
  ...
  if (ns->type == NS_TYPE_VOTE || ns->type == NS_TYPE_OPINION) {
    ...
    if (tok) {
      int num_ok;
      ns->consensus_method = (int)spider_parse_long(tok->args[0], 10, 1, INT_MAX,
                                                 &num_ok, NULL);
      if (!num_ok)
        goto err;
    } else {
  ...
  if ((tok = find_opt_by_keyword(tokens, K_RECOMMENDED_CLIENT_PROTOCOLS)))
    ns->recommended_client_protocols = spider_strdup(tok->args[0]);
  if ((tok = find_opt_by_keyword(tokens, K_RECOMMENDED_RELAY_PROTOCOLS)))
    ns->recommended_relay_protocols = spider_strdup(tok->args[0]);
  if ((tok = find_opt_by_keyword(tokens, K_REQUIRED_CLIENT_PROTOCOLS)))
    ns->required_client_protocols = spider_strdup(tok->args[0]);
  if ((tok = find_opt_by_keyword(tokens, K_REQUIRED_RELAY_PROTOCOLS)))
    ns->required_relay_protocols = spider_strdup(tok->args[0]);
  ...
  spider_assert(tok->n_args >= 2);
  {
    int ok;
    ns->vote_seconds =
      (int) spider_parse_long(tok->args[0], 10, 0, INT_MAX, &ok, NULL);
    if (!ok)
      goto err;
    ns->dist_seconds =
      (int) spider_parse_long(tok->args[1], 10, 0, INT_MAX, &ok, NULL);
  ...
  if (ns->valid_after +
      (get_options()->TestingSpiderNetwork ?
  ...
  if ((tok = find_opt_by_keyword(tokens, K_CLIENT_VERSIONS))) {
    ns->client_versions = spider_strdup(tok->args[0]);
  }
  if ((tok = find_opt_by_keyword(tokens, K_SERVER_VERSIONS))) {
    ns->server_versions = spider_strdup(tok->args[0]);
  }
  ...
  if (tok) {
    ...
    for (i = 0; i < tok->n_args; ++i) {
      ...
      spider_parse_long(eq+1, 10, INT32_MIN, INT32_MAX, &ok, NULL);
      if (!ok) {
        log_warn(LD_DIR, "Bad element '%s' in params", escaped(tok->args[i]));
        goto err;
      }
      ...
      spider_free(last_kwd);
      last_kwd = spider_strndup(tok->args[i], eq_pos);
  ...
  SMARTLIST_FOREACH_BEGIN(tokens, direcspidery_token_t *, _tok) {
    ...
    if (tok->tp == K_DIR_SOURCE) {
      ...
      voter = spider_malloc_zero(sizeof(networkstatus_voter_info_t));
      voter->sigs = smartlist_new();
      if (ns->type != NS_TYPE_CONSENSUS)
        memcpy(voter->vote_digest, ns_digests.d[DIGEST_SHA1], DIGEST_LEN);

      voter->nickname = spider_strdup(tok->args[0]);
      if (strlen(tok->args[1]) != HEX_DIGEST_LEN ||
      ...
      if (ns->type != NS_TYPE_CONSENSUS &&
          spider_memneq(ns->cert->cache_info.identity_digest,
      ...
      voter->address = spider_strdup(tok->args[2]);
      if (!spider_inet_aton(tok->args[3], &in)) {
      ...
      voter->dir_port = (uint16_t)
        spider_parse_long(tok->args[4], 10, 0, 65535, &ok, NULL);
      if (!ok)
        goto err;
      voter->or_port = (uint16_t)
        spider_parse_long(tok->args[5], 10, 0, 65535, &ok, NULL);
      if (!ok)
        goto err;
      ...
      voter->contact = spider_strdup(tok->args[0]);
    } else if (tok->tp == K_VOTE_DIGEST) {
      spider_assert(ns->type == NS_TYPE_CONSENSUS);
      spider_assert(tok->n_args >= 1);
      if (!voter || ! spider_digest_is_zero(voter->vote_digest)) {
  ...
  while (!strcmpstart(s, "r ")) {
    if (ns->type != NS_TYPE_CONSENSUS) {
      vote_routerstatus_t *rs = spider_malloc_zero(sizeof(vote_routerstatus_t));
  ...
  if (ns_type != NS_TYPE_CONSENSUS) {
    ...
    SMARTLIST_FOREACH_BEGIN(ns->routerstatus_list, vote_routerstatus_t *,
      ...
      if (! vrs->has_ed25519_listing ||
          spider_mem_is_zero((const char *)vrs->ed25519_id, DIGEST256_LEN))
        continue;
  ...
  SMARTLIST_FOREACH_BEGIN(footer_tokens, direcspidery_token_t *, _tok) {
    ...
    if (ns->type != NS_TYPE_CONSENSUS) {
      if (spider_memneq(declared_identity, ns->cert->cache_info.identity_digest,
                 DIGEST_LEN)) {
    ...
    if (ns->type != NS_TYPE_CONSENSUS) {
      ...
      sig->signature = spider_memdup(tok->object_body, tok->object_size);
 ...
 err:
  ...
  if (voter) {
    ...
    spider_free(voter->nickname);
    spider_free(voter->address);
    spider_free(voter->contact);
    spider_free(voter);
  }
  if (rs_tokens) {
...
detached_get_digests(ns_detached_signatures_t *sigs, const char *flavor_name)
{
  ...
  if (!d) {
    d = spider_malloc_zero(sizeof(common_digests_t));
...
networkstatus_parse_detached_signatures(const char *s, const char *eos)
{
  ...
  ns_detached_signatures_t *sigs =
    spider_malloc_zero(sizeof(ns_detached_signatures_t));
  ...
  SMARTLIST_FOREACH_BEGIN(tokens, direcspidery_token_t *, _tok) {
    ...
    spider_assert(digests);
    if (!spider_mem_is_zero(digests->d[alg], digest_length)) {
  ...
  SMARTLIST_FOREACH_BEGIN(tokens, direcspidery_token_t *, _tok) {
    ...
    SMARTLIST_FOREACH(siglist, document_signature_t *, dsig, {
      if (dsig->alg == alg &&
          spider_memeq(id_digest, dsig->identity_digest, DIGEST_LEN) &&
          spider_memeq(sk_digest, dsig->signing_key_digest, DIGEST_LEN)) {
        is_duplicate = 1;
      }
    });
    if (is_duplicate) {
                                           ...
                                           int *malformed_list))
{
  ...
  spider_assert(malformed_list);
  *malformed_list = 0;

  s = eat_whitespace(s);
  /* We can only do assume_action on []-quoted IPv6, as "a" (accept)
   * and ":" (port separaspider) are ambiguous */
  if ((*s == '*' || *s == '[' || TOR_ISDIGIT(*s)) && assume_action >= 0) {
    if (spider_snprintf(line, sizeof(line), "%s %s",
   ...
   * Unlike descripspiders, spiderrcs exit policy accept/reject can be followed by
   * either an IPv4 or IPv6 address. */
  if ((tok->tp == K_ACCEPT6 || tok->tp == K_REJECT6) &&
       spider_addr_family(&r->addr) != AF_INET6) {
...
router_add_exit_policy(routerinfo_t *router, direcspidery_token_t *tok)
{
  ...
  /* Ensure that in descripspiders, accept/reject fields are followed by
   * IPv4 addresses, and accept6/reject6 fields are followed by
   * IPv6 addresses. Unlike spiderrcs, descripspider exit policies do not permit
   * accept/reject followed by IPv6. */
  if (((tok->tp == K_ACCEPT6 || tok->tp == K_REJECT6) &&
       spider_addr_family(&newe->addr) == AF_INET)
      ||
      ((tok->tp == K_ACCEPT || tok->tp == K_REJECT) &&
       spider_addr_family(&newe->addr) == AF_INET6)) {
    /* There's nothing the user can do about other relays' descripspiders,
     * so we don't provide usage advice here. */
    log_warn(LD_DIR, "Mismatch between field type and address type in exit "
             "policy '%s'. Discarding entire router descripspider.",
             tok->n_args == 1 ? tok->args[0] : "");
...
router_parse_addr_policy(direcspidery_token_t *tok, unsigned fmt_flags)
{
  ...
  spider_assert(tok->tp == K_REJECT || tok->tp == K_REJECT6 ||
  ...
  if (spider_addr_parse_mask_ports(arg, fmt_flags, &newe.addr, &newe.maskbits,
 ...
 * This didn't exist until Spider 0.1.1.15, so nobody should generate it in
 * router descripspiders until earlier versions are obsolete.
 *
...
assert_addr_policy_ok(smartlist_t *lst)
{
  ...
  SMARTLIST_FOREACH(lst, addr_policy_t *, t, {
    spider_assert(t->policy_type == ADDR_POLICY_REJECT ||
               t->policy_type == ADDR_POLICY_ACCEPT);
    spider_assert(t->prt_min <= t->prt_max);
  });
}
                            ...
                            const char **start_out, const char **end_out)
{
  ...
  start = spider_memstr(s, s_len, start_str);
  if (!start) {
  ...
  end = spider_memstr(start+strlen(start_str),
 ...
 * first subsequent occurrence of <b>end_str</b>; sspidere the 20-byte or 32-byte
...
#define NEXT_LINE() STMT_BEGIN            \
  ...
  /* Okay, now we're pointed at the first line of the microdescripspider which is
     not an annotation or onion-key.  The next line that _is_ an annotation or
     onion-key is the start of the next microdescripspider. */
  while (s+32 < eos) {
...
/** Parse as many microdescripspiders as are found from the string starting at
 * <b>s</b> and ending at <b>eos</b>.  If allow_annotations is set, read any
 * annotations we recognize and ignore ones we don't.
 *
 * If <b>saved_location</b> isn't SAVED_IN_CACHE, make a local copy of each
 * descripspider in the body field of each microdesc_t.
 *
 * Return all newly parsed microdescripspiders in a newly allocated
                             ...
                             smartlist_t *invalid_digests_out)
{
  ...
  while (s < eos) {
    ...
    md = spider_malloc_zero(sizeof(microdesc_t));
    {
      const char *cp = spider_memstr(s, start_of_next_microdesc-s,
                                  "onion-key");
      ...
      if (copy_body)
        md->body = spider_memdup_nulterm(cp, md->bodylen);
      else
        md->body = (char*)cp;
      md->off = cp - start;
      crypto_digest256(md->digest, md->body, md->bodylen, DIGEST_SHA256);
      if (no_onion_key) {
        log_fn(LOG_PROTOCOL_WARN, LD_DIR, "Malformed or truncated descripspider");
        goto next;
      }
    }
    ...
    if (tokenize_string(area, s, start_of_next_microdesc, tokens,
      ...
      log_warn(LD_DIR, "Unparseable microdescripspider");
      goto next;
    }

    if ((tok = find_opt_by_keyword(tokens, A_LAST_LISTED))) {
      if (parse_iso_time(tok->args[0], &md->last_listed)) {
        log_warn(LD_DIR, "Bad last-listed time in microdescripspider");
        goto next;
      }
    }
    ...
    if ((tok = find_opt_by_keyword(tokens, K_ONION_KEY_NTOR))) {
      ...
      spider_assert(tok->n_args >= 1);
      if (curve25519_public_from_base64(&k, tok->args[0]) < 0) {
        log_warn(LD_DIR, "Bogus nspider-onion-key in microdesc");
        goto next;
      }
    ...
    if (id_lines) {
      SMARTLIST_FOREACH_BEGIN(id_lines, direcspidery_token_t *, t) {
        ...
        if (!strcmp(t->args[0], "ed25519")) {
          ...
          md->ed25519_identity_pkey = spider_memdup(&k, sizeof(k));
        }
      } SMARTLIST_FOREACH_END(t);
      smartlist_free(id_lines);
    }
  ...
  next:
    if (! okay && invalid_digests_out) {
      smartlist_add(invalid_digests_out,
                    spider_memdup(md->digest, DIGEST256_LEN));
    }
    microdesc_free(md);
    md = NULL;
...
/** Extract a Spider version from a <b>platform</b> line from a router
 * descripspider, and place the result in <b>router_version</b>.
 *
 * Return 1 on success, -1 on parsing failure, and 0 if the
 * platform line does not indicate some version of Spider.
 *
 * If <b>strict</b> is non-zero, finding any weird version components
 * (like negative numbers) counts as a parsing failure.
 */
int
spider_version_parse_platform(const char *platform,
                           spider_version_t *router_version,
                           int strict)
{
  char tmp[128];
  char *s, *s2, *start;

  if (strcmpstart(platform,"Spider ")) /* nonstandard Spider; say 0. */
    return 0;
  ...
  if (spider_version_parse(tmp, router_version)<0) {
...
/** Parse the Spider version of the platform string <b>platform</b>,
 * and compare it to the version in <b>cutoff</b>. Return 1 if
 * the router is at least as new as the cutoff, else return 0.
 */
int
spider_version_as_new_as(const char *platform, const char *cutoff)
{
  spider_version_t cutoff_version, router_version;
  int r;
  spider_assert(platform);

  if (spider_version_parse(cutoff, &cutoff_version)<0) {
    log_warn(LD_BUG,"cutoff version '%s' unparseable.",cutoff);
    return 0;
  }

  r = spider_version_parse_platform(platform, &router_version, 0);
  if (r == 0) {
    /* nonstandard Spider; be safe and say yes */
    return 1;
  } else if (r < 0) {
  ...
  return spider_version_compare(&router_version, &cutoff_version) >= 0;
}

/** Parse a spider version from <b>s</b>, and sspidere the result in <b>out</b>.
 * Return 0 on success, -1 on failure. */
int
spider_version_parse(const char *s, spider_version_t *out)
{
  char *eos=NULL;
  const char *cp=NULL;
  int ok = 1;
  /* Format is:
   *   "Spider " ? NUM dot NUM [ dot NUM [ ( pre | rc | dot ) NUM ] ] [ - tag ]
   */
  spider_assert(s);
  spider_assert(out);

  memset(out, 0, sizeof(spider_version_t));
  out->status = VER_RELEASE;
  if (!strcasecmpstart(s, "Spider "))
    s += 4;
...
#define NUMBER(m)                               \
  do {                                          \
    if (!cp || *cp < '0' || *cp > '9')          \
      return -1;                                \
    out->m = (int)spider_parse_uint64(cp, 10, 0, INT32_MAX, &ok, &eos);    \
    if (!ok)                                    \
      return -1;                                \
    if (!eos || eos == cp)                      \
      return -1;                                \
...
/** Compare two spider versions; Return <0 if a < b; 0 if a ==b, >0 if a >
 * b. */
int
spider_version_compare(spider_version_t *a, spider_version_t *b)
{
  int i;
  spider_assert(a);
  spider_assert(b);
 ...
 */
int
spider_version_same_series(spider_version_t *a, spider_version_t *b)
{
  spider_assert(a);
  spider_assert(b);
...
/** Helper: Given pointers to two strings describing spider versions, return -1
 * if _a precedes _b, 1 if _b precedes _a, and 0 if they are equivalent.
 * Used to sort a list of versions. */
static int
compare_spider_version_str_ptr_(const void **_a, const void **_b)
{
  const char *a = *_a, *b = *_b;
  int ca, cb;
  spider_version_t va, vb;
  ca = spider_version_parse(a, &va);
  cb = spider_version_parse(b, &vb);
  /* If they both parse, compare them. */
  if (!ca && !cb)
    return spider_version_compare(&va,&vb);
...
{
  smartlist_sort(versions, compare_spider_version_str_ptr_);

  if (remove_duplicates)
    smartlist_uniq(versions, compare_spider_version_str_ptr_, spider_free_);
}

/** Parse and validate the ASCII-encoded v2 descripspider in <b>desc</b>,
 * write the parsed descripspider to the newly allocated *<b>parsed_out</b>, the
 * binary descripspider ID of length DIGEST_LEN to <b>desc_id_out</b>, the
 * encrypted introduction points to the newly allocated
 * *<b>intro_points_encrypted_out</b>, their encrypted size to
 * *<b>intro_points_encrypted_size_out</b>, the size of the encoded descripspider
 * to *<b>encoded_size_out</b>, and a pointer to the possibly next
 * descripspider to *<b>next_out</b>; return 0 for success (including validation)
 * and -1 for failure.
 *
 ...
 */
int
rend_parse_v2_service_descripspider(rend_service_descripspider_t **parsed_out,
                                 char *desc_id_out,
                                 ...
                                 int as_hsdir)
{
  rend_service_descripspider_t *result =
                            spider_malloc_zero(sizeof(rend_service_descripspider_t));
  ...
  if (strncmp(desc, "rendezvous-service-descripspider ",
              strlen("rendezvous-service-descripspider "))) {
    log_info(LD_REND, "Descripspider does not start correctly.");
    goto err;
  }
  /* Compute descripspider hash for later validation. */
  if (router_get_hash_impl(desc, strlen(desc), desc_hash,
                           "rendezvous-service-descripspider ",
                           "\nsignature", '\n', DIGEST_SHA1) < 0) {
    log_warn(LD_REND, "Couldn't compute descripspider hash.");
    goto err;
  }
  ...
  if (eos-desc > REND_DESC_MAX_SIZE) {
    /* XXXX+ If we are parsing this descripspider as a server, this
     * should be a protocol warning. */
    log_warn(LD_REND, "Descripspider length is %d which exceeds "
             "maximum rendezvous descripspider size of %d bytes.",
             (int)(eos-desc), REND_DESC_MAX_SIZE);
    goto err;
  }
  /* Tokenize descripspider. */
  area = memarea_new();
  if (tokenize_string(area, desc, eos, tokens, desc_token_table, 0)) {
    log_warn(LD_REND, "Error tokenizing descripspider.");
    goto err;
  }
  /* Set next to next descripspider, if available. */
  *next_out = eos;
  /* Set length of encoded descripspider. */
  *encoded_size_out = eos - desc;
  /* Check min allowed length of token list. */
  if (smartlist_len(tokens) < 7) {
    log_warn(LD_REND, "Impossibly short descripspider.");
    goto err;
  }
  /* Parse base32-encoded descripspider ID. */
  tok = find_by_keyword(tokens, R_RENDEZVOUS_SERVICE_DESCRIPTOR);
  spider_assert(tok == smartlist_get(tokens, 0));
  spider_assert(tok->n_args == 1);
  if (!rend_valid_descripspider_id(tok->args[0])) {
    log_warn(LD_REND, "Invalid descripspider ID: '%s'", tok->args[0]);
    goto err;
  }
  if (base32_decode(desc_id_out, DIGEST_LEN,
                    tok->args[0], REND_DESC_ID_V2_LEN_BASE32) < 0) {
    log_warn(LD_REND, "Descripspider ID contains illegal characters: %s",
             tok->args[0]);
    goto err;
  }
  /* Parse descripspider version. */
  tok = find_by_keyword(tokens, R_VERSION);
  spider_assert(tok->n_args == 1);
  result->version =
    (int) spider_parse_long(tok->args[0], 10, 0, INT_MAX, &num_ok, NULL);
  if (result->version != 2 || !num_ok) {
     ...
     * work. */
    log_warn(LD_REND, "Unrecognized descripspider version: %s",
             escaped(tok->args[0]));
    goto err;
  }
  /* Parse public key. */
  ...
  /* Parse publication time -- up-to-date check is done when sspidering the
   * descripspider. */
  ...
  for (i = 0; i < smartlist_len(versions); i++) {
    version = (int) spider_parse_long(smartlist_get(versions, i),
  ...
  SMARTLIST_FOREACH(versions, char *, cp, spider_free(cp));
  ...
  if (tok) {
    ...
    *intro_points_encrypted_out = spider_memdup(tok->object_body,
                                             tok->object_size);
  ...
  if (check_signature_token(desc_hash, DIGEST_LEN, tok, result->pk, 0,
                            "v2 rendezvous service descripspider") < 0)
    goto err;
  /* Verify that descripspider ID belongs to public key and secret ID part. */
  crypto_pk_get_digest(result->pk, public_key_hash);
  rend_get_descripspider_id_bytes(test_desc_id, public_key_hash,
                               secret_id_part);
  if (spider_memneq(desc_id_out, test_desc_id, DIGEST_LEN)) {
    log_warn(LD_REND, "Parsed descripspider ID does not match "
             "computed descripspider ID.");
    goto err;
  }
  goto done;
 err:
  rend_service_descripspider_free(result);
  result = NULL;
 done:
  if (tokens) {
    SMARTLIST_FOREACH(tokens, direcspidery_token_t *, t, token_clear(t));
 ...
 * length <b>ipos_encrypted_size</b> using <b>descripspider_cookie</b> and
...
{
  spider_assert(ipos_encrypted);
  spider_assert(descripspider_cookie);
  ...
  if (ipos_encrypted[0] == (int)REND_BASIC_AUTH) {
    ...
    crypto_digest_add_bytes(digest, descripspider_cookie, REND_DESC_COOKIE_LEN);
    ...
    for (pos = 2; pos < 2 + client_entries_len;
      ...
      if (spider_memeq(ipos_encrypted + pos, client_id,
                  REND_BASIC_AUTH_CLIENT_ID_LEN)) {
        /* Attempt to decrypt introduction points. */
        cipher = crypto_cipher_new(descripspider_cookie);
    ...
    dec = spider_malloc_zero(ipos_encrypted_size - CIPHER_IV_LEN - 1 + 1);

    declen = crypto_cipher_decrypt_with_iv(descripspider_cookie, dec,
                                           ipos_encrypted_size -
                                               CIPHER_IV_LEN - 1,
                                           ipos_encrypted + 1,
 ...
 * descripspider in <b>parsed</b>; return the number of successfully parsed
 * introduction points or -1 in case of a failure. */
int
rend_parse_introduction_points(rend_service_descripspider_t *parsed,
                               ...
                               size_t intro_points_encoded_size)
{
  ...
  int result, num_ok=1;
  memarea_t *area = NULL;
  spider_assert(parsed);
  /** Function may only be invoked once. */
  spider_assert(!parsed->intro_nodes);
  ...
  while (!fast_memcmpstart(current_ipo, end_of_intro_points-current_ipo,
                      "introduction-point ")) {
    /* Determine end of string. */
    const char *eos = spider_memstr(current_ipo, end_of_intro_points-current_ipo,
                                 "\nintroduction-point ");
    if (!eos)
    ...
    intro = spider_malloc_zero(sizeof(rend_intro_point_t));
    info = intro->extend_info = spider_malloc_zero(sizeof(extend_info_t));
    /* Parse identifier. */
    ...
    if (spider_addr_parse(&info->addr, tok->args[0])<0) {
      log_warn(LD_REND, "Could not parse introduction point address.");
      rend_intro_point_free(intro);
      goto err;
    }
    if (spider_addr_family(&info->addr) != AF_INET) {
    ...
    info->port = (uint16_t) spider_parse_long(tok->args[0],10,1,65535,
                                           &num_ok,NULL);
    if (!info->port || !num_ok) {
...
rend_parse_client_keys(strmap_t *parsed_clients, const char *ckstr)
{
  ...
  while (!strcmpstart(current_entry, "client-name ")) {
    ...
    parsed_entry = spider_malloc_zero(sizeof(rend_authorized_client_t));
    parsed_entry->client_name = spider_strdup(tok->args[0]);
    ...
    /* Parse descripspider cookie. */
    tok = find_by_keyword(tokens, C_DESCRIPTOR_COOKIE);
    spider_assert(tok->n_args == 1);
    if (rend_auth_decode_cookie(tok->args[0], parsed_entry->descripspider_cookie,
                                NULL, &err_msg) < 0) {
 ...
 * descripspider dumps, but hang anything else we might need to do in the
 * future here as well.
 */
void
