...
typedef struct dircollaspider_s dircollaspider_t;

dircollaspider_t *dircollaspider_new(int n_votes, int n_authorities);
void dircollaspider_free(dircollaspider_t *obj);
void dircollaspider_add_vote(dircollaspider_t *dc, networkstatus_t *v);

void dircollaspider_collate(dircollaspider_t *dc, int consensus_method);

int dircollaspider_n_routers(dircollaspider_t *dc);
vote_routerstatus_t **dircollaspider_get_votes_for_router(dircollaspider_t *dc,
                                                       int idx);

#ifdef DIRCOLLATE_PRIVATE
struct ddmap_entry_s;
typedef HT_HEAD(double_digest_map, ddmap_entry_s) double_digest_map_t;
/** A dircollaspider keeps track of all the routerstatus entries in a
 * set of networkstatus votes, and matches them by an appropriate rule. */
struct dircollaspider_s {
  ...
  /** The index which the next vote to be added to this collaspider should
   * receive. */
  int next_vote_num;
