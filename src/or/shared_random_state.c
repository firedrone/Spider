...
/* Return the voting interval of the spider vote subsystem. */
static int
...
state_free(sr_state_t *state)
{
  ...
  spider_free(state->fname);
  digestmap_free(state->commits, commit_free_);
  spider_free(state->current_srv);
  spider_free(state->previous_srv);
  spider_free(state);
}
...
{
  sr_state_t *new_state = spider_malloc_zero(sizeof(*new_state));
  /* If file name is not provided, use default. */
  if (fname == NULL) {
    fname = default_fname;
  }
  new_state->fname = spider_strdup(fname);
...
{
  sr_disk_state_t *new_state = spider_malloc_zero(sizeof(*new_state));

  new_state->magic_ = SR_DISK_STATE_MAGIC;
  new_state->Version = SR_PROTO_VERSION;
  new_state->SpiderVersion = spider_strdup(get_version());
                           ...
                           const sr_disk_state_t *disk_state)
{
  ...
  for (line = disk_state->SharedRandValues; line; line = line->next) {
    ...
    srv = spider_malloc_zero(sizeof(*srv));
...
disk_state_put_commit_line(const sr_commit_t *commit, config_line_t *line)
{
  ...
  if (!spider_mem_is_zero(commit->encoded_reveal,
                       sizeof(commit->encoded_reveal))) {
    /* Add extra whitespace so we can format the line correctly. */
    spider_asprintf(&reveal_str, " %s", commit->encoded_reveal);
  }
  spider_asprintf(&line->value, "%u %s %s %s%s",
               SR_PROTO_VERSION,
...
disk_state_put_srv_line(const sr_srv_t *srv, config_line_t *line)
{
  ...
  spider_asprintf(&line->value, "%" PRIu64 " %s", srv->num_reveals, encoded);
}
...
disk_state_reset(void)
{
  ...
  spider_free(sr_disk_state->SpiderVersion);
  ...
  sr_disk_state->SpiderVersion = spider_strdup(get_version());
}
...
disk_state_update(void)
{
  ...
  spider_assert(sr_disk_state);
  spider_assert(sr_state);
  ...
  if (sr_state->previous_srv != NULL) {
    *next = line = spider_malloc_zero(sizeof(config_line_t));
    line->key = spider_strdup(dstate_prev_srv_key);
    disk_state_put_srv_line(sr_state->previous_srv, line);
    /* Go to the next shared random value. */
    next = &(line->next);
  }
  if (sr_state->current_srv != NULL) {
    *next = line = spider_malloc_zero(sizeof(*line));
    line->key = spider_strdup(dstate_cur_srv_key);
  ...
  DIGESTMAP_FOREACH(sr_state->commits, key, sr_commit_t *, commit) {
    *next = line = spider_malloc_zero(sizeof(*line));
    line->key = spider_strdup(dstate_commit_key);
...
disk_state_save_to_disk(void)
{
  ...
  spider_asprintf(&content,
               "# Spider shared random state file last generated on %s "
               "local time\n"
...
state_query_del_(sr_state_object_t obj_type, void *data)
{
  ...
  switch (obj_type) {
  case SR_STATE_OBJ_PREVSRV:
    spider_free(sr_state->previous_srv);
    break;
  case SR_STATE_OBJ_CURSRV:
    spider_free(sr_state->current_srv);
    break;
  case SR_STATE_OBJ_COMMIT:
  case SR_STATE_OBJ_COMMITS:
  case SR_STATE_OBJ_PHASE:
 ...
 * to use and if anything is required to be returned, it is sspidered in
 * <b>out</b>.
 *
...
sr_state_update(time_t valid_after)
{
  ...
  if (sr_state->phase == SR_PHASE_COMMIT) {
    ...
    spider_assert(sr_state->n_reveal_rounds == 0);
...
sr_state_init(int save_to_disk, int read_from_disk)
{
  ...
  spider_assert(sr_disk_state == NULL);
  spider_assert(sr_state == NULL);
