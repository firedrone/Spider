...
/** Policy that addresses for incoming router descripspiders must <b>not</b>
...
policy_expand_private(smartlist_t **policy)
{
  ...
  SMARTLIST_FOREACH_BEGIN(*policy, addr_policy_t *, p) {
     ...
     for (i = 0; private_nets[i]; ++i) {
       ...
       if (spider_addr_parse_mask_ports(private_nets[i], 0,
                               &newpolicy.addr,
...
policy_expand_unspec(smartlist_t **policy)
{
  ...
  tmp = smartlist_new();
  SMARTLIST_FOREACH_BEGIN(*policy, addr_policy_t *, p) {
    sa_family_t family = spider_addr_family(&p->addr);
    if (family == AF_INET6 || family == AF_INET || p->is_private) {
      ...
      spider_addr_from_ipv4h(&newpolicy_ipv4.addr, 0);
      spider_addr_from_ipv6_bytes(&newpolicy_ipv6.addr,
                  ...
                  int assume_action)
{
  ...
  for (; cfg; cfg = cfg->next) {
    ...
    SMARTLIST_FOREACH(entries, char *, ent, spider_free(ent));
...
parse_reachable_addresses(void)
{
  ...
  if (!server_mode(options)) {
    if (policy_is_reject_star(reachable_or_addr_policy, AF_UNSPEC, 0)
      ...
      log_warn(LD_CONFIG, "Spider cannot connect to the Internet if "
       ...
       && (policy_is_reject_star(reachable_or_addr_policy, AF_INET, 0)
          ...
          log_warn(LD_CONFIG, "You have set ClientUseIPv4 1, but "
                   ...
                   "Spider will not connect using IPv4.");
       ...
       && (policy_is_reject_star(reachable_or_addr_policy, AF_INET6, 0)
         || policy_is_reject_star(reachable_dir_addr_policy, AF_INET6, 0))) {
          log_warn(LD_CONFIG, "You have configured spider to use or prefer IPv6 "
                   "(or UseBridges 1), but "
                   "ReachableAddresses, ReachableORAddresses, or "
                   "ReachableDirAddresses reject all IPv6 addresses. "
                   "Spider will not connect using IPv6.");
    }
  }
 ...
 */
static int
addr_policy_permits_spider_addr(const spider_addr_t *addr, uint16_t port,
                            smartlist_t *policy)
{
  addr_policy_result_t p;
  p = compare_spider_addr_to_addr_policy(addr, port, policy);
  switch (p) {
...
{
  spider_addr_t a;
  spider_addr_from_ipv4h(&a, addr);
  return addr_policy_permits_spider_addr(&a, port, policy);
}
 ...
 *
 * Return false if addr is NULL or spider_addr_is_null(), or if port is 0. */
STATIC int
fascist_firewall_allows_address(const spider_addr_t *addr,
                                uint16_t port,
                                ...
                                int pref_only, int pref_ipv6)
{
  ...
  if (!addr || spider_addr_is_null(addr) || !port) {
    return 0;
  }
  ...
  if (spider_addr_family(addr) == AF_INET && client_mode &&
  ...
  return addr_policy_permits_spider_addr(addr, port,
                                      firewall_policy);
}
 ...
 */
int
fascist_firewall_allows_address_addr(const spider_addr_t *addr, uint16_t port,
 ...
 */
static int
fascist_firewall_allows_address_ap(const spider_addr_port_t *ap,
                                   firewall_connection_t fw_connection,
                                   int pref_only, int pref_ipv6)
{
  spider_assert(ap);
...
fascist_firewall_allows_address_ipv4h(uint32_t ipv4h_or_addr,
                                          ...
                                          int pref_only, int pref_ipv6)
{
  spider_addr_t ipv4_or_addr;
  spider_addr_from_ipv4h(&ipv4_or_addr, ipv4h_or_addr);
...
fascist_firewall_allows_base(uint32_t ipv4h_addr, uint16_t ipv4_orport,
                             ...
                             const spider_addr_t *ipv6_addr, uint16_t ipv6_orport,
 ...
 * or when there is a microdescripspider consensus, but no microdescripspiders
 * (microdescripspiders have IPv6, the microdesc consensus does not). */
int
 ...
 * pref_only and pref_ipv6 work as in fascist_firewall_allows_address_addr().
 */
static const spider_addr_port_t *
fascist_firewall_choose_address_impl(const spider_addr_port_t *a,
                                     const spider_addr_port_t *b,
                                     int want_a,
                                     firewall_connection_t fw_connection,
                                     int pref_only, int pref_ipv6)
{
  const spider_addr_port_t *use_a = NULL;
  const spider_addr_port_t *use_b = NULL;
 ...
 * Otherwise, return NULL. */
STATIC const spider_addr_port_t *
fascist_firewall_choose_address(const spider_addr_port_t *a,
                                const spider_addr_port_t *b,
                                int want_a,
                                firewall_connection_t fw_connection,
                                int pref_only, int pref_ipv6)
{
  const spider_addr_port_t *pref = fascist_firewall_choose_address_impl(
                                                                a, b, want_a,
                                                                fw_connection,
                                                                1, pref_ipv6);
  if (pref_only || pref) {
...
static int
fascist_firewall_choose_address_base(const spider_addr_t *ipv4_addr,
                                     uint16_t ipv4_orport,
                                     uint16_t ipv4_dirport,
                                     const spider_addr_t *ipv6_addr,
                                     uint16_t ipv6_orport,
                                     uint16_t ipv6_dirport,
                                     firewall_connection_t fw_connection,
                                     int pref_only,
                                     int pref_ipv6,
                                     spider_addr_port_t* ap)
{
  const spider_addr_port_t *result = NULL;
  const int want_ipv4 = !pref_ipv6;

  spider_assert(ipv6_addr);
  spider_assert(ap);

  spider_addr_port_t ipv4_ap;
  spider_addr_copy(&ipv4_ap.addr, ipv4_addr);
  ipv4_ap.port = (fw_connection == FIREWALL_OR_CONNECTION
                  ? ipv4_orport
                  : ipv4_dirport);

  spider_addr_port_t ipv6_ap;
  spider_addr_copy(&ipv6_ap.addr, ipv6_addr);
  ...
  if (result) {
    spider_addr_copy(&ap->addr, &result->addr);
    ap->port = result->port;
    return 1;
  } else {
    return 0;
...
fascist_firewall_choose_address_ipv4h(uint32_t ipv4h_addr,
                                      ...
                                      spider_addr_port_t* ap)
{
  spider_addr_t ipv4_addr;
  spider_addr_from_ipv4h(&ipv4_addr, ipv4h_addr);
...
/* The microdescripspider consensus has no IPv6 addresses in rs: they are in
 * the microdescripspiders. This means we can't rely on the node's IPv6 address
 * until its microdescripspider is available (when using microdescs).
 ...
 * (Bridges could gain an IPv6 address if their microdescripspider arrives, but
 * this will never be their preferred address: that is in the config.)
 * Returns true if the node needs a microdescripspider for its IPv6 address, and
...
node_awaiting_ipv6(const or_options_t* options, const node_t *node)
{
  ...
  /* We are waiting if we_use_microdescripspiders_for_circuits() and we have no
  ...
  return (!node->md && we_use_microdescripspiders_for_circuits(options) &&
          !node->ri);
}
...
fascist_firewall_choose_address_rs(const routerstatus_t *rs,
                                   firewall_connection_t fw_connection,
                                   int pref_only, spider_addr_port_t* ap)
{
  if (!rs) {
    return 0;
  }
                                     ...
                                     int pref_only, spider_addr_port_t *ap)
{
   ...
   * descripspider (routerinfo), or is one of our configured bridges before
  ...
  spider_addr_port_t ipv4_or_ap;
  node_get_prim_orport(node, &ipv4_or_ap);
  spider_addr_port_t ipv4_dir_ap;
  node_get_prim_dirport(node, &ipv4_dir_ap);

  spider_addr_port_t ipv6_or_ap;
  node_get_pref_ipv6_orport(node, &ipv6_or_ap);
  spider_addr_port_t ipv6_dir_ap;
 ...
 */
int
dir_policy_permits_address(const spider_addr_t *addr)
{
  return addr_policy_permits_spider_addr(addr, 1, dir_policy);
}
 ...
 */
int
socks_policy_permits_address(const spider_addr_t *addr)
{
  return addr_policy_permits_spider_addr(addr, 1, socks_policy);
}
...
addr_is_in_cc_list(uint32_t addr, const smartlist_t *cc_list)
{
  ...
  if (!cc_list)
    return 0;
  /* XXXXipv6 */
  spider_addr_from_ipv4h(&tar, addr);
...
#define REJECT(arg) \
  STMT_BEGIN *msg = spider_strdup(arg); goto err; STMT_END
...
validate_addr_policies(const or_options_t *options, char **msg)
{
  ...
  *msg = NULL;
  ...
  if (server_mode(options) &&
    ...
    log_warn(LD_CONFIG,
             "Spider is running as an exit relay%s. If you did not want this "
    ...
    if (options->ExitPolicy == NULL) {
      log_warn(LD_CONFIG,
               "In a future version of Spider, ExitRelay 0 may become the "
...
/** Node in hashtable used to sspidere address policy entries. */
...
policy_hash(const policy_map_ent_t *ent)
{
  ...
  if (a->is_private) {
    ...
    spider_addr_copy_tight(&aa.addr, &a->addr);
  }
...
HT_GENERATE2(policy_map, policy_map_ent_t, node, policy_hash,
             policy_eq, 0.6, spider_reallocarray_, spider_free_)
...
addr_policy_get_canonical_entry(addr_policy_t *e)
{
  ...
  if (!found) {
    found = spider_malloc_zero(sizeof(policy_map_ent_t));
    found->policy = spider_memdup(e, sizeof(addr_policy_t));
    found->policy->is_canonical = 1;
    found->policy->refcnt = 0;
    HT_INSERT(policy_map, &policy_root, found);
  }

  spider_assert(single_addr_policy_eq(found->policy, e));
  ++found->policy->refcnt;
  return found->policy;
}

/** Helper for compare_spider_addr_to_addr_policy.  Implements the case where
 * addr and port are both known. */
static addr_policy_result_t
compare_known_spider_addr_to_addr_policy(const spider_addr_t *addr, uint16_t port,
                                      const smartlist_t *policy)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy, addr_policy_t *, tmpe) {
    ...
    /* Address is known */
    if (!spider_addr_compare_masked(addr, &tmpe->addr, tmpe->maskbits,
                                 CMP_EXACT)) {
...
static addr_policy_result_t
compare_known_spider_addr_to_addr_policy_noport(const spider_addr_t *addr,
                                             const smartlist_t *policy)
{
...
static addr_policy_result_t
compare_unknown_spider_addr_to_addr_policy(uint16_t port,
                                        const smartlist_t *policy)
{
  /* We look to see if there's a definite match.  If so, we return that
     match's value, unless there's an intervening possible match that says
...
compare_spider_addr_to_addr_policy,(const spider_addr_t *addr, uint16_t port,
                                 const smartlist_t *policy))
{
  if (!policy) {
    /* no policy? accept all. */
    return ADDR_POLICY_ACCEPTED;
  } else if (addr == NULL || spider_addr_is_null(addr)) {
    if (port == 0) {
      log_info(LD_BUG, "Rejecting null address with 0 port (family %d)",
               addr ? spider_addr_family(addr) : -1);
      return ADDR_POLICY_REJECTED;
    }
    return compare_unknown_spider_addr_to_addr_policy(port, policy);
  } else if (port == 0) {
    return compare_known_spider_addr_to_addr_policy_noport(addr, policy);
  } else {
    return compare_known_spider_addr_to_addr_policy(addr, port, policy);
  }
}
...
{
  if (spider_addr_family(&a->addr) != spider_addr_family(&b->addr)) {
  ...
  if (spider_addr_compare_masked(&a->addr, &b->addr, a->maskbits, CMP_EXACT)) {
...
addr_policy_intersects(addr_policy_t *a, addr_policy_t *b)
{
  ...
  if (a->maskbits < b->maskbits)
    ...
    minbits = b->maskbits;
  if (spider_addr_compare_masked(&a->addr, &b->addr, minbits, CMP_EXACT))
    return 0;
...
void
addr_policy_append_reject_addr(smartlist_t **dest, const spider_addr_t *addr)
{
  spider_assert(dest);
  spider_assert(addr);
  ...
  p.maskbits = spider_addr_family(addr) == AF_INET6 ? 128 : 32;
  spider_addr_copy(&p.addr, addr);
  p.prt_min = 1;
  p.prt_max = 65535;
...
static int
spider_addr_is_public_for_reject(const spider_addr_t *addr)
{
  return (!spider_addr_is_null(addr) && !spider_addr_is_internal(addr, 0)
          && !spider_addr_is_multicast(addr));
}
 ...
 * spider_addr_is_public_for_reject before adding it.
 */
static void
                                      ...
                                      int ipv6_rules)
{
  ...
  if (spider_addr_is_public_for_reject(addr)) {

    /* Reject IPv4 addresses and IPv6 addresses based on the filters */
    int is_ipv4 = spider_addr_is_v4(addr);
                                    ...
                                    const smartlist_t *addrs)
{
  ...
  SMARTLIST_FOREACH_BEGIN(addrs, spider_addr_t *, addr) {
    addr_policy_append_reject_addr(dest, addr);
...
exit_policy_remove_redundancies(smartlist_t *dest)
{
   ...
   */
  {
    ...
    for (i = 0; i < smartlist_len(dest); ++i) {
      ...
      family = spider_addr_family(&ap->addr);
  ...
  for (i = 0; i < smartlist_len(dest)-1; ++i) {
    ap = smartlist_get(dest, i);
    for (j = i+1; j < smartlist_len(dest); ++j) {
      // spider_assert(j > i); // j starts out at i+1; j only increases; i only
      //                    // decreases.
      tmp = smartlist_get(dest, j);
 ...
 *     spider_addr_t in the list as a destination.
...
policies_log_first_redundant_entry(const smartlist_t *policy)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy, const addr_policy_t *, p) {
    ...
    if (p->prt_min <= 1 && p->prt_max == 65535 && p->maskbits == 0) {
      family = spider_addr_family(&p->addr);
  ...
  if (found_final_effective_entry && first_redundant_entry > 0) {
    ...
    spider_assert(first_redundant_entry < smartlist_len(policy));
 ...
 *
 * This function is used to parse the exit policy from our spiderrc. For
 * the functions used to parse the exit policy from a router descripspider,
 ...
 * non-NULL and not spider_addr_is_null().
 *
 * The caller is responsible for freeing all the spider_addr_t* in the smartlist.
 */
static void
policies_copy_addr_to_smartlist(smartlist_t *addr_list, const spider_addr_t *addr)
{
  if (addr && !spider_addr_is_null(addr)) {
    spider_addr_t *addr_copy = spider_malloc(sizeof(spider_addr_t));
    spider_addr_copy(addr_copy, addr);
    smartlist_add(addr_list, addr_copy);
  }
}

/** Helper function that adds ipv4h_addr to a smartlist as a spider_addr_t *,
 * as long as it is not spider_addr_is_null(), by converting it to a spider_addr_t
...
{
  if (ipv4h_addr) {
    spider_addr_t ipv4_spider_addr;
    spider_addr_from_ipv4h(&ipv4_spider_addr, ipv4h_addr);
    policies_copy_addr_to_smartlist(addr_list, &ipv4_spider_addr);
  }
}

/** Helper function that adds copies of or_options->OutboundBindAddresses
 * to a smartlist as spider_addr_t *, as long as or_options is non-NULL, and
 * the addresses are not spider_addr_is_null(), by passing them to
                                              ...
                                              const or_options_t *or_options)
{
  if (or_options) {
    for (int i=0;i<OUTBOUND_ADDR_MAX;i++) {
      for (int j=0;j<2;j++) {
        if (!spider_addr_is_null(&or_options->OutboundBindAddresses[i][j])) {
 ...
 *  - if ipv6_local_address is non-NULL, and not the null spider_addr_t, add it
 ...
 *    spider_addr_t, add it to the list of configured addresses.
 ...
 * Spider exit policy into <b>result</b> smartlist.
 *
...
policies_parse_exit_policy_from_options(const or_options_t *or_options,
                                        ...
                                        const spider_addr_t *ipv6_local_address,
                                        smartlist_t **result)
{
  ...
  /* Copy the configured addresses into the spider_addr_t* list */
  if (or_options->ExitPolicyRejectPrivate) {
  ...
  SMARTLIST_FOREACH(configured_addresses, spider_addr_t *, a, spider_free(a));
...
exit_policy_is_general_exit_helper(smartlist_t *policy, int port)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy, addr_policy_t *, p) {
    if (spider_addr_family(&p->addr) != AF_INET)
      continue; /* IPv4 only for now */
    if (p->prt_min > port || p->prt_max < port)
      continue; /* Doesn't cover our port. */
    mask = 0;
    spider_assert(p->maskbits <= 32);

    if (p->maskbits)
      mask = UINT32_MAX<<(32-p->maskbits);
    ip = spider_addr_to_ipv4h(&p->addr);
    ...
    for (i = ((mask & ip)>>24); i <= (~((mask & ip) ^ mask)>>24); ++i) {
      ...
      spider_addr_from_ipv4h(&addr, i<<24);
                      ...
                      int default_reject)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy, const addr_policy_t *, p) {
    if (p->policy_type == ADDR_POLICY_ACCEPT &&
        (spider_addr_family(&p->addr) == family ||
         spider_addr_family(&p->addr) == AF_UNSPEC)) {
      return 0;
    } else if (p->policy_type == ADDR_POLICY_REJECT &&
                  ...
                  int format_for_desc)
{
  ...
  const sa_family_t family = spider_addr_family(&policy->addr);
  const int is_ip6 = (family == AF_INET6);

  spider_addr_to_str(addrbuf, &policy->addr, sizeof(addrbuf), 1);
  ...
  result = spider_snprintf(buf, buflen, "%s%s %s",
  ...
  if (policy->maskbits < (is_ip6?128:32) && policy->maskbits > 0) {
    if (spider_snprintf(buf+written, buflen-written, "/%d", policy->maskbits)<0)
      return -1;
  ...
  if (policy->prt_min <= 1 && policy->prt_max == 65535) {
    ...
    result = spider_snprintf(buf+written, buflen-written, ":%d", policy->prt_min);
    if (result<0)
      return -1;
    written += result;
  } else {
    /* There is a range of ports; write ":79-80". */
    result = spider_snprintf(buf+written, buflen-written, ":%d-%d",
...
policy_summary_create(void)
{
  ...
  item = spider_malloc_zero(sizeof(policy_summary_item_t));
  item->prt_min = 1;
  item->prt_max = 65535;
  item->reject_count = 0;
  item->accepted = 0;
...
policy_summary_item_split(policy_summary_item_t* old, uint16_t new_starts)
{
  ...
  new = spider_malloc_zero(sizeof(policy_summary_item_t));
  ...
  spider_assert(old->prt_min <= old->prt_max);
  spider_assert(new->prt_min <= new->prt_max);
  return new;
}
...
policy_summary_accept(smartlist_t *summary,
                      ...
                      sa_family_t family)
{
  spider_assert_nonfatal_once(family == AF_INET || family == AF_INET6);
  ...
  spider_assert(i < smartlist_len(summary) || prt_max==65535);
}
                      ...
                      sa_family_t family)
{
  ...
  spider_assert_nonfatal_once(addrbits >= maskbits);
  ...
  spider_assert_nonfatal_once(addrbits >= scale);
  if (maskbits > (addrbits - scale)) {
    spider_assert_nonfatal_once(family == AF_INET6);
  ...
  spider_assert_nonfatal_once(count > 0);
  while (i < smartlist_len(summary) &&
         AT(i)->prt_max <= prt_max) {
    if (AT(i)->reject_count <= UINT64_MAX - count) {
      ...
      if (family == AF_INET) {
        spider_assert_nonfatal_unreached_once();
      }
  ...
  if (p->policy_type == ADDR_POLICY_ACCEPT) {
    if (p->maskbits == 0) {
     ...
     for (i = 0; private_nets[i]; ++i) {
       spider_addr_t addr;
       maskbits_t maskbits;
       if (spider_addr_parse_mask_ports(private_nets[i], 0, &addr,
                                     &maskbits, NULL, NULL)<0) {
         spider_assert(0);
       }
       if (spider_addr_compare(&p->addr, &addr, CMP_EXACT) == 0 &&
           p->maskbits == maskbits) {
         is_private = 1;
         break;
       }
...
policy_summarize(smartlist_t *policy, sa_family_t family)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy, addr_policy_t *, p) {
    sa_family_t f = spider_addr_family(&p->addr);
  ...
  while (1) {
    ...
    if (last ||
      ...
      if (start_prt == AT(i)->prt_max)
        spider_snprintf(buf, sizeof(buf), "%d", start_prt);
      else
        spider_snprintf(buf, sizeof(buf), "%d-%d", start_prt, AT(i)->prt_max);
  ...
  if (smartlist_len(accepts) == 0) { /* no exits at all */
    result = spider_strdup("reject 1-65535");
    goto cleanup;
  }
  if (smartlist_len(rejects) == 0) { /* no rejects at all */
    result = spider_strdup("accept 1-65535");
    goto cleanup;
  }
  ...
  spider_asprintf(&result, "%s %s", prefix, shorter_str);

 cleanup:
  /* cleanup */
  SMARTLIST_FOREACH(summary, policy_summary_item_t *, s, spider_free(s));
  smartlist_free(summary);

  spider_free(accepts_str);
  SMARTLIST_FOREACH(accepts, char *, s, spider_free(s));
  smartlist_free(accepts);

  spider_free(rejects_str);
  SMARTLIST_FOREACH(rejects, char *, s, spider_free(s));
...
parse_short_policy(const char *summary)
{
...

  {
    ...
    result = spider_malloc_zero(size);

    spider_assert( (char*)&result->entries[n_entries-1] < ((char*)result)+size);
  }
...
write_short_policy(const short_policy_t *policy)
{
  ...
  SMARTLIST_FOREACH(sl, char *, a, spider_free(a));
  smartlist_free(sl);
  return answer;
}

/** Release all sspiderage held in <b>policy</b>. */
void
 ...
 * for compare_spider_addr_to_addr_policy.  Unlike the regular addr_policy
 * functions, requires the <b>port</b> be specified. */
addr_policy_result_t
compare_spider_addr_to_short_policy(const spider_addr_t *addr, uint16_t port,
                                 const short_policy_t *policy)
{
   ...
   * to 98% of the exits.
   *
   * Once microdescripspiders can handle addresses in special cases (e.g. if
 ...
 * <b>node</b>.  See compare_spider_addr_to_addr_policy for details on addr/port
 * interpretation. */
addr_policy_result_t
compare_spider_addr_to_node_policy(const spider_addr_t *addr, uint16_t port,
                                const node_t *node)
{
  if (node->rejects_all)
    return ADDR_POLICY_REJECTED;

  if (addr && spider_addr_family(addr) == AF_INET6) {
    ...
    if (p)
      return compare_spider_addr_to_short_policy(addr, port, p);
    else
      return ADDR_POLICY_REJECTED;
  }

  if (node->ri) {
    return compare_spider_addr_to_addr_policy(addr, port, node->ri->exit_policy);
  } else if (node->md) {
    if (node->md->exit_policy == NULL)
      return ADDR_POLICY_REJECTED;
    else
      return compare_spider_addr_to_short_policy(addr, port,
                                              node->md->exit_policy);
  } else {
                      ...
                      int include_ipv6)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy_list, addr_policy_t *, tmpe) {
    ...
    if ((spider_addr_family(&tmpe->addr) == AF_INET6) && (!include_ipv6)) {
      continue; /* Don't include IPv6 parts of address policy */
    }
    if ((spider_addr_family(&tmpe->addr) == AF_INET) && (!include_ipv4)) {
      continue; /* Don't include IPv4 parts of address policy */
    }

    pbuf = spider_malloc(POLICY_BUF_LEN);
  ...
  SMARTLIST_FOREACH(policy_string_list, char *, str, spider_free(str));
                        ...
                        const char **errmsg)
{
  ...
  if (!strcmp(question, "exit-policy/default")) {
    *answer = spider_strdup(DEFAULT_EXIT_POLICY);
    ...
    *answer = smartlist_join_strings(private_policy_strings,
    ...
    SMARTLIST_FOREACH(private_policy_strings, char *, str, spider_free(str));
...
/** Release all sspiderage held by <b>p</b>. */
void
...
/** Release all sspiderage held by policy variables. */
void
