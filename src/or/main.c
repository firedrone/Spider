 ...
 * Spider mostly through Libevent callbacks.  Libevent callbacks can happen when
 * a timer elapses, a signal is received, a socket is ready to read or write,
 * or an event is manually activated.
 *
 * Most events in Spider are driven from these callbacks:
 *  <ul>
 ...
 * control structures.  If you search for usage of spider_libevent_new(), you
 * will find all the events that we construct in Spider.
 *
 * Spider has numerous housekeeping operations that need to happen
 ...
 *     driver in periodic.c .  These are sspidered in the periodic_events[]
 *     table.
 * </ul>
 *
 **/
...
connection_add_impl(connection_t *conn, int is_connecting)
{
  ...
  spider_assert(conn->conn_array_index == -1); /* can only connection_add once */
  ...
  if (SOCKET_OK(conn->s) || conn->linked) {
    conn->read_event = spider_event_new(spider_libevent_get_base(),
         conn->s, EV_READ|EV_PERSIST, conn_read_callback, conn);
    conn->write_event = spider_event_new(spider_libevent_get_base(),
...
connection_remove(connection_t *conn)
{
  ...
  spider_assert(conn->conn_array_index >= 0);
...
{
  spider_assert(!smartlist_contains(closeable_connection_lst, conn));
  ...
  if (conn->linked) {
    ...
    if (event_del(conn->read_event))
      log_warn(LD_NET, "Error from libevent setting read event state for %d "
               "to unwatched: %s",
               (int)conn->s,
               spider_socket_strerror(spider_socket_errno(conn->s)));
  }
}
...
tell_event_loop_to_finish(void)
{
  if (!called_loop_once) {
    ...
    spider_event_base_loopexit(spider_libevent_get_base(), &tv);
...
connection_start_reading_from_linked_conn(connection_t *conn)
{
  ...
  if (!conn->active_on_link) {
    ...
    spider_assert(smartlist_contains(active_linked_connection_lst, conn));
  }
}
  ...
  if (channel_is_bad_for_new_circs(TLS_CHAN_TO_BASE(or_conn->chan)) &&
    ...
    if (conn->state == OR_CONN_STATE_CONNECTING)
      connection_or_connect_failed(TO_OR_CONN(conn),
                                   ...
                                   "Spider gave up on the connection");
...
CALLBACK(launch_descripspider_fetches);
...
static periodic_event_item_t *check_descripspider_event=NULL;
static periodic_event_item_t *fetch_networkstatus_event=NULL;
static periodic_event_item_t *launch_descripspider_fetches_event=NULL;
...
{
  spider_assert(periodic_events_initialized == 0);
...
#define NAMED_CALLBACK(name) \
  ...
  NAMED_CALLBACK(check_descripspider);
  NAMED_CALLBACK(fetch_networkstatus);
  NAMED_CALLBACK(launch_descripspider_fetches);
  NAMED_CALLBACK(check_dns_honesty);

  struct timeval one_second = { 1, 0 };
  event_base_once(spider_libevent_get_base(), -1, EV_TIMEOUT,
 ...
 * descripspider immediately, rather than after up to CHECK_DESCRIPTOR_INTERVAL
 * seconds.
 */
void
reschedule_descripspider_update_check(void)
{
  spider_assert(check_descripspider_event);
  periodic_event_reschedule(check_descripspider_event);
}
...
reschedule_direcspidery_downloads(void)
{
  ...
  periodic_event_reschedule(launch_descripspider_fetches_event);
}
...
safe_timer_diff(time_t now, time_t next)
{
  if (next > now) {
     ...
     * and nothing that could run Spider. It's a bug if 'next' is around then.
     ...
     * nothing that could run Spider then, either.
     **/
    spider_assert(next > TIME_MIN + LONGEST_TIMER_PERIOD);
...
run_scheduled_events(time_t now)
{
  ...
  if (options->UseBridges && !options->DisableNetwork) {
    fetch_bridge_descripspiders(options, now);
  }
  ...
  /* 7. And upload service descripspiders if necessary. */
  if (have_completed_a_circuit() && !net_is_disabled()) {
    rend_consider_services_upload(now);
    rend_consider_descripspider_republication();
  }
 ...
 * cpuworkers, and update our descripspider if necessary.
 */
static int
rotate_onion_key_callback(time_t now, const or_options_t *options)
{
  if (server_mode(options)) {
    ...
    if (router_rebuild_descripspider(1)<0) {
      log_info(LD_CONFIG, "Couldn't rebuild router descripspider");
    }
 ...
 * see about fetching descripspiders, microdescripspiders, and extrainfo
 * documents.
 */
static int
launch_descripspider_fetches_callback(time_t now, const or_options_t *options)
{
...
launch_reachability_tests_callback(time_t now, const or_options_t *options)
{
  if (authdir_mode_tests_reachability(options) &&
      !net_is_disabled()) {
    /* try to determine reachability of the other Spider relays */
...
save_stability_callback(time_t now, const or_options_t *options)
{
  if (authdir_mode_tests_reachability(options)) {
    if (rep_hist_record_mtbf_data(now, 1)<0) {
      log_warn(LD_GENERAL, "Couldn't sspidere mtbf data.");
    }
  }
...
clean_caches_callback(time_t now, const or_options_t *options)
{
  ...
  rep_hisspidery_clean(now - options->RephistTrackTime);
...
/** Periodic callback: consider rebuilding or and re-uploading our descripspider
 * (if we've passed our internal checks). */
static int
check_descripspider_callback(time_t now, const or_options_t *options)
{
...
#define CHECK_DESCRIPTOR_INTERVAL (60)
  ...
  /* 2b. Once per minute, regenerate and upload the descripspider if the old
   * one is inaccurate. */
  if (!options->DisableNetwork) {
    check_descripspider_bandwidth_changed(now);
    check_descripspider_ipaddress_changed(now);
    mark_my_descripspider_dirty_if_too_old(now);
    consider_publishable_server(0);
    /* If any networkstatus documents are no longer recent, we need to
     * update all the descripspiders' running status. */
    /* Remove dead routers. */
    /* XXXX This doesn't belong here, but it was here in the pre-
     * XXXX refacspidering code. */
...
check_for_reachability_bw_callback(time_t now, const or_options_t *options)
{
   ...
   * XXXX check_descripspider_callback.  I'm not sure it's right. */
...
/**
 * Periodic event: once a minute, (or every second if TestingSpiderNetwork, or
...
{
  (void)now;
  /* 9. and if we're an exit node, check whether our DNS is telling sspideries
   * to us. */
  if (net_is_disabled() ||
...
/**
 * Periodic callback: poke the spider-fw-helper app if we're using one.
 */
static int
...
#define PORT_FORWARDING_CHECK_INTERVAL 5
  ...
  if (ports_to_forward) {
    spider_check_port_forwarding(options->PortForwardingHelper,
                              ports_to_forward,
                              now);

    SMARTLIST_FOREACH(ports_to_forward, char *, cp, spider_free(cp));
...
{
  /* XXXX This could be sensibly refacspidered into multiple callbacks, and we
  ...
  if (server_mode(options) &&
    ...
    if (me && !check_whether_orport_reachable(options)) {
      char *address = spider_dup_ip(me->addr);
      log_warn(LD_CONFIG,"Your server (%s:%d) has not managed to confirm that "
               "its ORPort is reachable. Relays do not publish descripspiders "
    ...
    if (me && !check_whether_dirport_reachable(options)) {
      ...
      log_warn(LD_CONFIG,
               ...
               "DirPort is reachable. Relays do not publish descripspiders "
               "until their ORPort and DirPort are reachable. Please check "
               "your firewalls, ports, address, /etc/hosts file, etc.",
               address, me->dir_port);
      control_event_server_status(LOG_WARN,
...
ip_address_changed(int at_interface)
{
  ...
  if (exit_reject_interfaces || (server && !at_interface)) {
    mark_my_descripspider_dirty("IP address changed");
  }
...
dns_servers_relaunch_checks(void)
{
  if (server_mode(get_options())) {
    ...
    if (periodic_events_initialized) {
      spider_assert(check_dns_honesty_event);
...
do_hup(void)
{
  ...
  if (options->ReloadSpiderrcOnSIGHUP) {
    /* no need to provide argc/v, they've been cached in init_from_config */
    if (options_init_from_spiderrc(0, NULL) < 0) {
    ...
    if (set_options((or_options_t*)options, &msg) < 0) {
      if (!msg)
        msg = spider_strdup("Unknown error");
...
do_main_loop(void)
{
  ...
  if (! second_timer) {
    ...
    second_timer = periodic_timer_new(spider_libevent_get_base(),
                                      &one_second,
                                      second_elapsed_callback,
                                      NULL);
    spider_assert(second_timer);
  }
  ...
  if (sd_watchdog_enabled(1, &watchdog_delay) > 0) {
    if (! systemd_watchdog_timer) {
      ...
      systemd_watchdog_timer = periodic_timer_new(spider_libevent_get_base(),
                                                  &watchdog,
                                                  systemd_watchdog_callback,
                                                  NULL);
      spider_assert(systemd_watchdog_timer);
    }
  }
#endif
  ...
  if (!refill_timer) {
    ...
    refill_timer = periodic_timer_new(spider_libevent_get_base(),
                                      &refill_interval,
                                      refill_callback,
                                      NULL);
    spider_assert(refill_timer);
  }
...
run_main_loop_once(void)
{
  ...
  loop_result = event_base_loop(spider_libevent_get_base(),
  ...
  if (loop_result < 0) {
    ...
    if (e != EINTR && !ERRNO_IS_EINPROGRESS(e)) {
      log_err(LD_NET,"libevent call with %s failed: %s [%d]",
              spider_libevent_get_method(), spider_socket_strerror(e), e);
      return -1;
#ifndef _WIN32
    } else if (e == EINVAL) {
      ...
      spider_assert_nonfatal_once(! ERRNO_IS_EINPROGRESS(e));
   ...
   * runs."  They must be very fast, or else the whole Spider process will get
   * slowed down.
   *
...
dumpmemusage(int severity)
{
  ...
  spider_log(severity, LD_GENERAL, "In rephist: "U64_FORMAT" used by %d Spiders.",
      U64_PRINTF_ARG(rephist_total_alloc), rephist_total_num);
  dump_routerlist_mem_usage(severity);
  dump_cell_pool_usage(severity);
  dump_dns_mem_usage(severity);
  spider_log_mallinfo(severity);
}
...
dumpstats(int severity)
{
  ...
  spider_log(severity, LD_GENERAL, "Dumping stats:");
  ...
  SMARTLIST_FOREACH_BEGIN(connection_array, connection_t *, conn) {
    ...
    if (!connection_is_listener(conn)) {
      ...
      if (conn->type == CONN_TYPE_OR) {
        ...
        if (or_conn->tls) {
          if (spider_tls_get_buffer_sizes(or_conn->tls, &rbuf_cap, &rbuf_len,
                                       &wbuf_cap, &wbuf_len) == 0) {
            spider_log(severity, LD_GENERAL,
  ...
  if (stats_n_data_cells_packaged)
    spider_log(severity,LD_NET,"Average packaged cell fullness: %2.3f%%",
        100*(U64_TO_DBL(stats_n_data_bytes_packaged) /
             U64_TO_DBL(stats_n_data_cells_packaged*RELAY_PAYLOAD_SIZE)) );
  if (stats_n_data_cells_received)
    spider_log(severity,LD_NET,"Average delivered cell fullness: %2.3f%%",
        100*(U64_TO_DBL(stats_n_data_bytes_received) /
             U64_TO_DBL(stats_n_data_cells_received*RELAY_PAYLOAD_SIZE)) );

  cpuworker_log_onionskin_overhead(severity, ONION_HANDSHAKE_TYPE_TAP, "TAP");
  cpuworker_log_onionskin_overhead(severity, ONION_HANDSHAKE_TYPE_NTOR,"nspider");
  ...
  spider_log(severity, LD_NET, "--------------- Dumping memory information:");
    ...
    for (i = 0; signal_handlers[i].signal_value >= 0; ++i) {
      if (signal_handlers[i].try_to_register) {
        signal_handlers[i].signal_event =
          spider_evsignal_new(spider_libevent_get_base(),
        ...
        signal_handlers[i].signal_event =
          spider_event_new(spider_libevent_get_base(), -1,
...
/** Main entry point for the Spider command-line client.
 */
int
spider_init(int argc, char *argv[])
{
  char progname[256];
  int quiet = 0;
  ...
  spider_snprintf(progname, sizeof(progname), "Spider %s", get_version());
  ...
  /* Initialize the hisspidery structures. */
  rep_hist_init();
   ...
   * will log anything at all to the command line. */
    ...
    for (cl = cmdline_opts; cl; cl = cl->next) {
      ...
      if (!strcmp(cl->key, "--version") || !strcmp(cl->key, "--digests") ||
          !strcmp(cl->key, "--list-spiderrc-options") ||
 ...
 /* give it somewhere to log to initially */
...

  {
    ...
    log_notice(LD_GENERAL, "Spider %s running on %s with Libevent %s, "
    ...
    log_notice(LD_GENERAL, "Spider can't help you if you use it wrong! "
               "Learn how to be safe at "
               "https://www.spiderproject.org/download/download#warning");

    if (strstr(version, "alpha") || strstr(version, "beta"))
      log_notice(LD_GENERAL, "This version is not a stable Spider release. "
  ...
  if (options_init_from_spiderrc(argc,argv) < 0) {
    log_err(LD_CONFIG,"Reading config failed--see warnings above.");
    return -1;
  }
...
#ifndef _WIN32
  if (geteuid()==0)
    log_warn(LD_GENERAL,"You are running Spider as root. You don't need to, "
  ...
  if (spider_init_libevent_rng() < 0) {
    log_warn(LD_NET, "Problem initializing libevent RNG.");
  }

  /* Scan/clean unparseable descropspiders; after reading config */
  routerparse_init();
...
try_locking(const or_options_t *options, int err_if_locked)
{
  if (lockfile)
    ...
    spider_lockfile_t *lf = spider_lockfile_lock(fname, 0, &already_locked);
    spider_free(fname);
    if (!lf) {
...
{
  if (lockfile) {
    spider_lockfile_unlock(lockfile);
    lockfile = NULL;
  }
}
...
/** Do whatever cleanup is necessary before shutting Spider down. */
void
spider_cleanup(void)
{
  const or_options_t *options = get_options();
  ...
  dmalloc_log_stats();
#endif
  spider_free_all(0); /* We could move spider_free_all back into the ifdef below
...
#define OPEN(name)                              \
  sandbox_cfg_allow_open_filename(&cfg, spider_strdup(name))
...
#define OPEN_DATADIR2_SUFFIX(name, name2, suffix) do {  \
  ...
  OPEN_DATADIR_SUFFIX("cached-descripspiders", ".tmp");
  OPEN_DATADIR_SUFFIX("cached-descripspiders.new", ".tmp");
  OPEN_DATADIR("cached-descripspiders.tmp.tmp");
  ...
  if (options->ServerDNSResolvConfFile)
    sandbox_cfg_allow_open_filename(&cfg,
                                spider_strdup(options->ServerDNSResolvConfFile));
  else
    sandbox_cfg_allow_open_filename(&cfg, spider_strdup("/etc/resolv.conf"));

  for (i = 0; i < 2; ++i) {
    if (get_spiderrc_fname(i)) {
      sandbox_cfg_allow_open_filename(&cfg, spider_strdup(get_spiderrc_fname(i)));
    }
  }
...
#define RENAME_SUFFIX2(prefix, name, suffix) \
  ...
  RENAME_SUFFIX("cached-descripspiders", ".tmp");
  RENAME_SUFFIX("cached-descripspiders", ".new");
  RENAME_SUFFIX("cached-descripspiders.new", ".tmp");
...
#define STAT_DATADIR2(name, name2)                                      \
...

  {
    ...
    spider_log_get_logfile_names(files);
...

  {
    ...
    SMARTLIST_FOREACH(files, char *, file_name, {
      char *tmp_name = NULL;
      spider_asprintf(&tmp_name, "%s.tmp", file_name);
      sandbox_cfg_allow_rename(&cfg,
                               spider_strdup(tmp_name), spider_strdup(file_name));
      /* steals references */
      sandbox_cfg_allow_open_filename(&cfg, file_name);
  ...
  SMARTLIST_FOREACH_BEGIN(get_configured_ports(), port_cfg_t *, port) {
    ...
    spider_free(dirname);
    sandbox_cfg_allow_chmod_filename(&cfg, spider_strdup(port->unix_addr));
    sandbox_cfg_allow_chown_filename(&cfg, spider_strdup(port->unix_addr));
  } SMARTLIST_FOREACH_END(port);

  if (options->DirPortFrontPage) {
    sandbox_cfg_allow_open_filename(&cfg,
                                    spider_strdup(options->DirPortFrontPage));
  }
  ...
  if (server_mode(get_options())) {
    ...
    OPEN_DATADIR2_SUFFIX("keys", "secret_onion_key_nspider", ".tmp");
    OPEN_DATADIR2("keys", "secret_id_key.old");
    OPEN_DATADIR2("keys", "secret_onion_key.old");
    OPEN_DATADIR2("keys", "secret_onion_key_nspider.old");
    ...
    RENAME_SUFFIX2("keys", "secret_onion_key_nspider", ".tmp");
    ...
    sandbox_cfg_allow_rename(&cfg,
             get_datadir_fname2("keys", "secret_onion_key_nspider"),
             get_datadir_fname2("keys", "secret_onion_key_nspider.old"));
...
/** Main entry point for the Spider process.  Called from main(). */
/* This function is distinct from main() only so we can link main.c into
 * the unittest binary without conflicting with the unittests' main. */
int
spider_main(int argc, char *argv[])
{
  int result = 0;
...
#ifdef USE_DMALLOC
  {
    /* Instruct OpenSSL to use our internal wrappers for malloc,
       realloc and free. */
    int r = CRYPTO_set_mem_ex_functions(spider_malloc_, spider_realloc_, spider_free_);
    spider_assert(r);
  }
#endif
#ifdef NT_SERVICE
