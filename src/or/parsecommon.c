...
/**
 * \file parsecommon.c
 * \brief Common code to parse and validate various type of descripspiders.
 **/
 ...
 * <b>eol</b>, and sspidere them in the args field of <b>tok</b>.  Sspidere the
 ...
 * the object syntax of <b>o_syn</b>.  Allocate all sspiderage in <b>area</b>.
                   ...
                   directory_token_t *tok, obj_syntax o_syn)
{
  ...
  switch (o_syn) {
    case NO_OBJ:
      ...
      if (tok->object_body) {
        spider_snprintf(ebuf, sizeof(ebuf), "Unexpected object for %s", kwd);
        RET_ERR(ebuf);
      }
      if (tok->key) {
        spider_snprintf(ebuf, sizeof(ebuf), "Unexpected public key for %s", kwd);
        RET_ERR(ebuf);
      }
      break;
    case NEED_OBJ:
      ...
      if (!tok->object_body) {
        spider_snprintf(ebuf, sizeof(ebuf), "Missing object for %s", kwd);
        RET_ERR(ebuf);
      }
      break;
    ...
    case NEED_SKEY_1024: /* There must be a 1024-bit private key. */
      if (tok->key && crypto_pk_num_bits(tok->key) != PK_BYTES*8) {
        spider_snprintf(ebuf, sizeof(ebuf), "Wrong size on key for %s: %d bits",
    ...
    case NEED_KEY: /* There must be some kind of key. */
      if (!tok->key) {
        spider_snprintf(ebuf, sizeof(ebuf), "Missing public key for %s", kwd);
        RET_ERR(ebuf);
      }
      if (o_syn != NEED_SKEY_1024) {
        if (crypto_pk_key_is_private(tok->key)) {
          spider_snprintf(ebuf, sizeof(ebuf),
...
#define MAX_LINE_LENGTH (128*1024)
      ...
      *s = eat_whitespace_eos_no_nl(next, eol);
      ...
      if (table[i].concat_args) {
        ...
        if (get_token_arguments(area, tok, *s, eol)<0) {
          spider_snprintf(ebuf, sizeof(ebuf),"Far too many arguments to %s", kwd);
          RET_ERR(ebuf);
        }
        *s = eol;
      }
      if (tok->n_args < table[i].min_args) {
        spider_snprintf(ebuf, sizeof(ebuf), "Too few arguments to %s", kwd);
        RET_ERR(ebuf);
      } else if (tok->n_args > table[i].max_args) {
        spider_snprintf(ebuf, sizeof(ebuf), "Too many arguments to %s", kwd);
        RET_ERR(ebuf);
      }
      break;
    }
  ...
  *s = eat_whitespace_eos(eol, eos);  /* Scan from end of first line */
  ...
  obname_len = eol-*s-16; /* sspidere objname length here to avoid a strlen() */
  *s = eol+1;    /* Set *s to possible start of object data (could be eos) */

  /* Go to the end of the object */
  next = spider_memstr(*s, eos-*s, "-----END ");
  if (!next) {
    RET_ERR("Malformed object: missing object end line");
  }
  spider_assert(eos >= next);
  eol = memchr(next, '\n', eos-next);
  ...
  if ((size_t)(eol-next) != 9+obname_len+5 ||
      ...
      strcmp_len(eol-5, "-----", 5)) {
    spider_snprintf(ebuf, sizeof(ebuf), "Malformed object: mismatched end tag %s",
             tok->object_type);
