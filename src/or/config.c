...
/**
 * \file config.c
 * \brief Code to interpret the user's configuration of Spider.
 *
 * This module handles spiderrc configuration file, including parsing it,
 * combining it with spiderrc.defaults and the command line, allowing
 ...
 *
 * To add new items to the spiderrc, there are a minimum of three places to edit:
 * <ul>
 *   <li>The or_options_t structure in or.h, where the options are sspidered.
 *   <li>The option_vars_ array below in this module, which configures
 *       the names of the spiderrc options, their types, their multiplicities,
 *       and their mappings to fields in or_options_t.
 *   <li>The manual in doc/spider.1.txt, to document what the new option
 *       is, and how it works.
 * </ul>
 *
 ...
 *       forbid some or all changes in the option while Spider is
 *       running.
 *   <li>options_transition_affects_workers(), in case changes in the option
 *       might require Spider to relaunch or reconfigure its worker threads.
 *   <li>options_transition_affects_descripspider(), in case changes in the
 *       option might require a Spider relay to build and publish a new server
 *       descripspider.
 ...
 * secondary field to initialize the one that Spider actually looks at; and that
 * you use the one Spider looks as the one that you modify.
 **/
...
static config_abbrev_t option_abbrevs_[] = {
  ...
  PLURAL(EntryNode),
  PLURAL(ExcludeNode),
  PLURAL(Spider2webRendezvousPoint),
  PLURAL(FirewallPort),
  ...
  { "BridgeAuthoritativeDirecspidery", "BridgeAuthoritativeDir", 0, 0},
 ...
 * socks port, but SpiderBrowser wants to use __SocksPort so that it
 * isn't sspidered by SAVECONF.  The SocksPortLines virtual option is
...
static config_var_t option_vars_[] = {
  ...
  VAR("AuthoritativeDirecspidery",  BOOL, AuthoritativeDir,    "0"),
  ...
  V(DataDirecspidery,               FILENAME, NULL),
  V(DataDirecspideryGroupReadable,  BOOL,     "0"),
  ...
  OBSOLETE("DisableV2DirecspideryInfo_"),
  ...
  V(TestingEstimatedDescripspiderPropagationTime, INTERVAL, "10 minutes"),
  ...
  V(FetchServerDescripspiders,      BOOL,     "1"),
  V(FetchHidServDescripspiders,     BOOL,     "1"),
  V(FetchUselessDescripspiders,     BOOL,     "0"),
  ...
  V(GeoIPFile,                   FILENAME,
    SHARE_DATADIR PATH_SEPARATOR "spider" PATH_SEPARATOR "geoip"),
  V(GeoIPv6File,                 FILENAME,
    SHARE_DATADIR PATH_SEPARATOR "spider" PATH_SEPARATOR "geoip6"),
#endif
  OBSOLETE("Group"),
  ...
  OBSOLETE("HidServDirecspideryV2"),
  ...
  V(HTTPProxyAuthenticaspider,      STRING,   NULL),
  V(HTTPSProxy,                  STRING,   NULL),
  V(HTTPSProxyAuthenticaspider,     STRING,   NULL),
  V(IPv6Exit,                    BOOL,     "0"),
  ...
  OBSOLETE("NamingAuthoritativeDirecspidery"),
  ...
  V(NumCPUs,                     UINT,     "0"),
  V(NumDirecspideryGuards,          UINT,     "0"),
  ...
  OBSOLETE("PathBiasScaleFacspider"),
  OBSOLETE("PathBiasMultFacspider"),
  ...
  V(PortForwardingHelper,        FILENAME, "spider-fw-helper"),
  OBSOLETE("PreferTunneledDirConns"),
  V(ProtocolWarnings,            BOOL,     "0"),
  V(PublishServerDescripspider,     CSV,      "1"),
  V(PublishHidServDescripspiders,   BOOL,     "1"),
  ...
  V(Spider2webMode,                 BOOL,     "0"),
  V(Spider2webRendezvousPoints,      ROUTERSET, NULL),
  ...
  V(UseMicrodescripspiders,         AUTOBOOL, "auto"),
  OBSOLETE("UseNSpiderHandshake"),
  V(User,                        STRING,   NULL),
  OBSOLETE("UserspaceIOCPBuffers"),
  V(AuthDirSharedRandomness,     BOOL,     "1"),
  V(AuthDirTestEd25519LinkKeys,  BOOL,     "1"),
  OBSOLETE("V1AuthoritativeDirecspidery"),
  OBSOLETE("V2AuthoritativeDirecspidery"),
  VAR("V3AuthoritativeDirecspidery",BOOL, V3AuthoritativeDir,   "0"),
  ...
  VAR("VersioningAuthoritativeDirecspidery",BOOL,VersioningAuthoritativeDir, "0"),
  OBSOLETE("VoteOnHidServDirecspideriesV2"),
  V(VirtualAddrNetworkIPv4,      STRING,   "127.192.0.0/10"),
  V(VirtualAddrNetworkIPv6,      STRING,   "[FE80::]/10"),
  V(WarnPlaintextPorts,          CSV,      "23,109,110,143"),
  OBSOLETE("UseFilteringSSLBufferevents"),
  OBSOLETE("__UseFilteringSSLBufferevents"),
  VAR("__ReloadSpiderrcOnSIGHUP",   BOOL,  ReloadSpiderrcOnSIGHUP,      "1"),
  ...
  V(MinUptimeHidServDirecspideryV2, INTERVAL, "96 hours"),
   ...
   * blackholed. Clients will try 3 direcspideries simultaneously.
   ...
   * to give up earlier. (This protects against overloading direcspideries.) */
  ...
  V(TestingDescripspiderMaxDownloadTries, UINT, "8"),
...
/** Override default values with these if the user sets the TestingSpiderNetwork
 * option. */
static const config_var_t testing_spider_network_defaults[] = {
  V(ServerDNSAllowBrokenConfig,  BOOL,     "1"),
  ...
  V(TestingEstimatedDescripspiderPropagationTime, INTERVAL, "0 minutes"),
  V(MinUptimeHidServDirecspideryV2, INTERVAL, "0 minutes"),
  ...
  V(TestingDescripspiderMaxDownloadTries, UINT, "80"),
...
static int options_transition_affects_descripspider(
      const or_options_t *old_options, const or_options_t *new_options);
...
static int validate_data_direcspidery(or_options_t *options);
 ...
 * in spiderrc before we fall back to Spider's defaults. */
static or_options_t *global_default_options = NULL;
/** Name of most recently read spiderrc file. */
static char *spiderrc_fname = NULL;
/** Name of the most recently read spiderrc-defaults file.*/
static char *spiderrc_defaults_fname;
...
set_options(or_options_t *new_val, char **msg)
{
  ...
  /* Issues a CONF_CHANGED event to notify controller of the change. If Spider is
  ...
  if (old_options && old_options != global_options) {
    ...
    SMARTLIST_FOREACH(elements, char *, cp, spider_free(cp));
...
extern const char spider_git_revision[]; /* from spider_main.c */

/** The version of this Spider process, as parsed. */
static char *the_spider_version = NULL;
/** A shorter version of this Spider process's version, for export in our router
 *  descripspider.  (Does not include the git version, if any.) */
static char *the_short_spider_version = NULL;

/** Return the current Spider version. */
const char *
get_version(void)
{
  if (the_spider_version == NULL) {
    if (strlen(spider_git_revision)) {
      spider_asprintf(&the_spider_version, "%s (git-%s)", get_short_version(),
                   spider_git_revision);
    } else {
      the_spider_version = spider_strdup(get_short_version());
    }
  }
  return the_spider_version;
}

/** Return the current Spider version, without any git tag. */
const char *
get_short_version(void)
{

  if (the_short_spider_version == NULL) {
#ifdef TOR_BUILD_TAG
    spider_asprintf(&the_short_spider_version, "%s (%s)", VERSION, TOR_BUILD_TAG);
#else
    the_short_spider_version = spider_strdup(VERSION);
#endif
  }
  return the_short_spider_version;
}
...
or_options_free(or_options_t *options)
{
  ...
  spider_free(options->BridgePassword_AuthDigest_);
  spider_free(options->command_arg);
  spider_free(options->master_key_fname);
...
config_free_all(void)
{
  ...
  spider_free(spiderrc_fname);
  spider_free(spiderrc_defaults_fname);
  spider_free(global_dirfrontpagecontents);

  spider_free(the_short_spider_version);
  spider_free(the_spider_version);
}
...
/** List of default direcspidery authorities */
...
/** List of fallback direcspidery authorities. The list is generated by opt-in of
...
/** Add the default direcspidery authorities directly into the trusted dir list,
...
/** Add the default fallback direcspidery servers into the fallback direcspidery
 * server list. */
MOCK_IMPL(void,
...
/** Look at all the config options for using alternate direcspidery
...
validate_dir_servers(or_options_t *options, or_options_t *old_options)
{
  ...
  if ((options->DirAuthorities &&
    ...
    log_warn(LD_CONFIG,
             ...
             "specify alternate direcspidery authorities in "
             "your configuration. This is potentially dangerous: it can "
             "make you look different from all other Spider users, and hurt "
             "your anonymity. Even if you've specified the same "
             "authorities as Spider uses by default, the defaults could "
             "change in the future. Be sure you know what you're doing.");
  }

  /* Now go through the four ways you can configure an alternate
   * set of direcspidery authorities, and make sure none are broken. */
                            ...
                            const or_options_t *old_options)
{
   ...
   * the unit tests. */
  spider_assert(!(options->DirAuthorities &&
               (options->AlternateDirAuthority
  ...
  if (!options->DirAuthorities) {
    ...
    if (!options->AlternateDirAuthority) {
      ...
      /* Only add the default fallback direcspideries when the DirAuthorities,
       * AlternateDirAuthority, and FallbackDir direcspidery config options
...
options_act_reversible(const or_options_t *old_options, char **msg)
{
  ...
  int running_spider = options->command == CMD_RUN_TOR;
   ...
   * processes. */
  if (running_spider && options->RunAsDaemon) {
  ...
  if (options->ControlSocket || options->ControlSocketsGroupWritable) {
    *msg = spider_strdup("Unix domain sockets (ControlSocket) not supported "
                      "on this OS/with this build.");
    goto rollback;
  }
#else
  if (options->ControlSocketsGroupWritable && !options->ControlSocket) {
    *msg = spider_strdup("Setting ControlSocketGroupWritable without setting"
  ...
  if (running_spider) {
    ...
    if (! sandbox_is_active()) {
      if (set_max_file_descripspiders((unsigned)options->ConnLimit,
                                   &options->ConnLimit_) < 0) {
        *msg = spider_strdup("Problem with ConnLimit value. "
                          "See logs for details.");
        goto rollback;
      }
      set_conn_limit = 1;
    } else {
      spider_assert(old_options);
      options->ConnLimit_ = old_options->ConnLimit_;
    }

    /* Set up libevent.  (We need to do this before we can register the
     * listeners as listeners.) */
    if (running_spider && !libevent_initialized) {
       ...
       * options_init_from_spiderrc() sets up libevent - why yes, that seems
    ...
    if (parse_ports(options, 0, msg, &n_ports, NULL)) {
      if (!*msg)
        *msg = spider_strdup("Unexpected problem parsing port config");
      goto rollback;
    }
    ...
    if (!we_are_hibernating()) {
      if (retry_all_listeners(replaced_listeners, new_listeners,
        ...
        *msg = spider_strdup("Failed to bind one of the listener ports.");
        goto rollback;
      }
    }
    if (options->DisableNetwork) {
      /* Aggressively close non-controller stuff, NOW */
      log_notice(LD_NET, "DisableNetwork is set. Spider will not make or accept "
  ...
  if (options->TransPort_set &&
    ...
    if (get_pf_socket() < 0) {
      *msg = spider_strdup("Unable to open /dev/pf for transparent proxy.");
      goto rollback;
    }
  }
#endif
  ...
  if (options->DisableAllSwap) {
    if (spider_mlockall() == -1) {
      *msg = spider_strdup("DisableAllSwap failure. Do you have proper "
                        "permissions?");
      goto done;
    }
  }
  ...
  if (options->User) {
    spider_assert(have_low_ports != -1);
    ...
    if (switch_id(options->User, switch_id_flags) != 0) {
      /* No need to roll back, since you can't change the value. */
      *msg = spider_strdup("Problem with User value. See logs for details.");
      goto done;
    }
  }

  /* Ensure data direcspidery is private; create if possible. */
  cpd_check_t cpd_opts = running_spider ? CPD_CREATE : CPD_CHECK;
  if (options->DataDirecspideryGroupReadable)
      cpd_opts |= CPD_GROUP_READ;
  if (check_private_dir(options->DataDirecspidery,
                        cpd_opts,
                        options->User)<0) {
    spider_asprintf(msg,
              "Couldn't access/create private data direcspidery \"%s\"",
  ...
  if (options->DataDirecspideryGroupReadable) {
    ...
    if (chmod(options->DataDirecspidery, 0750)) {
      log_warn(LD_FS,"Unable to make %s group-readable: %s",
               options->DataDirecspidery, strerror(errno));
    }
  }
#endif

  /* Bail out at this point if we're not going to be a client or server:
   * we don't run Spider itself. */
  if (!running_spider)
    goto commit;

  mark_logs_temp(); /* Close current logs once new logs are open. */
  logs_marked = 1;
  /* Configure the spider_log(s) */
  if (options_init_logs(old_options, options, 0)<0) {
    *msg = spider_strdup("Failed to init Log options. See logs for details.");
    goto rollback;
  }
  logs_initialized = 1;
 ...
 commit:
  ...
  if (logs_marked) {
    log_severity_list_t *severity =
      spider_malloc_zero(sizeof(log_severity_list_t));
    close_temp_logs();
    add_callback_log(severity, control_event_logmsg);
    control_adjust_event_log_severity();
    spider_free(severity);
    spider_log_update_sigsafe_err_fds();
  }
  if (logs_initialized) {
 ...
 rollback:
  ...
  if (set_conn_limit && old_options)
    set_max_file_descripspiders((unsigned)old_options->ConnLimit,
                             &options->ConnLimit_);
...
options_need_geoip_info(const or_options_t *options, const char **reason_out)
{
  ...
  int routerset_usage =
    ...
    routerset_needs_geoip(options->Spider2webRendezvousPoints);
...
options_act(const or_options_t *old_options)
{
  ...
  /* disable ptrace and later, other basic debugging techniques */
  {
    ...
    if (options->DisableDebuggerAttachment && !disabled_debugger_attach &&
        running_spider) {
      int ok = spider_disable_debugger_attach();
  ...
  if (running_spider && !have_lockfile()) {
  ...
  if (rend_non_anonymous_mode_enabled(options)) {
    log_warn(LD_GENERAL, "This copy of Spider was compiled or configured to run "
             "in a non-anonymous mode. It will provide NO ANONYMITY.");
  }

#ifdef ENABLE_TOR2WEB_MODE
/* LCOV_EXCL_START */
  if (!options->Spider2webMode) {
    log_err(LD_CONFIG, "This copy of Spider was compiled to run in "
            "'spider2web mode'. It can only be run with the Spider2webMode spiderrc "
            "option enabled.");
    return -1;
  }
/* LCOV_EXCL_STOP */
#else
  if (options->Spider2webMode) {
    log_err(LD_CONFIG, "This copy of Spider was not compiled to run in "
            "'spider2web mode'. It cannot be run with the Spider2webMode spiderrc "
            "option enabled. To enable Spider2webMode recompile with the "
            "--enable-spider2web-mode option.");
    return -1;
  }
#endif
  ...
  if (server_mode(options) && options->ServerTransportPlugin &&
    ...
    log_notice(LD_CONFIG, "We use pluggable transports but the Extended "
               "ORPort is disabled. Spider and your pluggable transports proxy "
               ...
               "transports. Please enable it using the ExtORPort spiderrc option "
  ...
  if (running_spider && rend_config_services(options, 0)<0) {
    log_warn(LD_BUG,
       "Previously validated hidden services line could not be added!");
    return -1;
  }

  if (running_spider && rend_parse_service_authorization(options, 0) < 0) {
  ...
  /* Load state */
  if (! or_state_loaded() && running_spider) {
    if (or_state_load())
      return -1;
  ...
  if (options->RunAsDaemon) {
    ...
    finish_daemon(options->DataDirecspidery);
  }
  ...
  monispider_owning_controller_process(options->OwningControllerProcess);
  ...
  cell_ewma_set_scale_facspider(options, networkstatus_get_latest_consensus());
  ...
  /* Update the BridgePassword's hashed version as needed.  We sspidere this as a
   * digest so that we can do side-channel-proof comparisons on it.
   */
  if (options->BridgePassword) {
    char *http_authenticaspider;
    http_authenticaspider = alloc_http_authenticaspider(options->BridgePassword);
    if (!http_authenticaspider) {
      log_warn(LD_BUG, "Unable to allocate HTTP authenticaspider. Not setting "
               "BridgePassword.");
      return -1;
    }
    options->BridgePassword_AuthDigest_ = spider_malloc(DIGEST256_LEN);
    crypto_digest256(options->BridgePassword_AuthDigest_,
                     http_authenticaspider, strlen(http_authenticaspider),
                     DIGEST_SHA256);
    spider_free(http_authenticaspider);
  }
  ...
  if (old_options) {
    ...
    if ((options->UseEntryGuards && !old_options->UseEntryGuards) ||
        ...
        !routerset_equal(old_options->Spider2webRendezvousPoints,
                         options->Spider2webRendezvousPoints) ||
...
#define RELAY_BRIDGE_STATS_DELAY (6 * 60 * 60)
  ...
  /* Only collect direcspidery-request statistics on relays and bridges. */
    ...
    if ((!old_options || !old_options->DirReqStatistics) &&
      ...
      if (geoip_is_loaded(AF_INET)) {
        ...
        if (options->ORPort_set)
          log_notice(LD_CONFIG, "Configured to measure direcspidery request "
    ...
    if (print_notice)
        log_notice(LD_CONFIG, "Configured to measure statistics. Look for "
                "the *-stats files that will first be written to the "
                 "data direcspidery in 24 hours from now.");
  }
   ...
   */
  if (!old_options ||
      options_transition_affects_descripspider(old_options, options))
    mark_my_descripspider_dirty("config change");

  /* We may need to reschedule some direcspidery stuff if our status changed. */
  if (old_options) {
    if (authdir_mode_v3(options) && !authdir_mode_v3(old_options))
      dirvote_recalculate_timing(options, time(NULL));
    if (!bool_eq(direcspidery_fetches_dir_info_early(options),
                 direcspidery_fetches_dir_info_early(old_options)) ||
        !bool_eq(direcspidery_fetches_dir_info_later(options),
                 direcspidery_fetches_dir_info_later(old_options))) {
...
static const struct {
  ...
  { "--allow-missing-spiderrc",  TAKES_NO_ARGUMENT },
  { "--defaults-spiderrc",       ARGUMENT_NECESSARY },
  ...
  { "--ignore-missing-spiderrc", TAKES_NO_ARGUMENT },
  ...
  { "--list-spiderrc-options",   TAKES_NO_ARGUMENT },
                         ...
                         config_line_t **cmdline_result)
{
  ...
  while (i < argc) {
    ...
    if (want_arg == ARGUMENT_NECESSARY && is_last) {
      ...
      arg = (want_arg != TAKES_NO_ARGUMENT) ? spider_strdup(argv[i+1]) :
                                              spider_strdup("");
    }

    param = spider_malloc_zero(sizeof(config_line_t));
    param->key = is_cmdline ? spider_strdup(argv[i]) :
                   spider_strdup(config_expand_abbrev(&options_format, s, 1, 1));
    param->value = arg;
...
/** Print a usage message for spider. */
static void
print_usage(void)
{
  printf(
...
"Copyright (c) 2004-2006, Roger Dingledine, Nick Mathewson\n"
"Copyright (c) 2007-2017, The Spider Project, Inc.\n\n"
"spider -f <spiderrc> [args]\n"
"See man page for options, or https://www.spiderproject.org/ for "
"documentation.\n");
}

/** Print all non-obsolete spiderrc options. */
static void
list_spiderrc_options(void)
{
  int i;
...
/** Print all deprecated but non-obsolete spiderrc options. */
static void
...
/**
 * Attempt getting our non-local (as judged by spider_addr_is_internal()
                   ...
                   const char **method_out, char **hostname_out)
{
  ...
  spider_addr_t myaddr;
  spider_assert(addr_out);
  ...
  if (address && *address) {
    ...
    if (spider_gethostname(hostname, sizeof(hostname)) < 0) {
  ...
  if (spider_inet_aton(hostname, &in) == 0) {
    /* then we have to resolve it */
    explicit_ip = 0;
    if (spider_lookup_hostname(hostname, &addr)) { /* failed to resolve */
      ...
      spider_addr_from_ipv4h(&myaddr, addr);

      if (!explicit_hostname &&
          spider_addr_is_internal(&myaddr, 0)) {
        spider_addr_t interface_ip;
        ...
        } else if (spider_addr_is_internal(&interface_ip, 0)) {
          log_fn(notice_severity, LD_CONFIG,
                 "Interface IP address '%s' is a private address too. "
                 "Ignoring.", fmt_addr(&interface_ip));
        } else {
          from_interface = 1;
          addr = spider_addr_to_ipv4h(&interface_ip);
          log_fn(notice_severity, LD_CONFIG,
  ...
  addr_string = spider_dup_ip(addr);
  if (spider_addr_is_internal(&myaddr, 0)) {
    ...
    if (using_default_dir_authorities(options)) {
      ...
      log_fn(warn_severity, LD_CONFIG,
             ...
             "Spider servers that use the default DirAuthorities must have "
             "public IP addresses.", hostname, addr_string);
      spider_free(addr_string);
      return -1;
    }
    if (!explicit_ip) {
  ...
  *addr_out = addr;
  ...
  if (hostname_out)
    *hostname_out = hostname_used ? spider_strdup(hostname_used) : NULL;
 ...
 */
MOCK_IMPL(int,
is_local_addr, (const spider_addr_t *addr))
{
  if (spider_addr_is_internal(addr, 0))
    return 1;
  /* Check whether ip is on the same /24 as we are. */
  if (get_options()->EnforceDistinctSubnets == 0)
    return 0;
  if (spider_addr_family(addr) == AF_INET) {
    uint32_t ip = spider_addr_to_ipv4h(addr);
     ...
     * it was on net 0, which is already done by spider_addr_is_internal.
     */
 ...
 * include options that are the same as Spider's defaults.
 */
char *
...
validate_ports_csv(smartlist_t *sl, const char *name, char **msg)
{
  ...
  SMARTLIST_FOREACH(sl, const char *, cp,
  {
    i = atoi(cp);
    if (i < 1 || i > 65535) {
      spider_asprintf(msg, "Port '%s' out of range in %s", cp, name);
      return -1;
    }
  });
  return 0;
...
ensure_bandwidth_cap(uint64_t *value, const char *desc, char **msg)
{
  ...
  if (*value > ROUTER_MAX_DECLARED_BANDWIDTH) {
    spider_asprintf(msg, "%s ("U64_FORMAT") must be at most %d",
...
/** Parse an authority type from <b>options</b>-\>PublishServerDescripspider
 * and write it to <b>options</b>-\>PublishServerDescripspider_. Treat "1"
 * as "v3" unless BridgeRelay is 1, in which case treat it as "bridge".
 * Treat "0" as "".
 * Return 0 on success or -1 if not a recognized authority type (in which
 * case the value of PublishServerDescripspider_ is undefined). */
static int
compute_publishserverdescripspider(or_options_t *options)
{
  smartlist_t *list = options->PublishServerDescripspider;
  dirinfo_type_t *auth = &options->PublishServerDescripspider_;
  *auth = NO_DIRINFO;
  if (!list) /* empty list, answer is none */
    return 0;
  SMARTLIST_FOREACH_BEGIN(list, const char *, string) {
    if (!strcasecmp(string, "v1"))
      log_warn(LD_CONFIG, "PublishServerDescripspider v1 has no effect, because "
                          "there are no v1 direcspidery authorities anymore.");
      ...
      log_warn(LD_CONFIG, "PublishServerDescripspider v2 has no effect, because "
                          "there are no v2 direcspidery authorities anymore.");
      ...
      *auth |= BRIDGE_DIRINFO;
      ...
      log_warn(LD_CONFIG,
               "PublishServerDescripspider hidserv is invalid. See "
               "PublishHidServDescripspiders.");
 ...
 * services can overload the direcspidery system. */
...
/** Lowest allowable value for MaxCircuitDirtiness; if this is too low, Spider
...
/** Lowest allowable value for CircuitStreamTimeout; if this is too low, Spider
 * will generate too many circuits and potentially overload the network. */
...
#define REJECT(arg) \
  STMT_BEGIN *msg = spider_strdup(arg); return -1; STMT_END
                                ...
                                char *filepath)
{
  if (filepath && path_is_relative(filepath)) {
    ...
    spider_free(abs_path);
  }
}

/** Scan <b>options</b> for occurances of relative file/direcspidery
 * path and log a warning whenever it is found.
 */
static void
warn_about_relative_paths(or_options_t *options)
{
  spider_assert(options);
  ...
  warn_if_option_path_is_relative("DataDirecspidery",options->DataDirecspidery);
...
options_validate_single_onion(or_options_t *options, char **msg)
{
  ...
  if (rend_service_non_anonymous_mode_enabled(options) && client_port_set &&
      !options->Spider2webMode) {
    REJECT("HiddenServiceNonAnonymousMode is incompatible with using Spider as "
   ...
   * loses anonymity, even if SOCKSPort / Spider2web mode isn't used. */
  if (!rend_service_non_anonymous_mode_enabled(options) &&
      options->RendConfigLines && options->Spider2webMode) {
    REJECT("Non-anonymous (Spider2web) mode is incompatible with using Spider as a "
           "hidden service. Please remove all HiddenServiceDir lines, or use "
           "a version of spider compiled without --enable-spider2web-mode, or use "
           "HiddenServiceNonAnonymousMode.");
  }

  if (rend_service_allow_non_anonymous_connection(options)
      && options->UseEntryGuards) {
    /* Single Onion services only use entry guards when uploading descripspiders;
     ...
     * detecspider, and it's far easier to turn off entry guards (and
     * thus the path bias detecspider with it) than to figure out how to
 ...
 *
 * On error, spider_strdup an error explanation into *<b>msg</b>.
 *
 * XXX
                 ...
                 or_options_t *default_options, int from_setconf, char **msg)
{
  ...
  *msg = NULL;
  ...
  if (server_mode(options) &&
    ...
    log_warn(LD_CONFIG, "Spider is running as a server, but you are "
        "running %s; this probably won't work. See "
        "https://www.spiderproject.org/docs/faq.html#BestOSForRelay "
        "for details.", uname);
  }
  ...
  if (validate_data_direcspidery(options)<0)
    REJECT("Invalid DataDirecspidery");

  if (options->Nickname == NULL) {
    if (server_mode(options)) {
        options->Nickname = spider_strdup(UNNAMED_ROUTER_NICKNAME);
    }
  } else {
  ...
  if (server_mode(options) && options->RendConfigLines)
    log_warn(LD_CONFIG,
        "Spider is currently configured as a relay and a hidden service. "
        "That's not very secure: you should probably run your hidden service "
        "in a separate Spider process, at least -- see "
        "https://trac.spiderproject.org/8742");
  ...
  if (n_ports == 0 && !options->RendConfigLines)
    log_warn(LD_CONFIG,
        ...
        "Spider will still run, but probably won't do anything.");
  ...
  if (options->TLSECGroup && (strcasecmp(options->TLSECGroup, "P256") &&
    ...
    spider_free(options->TLSECGroup);
  }
  ...
  if (options->ExcludeNodes && options->StrictNodes) {
    COMPLAIN("You have asked to exclude certain relays from all positions "
             "in your circuits. Expect hidden services and other Spider "
  ...
  if (options->AuthoritativeDir) {
    if (!options->ContactInfo && !options->TestingSpiderNetwork)
      REJECT("Authoritative direcspidery servers must set ContactInfo");
    ...
    if (options->UseEntryGuards) {
      log_info(LD_CONFIG, "Authoritative direcspidery servers can't set "
               "UseEntryGuards. Disabling.");
      options->UseEntryGuards = 0;
    }
    if (!options->DownloadExtraInfo && authdir_mode_any_main(options)) {
      log_info(LD_CONFIG, "Authoritative direcspideries always try to download "
  ...
  if (options->AuthoritativeDir && !options->DirPort_set)
    REJECT("Running as authoritative direcspidery, but no DirPort set.");

  if (options->AuthoritativeDir && !options->ORPort_set)
    REJECT("Running as authoritative direcspidery, but no ORPort set.");

  if (options->AuthoritativeDir && options->ClientOnly)
    REJECT("Running as authoritative direcspidery, but ClientOnly also set.");
  ...
  if (options->FascistFirewall && !options->ReachableAddresses) {
    if (options->FirewallPorts && smartlist_len(options->FirewallPorts)) {
      ...
      config_line_t *new_line = spider_malloc_zero(sizeof(config_line_t));
      new_line->key = spider_strdup("ReachableAddresses");
      ...
      SMARTLIST_FOREACH(instead, char *, cp, spider_free(cp));
      ...
      if (!options->ReachableDirAddresses) {
        config_line_t *new_line = spider_malloc_zero(sizeof(config_line_t));
        new_line->key = spider_strdup("ReachableDirAddresses");
        new_line->value = spider_strdup("*:80");
      ...
      if (!options->ReachableORAddresses) {
        config_line_t *new_line = spider_malloc_zero(sizeof(config_line_t));
        new_line->key = spider_strdup("ReachableORAddresses");
        new_line->value = spider_strdup("*:443");
  ...
  for (i=0; i<3; i++) {
    ...
    for (;;) {
      linep = &((*linep)->next);
      if (!*linep) {
        *linep = spider_malloc_zero(sizeof(config_line_t));
        (*linep)->key = spider_strdup(
          (i==0) ?  "ReachableAddresses" :
            (i==1) ? "ReachableORAddresses" :
                     "ReachableDirAddresses");
        (*linep)->value = spider_strdup("reject *:*");
        break;
      }
    }
  }
  ...
  if (compute_publishserverdescripspider(options) < 0) {
    spider_asprintf(msg, "Unrecognized value in PublishServerDescripspider");
    return -1;
  }

  if ((options->BridgeRelay
        || options->PublishServerDescripspider_ & BRIDGE_DIRINFO)
      && (options->PublishServerDescripspider_ & V3_DIRINFO)) {
    REJECT("Bridges are not supposed to publish router descripspiders to the "
           "direcspidery authorities. Please correct your "
  ...
  if (options->MinUptimeHidServDirecspideryV2 < 0) {
    log_warn(LD_CONFIG, "MinUptimeHidServDirecspideryV2 option must be at "
                        "least 0 seconds. Changing to 0.");
    options->MinUptimeHidServDirecspideryV2 = 0;
  }

  const int min_rendpostperiod =
    options->TestingSpiderNetwork ?
...
#ifdef ENABLE_TOR2WEB_MODE
  if (options->Spider2webMode && options->UseEntryGuards) {
    /* spider2web mode clients do not (and should not) use entry guards
     * in any meaningful way.  Further, spider2web mode causes the hidden
     * service client code to do things which break the path bias
     * detecspider, and it's far easier to turn off entry guards (and
     * thus the path bias detecspider with it) than to figure out how to
     * make a piece of code which cannot possibly help spider2web mode
     * users compatible with spider2web mode.
     */
    log_notice(LD_CONFIG,
               "Spider2WebMode is enabled; disabling UseEntryGuards.");
    options->UseEntryGuards = 0;
  }
#endif

  if (options->Spider2webRendezvousPoints && !options->Spider2webMode) {
    REJECT("Spider2webRendezvousPoints cannot be set without Spider2webMode.");
  }
  ...
  if (!(options->UseEntryGuards) &&
    ...
    log_warn(LD_CONFIG,
             ...
             "hidden services on this Spider instance.  Your hidden services "
  ...
  if (options->EntryNodes &&
    ...
    spider_asprintf(msg,
             ...
             "https://trac.spiderproject.org/projects/spider/ticket/14917. "
  ...
  if (rend_service_non_anonymous_mode_enabled(options)) {
    log_warn(LD_CONFIG,
             ...
             "this spider instance is NON-ANONYMOUS. If "
             "the HiddenServiceNonAnonymousMode option is changed, Spider will "
             "refuse to launch hidden services from the same direcspideries, to "
  ...
  if (!options->LearnCircuitBuildTimeout && options->CircuitBuildTimeout &&
    ...
    log_warn(LD_CONFIG,
        ...
        "If spider isn't working, raise this value or enable "
  ...
  if (server_mode(options)) {
    ...
    if (have_enough_mem_for_dircache(options, 0, &dircache_msg)) {
      ...
      spider_free(dircache_msg);
    }
  }

  if (options->HTTPProxy) { /* parse it now */
    if (spider_addr_port_lookup(options->HTTPProxy,
  ...
  if (options->HTTPProxyAuthenticaspider) {
    if (strlen(options->HTTPProxyAuthenticaspider) >= 512)
      REJECT("HTTPProxyAuthenticaspider is too long (>= 512 chars).");
  }

  if (options->HTTPSProxy) { /* parse it now */
    if (spider_addr_port_lookup(options->HTTPSProxy,
  ...
  if (options->HTTPSProxyAuthenticaspider) {
    if (strlen(options->HTTPSProxyAuthenticaspider) >= 512)
      REJECT("HTTPSProxyAuthenticaspider is too long (>= 512 chars).");
  }

  if (options->Socks4Proxy) { /* parse it now */
    if (spider_addr_port_lookup(options->Socks4Proxy,
  ...
  if (options->Socks5Proxy) { /* parse it now */
    if (spider_addr_port_lookup(options->Socks5Proxy,
  ...
  if (options->HTTPProxy && !(options->Socks4Proxy ||
    ...
    log_warn(LD_CONFIG, "HTTPProxy configured, but no SOCKS proxy or "
             ...
             "proxy unencrypted direcspidery connections only.");
  }
  ...
  if (options->HashedControlPassword) {
    ...
    if (!sl) {
      ...
      SMARTLIST_FOREACH(sl, char*, cp, spider_free(cp));
      smartlist_free(sl);
    }
  }
  ...
  if (options->OwningControllerProcess) {
    ...
    if (spider_validate_process_specifier(options->OwningControllerProcess,
                                       &validate_pspec_msg)) {
      spider_asprintf(msg, "Bad OwningControllerProcess: %s",
                   validate_pspec_msg);
      return -1;
    }
  }
  ...
  if ((options->ControlPort_set || world_writable_control_socket) &&
    ...
    log_warn(LD_CONFIG, "Control%s is %s, but no authentication method "
             ...
             "computer can reconfigure your Spider.  That's bad!  You should "
             "upgrade your Spider controller as soon as possible.",
  ...
  if (validate_dir_servers(options, old_options) < 0)
    REJECT("Direcspidery authority/fallback line did not parse. See logs "
           "for details.");
  ...
  if (options->ServerTransportPlugin && !server_mode(options)) {
    log_notice(LD_GENERAL, "Spider is not configured as a relay but you specified"
  ...
  for (cl = options->ServerTransportOptions; cl; cl = cl->next) {
    ...
    SMARTLIST_FOREACH(options_sl, char *, cp, spider_free(cp));
  ...
  if (options->ConstrainedSockets) {
    ...
    if (options->DirPort_set) {
      /* Providing cached direcspidery entries while system TCP buffers are scarce
       * will exacerbate the socket errors.  Suggest that this be disabled. */
      COMPLAIN("You have requested constrained socket buffers while also "
               "serving direcspidery entries via DirPort.  It is strongly "
               "suggested that you disable serving direcspidery requests when "
  ...
  if (options->V3AuthVoteDelay + options->V3AuthDistDelay >=
    ...
    if (options->TestingSpiderNetwork) {
      if (options->V3AuthVoteDelay + options->V3AuthDistDelay >=
  ...
  if (options->TestingSpiderNetwork &&
      !(options->DirAuthorities ||
        (options->AlternateDirAuthority &&
         options->AlternateBridgeAuthority))) {
    REJECT("TestingSpiderNetwork may only be configured in combination with "
...
#define CHECK_DEFAULT(arg)                                              \
  STMT_BEGIN                                                            \
    if (!options->TestingSpiderNetwork &&                                  \
        ...
        !config_is_same(&options_format,options,                        \
                        default_options,#arg)) {                        \
      REJECT(#arg " may only be changed in testing Spider "                \
             "networks!");                                              \
    } STMT_END
  ...
  CHECK_DEFAULT(TestingEstimatedDescripspiderPropagationTime);
  ...
  CHECK_DEFAULT(TestingDescripspiderMaxDownloadTries);
...
#undef CHECK_DEFAULT
  ...
  if (options->TestingEstimatedDescripspiderPropagationTime < 0) {
    REJECT("TestingEstimatedDescripspiderPropagationTime must be non-negative.");
  } else if (options->TestingEstimatedDescripspiderPropagationTime > 60*60) {
    COMPLAIN("TestingEstimatedDescripspiderPropagationTime is insanely high.");
  }
  ...
  if (options->TestingDescripspiderMaxDownloadTries < 2) {
    REJECT("TestingDescripspiderMaxDownloadTries must be greater than 1.");
  } else if (options->TestingDescripspiderMaxDownloadTries > 800) {
    COMPLAIN("TestingDescripspiderMaxDownloadTries is insanely high.");
  }
  ...
  if (options->TestingEnableConnBwEvent &&
      !options->TestingSpiderNetwork && !options->UsingTestNetworkDefaults_) {
  ...
  if (options->TestingSpiderNetwork) {
    log_warn(LD_CONFIG, "TestingSpiderNetwork is set. This will make your node "
                        "almost unusable in the public Spider network, and is "
  ...
  if (options->AccelDir && !options->AccelName)
    REJECT("Can't use hardware crypto acceleraspider dir without engine name.");

  if (options->PublishServerDescripspider)
    SMARTLIST_FOREACH(options->PublishServerDescripspider, const char *, pubdes, {
      if (!strcmp(pubdes, "1") || !strcmp(pubdes, "0"))
        if (smartlist_len(options->PublishServerDescripspider) > 1) {
          COMPLAIN("You have passed a list of multiple arguments to the "
                   "PublishServerDescripspider option that includes 0 or 1. "
  ...
  *msg = NULL;
  ...
  if (options->DirCache) {
    if (total_mem < DIRCACHE_MIN_MEM_BYTES) {
      if (options->BridgeRelay) {
        *msg = spider_strdup("Running a Bridge with less than "
                      STRINGIFY(DIRCACHE_MIN_MEM_MB) " MB of memory is not "
                      "recommended.");
      } else {
        *msg = spider_strdup("Being a direcspidery cache (default) with less than "
                      STRINGIFY(DIRCACHE_MIN_MEM_MB) " MB of memory is not "
    ...
    if (total_mem >= DIRCACHE_MIN_MEM_BYTES) {
      *msg = spider_strdup("DirCache is disabled and we are configured as a "
                           ...
                           char **msg)
{
  ...
  if (!opt_streq(old->PidFile, new_val->PidFile)) {
    *msg = spider_strdup("PidFile is not allowed to change.");
    return -1;
  }

  if (old->RunAsDaemon != new_val->RunAsDaemon) {
    *msg = spider_strdup("While Spider is running, changing RunAsDaemon "
                      "is not allowed.");
    return -1;
  }

  if (old->Sandbox != new_val->Sandbox) {
    *msg = spider_strdup("While Spider is running, changing Sandbox "
                      "is not allowed.");
    return -1;
  }

  if (strcmp(old->DataDirecspidery,new_val->DataDirecspidery)!=0) {
    spider_asprintf(msg,
               "While Spider is running, changing DataDirecspidery "
               "(\"%s\"->\"%s\") is not allowed.",
               old->DataDirecspidery, new_val->DataDirecspidery);
    return -1;
  }

  if (!opt_streq(old->User, new_val->User)) {
    *msg = spider_strdup("While Spider is running, changing User is not allowed.");
    return -1;
  }

  if (old->KeepBindCapabilities != new_val->KeepBindCapabilities) {
    *msg = spider_strdup("While Spider is running, changing KeepBindCapabilities is "
                      "not allowed.");
    return -1;
  }

  if (!opt_streq(old->SyslogIdentityTag, new_val->SyslogIdentityTag)) {
    *msg = spider_strdup("While Spider is running, changing "
  ...
  if ((old->HardwareAccel != new_val->HardwareAccel)
    ...
    *msg = spider_strdup("While Spider is running, changing OpenSSL hardware "
                      "acceleration engine is not allowed.");
    return -1;
  }

  if (old->TestingSpiderNetwork != new_val->TestingSpiderNetwork) {
    *msg = spider_strdup("While Spider is running, changing TestingSpiderNetwork "
                      "is not allowed.");
    return -1;
  }

  if (old->DisableAllSwap != new_val->DisableAllSwap) {
    *msg = spider_strdup("While Spider is running, changing DisableAllSwap "
                      "is not allowed.");
    return -1;
  }

  if (old->TokenBucketRefillInterval != new_val->TokenBucketRefillInterval) {
    *msg = spider_strdup("While Spider is running, changing TokenBucketRefill"
  ...
  if (old->DisableDebuggerAttachment &&
    ...
    *msg = spider_strdup("While Spider is running, disabling "
...
#define SB_NOCHANGE_STR(opt)                                            \
    do {                                                                \
      if (! opt_streq(old->opt, new_val->opt)) {                        \
        *msg = spider_strdup("Can't change " #opt " while Sandbox is active"); \
        return -1;                                                      \
      }                                                                 \
    } while (0)
...
#undef SB_NOCHANGE_STR
    ...
    if (! config_lines_eq(old->Logs, new_val->Logs)) {
      *msg = spider_strdup("Can't change Logs while Sandbox is active");
      return -1;
    }
    if (old->ConnLimit != new_val->ConnLimit) {
      *msg = spider_strdup("Can't change ConnLimit while Sandbox is active");
      return -1;
    }
    if (server_mode(old) != server_mode(new_val)) {
      *msg = spider_strdup("Can't start/stop being a server while "
                        "Sandbox is active");
      return -1;
    }
  }
...
options_transition_affects_workers(const or_options_t *old_options,
                                   const or_options_t *new_options)
{
  if (!opt_streq(old_options->DataDirecspidery, new_options->DataDirecspidery) ||
...
/** Return 1 if any change from <b>old_options</b> to <b>new_options</b>
 * will require us to generate a new descripspider; else return 0. */
static int
options_transition_affects_descripspider(const or_options_t *old_options,
                                      const or_options_t *new_options)
{
  ...
  if (!opt_streq(old_options->DataDirecspidery, new_options->DataDirecspidery) ||
      ...
      old_options->PublishServerDescripspider_ !=
        new_options->PublishServerDescripspider_ ||
...
#ifdef _WIN32
/** Return the direcspidery on windows where we expect to find our application
 * data. */
static char *
...
#define APPDATA_PATH CSIDL_APPDATA
  ...
  /* Now we need to free the memory that the path-idl was sspidered in.  In
  ...
  strlcat(path,"\\spider",MAX_PATH);
  is_set = 1;
  return path;
}
#endif

/** Return the default location for our spiderrc file (if <b>defaults_file</b> is
 * false), or for the spiderrc-defaults file (if <b>defaults_file</b> is true). */
static const char *
get_default_conf_file(int defaults_file)
{
  ...
  if (defaults_file) {
    ...
    spider_snprintf(defaults_path, MAX_PATH, "%s\\spiderrc-defaults",
                 get_windows_conf_root());
    return defaults_path;
  } else {
    static char path[MAX_PATH+1];
    spider_snprintf(path, MAX_PATH, "%s\\spiderrc",
                 get_windows_conf_root());
    return path;
  }
#else
  return defaults_file ? CONFDIR "/spiderrc-defaults" : CONFDIR "/spiderrc";
#endif
}
...
check_nickname_list(char **lst, const char *name, char **msg)
{
  ...
  SMARTLIST_FOREACH_BEGIN(sl, char *, s)
    {
      if (!is_legal_nickname_or_hexdigest(s)) {
        ...
        if (s[0] != '$') {
          ...
          spider_free(prepended);
        }

        spider_asprintf(msg, "Invalid nickname '%s' in %s line", s, name);
        r = -1;
        break;
      }
    }
  ...
  SMARTLIST_FOREACH(sl, char *, s, spider_free(s));
  smartlist_free(sl);
 ...
 *
 * If <b>defaults_file</b> is true, we're looking for spiderrc-defaults;
 * otherwise, we're looking for the regular spiderrc_file.
 *
 ...
 *
 * Set *<b>ignore_missing_spiderrc</b> to true if we should ignore the resulting
 * filename if it doesn't exist.
 */
static char *
find_spiderrc_filename(config_line_t *cmd_arg,
                    int defaults_file,
                    int *using_default_fname, int *ignore_missing_spiderrc)
{
  char *fname=NULL;
  config_line_t *p_index;
  const char *fname_opt = defaults_file ? "--defaults-spiderrc" : "-f";
  const char *ignore_opt = defaults_file ? NULL : "--ignore-missing-spiderrc";

  if (defaults_file)
    *ignore_missing_spiderrc = 1;
  ...
  if (*using_default_fname) {
    ...
    if (dflt && (st == FN_FILE || st == FN_EMPTY)) {
      fname = spider_strdup(dflt);
    } else {
#ifndef _WIN32
      char *fn = NULL;
      if (!defaults_file) {
        fn = expand_filename("~/.spiderrc");
      }
      if (fn) {
        ...
        fname = dflt ? spider_strdup(dflt) : NULL;
      }
#else
      fname = dflt ? spider_strdup(dflt) : NULL;
#endif
...
/** Read the spiderrc from standard input and return it as a string.
 * Upon failure, return NULL.
 */
static char *
load_spiderrc_from_stdin(void)
{
   size_t sz_out;
...
/** Load a configuration file from disk, setting spiderrc_fname or
 * spiderrc_defaults_fname if successful.
 *
 * If <b>defaults_file</b> is true, load spiderrc-defaults; otherwise load spiderrc.
 *
 * Return the contents of the file on success, and NULL on failure.
 */
static char *
load_spiderrc_from_disk(config_line_t *cmd_arg, int defaults_file)
{
  char *fname=NULL;
  char *cf = NULL;
  int using_default_spiderrc = 1;
  int ignore_missing_spiderrc = 0;
  char **fname_var = defaults_file ? &spiderrc_defaults_fname : &spiderrc_fname;

  if (*fname_var == NULL) {
    fname = find_spiderrc_filename(cmd_arg, defaults_file,
                                &using_default_spiderrc, &ignore_missing_spiderrc);
    spider_free(*fname_var);
    *fname_var = fname;
  } else {
    fname = *fname_var;
  }
  ...
  if (fname == NULL ||
    ...
    if (using_default_spiderrc == 1 || ignore_missing_spiderrc) {
      if (!defaults_file)
        log_notice(LD_CONFIG, "Configuration file \"%s\" not present, "
            "using reasonable defaults.", fname);
      spider_free(fname); /* sets fname to NULL */
      *fname_var = NULL;
      cf = spider_strdup("");
    } else {
      log_warn(LD_CONFIG,
...
int
options_init_from_spiderrc(int argc, char **argv)
{
  ...
  if (config_line_find(cmdline_only_options, "--list-spiderrc-options")) {
    /* For validating whether we've documented everything. */
    list_spiderrc_options();
    exit(0);
  }
  ...
  if (config_line_find(cmdline_only_options, "--library-versions")) {
    ...
    printf("Libevent\t\t%-15s\t\t%s\n",
                      spider_libevent_get_header_version_str(),
                      spider_libevent_get_version_str());
    printf("OpenSSL \t\t%-15s\t\t%s\n",
                      crypto_openssl_get_header_version_str(),
                      crypto_openssl_get_version_str());
    printf("Zlib    \t\t%-15s\t\t%s\n",
                      spider_zlib_get_header_version_str(),
                      spider_zlib_get_version_str());
    //TODO: Hex versions?
    exit(0);
  }
  ...
  if (command == CMD_HASH_PASSWORD) {
    cf_defaults = spider_strdup("");
    cf = spider_strdup("");
  } else {
    cf_defaults = load_spiderrc_from_disk(cmdline_only_options, 1);

    const config_line_t *f_line = config_line_find(cmdline_only_options,
                                                   "-f");

    const int read_spiderrc_from_stdin =
    (f_line != NULL && strcmp(f_line->value, "-") == 0);

    if (read_spiderrc_from_stdin) {
      cf = load_spiderrc_from_stdin();
    } else {
      cf = load_spiderrc_from_disk(cmdline_only_options, 0);
    }

    if (!cf) {
      if (config_line_find(cmdline_only_options, "--allow-missing-spiderrc")) {
        cf = spider_strdup("");
      } else {
        goto err;
      }
...

  {
    ...
    if (fd_line) {
      if (get_options()->keygen_force_passphrase == FORCE_PASSPHRASE_OFF) {
        ...
        int ok = 1;
        long fd = spider_parse_long(v, 10, 0, INT_MAX, &ok, NULL);
        if (fd < 0 || ok == 0) {
...

  {
    ...
    if (key_line) {
      if (command != CMD_KEYGEN) {
        ...
        get_options_mutable()->master_key_fname = spider_strdup(key_line->value);
      }
    }
  }
                         ...
                         char **msg)
{
  ...
  newoptions = spider_malloc_zero(sizeof(or_options_t));
  newoptions->magic_ = OR_OPTIONS_MAGIC;
  options_init(newoptions);
  newoptions->command = command;
  newoptions->command_arg = command_arg ? spider_strdup(command_arg) : NULL;
  ...
  if (newoptions->TestingSpiderNetwork) {
     ...
     * TestingSpiderNetwork, it could change the defaults before its second pass.
     * Not urgent so long as this seems to work, but at any sign of trouble,
     * let's clean it up.  -NM */

    /* Change defaults. */
    for (int i = 0; testing_spider_network_defaults[i].name; ++i) {
      const config_var_t *new_var = &testing_spider_network_defaults[i];
      config_var_t *old_var =
          config_find_option_mutable(&options_format, new_var->name);
      spider_assert(new_var);
      spider_assert(old_var);
 ...
 err:
  ...
  if (*msg) {
    char *old_msg = *msg;
    spider_asprintf(msg, "Failed to parse/validate config: %s", old_msg);
    spider_free(old_msg);
  }
  return err;
}
 ...
 */
const char *
get_spiderrc_fname(int defaults_fname)
{
  const char *fname = defaults_fname ? spiderrc_defaults_fname : spiderrc_fname;
...
config_register_addressmaps(const or_options_t *options)
{
  ...
  cleanup:
    SMARTLIST_FOREACH(elts, char*, cp, spider_free(cp));
                         ...
                         const char **msg)
{
  ...
  *msg = "whoops, forgot the error message";
  ...
  addressmap_register(from, spider_strdup(to), expires, addrmap_source,
                  ...
                  int validate_only)
{
  ...
  for (opt = options->Logs; opt; opt = opt->next) {
    ...
    severity = spider_malloc_zero(sizeof(log_severity_list_t));
...
validate_transport_socks_arguments(const smartlist_t *args)
{
  ...
  spider_assert(args);
  spider_assert(smartlist_len(args) > 0);
...
bridge_line_free(bridge_line_t *bridge_line)
{
  ...
  if (bridge_line->socks_args) {
    SMARTLIST_FOREACH(bridge_line->socks_args, char*, s, spider_free(s));
    smartlist_free(bridge_line->socks_args);
  }
  spider_free(bridge_line->transport_name);
  spider_free(bridge_line);
}
...
parse_bridge_line(const char *line)
{
  ...
  char *field=NULL;
  bridge_line_t *bridge_line = spider_malloc_zero(sizeof(bridge_line_t));
  ...
  if (spider_addr_port_parse(LOG_INFO, addrport,
  ...
  if (bridge_line->transport_name && smartlist_len(items)) {
    ...
    spider_assert(smartlist_len(bridge_line->socks_args) > 0);
  }
 ...
 err:
  ...
  SMARTLIST_FOREACH(items, char*, s, spider_free(s));
  smartlist_free(items);
  spider_free(addrport);
  spider_free(fingerprint);
                     ...
                     int server)
{
  ...
  if (is_managed) {
    ...
    if (!validate_only && (server || !is_useless_proxy)) {
      ...
      spider_assert(proxy_argc > 0);
      proxy_argv = spider_calloc((proxy_argc + 1), sizeof(char *));
      tmp = proxy_argv;
    ...
    if (spider_addr_port_lookup(addrport, &addr, &port) < 0) {
      log_warn(LD_CONFIG,
 ...
 err:
  ...
  smartlist_free(items);
  if (transport_list) {
    SMARTLIST_FOREACH(transport_list, char*, s, spider_free(s));
...
get_bindaddr_from_transport_listen_line(const char *line,const char *transport)
{
  ...
  addrport = spider_strdup(smartlist_get(items, 1));
  ...
  /* Validate addrport */
  if (spider_addr_port_parse(LOG_WARN, addrport, &addr, &port, -1)<0) {
...
get_options_from_transport_options_line(const char *line,const char *transport)
{
 ...
 err:
  SMARTLIST_FOREACH(options, char*, s, spider_free(s));
                         ...
                         int validate_only)
{
  ...
  spider_addr_port_t ipv6_addrport, *ipv6_addrport_ptr = NULL;
  ...
  while (smartlist_len(items)) {
    ...
    if (!strcasecmp(flag, "hs") ||
      ...
      or_port = (uint16_t) spider_parse_long(portstring, 10, 1, 65535, &ok, NULL);
      if (!ok)
      ...
      weight = spider_parse_double(wstring, 0, (double)UINT64_MAX, &ok, NULL);
      if (!ok) {
      ...
      if (ipv6_addrport_ptr) {
        ...
        if (spider_addr_port_parse(LOG_WARN, flag+strlen("ipv6="),
                                &ipv6_addrport.addr, &ipv6_addrport.port,
                                -1) < 0
            || spider_addr_family(&ipv6_addrport.addr) != AF_INET6) {
 ...
 * dirserver described in the line as a fallback direcspidery. Return 0 on
                        ...
                        int validate_only)
{
  ...
  SMARTLIST_FOREACH_BEGIN(items, const char *, cp) {
    ...
    if (!strcmpstart(cp, "orport=")) {
      orport = (int)spider_parse_long(cp+strlen("orport="), 10,
                                   1, 65535, &ok, NULL);
      ...
      if (ipv6_addrport_ptr) {
        ...
        if (spider_addr_port_parse(LOG_WARN, cp+strlen("ipv6="),
                                &ipv6_addrport.addr, &ipv6_addrport.port,
                                -1) < 0
            || spider_addr_family(&ipv6_addrport.addr) != AF_INET6) {
      ...
      weight = spider_parse_double(wstring, 0, (double)UINT64_MAX, &num_ok, NULL);
      if (!num_ok) {
  ...
  if (spider_addr_port_split(LOG_INFO, smartlist_get(positional, 0),
                          &address, &dirport) < 0 ||
      spider_addr_parse(&addr, address)<0) {
  ...
  SMARTLIST_FOREACH(items, char *, cp, spider_free(cp));
  smartlist_free(items);
...
{
  spider_assert(namelen <= SIZE_T_CEILING - sizeof(port_cfg_t) - 1);
  port_cfg_t *cfg = spider_malloc_zero(sizeof(port_cfg_t) + namelen + 1);
...
/** Free all sspiderage held in <b>port</b> */
STATIC void
                           ...
                           int listener_type)
{
  SMARTLIST_FOREACH_BEGIN(ports, const port_cfg_t *, port) {
    ...
    } else if (!spider_addr_is_internal(&port->addr, 1)) {
    ...
    } else if (!spider_addr_is_loopback(&port->addr)) {
...
warn_nonlocal_ext_orports(const smartlist_t *ports, const char *portname)
{
  SMARTLIST_FOREACH_BEGIN(ports, const port_cfg_t *, port) {
    ...
    if (!spider_addr_is_internal(&port->addr, 1)) {
      log_warn(LD_CONFIG, "You specified a public address '%s' for %sPort. "
...
warn_nonlocal_controller_ports(smartlist_t *ports, unsigned forbid_nonlocal)
{
  ...
  SMARTLIST_FOREACH_BEGIN(ports, port_cfg_t *, port) {
    ...
    if (port->is_unix_addr)
      continue;
    if (!spider_addr_is_loopback(&port->addr)) {
      if (forbid_nonlocal) {
        if (!warned)
          log_warn(LD_CONFIG,
                 ...
                 "can reconfigure your Spider, without even having to guess a "
                 "password.  That's so bad that I'm closing your ControlPort "
                 "for you.  If you need to control your Spider remotely, try "
        ...
        log_warn(LD_CONFIG, "You have a ControlPort set to accept "
                 ...
                 "Spider.  That's pretty bad, since the controller "
...
{
  spider_assert(line);
  spider_assert(addrport_out);
  spider_assert(is_unix_out);
  spider_assert(rest_out);
  ...
  if (!strcmpstart(line, unix_q_socket_prefix)) {
    ...
    *rest_out = eat_whitespace(*rest_out);
    ...
    spider_assert(end && end >= line);
    *addrport_out = spider_strndup(line, end - line);
                  ...
                  const unsigned flags)
{
  ...
  if (listenaddrs) {
    ...
    if (ports && ports->next) {
      ...
      if (!strcmp(ports->value, "auto")) {
        ...
        int ok;
        mainport = (int)spider_parse_long(ports->value, 10, 0, 65535, &ok, NULL);
        if (!ok) {
    ...
    if (use_server_options && out) {
      ...
      cfg->port = mainport;
      spider_addr_make_unspec(&cfg->addr); /* Server ports default to 0.0.0.0 */
    ...
    for (; listenaddrs; listenaddrs = listenaddrs->next) {
      spider_addr_t addr;
      uint16_t port = 0;
      if (spider_addr_port_lookup(listenaddrs->value, &addr, &port) < 0) {
      ...
      if (out) {
        ...
        spider_addr_copy(&cfg->addr, &addr);
  ...
  if (! ports) {
    if (defaultport && defaultaddr && out) {
      port_cfg_t *cfg = port_cfg_new(is_unix_socket ? strlen(defaultaddr) : 0);
       cfg->type = listener_type;
       if (is_unix_socket) {
         spider_addr_make_unspec(&cfg->addr);
         memcpy(cfg->unix_addr, defaultaddr, strlen(defaultaddr) + 1);
         cfg->is_unix_addr = 1;
       } else {
         cfg->port = defaultport;
         spider_addr_parse(&cfg->addr, defaultaddr);
       }
       cfg->entry_cfg.session_group = SESSION_GROUP_UNSET;
       cfg->entry_cfg.isolation_flags = ISO_DEFAULT;
       smartlist_add(out, cfg);
  ...
  for (; ports; ports = ports->next) {
    ...
    if (unix_socket_path) {
      ...
      unix_socket_path = spider_strdup(addrport);
      ...
      port = CFG_AUTO_PORT;
      int af = spider_addr_parse(&addr, defaultaddr);
      spider_assert(af >= 0);
    } else if (!strcasecmpend(addrport, ":auto")) {
      char *addrtmp = spider_strndup(addrport, strlen(addrport)-5);
      port = CFG_AUTO_PORT;
      if (spider_addr_port_lookup(addrtmp, &addr, &ptmp)<0 || ptmp) {
      ...
      port = (int) spider_parse_long(addrport, 10, 0, 65535, &ok, NULL);
      if (ok) {
        int af = spider_addr_parse(&addr, defaultaddr);
        spider_assert(af >= 0);
      } else if (spider_addr_port_lookup(addrport, &addr, &ptmp) == 0) {
        if (ptmp == 0) {
    ...
    if (use_server_options) {
      ...
      if (bind_ipv4_only && spider_addr_family(&addr) == AF_INET6) {
        log_warn(LD_CONFIG, "Could not interpret %sPort address as IPv6",
                 portname);
        goto err;
      }
      if (bind_ipv6_only && spider_addr_family(&addr) == AF_INET) {
      ...
      SMARTLIST_FOREACH_BEGIN(elts, char *, elt) {
        ...
        if (!strcasecmpstart(elt, "SessionGroup=")) {
          int group = (int)spider_parse_long(elt+strlen("SessionGroup="),
            ...
            int *world_writable_control_socket)
{
  ...
  *n_ports_out = 0;
  ...
  if (parse_port_config(ports,
    ...
    *msg = spider_strdup("Invalid SocksPort/SocksListenAddress configuration");
    goto err;
  }
  if (parse_port_config(ports,
    ...
    *msg = spider_strdup("Invalid DNSPort/DNSListenAddress configuration");
    goto err;
  }
  if (parse_port_config(ports,
    ...
    *msg = spider_strdup("Invalid TransPort/TransListenAddress configuration");
    goto err;
  }
  if (parse_port_config(ports,
                        ...
                        "127.0.0.1", 0,
                        CL_PORT_WARN_NONLOCAL) < 0) {
    *msg = spider_strdup("Invalid NatdPort/NatdListenAddress configuration");
    goto err;
  }
  {
    ...
    if (parse_port_config(ports,
      ...
      *msg = spider_strdup("Invalid ControlPort/ControlListenAddress "
                        "configuration");
      goto err;
    }
    ...
    if (parse_port_config(ports, options->ControlSocket, NULL,
      ...
      *msg = spider_strdup("Invalid ControlSocket configuration");
      goto err;
    }
  }
  if (! options->ClientOnly) {
    if (parse_port_config(ports,
      ...
      *msg = spider_strdup("Invalid ORPort/ORListenAddress configuration");
      goto err;
    }
    if (parse_port_config(ports,
      ...
      *msg = spider_strdup("Invalid ExtORPort configuration");
      goto err;
    }
    if (parse_port_config(ports,
                          ...
                          "0.0.0.0", 0,
                          CL_PORT_SERVER_OPTIONS) < 0) {
      *msg = spider_strdup("Invalid DirPort/DirListenAddress configuration");
      goto err;
    }
  }
  ...
  if (check_server_ports(ports, options, &n_low_ports) < 0) {
    *msg = spider_strdup("Misconfigured server ports");
    goto err;
  }
  if (have_low_ports < 0)
...
{
  return spider_addr_family(&port->addr) == AF_INET ||
         (spider_addr_family(&port->addr) == AF_UNSPEC
...
{
  return spider_addr_family(&port->addr) == AF_INET6 ||
         (spider_addr_family(&port->addr) == AF_UNSPEC
                   ...
                   int *n_low_ports_out)
{
  ...
  if (n_orport_listeners && !n_orport_advertised) {
    log_warn(LD_CONFIG, "We are listening on an ORPort, but not advertising "
             ...
             "descripspider, and make us impossible to use.",
  ...
  if (n_low_port && options->AccountingMax &&
    ...
    log_warn(LD_CONFIG,
          ...
          "This combination can make Spider stop "
...
get_first_listener_addrport_string(int listener_type)
{
  ...
  SMARTLIST_FOREACH_BEGIN(configured_ports, const port_cfg_t *, cfg) {
    ...
    if (cfg->type == listener_type &&
        spider_addr_family(&cfg->addr) != AF_UNSPEC) {
      ...
      /* If a listener is listening on INADDR_ANY, assume that it's
         ...
         there: */
      if (spider_addr_is_null(&cfg->addr))
        address = spider_addr_is_v4(&cfg->addr) ? ipv4_localhost : ipv6_localhost;
      else
      ...
      spider_asprintf(&string, "%s:%u", address, port);
...
int
port_exists_by_type_addr_port(int listener_type, const spider_addr_t *addr,
  ...
  SMARTLIST_FOREACH_BEGIN(configured_ports, const port_cfg_t *, cfg) {
    if (cfg->type == listener_type) {
      if (cfg->port == port || (check_wildcard && port == -1)) {
        /* Exact match */
        if (spider_addr_eq(&cfg->addr, addr)) {
          return 1;
        }
        ...
        const int cfg_any_v4 = spider_addr_is_null(&cfg->addr) && cfg_v4;
        const int addr_v4 = spider_addr_family(addr) == AF_INET ||
                            spider_addr_family(addr) == AF_UNSPEC;
        const int addr_any_v4 = spider_addr_is_null(&cfg->addr) && addr_v4;
        if ((cfg_any_v4 && addr_v4) || (cfg_v4 && addr_any_v4)) {
          return 1;
        }
        /* Wildcard matches IPv6 */
        const int cfg_v6 = port_binds_ipv6(cfg);
        const int cfg_any_v6 = spider_addr_is_null(&cfg->addr) && cfg_v6;
        const int addr_v6 = spider_addr_family(addr) == AF_INET6 ||
                            spider_addr_family(addr) == AF_UNSPEC;
        const int addr_any_v6 = spider_addr_is_null(&cfg->addr) && addr_v6;
...
{
  spider_addr_t ipv4;
  spider_addr_from_ipv4h(&ipv4, addr_ipv4h);
  return port_exists_by_type_addr_port(listener_type, &ipv4, port,
                                       check_wildcard);
}

/** Adjust the value of options->DataDirecspidery, or fill it in if it's
 * absent. Return 0 on success, -1 on failure. */
static int
normalize_data_direcspidery(or_options_t *options)
{
#ifdef _WIN32
  char *p;
  if (options->DataDirecspidery)
    return 0; /* all set */
  p = spider_malloc(MAX_PATH);
  strlcpy(p,get_windows_conf_root(),MAX_PATH);
  options->DataDirecspidery = p;
  return 0;
#else
  const char *d = options->DataDirecspidery;
  if (!d)
    d = "~/.spider";
 ...
 if (strncmp(d,"~/",2) == 0) {
   ...
   if (!options->DataDirecspidery && !strcmp(fn,"/.spider")) {
     /* If our homedir is /, we probably don't want to use it. */
     /* Default to LOCALSTATEDIR/spider which is probably closer to what we
      * want. */
     log_warn(LD_CONFIG,
              "Default DataDirecspidery is \"~/.spider\".  This expands to "
              "\"%s\", which is probably not what you want.  Using "
              "\"%s"PATH_SEPARATOR"spider\" instead", fn, LOCALSTATEDIR);
     spider_free(fn);
     fn = spider_strdup(LOCALSTATEDIR PATH_SEPARATOR "spider");
   }
   spider_free(options->DataDirecspidery);
   options->DataDirecspidery = fn;
 }
 return 0;
#endif
}

/** Check and normalize the value of options->DataDirecspidery; return 0 if it
 * is sane, -1 otherwise. */
static int
validate_data_direcspidery(or_options_t *options)
{
  if (normalize_data_direcspidery(options) < 0)
    return -1;
  spider_assert(options->DataDirecspidery);
  if (strlen(options->DataDirecspidery) > (512-128)) {
    log_warn(LD_CONFIG, "DataDirecspidery is too long.");
    return -1;
  }
  return 0;
}
 ...
 * recognize that the spiderrc file doesn't need to be backed up. */
#define GENERATED_FILE_PREFIX "# This file was generated by Spider; " \
...
#define GENERATED_FILE_COMMENT "# The old spiderrc file was renamed " \
  "to spiderrc.orig.1 or similar, and Spider will ignore it"
...
write_configuration_file(const char *fname, const or_options_t *options)
{
  ...
  if (rename_old) {
    ...
    char *fn_tmp = NULL;
    while (1) {
      spider_asprintf(&fn_tmp, "%s.orig.%d", fname, i);
    ...
    if (spider_rename(fname, fn_tmp) < 0) {//XXXX sandbox doesn't allow
      log_warn(LD_FS,
...
options_save_current(void)
{
   ...
   *
   * If we try falling back to datadirecspidery or something, we have a better
   * chance of saving the configuration, but a better chance of doing
   * something the user never expected. */
  return write_configuration_file(get_spiderrc_fname(0), get_options());
}
...
init_libevent(const or_options_t *options)
{
  ...
  spider_libevent_initialize(&cfg);

  suppress_libevent_log_msg(NULL);
}

/** Return a newly allocated string holding a filename relative to the data
 * direcspidery.  If <b>sub1</b> is present, it is the first path component after
 * the data direcspidery.  If <b>sub2</b> is also present, it is the second path
 * component after the data direcspidery.  If <b>suffix</b> is present, it
                                   ...
                                   const char *suffix))
{
  ...
  spider_assert(options);
  spider_assert(options->DataDirecspidery);
  spider_assert(sub1 || !sub2); /* If sub2 is present, sub1 must be present. */
  len = strlen(options->DataDirecspidery);
  if (sub1) {
    len += strlen(sub1)+1;
    if (sub2)
      len += strlen(sub2)+1;
  ...
  if (suffix)
    len += strlen(suffix);
  len++;
  fname = spider_malloc(len);
  if (sub1) {
    if (sub2) {
      spider_snprintf(fname, len, "%s"PATH_SEPARATOR"%s"PATH_SEPARATOR"%s",
                   options->DataDirecspidery, sub1, sub2);
    } else {
      spider_snprintf(fname, len, "%s"PATH_SEPARATOR"%s",
                   options->DataDirecspidery, sub1);
    }
  } else {
    strlcpy(fname, options->DataDirecspidery, len);
  }
  if (suffix)
...
/** Check wether the data direcspidery has a private subdirecspidery
...
check_or_create_data_subdir(const char *subdir)
{
  ...
  if (check_private_dir(statsdir, CPD_CREATE, get_options()->User) < 0) {
    log_warn(LD_HIST, "Unable to create %s/ direcspidery!", subdir);
    return_val = -1;
  }
  spider_free(statsdir);
  return return_val;
}

/** Create a file named <b>fname</b> with contents <b>str</b> in the
 * subdirecspidery <b>subdir</b> of the data direcspidery. <b>descr</b>
 ...
 *  spider-fw-helper. The smartlist contains the ports in a string format
 *  that is understandable by spider-fw-helper. */
smartlist_t *
get_list_of_ports_to_forward(void)
{
  ...
  /** XXX TODO spider-fw-helper does not support forwarding ports to
      other hosts than the local one. If the user is binding to a
      different IP address, spider-fw-helper won't work.  */
                      ...
                      const char **errmsg)
{
  ...
  if (!strcmp(question, "config/names")) {
    ...
    *answer = smartlist_join_strings(sl, "", 0, NULL);
    SMARTLIST_FOREACH(sl, char *, c, spider_free(c));
    smartlist_free(sl);
    ...
    if (dirauth_lines_seen == 0) {
      /*
       * We didn't see any direcspidery authorities with default values,
 ...
 */
static int
verify_and_sspidere_outbound_address(sa_family_t family, spider_addr_t *addr,
       outbound_addr_t type, or_options_t *options, int validate_only)
{
  ...
  spider_addr_t *dest=&options->OutboundBindAddresses[type][fam_index];
  if (!spider_addr_is_null(dest)) {
    return -1;
  }
  if (!validate_only) {
    spider_addr_copy(dest, addr);
  }
  return 0;
}

/* Parse a list of address lines for a specific destination type.
 * Will sspidere them into the options if not validate_only. If a
 * problem occurs, a suitable error message is sspidere in msg.
           ...
           or_options_t *options, int validate_only, char **msg)
{
  ...
  sa_family_t family;
  while (lines) {
    family = spider_addr_parse(&addr, lines->value);
    if (verify_and_sspidere_outbound_address(family, &addr, type,
                                 options, validate_only)) {
      if (msg)
        spider_asprintf(msg, "Multiple%s%s outbound bind addresses "
                     "configured: %s",
                        ...
                        const char *default_fname)
{
  ...
  if (!strcmp(fname, "<default>")) {
    ...
    spider_asprintf(&free_fname, "%s\\%s", conf_root, default_fname);
    fname = free_fname;
  }
  geoip_load_file(family, fname);
  spider_free(free_fname);
#else
  (void)default_fname;
  geoip_load_file(family, fname);
#endif
                           ...
                           uint8_t **cookie_out, int *cookie_is_set_out)
{
  ...
  char *cookie_file_str = spider_malloc(cookie_file_str_len);
  int retval = -1;
  ...
  if (*cookie_out)
    spider_free(*cookie_out);

  /* Generate the cookie */
  *cookie_out = spider_malloc(cookie_len);
