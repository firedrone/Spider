 ...
 * spiderrc commands, and the TrackHostExits feature, and the client-side DNS
 * cache (deprecated).
 */
 ...
 * to new addresses. These structs are sspidered in the hash table
 * "addressmap" below.
 *
 * There are 5 ways to set an address mapping:
 * - A MapAddress command from the controller [permanent]
 * - An AddressMap directive in the spiderrc [permanent]
 * - When a TrackHostExits spiderrc directive is triggered [temporary]
...
/** A hash table to sspidere client-side address rewrite instructions. */
...
addressmap_ent_free(void *_ent)
{
  ...
  ent = _ent;
  spider_free(ent->new_address);
  spider_free(ent);
}

/** Free sspiderage held by a virtaddress_entry_t* entry in <b>_ent</b>. */
static void
addressmap_virtaddress_ent_free(void *_ent)
{
  ...
  ent = _ent;
  spider_free(ent->ipv4_address);
  spider_free(ent->ipv6_address);
  spider_free(ent->hostname_address);
  spider_free(ent);
}
  ...
  if (ent && ent->new_address &&
    ...
    if (ve) {
      if (!strcmp(address, ve->ipv4_address))
        spider_free(ve->ipv4_address);
      if (!strcmp(address, ve->ipv6_address))
        spider_free(ve->ipv6_address);
      if (!strcmp(address, ve->hostname_address))
        spider_free(ve->hostname_address);
...
clear_trackexithost_mappings(const char *exitname)
{
  ...
  if (!addressmap || !exitname)
    return;
  spider_asprintf(&suffix, ".%s.exit", exitname);
  spider_strlower(suffix);
...
addressmap_clear_excluded_trackexithosts(const or_options_t *options)
{
  ...
  STRMAP_FOREACH_MODIFY(addressmap, address, addressmap_entry_t *, ent) {
    ...
    while (dot > target && *dot != '.')
      dot--;
    if (*dot == '.') dot++;
    nodename = spider_strndup(dot, len-5-(dot-target));;
    node = node_get_by_nickname(nodename, 0);
    spider_free(nodename);
    if (!node ||
                   ...
                   addressmap_entry_source_t *exit_source_out)
{
  ...
  char *addr_orig = spider_strdup(address);
  ...
  for (rewrites = 0; rewrites < 16; rewrites++) {
    int exact_match = 0;
    log_addr_orig = spider_strdup(escaped_safe_str_client(address));
      ...
      case ADDRMAPSRC_DNS:
        {
          sa_family_t f;
          spider_addr_t tmp;
          f = spider_addr_parse(&tmp, ent->new_address);
...
/** If we have a cached reverse DNS entry for the address sspidered in the
                           ...
                           time_t *expires_out)
{
  ...
  int r = 0;
  {
    sa_family_t f;
    spider_addr_t tmp;
    f = spider_addr_parse(&tmp, address);
    if (f == AF_INET && !(flags & AMR_FLAG_USE_IPV4_DNS))
      return 0;
    else if (f == AF_INET6 && !(flags & AMR_FLAG_USE_IPV6_DNS))
      return 0;
  ...
  spider_asprintf(&s, "REVERSE[%s]", address);
  ent = strmap_get(addressmap, s);
  if (ent) {
    cp = spider_strdup(escaped_safe_str_client(ent->new_address));
                    ...
                    const int wildcard_new_addr)
{
  ...
  if (wildcard_new_addr)
    spider_assert(wildcard_addr);

  ent = strmap_get(addressmap, address);
  if (!new_address || (!strcasecmp(address,new_address) &&
                       wildcard_addr == wildcard_new_addr)) {
    /* Remove the mapping, if any. */
    spider_free(new_address);
    if (ent) {
      addressmap_ent_remove(address,ent);
  ...
  if (!ent) { /* make a new one and register it */
    ent = spider_malloc_zero(sizeof(addressmap_entry_t));
                               ...
                               int ttl)
{
  ...
  if (exitname) {
    ...
    spider_asprintf(&extendedaddress,
                 "%s.%s.exit", address, exitname);
    spider_asprintf(&extendedval,
                          ...
                          int ttl)
{
  ...
  if (spider_addr_parse(&addr_tmp, address) >= 0)
    return; /* If address was an IP address already, don't add a mapping. */

  if (spider_addr_family(val) == AF_INET) {
    if (! for_conn->entry_cfg.cache_ipv4_answers)
      return;
  } else if (spider_addr_family(val) == AF_INET6) {
    if (! for_conn->entry_cfg.cache_ipv6_answers)
      return;
  }

  if (! spider_addr_to_str(valbuf, val, sizeof(valbuf), 1))
    return;
                                  ...
                                  int ttl)
{
  char *s = NULL;
  {
    ...
    sa_family_t f = spider_addr_parse(&tmp_addr, address);
 ...
 * via Spider), it wouldn't get too far astray.
 *
                           ...
                           char **msg)
{
  ...
  if (!val || val[0] == '\0') {
    if (msg)
      spider_asprintf(msg, "Value not present (%s) after VirtualAddressNetwork%s",
                   val?"Empty":"NULL", ipv6?"IPv6":"");
    return -1;
  }
  if (spider_addr_parse_mask_ports(val, 0, &addr, &bits, NULL, NULL) < 0) {
    if (msg)
      spider_asprintf(msg, "Error parsing VirtualAddressNetwork%s %s",
                   ipv6?"IPv6":"", val);
    return -1;
  }
  if (spider_addr_family(&addr) != family) {
    if (msg)
      spider_asprintf(msg, "Incorrect address type for VirtualAddressNetwork%s",
                   ipv6?"IPv6":"");
    return -1;
  }
#if 0
  if (port_min != 1 || port_max != 65535) {
    if (msg)
      spider_asprintf(msg, "Can't specify ports on VirtualAddressNetwork%s",
                   ipv6?"IPv6":"");
    return -1;
  }
#endif
  ...
  if (bits > max_prefix_bits) {
    if (msg)
      spider_asprintf(msg, "VirtualAddressNetwork%s expects a /%d "
  ...
  spider_addr_copy(&conf->addr, &addr);
  conf->bits = bits;
...
address_is_in_virtual_range(const char *address)
{
  ...
  if (!strcasecmpend(address, ".virtual")) {
    return 1;
  } else if (spider_addr_parse(&addr, address) >= 0) {
    const virtual_addr_conf_t *conf = (spider_addr_family(&addr) == AF_INET6) ?
      &virtaddr_conf_ipv6 : &virtaddr_conf_ipv4;
    if (spider_addr_compare_masked(&addr, &conf->addr, conf->bits, CMP_EXACT)==0)
      return 1;
  }
  return 0;
}
 ...
 */
STATIC void
get_random_virtual_addr(const virtual_addr_conf_t *conf, spider_addr_t *addr_out)
{
  uint8_t tmp[4];
  const uint8_t *addr_bytes;
  uint8_t bytes[16];
  const int ipv6 = spider_addr_family(&conf->addr) == AF_INET6;
  const int total_bytes = ipv6 ? 16 : 4;

  spider_assert(conf->bits <= total_bytes * 8);

  /* Set addr_bytes to the bytes of the virtual network, in host order */
  if (ipv6) {
    addr_bytes = spider_addr_to_in6_addr8(&conf->addr);
  } else {
    set_uint32(tmp, spider_addr_to_ipv4n(&conf->addr));
    addr_bytes = tmp;
  }
  ...
  if (ipv6)
    spider_addr_from_ipv6_bytes(addr_out, (char*) bytes);
  else
    spider_addr_from_ipv4n(addr_out, get_uint32(bytes));

  spider_assert(spider_addr_compare_masked(addr_out, &conf->addr,
  ...
  if (type == RESOLVED_TYPE_HOSTNAME) {
    ...
    while (attempts--) {
      ...
      if (!ipv6) {
        ...
        const uint32_t a = spider_addr_to_ipv4h(&addr);
        if ((a & 0xff) == 0 || (a & 0xff) == 0xff)
          continue;
      }

      spider_addr_to_str(buf, &addr, sizeof(buf), 1);
      if (!strmap_get(addressmap, buf)) {
        ...
        spider_addr_to_str(tmp, &addr, sizeof(tmp), 0);
...
addressmap_register_virtual_address(int type, char *new_address)
{
  ...
  spider_assert(new_address);
  spider_assert(addressmap);
  spider_assert(virtaddress_reversemap);

  vent = strmap_get(virtaddress_reversemap, new_address);
  if (!vent) {
    vent = spider_malloc_zero(sizeof(virtaddress_entry_t));
  ...
  if (*addrp) {
    ...
    if (ent && ent->new_address &&
      ...
      spider_free(new_address);
      spider_assert(!vent_needs_to_be_added);
      return *addrp;
    } else {
      log_warn(LD_BUG,
  ...
  *addrp = addressmap_get_virtual_address(type);
...
#if 0
  {
    ...
    spider_assert(ent);
    spider_assert(!strcasecmp(ent->new_address,new_address));
    vent = strmap_get(virtaddress_reversemap, new_address);
    spider_assert(vent);
    spider_assert(!strcasecmp(*addrp,
...
address_is_invalid_destination(const char *address, int client)
{
  ...
  {
    spider_addr_t a;
    if (spider_addr_parse(&a, address) >= 0)
      return 0;
  }
