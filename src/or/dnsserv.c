 ...
 * Spider client handle DNS requests, this module handles it.  It functions as a
 ...
 * that's how Spider represents client-side streams), which are kept associated
...
evdns_server_callback(struct evdns_server_request *req, void *data_)
{
  ...
  if (spider_addr_from_sockaddr(&spider_addr, sa, &port)<0) {
    log_warn(LD_APP, "Requesting address wasn't recognized.");
    evdns_server_request_respond(req, DNS_ERR_SERVERFAILED);
    return;
  }

  if (!socks_policy_permits_address(&spider_addr)) {
  ...
  spider_addr_copy(&TO_CONN(conn)->addr, &spider_addr);
  TO_CONN(conn)->port = port;
  TO_CONN(conn)->address = spider_addr_to_str_dup(&spider_addr);
  ...
  if (q->type == EVDNS_TYPE_A || q->type == EVDNS_TYPE_AAAA ||
    ...
    spider_assert(q->type == EVDNS_TYPE_PTR);
  ...
  * automapped), and then attached to a circuit. */
  ...
  q_name = spider_strdup(q->name); /* q could be freed in rewrite_and_attach */
                       ...
                       control_connection_t *control_conn)
{
  ...
  spider_addr_copy(&TO_CONN(conn)->addr, &control_conn->base_.addr);
#ifdef AF_UNIX
  /*
   * The control connection can be AF_UNIX and if so spider_addr_to_str_dup will
   * unhelpfully say "<unknown address type>"; say "(Spider_internal)"
   * instead.
   */
  if (control_conn->base_.socket_family == AF_UNIX) {
    TO_CONN(conn)->port = 0;
    TO_CONN(conn)->address = spider_strdup("(Spider_internal)");
  } else {
    TO_CONN(conn)->port = control_conn->base_.port;
    TO_CONN(conn)->address = spider_addr_to_str_dup(&control_conn->base_.addr);
  }
#else
  TO_CONN(conn)->port = control_conn->base_.port;
  TO_CONN(conn)->address = spider_addr_to_str_dup(&control_conn->base_.addr);
  ...
  entry_conn->original_dest_address = spider_strdup(name);
  ...
  * automapped), and then attached to a circuit. */
  ...
  q_name = spider_strdup(name); /* q could be freed in rewrite_and_attach */
  connection_ap_rewrite_and_attach_if_allowed(entry_conn, NULL, NULL);
  /* Now, the connection is marked if it was bad. */
                 ...
                 int ttl)
{
  ...
  if (answer_type == RESOLVED_TYPE_IPV6) {
    ...
    char *ans = spider_strndup(answer, answer_len);
...
dnsserv_configure_listener(connection_t *conn)
{
  ...
  spider_assert(conn->type == CONN_TYPE_AP_DNS_LISTENER);

  listener_conn = TO_LISTENER_CONN(conn);
  listener_conn->dns_server_port =
    spider_evdns_add_server_port(conn->s, 0, evdns_server_callback,
                              listener_conn);
}
