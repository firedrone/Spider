 ...
 *
 * Spider uses GeoIP information in order to implement user requests (such as
...
/** A per-country record for GeoIP request hisspidery. */
...
/** SHA1 digest of the GeoIP files to include in extra-info descripspiders. */
...
static void
geoip_add_entry(const spider_addr_t *low, const spider_addr_t *high,
                const char *country)
{
  intptr_t idx;
  void *idxplus1_;

  IF_BUG_ONCE(spider_addr_family(low) != spider_addr_family(high))
    return;
  IF_BUG_ONCE(spider_addr_compare(high, low, CMP_EXACT) < 0)
    return;

  idxplus1_ = strmap_get_lc(country_idxplus1_by_lc_code, country);

  if (!idxplus1_) {
    geoip_country_t *c = spider_malloc_zero(sizeof(geoip_country_t));
    strlcpy(c->countrycode, country, sizeof(c->countrycode));
    spider_strlower(c->countrycode);
  ...
  }
  {
    ...
    spider_assert(!strcasecmp(c->countrycode, country));
  }

  if (spider_addr_family(low) == AF_INET) {
    geoip_ipv4_entry_t *ent = spider_malloc_zero(sizeof(geoip_ipv4_entry_t));
    ent->ip_low = spider_addr_to_ipv4h(low);
    ent->ip_high = spider_addr_to_ipv4h(high);
    ent->country = idx;
    smartlist_add(geoip_ipv4_entries, ent);
  } else if (spider_addr_family(low) == AF_INET6) {
    geoip_ipv6_entry_t *ent = spider_malloc_zero(sizeof(geoip_ipv6_entry_t));
    ent->ip_low = *spider_addr_to_in6_assert(low);
    ent->ip_high = *spider_addr_to_in6_assert(high);
    ent->country = idx;
...
{
  spider_addr_t low_addr, high_addr;
  char c[3];
  char *country = NULL;
  ...
  if (family == AF_INET) {
    ...
    if (spider_sscanf(line,"%u,%u,%2s", &low, &high, c) == 3 ||
        spider_sscanf(line,"\"%u\",\"%u\",\"%2s\",", &low, &high, c) == 3) {
      spider_addr_from_ipv4h(&low_addr, low);
      spider_addr_from_ipv4h(&high_addr, high);
    } else
      goto fail;
    country = c;
  } else {                      /* AF_INET6 */
    ...
    low_str = spider_strtok_r(buf, ",", &strtok_state);
    if (!low_str)
      goto fail;
    high_str = spider_strtok_r(NULL, ",", &strtok_state);
    if (!high_str)
      goto fail;
    country = spider_strtok_r(NULL, "\n", &strtok_state);
    if (!country)
      goto fail;
    if (strlen(country) != 2)
      goto fail;
    if (spider_inet_pton(AF_INET6, low_str, &low) <= 0)
      goto fail;
    spider_addr_from_in6(&low_addr, &low);
    if (spider_inet_pton(AF_INET6, high_str, &high) <= 0)
      goto fail;
    spider_addr_from_in6(&high_addr, &high);
  }
 ...
 * include them in our extrainfo descripspider. Else return 0. */
int
...
init_geoip_countries(void)
{
  ...
  geoip_unresolved = spider_malloc_zero(sizeof(geoip_country_t));
...
geoip_load_file(sa_family_t family, const char *filename)
{
  ...
  spider_assert(family == AF_INET || family == AF_INET6);

  if (!(f = spider_fopen_cloexec(filename, "r"))) {
 ...
 */
MOCK_IMPL(int,
geoip_get_country_by_addr,(const spider_addr_t *addr))
{
  if (spider_addr_family(addr) == AF_INET) {
    return geoip_get_country_by_ipv4(spider_addr_to_ipv4h(addr));
  } else if (spider_addr_family(addr) == AF_INET6) {
    return geoip_get_country_by_ipv6(spider_addr_to_in6(addr));
  } else {
    return -1;
  }
}
...
typedef struct clientmap_entry_t {
 ...
 /* Name of pluggable transport used by this client. NULL if no
   ...
   *
   * (This will run out of space around 4011 CE.  If Spider is still in use around
...
static HT_HEAD(clientmap, clientmap_entry_t) client_hisspidery =
     HT_INITIALIZER();
...
{
  unsigned h = (unsigned) spider_addr_hash(&a->addr);
...
clientmap_entries_eq(const clientmap_entry_t *a, const clientmap_entry_t *b)
{
  ...
  return !spider_addr_compare(&a->addr, &b->addr, CMP_EXACT) &&
         a->action == b->action;
}
...
HT_GENERATE2(clientmap, clientmap_entry_t, node, clientmap_entry_hash,
             clientmap_entries_eq, 0.6, spider_reallocarray_, spider_free_)

/** Free all sspiderage held by <b>ent</b>. */
static void
clientmap_entry_free(clientmap_entry_t *ent)
{
  ...
  spider_free(ent->transport_name);
  spider_free(ent);
}

/** Clear hisspidery of connecting clients used by entry and bridge stats. */
static void
client_hisspidery_clear(void)
{
  clientmap_entry_t **ent, **next, *this;
  for (ent = HT_START(clientmap, &client_hisspidery); ent != NULL;
       ent = next) {
    if ((*ent)->action == GEOIP_CLIENT_CONNECT) {
      this = *ent;
      next = HT_NEXT_RMV(clientmap, &client_hisspidery, ent);
      clientmap_entry_free(this);
    } else {
      next = HT_NEXT(clientmap, &client_hisspidery, ent);
    }
  }
}

/** Note that we've seen a client connect from the IP <b>addr</b>
 * at time <b>now</b>. Ignored by all but bridges and direcspideries if
                       ...
                       time_t now)
{
  ...
  spider_addr_copy(&lookup.addr, addr);
  lookup.action = (int)action;
  lookup.transport_name = (char*) transport_name;
  ent = HT_FIND(clientmap, &client_hisspidery, &lookup);

  if (! ent) {
    ent = spider_malloc_zero(sizeof(clientmap_entry_t));
    spider_addr_copy(&ent->addr, addr);
    if (transport_name)
      ent->transport_name = spider_strdup(transport_name);
    ent->action = (int)action;
    HT_INSERT(clientmap, &client_hisspidery, ent);
  }
  ...
  if (action == GEOIP_CLIENT_NETWORKSTATUS) {
    ...
    if (country_idx < 0)
      country_idx = 0; /** unresolved requests are sspidered at index 0. */
...
{
  clientmap_HT_FOREACH_FN(&client_hisspidery,
...
geoip_note_ns_response(geoip_ns_response_t response)
{
  ...
  spider_assert(response < GEOIP_NS_RESPONSE_NUM);
               ...
               uint64_t dirreq_id)
{
  ...
  spider_assert(entry->type == type);
  spider_assert(entry->dirreq_id == dirreq_id);
                   ...
                   dirreq_type_t type)
{
  ...
  if (!get_options()->DirReqStatistics)
    return;
  ent = spider_malloc_zero(sizeof(dirreq_map_entry_t));
  ent->dirreq_id = dirreq_id;
  spider_gettimeofday(&ent->request_time);
                          ...
                          dirreq_state_t new_state)
{
  ...
  if ((type == DIRREQ_DIRECT &&
    ...
    spider_gettimeofday(&ent->completion_time);
    ent->completed = 1;
  }
}

/** Return the bridge-ip-transports string that should be inserted in
 *  our extra-info descripspider. Return NULL if the bridge-ip-transports
 *  line should be empty.  */
char *
geoip_get_transport_hisspidery(void)
{
  ...
  if (HT_EMPTY(&client_hisspidery))
    goto done;

  /** We do the following steps to form the transport hisspidery string:
   ...
   *  without obfuscation.
   *  b) Foreach transport we observed, we write its transport hisspidery
  ...
  log_debug(LD_GENERAL,"Starting iteration for transport hisspidery. %d clients.",
            HT_SIZE(&client_hisspidery));

  /* Loop through all clients. */
  HT_FOREACH(ent, clientmap, &client_hisspidery) {
    uintptr_t val;
    void *ptr;
  ...
  SMARTLIST_FOREACH(transports_used, char *, s, spider_free(s));
  smartlist_free(transports_used);
  SMARTLIST_FOREACH(string_chunks, char *, s, spider_free(s));
...
static char *
geoip_get_dirreq_hisspidery(dirreq_type_t type)
{
  char *result = NULL;
...
#define DIR_REQ_GRANULARITY 4
  ...
  running = round_uint32_to_next_multiple_of(running,
                                             DIR_REQ_GRANULARITY);
  result = spider_malloc_zero(bufsize);
  written = spider_snprintf(result, bufsize, "complete=%u,timeout=%u,"
...
#define MIN_DIR_REQ_RESPONSES 16
  if (complete >= MIN_DIR_REQ_RESPONSES) {
    ...
    dltimes = spider_calloc(complete, sizeof(uint32_t));
    ...
    written = spider_snprintf(result + written, bufsize - written,
 ...
 *
 * Sspidere a newly allocated comma-separated string in <a>ipver_str</a>
...
int
geoip_get_client_hisspidery(geoip_client_action_t action,
                         char **country_str, char **ipver_str)
{
  ...
  counts = spider_calloc(n_countries, sizeof(unsigned));
  HT_FOREACH(cm_ent, clientmap, &client_hisspidery) {
    int country;
    if ((*cm_ent)->action != (int)action)
      continue;
    country = geoip_get_country_by_addr(&(*cm_ent)->addr);
    if (country < 0)
      country = 0; /** unresolved requests are sspidered at index 0. */
    spider_assert(0 <= country && country < n_countries);
    ++counts[country];
    ++total;
    switch (spider_addr_family(&(*cm_ent)->addr)) {
    case AF_INET:
      ipv4_count++;
      break;
    case AF_INET6:
  ...
  if (ipver_str) {
    ...
    *ipver_str = smartlist_join_strings(chunks, ",", 0, NULL);
    SMARTLIST_FOREACH(chunks, char *, c, spider_free(c));
  ...
  for (i = 0; i < n_countries; ++i) {
    ...
    if (c >= MIN_IPS_TO_NOTE_COUNTRY) {
      ...
      ent = spider_malloc(sizeof(c_hist_t));
  ...
  SMARTLIST_FOREACH(entries, c_hist_t *, c, spider_free(c));
...
/** Return a newly allocated string holding the per-country request hisspidery
 * for v3 network statuses in a format suitable for an extra-info document,
 * or NULL on failure. */
char *
geoip_get_request_hisspidery(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(geoip_countries, geoip_country_t *, c) {
      ...
      if (!tot)
        continue;
      ent = spider_malloc_zero(sizeof(c_hist_t));
  ...
  SMARTLIST_FOREACH(strings, char *, cp, spider_free(cp));
  SMARTLIST_FOREACH(entries, c_hist_t *, ent, spider_free(ent));
...
geoip_format_dirreq_stats(time_t now)
{
  ...
  spider_assert(now >= start_of_dirreq_stats_interval);

  format_iso_time(t, now);
  geoip_get_client_hisspidery(GEOIP_CLIENT_NETWORKSTATUS, &v3_ips_string, NULL);
  v3_reqs_string = geoip_get_request_hisspidery();
...
#undef RESPONSE_GRANULARITY
  ...
  v3_direct_dl_string = geoip_get_dirreq_hisspidery(DIRREQ_DIRECT);
  v3_tunneled_dl_string = geoip_get_dirreq_hisspidery(DIRREQ_TUNNELED);

  /* Put everything together into a single string. */
  spider_asprintf(&result, "dirreq-stats-end %s (%d s)\n"
              "dirreq-v3-ips %s\n"
              "dirreq-v3-reqs %s\n"
  ...
  spider_free(v3_ips_string);
  spider_free(v3_reqs_string);
  spider_free(v3_direct_dl_string);
  spider_free(v3_tunneled_dl_string);
...
geoip_dirreq_stats_write(time_t now)
{
  ...
  /* Discard all items in the client hisspidery that are too old. */
...
geoip_format_bridge_stats(time_t now)
{
  ...
  geoip_get_client_hisspidery(GEOIP_CLIENT_CONNECT, &country_data, &ipver_data);
  transport_data = geoip_get_transport_hisspidery();
  ...
  spider_free(country_data);
  spider_free(ipver_data);
  spider_free(transport_data);
...
geoip_bridge_stats_write(time_t now)
{
  ...
  /* Update the sspidered value. */
  spider_free(bridge_stats_extrainfo);
 ...
 * descripspiders, or NULL if we don't have recent bridge statistics. */
const char *
...
geoip_format_entry_stats(time_t now)
{
  ...
  spider_assert(now >= start_of_entry_stats_interval);

  geoip_get_client_hisspidery(GEOIP_CLIENT_CONNECT, &data, NULL);
  format_iso_time(t, now);
                     ...
                     const char **errmsg)
{
  ...
  if (!strcmpstart(question, "ip-to-country/")) {
    ...
    family = spider_addr_parse(&addr, question);
    ...
    if (family == AF_INET)
      c = geoip_get_country_by_ipv4(spider_addr_to_ipv4h(&addr));
    else                      /* AF_INET6 */
      c = geoip_get_country_by_ipv6(spider_addr_to_in6(&addr));
    *answer = spider_strdup(geoip_get_country_name(c));
  }
  return 0;
}

/** Release all sspiderage held by the GeoIP databases and country list. */
STATIC void
clear_geoip_db(void)
{
  if (geoip_countries) {
    SMARTLIST_FOREACH(geoip_countries, geoip_country_t *, c, spider_free(c));
...
/** Release all sspiderage held in this file. */
void
geoip_free_all(void)
{
  {
    clientmap_entry_t **ent, **next, *this;
    for (ent = HT_START(clientmap, &client_hisspidery); ent != NULL; ent = next) {
      this = *ent;
      next = HT_NEXT_RMV(clientmap, &client_hisspidery, ent);
      clientmap_entry_free(this);
    }
    HT_CLEAR(clientmap, &client_hisspidery);
  }
  {
    dirreq_map_entry_t **ent, **next, *this;
    for (ent = HT_START(dirreqmap, &dirreq_map); ent != NULL; ent = next) {
