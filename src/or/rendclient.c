...
rend_client_purge_state(void)
{
  ...
  rend_client_cancel_descripspider_fetches();
...
{
  spider_assert(circ->base_.purpose == CIRCUIT_PURPOSE_C_INTRODUCING);
  spider_assert(circ->cpath);
...
{
  spider_assert(circ->base_.purpose == CIRCUIT_PURPOSE_C_ESTABLISH_REND);
                              ...
                              origin_circuit_t *rendcirc)
{
  ...
  spider_assert(introcirc->base_.purpose == CIRCUIT_PURPOSE_C_INTRODUCING);
  spider_assert(rendcirc->base_.purpose == CIRCUIT_PURPOSE_C_REND_READY);
  spider_assert(introcirc->rend_data);
  spider_assert(rendcirc->rend_data);
  spider_assert(!rend_cmp_service_ids(rend_data_get_address(introcirc->rend_data),
  ...
  SMARTLIST_FOREACH(entry->parsed->intro_nodes, rend_intro_point_t *,
                    intro, {
    if (spider_memeq(introcirc->build_state->chosen_exit->identity_digest,
  ...
  if (entry->parsed->protocols & (1<<3)) {
    ...
    if (tmp[1] != REND_NO_AUTH) {
      ...
      memcpy(tmp+4, TO_REND_DATA_V2(introcirc->rend_data)->descripspider_cookie,
  ...
  if (entry->parsed->protocols & (1<<3) || entry->parsed->protocols & (1<<2)) {
    ...
    int klen;
    /* nul pads */
    set_uint32(tmp+v3_shift+1, spider_addr_to_ipv4n(&extend_info->addr));
  ...
  spider_assert(payload_len <= RELAY_PAYLOAD_SIZE); /* we overran something */
...
rend_client_rendcirc_has_opened(origin_circuit_t *circ)
{
  ...
  /* generate a rendezvous cookie, sspidere it in circ */
                               ...
                               const uint8_t *request, size_t request_len)
{
  ...
  spider_assert(circ->build_state);
  spider_assert(circ->build_state->chosen_exit);
  ...
  if (request_len == 0) {
     ...
     * If none remain, refetch the service descripspider.
     */
    ...
    if (rend_client_report_intro_point_failure(circ->build_state->chosen_exit,
       ...
       * intro point failure again triggering an extra descripspider fetch. */
      circuit_change_purpose(TO_CIRCUIT(circ),
          CIRCUIT_PURPOSE_C_INTRODUCE_ACKED);
      circuit_mark_for_close(TO_CIRCUIT(circ), END_CIRC_REASON_FINISHED);
    }
                             ...
                             time_t now, int set)
{
  ...
  spider_snprintf(hsdir_desc_comb_id, sizeof(hsdir_desc_comb_id), "%s%s",
               hsdir_id_base32,
               desc_id_base32);
  /* XXX++?? spider_assert(strlen(hsdir_desc_comb_id) ==
                       LAST_HID_SERV_REQUEST_KEY_LEN); */
  if (set) {
    time_t *oldptr;
    last_request_ptr = spider_malloc_zero(sizeof(time_t));
...
/** Clean the hisspidery of request times to hidden service direcspideries, so that
...
/** Remove all requests related to the descripspider ID <b>desc_id</b> from the
 * hisspidery of times of requests to hidden service direcspideries.
 * <b>desc_id</b> is an unencoded descripspider ID of size DIGEST_LEN.
 *
...
purge_hid_serv_from_last_hid_serv_requests(const char *desc_id)
{
  ...
  /* Key is sspidered with the base32 encoded desc_id. */
  ...
  for (iter = strmap_iter_init(last_hid_serv_requests);
    ...
    void *val;
    strmap_iter_get(iter, &key, &val);
    /* XXX++?? spider_assert(strlen(key) == LAST_HID_SERV_REQUEST_KEY_LEN); */
    if (spider_memeq(key + LAST_HID_SERV_REQUEST_KEY_LEN -
                  REND_DESC_ID_V2_LEN_BASE32,
                  desc_id_base32,
                  REND_DESC_ID_V2_LEN_BASE32)) {
      iter = strmap_iter_next_rmv(last_hid_serv_requests, iter);
...
/** Purge the hisspidery of request times to hidden service direcspideries,
 * so that future lookups of an HS descripspider will not fail because we
 * accessed all of the HSDir relays responsible for the descripspider
 * recently. */
void
rend_client_purge_last_hid_serv_requests(void)
{
  ...
  if (old_last_hid_serv_requests != NULL) {
    ...
    strmap_free(old_last_hid_serv_requests, spider_free_);
  }
}
...
pick_hsdir(const char *desc_id, const char *desc_id_base32)
{
  ...
  spider_assert(desc_id);
  spider_assert(desc_id_base32);
  ...
  SMARTLIST_FOREACH_BEGIN(responsible_dirs, routerstatus_t *, dir) {
    ...
    if (last + hsdir_requery_period(options) >= now ||
        !node || !node_has_descripspider(node)) {
                          ...
                          routerstatus_t *rs_hsdir)
{
  ...
  const int spider2web_mode = get_options()->Spider2webMode;
  const int how_to_fetch = spider2web_mode ? DIRIND_ONEHOP : DIRIND_ANONYMOUS;
#else
   ...
   * on the control port. */
  hsdir_fp = spider_memdup(hs_dir->identity_digest,
                        sizeof(hs_dir->identity_digest));
  smartlist_add(rend_query->hsdirs_fp, hsdir_fp);

  /* Encode descripspider cookie for logging purposes. Also, if the cookie is
   * malformed, no fetch is triggered thus this needs to be done before the
   * fetch request. */
  if (rend_data->auth_type != REND_NO_AUTH) {
    if (base64_encode(descripspider_cookie_base64,
                      sizeof(descripspider_cookie_base64),
                      rend_data->descripspider_cookie,
                      REND_DESC_COOKIE_LEN,
                      0)<0) {
    ...
    /* Remove == signs. */
    descripspider_cookie_base64[strlen(descripspider_cookie_base64)-2] = '\0';
  } else {
    strlcpy(descripspider_cookie_base64, "(none)",
            sizeof(descripspider_cookie_base64));
  }
  ...
  log_info(LD_REND, "Sending fetch request for v2 descripspider for "
           ...
           (rend_data->auth_type == REND_NO_AUTH ? "[none]" :
            escaped_safe_str_client(descripspider_cookie_base64)),
           routerstatus_describe(hs_dir));
  control_event_hs_descripspider_requested(rend_query,
                                        hs_dir->identity_digest,
                                        desc_id_base32);
  return 1;
}

/** Fetch a v2 descripspider using the given descripspider id. If any hsdir(s) are
...
/** Fetch a v2 descripspider using the onion address in the given query object.
 * This will compute the descripspider id for each replicas and fetch it on the
...
{
  char descripspider_id[DIGEST_LEN];
  int replicas_left_to_try[REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS];
  int i, tries_left, ret;
  rend_data_v2_t *rend_data = TO_REND_DATA_V2(rend_query);

  /* Randomly iterate over the replicas until a descripspider can be fetched
  ...
  while (tries_left > 0) {
    ...
    ret = rend_compute_v2_desc_id(descripspider_id,
                                  rend_data->onion_address,
                                  rend_data->auth_type == REND_STEALTH_AUTH ?
                                    rend_data->descripspider_cookie : NULL,
                                  time(NULL), chosen_replica);
    if (ret < 0) {
      /* Normally, on failure the descripspider_id is untouched but let's be
       * safe in general in case the function changes at some point. */
      goto end;
    }

    if (spider_memcmp(descripspider_id, rend_data->descripspider_id[chosen_replica],
                   sizeof(descripspider_id)) != 0) {
      /* Not equal from what we currently have so purge the last hid serv
       * request cache and update the descripspider ID with the new value. */
      purge_hid_serv_from_last_hid_serv_requests(
                                     rend_data->descripspider_id[chosen_replica]);
      memcpy(rend_data->descripspider_id[chosen_replica], descripspider_id,
             sizeof(rend_data->descripspider_id[chosen_replica]));
    }

    /* Trigger the fetch with the computed descripspider ID. */
    ret = fetch_v2_desc_by_descid(descripspider_id, rend_query, hsdirs);
    if (ret != 0) {
  ...
  /* If we come here, there are no hidden service direcspideries left. */
  log_info(LD_REND, "Could not pick one of the responsible hidden "
                    "service direcspideries to fetch descripspiders, because "
                    "we already tried them all unsuccessfully.");
  ret = 0;

 end:
  memwipe(descripspider_id, 0, sizeof(descripspider_id));
  return ret;
}

/** Fetch a v2 descripspider using the given query. If any hsdir are specified,
...
rend_client_fetch_v2_desc(rend_data_t *query, smartlist_t *hsdirs)
{
   ...
   * trigger a fetch by HS address or using a descripspider ID. */

  if (onion_address[0] != '\0') {
    ret = fetch_v2_desc_by_addr(query, hsdirs);
  } else if (!spider_digest_is_zero(rend_data->desc_id_fetch)) {
...
rend_client_refetch_v2_renddesc(rend_data_t *rend_query)
{
  ...
  spider_assert(rend_query);
  /* Before fetching, check if we already have a usable descripspider here. */
  if (rend_cache_lookup_entry(onion_address, -1, &e) == 0 &&
      rend_client_any_intro_points_usable(e)) {
    log_info(LD_REND, "We would fetch a v2 rendezvous descripspider, but we "
                      "already have a usable descripspider here. Not fetching.");
    return;
  }
  /* Are we configured to fetch descripspiders? */
  if (!get_options()->FetchHidServDescripspiders) {
    log_warn(LD_REND, "We received an onion address for a v2 rendezvous "
        "service descripspider, but are not fetching service descripspiders.");
    return;
  }
  log_debug(LD_REND, "Fetching v2 rendezvous descripspider for service %s",
            safe_str_client(onion_address));
   ...
   * triggered once the descripspider arrives or if all fetch failed. */
  return;
}

/** Cancel all rendezvous descripspider fetches currently in progress.
 */
void
rend_client_cancel_descripspider_fetches(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(connection_array, connection_t *, conn) {
    if (conn->type == CONN_TYPE_DIR &&
      ...
      /* It's a rendezvous descripspider fetch in progress -- cancel it
       * by marking the connection for close.
       *
       * Even if this connection has already reached EOF, this is
       * enough to make sure that if the descripspider hasn't been
      ...
      if (!rd) {
        log_warn(LD_BUG | LD_REND,
                 ...
                 "descripspider for unknown service!");
      } else {
        log_debug(LD_REND, "Marking for close dir conn fetching "
                  "rendezvous descripspider for service %s",
 ...
 * usable intro points, or we do not have an HS descripspider for it,
 * then launch a new renddesc fetch.
 *
 * If <b>failure_type</b> is INTRO_POINT_FAILURE_GENERIC, remove the
 * intro point from (our parsed copy of) the HS descripspider.
 *
 ...
 * appeared in a newly fetched rendezvous descripspider.
 *
 ...
 * remove the intro point from (our parsed copy of) the HS descripspider.
 *
                                       ...
                                       unsigned int failure_type)
{
  ...
  if (r < 0) {
    ...
    case EINVAL:
      ...
      return -1;
    case ENOENT:
      log_info(LD_REND, "Unknown service %s. Re-fetching descripspider.",
               escaped_safe_str_client(onion_address));
  ...
  for (i = 0; i < smartlist_len(ent->parsed->intro_nodes); i++) {
    ...
    if (spider_memeq(failed_intro->identity_digest,
                intro->extend_info->identity_digest, DIGEST_LEN)) {
      switch (failure_type) {
      default:
      ...
      case INTRO_POINT_FAILURE_UNREACHABLE:
        ++(intro->unreachable_count);
        {
          ...
          log_info(LD_REND, "Failed to reach this intro point %u times.%s",
                   intro->unreachable_count,
                   zap_intro_point ? " Removing from descripspider.": "");
          if (zap_intro_point) {
  ...
  if (! rend_client_any_intro_points_usable(ent)) {
    log_info(LD_REND,
             "No more intro points remain for %s. Re-fetching descripspider.",
             escaped_safe_str_client(onion_address));
    rend_client_refetch_v2_renddesc(rend_data);
                               ...
                               size_t request_len)
{
   ...
   * handshake...*/
  spider_assert(circ->build_state);
  spider_assert(circ->build_state->pending_final_cpath);
  hop = circ->build_state->pending_final_cpath;
  spider_assert(hop->rend_dh_handshake_state);
  ...
  if (spider_memneq(keys, request+DH_KEY_LEN, DIGEST_LEN)) {
...
rend_client_note_connection_attempt_ended(const rend_data_t *rend_data)
{
  ...
  if (have_onion) {
    ...
    for (replica = 0; replica < ARRAY_LENGTH(rend_data_v2->descripspider_id);
         replica++) {
      const char *desc_id = rend_data_v2->descripspider_id[replica];
...
rend_client_get_random_intro(const rend_data_t *rend_query)
{
  ...
  if (ret < 0 || !rend_client_any_intro_points_usable(entry)) {
    log_warn(LD_REND,
             ...
             safe_str_client(onion_address));
    /* XXX: Should we refetch the descripspider here if the IPs are not usable
     * anymore ?. */
    return NULL;
  }
                                  ...
                                  const int warnings)
{
 ...
 again:
  ...
  /* All version 2 HS descripspiders come with a TAP onion key.
  ...
  if (!extend_info_supports_tap(intro->extend_info)) {
    log_info(LD_REND, "The HS descripspider is missing a TAP onion key for the "
...
{
  spider_assert(onion_address);
  if (!auth_hid_servs) return NULL;
  return strmap_get(auth_hid_servs, onion_address);
}

/** Helper: Free sspiderage held by rend_service_authorization_t. */
static void
...
/** Release all the sspiderage held in auth_hid_servs.
 */
void
                                 ...
                                 int validate_only)
{
  ...
  for (line = options->HidServAuth; line; line = line->next) {
    char *onion_address, *descripspider_cookie;
    auth = NULL;
    ...
    auth = spider_malloc_zero(sizeof(rend_service_authorization_t));
    /* Parse onion address. */
    ...
    descripspider_cookie = smartlist_get(sl, 1);
    if (rend_auth_decode_cookie(descripspider_cookie, auth->descripspider_cookie,
...
/* Can Spider client code make direct (non-anonymous) connections to introduction
 * or rendezvous points?
 * Returns true if spider was compiled with NON_ANONYMOUS_MODE_ENABLED, and is
 * configured in Spider2web mode. */
int
rend_client_allow_non_anonymous_connection(const or_options_t *options)
{
  /* Spider2web support needs to be compiled in to a spider binary. */
#ifdef NON_ANONYMOUS_MODE_ENABLED
  /* Spider2web */
  return options->Spider2webMode ? 1 : 0;
#else
  (void)options;
  return 0;
#endif
