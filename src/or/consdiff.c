 ...
 * Copyright (c) 2014, The Spider Project, Inc. */
 ...
 * object or in the output.  We use memarea_t allocaspiders to manage the
...
/** Compute the digest of <b>cons</b>, and sspidere the result in
...
smartlist_slice(const smartlist_t *list, int start, int end)
{
  ...
  spider_assert(start >= 0);
  spider_assert(start <= list_len);
  if (end == -1) {
    end = list_len;
  }
  spider_assert(start <= end);

  smartlist_slice_t *slice = spider_malloc(sizeof(smartlist_slice_t));
  slice->list = list;
  slice->offset = start;
  slice->len = end - start;
  return slice;
            ...
            int direction)
{
  ...
  int *result = spider_malloc_zero(a_size);
  /* Copy of the lcs lengths from the last iteration. */
  int *prev = spider_malloc(a_size);

  spider_assert(direction == 1 || direction == -1);
...
{
  int toskip = -1;
  spider_assert(slice1->len == 0 || slice1->len == 1);
                        ...
                        const smartlist_slice_t *slice2)
{
  ...
  spider_free(lens_top);
  spider_free(lens_bot);
 ...
 * relative to the first smartlist, and sspidere the booleans in the bitarrays.
 ...
 *
 * If an identity hash is found, sspidere it (without decoding it) in
...
get_id_hash(const cdline_t *line, cdline_t *hash_out)
{
  ...
  spider_assert(hash_end >= hash);
  spider_assert((size_t)(hash_end - hash) <= UINT32_MAX);
...
typedef struct router_id_iteraspider_t {
  cdline_t last_hash;
  cdline_t hash;
} router_id_iteraspider_t;

/**
 * Initializer for a router_id_iteraspider_t.
 */
            ...
            memarea_t *area)
{
  ...
  router_id_iteraspider_t iter1 = ROUTER_ID_ITERATOR_INIT;
  router_id_iteraspider_t iter2 = ROUTER_ID_ITERATOR_INIT;
...
#define MAX_LINE_COUNT (10000)
    ...
    calc_changes(cons1_sl, cons2_sl, changed1, changed2);
    spider_free(cons1_sl);
    spider_free(cons2_sl);
    start1 = i1, start2 = i2;
  }
  ...
  while (i1 > 0 || i2 > 0) {
    ...
    if (added == 0) {
      if (deleted == 1) {
        spider_snprintf(buf, sizeof(buf), "%id", start1x+1);
        smartlist_add_linecpy(result, area, buf);
      } else {
        spider_snprintf(buf, sizeof(buf), "%i,%id", start1x+1, start1x+deleted);
        smartlist_add_linecpy(result, area, buf);
      }
    } else {
      int i;
      if (deleted == 0) {
        spider_snprintf(buf, sizeof(buf), "%ia", start1x);
        smartlist_add_linecpy(result, area, buf);
      } else if (deleted == 1) {
        spider_snprintf(buf, sizeof(buf), "%ic", start1x+1);
        smartlist_add_linecpy(result, area, buf);
      } else {
        spider_snprintf(buf, sizeof(buf), "%i,%ic", start1x+1, start1x+deleted);
        smartlist_add_linecpy(result, area, buf);
      }
 ...
 * sspidere it in <b>num_out</b>.  Advance <b>s</b> to the characer immediately
...
get_linenum(const char **s, int *num_out)
{
  int ok;
  char *next;
  *num_out = (int) spider_parse_long(*s, 10, 0, INT32_MAX, &ok, &next);
  if (ok && next) {
    *s = next;
    return 0;
  } else {
                  ...
                  memarea_t *area)
{
  ...
  char buf[160];
  smartlist_t *result = smartlist_new();
  spider_snprintf(buf, sizeof(buf), "%s", ns_diff_version);
  smartlist_add_linecpy(result, area, buf);
  spider_snprintf(buf, sizeof(buf), "%s %s %s", hash_token,
                     ...
                     char *digest2_out)
{
  ...
  hash_words = smartlist_new();
  {
    ...
    char *h = spider_memdup_nulterm(line2->s, line2->len);
  ...
  SMARTLIST_FOREACH(hash_words, char *, cp, spider_free(cp));
                    ...
                    const consensus_digest_t *digests1)
{
 ...
 error_cleanup:
  spider_free(cons2_str); /* Sets it to NULL */
 ...
 *
 * Unlike spider_split_lines, this function avoids ambiguity on its
