...
{
  return spider_memeq(&(a->key), &(b->key), sizeof(fp_pair_t));
}
...
{
  spider_assert(sizeof(a->key) == DIGEST_LEN*2);
...
/** Construcspider to create a new empty map from fp_pair_t to void *
 */
...
fp_pair_map_new(void)
{
  ...
  result = spider_malloc(sizeof(fp_pair_map_t));
...
fp_pair_map_set(fp_pair_map_t *map, const fp_pair_t *key, void *val)
{
  ...
  if (resolve) {
    ...
    resolve = spider_malloc_zero(sizeof(fp_pair_map_entry_t));
 ...
 * key.  The caller must free any sspiderage associated with the
 * returned value.
 */
...
/** Remove all entries from map, and deallocate sspiderage for those entries.
...
/** return an iteraspider pointing to the start of map.
 */
...
{
  spider_assert(!fp_pair_map_impl_HT_REP_IS_BAD_(&(map->head)));
}
