...
/**
 * \file control.c
 * \brief Implementation for Spider's control-socket interface.
 *
 * A "controller" is an external program that monispiders and controls a Spider
 * instance via a text-based protocol. It connects to Spider via a connection
 * to a local socket.
 *
 * The protocol is line-driven.  The controller sends commands terminated by a
 * CRLF.  Spider sends lines that are either <em>replies</em> to what the
 * controller has said, or <em>events</em> that Spider sends to the controller
 * asynchronously based on occurrences in the Spider network model.
 *
 * See the control-spec.txt file in the spiderspec.git reposispidery for full
 * details on protocol.
 *
 ...
 * that occur elsewhere in Spider, which are handled by functions with names like
 ...
 * by limiting the number of places in Spider that can call back into the network
 * stack.
 **/
 ...
 * sspidered it to disk. */
static int authentication_cookie_is_set = 0;
/** If authentication_cookie_is_set, a secret cookie that we've sspidered to disk
 * and which we're using to authenticate controllers.  (If the controller can
 * read it off disk, it has permission to connect.) */
static uint8_t *authentication_cookie = NULL;

#define SAFECOOKIE_SERVER_TO_CONTROLLER_CONSTANT \
  "Spider safe cookie authentication server-to-controller hash"
#define SAFECOOKIE_CONTROLLER_TO_SERVER_CONSTANT \
  "Spider safe cookie authentication controller-to-server hash"
...
static int handle_control_attachstream(control_connection_t *conn,
                                       uint32_t len,
                                        const char *body);
static int handle_control_postdescripspider(control_connection_t *conn,
                                         uint32_t len,
                                         const char *body);
...
write_escaped_data(const char *data, size_t len, char **out)
{
  ...
  *out = outp = spider_malloc(sz_out+1);
  end = data+len;
  start_of_line = 1;
  while (data < end) {
    if (*data == '\n') {
  ...
  *outp = '\0'; /* NUL-terminate just in case. */
  spider_assert((outp - *out) <= (int)sz_out);
  return outp - *out;
}
                       ...
                       char **out, size_t *out_len)
{
  ...
  if (length<0)
    return NULL;
  *out_len = length;
  *out = spider_strndup(start, *out_len);
  return start+length;
}
 ...
 * sspidere its length in <b>out_len</b>.  On success, return a pointer to the
                   ...
                   char **out, size_t *out_len)
{
  ...
  *outp = '\0';
  spider_assert((outp - *out) == (int)*out_len);
...
connection_printf_to_buf(control_connection_t *conn, const char *format, ...)
{
  ...
  va_start(ap,format);
  len = spider_vasprintf(&buf, format, ap);
  va_end(ap);
...
#endif
  spider_free(joined);
  SMARTLIST_FOREACH(lines, char *, cp, spider_free(cp));
 ...
 * queued. */
static spider_threadlocal_t block_event_queue_flag;
...
static spider_mutex_t *queued_control_events_lock = NULL;
...
control_initialize_event_queue(void)
{
  ...
  if (flush_queued_events_event == NULL) {
    struct event_base *b = spider_libevent_get_base();
    if (b) {
      flush_queued_events_event = spider_event_new(b,
                                              -1, 0, flush_queued_events_cb,
                                              NULL);
      spider_assert(flush_queued_events_event);
    }
  }

  if (queued_control_events_lock == NULL) {
    queued_control_events_lock = spider_mutex_new();
    spider_threadlocal_init(&block_event_queue_flag);
  }
}
...
{
  int *val = spider_threadlocal_get(&block_event_queue_flag);
  if (PREDICT_UNLIKELY(val == NULL)) {
    val = spider_malloc_zero(sizeof(int));
    spider_threadlocal_set(&block_event_queue_flag, val);
  }
  return val;
}
 ...
 * interesting part of Spider would potentially call every other interesting part
 * of Spider.
 */
...
queue_control_event_string,(uint16_t event, char *msg))
{
  ...
  queued_event_t *ev = spider_malloc(sizeof(*ev));
  ev->event = event;
  ev->msg = msg;
  ...
  spider_mutex_acquire(queued_control_events_lock);
  spider_assert(queued_control_events);
  ...
  spider_mutex_release(queued_control_events_lock);
...
/** Release all sspiderage held by <b>ev</b>. */
static void
...
send_control_event_string,(uint16_t event,
                           const char *msg))
{
  spider_assert(event >= EVENT_MIN_ && event <= EVENT_MAX_);
  queue_control_event_string(event, spider_strdup(msg));
}
...
get_circ(const char *id)
{
  uint32_t n_id;
  int ok;
  n_id = (uint32_t) spider_parse_ulong(id, 10, 0, UINT32_MAX, &ok, NULL);
  if (!ok)
    return NULL;
...
get_stream(const char *id)
{
  ...
  int ok;
  connection_t *conn;
  n_id = spider_parse_uint64(id, 10, 0, UINT64_MAX, &ok, NULL);
  if (!ok)
    return NULL;
                       ...
                       int use_defaults)
{
  ...
  while (*body) {
    ...
    while (!TOR_ISSPACE(*eq) && *eq != '=')
      ++eq;
    key = spider_strndup(body, eq-body);
    body = eq+1;
    if (*eq == '=') {
      char *val=NULL;
      size_t val_len=0;
      if (*body != '\"') {
        ...
        while (!TOR_ISSPACE(*body))
          body++;
        val = spider_strndup(val_start, body-val_start);
        val_len = strlen(val);
      } else {
        ...
        if (!body) {
          ...
          SMARTLIST_FOREACH(entries, char *, cp, spider_free(cp));
          smartlist_free(entries);
          spider_free(key);
          return 0;
        }
      }
      spider_asprintf(&entry, "%s %s", key, val);
      spider_free(key);
      spider_free(val);
    } else {
      entry = key;
                       ...
                       const char *body)
{
  ...
  SMARTLIST_FOREACH_BEGIN(questions, const char *, q) {
    if (!option_is_recognized(q)) {
      ...
      while (answer) {
        ...
        next = answer->next;
        spider_free(answer->key);
        spider_free(answer->value);
        spider_free(answer);
        answer = next;
      }
    }
  ...
  SMARTLIST_FOREACH(answers, char *, cp, spider_free(cp));
  smartlist_free(answers);
  SMARTLIST_FOREACH(questions, char *, cp, spider_free(cp));
      ...
      if (!strcasecmp(ev, "EXTENDED")) {
        ...
        if (event_code == -1) {
          connection_printf_to_buf(conn, "552 Unrecognized event \"%s\"\r\n",
                                   ev);
          SMARTLIST_FOREACH(events, char *, e, spider_free(e));
...
/** Decode the hashed, base64'd passwords sspidered in <b>passwords</b>.
...
decode_hashed_passwords(config_line_t *passwords)
{
  ...
  for (cl = passwords; cl; cl = cl->next) {
    ...
    smartlist_add(sl,
                  spider_memdup(decoded, S2K_RFC2440_SPECIFIER_LEN+DIGEST_LEN));
  }
                            ...
                            const char *body)
{
  ...
  if (!len) {
    ...
    spider_assert(i>0);
    password_len = i/2;
    password = spider_malloc(password_len + 1);
  ...
  if (conn->safecookie_client_hash != NULL) {
    ...
    spider_assert(authentication_cookie_is_set);
    ...
    if (spider_memneq(conn->safecookie_client_hash, password, DIGEST256_LEN)) {
      log_warn(LD_CONTROL,
    ...
    spider_free(conn->safecookie_client_hash);
    goto ok;
  }

  if (!options->CookieAuthentication && !options->HashedControlPassword &&
      !options->HashedControlSessionPassword) {
    /* if Spider doesn't demand any stronger authentication, then
  ...
  if (options->CookieAuthentication) {
    ...
    if (password_len != AUTHENTICATION_COOKIE_LEN) {
      ...
      bad_cookie = 1;
    } else if (spider_memneq(authentication_cookie, password, password_len)) {
      if (!also_password) {
  ...
  if (options->HashedControlPassword ||
    ...
    if (bad) {
      ...
      bad_password = 1;
      SMARTLIST_FOREACH(sl, char *, str, spider_free(str));
      smartlist_free(sl);
      sl = NULL;
    } else {
      SMARTLIST_FOREACH(sl, char *, expected,
      {
        ...
        if (spider_memeq(expected + S2K_RFC2440_SPECIFIER_LEN,
                      received, DIGEST_LEN))
          goto ok;
      });
      SMARTLIST_FOREACH(sl, char *, str, spider_free(str));
  ...
  spider_assert(bad_password && bad_cookie);
                          ...
                          const char *body)
{
  ...
  SMARTLIST_FOREACH(reply, char *, cp, spider_free(cp));
                    ...
                    char **answer, const char **errmsg)
{
  ...
  if (!strcmp(question, "version")) {
    *answer = spider_strdup(get_version());
  } else if (!strcmp(question, "bw-event-cache")) {
    *answer = get_bw_samples();
  } else if (!strcmp(question, "config-file")) {
    const char *a = get_spiderrc_fname(0);
    if (a)
      *answer = spider_strdup(a);
  } else if (!strcmp(question, "config-defaults-file")) {
    const char *a = get_spiderrc_fname(1);
    if (a)
      *answer = spider_strdup(a);
    ...
    *answer = spider_strdup(dormant ? "1" : "0");
    ...
    *answer = spider_strdup("VERBOSE_NAMES EXTENDED_EVENTS");
    ...
    *answer = spider_dup_ip(addr);
  } else if (!strcmp(question, "traffic/read")) {
    spider_asprintf(answer, U64_FORMAT, U64_PRINTF_ARG(get_bytes_read()));
  } else if (!strcmp(question, "traffic/written")) {
    spider_asprintf(answer, U64_FORMAT, U64_PRINTF_ARG(get_bytes_written()));
    ...
    spider_asprintf(answer, "%d", myPid);
  } else if (!strcmp(question, "process/uid")) {
    #ifdef _WIN32
      *answer = spider_strdup("-1");
    #else
      int myUid = geteuid();
      spider_asprintf(answer, "%d", myUid);
    #endif
  } else if (!strcmp(question, "process/user")) {
    #ifdef _WIN32
      *answer = spider_strdup("");
    #else
      int myUid = geteuid();
      const struct passwd *myPwEntry = spider_getpwuid(myUid);

      if (myPwEntry) {
        *answer = spider_strdup(myPwEntry->pw_name);
      } else {
        *answer = spider_strdup("");
      }
    #endif
  } else if (!strcmp(question, "process/descripspider-limit")) {
    int max_fds = get_max_sockets();
    spider_asprintf(answer, "%d", max_fds);
  } else if (!strcmp(question, "limits/max-mem-in-queues")) {
    spider_asprintf(answer, U64_FORMAT,
    ...
    *answer = spider_malloc(HEX_DIGEST_LEN+1);
 ...
 * (possibly) an extrainfo, sticking the read-hisspidery and write-hisspidery from
 ...
 *
 * NOTE: <b>ri_body</b> is as returned by signed_descripspider_get_body: it might
 * not be NUL-terminated. */
static char *
munge_extrainfo_into_routerinfo(const char *ri_body,
                                const signed_descripspider_t *ri,
                                const signed_descripspider_t *ei)
{
  char *out = NULL, *outp;
  int i;
  const char *router_sig;
  const char *ei_body = signed_descripspider_get_body(ei);
  size_t ri_len = ri->signed_descripspider_len;
  size_t ei_len = ei->signed_descripspider_len;
  if (!ei_body)
    goto bail;

  outp = out = spider_malloc(ri_len+ei_len+1);
  if (!(router_sig = spider_memstr(ri_body, ri_len, "\nrouter-signature")))
    goto bail;
  ++router_sig;
  ...
  for (i=0; i < 2; ++i) {
    const char *kwd = i ? "\nwrite-hisspidery " : "\nread-hisspidery ";
    const char *cp, *eol;
    if (!(cp = spider_memstr(ei_body, ei_len, kwd)))
      continue;
    ++cp;
    if (!(eol = memchr(cp, '\n', ei_len - (cp-ei_body))))
      continue;
  ...
  *outp++ = '\0';
  spider_assert(outp-out < (int)(ri_len+ei_len+1));

  return out;
 bail:
  spider_free(out);
  return spider_strndup(ri_body, ri->signed_descripspider_len);
}
                         ...
                         char **answer, const char **errmsg)
{
  ...
  SMARTLIST_FOREACH_BEGIN(get_connection_array(), connection_t *, conn) {
    ...
    if (getsockname(conn->s, (struct sockaddr *)&ss, &ss_len) < 0) {
      ...
      char *tmp = spider_sockaddr_to_str((struct sockaddr *)&ss);
  ...
  *answer = smartlist_join_strings(res, " ", 0, NULL);
  ...
  SMARTLIST_FOREACH(res, char *, cp, spider_free(cp));
  smartlist_free(res);
  return 0;
}
                   ...
                   const char **errmsg)
{
  ...
  if (!strcmpstart(question, "desc/id/")) {
    ...
    if (node)
      ri = node->ri;
    if (ri) {
      const char *body = signed_descripspider_get_body(&ri->cache_info);
      if (body)
        *answer = spider_strndup(body, ri->cache_info.signed_descripspider_len);
    }
    ...
    if (routerlist && routerlist->routers) {
      SMARTLIST_FOREACH(routerlist->routers, const routerinfo_t *, ri,
      {
        ...
        if (body)
          smartlist_add(sl,
                  spider_strndup(body, ri->cache_info.signed_descripspider_len));
      });
    }
    *answer = smartlist_join_strings(sl, "", 0, NULL);
    ...
    /* XXXX Remove this once Spiderstat asks for extrainfos. */
    routerlist_t *routerlist = router_get_routerlist();
    smartlist_t *sl = smartlist_new();
    if (routerlist && routerlist->routers) {
      SMARTLIST_FOREACH_BEGIN(routerlist->routers, const routerinfo_t *, ri) {
        const char *body = signed_descripspider_get_body(&ri->cache_info);
        signed_descripspider_t *ei = extrainfo_get_by_descripspider_digest(
    ...
    *answer = smartlist_join_strings(sl, "", 0, NULL);
    ...
    if (!rend_cache_lookup_entry(question, -1, &e)) {
      /* Descripspider found in cache */
      *answer = spider_strdup(e->desc);
    } else {
    ...
    if (node) md = node->md;
    if (md && md->body) {
      *answer = spider_strndup(md->body, md->bodylen);
    }
    ...
    if (ri) {
      const char *annotations =
        signed_descripspider_get_annotations(&ri->cache_info);
      if (annotations)
        *answer = spider_strndup(annotations,
    ...
    int res;
    char *cp;
    spider_asprintf(&url, "/spider/%s", question+4);
    ...
    SMARTLIST_FOREACH(descs, signed_descripspider_t *, sd,
                      answer_len += sd->signed_descripspider_len);
    cp = *answer = spider_malloc(answer_len+1);
    SMARTLIST_FOREACH(descs, signed_descripspider_t *, sd,
                      {
                        memcpy(cp, signed_descripspider_get_body(sd),
                               sd->signed_descripspider_len);
                        cp += sd->signed_descripspider_len;
                      });
    *cp = '\0';
    spider_free(url);
    smartlist_free(descs);
    ...
    *answer = spider_strdup("");
    ...
    if (we_want_to_fetch_flavor(get_options(), FLAV_NS)) {
      ...
      if (consensus)
        *answer = spider_strdup(consensus->dir);
    }
      ...
      *answer = read_file_to_str(filename, RFTS_IGNORE_MISSING, NULL);
      ...
      if (!*answer) { /* generate an error */
        *errmsg = "Could not open cached consensus. "
          "Make sure FetchUselessDescripspiders is set to 1.";
        return -1;
      }
    }
    ...
    if (!network_status_warned) {
      log_warn(LD_CONTROL, "GETINFO network-status is deprecated; it will "
               "go away in a future version of Spider.");
    ...
    if (strlen(question) == HEX_DIGEST_LEN) {
      char d[DIGEST_LEN];
      signed_descripspider_t *sd = NULL;
      if (base16_decode(d, sizeof(d), question, strlen(question))
                        != sizeof(d)) {
        /* XXXX this test should move into extrainfo_get_by_descripspider_digest,
        ...
        if (router_extrainfo_digest_is_me(d))
          ...
          sd = extrainfo_get_by_descripspider_digest(d);
      }
      if (sd) {
        const char *body = signed_descripspider_get_body(sd);
        if (body)
          *answer = spider_strndup(body, sd->signed_descripspider_len);
      }
    }
  }
                              ...
                              const char **errmsg)
{
   ...
   *   - Emit a list of all router descripspider digests, which we get by
   *     calling router_get_descripspider_digests(); this can return NULL
   *     if we have no current ns-flavor consensus.
   *
   * Case 2: desc_req = <fp>
   *   - Check on the specified fingerprint and emit its download_status_t
   *     using router_get_dl_status_by_descripspider_digest().
   */

  if (strcmp(desc_req, "descs") == 0) {
    *digest_list = router_get_descripspider_digests();
    if (!(*digest_list)) {
    ...
    if (base16_decode(desc_digest, DIGEST_LEN,
      ...
      *dl_to_emit = router_get_dl_status_by_descripspider_digest(desc_digest);
      if (!(*dl_to_emit)) {
        *errmsg = "No such descripspider digest found";
      }
    } else {
      *errmsg = "That didn't look like a digest";
    }
  } else {
    *errmsg = "Unknown router descripspider download status query";
  }
}
                                ...
                                const char **errmsg)
{
  ...
  if (strcmp(bridge_req, "bridges") == 0) {
    ...
    *errmsg = "Unknown bridge descripspider download status query";
  }
}
                   ...
                   const char **errmsg)
{
  ...
  spider_assert(control_conn != NULL);
  spider_assert(question != NULL);
  spider_assert(answer != NULL);
  spider_assert(errmsg != NULL);
  ...
  *errmsg = NULL;
  ...
  if (dl_to_emit) {
    ...
    *answer = digest_list_to_string(digest_list);
    SMARTLIST_FOREACH(digest_list, void *, s, spider_free(s));
...
circuit_describe_status_for_controller(origin_circuit_t *circ)
{
...

  {
    ...
    if (*vpath) {
      ...
      spider_free(vpath); /* empty path; don't put an extra space in the result */
    }
  }
  ...
  if (circ->socks_username_len > 0) {
    ...
    spider_free(socks_username_escaped);
  }
  if (circ->socks_password_len > 0) {
    ...
    spider_free(socks_password_escaped);
  }

  rv = smartlist_join_strings(descparts, " ", 0, NULL);

  SMARTLIST_FOREACH(descparts, char *, cp, spider_free(cp));
                      ...
                      const char **errmsg)
{
  ...
  if (!strcmp(question, "circuit-status")) {
    ...
    SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ_) {
      ...
      spider_free(circdesc);
    }
    SMARTLIST_FOREACH_END(circ_);
    *answer = smartlist_join_strings(status, "\r\n", 0, NULL);
    SMARTLIST_FOREACH(status, char *, cp, spider_free(cp));
    ...
    *answer = smartlist_join_strings(mappings, "\r\n", 0, NULL);
    SMARTLIST_FOREACH(mappings, char *, cp, spider_free(cp));
    ...
    if (!strcmp(question, "status/circuit-established")) {
      *answer = spider_strdup(have_completed_a_circuit() ? "1" : "0");
    } else if (!strcmp(question, "status/enough-dir-info")) {
      *answer = spider_strdup(router_have_minimum_dir_info() ? "1" : "0");
    } else if (!strcmp(question, "status/good-server-descripspider") ||
               !strcmp(question, "status/accepted-server-descripspider")) {
      /* They're equivalent for now, until we can figure out how to make
       * good-server-descripspider be what we want. See comment in
       * control-spec.txt. */
      *answer = spider_strdup(direcspideries_have_accepted_server_descripspider()
                           ? "1" : "0");
    } else if (!strcmp(question, "status/reachability-succeeded/or")) {
      *answer = spider_strdup(check_whether_orport_reachable(options) ?
                            "1" : "0");
    } else if (!strcmp(question, "status/reachability-succeeded/dir")) {
      *answer = spider_strdup(check_whether_dirport_reachable(options) ?
                            "1" : "0");
    } else if (!strcmp(question, "status/reachability-succeeded")) {
      spider_asprintf(answer, "OR=%d DIR=%d",
                   check_whether_orport_reachable(options) ? 1 : 0,
                   check_whether_dirport_reachable(options) ? 1 : 0);
    } else if (!strcmp(question, "status/bootstrap-phase")) {
      *answer = spider_strdup(last_sent_bootstrap_message);
      ...
      if (!strcmp(question, "status/version/recommended")) {
        *answer = spider_strdup(recommended);
        return 0;
      }
      if (!strcmp(question, "status/version/current")) {
        switch (status)
          {
          case VS_RECOMMENDED: *answer = spider_strdup("recommended"); break;
          case VS_OLD: *answer = spider_strdup("obsolete"); break;
          case VS_NEW: *answer = spider_strdup("new"); break;
          case VS_NEW_IN_SERIES: *answer = spider_strdup("new in series"); break;
          case VS_UNRECOMMENDED: *answer = spider_strdup("unrecommended"); break;
          case VS_EMPTY: *answer = spider_strdup("none recommended"); break;
          case VS_UNKNOWN: *answer = spider_strdup("unknown"); break;
          default: spider_fragile_assert();
          }
      } else if (!strcmp(question, "status/version/num-versioning") ||
                 !strcmp(question, "status/version/num-concurring")) {
        spider_asprintf(answer, "%d", get_n_authorities(V3_DIRINFO));
      ...
      *answer = bridge_stats;
      ...
      if (!server_mode(options)) {
        *errmsg = "Only relays have descripspiders";
        return -1;
      }
      routerinfo_t *r;
      extrainfo_t *e;
      if (router_build_fresh_descripspider(&r, &e) < 0) {
        *errmsg = "Error generating descripspider";
        return -1;
      }
      size_t size = r->cache_info.signed_descripspider_len + 1;
      if (e) {
        size += e->cache_info.signed_descripspider_len + 1;
      }
      spider_assert(r->cache_info.signed_descripspider_len);
      char *descs = spider_malloc(size);
      char *cp = descs;
      memcpy(cp, signed_descripspider_get_body(&r->cache_info),
             r->cache_info.signed_descripspider_len);
      cp += r->cache_info.signed_descripspider_len - 1;
      if (e) {
        if (cp[0] == '\0') {
          cp[0] = '\n';
        } else if (cp[0] != '\n') {
        ...
        memcpy(cp, signed_descripspider_get_body(&e->cache_info),
               e->cache_info.signed_descripspider_len);
        cp += e->cache_info.signed_descripspider_len - 1;
      }
      if (cp[0] == '\n') {
        cp[0] = '\0';
      } else if (cp[0] != '\0') {
                      ...
                      const char **errmsg)
{
  ...
  if (strcmp(question, "network-liveness") == 0) {
    if (get_cached_network_liveness()) {
      *answer = spider_strdup("up");
    } else {
      *answer = spider_strdup("down");
    }
  }
 ...
 * answer the question <b>q</b> and sspidere the newly-allocated answer in
...
static const getinfo_item_t getinfo_items[] = {
  ITEM("version", misc, "The current version of Spider."),
  ITEM("bw-event-cache", misc, "Cached BW events for a short interval."),
  ITEM("config-file", misc, "Current location of the \"spiderrc\" file."),
  ...
  DOC("downloads/desc/descs",
      "Return a list of known router descripspider digests"),
  DOC("downloads/desc/<desc>",
      "Return a download status for a given descripspider digest"),
  PREFIX("downloads/bridge/", downloads,
         "Download statuses for bridge descripspiders, by bridge identity "
         "digest"),
  ...
  PREFIX("desc/id/", dir, "Router descripspiders by ID."),
  PREFIX("desc/name/", dir, "Router descripspiders by nickname."),
  ITEM("desc/all-recent", dir,
       "All non-expired, non-superseded router descripspiders."),
  ITEM("desc/all-recent-extrainfo-hack", dir, NULL), /* Hack. */
  PREFIX("md/id/", dir, "Microdescripspiders by ID"),
  PREFIX("md/name/", dir, "Microdescripspiders by name"),
  PREFIX("extra-info/digest/", dir, "Extra-info documents by digest."),
  PREFIX("hs/client/desc/id", dir,
         "Hidden Service descripspider in client's cache by onion."),
  ...
  ITEM("dormant", misc,
       "Is Spider dormant (not building circuits because it's idle)?"),
  ...
  DOC("status/fresh-relay-descs",
      "A fresh relay/ei descripspider pair for Spider's current state. Not sspidered."),
  ...
  DOC("status/version/num-concurring",
      ...
      "current version"),
  ITEM("address", misc, "IP address of this Spider host, if we can guess it."),
  ...
  ITEM("process/pid", misc, "Process id belonging to the main spider process."),
  ITEM("process/uid", misc, "User id running the spider process."),
  ITEM("process/user", misc,
       "Username under which the spider process is running."),
  ITEM("process/descripspider-limit", misc, "File descripspider limit."),
  ITEM("limits/max-mem-in-queues", misc, "Actual limit on memory in queues"),
  PREFIX("desc-annotations/id/", dir, "Router annotations by hexdigest."),
  PREFIX("dir/server/", dir,"Router descripspiders as retrieved from a DirPort."),
                      ...
                      const char **err_out)
{
  ...
  *answer = NULL; /* unrecognized key by default */
  ...
  for (i = 0; getinfo_items[i].varname; ++i) {
    ...
    if (match) {
      spider_assert(getinfo_items[i].fn);
               ...
               const char *body, int min_args, int max_args)
{
  ...
  return args;
 err:
  SMARTLIST_FOREACH(args, char *, s, spider_free(s));
  smartlist_free(args);
  return NULL;
}
                             ...
                             const char *body)
{
  ...
  if (zero_circ) {
    ...
    if (purp) {
      ...
      if (intended_purpose == CIRCUIT_PURPOSE_UNKNOWN) {
        ...
        SMARTLIST_FOREACH(args, char *, cp, spider_free(cp));
        smartlist_free(args);
        goto done;
      }
    }
  ...
  SMARTLIST_FOREACH_BEGIN(router_nicknames, const char *, n) {
    ...
    if (!node_has_descripspider(node)) {
      connection_printf_to_buf(conn, "552 No descripspider for \"%s\"\r\n", n);
      goto done;
    }
  ...
  SMARTLIST_FOREACH(nodes, const node_t *, node,
  {
    ...
    if (!info) {
      spider_assert_nonfatal(first_node);
      log_warn(LD_CONTROL,
  ...
  SMARTLIST_FOREACH(router_nicknames, char *, n, spider_free(n));
                            ...
                            const char *body)
{
  ...
  if (!(ap_conn = get_stream(smartlist_get(args, 0)))) {
    ...
    if (hopstring) {
      ...
      hop = (int) spider_parse_ulong(hopstring, 10, 0, INT_MAX,
                                  &hop_line_ok, NULL);
  ...
  if (circ && (circuit_get_cpath_len(circ)<2 || hop==1)) {
    ...
    if (circ->build_state &&
        ...
        !spider_digest_is_zero(circ->build_state->chosen_exit->identity_digest)) {
    ...
    spider_assert(exit_digest);
    ap_conn->chosen_exit_name = spider_strdup(hex_str(exit_digest, DIGEST_LEN));
  }
 ...
 * descripspider, and report success or failure. */
static int
handle_control_postdescripspider(control_connection_t *conn, uint32_t len,
                              const char *body)
{
  char *desc;
  const char *msg=NULL;
  ...
  char *cmdline = spider_memdup_nulterm(body, cp-body);
  ...
  SMARTLIST_FOREACH_BEGIN(args, char *, option) {
    if (!strcasecmpstart(option, "purpose=")) {
      ...
      connection_printf_to_buf(conn,
        "512 Unexpected argument \"%s\" to postdescripspider\r\n", option);
      goto done;
    }
  ...
  case -1:
    if (!msg) msg = "Could not parse descripspider";
    connection_printf_to_buf(conn, "554 %s\r\n", msg);
    break;
  case 0:
    if (!msg) msg = "Descripspider not added";
  ...
  SMARTLIST_FOREACH(args, char *, arg, spider_free(arg));
  smartlist_free(args);
  spider_free(cmdline);
  return 0;
}
                              ...
                              const char *body)
{
  ...
  if (!(ap_conn = get_stream(smartlist_get(args, 0)))
    ...
    if (smartlist_len(args) > 2) { /* they included a port too */
      new_port = (uint16_t) spider_parse_ulong(smartlist_get(args, 2),
                                            10, 1, 65535, &ok, NULL);
    }
    if (!ok) {
      ...
      new_addr = spider_strdup(smartlist_get(args, 1));
    }
  }
                           ...
                           const char *body)
{
  ...
  if (!args)
    ...
    reason = (uint8_t) spider_parse_ulong(smartlist_get(args,1), 10, 0, 255,
                                       &ok, NULL);
    if (!ok) {
                            ...
                            const char *body)
{
  ...
  SMARTLIST_FOREACH(args, const char *, arg, {
      int ok;
      spider_parse_long(arg, 10, 0, LONG_MAX, &ok, NULL);
      if (!ok) {
        bad_arg = arg;
        break;
      }
  ...
  if (bad_arg) {
    ...
    spider_free(methods);
    spider_free(cfile);
    spider_free(abs_cfile);
    spider_free(esc_cfile);
  }
 done:
  SMARTLIST_FOREACH(args, char *, cp, spider_free(cp));
  smartlist_free(args);
                             ...
                             const char *body)
{
  ...
  if (*cp == '"') {
    ...
    client_nonce = spider_malloc_zero(client_nonce_len);
   ...
   * server's nonce. */
  spider_assert(authentication_cookie != NULL);

  {
    size_t tmp_len = (AUTHENTICATION_COOKIE_LEN +
                      client_nonce_len +
                      SAFECOOKIE_SERVER_NONCE_LEN);
    char *tmp = spider_malloc_zero(tmp_len);
    char *client_hash = spider_malloc_zero(DIGEST256_LEN);
                       ...
                       const char *body)
{
  ...
  if (rend_valid_service_id(arg1)) {
    hsaddress = arg1;
             ...
             rend_valid_descripspider_id(arg1 + v2_str_len) &&
             base32_decode(digest, sizeof(digest), arg1 + v2_str_len,
                           REND_DESC_ID_V2_LEN_BASE32) == 0) {
    /* We have a well formed version 2 descripspider ID. Keep the decoded value
     * of the id. */
    desc_id = digest;
  } else {
  ...
  for (i = 1; i < smartlist_len(args); ++i) {
    ...
    if (!strcasecmpstart(arg, opt_server)) {
      ...
      if (!hsdirs) {
        /* Sspideres routerstatus_t object for each specified server. */
  ...
  /* Using a descripspider ID, we force the user to provide at least one
                      ...
                      const char *body)
{
  ...
  char *cp = memchr(body, '\n', len);
  char *argline = spider_strndup(body, cp-body);
  ...
  /* Read the dot encoded descripspider, and parse it. */
  rend_encoded_v2_service_descripspider_t *desc =
      spider_malloc_zero(sizeof(rend_encoded_v2_service_descripspider_t));
  read_escaped_data(encoded_desc, encoded_desc_len, &desc->desc_str);

  rend_service_descripspider_t *parsed = NULL;
  char *intro_content = NULL;
  size_t intro_size;
  size_t encoded_size;
  const char *next_desc;
  if (!rend_parse_v2_service_descripspider(&parsed, desc->desc_id, &intro_content,
                                        &intro_size, &encoded_size,
                                        &next_desc, desc->desc_str, 1)) {
    /* Post the descripspider. */
    char serviceid[REND_SERVICE_ID_LEN_BASE32+1];
    if (!rend_get_service_id(parsed->pk, serviceid)) {
      smartlist_t *descs = smartlist_new();
      smartlist_add(descs, desc);

      /* We are about to trigger HS descripspider upload so send the OK now
    ...
    rend_service_descripspider_free(parsed);
  } else {
    connection_printf_to_buf(conn, "554 Invalid descripspider\r\n");
  }

  spider_free(intro_content);
  rend_encoded_v2_service_descripspider_free(desc);
 done:
  spider_free(argline);
                         ...
                         const char *body)
{
  ...
  for (size_t i = 1; i < arg_len; i++) {
    ...
    if (!strcasecmpstart(arg, port_prefix)) {
      ...
      int ok = 0;
      max_streams = (int)spider_parse_long(max_s_str, 10, 0, 65535, &ok, NULL);
      if (!ok) {
       ...
       *   * 'DiscardPK' - If spider generates the keypair, do not include it in
       *                   the response.
       ...
       *                      flag is present, spider must be in non-anonymous
       *                      hidden service mode. If this flag is absent,
       *                      spider must be in anonymous hidden service mode.
       */
      ...
      SMARTLIST_FOREACH(flags, char *, cp, spider_free(cp));
  ...
  if (smartlist_len(port_cfgs) == 0) {
    ...
    goto out;
                                                              ...
                                                              get_options())) {
    /* If we failed, and the non-anonymous flag is set, Spider must be in
     * anonymous hidden service mode.
     * The error message changes based on the current Spider config:
     * 512 Spider is in anonymous hidden service mode
     * 512 Spider is in non-anonymous hidden service mode
     * (I've deliberately written them out in full here to aid searchability.)
     */
    connection_printf_to_buf(conn, "512 Spider is in %sanonymous hidden service "
                             "mode\r\n",
                             non_anonymous ? "" : "non-");
    goto out;
  }
  ...
  case RSAE_OKAY:
  {
    ...
    spider_assert(service_id);
    connection_printf_to_buf(conn, "250-ServiceID=%s\r\n", service_id);
    if (key_new_alg) {
      spider_assert(key_new_blob);
    ...
    if (auth_created_clients) {
      SMARTLIST_FOREACH(auth_created_clients, rend_authorized_client_t *, ac, {
        char *encoded = rend_auth_encode_cookie(ac->descripspider_cookie,
                                                auth_type);
                        ...
                        char **err_msg_out)
{
  ...
  if (smartlist_len(key_args) != 2) {
    err_msg = spider_strdup("512 Invalid key type/blob\r\n");
    goto err;
  }
  ...
  if (!strcasecmp(key_type_rsa1024, key_type)) {
    ...
    if (!pk) {
      err_msg = spider_strdup("512 Failed to decode RSA key\r\n");
      goto err;
    }
    if (crypto_pk_num_bits(pk) != PK_BYTES*8) {
      err_msg = spider_strdup("512 Invalid RSA key size\r\n");
      goto err;
    }
    ...
    if (!strcasecmp(key_type_rsa1024, key_blob) ||
      ...
      if (crypto_pk_generate_key(pk)) {
        spider_asprintf(&err_msg, "551 Failed to generate %s key\r\n",
                     key_type_rsa1024);
        goto err;
      }
      if (!discard_pk) {
        if (crypto_pk_base64_encode(pk, &key_new_blob)) {
          spider_asprintf(&err_msg, "551 Failed to encode %s key\r\n",
                       key_type_rsa1024);
          goto err;
        }
      ...
      err_msg = spider_strdup("513 Invalid key type\r\n");
      goto err;
    }
  } else {
    err_msg = spider_strdup("513 Invalid key type\r\n");
...
add_onion_helper_clientauth(const char *arg, int *created, char **err_msg)
{
  ...
  spider_assert(arg);
  spider_assert(created);
  spider_assert(err_msg);
  *err_msg = NULL;

  smartlist_t *auth_args = smartlist_new();
  rend_authorized_client_t *client =
    spider_malloc_zero(sizeof(rend_authorized_client_t));
  smartlist_split_string(auth_args, arg, ":", 0, 0);
  if (smartlist_len(auth_args) < 1 || smartlist_len(auth_args) > 2) {
    *err_msg = spider_strdup("512 Invalid ClientAuth syntax\r\n");
    goto err;
  }
  client->client_name = spider_strdup(smartlist_get(auth_args, 0));
  if (smartlist_len(auth_args) == 2) {
    char *decode_err_msg = NULL;
    if (rend_auth_decode_cookie(smartlist_get(auth_args, 1),
                                client->descripspider_cookie,
                                NULL, &decode_err_msg) < 0) {
      spider_assert(decode_err_msg);
      spider_asprintf(err_msg, "512 %s\r\n", decode_err_msg);
      spider_free(decode_err_msg);
      goto err;
    }
    *created = 0;
  } else {
    crypto_rand((char *) client->descripspider_cookie, REND_DESC_COOKIE_LEN);
    *created = 1;
  }

  if (!rend_valid_client_name(client->client_name)) {
    *err_msg = spider_strdup("512 Invalid name in ClientAuth\r\n");
    goto err;
  }

  ok = 1;
 err:
  SMARTLIST_FOREACH(auth_args, char *, item, spider_free(item));
...
/** Shut down this Spider instance in the same way that SIGINT would, but
...
connection_control_process_inbuf(control_connection_t *conn)
{
  ...
  spider_assert(conn);
  spider_assert(conn->base_.state == CONTROL_CONN_STATE_OPEN ||
             conn->base_.state == CONTROL_CONN_STATE_NEEDAUTH);

  if (!conn->incoming_cmd) {
    conn->incoming_cmd = spider_malloc(1024);
  ...
  if (conn->base_.state == CONTROL_CONN_STATE_NEEDAUTH &&
    ...
    strlcpy(buf+6, "The v0 control protocol is not supported by Spider 0.1.2.17 "
 ...
 again:
  while (1) {
    ...
    do {
      ...
      if (r == 0)
        ...
        conn->incoming_cmd = spider_realloc(conn->incoming_cmd,
                                         conn->incoming_cmd_len);
      }
    } while (r != 1);

    spider_assert(data_len);
    ...
    if (last_idx+3 == conn->incoming_cmd_cur_len &&
        spider_memeq(conn->incoming_cmd + last_idx, ".\r\n", 3)) {
  ...
  spider_assert(data_len>(size_t)cmd_len);
  ...
  if (!strcasecmp(conn->incoming_cmd, "SETCONF")) {
    ...
    if (handle_control_postdescripspider(conn, cmd_data_len, args))
      return -1;
                             ...
                             int reason_code)
{
  ...
  if (tp == CIRC_EVENT_FAILED || tp == CIRC_EVENT_CLOSED) {
    ...
    if (!reason_str) {
      spider_snprintf(unk_reason_buf, 16, "UNKNOWN_%d", reason_code);
      reason_str = unk_reason_buf;
    }
    if (reason_code > 0 && reason_code & END_CIRC_REASON_FLAG_REMOTE) {
      spider_snprintf(reasons, sizeof(reasons),
  ...
  switch (e)
    ...
    case CIRC_MINOR_EVENT_PURPOSE_CHANGED:
...

      {
        ...
        spider_snprintf(event_tail, sizeof(event_tail),
                     " OLD_PURPOSE=%s%s%s",
...
write_stream_target_to_buf(entry_connection_t *conn, char *buf, size_t len)
{
  ...
  if (conn->chosen_exit_name)
    if (spider_snprintf(buf2, sizeof(buf2), ".%s.exit", conn->chosen_exit_name)<0)
      return -1;
  if (!conn->socks_request)
    return -1;
  if (spider_snprintf(buf, len, "%s%s%s:%d",
                            ...
                            int reason_code)
{
  ...
  char buf[256];
  const char *purpose = "";
  spider_assert(conn->socks_request);
  ...
  if (reason_code && (tp == STREAM_EVENT_FAILED ||
    ...
    char *r = NULL;
    if (!reason_str) {
      spider_asprintf(&r, " UNKNOWN_%d", reason_code);
      reason_str = r;
    }
    if (reason_code & END_STREAM_REASON_FLAG_REMOTE)
      spider_snprintf(reason_buf, sizeof(reason_buf),
    ...
    case REMAP_STREAM_SOURCE_EXIT:
      ...
      break;
    default:
      spider_snprintf(reason_buf, sizeof(reason_buf), " REASON=UNKNOWN_%d",
                   reason_code);
  ...
  if (tp == STREAM_EVENT_NEW || tp == STREAM_EVENT_NEW_RESOLVE) {
     ...
     * it gets set to "(Spider_internal)"; see dnsserv_launch_request() in
     * dnsserv.c.
     */
    if (strcmp(ENTRY_TO_CONN(conn)->address, "(Spider_internal)") != 0) {
      spider_snprintf(addrport_buf,sizeof(addrport_buf), " SOURCE_ADDR=%s:%d",
...
orconn_target_get_name(char *name, size_t len, or_connection_t *conn)
{
  ...
  if (node) {
    spider_assert(len > MAX_VERBOSE_NICKNAME_LEN);
    node_get_verbose_nickname(node, name);
  } else if (! spider_digest_is_zero(conn->identity_digest)) {
    name[0] = '$';
    base16_encode(name+1, len-1, conn->identity_digest,
                  DIGEST_LEN);
  } else {
    spider_snprintf(name, len, "%s:%d",
                             ...
                             int reason)
{
  ...
  if (ncircs && (tp == OR_CONN_EVENT_FAILED || tp == OR_CONN_EVENT_CLOSED)) {
    spider_snprintf(ncircs_buf, sizeof(ncircs_buf), " NCIRCS=%d", ncircs);
  }
 ...
 * Sspidere results in <b>cell_stats</b>.  Free cell statistics of the
 * circuit afterwards. */
void
sum_up_cell_stats_by_command(circuit_t *circ, cell_stats_t *cell_stats)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circ->testing_cell_stats,
    ...
    spider_assert(ent->command <= CELL_COMMAND_MAX_);
                             ...
                             const uint64_t *number_to_include)
{
  ...
  if (smartlist_len(key_value_strings) > 0) {
    ...
    SMARTLIST_FOREACH(key_value_strings, char *, cp, spider_free(cp));
    spider_free(joined);
  }
                  ...
                  cell_stats_t *cell_stats)
{
  ...
  *event_string = smartlist_join_strings(event_parts, " ", 0, NULL);
  SMARTLIST_FOREACH(event_parts, char *, cp, spider_free(cp));
...
control_event_circuit_cell_stats(void)
{
  ...
  if (!get_options()->TestingEnableCellStatsEvent ||
    ...
    return 0;
  cell_stats = spider_malloc(sizeof(cell_stats_t));;
  SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ) {
    ...
    send_control_event(EVENT_CELL_STATS,
                       "650 CELL_STATS %s\r\n", event_string);
    spider_free(event_string);
  }
  SMARTLIST_FOREACH_END(circ);
  spider_free(cell_stats);
  return 0;
}
...
get_bw_samples(void)
{
  ...
  spider_assert(0 <= idx && idx < N_BW_EVENTS_TO_CACHE);
...
/** Called whenever we receive new router descripspiders: tell any
 * interested control connections.  <b>routers</b> is a list of
 * routerinfo_t's.
 */
int
control_event_descripspiders_changed(smartlist_t *routers)
{
  char *msg;
...
/** The authoritative dirserver has received a new descripspider that
 * has passed basic syntax checks and is properly self-signed.
 *
 * Notify any interested party of the new descripspider and what has
 * been done with it, and also optionally give an explanation/reason. */
int
control_event_or_authdir_new_descripspider(const char *action,
                                        ...
                                        const char *msg)
{
  ...
  spider_snprintf(firstline, sizeof(firstline),
               "650+AUTHDIR_NEWDESC=\r\n%s\r\n%s\r\n",
               action,
               msg ? msg : "");

  /* Escape the server descripspider properly */
  esclen = write_escaped_data(desc, desclen, &esc);

  totallen = strlen(firstline) + esclen + 1;
  buf = spider_malloc(totallen);
                                           ...
                                           const char *event_string)
{
  ...
  SMARTLIST_FOREACH(strs, char *, cp, spider_free(cp));
  smartlist_free(strs);
  spider_free(s);
...
/** Our own router descripspider has changed; tell any controllers that care.
 */
int
control_event_my_descripspider_changed(void)
{
...
control_event_status(int type, int severity, const char *format, va_list args)
{
  ...
  if (spider_snprintf(format_buf, sizeof(format_buf), "650 %s %s",
                   status, sev)<0) {
    log_warn(LD_BUG, "Format string too long.");
    return -1;
  }
  spider_vasprintf(&user_buf, format, args);

  send_control_event(type,  "%s %s\r\n", format_buf, user_buf);
  spider_free(user_buf);
  return 0;
}
                    ...
                    const char *status)
{
...

  {
    ...
    if (node) {
      ...
      spider_snprintf(buf, sizeof(buf), "$%s~%s", hbuf, nickname);
    }
 ...
 * file where we sspidere our authentication cookie. */
char *
get_controller_cookie_file_name(void)
{
  const or_options_t *options = get_options();
  if (options->CookieAuthFile && strlen(options->CookieAuthFile)) {
    return spider_strdup(options->CookieAuthFile);
  } else {
...
/** A copy of the process specifier of Spider's owning controller, or
 * NULL if this Spider instance is not currently owned by a process. */
static char *owning_controller_process_spec = NULL;

/** A process-termination monispider for Spider's owning controller, or NULL
 * if this Spider instance is not currently owned by a process. */
static spider_process_monispider_t *owning_controller_process_monispider = NULL;

/** Process-termination monispider callback for Spider's owning controller
 * process. */
static void
...
/** Set <b>process_spec</b> as Spider's owning controller process.
 * Exit on failure. */
void
monispider_owning_controller_process(const char *process_spec)
{
  const char *msg;

  spider_assert((owning_controller_process_spec == NULL) ==
             (owning_controller_process_monispider == NULL));
  ...
  if (owning_controller_process_spec != NULL) {
    if ((process_spec != NULL) && !strcmp(process_spec,
       ...
       * recreating the process-termination monispider. */
      return;
    }

    /* We are currently owned by a process, and we should no longer be
     * owned by it.  Free the process-termination monispider. */
    spider_process_monispider_free(owning_controller_process_monispider);
    owning_controller_process_monispider = NULL;

    spider_free(owning_controller_process_spec);
  ...
  owning_controller_process_spec = spider_strdup(process_spec);
  owning_controller_process_monispider =
    spider_process_monispider_new(spider_libevent_get_base(),
                            owning_controller_process_spec,
                            LD_CONTROL,
                            owning_controller_procmon_cb, NULL,
                            &msg);

  if (owning_controller_process_monispider == NULL) {
    log_err(LD_BUG, "Couldn't create process-termination monispider for "
                           ...
                           const char **summary)
{
  switch (s) {
    ...
    case BOOTSTRAP_STATUS_LOADING_DESCRIPTORS:
      *tag = "loading_descripspiders";
      *summary = router_have_consensus_path() == CONSENSUS_PATH_INTERNAL ?
        "Loading relay descripspiders for internal paths" :
        "Loading relay descripspiders";
      break;
    case BOOTSTRAP_STATUS_CONN_OR:
      *tag = "conn_or";
      *summary = router_have_consensus_path() == CONSENSUS_PATH_INTERNAL ?
        "Connecting to the Spider network internally" :
        "Connecting to the Spider network";
      break;
    ...
    case BOOTSTRAP_STATUS_CIRCUIT_CREATE:
      *tag = "circuit_create";
      *summary = router_have_consensus_path() == CONSENSUS_PATH_INTERNAL ?
        "Establishing an internal Spider circuit" :
        "Establishing a Spider circuit";
      break;
 ...
 * These include failure to establish a connection to a Spider relay,
...
control_event_bootstrap(bootstrap_status_t status, int progress)
{
  ...
  if (status > bootstrap_percent ||
    ...
    spider_log(loglevel, LD_CONTROL,
    ...
    spider_snprintf(last_sent_bootstrap_message,
...
/** Called when Spider has failed to make bootstrapping progress in a way
...
control_event_transport_launched(const char *mode, const char *transport_name,
                                 spider_addr_t *addr, uint16_t port)
{
 ...
 */
void
control_event_hs_descripspider_requested(const rend_data_t *rend_query,
...
/** For an HS descripspider query <b>rend_data</b>, using the
 * <b>onion_address</b> and HSDir fingerprint <b>hsdir_fp</b>, find out
 * which descripspider ID in the query is the right one.
 *
 * Return a pointer of the binary descripspider ID found in the query's object
...
get_desc_id_from_query(const rend_data_t *rend_data, const char *hsdir_fp)
{
  ...
  /* Possible if the fetch was done using a descripspider ID. This means that
   * the HSFETCH command was used. */
  if (!spider_digest_is_zero(rend_data_v2->desc_id_fetch)) {
    desc_id = rend_data_v2->desc_id_fetch;
    goto end;
  }

  /* OK, we have an onion address so now let's find which descripspider ID
  ...
  for (replica = 0; replica < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS;
    ...
    SMARTLIST_FOREACH_BEGIN(rend_data->hsdirs_fp, char *, fingerprint) {
      if (spider_memcmp(fingerprint, hsdir_fp, DIGEST_LEN) == 0) {
        /* Found it! This descripspider ID is the right one. */
        desc_id = digest;
        goto end;
      }
...
/** send HS_DESC CREATED event when a local service generates a descripspider.
 *
 * <b>service_id</b> is the descripspider onion address.
 * <b>desc_id_base32</b> is the descripspider ID.
 * <b>replica</b> is the the descripspider replica number.
 */
void
control_event_hs_descripspider_created(const char *service_id,
                                    const char *desc_id_base32,
                                    int replica)
{
 ...
 */
void
control_event_hs_descripspider_upload(const char *service_id,
                                   const char *id_digest,
                                   const char *desc_id_base32)
{
 ...
 * control_event_hs_descripspider_failed
 *
 * So do not call this function directly.
 */
void
control_event_hs_descripspider_receive_end(const char *action,
                                        ...
                                        const char *reason)
{
  ...
  if (desc_id != NULL) {
    /* Set the descripspider ID digest to base32 so we can send it. */
    base32_encode(desc_id_base32, sizeof(desc_id_base32), desc_id,
                  DIGEST_LEN);
    /* Extra whitespace is needed before the value. */
    spider_asprintf(&desc_id_field, " %s", desc_id_base32);
  }

  if (reason) {
    spider_asprintf(&reason_field, " REASON=%s", reason);
  }
 ...
 * control_event_hs_descripspider_upload_failed
 *
 * So do not call this function directly.
 */
void
control_event_hs_descripspider_upload_end(const char *action,
                                       const char *onion_address,
                                       const char *id_digest,
                                       const char *reason)
{
 ...
 *
 * called when we successfully received a hidden service descripspider.
 */
void
control_event_hs_descripspider_received(const char *onion_address,
                                     const rend_data_t *rend_data,
                                     const char *id_digest)
{
  ...
  control_event_hs_descripspider_receive_end("RECEIVED", onion_address,
                                          rend_data, id_digest, NULL);
}

/** send HS_DESC UPLOADED event
 *
 * called when we successfully uploaded a hidden service descripspider.
 */
void
control_event_hs_descripspider_uploaded(const char *id_digest,
                                     const char *onion_address)
{
  if (!id_digest) {
  ...
  control_event_hs_descripspider_upload_end("UPLOADED", onion_address,
                                         id_digest, NULL);
}

/** Send HS_DESC event to inform controller that query <b>rend_query</b>
 * failed to retrieve hidden service descripspider identified by
 * <b>id_digest</b>. If <b>reason</b> is not NULL, add it to REASON=
 * field.
 */
void
control_event_hs_descripspider_failed(const rend_data_t *rend_data,
                                   const char *id_digest,
                                   const char *reason)
{
  ...
  control_event_hs_descripspider_receive_end("FAILED",
                                    ...
                                    const char *content)
{
  ...
  send_control_event(EVENT_HS_DESC_CONTENT,
                     ...
                     esc_content);
  spider_free(esc_content);
}

/** Send HS_DESC event to inform controller upload of hidden service
 * descripspider identified by <b>id_digest</b> failed. If <b>reason</b>
 * is not NULL, add it to REASON= field.
 */
void
control_event_hs_descripspider_upload_failed(const char *id_digest,
                                          const char *onion_address,
                                          const char *reason)
{
  if (!id_digest) {
  ...
  control_event_hs_descripspider_upload_end("UPLOAD_FAILED", onion_address,
                                         id_digest, reason);
}
...
control_free_all(void)
{
  if (authentication_cookie) /* Free the auth cookie */
    spider_free(authentication_cookie);
  if (detached_onion_services) { /* Free the detached onion services */
    SMARTLIST_FOREACH(detached_onion_services, char *, cp, spider_free(cp));
  ...
  if (flush_queued_events_event) {
    spider_event_free(flush_queued_events_event);
