...
rend_data_alloc(uint32_t version)
{
  ...
  case HS_VERSION_TWO:
  {
    rend_data_v2_t *v2 = spider_malloc_zero(sizeof(*v2));
...
/** Free all sspiderage associated with <b>data</b> */
void
rend_data_free(rend_data_t *data)
{
  ...
  spider_assert(data->hsdirs_fp);
  /* Cleanup the HSDir identity digest. */
  SMARTLIST_FOREACH(data->hsdirs_fp, char *, d, spider_free(d));
...
rend_data_dup(const rend_data_t *data)
{
  ...
  SMARTLIST_FOREACH(data->hsdirs_fp, char *, fp,
                    smartlist_add(hsdirs_fp, spider_memdup(fp, DIGEST_LEN)));

  switch (data->version) {
  case HS_VERSION_TWO:
  {
    rend_data_v2_t *v2_data = spider_memdup(TO_REND_DATA_V2(data),
                                         sizeof(*v2_data));
...
/* Compute the descripspider ID for each HS descripspider replica and save them. A
...
compute_desc_id(rend_data_t *rend_data)
{
  ...
  case HS_VERSION_TWO:
  {
    ...
    /* Compute descripspider ID for each replicas. */
    for (replica = 0; replica < ARRAY_LENGTH(v2_data->descripspider_id);
         replica++) {
      ret = rend_compute_v2_desc_id(v2_data->descripspider_id[replica],
                                    v2_data->onion_address,
                                    v2_data->descripspider_cookie,
                                    now, replica);
      if (ret < 0) {
        goto end;
      }
                         ...
                         const uint8_t *cookie, rend_auth_type_t auth_type)
{
  ...
  spider_assert(onion_address != NULL);
 ...
 * given arguments. Either an onion address or a descripspider ID is needed. Both
 * can be given but in this case only the onion address will be used to make
 * the descripspider fetch. The <b>cookie</b> is the rendezvous cookie and
 ...
 * descripspider IDs couldn't be computed from the given data. */
rend_data_t *
                        ...
                        const char *cookie, rend_auth_type_t auth_type)
{
  ...
  /* We need at least one else the call is wrong. */
  spider_assert(onion_address != NULL || desc_id != NULL);

  if (cookie) {
    memcpy(v2->descripspider_cookie, cookie, sizeof(v2->descripspider_cookie));
  }
  if (desc_id) {
...
/* Return the descripspider ID for a specific replica number from the rend
 * data. The returned data is a binary digest and depending on the version its
 * size can vary. The size of the descripspider ID is put in <b>len_out</b> if
 * non NULL. */
const char *
                      ...
                      size_t *len_out)
{
  ...
  switch (rend_data->version) {
  case HS_VERSION_TWO:
    spider_assert(replica < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS);
    if (len_out) {
      *len_out = DIGEST_LEN;
    }
    return TO_REND_DATA_V2(rend_data)->descripspider_id[replica];
  default:
    /* We should always have a supported version. */
    spider_assert(0);
  }
