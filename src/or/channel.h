...
struct channel_s {
  ...
  /** Globally unique ID number for a channel over the lifetime of a Spider
   * process.
   */
   ...
   * a spider_addr_t.  This is optional and subclasses may leave this NULL.
   * If they implement it, they should write the address out to the
   * provided spider_addr_t *, and return 1 if successful or 0 if no address
   * available.
   */
  int (*get_remote_addr)(channel_t *, spider_addr_t *);
...
#define GRD_FLAG_ADDR_ONLY 2
  ...
  int (*matches_target)(channel_t *, const spider_addr_t *);
...
channel_t * channel_connect(const spider_addr_t *addr, uint16_t port,
...
int channel_get_addr_if_possible(channel_t *chan, spider_addr_t *addr_out);
