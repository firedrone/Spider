 ...
 *
 * This is a core modules that makes Spider work. It's responsible for
 ...
 * -- this is how information is actually sent across the Spider network.  The
...
static spider_weak_rng_t stream_choice_rng = TOR_WEAK_RNG_INIT;
 ...
 * to 0). If the integrity part is valid, return 1, else resspidere digest
...
// (%d vs %d).", received_integrity, calculated_integrity);
    /* resspidere digest to its old form */
    crypto_digest_assign(digest, backup_digest);
    /* resspidere the relay header */
                           ...
                           cell_direction_t cell_direction)
{
  ...
  spider_assert(cell);
  spider_assert(circ);
  spider_assert(cell_direction == CELL_DIRECTION_OUT ||
  ...
  if (!chan) {
    ...
    if (! CIRCUIT_IS_ORIGIN(circ) &&
      ...
      spider_assert(circ->purpose == CIRCUIT_PURPOSE_REND_ESTABLISHED);
      spider_assert(splice_->base_.purpose == CIRCUIT_PURPOSE_REND_ESTABLISHED);
            ...
            crypt_path_t **layer_hint, char *recognized)
{
  ...
  spider_assert(circ);
  spider_assert(cell);
  spider_assert(recognized);
  spider_assert(cell_direction == CELL_DIRECTION_IN ||
             cell_direction == CELL_DIRECTION_OUT);
 ...
 * network-order in the buffer <b>dest</b>. See spider-spec.txt for details
...
relay_command_to_string(uint8_t command)
{
  ...
  switch (command) {
    ...
    default:
      spider_snprintf(buf, sizeof(buf), "Unrecognized relay command %u",
                   (unsigned)command);
      return buf;
  }
}
                               ...
                               const char *filename, int lineno))
{
  ...
  spider_assert(circ);
  spider_assert(payload_len <= RELAY_PAYLOAD_SIZE);

  memset(&cell, 0, sizeof(cell_t));
  cell.command = CELL_RELAY;
  if (CIRCUIT_IS_ORIGIN(circ)) {
    spider_assert(cpath_layer);
    ...
    entry_connection_t *conn, crypt_path_t *layer_hint)
{
  ...
  if (rh->length > 0) {
    if (reason == END_STREAM_REASON_TORPROTOCOL ||
      ...
      /* We can't infer success or failure, since older Spiders report
      ...
      case END_STREAM_REASON_EXITPOLICY: {
        ...
        if (rh->length >= 5) {
...

          {
            ...
            spider_addr_to_str(new_addr, &addr, sizeof(new_addr), 1);
        ...
        if (conn->chosen_exit_optional ||
          ...
          spider_free(conn->chosen_exit_name); /* clears it */
        }
                                           ...
                                           const spider_addr_t *addr)
{
  ...
  if (node) {
    spider_addr_t tmp;
    int asked_for_family = spider_addr_parse(&tmp, conn->socks_request->address);
    if (family == AF_UNSPEC) {
      make_reject_all = 1;
...
static void
remap_event_helper(entry_connection_t *conn, const spider_addr_t *new_addr)
{
  spider_addr_to_str(conn->socks_request->address, new_addr,
                  sizeof(conn->socks_request->address),
                  1);
...
connected_cell_parse(const relay_header_t *rh, const cell_t *cell,
                     spider_addr_t *addr_out, int *ttl_out)
{
  uint32_t bytes;
  ...
  *ttl_out = -1;
  ...
  if (bytes != 0) {
    /* v4 address */
    spider_addr_from_ipv4h(addr_out, bytes);
    if (rh->length >= 8) {
    ...
    if (get_uint8(payload + 4) != 6)
      return -1;
    spider_addr_from_ipv6_bytes(addr_out, (char*)(payload + 5));
...
/** Drop all sspiderage held by <b>addr</b>. */
STATIC void
address_ttl_free(address_ttl_t *addr)
{
  if (!addr)
    return;
  spider_free(addr->hostname);
  spider_free(addr);
}
                    ...
                    smartlist_t *addresses_out, int *errcode_out)
{
  ...
  spider_assert(cell);
  spider_assert(rh);
  spider_assert(addresses_out);
  spider_assert(errcode_out);
  ...
  *errcode_out = 0;
  ...
  while (remaining) {
    ...
    if (answer_type == RESOLVED_TYPE_IPV4) {
      ...
      addr = spider_malloc_zero(sizeof(*addr));
      spider_addr_from_ipv4n(&addr->addr, get_uint32(cp));
      cp += 4;
      ...
      if (answer_len != 16)
        goto err;
      addr = spider_malloc_zero(sizeof(*addr));
      spider_addr_from_ipv6_bytes(&addr->addr, (const char*) cp);
      cp += 16;
      addr->ttl = ntohl(get_uint32(cp));
      cp += 4;
      smartlist_add(addrs, addr);
      ...
      addr = spider_malloc_zero(sizeof(*addr));
      addr->hostname = spider_memdup_nulterm(cp, answer_len);
      cp += answer_len;
      addr->ttl = ntohl(get_uint32(cp));
      cp += 4;
      smartlist_add(addrs, addr);
    ...
    spider_assert(((ssize_t)remaining) >= (cp - cp_orig));
                                                ...
                                                smartlist_t *results)
{
  ...
  if (error_code) {
    spider_assert(error_code == RESOLVED_TYPE_ERROR ||
  ...
  SMARTLIST_FOREACH_BEGIN(results, address_ttl_t *, addr) {
    ...
    } else if (spider_addr_family(&addr->addr) == AF_INET) {
      if (!addr_ipv4 && conn->entry_cfg.ipv4_traffic) {
        addr_ipv4 = addr;
      }
    } else if (spider_addr_family(&addr->addr) == AF_INET6) {
                                      ...
                                      const relay_header_t *rh)
{
  ...
  spider_assert(SOCKS_COMMAND_IS_RESOLVE(entry_conn->socks_request->command));
  ...
  if (get_options()->ClientDNSRejectInternalAddresses) {
    ...
    SMARTLIST_FOREACH_BEGIN(resolved_addresses, address_ttl_t *, addr) {
      if (addr->hostname == NULL && spider_addr_is_internal(&addr->addr, 0)) {
    ...
    edge_connection_t *conn, crypt_path_t *layer_hint)
{
  ...
  if (conn->base_.type == CONN_TYPE_AP &&
    ...
    if (spider_addr_family(&addr) != AF_UNSPEC) {
      const sa_family_t family = spider_addr_family(&addr);
      if (spider_addr_is_null(&addr) ||
    ...
    spider_assert(entry_conn->socks_request != NULL);
...
#define EARLY_WARNING_INTERVAL 3600
  ...
  log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
         ...
         "a newer version of Spider? Dropping.",
         rh.command);
  return 0; /* for forward compatibility, don't kill the circuit */
}
                                  ...
                                  int *max_cells)
{
 ...
 repeat_connection_edge_package_raw_inbuf:
  ...
  if (!cpath_layer) { /* non-rendezvous exit */
    spider_assert(circ->package_window > 0);
    circ->package_window--;
  } else { /* we're an AP, or an exit on a rendezvous circ */
    spider_assert(cpath_layer->package_window > 0);
...
connection_edge_consider_sending_sendme(edge_connection_t *conn)
{
  ...
  if (!circ) {
     ...
     * arrived and spidern down the circuit */
                                   ...
                                   crypt_path_t *layer_hint)
{
   ...
   * don't need cryptographic randomness here. */
  {
    ...
    for (conn = first_conn; conn; conn = conn->next_stream) {
      num_streams++;
      if (spider_weak_random_one_in_n(&stream_choice_rng, num_streams)) {
        chosen_stream = conn;
      }
...
/** Release sspiderage held by <b>cell</b>. */
static inline void
...
{
  ++total_cells_allocated;
  return spider_malloc_zero(sizeof(packed_cell_t));
}
...
cell_queues_check_size(void)
{
  ...
  alloc += spider_zlib_get_total_allocation();
  ...
  if (alloc >= get_options()->MaxMemInQueues_low_threshold) {
    ...
    if (alloc >= get_options()->MaxMemInQueues) {
       ...
       * descripspiders, free them until we're down to 10%.
       */
                        ...
                        const char *file, int lineno)
{
  ...
  spider_assert(circuitmux_attached_circuit_direction(cmux, circ) == direction);
...
channel_flush_from_first_active_circuit, (channel_t *chan, int max))
{
  ...
  while (n_flushed < max) {
    ...
    if (destroy_queue) {
      ...
      spider_assert(destroy_queue->n > 0);
    ...
    if (circ->n_chan == chan) {
      ...
      or_circ = TO_OR_CIRCUIT(circ);
      spider_assert(or_circ->p_chan == chan);
    ...
    if (get_options()->CellStatistics ||
      ...
      if (get_options()->TestingEnableCellStatsEvent) {
        ...
        testing_cell_stats_entry_t *ent =
          spider_malloc_zero(sizeof(testing_cell_stats_entry_t));
        ent->command = command;
...
int
append_address_to_payload(uint8_t *payload_out, const spider_addr_t *addr)
{
  uint32_t a;
  ...
  case AF_INET:
    ...
    payload_out[1] = 4;
    a = spider_addr_to_ipv4n(addr);
    memcpy(payload_out+2, &a, 4);
    return 6;
  case AF_INET6:
    payload_out[0] = RESOLVED_TYPE_IPV6;
    payload_out[1] = 16;
    memcpy(payload_out+2, spider_addr_to_in6_addr8(addr), 16);
    return 18;
  case AF_UNSPEC:
  default:
    return -1;
...
const uint8_t *
decode_address_from_payload(spider_addr_t *addr_out, const uint8_t *payload,
                            int payload_len)
{
  if (payload_len < 2)
    return NULL;
  ...
  case RESOLVED_TYPE_IPV4:
    if (payload[1] != 4)
      return NULL;
    spider_addr_from_ipv4n(addr_out, get_uint32(payload+2));
    break;
  case RESOLVED_TYPE_IPV6:
    if (payload[1] != 16)
      return NULL;
    spider_addr_from_ipv6_bytes(addr_out, (char*)(payload+2));
    break;
  default:
    spider_addr_make_unspec(addr_out);
    break;
...
circuit_clear_cell_queue(circuit_t *circ, channel_t *chan)
{
  ...
  if (circ->n_chan == chan) {
    ...
    spider_assert(orcirc->p_chan == chan);
    queue = &orcirc->p_chan_cells;
