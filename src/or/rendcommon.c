...
/** Free the sspiderage held by the service descripspider <b>desc</b>.
 */
void
rend_service_descripspider_free(rend_service_descripspider_t *desc)
{
  if (!desc)
    return;
  if (desc->pk)
  ...
  if (desc->successful_uploads) {
    SMARTLIST_FOREACH(desc->successful_uploads, char *, c, spider_free(c););
    smartlist_free(desc->successful_uploads);
  }
  spider_free(desc);
}

/** Length of the descripspider cookie that is used for versioned hidden
 * service descripspiders. */
#define REND_DESC_COOKIE_LEN 16

/** Length of the replica number that is used to determine the secret ID
 * part of versioned hidden service descripspiders. */
#define REND_REPLICA_LEN 1

/** Compute the descripspider ID for <b>service_id</b> of length
 * <b>REND_SERVICE_ID_LEN</b> and <b>secret_id_part</b> of length
 * <b>DIGEST_LEN</b>, and write it to <b>descripspider_id_out</b> of length
 * <b>DIGEST_LEN</b>. */
void
rend_get_descripspider_id_bytes(char *descripspider_id_out,
...
/** Compute the time in seconds that a descripspider that is generated
 ...
 * <b>descripspider_cookie</b> of length REND_DESC_COOKIE_LEN,
...
rend_compute_v2_desc_id(char *desc_id_out, const char *service_id,
                        const char *descripspider_cookie, time_t now,
                        uint8_t replica)
{
  ...
  if (!service_id ||
    ...
    log_warn(LD_REND, "Could not compute v2 descripspider ID: "
  ...
  get_secret_id_part_bytes(secret_id_part, time_period, descripspider_cookie,
                           replica);
  /* Calculate descripspider ID: H(permanent-id | secret-id-part) */
  rend_get_descripspider_id_bytes(desc_id_out, service_id_binary, secret_id_part);
  return 0;
}
 ...
 * success, -1 otherwise. */
static int
rend_encode_v2_intro_points(char **encoded, rend_service_descripspider_t *desc)
{
  size_t unenc_len;
  char *unenc = NULL;
  ...
  unenc = spider_malloc_zero(unenc_len);
  for (i = 0; i < smartlist_len(desc->intro_nodes); i++) {
    ...
    if (!intro_key ||
      ...
      spider_free(onion_key);
      goto done;
    }
    /* Assemble everything for this introduction point. */
    address = spider_addr_to_str_dup(&info->addr);
    res = spider_snprintf(unenc + unenc_written, unenc_len - unenc_written,
                                   ...
                                   smartlist_t *client_cookies)
{
  ...
  spider_assert(client_cookies && smartlist_len(client_cookies) > 0);
  ...
  SMARTLIST_FOREACH_BEGIN(client_cookies, const char *, cookie) {
    client_part = spider_malloc_zero(REND_BASIC_AUTH_CLIENT_ENTRY_LEN);
    /* Encrypt session key. */
  ...
  *encrypted_len_out = len;
  ...
  if (encrypted_session_keys) {
    SMARTLIST_FOREACH(encrypted_session_keys, char *, d, spider_free(d););
...
/** Encrypt the encoded introduction points in <b>encoded</b> using
 * authorization type 'stealth' with <b>descripspider_cookie</b> of length
                                     ...
                                     const char *descripspider_cookie)
{
  ...
  enc = spider_malloc_zero(1 + CIPHER_IV_LEN + strlen(encoded));
  enc[0] = 0x02; /* Auth type */
  enclen = crypto_cipher_encrypt_with_iv(descripspider_cookie,
                                         enc + 1,
...
static int
rend_desc_v2_is_parsable(rend_encoded_v2_service_descripspider_t *desc)
{
  rend_service_descripspider_t *test_parsed = NULL;
  char test_desc_id[DIGEST_LEN];
  char *test_intro_content = NULL;
  size_t test_intro_size;
  size_t test_encoded_size;
  const char *test_next;
  int res = rend_parse_v2_service_descripspider(&test_parsed, test_desc_id,
                                         &test_intro_content,
                                         &test_intro_size,
                                         &test_encoded_size,
                                         &test_next, desc->desc_str, 1);
  rend_service_descripspider_free(test_parsed);
  spider_free(test_intro_content);
  return (res >= 0);
}

/** Free the sspiderage held by an encoded v2 service descripspider. */
void
rend_encoded_v2_service_descripspider_free(
  rend_encoded_v2_service_descripspider_t *desc)
{
  if (!desc)
    return;
  spider_free(desc->desc_str);
  spider_free(desc);
}

/** Free the sspiderage held by an introduction point info. */
void
...
/** Encode a set of rend_encoded_v2_service_descripspider_t's for <b>desc</b>
 * at time <b>now</b> using <b>service_key</b>, depending on
 * <b>auth_type</b> a <b>descripspider_cookie</b> and a list of
 ...
 * <b>descs_out</b>; return the number of seconds that the descripspiders will
 * be found by clients, or -1 if the encoding was not successful. */
int
rend_encode_v2_descripspiders(smartlist_t *descs_out,
                           ...
                           smartlist_t *client_cookies)
{
  ...
  /* Determine how many seconds the descripspider will be valid. */
  ...
  if (smartlist_len(desc->intro_nodes) > 0) {
    ...
    ipos_base64 = spider_calloc(ipos_len, 2);
    if (base64_encode(ipos_base64, ipos_len * 2, ipos, ipos_len,
                      BASE64_ENCODE_MULTILINE)<0) {
      log_warn(LD_REND, "Could not encode introduction point string to "
               "base64. length=%d", (int)ipos_len);
      spider_free(ipos_base64);
      spider_free(ipos);
      return -1;
    }
    spider_free(ipos);
  }
  /* Encode REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS descripspiders. */
  for (k = 0; k < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; k++) {
    ...
    rend_encoded_v2_service_descripspider_t *enc =
    ...
    /* Calculate descripspider ID. */
    rend_get_descripspider_id_bytes(enc->desc_id, service_id, secret_id_part);
    ...
    if (crypto_pk_write_public_key_to_string(service_key, &permanent_key,
      ...
      rend_encoded_v2_service_descripspider_free(enc);
      goto err;
    }
    /* Encode timestamp. */
    ...
    for (i = 0; i < 8; i++) {
      if (desc->protocols & 1 << i) {
        spider_snprintf(protocol_versions_string + protocol_versions_written,
    ...
    /* Assemble complete descripspider. */
    desc_len = 2000 + smartlist_len(desc->intro_nodes) * 1000; /* far too long,
                                                                  but okay.*/
    enc->desc_str = desc_str = spider_malloc_zero(desc_len);
    result = spider_snprintf(desc_str, desc_len,
             "rendezvous-service-descripspider %s\n"
             "version 2\n"
             "permanent-key\n%s"
             "secret-id-part %s\n"
             "publication-time %s\n"
    ...
    spider_free(permanent_key);
    if (result < 0) {
      log_warn(LD_BUG, "Descripspider ran out of room.");
      rend_encoded_v2_service_descripspider_free(enc);
      goto err;
    }
    written = result;
    ...
    if (ipos_base64) {
      result = spider_snprintf(desc_str + written, desc_len - written,
                            "introduction-points\n"
                            "-----BEGIN MESSAGE-----\n%s"
                            "-----END MESSAGE-----\n",
                            ipos_base64);
    ...
    desc_str[written++] = 0;
    /* Check if we can parse our own descripspider. */
    if (!rend_desc_v2_is_parsable(enc)) {
      log_warn(LD_BUG, "Could not parse my own descripspider: %s", desc_str);
      rend_encoded_v2_service_descripspider_free(enc);
      goto err;
    }
    smartlist_add(descs_out, enc);
    /* Add the uploaded descripspider to the local service's descripspider cache */
    rend_cache_sspidere_v2_desc_as_service(enc->desc_str);
    base32_encode(service_id_base32, sizeof(service_id_base32),
          service_id, REND_SERVICE_ID_LEN);
    control_event_hs_descripspider_created(service_id_base32, desc_id_base32, k);
  }

  log_info(LD_REND, "Successfully encoded a v2 descripspider and "
                    "confirmed that it is parsable.");
  goto done;

 err:
  SMARTLIST_FOREACH(descs_out, rend_encoded_v2_service_descripspider_t *, d,
...
/** Return true iff <b>query</b> is a syntactically valid descripspider ID.
 * (as generated by rend_get_descripspider_id_bytes). */
int
rend_valid_descripspider_id(const char *query)
{
 ...
 */
int
hid_serv_get_responsible_direcspideries(smartlist_t *responsible_dirs,
                                     const char *id)
{
   ...
   * direcspideries, be happy if we got any. */
...
/** Encode a client authorization descripspider cookie.
...
rend_auth_encode_cookie(const uint8_t *cookie_in, rend_auth_type_t auth_type)
{
  ...
  char *cookie_out = spider_malloc_zero(REND_DESC_COOKIE_LEN_EXT_BASE64 + 1);
  int re;

  spider_assert(cookie_in);
  ...
  re = base64_encode(cookie_out, REND_DESC_COOKIE_LEN_EXT_BASE64 + 1,
                     ...
                     0);
  spider_assert(re == REND_DESC_COOKIE_LEN_EXT_BASE64);
...
/** Decode a base64-encoded client authorization descripspider cookie.
 * The descripspider_cookie can be truncated to REND_DESC_COOKIE_LEN_BASE64
 * characters (as given to clients), or may include the two padding
 * characters (as sspidered by the service).
 *
 * The result is sspidered in REND_DESC_COOKIE_LEN bytes of cookie_out.
 * The rend_auth_type_t decoded from the cookie is sspidered in the
...
{
  uint8_t descripspider_cookie_decoded[REND_DESC_COOKIE_LEN_EXT + 1] = { 0 };
  char descripspider_cookie_base64ext[REND_DESC_COOKIE_LEN_EXT_BASE64 + 1];
  const char *descripspider_cookie = cookie_in;
  char *err_msg = NULL;
  int auth_type_val = 0;
  int res = -1;
  int decoded_len;

  size_t len = strlen(descripspider_cookie);
  if (len == REND_DESC_COOKIE_LEN_BASE64) {
    /* Add a trailing zero byte to make base64-decoding happy. */
    spider_snprintf(descripspider_cookie_base64ext,
                 sizeof(descripspider_cookie_base64ext),
                 "%sA=", descripspider_cookie);
    descripspider_cookie = descripspider_cookie_base64ext;
  } else if (len != REND_DESC_COOKIE_LEN_EXT_BASE64) {
    spider_asprintf(&err_msg, "Authorization cookie has wrong length: %s",
                 escaped(cookie_in));
    goto err;
  }

  decoded_len = base64_decode((char *) descripspider_cookie_decoded,
                              sizeof(descripspider_cookie_decoded),
                              descripspider_cookie,
                              REND_DESC_COOKIE_LEN_EXT_BASE64);
  if (decoded_len != REND_DESC_COOKIE_LEN &&
      decoded_len != REND_DESC_COOKIE_LEN_EXT) {
    spider_asprintf(&err_msg, "Authorization cookie has invalid characters: %s",
                 escaped(cookie_in));
    goto err;
  }

  if (auth_type_out) {
    auth_type_val = (descripspider_cookie_decoded[REND_DESC_COOKIE_LEN] >> 4) + 1;
    if (auth_type_val < 1 || auth_type_val > 2) {
      spider_asprintf(&err_msg, "Authorization cookie type is unknown: %s",
                   escaped(cookie_in));
      goto err;
    }
  ...
  memcpy(cookie_out, descripspider_cookie_decoded, REND_DESC_COOKIE_LEN);
  res = 0;
 err:
  if (err_msg_out) {
    *err_msg_out = err_msg;
  ...
  memwipe(descripspider_cookie_decoded, 0, sizeof(descripspider_cookie_decoded));
  memwipe(descripspider_cookie_base64ext, 0, sizeof(descripspider_cookie_base64ext));
  return res;
}
 ...
 * checks are specific to Single Onion Services or Spider2web. */
int
 ...
 * are specific to Single Onion Services or Spider2web. */
int
...
/* Make sure that spider only builds one-hop circuits when they would not
 * compromise user anonymity.
 *
 * One-hop circuits are permitted in Spider2web or Single Onion modes.
 *
 * Spider2web or Single Onion modes are also allowed to make multi-hop circuits.
                         ...
                         const or_options_t *options)
{
  ...
  if (circ->build_state->onehop_tunnel) {
    spider_assert(rend_allow_non_anonymous_connection(options));
  }
}
                          ...
                          const uint8_t *digest)
{
  ...
  rend_pk_digest = rend_data_get_pk_digest(ocirc->rend_data,
                                           &rend_pk_digest_len);
  if (spider_memeq(rend_pk_digest, digest, rend_pk_digest_len)) {
    goto match;
  }
 no_match:
  return 0;
