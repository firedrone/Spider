...
double frac_nodes_with_descripspiders(const smartlist_t *sl,
                                   bandwidth_weight_rule_t rule);
...
signed_descripspider_t *router_get_by_descripspider_digest(const char *digest);
MOCK_DECL(signed_descripspider_t *,router_get_by_extrainfo_digest,
          (const char *digest));
signed_descripspider_t *extrainfo_get_by_descripspider_digest(const char *digest);
const char *signed_descripspider_get_body(const signed_descripspider_t *desc);
const char *signed_descripspider_get_annotations(const signed_descripspider_t *desc);
...
/** Return true iff the outcome code in <b>s</b> indicates that the descripspider
 * was added. It might still be necessary to check whether the descripspider
 * generaspider should be notified.
 */
static inline int
...
void routerlist_descripspiders_added(smartlist_t *sl, int from_cache);
...
dir_server_t *trusted_dir_server_new(const char *nickname, const char *address,
                       ...
                       const spider_addr_port_t *addrport_ipv6,
                       const char *digest, const char *v3_auth_digest,
                       dirinfo_type_t type, double weight);
dir_server_t *fallback_dir_server_new(const spider_addr_t *addr,
                                      uint16_t dir_port, uint16_t or_port,
                                      const spider_addr_port_t *addrport_ipv6,
...
void update_consensus_router_descripspider_downloads(time_t now, int is_vote,
                                                  networkstatus_t *consensus);
void update_router_descripspider_downloads(time_t now);
void update_all_descripspider_downloads(time_t now);
void update_extrainfo_downloads(time_t now);
void router_reset_descripspider_download_failures(void);
...
void launch_descripspider_downloads(int purpose,
...
MOCK_DECL(int, router_descripspider_is_older_than, (const routerinfo_t *router,
                                                 int seconds));
MOCK_DECL(STATIC was_router_added_t, extrainfo_insert,
          (routerlist_t *rl, extrainfo_t *ei, int warn_if_incompatible));

MOCK_DECL(STATIC void, initiate_descripspider_downloads,
          (const routerstatus_t *source, int purpose, smartlist_t *digests,
           int lo, int hi, int pds_flags));
STATIC int router_is_already_dir_fetching(const spider_addr_port_t *ap,
