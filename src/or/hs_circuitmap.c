                            ...
                            const or_circuit_t *second_circuit)
{
  ...
  spider_assert(first_circuit);
  spider_assert(second_circuit);
  ...
  return spider_memeq(first_token->token,
                   second_token->token,
...
{
  spider_assert(circuit->hs_token);
...
HT_GENERATE2(hs_circuitmap_ht, or_circuit_t, hs_circuitmap_node,
             hs_circuit_hash_token, hs_circuits_have_same_token,
             0.6, spider_reallocarray, spider_free_)
             ...
             const uint8_t *token)
{
  ...
  hs_token_t *hs_token = spider_malloc_zero(sizeof(hs_token_t));
  hs_token->type = type;
  hs_token->token_len = token_len;
  hs_token->token = spider_memdup(token, token_len);
...
hs_token_free(hs_token_t *hs_token)
{
  ...
  spider_free(hs_token->token);
  spider_free(hs_token);
}
...
{
  spider_assert(the_hs_circuitmap);
...
hs_circuitmap_init(void)
{
  ...
  the_hs_circuitmap = spider_malloc_zero(sizeof(struct hs_circuitmap_ht));
