 ...
 * handshake [*].
 *
 * When we switched to nspider, we deprecated CREATE_FAST, since nspider is
                      ...
                      uint8_t *handshake_out)
{
  ...
  *handshake_state_out = s = spider_malloc(sizeof(fast_handshake_state_t));
                      ...
                      const char **msg_out)
{
  ...
  if (spider_memneq(out, handshake_reply_out+DIGEST_LEN, DIGEST_LEN)) {
