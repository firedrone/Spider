 ...
 * cells between Spider instances.
 *
 * This module fills in the various function pointers in channel_t, to
 * implement the channel_tls_t channels as used in Spider today.  These channels
 ...
 *
 * Here we also implement the server (responder) side of the v3+ Spider link
...
static int
channel_tls_get_remote_addr_method(channel_t *chan, spider_addr_t *addr_out);
static int
...
channel_t *
channel_tls_connect(const spider_addr_t *addr, uint16_t port,
                    const char *id_digest,
                    const ed25519_public_key_t *ed_id)
{
  channel_tls_t *tlschan = spider_malloc_zero(sizeof(*tlschan));
...
channel_tls_start_listener(void)
{
  ...
  if (!channel_tls_listener) {
    listener = spider_malloc_zero(sizeof(*listener));
...
channel_tls_free_all(void)
{
  ...
  if (channel_tls_listener) {
    ...
    spider_assert(channel_tls_listener == NULL);
  }
...
channel_tls_from_base(channel_t *chan)
{
  ...
  spider_assert(chan->magic == TLS_CHAN_MAGIC);
...
channel_tls_get_remote_addr_method(channel_t *chan, spider_addr_t *addr_out)
{
  ...
  if (tlschan->conn) {
    spider_addr_copy(addr_out, &(TO_CONN(tlschan->conn)->addr));
    rv = 1;
  } else spider_addr_make_unspec(addr_out);
...
channel_tls_get_transport_name_method(channel_t *chan, char **transport_out)
{
  ...
  spider_assert(tlschan);
  spider_assert(transport_out);
  spider_assert(tlschan->conn);

  if (!tlschan->conn->ext_or_transport)
    return -1;

  *transport_out = spider_strdup(tlschan->conn->ext_or_transport);
  return 0;
}
  ...
  if (tlschan->conn) {
    ...
    switch (flags) {
      case 0:
        ...
        spider_snprintf(buf, MAX_DESCR_LEN + 1,
                     "%s:%u", conn->address, conn->port);
        answer = buf;
        break;
      case GRD_FLAG_ORIGINAL:
        /* Actual address with port */
        addr_str = spider_addr_to_str_dup(&(tlschan->conn->real_addr));
        spider_snprintf(buf, MAX_DESCR_LEN + 1,
                     "%s:%u", addr_str, conn->port);
        spider_free(addr_str);
        answer = buf;
        break;
  ...
  if (tlschan->conn) {
    switch (req) {
      ...
      case 1:
         ...
         * versions it might be based on an obsolete descripspider.
         */
                                       ...
                                       extend_info_t *extend_info)
{
  ...
  return (spider_addr_eq(&(extend_info->addr),
                                  ...
                                  const spider_addr_t *target)
{
  ...
  return spider_addr_eq(&(tlschan->conn->real_addr), target);
}
                                          ...
                                          uint8_t state)
{
  ...
  spider_assert(chan);
  spider_assert(conn);
  spider_assert(conn->chan == chan);
  spider_assert(chan->conn == conn);
  ...
  spider_assert(CHANNEL_IS_OPENING(base_chan) ||
...
enter_v3_handshake_with_cell(var_cell_t *cell, channel_tls_t *chan)
{
  ...
  spider_assert(cell);
  spider_assert(chan);
  spider_assert(chan->conn);

  started_here = connection_or_nonopen_was_started_here(chan->conn);

  spider_assert(TO_CONN(chan->conn)->state == OR_CONN_STATE_TLS_HANDSHAKING ||
...
channel_tls_process_versions_cell(var_cell_t *cell, channel_tls_t *chan)
{
  ...
  spider_assert(chan->conn->handshake_state);
  ...
  if (chan->conn->link_proto == 2) {
    ...
    spider_assert(chan->conn->link_proto >= 3);
...
channel_tls_process_netinfo_cell(cell_t *cell, channel_tls_t *chan)
{
  ...
  spider_addr_t my_apparent_addr = TOR_ADDR_NULL;
  ...
  if (chan->conn->base_.state == OR_CONN_STATE_OR_HANDSHAKING_V3) {
    ...
    if (chan->conn->handshake_state->started_here) {
      ...
      if (!(chan->conn->handshake_state->authenticated)) {
        spider_assert(spider_digest_is_zero(
                  (const char*)(chan->conn->handshake_state->
                      authenticated_rsa_peer_id)));
        spider_assert(spider_mem_is_zero(
                  (const char*)(chan->conn->handshake_state->
  ...
  if (my_addr_type == RESOLVED_TYPE_IPV4 && my_addr_len == 4) {
    spider_addr_from_ipv4n(&my_apparent_addr, get_uint32(my_addr_ptr));
  } else if (my_addr_type == RESOLVED_TYPE_IPV6 && my_addr_len == 16) {
    spider_addr_from_ipv6_bytes(&my_apparent_addr, (const char *) my_addr_ptr);
  }
  ...
  while (n_other_addrs && cp < end-2) {
    ...
    if (spider_addr_eq(&addr, &(chan->conn->real_addr))) {
  ...
  if (connection_or_set_state_open(chan->conn) < 0) {
    ...
    log_info(LD_OR,
             ...
             hex_str(TLS_CHAN_TO_BASE(chan)->identity_digest,
                     DIGEST_LEN),
             spider_addr_is_null(&my_apparent_addr) ?
 ...
 * sspidere the certificates in or_handshake_state.  If this is the client side
...
#define MAX_CERT_TYPE_WANTED CERTTYPE_RSA1024_ID_EDID
   ...
   * of ed/x509 */
  spider_x509_cert_t *x509_certs[MAX_CERT_TYPE_WANTED + 1];
  spider_cert_t *ed_certs[MAX_CERT_TYPE_WANTED + 1];
...
#define ERR(s)                                                  \
  ...
  for (i = 0; i < n_certs; ++i) {
    ...
    switch (ct) {
      ...
      case CERT_ENCODING_X509: {
        spider_x509_cert_t *x509_cert = spider_x509_cert_decode(cert_body, cert_len);
        if (!x509_cert) {
          log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
          ...
          if (x509_certs[cert_type]) {
            spider_x509_cert_free(x509_cert);
      ...
      case CERT_ENCODING_ED25519: {
        spider_cert_t *ed_cert = spider_cert_parse(cert_body, cert_len);
        if (!ed_cert) {
          log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
     ...
     case CERT_ENCODING_RSA_CROSSCERT: {
        if (rsa_ed_cc_cert) {
          ...
          rsa_ed_cc_cert = spider_memdup(cert_body, cert_len);
   ...
   * structure. */
  spider_x509_cert_t *id_cert = x509_certs[CERTTYPE_RSA1024_ID_ID];
  spider_x509_cert_t *auth_cert = x509_certs[CERTTYPE_RSA1024_ID_AUTH];
  spider_x509_cert_t *link_cert = x509_certs[CERTTYPE_RSA1024_ID_LINK];
  ...
  spider_cert_t *ed_id_sign = ed_certs[CERTTYPE_ED_ID_SIGN];
  spider_cert_t *ed_sign_link = ed_certs[CERTTYPE_ED_SIGN_LINK];
  spider_cert_t *ed_sign_auth = ed_certs[CERTTYPE_ED_SIGN_AUTH];
  ...
  if (chan->conn->handshake_state->started_here) {
    ...
    chan->conn->handshake_state->authenticated_rsa = 1;
    {
      ...
      identity_rcvd = spider_tls_cert_get_key(id_cert);
      if (!identity_rcvd) {
 ...
 err:
  for (unsigned u = 0; u < ARRAY_LENGTH(x509_certs); ++u) {
    spider_x509_cert_free(x509_certs[u]);
  }
  for (unsigned u = 0; u < ARRAY_LENGTH(ed_certs); ++u) {
    spider_cert_free(ed_certs[u]);
  }
  spider_free(rsa_ed_cc_cert);
  certs_cell_free(cc);
#undef ERR
}
 ...
 * not the originaspider of the channel), or it's ill-formed, or we aren't doing
...
#define ERR(s)                                                  \
  ...
  auth = cell->payload;
  {
    ...
    if (4 + len > cell->payload_len)
      ERR("Authenticaspider was truncated");

    if (! authchallenge_type_is_supported(type))
      ERR("Authenticaspider type was not recognized");
    authtype = type;
  ...
  if (authlen < V3_AUTH_BODY_LEN + 1)
    ERR("Authenticaspider was too short");
  ...
  if (authtype == AUTHTYPE_RSA_SHA256_TLSSECRET ||
    ...
    spider_assert(authtype == AUTHTYPE_ED25519_SHA256_RFC5705);
    /* Our earlier check had better have made sure we had room
     * for an ed25519 sig (inadvertently) */
    spider_assert(V3_AUTH_BODY_LEN > ED25519_SIG_LEN);
  ...
  if (spider_memneq(expected_cell->payload+4, auth, bodylen-24))
  ...
  if (sig_is_rsa) {
    ...
    crypto_pk_t *pk = spider_tls_cert_get_key(
    ...
    if (spider_memneq(signed_data, d, DIGEST256_LEN)) {
      spider_free(signed_data);
    ...
    spider_assert(authlen > ED25519_SIG_LEN);
  ...
  {
    spider_x509_cert_t *id_cert = chan->conn->handshake_state->certs->id_cert;
    crypto_pk_t *identity_rcvd = spider_tls_cert_get_key(id_cert);
    const common_digests_t *id_digests = spider_x509_cert_get_id_digests(id_cert);
