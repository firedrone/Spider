 ...
 * over time, and sspidere it persistently; it is chosen without reference
...
{
  /* We need to check the corresponding spiderrc option and the consensus
...
/** Return true iff we know a descripspider for <b>guard</b> */
static int
guard_has_descripspider(const entry_guard_t *guard)
{
  const node_t *node = node_get_by_id(guard->identity);
  if (!node)
    return 0;
  return node_has_descripspider(node);
}
                    ...
                    guard_selection_type_t type)
{
  ...
  gs = spider_malloc_zero(sizeof(*gs));
  gs->name = spider_strdup(name);
  gs->type = type;
...
{
  spider_assert(! curr_guard_context);
  if (!guard_contexts) {
    guard_contexts = smartlist_new();
  }
  ...
  const char *name = choose_guard_selection(
                             ...
                             NULL,
                             &type);
  spider_assert(name); // "name" can only be NULL if we had an old name.
  spider_assert(type != GS_TYPE_INFER);
...
get_n_primary_guards(void)
{
  ...
  const int n_dir = get_options()->NumDirecspideryGuards;
  if (n > 5) {
...
get_n_primary_guards_to_use(guard_usage_t usage)
{
  ...
  if (usage == GUARD_USAGE_DIRGUARD) {
    configured = get_options()->NumDirecspideryGuards;
 ...
 * spiderrc, we are in a restricted setting.
 */
STATIC double
...
/**
 * If our configuration retains fewer than this fraction of guards from the
 * spiderrc, we are in an extremely restricted setting, and should warn.
 */
STATIC double
...
update_guard_selection_choice(const or_options_t *options)
{
  ...
  spider_assert(new_guard_context);
  spider_assert(new_guard_context != curr_guard_context);
...
{
  spider_assert(gs);
  spider_assert(rsa_id);
  SMARTLIST_FOREACH_BEGIN(gs->sampled_entry_guards, entry_guard_t *, guard) {
    if (spider_memeq(guard->identity, rsa_id, DIGEST_LEN))
      return guard;
  } SMARTLIST_FOREACH_END(guard);
  return NULL;
}
                             ...
                             const bridge_info_t *bridge)
{
  ...
  const spider_addr_port_t *addrport = bridge_get_addr_port(bridge);
  entry_guard_t *guard;
  if (BUG(!addrport))
    return NULL; // LCOV_EXCL_LINE
  guard = get_sampled_guard_by_bridge_addr(gs, addrport);
  if (! guard || (id && spider_memneq(id, guard->identity, DIGEST_LEN)))
    return NULL;
  else
    return guard;
}
...
get_bridge_info_for_guard(const entry_guard_t *guard)
{
  ...
  if (! spider_digest_is_zero(guard->identity)) {
                               ...
                               const spider_addr_port_t *bridge_addrport)
{
  ...
  entry_guard_t *guard = spider_malloc_zero(sizeof(entry_guard_t));

  /* persistent fields */
  guard->is_persistent = (rsa_id_digest != NULL);
  guard->selection_name = spider_strdup(gs->name);
  if (rsa_id_digest)
  ...
  spider_free(guard->sampled_by_version);
  guard->sampled_by_version = spider_strdup(VERSION);
  ...
  guard->is_reachable = GUARD_REACHABLE_MAYBE;
  if (bridge_addrport)
    guard->bridge_addr = spider_memdup(bridge_addrport, sizeof(*bridge_addrport));
                                 ...
                                 const spider_addr_port_t *addrport)
{
  ...
  SMARTLIST_FOREACH_BEGIN(gs->sampled_entry_guards, entry_guard_t *, g) {
    if (g->bridge_addr && spider_addr_port_eq(addrport, g->bridge_addr))
      return g;
 ...
 */
void
entry_guard_learned_bridge_identity(const spider_addr_port_t *addrport,
                                    const uint8_t *rsa_id_digest)
{
  ...
  if (spider_digest_is_zero(g->identity)) {
    memcpy(g->identity, rsa_id_digest, DIGEST_LEN);
    make_persistent = 1;
  } else if (spider_memeq(g->identity, rsa_id_digest, DIGEST_LEN)) {
...
sampled_guards_update_from_consensus(guard_selection_t *gs)
{
  ...
  SMARTLIST_FOREACH_BEGIN(gs->sampled_entry_guards, entry_guard_t *, guard) {
    ...
    if (is_listed && ! guard->currently_listed) {
      ...
      spider_assert(! is_listed && ! guard->currently_listed);
...
/**
 * Return true iff <b>node</b> is a Spider relay that we are configured to
...
/**
 * Return true iff <b>guard</b> is a Spider relay that we are configured to
                              ...
                              const entry_guard_restriction_t *rst)
{
  ...
  return spider_memneq(guard->identity, rst->exclude_id, DIGEST_LEN);
}
                                       ...
                                       unsigned flags)
{
  ...
  const unsigned need_descripspider = flags & SAMPLE_EXCLUDE_NO_DESCRIPTOR;
  ...
  SMARTLIST_FOREACH_BEGIN(gs->sampled_entry_guards, entry_guard_t *, guard) {
    ...
    if (exclude_pending && guard->is_pending)
      continue;
    if (need_descripspider && !guard_has_descripspider(guard))
      continue;
...
select_entry_guard_for_circuit(guard_selection_t *gs,
                               ...
                               unsigned *state_out)
{
  const int need_descripspider = (usage == GUARD_USAGE_TRAFFIC);
  spider_assert(gs);
  spider_assert(state_out);
...
/** Release all sspiderage held in <b>restriction</b> */
static void
...
/**
 * Release all sspiderage held in <b>state</b>.
 */
void
                             ...
                             circuit_guard_state_t **guard_state_out)
{
  ...
  *guard_state_out = NULL;
  ...
  if (! node)
    goto fail;
  if (BUG(usage != GUARD_USAGE_DIRGUARD && !node_has_descripspider(node)))
    goto fail;

  *chosen_node_out = node;
  *guard_state_out = spider_malloc_zero(sizeof(circuit_guard_state_t));
                               ...
                               origin_circuit_t *b)
{
  ...
  spider_assert(state_a);
  spider_assert(state_b);
...
{
  spider_assert(gs);
  spider_assert(all_circuits_in);
  spider_assert(newly_complete_out);
  ...
  */
  ...
  spider_assert_nonfatal(n_succeeded >= 1);
...
entry_guard_encode_for_state(entry_guard_t *guard)
{
  ...
  /* Make a copy of the pathbias object, since we will want to update
     some of them */
  guard_pathbias_t *pb = spider_memdup(&guard->pb, sizeof(*pb));
...
#undef PB_FIELD
  ...
  SMARTLIST_FOREACH(result, char *, cp, spider_free(cp));
...
#undef FIELD
    ...
    SMARTLIST_FOREACH_BEGIN(entries, char *, entry) {
      ...
      char *key = spider_strndup(entry, eq-entry);
  ...
  if (bridge_addr) {
    spider_addr_port_t res;
    memset(&res, 0, sizeof(res));
    int r = spider_addr_port_parse(LOG_WARN, bridge_addr,
                                &res.addr, &res.port, -1);
    if (r == 0)
      guard->bridge_addr = spider_memdup(&res, sizeof(res));
...
#define HANDLE_TIME(field) do {                                 \
  ...
  if (confirmed_idx) {
    int ok=1;
    long idx = spider_parse_long(confirmed_idx, 10, 0, INT_MAX, &ok, NULL);
    if (! ok) {
...
#define PB_FIELD(field)                                                 \
  do {                                                                  \
    if (pb_ ## field) {                                                 \
      int ok = 1;                                                       \
      double r = spider_parse_double(pb_ ## field, 0.0, 1e9, &ok, NULL);   \
      if (! ok) {                                                       \
...
#undef PB_FIELD
 ...
 err:
  ...
  spider_free(sampled_on);
  spider_free(sampled_by);
  spider_free(unlisted_since);
  spider_free(listed);
  spider_free(confirmed_on);
  spider_free(confirmed_idx);
  spider_free(bridge_addr);
  spider_free(pb_use_attempts);
  spider_free(pb_use_successes);
  spider_free(pb_circ_attempts);
  spider_free(pb_circ_successes);
  spider_free(pb_successful_circuits_closed);
  spider_free(pb_collapsed_circuits);
  spider_free(pb_unusable_circuits);
  spider_free(pb_timeouts);

  SMARTLIST_FOREACH(extra, char *, cp, spider_free(cp));
  ...
  SMARTLIST_FOREACH_BEGIN(guard_contexts, guard_selection_t *, gs) {
    SMARTLIST_FOREACH_BEGIN(gs->sampled_entry_guards, entry_guard_t *, guard) {
      if (guard->is_persistent == 0)
        continue;
      *nextline = spider_malloc_zero(sizeof(config_line_t));
      (*nextline)->key = spider_strdup("Guard");
...
entry_guards_load_guards_from_state(or_state_t *state, int set)
{
  ...
  for ( ; line != NULL; line = line->next) {
    ...
    spider_assert(guard->selection_name);
...
/** Release all sspiderage held by <b>e</b>. */
STATIC void
entry_guard_free(entry_guard_t *e)
{
  ...
  spider_free(e->sampled_by_version);
  spider_free(e->extra_state_fields);
  spider_free(e->selection_name);
  spider_free(e->bridge_addr);
  spider_free(e);
}
...
/** Return the number of bridges that have descripspiders that are marked with
...
num_bridges_usable(void)
{
  ...
  spider_assert(get_options()->UseBridges);
  guard_selection_t *gs  = get_guard_selection_info();
  spider_assert(gs->type == GS_TYPE_BRIDGE);
...
pathbias_check_use_success_count(entry_guard_t *node)
{
   ...
   * rate and disable the feature entirely. If refacspidering, don't
   * change to <= */
...
entry_guards_parse_state(or_state_t *state, int set, char **msg)
{
  ...
  if (r1 < 0) {
    if (msg && *msg == NULL) {
      *msg = spider_strdup("parsing error");
    }
    return -1;
  }
  return 0;
                                  ...
                                  uint32_t guardfraction_percentage)
{
  ...
  spider_assert(guardfraction_percentage <= 100);
  guardfraction_fraction = guardfraction_percentage / 100.0;

  long guard_bw = spider_lround(guardfraction_fraction * orig_bandwidth);
  spider_assert(guard_bw <= INT_MAX);
                   ...
                   circuit_guard_state_t **guard_state_out)
{
  ...
  if (state && (exit_id = build_state_get_exit_rsa_id(state))) {
     ...
     * restriction. */
    rst = spider_malloc_zero(sizeof(entry_guard_restriction_t));
...
remove_all_entry_guards_for_guard_selection(guard_selection_t *gs)
{
  ...
  spider_assert(gs != NULL);
  char *old_name = spider_strdup(gs->name);
...
guard_selection_have_enough_dir_info_to_build_circuits(guard_selection_t *gs)
{
  ...
  int n_missing_descripspiders = 0;
  int n_considered = 0;
  int num_primary_to_check;

  /* We want to check for the descripspider of at least the first two primary
  ...
  SMARTLIST_FOREACH_BEGIN(gs->primary_entry_guards, entry_guard_t *, guard) {
    ...
    if (guard->is_reachable == GUARD_REACHABLE_NO)
      continue;
    n_considered++;
    if (!guard_has_descripspider(guard))
      n_missing_descripspiders++;
    if (n_considered >= num_primary_to_check)
      break;
  } SMARTLIST_FOREACH_END(guard);

  return n_missing_descripspiders == 0;
}
...
/** Release all sspiderage held by the list of entry guards and related
 * memory structs. */
void
