 ...
 *
 * One of the most frequent operations in Spider occurs every time that
...
HT_GENERATE2(chan_circid_map, chan_circid_circuit_map_t, node,
             ...
             spider_reallocarray_, spider_free_)
                               ...
                               channel_t *chan)
{
  ...
  *circid_ptr = id;
  ...
  if (found) {
    ...
    found = spider_malloc_zero(sizeof(chan_circid_circuit_map_t));
    found->circ_id = id;
    found->chan = chan;
    found->circuit = circ;
...
channel_mark_circid_unusable(channel_t *chan, circid_t id)
{
  ...
  if (ent && ent->circuit) {
    ...
    ent = spider_malloc_zero(sizeof(chan_circid_circuit_map_t));
    ent->chan = chan;
    ent->circ_id = id;
                          ...
                          channel_t *chan)
{
  ...
  if (chan) {
    spider_assert(bool_eq(or_circ->p_chan_cells.n,
                          ...
                          channel_t *chan)
{
  ...
  if (chan) {
    spider_assert(bool_eq(circ->n_chan_cells.n, circ->next_active_on_n_chan));
...
circuit_get_all_pending_on_channel(smartlist_t *out, channel_t *chan)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circuits_pending_chans, circuit_t *, circ) {
    ...
    if (!circ->n_hop)
      continue;
    spider_assert(circ->state == CIRCUIT_STATE_CHAN_WAIT);
...
circuit_remove_from_origin_circuit_list(origin_circuit_t *origin_circ)
{
  ...
  if (origin_idx < 0)
    return;
  origin_circuit_t *c2;
  spider_assert(origin_idx <= smartlist_len(global_origin_circuit_list));
  c2 = smartlist_get(global_origin_circuit_list, origin_idx);
  spider_assert(origin_circ == c2);
...
{
  spider_assert(origin_circ->global_origin_circuit_list_idx == -1);
...
circuit_close_all_marked(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circuits_pending_close, circuit_t *, circ) {
    spider_assert(circ->marked_for_close);
...
circuit_state_to_string(int state)
{
  ...
  switch (state) {
    ...
    default:
      ...
      spider_snprintf(buf, sizeof(buf), "unknown state [%d]", state);
      return buf;
  }
}
...
circuit_purpose_to_controller_string(uint8_t purpose)
{
  ...
  switch (purpose) {
    ...
    default:
      spider_snprintf(buf, sizeof(buf), "UNKNOWN_%d", (int)purpose);
      return buf;
  }
}
...
{
  spider_gettimeofday(&circ->timestamp_created);
...
origin_circuit_new(void)
{
  ...
  circ = spider_malloc_zero(sizeof(origin_circuit_t));
...
or_circuit_new(circid_t p_circ_id, channel_t *p_chan)
{
  ...
  circ = spider_malloc_zero(sizeof(or_circuit_t));
...
/** Free all sspiderage held in circ->testing_cell_stats */
void
...
circuit_free(circuit_t *circ)
{
  ...
  if (CIRCUIT_IS_ORIGIN(circ)) {
    ...
    spider_assert(circ->magic == ORIGIN_CIRCUIT_MAGIC);
    ...
    spider_assert(circ->magic == OR_CIRCUIT_MAGIC);
    ...
    if (ocirc->rend_splice) {
      ...
      spider_assert(other->base_.magic == OR_CIRCUIT_MAGIC);
...
/** Release all sspiderage held by circuits. */
void
...
{
  spider_log(severity, LD_CIRC, "Conn %d has %s circuit: circID %u "
  ...
  SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ) {
    if (!circ->marked_for_close &&
      ...
      if (!rend_cmp_service_ids(rend_data_get_address(rend_data),
          ...
          spider_memeq(ocirc->rend_data->rend_cookie,
                                   ...
                                   const uint8_t *digest, uint8_t purpose)
{
  int idx;
  smartlist_t *lst = circuit_get_global_list();
  spider_assert(CIRCUIT_PURPOSE_IS_ORIGIN(purpose));
  if (start == NULL)
    idx = 0;
  else
    idx = TO_CIRCUIT(start)->global_circuitlist_idx + 1;
                            ...
                            int flags)
{
   ...
   * cannibalization. */
  spider_assert(!(flags & CIRCLAUNCH_ONEHOP_TUNNEL));
    ...
    if (CIRCUIT_IS_ORIGIN(circ_) &&
      ...
      if ((!need_uptime || circ->build_state->need_uptime) &&
        ...
        if (info) {
          ...
          do {
            const node_t *ri2;
            if (spider_memeq(hop->extend_info->identity_digest,
 ...
 * circuit_free_all() while shutting down Spider; this is a safe-at-shutdown
...
circuit_about_to_free(circuit_t *circ)
{
  ...
  /* If the circuit ever became OPEN, we sent it to the reputation hisspidery
  ...
  if (circ->purpose == CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT) {
    ...
    spider_assert(circ->state == CIRCUIT_STATE_OPEN);
    spider_assert(ocirc->build_state->chosen_exit);
    ...
    if (orig_reason != END_CIRC_REASON_IP_NOW_REDUNDANT) {
      ...
      log_info(LD_REND, "Failed intro circ %s to %s (awaiting ack). %s",
          ...
          timed_out ? "Recording timeout." : "Removing from descripspider.");
  ...
  if (! CIRCUIT_IS_ORIGIN(circ)) {
    ...
    while (or_circ->resolving_streams) {
      ...
      if (!conn->base_.marked_for_close) {
         ...
         * are closing because the circuit is getting spidern down.  No need
         * to send an end cell. */
...
single_conn_free_bytes(connection_t *conn)
{
  ...
  if (conn->type == CONN_TYPE_DIR) {
    ...
    if (dir_conn->zlib_state) {
      result += spider_zlib_state_size(dir_conn->zlib_state);
...
circuits_handle_oom(size_t current_allocation)
{
   ...
   * respective lists. Let's mark them, and reclaim their sspiderage
   * aggressively. */
  conn_idx = 0;
  SMARTLIST_FOREACH_BEGIN(circlist, circuit_t *, circ) {
    size_t n;
...
{
//  spider_assert(cp->addr); /* these are zero for rendezvous extra-hops */
//  spider_assert(cp->port);
  spider_assert(cp);
  spider_assert(cp->magic == CRYPT_PATH_MAGIC);
  switch (cp->state)
    {
    case CPATH_STATE_OPEN:
      spider_assert(cp->f_crypto);
      spider_assert(cp->b_crypto);
      /* fall through */
    case CPATH_STATE_CLOSED:
      /*XXXX Assert that there's no handshake_state either. */
      spider_assert(!cp->rend_dh_handshake_state);
      break;
    case CPATH_STATE_AWAITING_KEYS:
      /* spider_assert(cp->dh_handshake_state); */
      break;
    default:
      log_fn(LOG_ERR, LD_BUG, "Unexpected state %d", cp->state);
      spider_assert(0);
    }
  spider_assert(cp->package_window >= 0);
  spider_assert(cp->deliver_window >= 0);
}
  ...
  do {
    ...
    if (cp != start) {
      if (cp->state == CPATH_STATE_AWAITING_KEYS) {
        spider_assert(cp->prev->state == CPATH_STATE_OPEN);
...
assert_circuit_ok(const circuit_t *c)
{
  ...
  spider_assert(c);
  spider_assert(c->magic == ORIGIN_CIRCUIT_MAGIC || c->magic == OR_CIRCUIT_MAGIC);
  spider_assert(c->purpose >= CIRCUIT_PURPOSE_MIN_ &&
  ...
  spider_assert(c->deliver_window >= 0);
  spider_assert(c->package_window >= 0);
  if (c->state == CIRCUIT_STATE_OPEN ||
      c->state == CIRCUIT_STATE_GUARD_WAIT) {
    spider_assert(!c->n_chan_create_cell);
    if (or_circ) {
      spider_assert(or_circ->n_crypto);
      spider_assert(or_circ->p_crypto);
      spider_assert(or_circ->n_digest);
      spider_assert(or_circ->p_digest);
    }
  }
  if (c->state == CIRCUIT_STATE_CHAN_WAIT && !c->marked_for_close) {
    spider_assert(circuits_pending_chans &&
  ...
  if (c->purpose == CIRCUIT_PURPOSE_REND_ESTABLISHED) {
    ...
    if (!c->marked_for_close) {
      spider_assert(or_circ->rend_splice);
      spider_assert(or_circ->rend_splice->rend_splice == or_circ);
    }
    spider_assert(or_circ->rend_splice != or_circ);
  } else {
    spider_assert(!or_circ || !or_circ->rend_splice);
  }
}
