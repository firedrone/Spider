 ...
 *
 * ::managed_proxy_list contains all the managed proxies this spider
 ...
 *
 * While we are serially parsing spiderrc, we sspidere all the transports
 * that a proxy should spawn in its <em>transports_to_launch</em>
 * element.
 *
 * When we finish reading the spiderrc, we spawn the managed proxy and
 ...
 * read happens immediately after spider is launched.).
 *
 * We mark all managed proxies and transports to signify that they
 * must be removed if they don't contribute by the new spiderrc
 ...
 * new spiderrc wants them to support
 ...
 * spiderrc.
 *
 * We then start parsing spiderrc again.
 *
 ...
 *
 * During the post-config-read spiderrc parsing, we unmark all transports
 * spawned by managed proxies that we find in our spiderrc.
...
/** A list of pluggable transports found in spiderrc. */
...
static transport_t *
transport_new(const spider_addr_t *addr, uint16_t port,
              const char *name, int socks_ver,
              const char *extra_info_args)
{
  transport_t *t = spider_malloc_zero(sizeof(transport_t));

  spider_addr_copy(&t->addr, addr);
  t->port = port;
  t->name = spider_strdup(name);
  t->socks_version = socks_ver;
  if (extra_info_args)
    t->extra_info_args = spider_strdup(extra_info_args);
...
transport_free(transport_t *transport)
{
  ...
  spider_free(transport->name);
  spider_free(transport->extra_info_args);
  spider_free(transport);
}
...
transport_copy(const transport_t *transport)
{
  ...
  spider_assert(transport);

  new_transport = spider_malloc_zero(sizeof(transport_t));

  new_transport->socks_version = transport->socks_version;
  new_transport->name = spider_strdup(transport->name);
  spider_addr_copy(&new_transport->addr, &transport->addr);
...
transport_resolve_conflicts(const transport_t *t)
{
  /* This is how we resolve transport conflicts:
     ...
     we either have duplicate spiderrc lines OR we are here post-HUP and
     ...
     * if it's marked for removal, it means that it either has a lower
     priority than 't' in spiderrc (otherwise the mark would have been
     cleared by the paragraph above), or it doesn't exist at all in
     the post-HUP spiderrc. We destroy the old transport and register 't'.
     * if it's *not* marked for removal, it means that it was newly
     added in the post-HUP spiderrc or that it's of higher priority, in
     this case we ignore 't'. */
  transport_t *t_tmp = transport_get_by_name(t->name);
  if (t_tmp) { /* same name */
    if (spider_addr_eq(&t->addr, &t_tmp->addr) && (t->port == t_tmp->port)) {
      /* same name *and* addrport */
      t_tmp->marked_for_removal = 0;
      return 1;
    } else { /* same name but different addrport */
      char *new_transport_addrport =
        spider_strdup(fmt_addrport(&t->addr, t->port));
      if (t_tmp->marked_for_removal) { /* marked for removal */
        ...
        spider_free(new_transport_addrport);
...
MOCK_IMPL(int,
transport_add_from_config, (const spider_addr_t *addr, uint16_t port,
                            const char *name, int socks_ver))
{
...
assert_unconfigured_count_ok(void)
{
  ...
  if (!managed_proxy_list) {
    spider_assert(unconfigured_proxies_n == 0);
    return;
  }
  ...
  spider_assert(n_completed + unconfigured_proxies_n ==
...
{
  spider_assert(mp->transports_to_launch);
...
proxy_needs_restart(const managed_proxy_t *mp)
{
  ...
  spider_assert(smartlist_len(mp->transports_to_launch) > 0);
  spider_assert(mp->conf_state == PT_PROTO_COMPLETED);
...
proxy_prepare_for_restart(managed_proxy_t *mp)
{
  ...
  spider_process_handle_destroy(mp->process_handle, 1);
...
launch_managed_proxy(managed_proxy_t *mp)
{
  ...
  retval = spider_spawn_background(NULL,
                                (const char **)mp->argv,
                                env,
                                &mp->process_handle);
#else
  retval = spider_spawn_background(mp->argv[0],
                                (const char **)mp->argv,
                                env,
                                &mp->process_handle);
#endif
  ...
  log_info(LD_CONFIG, "Managed proxy at '%s' has spawned with PID '%d'.",
           mp->argv[0], spider_process_get_pid(mp->process_handle));
...
pt_configure_remaining_proxies(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(tmp,  managed_proxy_t *, mp) {
    spider_assert(mp->conf_state != PT_PROTO_BROKEN &&
  ...
  if (at_least_a_proxy_config_finished)
    mark_my_descripspider_dirty("configured managed proxies");
}
...
configure_proxy(managed_proxy_t *mp)
{
  ...
  spider_assert(mp->conf_state != PT_PROTO_INFANT);
  spider_assert(mp->process_handle);

  proxy_output =
    spider_get_lines_from_handle(spider_process_get_stdout_pipe(mp->process_handle),
                              &stream_status);
  if (!proxy_output) { /* failed to get input from proxy */
    if (stream_status != IO_STREAM_EAGAIN) { /* bad stream status! */
      ...
      log_warn(LD_GENERAL, "The communication stream of managed proxy '%s' "
               ...
               "This might be a bug of the managed proxy, a bug of Spider, or "
  ...
  if (proxy_output) {
    SMARTLIST_FOREACH(proxy_output, char *, cp, spider_free(cp));
                      ...
                      int also_terminate_process)
{
  ...
  SMARTLIST_FOREACH(mp->transports_to_launch, char *, t, spider_free(t));
  ...
  spider_process_handle_destroy(mp->process_handle, also_terminate_process);
  mp->process_handle = NULL;

  spider_free(mp);
}

/** Convert the spider proxy options to a URI suitable for TOR_PT_PROXY.
...
get_pt_proxy_uri(void)
{
  ...
  if (options->Socks4Proxy || options->Socks5Proxy || options->HTTPSProxy) {
    ...
    if (options->Socks4Proxy) {
      spider_addr_to_str(addr, &options->Socks4ProxyAddr, sizeof(addr), 1);
      spider_asprintf(&uri, "socks4a://%s:%d", addr, options->Socks4ProxyPort);
    } else if (options->Socks5Proxy) {
      spider_addr_to_str(addr, &options->Socks5ProxyAddr, sizeof(addr), 1);
      if (!options->Socks5ProxyUsername && !options->Socks5ProxyPassword) {
        spider_asprintf(&uri, "socks5://%s:%d", addr, options->Socks5ProxyPort);
      } else {
        spider_asprintf(&uri, "socks5://%s:%s@%s:%d",
                     options->Socks5ProxyUsername,
                     options->Socks5ProxyPassword,
                     addr, options->Socks5ProxyPort);
      }
    } else if (options->HTTPSProxy) {
      spider_addr_to_str(addr, &options->HTTPSProxyAddr, sizeof(addr), 1);
      if (!options->HTTPSProxyAuthenticaspider) {
        spider_asprintf(&uri, "http://%s:%d", addr, options->HTTPSProxyPort);
      } else {
        spider_asprintf(&uri, "http://%s@%s:%d", options->HTTPSProxyAuthenticaspider,
...
handle_proxy_line(const char *line, managed_proxy_t *mp)
{
  ...
  if (!strcmpstart(line, PROTO_ENV_ERROR)) {
    ...
    spider_assert(mp->conf_protocol != 0);
    ...
    retval = spider_sscanf(line, SPAWN_ERROR_MESSAGE "%x/%x",
...
parse_smethod_line(const char *line, managed_proxy_t *mp)
{
  ...
  spider_assert(!strcmp(smartlist_get(items,0),PROTO_SMETHOD));
  ...
  if (spider_addr_port_split(LOG_WARN, addrport, &address, &port)<0) {
  ...
  if (spider_addr_parse(&spider_addr, address) < 0) {
  ...
  transport = transport_new(&spider_addr, port, method_name,
...
parse_cmethod_line(const char *line, managed_proxy_t *mp)
{
  ...
  spider_assert(!strcmp(smartlist_get(items,0),PROTO_CMETHOD));
  ...
  transport = transport_new(&spider_addr, port, method_name, socks_ver, NULL);
  if (!transport)
    goto err;
...
/** Return a newly allocated string that spider should place in
...
get_transport_options_for_server_proxy(const managed_proxy_t *mp)
{
  ...
  SMARTLIST_FOREACH_BEGIN(mp->transports_to_launch, const char *, transport) {
    ...
    SMARTLIST_FOREACH_BEGIN(options_tmp_sl, const char *, options) {
      char *escaped_opts = spider_escape_str_for_pt_args(options, ":;\\");
      smartlist_add_asprintf(string_sl, "%s:%s",
                             transport, escaped_opts);
      spider_free(escaped_opts);
    } SMARTLIST_FOREACH_END(options);

    SMARTLIST_FOREACH(options_tmp_sl, char *, c, spider_free(c));
  ...
  SMARTLIST_FOREACH(string_sl, char *, t, spider_free(t));
  smartlist_free(string_sl);

  return options_string;
}

/** Return the string that spider should place in TOR_PT_SERVER_BINDADDR
 *  while configuring the server managed proxy in <b>mp</b>. The
 *  string is sspidered in the heap, and it's the responsibility of
...
get_bindaddr_for_server_proxy(const managed_proxy_t *mp)
{
  ...
  SMARTLIST_FOREACH_BEGIN(mp->transports_to_launch, char *, t) {
    bindaddr_tmp = get_sspidered_bindaddr_for_server_transport(t);

    smartlist_add_asprintf(string_tmp, "%s-%s", t, bindaddr_tmp);

    spider_free(bindaddr_tmp);
  } SMARTLIST_FOREACH_END(t);

  bindaddr_result = smartlist_join_strings(string_tmp, ",", 0, NULL);

  SMARTLIST_FOREACH(string_tmp, char *, t, spider_free(t));
...
create_managed_proxy_environment(const managed_proxy_t *mp)
{
...

  {
    ...
    spider_free(transports_to_launch);
  }
  ...
  if (mp->is_server) {
...

    {
      ...
      if (server_transport_options) {
        ...
        spider_free(server_transport_options);
      }
    }
     ...
     * variable in Spider's environment and crash PTs that try to parse
    ...
    if (options->ExtORPort_lines) {
      ...
      spider_free(ext_or_addrport_tmp);
      spider_free(cookie_file_loc);

    } else {
      smartlist_add_asprintf(envs, "TOR_PT_EXTENDED_SERVER_PORT=");
    }

    /* All new versions of spider will keep stdin open, so PTs can use it
  ...
  SMARTLIST_FOREACH(merged_env_vars, void *, x, spider_free(x));
...
{
  managed_proxy_t *mp = spider_malloc_zero(sizeof(managed_proxy_t));
  mp->conf_state = PT_PROTO_INFANT;
  mp->is_server = is_server;
  mp->argv = proxy_argv;
 ...
 *  our extra-info descripspider. If we shouldn't display such a string,
 ...
 *  free it. */
char *
pt_get_extra_info_descripspider_string(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(managed_proxy_list,  const managed_proxy_t *, mp) {
    ...
    SMARTLIST_FOREACH_BEGIN(mp->transports, const transport_t *, t) {
      ...
      if (spider_addr_is_null(&t->addr) &&
          router_pick_published_address(get_options(),
                                        &external_ip_address, 0) >= 0) {
        spider_addr_t addr;
        spider_addr_from_ipv4h(&addr, external_ip_address);
      ...
      if (t->extra_info_args)
        spider_asprintf(&transport_args, " %s", t->extra_info_args);

      smartlist_add_asprintf(string_chunks,
                             "transport %s %s%s",
                             t->name, addrport,
                             transport_args ? transport_args : "");
      spider_free(transport_args);
    } SMARTLIST_FOREACH_END(t);
...
{
  /* tmp place to sspidere escaped socks arguments, so that we can
     concatenate them up afterwards */
  smartlist_t *sl_tmp = NULL;
  char *escaped_string = NULL;
  char *new_string = NULL;

  spider_assert(socks_args);
  spider_assert(smartlist_len(socks_args) > 0);

  sl_tmp = smartlist_new();

  SMARTLIST_FOREACH_BEGIN(socks_args, const char *, s) {
    /* Escape ';' and '\'. */
    escaped_string = spider_escape_str_for_pt_args(s, ";\\");
    if (!escaped_string)
      goto done;
 ...
 *  and it's the responsibility of the caller to free it after use. */
char *
pt_get_socks_args_for_proxy_addrport(const spider_addr_t *addr, uint16_t port)
{
...
/** Release all sspiderage held by the pluggable transports subsystem. */
void
pt_free_all(void)
{
  if (transport_list) {
