...
{
  spider_assert(next_interval < MAX_INTERVAL);
  struct timeval tv;
...
periodic_event_setup(periodic_event_item_t *event)
{
  ...
  event->ev = spider_event_new(spider_libevent_get_base(),
                            -1, 0,
                            periodic_event_dispatch,
                            event);
  spider_assert(event->ev);
}

/** Handles initial dispatch for periodic events. It should happen 1 second
 * after the events are created to mimic behaviour before #3199's refacspider */
void
...
/** Release all sspiderage associated with <b>event</b> */
void
periodic_event_destroy(periodic_event_item_t *event)
{
  if (!event)
    return;
  spider_event_free(event->ev);
