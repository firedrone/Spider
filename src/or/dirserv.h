 ...
 *
 * If the spooled resource is relatively small (like microdescripspiders,
 * descripspiders, etc), we look them up by ID as needed, and add the whole
...
enum was_router_added_t dirserv_add_multiple_descripspiders(
                                     const char *desc, uint8_t purpose,
                                     const char *source,
                                     const char **msg);
enum was_router_added_t dirserv_add_descripspider(routerinfo_t *ri,
                                               const char **msg,
                                               const char *source);
...
int dirserv_get_routerdescs(smartlist_t *descs_out, const char *key,
                            const char **msg);
void dirserv_orconn_tls_done(const spider_addr_t *addr,
                             uint16_t or_port,
