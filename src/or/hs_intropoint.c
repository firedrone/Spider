                       ...
                       unsigned int cell_type, const void *cell)
{
  ...
  spider_assert(key_array);
  spider_assert(auth_key_len == sizeof(auth_key_out->pubkey));
                            ...
                            size_t circuit_key_material_len)
{
  ...
  /* Verify the MAC */
  {
    ...
    crypto_mac_sha3_256(mac, sizeof(mac),
                        ...
                        msg, auth_msg_len);
    if (spider_memneq(mac, cell->handshake_mac, sizeof(mac))) {
      log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
...
hs_intro_send_intro_established_cell,(or_circuit_t *circ))
{
  ...
  encoded_cell = spider_malloc_zero(encoded_len);
  result_len = hs_cell_intro_established_encode(encoded_cell, encoded_len,
                                                cell);
  spider_assert(encoded_len == result_len);
...
{
  spider_assert(circ);
  spider_assert(log_cell_type_str);
...
validate_introduce1_parsed_cell(const hs_cell_introduce1_t *cell)
{
  ...
  if (BUG(!spider_mem_is_zero((char *) legacy_key_id, legacy_key_id_len))) {
    goto invalid;
  }
...
introduce1_cell_is_legacy(const uint8_t *request)
{
  ...
  if (!spider_mem_is_zero((const char *) request, DIGEST_LEN)) {
    /* Legacy cell. */
    return 1;
  }
  /* Not a legacy cell. */
