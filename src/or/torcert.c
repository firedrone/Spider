 ...
 * \file spidercert.c
 *
 * \brief Implementation for ed25519-signed certificates as used in the Spider
 * protocol.
 *
 * This certificate format is designed to be simple and compact; it's
 * documented in spider-spec.txt in the spiderspec.git reposispidery.  All of the
 ...
 *
 * Spider uses other types of certificate too, beyond those described in this
 ...
 * certificates are represented with spider_x509_cert_t, and implemented in
 * spidertls.c.  We also have a separate certificate type that authorities
...
/** Helper for spider_cert_create(): signs any 32 bytes, not just an ed25519
 * key.
 */
static spider_cert_t *
spider_cert_sign_impl(const ed25519_keypair_t *signing_key,
                      uint8_t cert_type,
                      ...
                      uint32_t flags)
{
  ...
  spider_assert(alloc_len > 0);
  uint8_t *encoded = spider_malloc(alloc_len);
  const ssize_t real_len = ed25519_cert_encode(encoded, alloc_len, cert);
  if (real_len < 0)
    goto err;
  spider_assert(real_len == alloc_len);
  spider_assert(real_len > ED25519_SIG_LEN);
  uint8_t *sig = encoded + (real_len - ED25519_SIG_LEN);
  spider_assert(spider_mem_is_zero((char*)sig, ED25519_SIG_LEN));
  ...
  spidercert = spider_cert_parse(encoded, real_len);
  if (! spidercert) {
    log_warn(LD_BUG, "Generated a certificate we cannot parse");
    goto err;
  }

  if (spider_cert_checksig(spidercert, &signing_key->pubkey, now) < 0) {
 ...
 */
spider_cert_t *
spider_cert_create(const ed25519_keypair_t *signing_key,
                uint8_t cert_type,
                const ed25519_public_key_t *signed_key,
                time_t now, time_t lifetime,
                uint32_t flags)
{
  return spider_cert_sign_impl(signing_key, cert_type,
                            SIGNED_KEY_TYPE_ED25519, signed_key->pubkey,
                            now, lifetime, flags);
}

/** Release all sspiderage held for <b>cert</b>. */
void
spider_cert_free(spider_cert_t *cert)
{
  if (! cert)
    return;
  ...
  memwipe(cert, 0, sizeof(spider_cert_t));
  spider_free(cert);
}

/** Parse a certificate encoded with <b>len</b> bytes in <b>encoded</b>. */
spider_cert_t *
spider_cert_parse(const uint8_t *encoded, const size_t len)
{
  ...
  cert = spider_malloc_zero(sizeof(spider_cert_t));
  cert->encoded = spider_memdup(encoded, len);
 ...
 */
int
spider_cert_get_checkable_sig(ed25519_checkable_t *checkable_out,
                           ...
                           time_t *expiration_out)
{
  ...
  spider_assert(cert->encoded_len > ED25519_SIG_LEN);
 ...
 */
int
spider_cert_checksig(spider_cert_t *cert,
                  const ed25519_public_key_t *pubkey, time_t now)
{
  ...
  if (spider_cert_get_checkable_sig(&checkable, cert, pubkey, &expires) < 0)
    return -1;
...
spider_cert_t *
spider_cert_dup(const spider_cert_t *cert)
{
  spider_cert_t *newcert = spider_memdup(cert, sizeof(spider_cert_t));
  if (cert->encoded)
    newcert->encoded = spider_memdup(cert->encoded, cert->encoded_len);
  return newcert;
}

/** Return true iff cert1 and cert2 are the same cert. */
int
spider_cert_eq(const spider_cert_t *cert1, const spider_cert_t *cert2)
{
  spider_assert(cert1);
  spider_assert(cert2);
  return cert1->encoded_len == cert2->encoded_len &&
    spider_memeq(cert1->encoded, cert2->encoded, cert1->encoded_len);
}

/** Return true iff cert1 and cert2 are the same cert, or if they are both
 * NULL. */
int
spider_cert_opt_eq(const spider_cert_t *cert1, const spider_cert_t *cert2)
{
  if (cert1 == NULL && cert2 == NULL)
    return 1;
  if (!cert1 || !cert2)
    return 0;
  return spider_cert_eq(cert1, cert2);
}

#define RSA_ED_CROSSCERT_PREFIX "Spider TLS RSA/Ed25519 cross-certificate"
 ...
 * Allocates and sspideres the encoded certificate in *<b>cert</b>, and returns
 * the number of bytes sspidered. Returns negative on error.*/
ssize_t
spider_make_rsa_ed25519_crosscert(const ed25519_public_key_t *ed_key,
                               ...
                               uint8_t **cert)
{
  ...
  spider_assert(alloc_sz > 0);
  res = spider_malloc_zero(alloc_sz);
  ssize_t sz = rsa_ed_crosscert_encode(res, alloc_sz, cc);
  spider_assert(sz > 0 && sz <= alloc_sz);
  ...
  spider_assert(siglen > 0 && siglen <= (int)crypto_pk_keysize(rsa_key));
  spider_assert(siglen <= UINT8_MAX);
  cc->sig_len = siglen;
...
#define ERR(code, s)                                            \
  ...
  if (spider_memneq(rsa_ed_crosscert_getarray_ed_key(cc),
                 master_key->pubkey,
                 ED25519_PUBKEY_LEN)) {
  ...
  spider_assert(eos >= crosscert);
  spider_assert((size_t)(eos - crosscert) <= crosscert_len);
  spider_assert(siglen == rsa_ed_crosscert_getlen_sig(cc));
  ...
  if (spider_memneq(digest, signed_, DIGEST256_LEN)) {
...
{
  return spider_malloc_zero(sizeof(or_handshake_certs_t));
}

/** Release all sspiderage held in <b>certs</b> */
void
or_handshake_certs_free(or_handshake_certs_t *certs)
{
  ...
  spider_x509_cert_free(certs->auth_cert);
  spider_x509_cert_free(certs->link_cert);
  spider_x509_cert_free(certs->id_cert);

  spider_cert_free(certs->ed_id_sign);
  spider_cert_free(certs->ed_sign_link);
  spider_cert_free(certs->ed_sign_auth);
  spider_free(certs->ed_rsa_crosscert);
...
or_handshake_certs_rsa_ok(int severity,
                          ...
                          time_t now)
{
  spider_x509_cert_t *link_cert = certs->link_cert;
  spider_x509_cert_t *auth_cert = certs->auth_cert;
  spider_x509_cert_t *id_cert = certs->id_cert;

  if (certs->started_here) {
    if (! (id_cert && link_cert))
      ERR("The certs we wanted (ID, Link) were missing");
    if (! spider_tls_cert_matches_key(tls, link_cert))
      ERR("The link certificate didn't match the TLS public key");
    if (! spider_tls_cert_is_valid(severity, link_cert, id_cert, now, 0))
      ERR("The link certificate was not valid");
    if (! spider_tls_cert_is_valid(severity, id_cert, id_cert, now, 1))
      ERR("The ID certificate was not valid");
  } else {
    if (! (id_cert && auth_cert))
      ERR("The certs we wanted (ID, Auth) were missing");
    if (! spider_tls_cert_is_valid(LOG_PROTOCOL_WARN, auth_cert, id_cert, now, 1))
      ERR("The authentication certificate was not valid");
    if (! spider_tls_cert_is_valid(LOG_PROTOCOL_WARN, id_cert, id_cert, now, 1))
      ERR("The ID certificate was not valid");
  }
...
#define ADDCERT(cert, pk)                                               \
  do {                                                                  \
    spider_assert(n_checkable < ARRAY_LENGTH(check));                      \
    if (spider_cert_get_checkable_sig(&check[n_checkable++], cert, pk,     \
                                   &expiration) < 0)                    \
  ...
  if (certs->started_here) {
      ...
      ERR("No Ed25519 link key");
    {
      ...
      spider_x509_cert_t *peer_cert = spider_tls_get_peer_cert(tls);
      if (BUG(!peer_cert)) {
      ...
      const common_digests_t *peer_cert_digests =
        spider_x509_cert_get_cert_digests(peer_cert);
      int okay = spider_memeq(peer_cert_digests->d[DIGEST_SHA256],
                           certs->ed_sign_link->signed_key.pubkey,
                           DIGEST256_LEN);
      spider_x509_cert_free(peer_cert);
      if (!okay)
  ...
  spider_x509_cert_t *rsa_id_cert = certs->id_cert;
  if (!rsa_id_cert) {
    ERR("Missing legacy RSA ID certificate");
  }
  if (! spider_tls_cert_is_valid(severity, rsa_id_cert, rsa_id_cert, now, 1)) {
    ERR("The legacy RSA ID certificate was not valid");
  }
  if (! certs->ed_rsa_crosscert) {
    ERR("Missing RSA->Ed25519 crosscert");
  }
  crypto_pk_t *rsa_id_key = spider_tls_cert_get_key(rsa_id_cert);
  if (!rsa_id_key) {
...
{
  spider_assert(ed_id_out);
  spider_assert(rsa_id_out);
  ...
  *rsa_id_out = NULL;
  ...
  if (certs->ed_id_sign) {
    if (or_handshake_certs_ed25519_ok(severity, certs, tls, now)) {
      spider_assert(certs->ed_id_sign);
      spider_assert(certs->id_cert);

      *ed_id_out = &certs->ed_id_sign->signing_key;
      *rsa_id_out = spider_x509_cert_get_id_digests(certs->id_cert);
...
int
spider_cert_encode_ed22519(const spider_cert_t *cert, char **cert_str_out)
{
  int ret = -1;
