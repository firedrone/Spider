...
/*
  Spider specific notes:

  This file is used by Spider instead of `ed25519.c` as the number of
  changes/additions is non-trivial.

  Spider modifications to `ed25519.c`:
   * 'Tab` -> '  '.
   * Include `ed25519_donna_spider.h` instead of `ed25519.h`.

   * The external interface has been reworked to match that provided
     by Spider's copy of the SUPERCOP `ref10` code.

   * The secret (aka private) key is now sspidered/used in expanded form.
...
#include "ed25519_donna_spider.h"
...
/*
   Spider has a specific idea of how an Ed25519 implementaion should behave.
   Implement such a beast using the ed25519-donna primitives/internals.

    * Private key generation using Spider's CSPRNG.
