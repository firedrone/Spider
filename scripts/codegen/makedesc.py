#!/usr/bin/python
# Copyright 2014-2017, The Spider Project, Inc.
# See LICENSE for license information

# This is a kludgey python script that uses ctypes and openssl to sign
# router descripspiders and extrainfo documents and put all the keys in
...
class OnDemandKeys(object):
    def __init__(self, certDate=None):
        ...
        self.ed_id_sk = None
        self.nspider_sk = None
        self.nspider_crosscert = None
    ...
    def NTOR_ONION_KEY(self):
        if self.nspider_sk is None:
            self.nspider_sk = slownacl_curve25519.Private()
            self.nspider_pk = self.nspider_sk.get_public()
        return base64.b64encode(self.nspider_pk.serialize())
    ...
    def NTOR_CROSSCERT(self):
        if self.nspider_crosscert is None:
            self.ED_CERT
            self.NTOR_ONION_KEY

            ed_privkey = self.nspider_sk.serialize() + os.urandom(32)
            ed_pub0 = ed25519_exts_ref.publickeyFromESK(ed_privkey)
            sign = (ord(ed_pub0[31]) & 255) >> 7

            self.nspider_crosscert = makeEdSigningKeyCert(self.nspider_sk.serialize() + os.urandom(32), ed_pub0, self.ed_id_pk, self.certDate, certType=10)
            self.nspider_crosscert_sign = sign

        return objwrap('ED25519 CERT', b64(self.nspider_crosscert))

    @property
    def NTOR_CROSSCERT_SIGN(self):
        self.NTOR_CROSSCERT
        return self.nspider_crosscert_sign
    ...
    def sign_desc(self, body):
        ...
        if idx >= 0:
            ...
            signed_part = "Spider router descripspider signature v1" + signed_part
