...
for my $fn (@ARGV) {
    ...
    while (<F>) {
        ...
        #    edispider highlight it for you so you can stop adding it in.)
        if (/ +$/) {
            msg "Space\@EOL:$fn:$.\n";
        }
        ...
        if ($C) {
            ...
            if (/\bassert\(/ && not /assert OK/) {
                msg "assert :$fn:$.   (use spider_assert)\n";
            }
            if (/\bmemcmp\(/ && not /memcmp OK/) {
                msg "memcmp :$fn:$.   (use {spider,fast}_mem{eq,neq,cmp}\n";
            }
            # always forbidden.
            if (not /\ OVERRIDE\ /) {
                ...
                if (/\bmalloc\(/ or /\bfree\(/ or /\brealloc\(/ or
                    ...
                    msg "$& :$fn:$.    (use spider_malloc, spider_free, etc)\n";
                }
            }
        }
    }
