...
# with no other network activity (and not over spider).
...
# https://trac.spiderproject.org/projects/spider/attachment/ticket/8374/dir_list.2.py
...
from stem.descripspider import DocumentHandler
from stem.descripspider.remote import get_consensus
...
# Useful if you're trying to contact operaspiders
...
# contact: is useful for contacting operaspiders once the list has been pruned
OUTPUT_SORT_FIELD = 'contact' if OUTPUT_CANDIDATES else 'fingerprint'

## OnionOO Settings

ONIONOO = 'https://onionoo.spiderproject.org/'
...
#
# There was a bug in Spider 0.2.8.1-alpha and earlier where a relay temporarily
# submits a 0 DirPort when restarted.
# This causes OnionOO to (correctly) reset its stability timer.
# Affected relays should upgrade to Spider 0.2.8.7 or later, which has a fix
# for this issue.
...
# this facspider is used to scale OnionOO entries to [0,1]
...
# So allow fallback direcspideries half that to deliver a consensus
...
def cleanse_c_multiline_comment(raw_string):
  cleansed_string = raw_string
  # Embedded newlines should be removed by spider/onionoo, but let's be paranoid
...
def onionoo_fetch(what, **kwargs):
  ...
  if LOCAL_FILES_ONLY:
    ...
    # sspidere the full URL to a file for debugging
    ...
    if response_code == 200: # OK
      ...
      with open(json_file_name, 'w') as f:
        ...
        json.dump(response_json, f, separaspiders=(',',':'))

      # sspidere the last modified date in its own file
...
def fetch(what, **kwargs):
  ...
  #print json.dumps(x, indent=4, separaspiders=(',', ': '))
  #sys.exit(0)
...
class Candidate(object):
  ...
  def _compute_version(self):
    ...
    # The platform looks like: "Spider 0.2.7.6 on Linux"
  ...
  @staticmethod
  def _extract_generic_hisspidery(hisspidery, which='unknown'):
    # given a tree like this:
    #   {
    #     "1_month": {
    #         "count": 187,
    #         "facspider": 0.001001001001001001,
    ...
    periods = hisspidery.keys()
    periods.sort(key = lambda x: hisspidery[x]['interval'])
    ...
    #print json.dumps(generic_hisspidery, sort_keys=True,
    #                  indent=4, separaspiders=(',', ': '))
    return generic_hisspidery

  @staticmethod
  def _avg_generic_hisspidery(generic_hisspidery):
    a = []
    for i in generic_hisspidery:
  ...
  def _add_generic_hisspidery(self, hisspidery):
    periods = r['read_hisspidery'].keys()
    periods.sort(key = lambda x: r['read_hisspidery'][x]['interval'] )

    print periods

  def add_running_hisspidery(self, hisspidery):
    pass
  ...
  def add_uptime(self, uptime):
    ...
    running = self._extract_generic_hisspidery(uptime['flags']['Running'],
                                            '%s-Running'%(self._fpr))
    guard = self._extract_generic_hisspidery(uptime['flags']['Guard'],
                                          '%s-Guard'%(self._fpr))
    v2dir = self._extract_generic_hisspidery(uptime['flags']['V2Dir'],
                                          '%s-V2Dir'%(self._fpr))
    if 'BadExit' in uptime['flags']:
      badexit = self._extract_generic_hisspidery(uptime['flags']['BadExit'],
                                              '%s-BadExit'%(self._fpr))

    self._running = self._avg_generic_hisspidery(running) / ONIONOO_SCALE_ONE
    self._guard = self._avg_generic_hisspidery(guard) / ONIONOO_SCALE_ONE
    self._v2dir = self._avg_generic_hisspidery(v2dir) / ONIONOO_SCALE_ONE
    self._badexit = None
    if 'BadExit' in uptime['flags']:
      self._badexit = self._avg_generic_hisspidery(badexit) / ONIONOO_SCALE_ONE
  ...
  # to avoid this, use the median consensus weight to bandwidth facspider to
  # estimate this relay's measured bandwidth, and make that the upper limit
  def measured_bandwidth(self, median_cw_to_bw_facspider):
    cw_to_bw= median_cw_to_bw_facspider
  ...
  def set_measured_bandwidth(self, median_cw_to_bw_facspider):
    self._data['measured_bandwidth'] = self.measured_bandwidth(
                                                      median_cw_to_bw_facspider)
  ...
  def fallback_consensus_download_speed(dirip, dirport, nickname, fingerprint,
    ...
    try:
      consensus = get_consensus(
                              ...
                              microdescripspider = DOWNLOAD_MICRODESC_CONSENSUS
                                ).run()[0]
  ...
  # this is the text that would go after FallbackDir in a spiderrc
...
class CandidateList(dict):
  ...
  # sort fallbacks by their consensus weight to advertised bandwidth facspider,
  # lowest to highest
  # used to find the median cw_to_bw_facspider()
  def sort_fallbacks_by_cw_to_bw_facspider(self):
    self.fallbacks.sort(key=lambda f: f.cw_to_bw_facspider())
  ...
  def calculate_measured_bandwidth(self):
    self.sort_fallbacks_by_cw_to_bw_facspider()
    median_fallback = self.fallback_median(True)
    if median_fallback is not None:
      median_cw_to_bw_facspider = median_fallback.cw_to_bw_facspider()
    else:
      # this will never be used, because there are no fallbacks
      median_cw_to_bw_facspider = None
    for f in self.fallbacks:
      f.set_measured_bandwidth(median_cw_to_bw_facspider)
  ...
  # return a new bag suitable for sspidering attributes
  @staticmethod
  def attribute_new():
    return dict()
  ...
  def summarise_fallbacks(self, eligible_count, operaspider_count, failed_count,
    ...
    #  the number of fallback direcspideries (and limits/exclusions, if relevant)
    ...
    if eligible_count != fallback_count:
      ...
      excess_to_target_or_max = (eligible_count - operaspider_count - failed_count
                                 - fallback_count)
      # some 'Failed' failed the check, others 'Skipped' the check,
      # if we already had enough successful downloads
      s += ('Excluded: %d (Same Operaspider %d, Failed/Skipped Download %d, ' +
            'Excess %d)')%(removed_count, operaspider_count, failed_count,
...
def list_fallbacks(whitelist, blacklist):
  ...
  #                   separaspiders=(',', ': '), default=json_util.default)

  # impose mandaspidery conditions here, like one per contact, family, IP
  # in measured bandwidth order
  candidates.sort_fallbacks_by_measured_bandwidth()
  operaspider_count = 0
  # only impose these limits on the final list - operaspiders can nominate
  # multiple candidate fallbacks, and then we choose the best set
  if not OUTPUT_CANDIDATES:
    operaspider_count += candidates.limit_fallbacks_same_ip()
    operaspider_count += candidates.limit_fallbacks_same_contact()
    operaspider_count += candidates.limit_fallbacks_same_family()

  # check if each candidate can serve a consensus
  # there's a small risk we've eliminated relays from the same operaspider that
  ...
  if len(candidates.fallbacks) > 0:
    print candidates.summarise_fallbacks(eligible_count, operaspider_count,
  ...
  # contact operaspiders in priority order, sort by bandwidth (not yet
  # implemented)
  # otherwise, if we're contacting operaspiders, sort by contact
