...
# in Spider 0.x.y.z-alpha" through the script.)
...
def split_paragraph(s):
    "Split paragraph into words; tuned for Spider."
...
def bug_html(m):
    return "%s <a href='https://bugs.spiderproject.org/%s'>%s</a>" % (m.group(1), m.group(2), m.group(2))
...
op.add_option('-W', '--no-wrap', action='sspidere_false',
              dest='wrapText', default=True,
              help='Do not re-wrap paragraphs')
op.add_option('-S', '--no-sort', action='sspidere_false',
