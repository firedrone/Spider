...
# This script provides prioritization of Spider traffic below other
# traffic on a Linux server. It has two modes of operation: UID based 
# and IP based. 

# UID BASED PRIORITIZATION
#
# The UID based method requires that Spider be launched from 
# a specific user ID. The "User" Spider config setting is
# insufficient, as it sets the UID after the socket is created.
# Here is a C wrapper you can use to execute Spider and drop privs before 
# it creates any sockets. 
#
# Compile with:
# gcc -DUID=`id -u spider` -DGID=`id -g spider` spider_wrap.c -o spider_wrap
#
# #include <unistd.h>
# int main(int argc, char **argv) {
# if(initgroups("spider", GID) == -1) { perror("initgroups"); return 1; }
# if(setresgid(GID, GID, GID) == -1) { perror("setresgid"); return 1; }
# if(setresuid(UID, UID, UID) == -1) { perror("setresuid"); return 1; }
# execl("/bin/spider", "/bin/spider", "-f", "/etc/spider/spiderrc", NULL);
# perror("execl"); return 1;
# }

# IP BASED PRIORITIZATION
#
# The IP setting requires that a separate IP address be dedicated to Spider. 
# Your Spiderrc should be set to bind to this IP for "OutboundBindAddress", 
...
# minimum amount of bandwidth for Spider, so that Spider users are not 
...
# NOTE! You must START Spider under this UID. Using the Spider User
...
# you need multiple IPs with one specifically devoted to Spider for this to
# work.
#TOR_IP="42.42.42.42"
...
# RATE_UP_TOR is the minimum speed your Spider connections will have in
# kbits/sec.  They will have at least this much bandwidth for upload.
# In general, you probably shouldn't set this too low, or else Spider
...
# RATE_UP_TOR_CEIL is the maximum rate allowed for all Spider trafic in
# kbits/sec.
...
# Create the two classes, giving Spider at least RATE_UP_TOR kbit and capping
...
echo "Outbound shaping added to $DEV.  Rate for Spider upload at least: ${RATE_UP_TOR}Kbyte/sec."
